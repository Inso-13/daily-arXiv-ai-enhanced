...
Motivation: 解决现有方法在语义3D运动迁移方面的不足，特别是跨类别对象间的运动传输问题，以及确保运动保真度和结构一致性的挑战。
...
Conclusion: 该方法成功实现了跨类别的语义3D运动迁移，为相关领域提供了有效的解决方案和基准数据集。
...
TL;DR: 本文提出了一种基于体积距离函数的CAD边界表示(B-Rep)新几何表示方法BR-DF，将表面网格几何编码为SDF，将顶点、边、面及其拓扑信息编码为每面UDF，并通过改进的Marching Cubes算法实现100%成功率的B-Rep模型转换。


<details>
  <summary>Details</summary>
Motivation: 解决传统CAD B-Rep生成方法成功率不高的问题，通过体积距离函数实现更可靠和稳定的CAD模型生成。

Method: 提出BR-DF表示方法，使用SDF编码表面几何，每面UDF编码拓扑信息，采用多分支潜在扩散模型和3D U-Net联合生成SDF和UDF。

Result: 在CAD生成性能上与SOTA方法相当，同时实现了前所未有的100%成功率生成B-Rep模型。

Conclusion: BR-DF为CAD B-Rep提供了一种新颖可靠的表示方法，解决了传统方法转换失败的问题，具有重要的实际应用价值。
...
TL;DR: 提出InstructMix2Mix框架，将2D扩散模型编辑能力蒸馏到预训练多视图扩散模型中，解决稀疏输入视图下的多视图图像编辑问题，显著提升跨视图一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于单场景神经场或时序注意力机制的方法在多视图图像编辑中容易产生伪影和不一致编辑，需要改进跨视图一致性。

Method: 使用多视图扩散学生替代SDS中的传统神经场整合器，采用增量学生更新、专用教师噪声调度器和注意力修改来增强跨视图一致性。

Result: 实验表明I-Mix2Mix显著提高了多视图一致性，同时保持高质量的单帧编辑效果。

Conclusion: 该方法成功将2D扩散模型编辑能力蒸馏到多视图扩散模型中，有效解决了多视图编辑的跨视图一致性问题。
...
TL;DR: SkinR1是一个新颖的皮肤病视觉语言模型，通过结合基于教科书的推理和强化学习，解决了数据异构性、缺乏诊断依据和泛化能力有限的问题，在多个皮肤病数据集上实现了优越的诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在临床推理中的三个主要限制：数据异构性、缺乏可靠的诊断推理依据，以及在小规模密集标注数据集上训练的模型难以泛化到大规模稀疏标注数据集的问题。

Method: 1) 设计基于教科书的推理生成器，合成高保真、层次感知和鉴别诊断信息轨迹；2) 利用构建的轨迹进行监督微调，赋予模型基础推理能力；3) 开发结合疾病层次结构的新型强化学习范式，将推理模式转移到大规模稀疏数据。

Result: 在多个皮肤病数据集上的广泛实验表明，SkinR1实现了优越的诊断准确性。消融研究证明了监督微调所建立推理基础的重要性。

Conclusion: SkinR1通过统一的端到端框架成功解决了皮肤病视觉语言模型的关键挑战，结合基于教科书的推理和强化学习，在诊断准确性和泛化能力方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感专用CLIP变体仍受限于全局对齐能力，无法有效捕捉细粒度细节。主要问题包括：遥感图像-文本数据集生成全局标注时未充分利用对象级监督；直接应用通用区域-文本对齐方法到遥感数据会导致性能下降。
...
Conclusion: FarSLIP通过改进的细粒度对齐策略有效解决了CLIP在遥感领域的空间感知限制，为遥感视觉-语言理解提供了新的解决方案。
...
TL;DR: nnMIL是一个简单但广泛适用的多实例学习框架，通过随机采样和轻量级聚合器，将病理基础模型连接到稳健的幻灯片级临床推理，在40,000张WSI上表现优于现有方法。
...
Method: 提出X-WIN模型，通过预测3D空间中的各种变换下的CXR来内化3D解剖结构知识；引入亲和力引导的对比对齐损失；通过掩码图像建模和域分类器整合真实CXR。

Result: X-WIN在多样下游任务上优于现有基础模型，在线性探测和少样本微调中表现优异，并能够渲染2D投影以重建3D CT体积。

Conclusion: X-WIN通过从CT中提取3D知识有效提升了CXR的表示学习和诊断能力，展示了在医学影像分析中的强大潜力。
...
Method: 基于每帧深度和内容显著性，将帧分解为少量几何一致的层，配备软alpha带和边缘深度缓存，通过深度加权变形和前向后alpha合成实现视差校正的新视图合成。
...
TL;DR: 该论文提出了首个长周期工作流基准数据集，包含580个多模态人类活动序列，支持三个评估任务，并提出了一个轻量级的无训练基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注短周期活动，而具有低对比度模式的长周期工作流研究不足，需要建立基准和方法来填补这一空白。

Method: 提出了一个轻量级、无需训练的方法来建模多样的周期性工作流模式，该方法无需标注和重新训练。

Result: 实验表明：基准对现有方法构成挑战；基线方法在所有评估任务中显著优于竞争方法；在真实应用中具有与传统监督方法相当的部署优势。

Conclusion: 该研究填补了长周期工作流研究的空白，提出的基准和基线方法为相关领域提供了重要工具和参考。
...
TL;DR: 提出了一种低成本的多视角视频流同步方法，使用LED时钟编码时间信息，实现毫秒级时间对齐，支持可见光和红外传感器。


<details>
  <summary>Details</summary>
Motivation: 解决异构相机系统（专业与消费级设备、可见光与红外传感器、有无音频系统）在多视角应用中缺乏硬件同步能力的问题。

Method: 使用定制LED时钟，通过红色和红外LED编码时间信息，从记录帧中视觉解码曝光窗口（开始和结束时间）实现同步。

Result: 与硬件同步相比，残差误差为1.34ms RMSE；在大型手术记录中验证了25+异构相机的同步效果；优于基于光、音频和时间码的同步方法。

Conclusion: 该方法简化了同步流程，扩展了在无约束环境（工业和临床应用）中基于视觉的先进传感技术的应用。
...
Method: 采用闭环配置系统，结合基于无透镜散斑传感的定制CNN和基于USB摄像头与VGG16迁移学习的材料分类方法，以及单独的烟雾检测深度学习模型。

Result: 实验证明抽气泵能耗显著降低20%-50%，系统能在空闲时自动停止，工作时动态调节功率。

Conclusion: 该深度学习方法有效实现了激光切割过程的能源节约，为制造业可持续发展做出了重要贡献。
...
Method: 提出纯特征知识蒸馏框架，引入基于潜在表示几何特性的知识质量度量来识别教师模型中最有效的层，仅使用特征损失训练学生模型。

Result: 在三个图像分类数据集和四种不同的师生模型对上（包括CNN和ViT），该方法实现了最先进的性能，top-1准确率相比标准方法提升高达15%。

Conclusion: 仅使用特征损失的知识蒸馏框架是有效的，通过知识质量度量识别最优教师层可以显著提升蒸馏性能，代码已公开。
...
Method: 采用多阶段训练流程，包括数据收集、处理、过滤和聚类，结合自监督微调(SFT)和基于强化学习(RL)的后训练等质量增强技术，以及新颖的架构、训练和推理优化。

Result: Kandinsky 5.0在人类评估中展现出高生成速度和最先进的性能，能够适应广泛的生成应用。

Conclusion: 该报告与开源代码和训练检查点的发布将显著推进研究社区高质量生成模型的发展和可访问性。
...
Method: 开发了包含500个图像-HTML对的基准数据集，由金融专家标注超过700个数值和时间事实。建立了LLM-as-Judge评估流程，执行结构化事实提取和上下文验证。

Result: 最强的专有模型实现了最高的事实准确性，但在视觉复杂的数值和时间上下文中仍存在显著错误。

Conclusion: FinCriticalED为推进金融和其他精度关键领域的视觉事实精确性提供了严格基础，将评估从词汇重叠转向领域关键事实正确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的跨模态知识蒸馏方法忽略了模态特定知识获取和模态共同知识抗遗忘之间的相互干扰，导致协同遗忘问题。

Method: 提出了模态共同提示模块和模态特定提示模块来显式解耦和纯化不同模态的判别信息，并设计了跨模态知识对齐模块在双模态原型的基础上平衡地对齐新旧知识。
...
TL;DR: 提出基于结构化复值2D高斯基元的新全息图表示方法，替代逐像素信息存储，将参数搜索空间减少高达10:1，实现更低VRAM使用、更快优化和更高保真度重建。


<details>
  <summary>Details</summary>
Motivation: 现有全息图方法存在参数搜索空间大、计算效率低的问题，需要一种更高效的全息图表示方法来提升计算机生成全息系统的可扩展性。

Method: 开发基于结构化复值2D高斯基元的新表示方法，构建可微分光栅化器，并与GPU优化的自由空间光传播核集成，支持端到端训练。

Result: 实验显示该方法比现有方法VRAM使用降低2.5倍，优化速度提升50%，重建保真度更高，并能有效抑制噪声伪影。
...
TL;DR: 该研究探讨计算机视觉模型在训练过程中是否表现出与儿童发展轨迹一致的发展对齐现象，发现ResNet-50模型在几何和数字概念学习方面确实存在这种对齐。


<details>
  <summary>Details</summary>
Motivation: 基于先前研究发现计算机视觉模型在图像分类任务中能学习到与成人相似的几何和数字概念表示，本研究旨在验证这些模型是否在训练过程中表现出与儿童发展轨迹一致的发展对齐。

Method: 使用ResNet-50模型进行详细案例研究，分析其在几何（欧几里得几何、几何图形、度量性质、拓扑等）和数字概念（心理数字线表示）学习过程中的性能改进模式。

Result: 发现ResNet-50模型在某些几何概念类别（欧几里得几何、几何图形、度量性质、拓扑）上表现出发展对齐，但在其他类别（手性图形、几何变换、对称图形）上未发现对齐；在数字概念方面，模型随着经验积累发展出类似人类的心理数字线表示。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法将HOI检测和生成作为两个独立任务处理，阻碍了全面的交互理解发展。需要统一框架来促进这两个任务之间的知识共享。

Method: 引入对称的交互感知注意力模块和统一的半监督学习范式，在有限标注下实现图像与交互语义之间的有效双向映射。

Result: 在HOI检测和生成任务上都达到了最先进的性能：长尾HOI检测准确率提升4.9%，开放词汇生成任务的交互指标提升42.0%。

Conclusion: UniHOI通过统一建模HOI检测和生成，有效促进了知识共享和泛化能力，为全面交互理解提供了有效解决方案。
...
TL;DR: 提出DLRRF模型解决HSI和MSI融合中的图像间变异性问题，通过建模光谱退化算子变化和分解低秩残差分量来恢复空间细节，在PnP框架下使用PAO算法求解。


<details>
  <summary>Details</summary>
Motivation: HSI与MSI融合时存在光谱变异性和空间局部变化等图像间变异性，现有方法直接对图像进行变换会加剧融合模型的不适定性，影响融合性能。

Method: 将光谱变异性建模为光谱退化算子变化；将目标HSI分解为低秩和残差分量以恢复丢失的空间细节；利用光谱相关性进行降维；引入隐式正则化器利用空间先验信息；在PnP框架下使用PAO算法求解。

Result: 广泛的数值实验表明DLRRF在存在图像间变异性的HSI和MSI融合中取得了优越性能。

Conclusion: DLRRF模型能有效处理HSI和MSI融合中的图像间变异性问题，通过退化建模和低秩残差分解提高了融合质量。
...
TL;DR: CellGenNet是一个基于知识蒸馏的细胞分割框架，通过师生架构在有限监督下实现跨组织稳健细胞分割，使用软伪标签和混合损失函数提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 解决显微镜全玻片图像中由于染色、成像条件和组织形态变化导致的细胞核分割不准确问题，在有限监督下实现稳健的跨组织细胞分割。

Method: 采用师生架构，能力强的教师模型在稀疏标注上训练并生成软伪标签；学生模型使用结合真实标签和教师概率目标的联合目标函数，采用二元交叉熵和Tversky损失的混合损失函数，通过一致性正则化和分层dropout稳定特征表示。

Result: 在多种癌症组织全玻片图像上的实验表明，CellGenNet相比监督和半监督基线方法提高了分割精度和泛化能力。
...
Result: ProPL在多种指标上优于现有最先进方法，为通用超声图像分割建立了新基准。

Conclusion: 该框架在5个器官和8个分割任务的综合超声数据集上表现出色，证明了其在通用超声图像分割任务中的有效性。
...
TL;DR: 评估现有多模态大语言模型在垂直书写日文文本上的阅读能力，发现现有模型在垂直文本上表现较差，但通过合成数据集微调可以显著提升性能。
...
Result: 现有MLLMs在垂直书写日文文本上的表现比水平文本差，但使用合成数据集训练可以显著提升模型处理垂直文本的能力。

Conclusion: 垂直书写日文文本对MLLMs构成挑战，但通过适当的训练数据可以改善模型性能，为多语言文档理解提供重要支持。
...
TL;DR: 本文提出了VR-Bench基准测试，用于评估视频模型通过视频生成进行推理的能力，特别是在空间规划和多步推理任务中。研究发现SFT能有效激发视频模型的推理能力，视频模型在空间感知方面优于领先的视觉语言模型，并且通过多样化采样可提高推理可靠性。


<details>
  <summary>Details</summary>
Motivation: 类比语言模型从文本生成发展到文本推理的发展路径，探索视频模型是否能够通过视频生成进行推理。视频在空间布局和时间连续性方面具有优势，是空间推理的理想载体。

Method: 引入VR-Bench基准测试，包含7,920个程序生成的视频，涵盖五种迷宫类型和多样视觉风格。使用监督微调(SFT)激发视频模型的推理能力，并研究测试时缩放效应。
...
Conclusion: BokehFlow成功解决了无需深度输入的可控散景渲染挑战，提供高质量的视觉效果和灵活的语义控制能力。
...
TL;DR: MambaTrack3D是一个基于状态空间模型Mamba的新型3D单目标跟踪框架，专门针对高时间变化(HTV)的室外动态环境设计，通过帧间传播和分组特征增强模块解决了现有方法计算复杂度高、时间冗余和几何先验利用不足的问题。


<details>
  <summary>Details</summary>
Motivation: 解决动态室外环境中高时间变化带来的3D单目标跟踪挑战，现有基于内存的跟踪器存在二次计算复杂度、时间冗余和几何先验利用不足的问题。

Method: 提出基于Mamba的帧间传播模块(MIP)替代传统单帧特征提取，实现近线性复杂度；引入分组特征增强模块(GFEM)在通道级别分离前景和背景语义，减少内存库中的时间冗余。
...
Conclusion: MambaTrack3D实现了优越的精度-效率权衡，在专门的HTV和传统跟踪场景中均表现出鲁棒性能，具有强大的泛化能力。
...
TL;DR: 该论文提出了一个结构感知的流水线，从真实的手术培训师-学员对话中学习手术动作本体，并利用它来指导反馈生成。通过挖掘器械-动作-目标三元组、微调视频到IAT的模型，以及使用IAT表示引导GPT-4o生成临床基础的手术培训师风格反馈，显著提高了反馈生成的质量和临床相关性。


<details>
  <summary>Details</summary>
Motivation: 高质量的手术中反馈对提升学员表现和长期技能获取至关重要。自动化生成自然、类似培训师风格的反馈能够提供及时、可访问且一致的指导，但需要能够理解临床相关表示的模型。
...
Result: 在视频到IAT识别任务中，上下文注入和时间跟踪带来一致的AUC提升（器械：0.67到0.74；动作：0.60到0.63；组织：0.74到0.79）。在反馈文本生成任务中，仅使用视频的GPT-4o得分为2.17，而IAT条件化达到2.44（+12.4%），将可接受生成的比例从21%提高到42%。传统文本相似性指标也有改善：词错误率降低15-31%，ROUGE增加9-64%。

Conclusion: 在明确IAT结构中基础生成提高了保真度，并产生了临床医生可验证的理由，支持在手术培训中的可审计使用。
...
TL;DR: 本文提出TiCAL框架，通过典型性估计和一致性感知来解决多模态情感识别中的模态间情感冲突问题，在双曲空间中嵌入特征以捕捉细粒度情感差异，在CMU-MOSEI和MER2023数据集上取得显著性能提升。
...
Method: 提出TiCAL框架，受人类情感感知的阶段特性启发，通过伪单模态情感标签和典型性估计动态评估训练样本的一致性，在双曲空间中嵌入特征以捕捉情感类别的细粒度差异。

Result: 在CMU-MOSEI和MER2023等基准数据集上的广泛实验验证了TiCAL的有效性，特别是在模态不一致性高的样本上表现优异，相比最先进的DMD方法提升了约2.6%。

Conclusion: TiCAL框架通过一致性感知学习有效缓解了模态间情感冲突问题，提高了多模态情感识别的整体准确性。
...
Method: 通过全面研究不同dMLLM架构和任务下视觉令牌冗余的演变规律，分析视觉令牌剪枝对模型响应和效率的影响。
...
TL;DR: 本文提出高斯混合方法替代传统的alpha混合，解决3D高斯溅射在未见采样率下的模糊和阶梯伪影问题，保持实时渲染速度且无需额外内存开销。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在训练时未见采样率下合成视图时会出现明显的视觉差异，包括缩放时的侵蚀性模糊伪影和缩小时的扩张性阶梯伪影，这些源于alpha混合的基本限制。

Method: 提出高斯混合方法，将alpha和透射率视为空间变化分布而非标量，考虑像素区域内alpha值的空间分布来更新透射率，使附近背景溅射能贡献到最终渲染中。

Result: 高斯混合能有效捕捉训练时未见各种采样率下的细节，在未见和已见采样率下均持续优于现有新颖视图合成模型。

Conclusion: 高斯混合作为即插即用替代方案，可轻松集成到现有3DGS框架中，无需额外成本即可提升渲染质量。
...
TL;DR: 开发了一个事件触发系统来检测老年人的三种事件：看门狗、危险通知和照片链接，同时考虑了老年人的身体和心理状态。


<details>
  <summary>Details</summary>
Motivation: 考虑到老年人缺乏技术经验，设计直观的操作方式，通过社交媒体促进老年人与亲属之间的沟通，同时关注老年人的身心健康。

Method: 采用GMM背景建模检测访客和老年人的运动行为，使用SVM机器学习分析捕获的图像，在家庭场景中设置实验，5个家庭参与记录三类生活活动事件。

Result: 系统成功检测和记录了三种类型的事件，并通过直观的操作方式实现了老年人与亲属的社交媒体沟通。

Conclusion: 该事件触发系统能够有效监测老年人的生活活动，并通过简单直观的操作促进老年人与家人的沟通，对老年人护理具有实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有的少样本分割方法主要关注从支持集中提取提示来激活SAM，但这种设计在适应未知类别时容易导致有偏的解码过程，无法充分激活SAM的泛化能力。

Method: 提出无偏语义解码策略，设计两种特征增强策略：图像级的全局补充和像素级的局部引导，利用CLIP的语义对齐能力丰富原始SAM特征；并提出可学习的视觉-文本目标提示生成器。

Result: 该方法在不重新训练视觉基础模型的情况下，通过具有语义区分性的特征和富含目标信息的提示引导，将注意力集中在目标区域。

Conclusion: USD策略有效解决了SAM在少样本分割中的提示依赖问题，通过同时利用支持和查询集信息，结合CLIP的语义指导，实现了更无偏和准确的分割结果。
...
<details>
  <summary>Details</summary>
Motivation: 传统主动学习方法在医学影像标注中，单一采集策略在不同学习阶段表现不一致，需要一种能够自适应调整策略重要性的方法，以最大化有限标注预算的效用。

Method: 提出Cyclical and Performance-Adaptive Multi-Strategy Active Learning (WaveFuse-AL)框架，集成周期性（正弦）时间先验和性能驱动适应，动态调整多种采集策略的重要性权重。

Result: 在APTOS-2019（多类分类）、RSNA肺炎检测（二分类）和ISIC-2018（皮肤病变分割）三个医学影像基准测试中，WaveFuse-AL在十二个指标测量中的十个上实现了统计显著的性能提升。

Conclusion: WaveFuse-AL通过自适应融合多种策略，在医学影像主动学习中实现了更一致和优越的性能，有效降低了标注成本。
...
TL;DR: SceneEdited是首个专门为通过3D点云更新支持高清地图维护研究而设计的城市规模数据集，包含800多个最新场景，覆盖73公里驾驶区域，模拟了23,000多个合成物体变化。


<details>
  <summary>Details</summary>
Motivation: 高清地图会因环境变化而过时，现有技术虽然能检测变化，但在将2D图像变化检测结果更新到3D地图方面存在明显差距。

Method: 创建包含校准RGB图像、LiDAR扫描和详细变化掩码的数据集，提供基于图像的结构从运动流程的基线方法，以及支持可扩展性、可跟踪性和可移植性的工具包。

Result: 数据集覆盖约3平方公里的城市区域，包含2000多个过时版本，模拟了路边基础设施、建筑物、立交桥和电线杆等现实城市修改。
...
### [36] [Physics-Based Benchmarking Metrics for Multimodal Synthetic Images](https://arxiv.org/abs/2511.15204)
...
Method: 采用三阶段架构：多模态特征提取（目标检测和VLMs）、置信度加权组件融合、基于大语言模型的物理引导推理。

Result: PCMDE能够有效执行结构和关系约束（如对齐、位置、一致性）的强制实施。

Conclusion: PCMDE通过结合大语言模型和物理约束推理，克服了现有多模态评估指标的局限性。
...
TL;DR: 本文提出了一种新的人类运动预测评估指标MMCM，通过聚类方法定义运动模式，同时评估预测运动的多样性和合理性。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标仅关注预测运动的分布广度，但忽略了运动模式的多样性和运动合理性。单一过去序列可能对应多种未来运动，需要同时评估覆盖多个运动模式的能力和运动合理性。

Method: 提出MMCM指标：1）通过聚类将运动空间划分为多个模式；2）使用运动数据集收集可能的未来运动来识别有效模式；3）评估预测运动是否分布在多个有效模式中。

Result: 实验验证了聚类方法能够产生合理的模式定义，且MMCM能够准确评估多模态预测的质量。

Conclusion: MMCM是首个同时考虑运动模式多样性和运动合理性的评估指标，能够更全面地评估人类运动预测的多模态性能。
...
TL;DR: 本文提出了一种新的指令引导病变分割（ILS）范式，用于胸部X光片（CXR）的多种病变类型分割，并构建了首个大规模指令-答案数据集MIMIC-ILS，包含110万对指令-答案，覆盖7种主要病变类型。
...
Method: 采用全自动多模态流水线从胸部X光图像和对应报告中生成标注，构建MIMIC-ILS数据集，并基于此微调视觉语言模型ROSALIA。

Result: ROSALIA模型在新提出的任务中实现了高分割精度和文本准确性，验证了流水线的有效性和MIMIC-ILS作为基础资源的价值。

Conclusion: ILS范式能够基于简单用户指令分割多种病变类型，MIMIC-ILS数据集为像素级CXR病变定位提供了重要基础资源。
...
TL;DR: 提出了一种结合视觉变换器和残差CNN的混合架构BrainRotViT，用于从结构MRI准确估计大脑年龄，在11个数据集上达到3.34年的MAE，优于现有方法，并能识别与神经退行性疾病相关的脑区。


<details>
  <summary>Details</summary>
Motivation: 传统回归和CNN方法存在手动特征工程、感受野有限和在异构数据上过拟合的问题，而纯变换器模型需要大数据集和高计算成本，因此需要一种更高效、可泛化的脑年龄估计方法。

Method: 提出BrainRotViT混合架构：首先在辅助年龄和性别分类任务上训练ViT编码器学习切片级特征，然后冻结编码器应用于所有矢状切片生成嵌入向量2D矩阵，最后通过包含性别信息的残差CNN回归器估计连续脑年龄。

Result: 在11个MRI数据集（超过130个采集点）上验证，MAE为3.34年（Pearson r=0.98，Spearman ρ=0.97，R²=0.95），优于基线和最先进模型；在4个独立队列中MAE在3.77-5.04年之间；脑年龄差分析与阿尔茨海默病、认知障碍和自闭症谱系障碍相关。

Conclusion: 该方法提供了一个高效、可解释和可泛化的脑年龄预测框架，弥合了CNN和变换器方法之间的差距，为衰老和神经退行性疾病研究开辟了新途径。
...
TL;DR: Insert In Style是一个零样本生成框架，能够在风格化域中插入真实世界对象，无需在线微调即可实现高保真度合成。
...
Method: 提出了统一框架，包含多阶段训练协议和专门的掩码注意力架构，实现了身份、风格和合成的表示解耦。
...
<details>
  <summary>Details</summary>
Motivation: 现有量化方法存在局限性：QAT需要昂贵的重新训练，而PTQ方法要么只能部分量化非线性函数，要么无法实现完全整数推理。需要一种无需重新训练就能实现全整数推理的PTQ方法。

Method: 提出IPTQ-ViT框架，包含：1)针对视觉数据优化的多项式GELU近似函数；2)基于位移的Softmax近似函数；3)集成量化敏感性、扰动和计算成本的统一度量来选择每层最优近似函数。

Result: 在图像分类任务上实现最高6.44%(平均1.78%)的top-1准确率提升，目标检测任务上提升1.0 mAP。在W8A8和W4A8设置下优于部分浮点PTQ方法，准确率和延迟与整数QAT方法相当。

Conclusion: IPTQ-ViT是首个无需重新训练就能实现全整数推理的视觉Transformer PTQ框架，在保持准确率的同时显著提升了推理效率。
...
TL;DR: SkinGPT-R1是一个专注于皮肤科的视觉语言模型，通过显式的诊断思维链推理实现可验证的皮肤病诊断。该模型在DermBench基准测试中排名第一，平均得分4.031/5，相比Vision-R1提升了41%。


<details>
  <summary>Details</summary>
Motivation: 为了解决皮肤病诊断中推理过程不透明、难以验证的问题，需要开发能够提供明确、逐步、可验证诊断推理的皮肤科专用视觉语言模型。

Method: 构建了DermCoT语料库（包含10,000个训练案例和3,000个认证案例），定义了DermEval六维度评估器和DermBench基准，并采用皮肤科感知的视觉蒸馏技术。

Result: 在DermBench基准测试中，SkinGPT-R1在14个模型中排名第一，平均得分4.031/5，相比Vision-R1提升41%。在三个皮肤病分类基准上表现稳定且具有竞争力。

Conclusion: DermCoT思维链监督显著提升了基础模型性能，皮肤科感知视觉蒸馏进一步提高了叙述质量和识别能力，证明了该方法在皮肤病诊断中的有效性。
...
Method: 提出SplitFlux方法，包括两个关键组件：(1) 秩约束适应：压缩特定块的秩并放大更新幅度，防止内容泄漏到风格块；(2) 视觉门控LoRA：将内容LoRA分为不同秩的两个分支，高秩分支保留主体信息，低秩分支编码残差细节。
...
Method: GQN采用图查询的新概念，在鸟瞰图空间动态关注，构建物体特定图，通过EdgeFocus进行关系推理，DeepContext Pooling进行上下文聚合。

Result: 在NuScenes数据集上，GQN将相对mAP提升高达+53%，包括比最强先前的雷达方法提升+8.2%，同时将峰值图构建开销减少80%，FLOPs成本适中。
...
### [45] [HV-Attack: Hierarchical Visual Attack for Multimodal Retrieval Augmented Generation](https://arxiv.org/abs/2511.15435)
*Linyin Luo,Yujuan Ding,Yunshan Ma,Wenqi Fan,Hanjiang Lai*

Main category: cs.CV

TL;DR: 本文提出了一种针对多模态检索增强生成(MRAG)系统的视觉攻击方法，通过在用户输入的图像中添加难以察觉的扰动，破坏检索器和生成器的功能，而无需篡改其他组件。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注MRAG系统的知识投毒攻击，但本文考虑仅通过视觉扰动攻击MRAG系统的新场景，这更具挑战性因为需要克服微调检索器和大规模生成器的鲁棒性。

Method: 提出分层视觉攻击方法，通过两阶段策略：首先破坏跨模态对齐使检索器召回不相关知识，然后破坏多模态语义对齐来混淆生成器的生成过程。

Result: 在OK-VQA和InfoSeek数据集上的实验表明，该方法能显著降低MRAG系统的检索和生成性能，验证了视觉攻击的有效性。

Conclusion: 视觉攻击对MRAG系统构成严重威胁，揭示了仅通过图像扰动就能破坏整个MRAG链的新安全漏洞。
...
Method: 基于文本到图像生成的一阶段合成管道，包含交叉注意力细化策略利用扩散模型交叉注意力图优化边界框标注，以及背景遮挡建模策略在潜在空间中显式建模背景遮挡以增强图像复杂性。
...
<details>
  <summary>Details</summary>
Motivation: 为了利用CNN和Transformer各自的优势，CNN擅长提取局部特征而Transformer擅长捕捉长距离依赖关系，以提升MPox检测性能。

Method: 构建了RS-CA-HSICT框架，包含HSICT模块、残差CNN模块、空间CNN块和通道注意力机制，通过集成CNN的抽象表示和定制化ICT块实现多头注意力和结构化CNN层。

Result: 在两个MPox数据集上的实验结果显示，分类准确率高达98.30%，F1分数达到98.13%，优于现有的CNN和ViT方法。

Conclusion: 所提出的混合深度学习框架有效结合了CNN和Transformer的优势，在MPox检测任务中取得了优异的性能表现。
...
### [48] [Evaluating Low-Light Image Enhancement Across Multiple Intensity Levels](https://arxiv.org/abs/2511.15496)
...
<details>
  <summary>Details</summary>
Motivation: 现有的低光增强方法通常依赖单一光照条件下的配对数据，缺乏对不同光照强度的理解，限制了算法在多样化光照场景下的性能评估和改进。

Method: 引入MILL数据集，包含在固定相机设置和精确照度测量下捕获的不同光照强度图像；基于该数据集的多光照结构提出改进方法，增强算法在多样化光照条件下的鲁棒性。

Result: 改进方法在Full HD图像上实现了DSLR相机10 dB PSNR和智能手机2 dB PSNR的提升；基准测试显示现有方法在不同光照强度下性能差异显著。

Conclusion: MILL数据集为低光增强算法的全面评估提供了重要工具，提出的改进方法显著提升了算法在多样化光照条件下的性能表现。
...
TL;DR: Uni-Adapter是一种无需训练的在线测试时自适应策略，通过动态原型学习和图标签平滑来解决3D视觉语言基础模型在实际场景中的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 3D视觉语言基础模型在开放世界点云处理任务中表现出色，但在实际应用中面对噪声、不完整或分布偏移数据时性能下降。需要一种无需重新训练的自适应方法来提升模型在异质数据分布下的表现。

Method: 提出基于动态原型学习的训练免费在线测试时自适应策略：1）定义3D缓存存储类别特定的聚类中心作为原型；2）使用图标签平滑模块捕获原型间相似性；3）通过熵加权聚合统一原始模型和优化缓存的预测结果。

Result: 在多个3D基准测试上实现最先进性能：ModelNet-40C提升10.55%，ScanObjectNN-C提升8.26%，ShapeNet-C提升4.49%，显著优于源3D视觉语言基础模型。

Conclusion: Uni-Adapter有效缓解了分布偏移问题，无需重新训练即可显著提升3D视觉语言基础模型在异质数据分布下的性能，为实际应用提供了有效的自适应解决方案。
...
TL;DR: 本研究提出了一种新颖的多模态Transformer模型，通过融合雷达、RGB视频、红外视频和音频数据，实现了无人机检测和空中物体识别的高性能分类。
...
Method: 设计并评估了一种多模态Transformer模型，利用自注意力机制融合来自雷达、RGB视频、红外视频和音频的不同特征，学习全面、互补且高度区分的表示用于分类。

Result: 在独立测试集上表现出色，实现了0.9812准确率、0.9873召回率、0.9787精确率、0.9826 F1分数和0.9954特异性，在区分无人机与其他空中物体方面表现出特别高的精确率和召回率。

Conclusion: 本研究通过Transformer架构的多模态数据融合，在空域物体分类方面取得了显著进展，为复杂空域中的无人机检测和监控提供了高精度、高弹性的解决方案。
...
Method: 设计了潜在特征空间对齐模块（LFSAM）来弥合中间特征空间与潜在空间之间的语义鸿沟；开发了确定性反演流匹配（DIFM）将离流形特征投影到目标流形；提出了基于大视觉语言模型的两种量化指标。
...
### [52] [CompTrack: Information Bottleneck-Guided Low-Rank Dynamic Token Compression for Point Cloud Tracking](https://arxiv.org/abs/2511.15580)
...
<details>
  <summary>Details</summary>
Motivation: 现有3D点云跟踪器面临双重冗余挑战：空间冗余（背景噪声影响精度）和信息冗余（前景内部冗余影响效率），需要系统性地解决这些问题。

Method: 提出CompTrack框架，包含空间前景预测器(SFP)模块过滤背景噪声，以及基于信息瓶颈理论的动态令牌压缩(IB-DTC)模块，通过在线SVD分析自适应压缩前景冗余信息。

Result: 在KITTI、nuScenes和Waymo数据集上的广泛实验表明，CompTrack实现了顶级的跟踪性能，并在单个RTX 3090 GPU上达到实时90 FPS的效率。

Conclusion: CompTrack通过系统性地消除点云中的空间和信息冗余，在保持高精度的同时实现了实时高效的3D目标跟踪。
...
TL;DR: 提出了一种基于自适应阈值模式的指纹伪造检测算法，通过小波变换和SVM分类器来区分真假指纹，并对各种失真具有较强鲁棒性。
...
Result: 在定量比较中，该方法在90%像素缺失场景下准确率比同类方法高约8%，在70x70块缺失场景下准确率高约5%。

Conclusion: 提出的方法在处理各种失真方面表现出改进的抵抗能力，在应对环境现象或恶意用户操作引起的失真挑战方面具有新颖性。
...
TL;DR: 提出了一种轻量级、模型无关的后处理框架，用于无人机导航中的开放集检测，能够同时处理已知目标、未知对象和背景的三分类问题。


<details>
  <summary>Details</summary>
Motivation: 开发可靠的无人机导航系统需要能够区分训练期间见过的对象和未见过的对象的鲁棒空对空目标检测器。现有方法主要处理闭集检测，无法同时处理已知目标（ID）和分布外（OOD）对象，且通常使用单一不确定性评分和阈值方法，限制了灵活性。

Method: 采用轻量级、模型无关的后处理框架，通过融合方案聚合多个置信度估计和每个检测的特征，使用紧凑的多层感知机（MLP）。将不同的logit变体纳入MLP，在不影响吞吐量的情况下持续提升性能。

Result: 在二分类中平均超过基于阈值的基线方法2.7% AUROC，同时保持或改善开放集mAP。在闭集mAP上提升高达9个点（18%相对增益），支持鲁棒的三分类，这对无人机安全导航至关重要。

Conclusion: 该方法超越了基于阈值的基线方法，在保持或改善开放集mAP的同时显著提升了闭集检测性能，为无人机安全导航提供了关键的开放集检测能力。
...
TL;DR: ZOMG是一个零样本、开放词汇的运动理解框架，能够将运动序列分解为语义对齐的子动作，无需任何标注或微调。


<details>
  <summary>Details</summary>
Motivation: 理解复杂人类活动需要将运动分解为细粒度、语义对齐的子动作。现有方法依赖密集监督和预定义动作类别，在开放词汇的真实场景中不可行。

Method: ZOMG集成了语言语义分割（利用大语言模型将指令分解为有序子动作单元）和软掩码优化（学习实例特定的时间掩码来关注子动作关键帧，同时保持段内连续性和段间分离性）。

Result: 在三个运动-语言数据集上的实验表明，ZOMG在运动定位性能上达到了最先进的效果和效率，在HumanML3D基准上比先前方法提高了+8.7% mAP。

Conclusion: ZOMF为无标注运动理解建立了新范式，并在下游检索任务中表现出显著改进。
...
TL;DR: AutoPrune是一种新颖的LLM自剪枝方法，通过图驱动思维链和偏斜感知动态稀疏分配，自动设计最优剪枝算法，无需专家知识，解决了高剪枝率下的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM剪枝方法严重依赖人工设计，导致巨大的人工成本和专家知识需求，且在高剪枝率下存在严重的异常值问题导致性能急剧下降。
...
TL;DR: 本文提出了一种简单的后处理框架，通过保形预测为语义分割模型构建置信掩码，提供图像级别的假阳性控制保证。


<details>
  <summary>Details</summary>
Motivation: 深度分割模型很少提供明确的统计误差保证，而可靠的语义分割对临床决策至关重要，特别是在过度分割可能带来临床后果的场景中。

Method: 使用预训练分割模型，通过增加分数阈值或形态学腐蚀定义收缩掩码族，利用标记校准集通过保形预测选择收缩参数。

Result: 在息肉分割基准测试中实现了目标级别的经验有效性，方法具有模型无关性，无需重新训练。

Conclusion: 该框架在过度分割可能产生临床后果的场景中实现了实用的、风险感知的分割，为分割模型提供了有限样本保证。
...
TL;DR: 本文提出了D4C框架，这是首个专门为CLIP模型设计的数据自由量化方法，通过生成语义丰富且结构多样的伪图像来解决现有DFQ技术在CLIP上的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 数据自由量化在隐私敏感场景中具有重要价值，但现有方法直接应用于CLIP等视觉语言模型时会出现显著性能下降，主要原因是合成样本语义内容不足和图像内部多样性低。
...
Conclusion: D4C通过合成语义丰富且结构多样的伪图像，有效弥合了数据自由量化在CLIP上的性能差距，在各种比特宽度和模型上都验证了其有效性。
...
TL;DR: WarNav是一个基于DATTALION开源库图像构建的真实世界数据集，专门用于在冲突影响环境中开发自主地面车辆导航的语义分割模型，填补了传统城市驾驶资源与危险战区场景之间的空白。


<details>
  <summary>Details</summary>
Motivation: 解决传统城市驾驶数据集与危险战区无人系统操作场景之间的关键差距，促进高风险场景下自主车辆的鲁棒性和安全性研究。

Method: 从DATTALION开源库构建真实世界数据集，使用最先进的语义分割模型建立性能基准，分析训练数据环境的影响，并提出在目标图像无标注情况下实现有效导航性的初步方法。

Result: 建立了WarNav数据集的性能基准，展示了在无标注目标图像约束下实现有效导航性的可行性。

Conclusion: WarNav数据集为在极端操作环境中开发自主导航系统提供了重要资源，推动了在数据标注有限情况下高风险场景自主车辆的研究进展。
...
Method: 提出EASD端到端脉冲相机检测器，采用双分支设计：基于时间的纹理加特征融合分支用于全局跨切片语义，熵选择注意力分支用于目标中心细节。

Result: 创建了DSEC Spike首个面向驾驶的模拟脉冲检测基准数据集，填补了数据空白。

Conclusion: EASD方法能够有效处理脉冲相机的稀疏输出，为自动驾驶中的目标检测提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常只在单一尺度上对齐HE切片和对应的ST谱，忽略了细粒度细胞结构及其空间组织，需要开发能够捕捉多层次组织结构的框架。

Method: 提出Sigmma框架，采用多尺度对比对齐确保不同尺度的表示在模态间保持一致，通过图结构表示细胞相互作用并整合图间和图内关系，从细到粗捕捉组织微环境中的细胞-细胞相互作用。

Result: Sigmma学习的表示能更好地捕捉跨模态对应关系，在基因表达预测任务中平均提升9.78%，在跨模态检索任务中平均提升26.93%，并在下游分析中学习到有意义的多组织组织。

Conclusion: Sigmma通过多尺度对比对齐和图结构建模，有效解决了现有方法的局限性，为计算病理学中的多模态学习提供了更强大的框架。
...
Conclusion: 研究表明，结合YOLOv9-SAM2分割和分层分类的AI管道能够有效估计金枪鱼渔获的物种组成，为电子监控系统的自动化分析提供了可行解决方案。
...
TL;DR: FunnyNodules是一个完全参数化的合成医学图像数据集，用于系统分析医学AI模型中的基于属性的推理，通过可控的视觉属性生成肺结节样形状。


<details>
  <summary>Details</summary>
Motivation: 密集标注的医学图像数据集稀缺，特别是包含诊断推理过程的数据，这对于开发可解释AI模型至关重要。

Method: 生成抽象的肺结节样形状，具有可控的视觉属性（如圆度、边缘锐度、毛刺等），目标类别通过预定义的属性组合得出，完全控制属性与诊断类别之间的决策规则。
...
TL;DR: EAR是一种受人类视觉系统启发的自回归图像生成方法，采用从中心向外螺旋展开的生成顺序，结合长度自适应解码策略，在保持图像质量的同时显著提升生成效率。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视觉生成方法存在效率低下问题，要么需要逐令牌解码，要么多尺度表示复杂。受人类视觉系统中心向外感知模式启发，希望开发更高效的自回归生成范式。

Method: 提出扩展自回归表示(EAR)，采用螺旋顺序从中心向外展开图像令牌，保持空间连续性并支持并行解码。引入长度自适应解码策略，动态调整每步预测的令牌数量。

Result: 在ImageNet上的大量实验表明，EAR在单尺度自回归模型中实现了保真度和效率之间的最先进权衡，显著降低计算成本的同时提升生成质量。

Conclusion: EAR为可扩展且认知对齐的自回归图像生成设定了新方向，通过生物启发设计在效率和生成质量方面取得显著改进。
...
Method: 采用混合深度学习框架，结合CNN、ViT和GNN技术；使用GAN进行数据增强以平衡类别分布；采用自监督对比预训练方法从有限标注数据中学习更多特征。
...
TL;DR: 本文评估了多种人体姿态预测算法，发现可复现性问题，并提出了统一的训练评估流程。通过类比语音理解任务，展示了语音模型可有效适应姿态预测任务并提升性能。最后评估了模型在噪声姿态数据下的鲁棒性，提出了新的数据集变体，并展示了无监督微调对性能恢复的效果。


<details>
  <summary>Details</summary>
Motivation: 人体姿态预测在动作识别、自动驾驶和人机交互等领域有重要应用，但现有算法存在可复现性问题，且缺乏对真实噪声场景下模型鲁棒性的评估。

Method: 提出统一的训练和评估流程，将语音模型适配到姿态预测任务，并引入基于姿态估计器获得噪声关节坐标的新数据集变体来评估模型鲁棒性，使用无监督微调进行性能恢复。

Result: 语音模型在姿态预测任务中能提升当前最先进性能，但估计的姿态数据会导致显著性能下降，无监督微调可部分恢复性能。

Conclusion: 语音模型可有效用于人体姿态预测，但模型对噪声姿态数据敏感，无监督微调是改善鲁棒性的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 鉴于反取证的重要性，需要开发先进的对抗攻击来评估AI生成图像检测器的安全性，但目前这方面的研究还不够充分。
...
TL;DR: KDF+是一种用于LiDAR地点识别的持续学习框架，通过损失感知采样策略和排练增强机制解决灾难性遗忘问题，在多个基准测试中表现优于现有方法。
...
Method: 提出KDF+框架，包含损失感知采样策略（根据样本损失值估计学习难度并优先选择困难样本）和排练增强机制（在训练新任务时进一步优化记忆样本）。
...
TL;DR: 提出一种多模态深度学习方法，利用单张X射线图像信息来补全3D超声中被遮挡的脊柱解剖结构，解决超声成像中骨骼遮挡导致的视野受限问题。


<details>
  <summary>Details</summary>
Motivation: 超声成像在脊柱手术中具有无辐射、实时可视化的优势，但受限于骨骼的声影效应，无法完整显示椎体等解剖结构。需要一种方法来克服这一限制。

Method: 生成配对的训练数据：模拟X射线扫描的2D侧位椎体视图和模拟超声成像中可见性受限的3D部分椎体表示。通过多模态深度学习整合两种成像模式的形态信息。
...
TL;DR: 本文综述了通过增加推理时计算来提高预训练大语言模型预测准确性的技术，特别关注问题分解方式和子问题组织结构，将多种方法统一在同一框架下分析。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过增加推理时的计算资源来提升预训练大语言模型的预测准确性，为不同应用场景提供更有效的推理策略。

Method: 采用分类法对测试时扩展方法进行系统梳理，重点关注问题分解为子问题的方式以及子问题的拓扑结构（顺序、并行或树状结构）。

Result: 成功将Chain-of-Thought、Branch-Solve-Merge、Tree-of-Thought等多种方法统一在同一分析框架下，并综合分析了各自的优缺点。

Conclusion: 提出了未来研究的有前景方向，为改进大语言模型推理性能提供了系统性的理论框架和实践指导。
...
TL;DR: 研究表明，大型语言模型在推理过程中很早就内部确定了最终答案的正确性，即使需要更长的输出来得出明确答案。对于难题，预测准确率下降揭示了选择偏差：难题在长推理链中比例过高。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在推理过程中何时内部确定最终结果，了解模型内部自我评估能力的形成时机。

Method: 通过在推理过程的前t个token后的隐藏状态上训练线性分类器，预测最终答案的正确性。

Result: 模型在仅经过几个token后就能高度预测最终正确性；对于难题，预测准确率下降，表明难题在长推理链中比例过高。

Conclusion: 推理模型的内部自我评估能力在仅几个token后就趋于形成，这对可解释性和推理时控制具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型的跨语言知识迁移具有挑战性，因为目标语言的正确答案可能来自真正的知识转移，也可能来自预训练期间的先前暴露。

Method: 开发了LiveCLKTBench自动化生成管道，识别现实领域中的自包含、时间敏感知识实体，基于时间发生进行筛选，并验证模型知识。使用这些实体的文档生成事实性问题，翻译成多种语言以评估跨语言边界的可迁移性。

Result: 评估了多个LLM在五种语言上的表现，发现跨语言迁移受语言距离强烈影响，且在不同语言方向上通常不对称。较大模型能改善迁移，但收益随规模增加而递减，且在不同领域间变化。
...
TL;DR: COMPASS是一个轻量级可解释的控制框架，通过在解码过程中嵌入基于模型的反馈循环来减少LLM的事实错误，使用PID控制器动态调节注意力头以保持事实一致性。
...
Method: 引入COMPASS框架，通过上下文依赖分数(CRS)量化上下文依赖度，作为注意力头如何将生成基于证据的在线探针。使用PID控制器动态调节注意力头，无需重新训练或多轮解码。

Result: 在多个基准测试中(HotpotQA、XSum、HaluEval、RAGTruth)，COMPASS持续减少上下文幻觉率(绝对降低2.8%至5.8%)，并揭示了不同注意力头对证据对齐的贡献。

Conclusion: 反馈驱动的可解释性为科学理解LLM行为提供了一条途径，COMPASS框架能够有效减少事实错误并增强模型的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 自发语音合成面临捕捉自然对话流程的挑战，包括话轮转换、停顿和不流畅现象。现有研究很少探索显式韵律分割数据集及其对自发语音合成的影响。

Method: 使用非自回归模型FastSpeech 2，在巴西葡萄牙语数据集上比较手动和自动韵律分割标注的效果，分析其对语音合成质量的影响。

Result: 实验结果显示：韵律分割训练能产生更清晰和声学自然的语音；自动分割产生更规则的片段，手动分割引入更大变异性，有助于更自然的韵律；两种方法都能重现预期的核心重音模式，但韵律模型与自然前核心轮廓更接近。

Conclusion: 韵律分割标注能提升语音合成的自然度和清晰度，手动分割在韵律自然性方面表现更优。所有数据集、源代码和训练模型已公开供研究使用。
...
TL;DR: EasyMED是一个基于多智能体框架的标准化患者模拟系统，通过Patient Agent、Auxiliary Agent和Evaluation Agent实现真实对话、事实一致性和可操作反馈，在SPBench基准测试中达到与人类标准化患者相当的学习效果，同时具有更好的灵活性、心理安全性和成本效益。


<details>
  <summary>Details</summary>
Motivation: 现有的标准化患者(SP)在临床技能培训中成本高昂、灵活性差且难以扩展，而现有的基于大语言模型的SP模拟器存在行为不一致且缺乏与人类SP的严格比较的问题。

Method: 提出EasyMED多智能体框架，包含Patient Agent实现真实对话、Auxiliary Agent确保事实一致性、Evaluation Agent提供可操作反馈，并引入SPBench基准测试包含14个专科的真实SP-医生交互数据和8个专家定义评估标准。

Result: 实验表明EasyMED与人类标准化患者的学习效果相当，对基础较差的学生产生更大的技能提升，同时提供更好的灵活性、心理安全性和成本效率。

Conclusion: EasyMED框架能够有效替代传统标准化患者，在保持学习效果的同时显著降低成本并提高可扩展性，特别适合基础较差学生的临床技能培训。
...
TL;DR: HTP通过分层令牌预置解决LLM嵌入中的因果注意力限制，在块级别预置摘要令牌并采用均值池化，显著提升长文档嵌入性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的因果注意力机制限制了从后向前的信息流动，降低了表示质量。现有方法通过预置单个摘要令牌过度压缩信息，在长文档上表现不佳。

Method: 提出分层令牌预置(HTP)：将输入分区为块，在后续块中预置块级摘要令牌，创建多个后向信息流路径；用均值池化替代最后令牌池化以解决读取级过度压缩问题。

Result: 在11个检索数据集和30个通用嵌入基准测试中实现一致的性能提升，尤其在长上下文设置中表现突出。
...
TL;DR: 提出基于概率建模、信息论、三角信号分析和贝叶斯不确定性估计的数学框架，用于理解、测量和缓解大语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然功能强大，但仍容易产生看似合理但事实错误或缺乏支持的幻觉输出，需要系统性的解决方案。

Method: 分析自回归过程中的误差累积，提出改进的不确定性度量指标（包括语义和相位感知变体），开发对比解码、检索增强基础、事实对齐和弃权等原则性缓解策略。

Result: 建立了一个统一框架，将校准、检索和对齐等领域的最新进展联系起来，支持更安全可靠的大语言模型。

Conclusion: 该数学框架为理解和缓解大语言模型幻觉问题提供了理论基础和实践方法，有助于提升模型的可信度和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM辅导系统未能捕捉学生知识随能力、概念差距和遗忘模式的动态演变，特别是在需要精细校准的数学辅导中。

Method: TASA维护结构化学生画像记录能力档案和事件记忆，结合连续遗忘曲线与知识追踪，动态更新学生掌握状态并生成情境适当、难度校准的问题和解释。
...
TL;DR: 本文提出了一个评估印度语言视觉语言模型（VLM）的可扩展框架，并创建了HinTel-AlignBench基准测试，包含印地语和泰卢固语的多样化数据集，发现模型在印度语言上的表现平均比英语差8.3分（印地语）和5.5分（泰卢固语）。


<details>
  <summary>Details</summary>
Motivation: 解决当前多语言VLM评估的四个主要限制：依赖未经验证的自动翻译、任务/领域覆盖范围狭窄、样本量有限、缺乏文化和本地来源的问答数据。

Method: 提出半自动数据集创建框架，结合回译、过滤和人工验证；创建包含适应英语数据集和本地印度数据集的最全面的印地语和泰卢固语视觉语言基准测试。
...
TL;DR: 本文首次全面研究将内在维度与可解释的文本属性联系起来，发现ID与基于熵的指标互补，在控制长度后两者不相关；ID表现出稳健的体裁分层，科学文本ID最低，创意/观点写作ID最高；通过稀疏自编码器识别出科学信号降低ID，人性化信号增加ID的因果特征。


<details>
  <summary>Details</summary>
Motivation: 内在维度是现代LLM分析的重要工具，但对其文本决定因素的研究不足。本文旨在通过跨编码器分析、语言特征和稀疏自编码器，首次全面研究将内在维度与可解释的文本属性联系起来。

Method: 使用跨编码器分析、语言特征提取和稀疏自编码器（SAEs）方法，通过控制文本长度、分析不同体裁文本的ID差异，并进行转向实验验证因果效应。

Result: 发现ID与熵基指标互补且不相关；科学文本ID最低（约8），百科内容中等（约9），创意/观点写作最高（约10.5）；科学信号（正式语气、报告模板、统计数据）降低ID，人性化信号（个性化、情感、叙事）增加ID。

Conclusion: 对于当代模型，科学写作相对"简单"，而小说、观点和情感增加了表示自由度。多面分析为ID的正确使用和基于ID结果的合理解释提供了实践指导。
...
TL;DR: OEMA是一个用于临床命名实体识别的零样本多智能体协作框架，通过自标注器生成示例、鉴别器过滤和预测器推理，在MTSamples和VAERS数据集上达到最先进的精确匹配性能。
...
Result: 在MTSamples和VAERS数据集上，OEMA实现了最先进的精确匹配性能；在相关匹配下，性能与监督式BioClinicalBERT相当并超越CRF。
...
TL;DR: 本文提出了C3（Context Cascade Compression）方法，通过级联不同大小的LLM来处理长文本压缩和解码任务，在20倍压缩比下达到98%的解码准确率。
...
Method: 使用小型LLM作为第一阶段进行文本压缩，将长上下文压缩为一组潜在token（如32或64长度）；大型LLM作为第二阶段在压缩后的上下文中执行解码任务。

Result: 在20倍压缩比下达到98%解码准确率（DeepSeek-OCR约为60%），40倍压缩比下仍保持约93%准确率。
...
TL;DR: 该论文报告了TeamNRC参与BHASHA-Task 1语法错误纠正共享任务的结果，使用零/少样本提示语言模型的方法在泰卢固语和印地语中分别获得第4和第2名，并将实验扩展到泰米尔语、马拉雅拉姆语和孟加拉语，重点关注数据质量和评估指标问题。


<details>
  <summary>Details</summary>
Motivation: 探索使用不同规模的语言模型（4B到大型专有模型）进行零/少样本提示，在印度语言语法错误纠正任务中的有效性，并评估数据质量和评估指标的适用性。

Method: 采用零/少样本提示方法，使用不同规模的语言模型（4B到大型专有模型）对5种印度语言进行语法错误纠正。

Result: 在泰卢固语中获得第4名（GLEU得分83.78），在印地语中获得第2名（GLEU得分84.31），并扩展到其他三种语言。结果突显了小语言模型的潜力，同时揭示了数据质量和评估指标方面的问题。

Conclusion: 小语言模型在印度语言语法错误纠正任务中具有潜力，但需要关注高质量数据集创建和适用于印度语言脚本的适当评估指标。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语方言的情感分析面临语言多样性和标注数据稀缺的挑战，特别是在酒店评论这样的专业领域。

Method: 采用SetFit（句子转换器微调）框架，这是一种数据高效的小样本学习技术。

Result: 在官方评估集上，系统取得了73%的F1分数，在26个参与者中排名第12位。

Conclusion: 这项工作展示了小样本学习在处理专业领域（如酒店评论）中微妙阿拉伯语方言文本时解决数据稀缺问题的潜力。
...
TL;DR: 研究发现诗歌形式的对抗性提示可以作为通用单轮越狱技术，在25个前沿大语言模型中实现高攻击成功率，最高超过90%。诗歌攻击在多个风险领域有效，且比散文基线攻击成功率高出18倍。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型安全机制的局限性，验证风格变化是否能绕过当代安全机制，揭示当前对齐方法和评估协议的根本缺陷。

Method: 将1200个MLCommons有害提示通过标准化元提示转换为诗歌形式，使用集成开放权重法官模型和人工验证子集进行评估，手动解决分歧。
...
TL;DR: HEAD-QA v2是西班牙语/英语医疗保健多选推理数据集的扩展版本，包含超过12,000个来自西班牙专业考试的问题，用于评估开源LLM在医疗推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 响应医疗保健推理领域对高质量数据集的需求，该数据集旨在捕捉医疗推理的语言和概念复杂性。

Method: 扩展数据集至12,000多个问题，涵盖十年西班牙专业考试内容；使用提示、检索增强生成和基于概率的答案选择方法对多个开源LLM进行基准测试；提供多语言版本支持未来研究。

Result: 结果表明性能主要受模型规模和内在推理能力驱动，复杂推理策略带来的改进有限。

Conclusion: HEAD-QA v2为推进生物医学推理研究和模型改进提供了可靠的资源。
...
### [88] [NAMeGEn: Creative Name Generation via A Novel Agent-based Multiple Personalized Goal Enhancement Framework](https://arxiv.org/abs/2511.15408)
*Shanlin Zhou,Xinpeng Wang,Jianxun Lian,Zhenghao Liu,Laks V. S. Lakshmanan,Xiaoyuan Yi,Yongtao Hao*

Main category: cs.CL

TL;DR: 论文提出了NAMEGEn框架，用于解决创意自然语言生成中的多目标灵活性和解释复杂性挑战，特别针对中文起名这一短文本生成任务。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在创意自然语言生成中面临两个主要挑战：(1)多目标灵活性：难以同时满足用户个性化、细粒度和多元化的需求；(2)解释复杂性：需要理解和解释隐含意义以增强用户感知。

Method: 提出NAMEGEn多智能体优化框架，通过目标提取、名字生成和评估的迭代交替过程来满足多样化需求并生成准确解释。构建了包含17k+首古诗的语料库以增强美学效果，并引入CBNames基准测试。

Result: 大量实验表明，NAMEGEn能够有效生成满足多样化个性化需求的创意名字，并提供有意义的解释，在无需训练的情况下优于六种基于不同LLM骨干的基线方法。

Conclusion: NAMEGEn框架成功解决了创意自然语言生成中的关键挑战，特别是在短文本生成任务中表现出色，为个性化需求提供了有效的解决方案。
...
TL;DR: SPRING Lab开发了用于ASRU MADASR 2.0挑战赛的系统，专注于改进8种语言33种方言的语言和方言识别。采用多解码器架构和音素通用标签集作为中间表示，在Track 2中在3种语言上超越了基线WER/CER，并获得了最高的语言ID和方言ID准确率。
...
Method: 使用多解码器架构，以音素通用标签集作为中间表示的新颖训练方法，并讨论了将音素空间增益转换为相应字形表示的各种方法。

Result: 在CLS空间中性能优于基线，在Track 2中3种语言的WER/CER超越了基线，在所有参赛团队中获得了最高的语言ID和方言ID准确率。
...
TL;DR: LLM-MemCluster是一个新颖的LLM原生文本聚类框架，通过动态内存和双提示策略实现端到端的聚类，无需调优即可在多个基准数据集上显著优于基线方法。
...
Method: 提出LLM-MemCluster框架，将聚类重新概念化为完全LLM原生的任务。利用动态内存实现状态感知，采用双提示策略让模型能够推理并确定聚类数量。
...
TL;DR: 本文提出了LPDS数据结构和pelican nlp工具包，旨在解决语言处理领域缺乏标准化和可重复性的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型等AI语言处理技术的发展带来了语言数据定量分析方法的变化，但存在数据组织和共享缺乏标准化、处理方法缺乏可重复性等挑战。

Method: 1. 提出LPDS数据结构，受神经科学BIDS标准启发，提供文件夹结构和文件命名规范；2. 开发pelican nlp模块化Python包，支持从数据清洗到复杂语言和声学特征提取的完整处理流程。

Result: 构建了端到端的语言数据处理流程，通过单一配置文件即可执行整个处理工作流，输出预处理数据或标准化的语言声学特征及结果汇总。
...
TL;DR: Mera Multi是一个针对俄语的多模态评估框架，包含文本、图像、音频和视频四种模态的18个评估任务，为通用模型和模态特定架构提供基准测试。


<details>
  <summary>Details</summary>
Motivation: 目前多模态大语言模型发展迅速，但其智能水平、局限性和风险尚未充分理解，特别是在俄语领域缺乏多模态基准测试。

Method: 构建了包含18个全新评估任务的多模态基准，涵盖文本、图像、音频和视频模态，采用指令式评估方法，并设计了防止基准泄漏的方法论。

Result: 为闭源和开源模型提供了基线结果，创建了关注俄语文化和语言特性的数据集，并提供了统一提示和指标。
...
Method: 提出了课程调优框架，从初级到高级训练模型；创建评估系统，检查基于水平的语法覆盖、写作错误、词汇和句法复杂性以及整体评分；构建HSKAgent，在10K学习者作文上微调。
...
TL;DR: 本文介绍了FERMAT强化学习环境，用于自动化发现数学理论和概念，并探索了使用进化算法自动评估数学对象有趣性的方法。


<details>
  <summary>Details</summary>
Motivation: 解决人工智能中开放式的数学理论发现这一重大挑战，通过构建符号化操作的强化学习环境来模拟概念发现和定理证明过程。

Method: 引入FERMAT强化学习环境，采用基于LLM的进化算法，包含函数抽象机制，用于合成非平凡的有趣性度量标准。

Result: 在初等数论和有限域领域，该方法相比硬编码基线取得了显著改进，能够发现更有趣的数学对象。
...
<details>
  <summary>Details</summary>
Motivation: 研究多智能体科学推理中的信念形成和认知孤岛问题，提供一种可重现的方法来观察和测试这些动态过程，这在人类专家中是不可能的。

Method: 构建多智能体医疗案例模拟器，使用具有特定角色先验的LLM智能体（如神经科医生、传染病专家），通过共享医疗记录和与主持人的交互，在关键诊断时刻设置断点进行前后信念查询。

Result: 智能体信念往往反映现实世界的学科立场，包括过度依赖经典研究和抵制反证，这些信念可以被追踪和询问。
...
<details>
  <summary>Details</summary>
Motivation: 灾害事件的位置数据对风险评估至关重要，但现有数据库如EM-DAT中的位置信息通常是非结构化的文本形式，存在粒度不一致和拼写问题，难以与空间数据集集成。

Method: 开发了基于GPT-4o的自动化工作流，处理文本位置信息，并通过交叉验证GADM、OpenStreetMap和Wikidata三个独立的地理信息库来分配几何形状和可靠性评分。

Result: 应用于2000-2024年的EM-DAT数据集，成功地理编码了14,215个事件的17,948个独特位置，无需人工干预，覆盖所有灾害类型。

Conclusion: 该方法展示了LLM从非结构化文本中提取和构建地理信息的潜力，为相关分析提供了可扩展且可靠的方法，并支持灵活的重映射到首选框架。
...
TL;DR: 本文记录了Project Rachel行动研究项目，创建并追踪了一个名为Rachel So的完整AI学术身份，通过发表AI生成的研究论文来调查学术界对AI作者身份的反应。
...
Result: Rachel So成功发表了10多篇论文，获得了引用，并收到了同行评审邀请，表明AI作者身份在学术生态系统中能够获得一定程度的认可。
...
<details>
  <summary>Details</summary>
Motivation: 确保AI系统（如自动驾驶汽车）的可信性和安全性，关键在于训练和测试数据集的数据相关安全属性，特别是代表性——场景数据反映系统设计安全运行条件（ODD）或预期遇到条件（TOD）的程度。
...
TL;DR: 本文提出了OpenBioLLM，一个模块化的多智能体框架，用于基因组问答任务。它通过引入专门的智能体进行工具路由、查询生成和响应验证，在保持性能的同时显著降低了延迟。


<details>
  <summary>Details</summary>
Motivation: GeneGPT虽然通过结合领域特定API和大型语言模型解决了基因组问答的挑战，但其依赖专有模型限制了可扩展性、增加了运营成本，并引发数据隐私和泛化能力的担忧。

Method: 首先使用开源模型（Llama 3.1、Qwen2.5、Qwen2.5 Coder）在单体架构中复现GeneGPT，然后开发OpenBioLLM模块化多智能体框架，引入专门的智能体进行工具路由、查询生成和响应验证。

Result: OpenBioLLM在90%以上的基准任务中匹配或超越了GeneGPT的性能，在Gene-Turing和GeneHop上分别获得0.849和0.830的平均分数，同时将延迟降低了40-50%。

Conclusion: 研究结果表明开源多智能体系统在基因组问答任务中具有巨大潜力，OpenBioLLM在不进行额外微调或工具特定预训练的情况下，使用更小的开源模型实现了优异的性能。
...
TL;DR: Rogue One是一个基于LLM的多智能体框架，通过三个专业智能体（科学家、提取器、测试器）的协同工作，结合外部知识检索和丰富的定性反馈机制，实现知识引导的自动特征提取。
...
Method: 采用去中心化的三智能体系统：科学家负责特征发现，提取器生成特征，测试器验证特征。引入RAG系统整合外部知识，采用"泛滥-修剪"策略平衡特征探索与利用，并提供丰富的定性反馈。

Result: 在19个分类和9个回归数据集上显著优于现有最优方法，并能发现新颖、可测试的假设（如心肌数据集中的新生物标志物）。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全评估主要关注输出层面的判断，很少捕捉推理过程中的动态风险，而大型推理模型的链式思维能力可能引入新的安全风险，如有害内容的渐进注入或被误导性理由合理化。
...
Conclusion: SafeRBench为大型推理模型提供了全面的安全评估框架，能够识别传统方法难以捕捉的动态推理过程安全风险。
...
<details>
  <summary>Details</summary>
Motivation: LLM在大量数据集上训练，常包含受版权保护材料，引发未经授权使用的担忧。现有成员推理攻击方法因LLM过度自信、缺乏真实训练数据和依赖经验阈值而面临挑战。

Method: 采用两阶段策略：(1) 将文件分割成小片段以减少对大规模训练数据的依赖；(2) 基于不确定性的无监督聚类，无需经验调优阈值。

Result: 在LLaMA 7b和LLaMA2 7b上检测训练数据文件分别达到90.1%和91.6%的平均平衡准确率，相比SOTA基线相对提升超过90%，最高达93.8%平衡准确率。在GPT-J 6B上保持高性能。
...
<details>
  <summary>Details</summary>
Motivation: AI研究正转向复杂问题解决，推理AI缺乏需求饱和点，性能随计算投入指数级增长，而效率改进正接近物理极限

Method: 分析推理AI的计算扩展特性，讨论效率改进的物理限制，提出在系统优化和治理中嵌入明确限制的研究和政策方向

Result: 识别出推理AI缺乏自然饱和点，性能持续随计算投入指数增长，效率提升面临物理极限

Conclusion: 仅靠效率无法实现可持续推理AI，需要在优化和治理中嵌入明确限制以控制计算增长
...
TL;DR: 论文探讨了AI研究中两种对立的情报观：情报现实主义（认为情报是单一、普遍的）和情报多元主义（认为情报是多样、情境依赖的），分析了这两种观点如何影响方法论、解释和风险评估。


<details>
  <summary>Details</summary>
Motivation: 揭示AI研究中隐含的两种基本情报观念如何塑造研究方法和风险评估，促进对这些根本分歧的明确理解。

Method: 通过分析当前AI研究中的辩论，展示这两种情报观如何影响模型选择、基准设计、实验验证等研究方法论，以及对相同经验现象的不同解释。

Result: 发现情报现实主义者和多元主义者在方法论、现象解释和风险评估方面存在根本分歧：前者关注超级智能风险并寻求统一解决方案，后者关注跨领域多样化威胁并寻求情境特定解决方案。
...
### [105] [Octopus: Agentic Multimodal Reasoning with Six-Capability Orchestration](https://arxiv.org/abs/2511.15351)
*Yifu Guo,Zishan Xu,Zhiyuan Yao,Yuquan Lu,Jiaye Lin,Sen Hu,Zhenheng Tang,Yingchao Li,Huacan Wang,Ronghao Chen*

Main category: cs.AI

TL;DR: 提出Octopus：一种具有六种能力协调的多模态智能推理新范式，能够自主探索推理路径并动态选择最合适的能力，在Octopus-Bench基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推理模型存在架构限制，缺乏自主探索多样化推理路径的能力，无法适应动态变化的现实任务需求。受人类互补思维能力的启发，提出需要协调多种核心能力。

Method: 定义多模态推理的六种核心能力，构建Octopus-Bench评估基准，开发能够自主探索推理过程并动态选择最合适能力的Octopus系统。

Result: 实验结果显示，Octopus在Octopus-Bench基准测试中的绝大多数任务上取得了最佳性能。

Conclusion: 能力协调在智能多模态推理中起着关键作用，Octopus范式通过六种能力协调实现了优越的推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多任务基准主要评估智能体在无关任务间切换策略的能力，而非测试智能体在多个相互作用的挑战中进行深度推理的能力。因此需要开发能同时引发多个经典RL挑战的单一环境。

Method: 基于《文明V》游戏构建综合性挑战环境，要求智能体在单一环境中同时应对部分可观测性、信用分配、表示学习、巨大动作空间等挑战，进行长期规划和深度推理。

Result: 提出了Terra Nova环境，该环境排除了仅聚合无关任务的基准，专注于测试智能体在多个相互作用变量间的综合理解能力。

Conclusion: Terra Nova为强化学习研究提供了更真实的测试平台，强调智能体在复杂、长期、多挑战环境中的综合推理能力，而非简单的任务切换能力。
...
TL;DR: 该论文研究了智能体如何通过交互学习获得类似人类的物理推理能力，提出了IPR（交互物理推理器）框架，使用世界模型来增强VLM的策略，并引入PhysCode作为物理中心动作编码，在1000+游戏中预训练后表现出强大的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究智能体是否能够通过交互学习获得类似人类的物理和因果推理能力，并随着经验积累持续改进。

Method: 提出IPR框架，使用世界模型推演来评分和增强VLM的策略，引入PhysCode作为物理中心动作编码，在1000+异构游戏上进行预训练。

Result: IPR在三个推理级别上表现稳健，整体性能与GPT-5相当，在好奇心级别上超越GPT-5，性能随训练游戏和交互步骤增加而提升，并能零样本迁移到未见游戏。

Conclusion: 物理中心的交互学习是实现持续改进物理推理能力的可行路径。
...
<details>
  <summary>Details</summary>
Motivation: DeFi交易意图理解因复杂的智能合约交互、多维度链上/链下因素和不透明的十六进制日志而具有挑战性，现有方法缺乏深度语义洞察。

Method: TIM框架包含：DeFi意图分类法、元级规划器动态协调领域专家分解多视角意图分析、问题求解器处理多模态链上/链下数据、认知评估器减轻LLM幻觉并确保可验证性。

Result: 实验表明TIM显著优于机器学习模型、单一LLM和单一智能体基线，分析了意图推理中的核心挑战。

Conclusion: TIM为DeFi中用户动机提供了更可靠的理解，为复杂的区块链活动提供上下文感知解释。
