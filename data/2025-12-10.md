...
TL;DR: 使用Planet Labs四波段卫星影像和深度学习模型，实现对苏丹武装冲突中火灾损害的近实时监测，相比基线方法能更准确捕捉活跃火灾和烧焦区域。
...
Method: 采用Planet Labs的四波段卫星影像，结合深度学习模型，开发了自动化监测方法。通过苏丹的五个案例研究验证了方法的有效性。

Result: 相比基线方法，自动化方法能更准确地捕捉活跃火灾和烧焦区域。实验表明，使用八波段影像或时间序列影像只能带来边际增益。

Conclusion: 深度学习模型结合四波段卫星影像可以有效监测武装冲突中的火灾损害，实现最小延迟的近实时监测，为冲突地区的快速响应提供技术支持。
...
TL;DR: LivingSwap是首个视频参考引导的人脸交换模型，通过关键帧作为条件信号注入目标身份，结合视频参考引导实现时间拼接，在长视频序列中保持稳定的身份一致性和高保真重建。
...
Method: 1. 使用关键帧作为条件信号注入目标身份，实现灵活可控的编辑；2. 结合关键帧条件和视频参考引导，进行时间拼接以确保长视频序列中的稳定身份保持和高保真重建；3. 为解决参考引导训练数据稀缺问题，构建了配对人脸交换数据集Face2Face，并通过反转数据对确保可靠的真实监督。

Result: 大量实验表明，该方法取得了最先进的结果，能够无缝地将目标身份与源视频的表情、光照和运动相结合，同时显著减少了生产流程中的手动工作量。

Conclusion: LivingSwap作为首个视频参考引导的人脸交换模型，通过创新的关键帧条件和时间拼接方法，有效解决了长视频序列中人脸交换的高保真度和时间一致性问题，为影视制作提供了高效的解决方案。
...
TL;DR: FRIEDA是一个评估大视觉语言模型复杂开放式地图推理能力的基准，涵盖拓扑、度量和方向三类空间关系，要求多步推理和跨地图关联，现有模型表现远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 地图推理是解释地理关系的关键认知能力，在灾害响应和城市规划等任务中至关重要，但目前缺乏专门评估。现有地图VQA研究常将地图视为图表的特例，但地图理解需要处理分层符号系统、空间关系以及跨地图推理，这些在图表式评估中未被充分捕捉。

Method: 从各领域和地理区域的文档报告中收集真实地图图像，基于GIS文献分类，涵盖拓扑（边界、相等、相交、包含）、度量（距离）和方向（方位）三类空间关系。所有问题都需要多步推理，许多需要跨地图关联和推理。在两种设置下评估：直接设置（提供相关地图）和上下文设置（模型需先识别相关地图）。

Result: 评估了11个最先进的LVLM，即使最强的Gemini-2.5-Pro和GPT-5-Think模型也仅达到38.20%和37.20%的准确率，远低于人类84.87%的表现。结果显示在多步地图推理方面存在持续差距。

Conclusion: FRIEDA作为一个严格的基准，揭示了LVLM在空间智能方面的显著不足，为推动地图推理能力进步提供了重要工具。地图推理需要专门的评估框架，不能简单视为图表理解的特例。
...
TL;DR: 该论文提出了一种基于频域掩码的通用深度伪造检测方法，通过在训练中引入随机掩码和几何变换，特别是频域掩码，来增强检测器对未见生成模型的泛化能力，同时保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成图像技术的快速发展，需要能够检测各种生成模型（包括未见过的模型）的通用深度伪造检测方法。传统方法过度依赖空间特征或大规模预训练模型，缺乏对新出现的深度伪造的泛化能力，且计算开销大，不符合绿色AI的要求。

Method: 提出频域掩码训练策略，在训练过程中对图像应用随机掩码和几何变换，特别强调频域掩码的优越泛化特性。该方法不依赖大规模预训练模型，而是通过频域操作增强检测器的泛化能力。

Result: 在GAN和扩散模型生成的图像数据集上实现了最先进的泛化性能，同时在显著模型剪枝下仍能保持检测性能，提供了可扩展且资源节约的解决方案。

Conclusion: 频域掩码方法为可持续和可泛化的深度伪造检测提供了实用途径，在保持高性能的同时减少了计算资源需求，符合绿色AI的发展方向。
...
TL;DR: M2A提出了一种简单的持续测试时自适应方法，通过随机掩码生成不同视图，结合一致性损失和熵最小化损失进行适应，无需复杂的掩码设计或不确定性校准。


<details>
  <summary>Details</summary>
Motivation: 现有持续测试时自适应方法依赖校准的不确定性或稳定的注意力分数，并引入额外复杂性。作者质疑是否需要定制化的掩码设计，在强干扰下简单随机掩码是否足够。

Method: M2A方法生成掩码视图序列（空间或频率域），使用两个目标函数：掩码一致性损失（对齐不同视图的预测）和熵最小化损失（鼓励自信输出）。研究了空间掩码（补丁vs像素）和频率掩码（全频vs低频vs高频）两种类型。

Result: 在CIFAR10C/CIFAR100C/ImageNetC（严重程度5）上，M2A（空间）达到8.3%/19.8%/39.2%的平均错误率，优于或匹配强基线；M2A（频率）表现较差。消融实验表明简单随机掩码有效且鲁棒。

Conclusion: 简单随机掩码调度结合一致性和熵目标足以驱动有效的测试时自适应，无需依赖不确定性或注意力信号。空间掩码优于频率掩码。
...
TL;DR: CVP是一个受人类中央-外周视觉启发的多模态空间推理框架，通过目标亲和性令牌和全局网格组件实现结构化场景理解，在3D场景理解基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖点云、体素或补丁特征等非结构化表示，通过坐标嵌入隐式注入场景上下文，这导致空间推理能力有限，缺乏显式的高层次结构理解。

Method: 提出中央-外周视觉启发框架(CVP)，包含两个互补组件：1)目标亲和性令牌（类似中央视觉），引导模型关注查询相关对象；2)全局网格（类似外周视觉），捕捉全局场景上下文和空间布局。这些组件在大型多模态模型架构中协同工作。

Result: CVP在一系列3D场景理解基准测试中实现了最先进的性能。

Conclusion: 通过模拟人类视觉系统的中央-外周视觉机制，CVP框架能够实现结构化、上下文感知的复杂3D环境理解，显著提升了多模态空间推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的非均匀雾霾去除对可靠视觉感知至关重要，但现有方法面临挑战。基于Transformer的方法虽然擅长捕捉全局上下文，但二次计算复杂度阻碍了实时部署

Method: 提出基于多状态感知范式的Fourier-RWKV框架，整合三种感知状态：1) 空间形式感知：通过可变形四方向令牌移位动态调整感受野；2) 频域感知：在傅里叶混合块中将RWKV的WKV注意力机制扩展到傅里叶域；3) 语义关系感知：通过语义桥接模块实现动态语义核融合，精确对齐编码器-解码器特征

Result: 在多个基准测试上的广泛实验表明，Fourier-RWKV在多样化雾霾场景中实现了最先进的性能，同时显著降低了计算开销

Conclusion: Fourier-RWKV在恢复质量和实际效率之间建立了有利的权衡，为实时去雾应用提供了有效解决方案
...
TL;DR: GeoLoom是一个用于几何图形生成的文本到图表生成框架，通过形式化语言和坐标求解器实现高精度几何图生成


<details>
  <summary>Details</summary>
Motivation: 高质量的几何图形生成既具有挑战性又有机遇：它要求严格的空间精度，同时提供了明确的约束来指导生成。受几何问题求解中形式化语言和符号求解器在增强正确性和可解释性方面的最新进展启发

Method: GeoLoom包含两个核心组件：1）自动形式化模块，将自然语言翻译为专门设计的面向生成的形式语言GeoLinua；2）坐标求解器，使用高效的蒙特卡洛优化将形式约束映射到精确坐标。还引入了GeoNF数据集来支持该框架

Result: GeoLoom在结构保真度方面显著优于最先进的基线方法，为可解释和可扩展的图表生成提供了原则性基础

Conclusion: GeoLoom通过形式化语言和约束求解的方法，为几何图形生成提供了一个可解释、可扩展且具有数学基础的框架，显著提高了生成图表的空间精度和结构保真度
...
TL;DR: 该论文提出了视觉知识库(Visual Knowledge Base)框架VisKnow，通过构建多模态对象知识图谱来增强对象理解，并以动物知识库AnimalKB为例展示了其应用价值。


<details>
  <summary>Details</summary>
Motivation: 当前计算机视觉中的对象理解通常局限于类别标签识别，缺乏对对象组件、外观特征、类别关系、背景知识等的全面感知。现有的多模态数据通常是任务导向的，缺乏系统组织，无法实现预期的对象类别理解。

Method: 提出了VisKnow框架，通过结合专家设计和大规模模型应用，从丰富的对齐文本和图像源知识中提取多模态、对象级知识，构建结构化知识图谱。具体构建了AnimalKB知识库，涵盖406个动物类别，包含22K文本知识三元组、420K图像及区域标注。

Result: AnimalKB增强了零样本识别和细粒度视觉问答等对象级视觉任务，并为知识图谱补全和部件分割提供了具有挑战性的基准。实验展示了视觉知识库在推进视觉理解和实际应用方面的潜力。

Conclusion: 自动构建视觉知识库能够显著推进视觉理解及其实际应用，VisKnow框架为系统组织多模态对象知识提供了有效方法，AnimalKB展示了该方法在具体领域中的应用价值。
...
TL;DR: 该论文探索了在3D目标检测中应用提示调优方法的有效性，研究了基于Waymo数据集训练的模型作为基础模型适应其他场景的能力，并提出了场景导向的提示池方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型展现出的强大泛化能力，通过微调和提示调优等技术可以适应各种下游任务。本文旨在探索在3D目标检测领域中常见提示调优方法的有效性，研究基于大规模Waymo数据集训练的模型能否作为基础模型适应其他3D目标检测场景。

Method: 论文依次研究了提示令牌和提示生成器的影响，并进一步提出了场景导向的提示池（SOP²）。该方法旨在通过提示池技术提升3D目标检测的性能。

Result: 论文证明了提示池在3D目标检测中的有效性，展示了基于Waymo数据集训练的基础模型可以通过提示调优适应其他场景。

Conclusion: 该研究为未来研究者深入探索提示在3D领域的潜力提供了启发，证明了提示调优方法在3D目标检测中的适用性和有效性。
...
TL;DR: MM-CoT是一个诊断性基准测试，专门设计用于评估多模态模型在思维链推理中的视觉基础和逻辑连贯性，要求模型选择同时满足视觉一致性和逻辑连贯性的事件链，而不是生成自由形式的解释。
...
<details>
  <summary>Details</summary>
Motivation: 当前深度补全方法使用的稀疏深度数据通常是从密集真值中随机均匀采样，忽略了真实传感器具有几何依赖性和空间非均匀可靠性的事实，导致训练条件不真实。

Method: 提出法线引导的稀疏深度采样策略：1）在RGB-D点云上使用PCA进行表面法线估计；2）计算每个像素的深度可靠性度量；3）根据可靠性分布采样稀疏深度点；4）将该方法与Marigold-DC扩散模型集成。

Result: 在NYU Depth v2数据集上的实验表明，几何感知的稀疏深度采样提高了精度，减少了边缘和不连续区域的伪影，创造了更真实的训练条件，更好地反映了真实传感器行为。

Conclusion: 通过考虑传感器可靠性的几何依赖性，提出的采样策略能够生成更真实的训练数据，从而改进深度补全模型的性能，特别是在边缘和不连续区域。
...
<details>
  <summary>Details</summary>
Motivation: 当前相机BEV感知面临性能与部署可行性之间的根本矛盾：先进性能依赖计算密集的视角变换和平台特定的定制内核，阻碍车载部署。

Method: 采用"算法快速"和"设计可部署"两大原则：1) 通过确定性预排序策略将视角变换分解为标准索引-收集-重塑流水线，使用原生算子；2) 集成端到端深度感知融合机制，结合时间聚合和数据增强。

Result: 在nuScenes基准测试中达到0.359 NDS的SOTA性能，在Tesla T4等车载硬件上超过134 FPS，无需定制插件。

Conclusion: FastBEV++提供了一种成熟可扩展的设计理念，解决了BEV感知中性能与部署可行性的矛盾，为生产级自动驾驶系统提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型在处理视觉补丁标记时面临二次计算成本问题，现有方法存在权衡：连续压缩会稀释高级语义（如物体身份），而离散量化会丢失细粒度细节（如纹理）。

Method: 提出混合框架HTC-VLM，通过双通道分离语义和外观：连续通道使用ViT补丁处理细粒度细节，离散通道使用MGVQ量化生成四个标记的符号锚点，通过解缠注意力掩码和瓶颈机制融合为580个标记的混合序列，最终压缩为单个voco标记。

Result: 在七个基准测试（GQA、VQAv2、MMBench、MME、POPE、SEED-Bench、ScienceQA-Image）上平均性能保持率达到87.2%，优于领先的连续基线方法的81.0%，实现580:1的压缩比。注意力分析显示压缩标记优先考虑离散锚点，验证了其语义引导作用。

Conclusion: 研究表明，简约的混合设计能够解决效率与保真度之间的困境，推动可扩展视觉语言模型的发展，证明了通过分离语义和外观的双通道方法可以实现高效且准确的视觉表示。
...
<details>
  <summary>Details</summary>
Motivation: 解决乳腺超声图像分割中局部特征提取、全局依赖学习、噪声抑制和边界保持等挑战，提高病变诊断性能并支持临床决策。
...
Conclusion: 提出的Residual-SwinCA-Net框架显著提升了乳腺病变分割性能，增强了诊断准确性，有助于及时临床决策。
...
Conclusion: FTKD方法有效将未来帧知识从离线教师模型转移到在线学生模型，显著提升相机时序3D目标检测性能
...
TL;DR: 提出QHP方法解决少样本3D点云分割中原型偏差问题，通过查询感知的枢纽原型学习和原型分布优化，显著提升分割性能
...
Method: 提出查询感知枢纽原型（QHP）学习方法：1）枢纽原型生成（HPG）模块：构建查询点和支持点的二分图，识别频繁链接的支持枢纽，生成查询相关的原型以更好地捕获跨集语义；2）原型分布优化（PDO）模块：使用纯度重加权对比损失，通过将坏枢纽和离群原型拉近对应类中心来优化原型表示。

Result: 在S3DIS和ScanNet数据集上的大量实验表明，QHP相比最先进方法实现了显著的性能提升，有效缩小了原型与查询集之间的语义差距。

Conclusion: QHP方法通过显式建模支持集和查询集之间的语义相关性，解决了少样本3D点云分割中的原型偏差问题，提高了分割性能。
...
TL;DR: RLCNet是一个端到端可训练的深度学习框架，用于LiDAR、RADAR和摄像头等多模态传感器的在线同时标定，通过加权移动平均和异常值剔除实现实时动态调整，在真实数据集上表现出优越的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆中LiDAR、RADAR和摄像头等传感器的精确外参标定对于可靠感知至关重要，但由于机械振动和动态环境中的累积传感器漂移等因素，这仍然是一个具有挑战性的问题。

Method: 提出了RLCNet，一个新颖的端到端可训练的深度学习框架，用于多模态传感器的同时在线标定。为了支持实时操作，引入了包含加权移动平均和异常值剔除的在线标定框架，能够动态调整标定参数，减少预测噪声并提高对漂移的鲁棒性。

Result: 在真实世界数据集上验证，RLCNet设计用于实际部署，在不同条件下表现出鲁棒性能。消融研究强调了架构选择的重要性，与现有方法的比较证明了所提方法在准确性和鲁棒性方面的优越性。

Conclusion: RLCNet为自动驾驶车辆的多模态传感器在线标定提供了一个有效的解决方案，通过深度学习框架和实时调整机制，在动态环境中实现了准确、鲁棒的标定性能。
...
<details>
  <summary>Details</summary>
Motivation: 从第三人称视角视频生成第一人称视角视频能为沉浸式理解开辟新可能，但由于相机姿态变化极大、视角重叠极少，这一任务极具挑战性

Method: EgoX框架利用大规模视频扩散模型的预训练时空知识，通过轻量级LoRA适配；采用统一的条件策略，通过宽度和通道级联结合第三人称和第一人称先验；引入几何引导的自注意力机制选择性关注空间相关区域

Result: 该方法实现了连贯且真实的第一人称视频生成，在未见过的和真实世界视频中展现出强大的可扩展性和鲁棒性

Conclusion: EgoX框架成功解决了从第三人称到第一人称视频转换的几何一致性和视觉保真度问题，为沉浸式视频理解提供了有效解决方案
...
TL;DR: PAVAS是一种物理感知的视频到音频合成方法，通过物理驱动音频适配器将物理推理融入潜在扩散模型，利用物体质量和速度等物理参数生成更真实的声音。
...
Method: 1. 使用物理参数估计器（PPE）通过视觉语言模型估计物体质量，通过基于分割的动态3D重建模块恢复运动轨迹计算速度；2. 通过物理驱动音频适配器（Phy-Adapter）将这些物理参数融入潜在扩散模型；3. 提出VGG-Impact基准和音频物理相关性系数（APCC）评估指标。

Result: PAVAS在定量和定性评估中均优于现有V2A模型，能够生成物理上合理且感知上连贯的音频，特别是在物体-物体交互场景中表现出色。

Conclusion: 将物理推理融入视频到音频合成能够显著提升生成声音的物理真实性和感知质量，为更真实的视听内容生成提供了新方向。
...
TL;DR: OpenSubject是一个包含250万样本、435万图像的大规模视频衍生数据集，用于主题驱动的图像生成和编辑，通过四阶段流程构建，显著提升了复杂场景下的生成和编辑性能。
...
Method: 采用四阶段流程构建数据集：1）视频筛选（分辨率与美学过滤）；2）跨帧主题挖掘与配对（基于VLM的类别共识、局部定位和多样性感知配对）；3）身份保持参考图像合成（分割图引导的外绘和框引导的内绘，结合几何感知增强和不规则边界侵蚀）；4）验证与标注（VLM验证样本，失败样本重新合成，构建长短描述）。

Result: 构建了包含250万样本、435万图像的大规模数据集OpenSubject，并建立了覆盖主题驱动生成和编辑的基准测试。实验表明，使用OpenSubject训练能显著提升生成和编辑性能，特别是在复杂场景中。

Conclusion: OpenSubject数据集通过系统化的构建流程解决了主题驱动生成中的身份保持和复杂场景处理问题，为相关研究提供了高质量的训练数据和评估基准。
...
TL;DR: Terrain Diffusion：一种基于扩散模型的无限地形生成系统，作为Perlin噪声的AI时代替代品，结合了扩散模型的高保真度和传统程序化噪声的无缝无限扩展、种子一致性、恒定时间随机访问等特性。


<details>
  <summary>Details</summary>
Motivation: 传统程序化噪声（如Perlin噪声）虽然快速且无限，但在真实感和大尺度连贯性方面存在根本限制。需要一种AI时代的新方法，既能保持程序化噪声的优势特性，又能提供更高的真实感和大规模连贯性。

Method: 1. InfiniteDiffusion算法：实现无限生成，支持无缝实时合成无边景观；2. 分层扩散模型堆栈：将行星级上下文与局部细节耦合；3. 紧凑拉普拉斯编码：稳定地球尺度动态范围的输出；4. 开源无限张量框架：支持无界张量的恒定内存操作；5. 少步一致性蒸馏：实现高效生成。

Result: 建立扩散模型作为程序化世界生成的实用基础，能够连贯、可控、无限制地合成整个行星，同时保持无缝无限扩展、种子一致性和恒定时间随机访问等关键特性。

Conclusion: Terrain Diffusion成功将扩散模型的高保真度与传统程序化噪声的关键特性相结合，为AI时代的程序化世界生成提供了新的基础框架，能够生成真实感强、大规模连贯的无限地形。
...
TL;DR: GeoDM提出了一种几何感知的数据集蒸馏框架，通过在欧几里得、双曲和球面流形的笛卡尔积空间中操作，捕获数据的平坦、层次和循环结构，优于现有方法。
...
Conclusion: GeoDM通过在多几何乘积空间中操作，成功捕获了数据的复杂几何结构，为数据集蒸馏提供了更有效的几何感知框架，显著提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 牙齿标志点检测在现代临床正畸中至关重要，但牙齿几何结构复杂且个体差异大，需要先进技术来实现精确可靠的3D检测。

Method: 通过组织3DTeethLand挑战赛，与国际医学图像计算和计算机辅助干预会议合作，征集牙齿3D扫描标志点检测算法，并创建首个公开数据集。

Result: 建立了首个公开的3D牙齿标志点检测数据集，为评估该领域最先进方法提供了宝贵资源，并鼓励社区提供方法学贡献。

Conclusion: 3DTeethLand挑战赛为解决具有重要临床意义的牙齿标志点检测问题提供了平台和数据集，推动了该领域的技术发展。
...
TL;DR: GeoDiffMM：基于扩散模型的拉格朗日视频运动放大框架，利用光流作为几何线索，通过无噪声光流增强策略和条件扩散过程实现结构一致的运动放大，有效抑制光子噪声。


<details>
  <summary>Details</summary>
Motivation: 现有欧拉方法通过解耦表示学习处理放大引起的噪声，但在运动位移非常小时仍难以将光子噪声与真实微运动分离，需要更有效的噪声抑制和结构保持方法。

Method: 1) 无噪声光流增强策略：合成无光子噪声的多样化非刚性运动场作为监督；2) 扩散运动放大器：在去噪过程中以光流作为几何先验和可学习放大因子控制幅度；3) 基于光流的视频合成：将放大后的运动高保真映射回图像域。

Result: 在真实和合成数据集上的广泛实验表明，GeoDiffMM优于现有最先进方法，显著改善了运动放大效果，特别是在抑制噪声和保持结构一致性方面表现优异。

Conclusion: GeoDiffMM通过将扩散模型与几何条件光流相结合，实现了结构一致的运动放大，有效解决了小位移运动放大中的光子噪声问题，为视频运动放大提供了新框架。
...
TL;DR: 提出了一种基于四元数的低秩支持矩阵机（LSQMM）用于彩色图像分类，将RGB通道作为纯四元数处理，通过四元数代数保持通道间的内在耦合关系，并引入四元数核范数正则化来促进低秩结构。


<details>
  <summary>Details</summary>
Motivation: 传统彩色图像分类方法将特征表示为向量、矩阵或三阶张量，但未能有效保持RGB通道间的内在耦合关系。四元数在图像恢复和去噪任务中已显示出优势，因此探索其在分类任务中的应用潜力。

Method: 提出低秩支持四元数矩阵机（LSQMM），将RGB通道作为纯四元数处理，在铰链损失中添加四元数核范数正则化项以促进低秩结构，并设计基于ADMM的迭代算法来求解四元数优化模型。

Result: 在多个彩色图像分类数据集上的实验表明，LSQMM在分类准确率、鲁棒性和计算效率方面优于支持向量机、支持矩阵机和支持张量机等现有方法。

Conclusion: LSQMM通过四元数代数有效保持了彩色图像通道间的耦合关系，结合低秩正则化提升了分类性能，为彩色图像分类提供了一种新颖有效的方法。
...
TL;DR: 该研究系统分析了Glaze和Nightshade等图像保护机制，发现它们通过结构化、低熵的扰动在特征层面变形图像，而非引入语义错位，解释了为何保护信号视觉上微妙但可检测。


<details>
  <summary>Details</summary>
Motivation: 虽然Glaze和Nightshade等图像保护机制在经验上有效，但其内部结构、可检测性和表征行为仍不清楚。研究旨在通过可解释AI分析来理解这些保护机制的工作原理。

Method: 采用统一框架整合白盒特征空间检查和黑盒信号级探测，包括潜在空间聚类、特征通道激活分析、基于遮挡的空间敏感性映射和频域特征分析。

Result: 保护机制是结构化、低熵的扰动，与底层图像内容紧密耦合；保护图像保留了内容驱动的特征组织，带有保护特定的子结构；可检测性受扰动熵、空间部署和频率对齐的交互效应控制；Glaze和Nightshade沿主导图像对齐频率轴重新分配能量而非引入扩散噪声。

Conclusion: 当代图像保护通过结构化特征级变形而非语义错位运作，解释了保护信号视觉微妙但可检测的原因。该研究推进了对抗性图像保护的可解释性，并为生成AI系统的未来防御和检测策略设计提供了信息。
...
TL;DR: PointDico是一种新颖的3D表示学习方法，通过结合扩散模型和对比模型的优势，解决了现有方法在无序、不均匀密度的3D点云数据上的局限性，在多个基准测试中达到了新的最先进性能。
...
Result: PointDico在3D表示学习中达到了新的最先进性能：在ScanObjectNN上获得94.32%的准确率，在ShapeNetPart上获得86.5%的实例mIoU。

Conclusion: 通过结合扩散模型和对比模型的优势，PointDico成功解决了3D点云表示学习的挑战，为无序、不均匀密度的3D数据提供了有效的自监督表示学习方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法在处理遥感图像融合时难以适应特征表示中的区域异质性，现有自适应卷积方法存在计算成本过高和捕捉异质区域能力有限的问题。

Method: 提出Bi^2MAC方法，设计轻量级模块生成软硬掩码，分别用于初步调制输入特征和引导不同类型区域进入不同处理分支。冗余特征进入紧凑分支进行低成本全局处理，异质特征进入专注分支投入更多计算资源进行细粒度建模。

Result: 在多个基准数据集上的实验表明，Bi^2MAC实现了最先进的性能，同时显著降低了训练时间和参数数量，在自适应卷积模型中计算成本最小。

Conclusion: Bi^2MAC通过智能分配计算资源，有效利用不同类型区域的信息，在遥感图像融合任务中取得了计算效率和性能的良好平衡。
...
Method: 提出混合溅射机制，包括反射烘焙高斯追踪（将视角相关反射烘焙到高斯基元内）、基于瓦片的高斯溅射渲染反射，以及反射敏感高斯剪枝来减少模型大小

Result: 在Ref-NeRF和NeRF-Casting复杂反射场景上实现约7倍渲染加速，高斯基元数量比类似基于光线追踪的高斯溅射基线减少4倍

Conclusion: HybridSplat成为复杂反射场景的新SOTA方法，在保持反射渲染质量的同时显著提升渲染速度和减少内存占用
...
Method: 提出DINO-BOLDNet框架，整合冻结的自监督DINOv3编码器和轻量级可训练解码器。使用DINOv3提取切片内结构表示，通过独立的切片注意力模块融合相邻切片上下文信息，多尺度生成解码器恢复细粒度功能对比度，并采用基于DINO的感知损失确保预测与真实BOLD在transformer特征空间中的结构纹理一致性。

Result: 在248名受试者的临床数据集上，DINO-BOLDNet在PSNR和MS-SSIM指标上均超越了条件GAN基线，证明了该方法的有效性。

Conclusion: 这是首个能够直接从T1w图像生成平均BOLD图像的框架，展示了自监督transformer引导在结构到功能映射方面的潜力，为BOLD信息恢复提供了新的技术途径。
...
TL;DR: 提出TrackingWorld方法，从单目视频中实现世界坐标系下的密集3D跟踪，解决了现有方法无法分离相机运动与前景动态运动、无法跟踪新出现动态物体的问题。
...
Conclusion: TrackingWorld方法成功解决了现有单目3D跟踪的两个关键限制，能够有效分离相机运动与动态运动，并能跟踪新出现的动态物体，为世界坐标系下的密集3D跟踪提供了有效解决方案。
...
Method: 提出SCU-CGAN模型，集成U-Net架构、CBAM注意力机制和额外判别器，从非火灾图像生成逼真的火灾图像

Result: SCU-CGAN在图像质量评估中优于现有模型，KID分数比CycleGAN提高41.5%。增强后的数据集显著提升了火灾检测模型精度，YOLOv5 nano模型的mAP@0.5:0.95指标提高了56.5%

Conclusion: SCU-CGAN能有效生成高质量火灾图像，数据增强方法显著提升火灾检测模型性能，无需改变模型结构，为火灾检测领域提供了有效的数据增强解决方案
...
TL;DR: 该论文发现多模态大语言模型中视觉和文本token的范数差异导致"不对称更新动态"，提出在视觉投影器后添加LayerNorm层进行范数对齐，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型中普遍采用的Pre-Norm架构存在一个关键缺陷：视觉token的高范数与文本token的低范数之间存在严重不平衡，这种范数差异导致视觉token更新缓慢，阻碍有效的跨模态特征融合。

Method: 首先通过理论分析证明范数不平衡会导致"不对称更新动态"，然后提出简单有效的解决方案：在视觉投影器后插入一个精心初始化的LayerNorm层，强制实现视觉和文本token的范数对齐。

Result: 在LLaVA-1.5架构上的实验表明，该干预措施不仅在广泛的多模态基准测试中带来显著性能提升，而且在纯文本评估（如MMLU）上也表现出改进，表明解决架构不平衡能产生更全面的模型能力。

Conclusion: 多模态大语言模型中的范数差异是一个普遍存在的关键问题，通过简单的LayerNorm层插入实现范数对齐，可以有效解决不对称更新动态，显著提升模型的跨模态融合能力和整体性能。
...
TL;DR: 提出一种在复杂光照条件下同时进行图像增强和噪声抑制的新框架，结合梯度域加权引导滤波、Retinex分解和多曝光融合等技术，在真实数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在挑战性光照条件下，捕获的图像常出现各种退化问题，影响视觉应用性能。现有方法要么显著放大固有噪声，要么只在特定光照条件下有效，需要一种能在复杂光照下同时增强图像并抑制噪声的解决方案。
...
Conclusion: 提出的框架能有效处理复杂光照条件下的图像增强和噪声抑制问题，在真实应用场景中表现出优越性能。
...
TL;DR: 该研究探讨面部美化处理对生物识别系统的影响，提出利用美颜评估算法来改进面部美化检测，在未知攻击算法情况下实现了1.1%的D-EER检测性能。


<details>
  <summary>Details</summary>
Motivation: 面部美化处理在社交媒体和广告中广泛使用，但当美化后的图像作为生物识别样本时，会对人脸识别系统构成挑战。现有研究表明面部美化会干扰人脸识别，因此需要有效的美化检测方法。

Method: 研究分析美颜评估算法在美化图像中的变化，评估基于人工智能的特征提取方法，探索是否可以利用面部美颜特征来提高美化检测率。

Result: 在攻击美化算法未知的场景下，单张图像检测达到了1.1%的D-EER（检测等错误率），显示出良好的检测性能。

Conclusion: 面部美颜特征可以有效用于改进面部美化检测，在未知攻击算法的情况下仍能实现高精度的检测性能，为生物识别系统的安全性提供了重要保障。
...
<details>
  <summary>Details</summary>
Motivation: 随着电子监控系统的广泛采用，收集的视频数据量超过了人工审查的可行范围，需要自动化解决方案来处理鱼类重识别任务。

Method: 使用AutoFish数据集（模拟电子监控系统的传送带场景），开发了包含硬三元组挖掘和自定义图像变换管道的深度学习流水线，比较了Vision Transformer的Swin-T架构和CNN的ResNet-50架构。

Result: Swin-T架构在mAP@k和Rank-1准确率上显著优于ResNet-50，达到41.65% mAP@k和90.43% Rank-1准确率。主要挑战是区分同物种的视觉相似个体，视角不一致比部分遮挡影响更大。

Conclusion: 优化的深度学习流水线结合硬三元组挖掘和自定义图像变换能有效提升鱼类重识别性能，Vision Transformer架构在区分相似鱼类方面优于传统CNN架构，为电子监控系统的自动化分析提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像的人体网格恢复方法在视频应用中存在时间不一致性和遮挡性能下降的问题，需要不依赖额外训练就能解决这些挑战的方案

Method: 提出训练免费框架：1）使用可提示视频分割模型生成身份一致的面具片段；2）遮挡感知模块恢复缺失区域；3）精炼后的面具片段指导SAM 3D Body生成一致的全身体网格轨迹；4）基于填充的并行策略支持高效多人推理

Result: 实验结果表明，SAM-Body4D在具有挑战性的野外视频中实现了改进的时间稳定性和鲁棒性，且无需任何重新训练

Conclusion: 该方法通过利用视频中固有的人类连续性，解决了时间不一致和遮挡问题，为视频中的人体网格恢复提供了有效的训练免费解决方案
...
TL;DR: OneClip-RAG：一种基于单视频片段检索增强的高效多模态大语言模型长视频理解框架，通过查询引导的视频分块算法和渐进式训练，显著提升长视频处理性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs由于内存开销过大，只能处理有限帧数的视频，无法有效处理长视频内容。需要一种既能保持知识完整性又能保证语义连贯性的长视频理解方案。

Method: 提出OneClip-RAG框架，包含：1）查询引导的视频分块算法，统一分块和跨模态检索；2）SynLongVideo数据集构建；3）渐进式训练策略；4）可插拔到现有MLLMs的架构。

Result: 实验表明：1）显著提升MLLMs性能，如InternLV2 8B和Qwen2-VL 7B在MLVU上达到GPT-4o水平；2）高效处理长视频，LLaVA-Video可在单张4090 GPU上2.2分钟内处理1小时视频。

Conclusion: OneClip-RAG是一种高效的长视频理解解决方案，通过检索增强范式解决了MLLMs内存限制问题，在性能和效率上都取得了显著提升，具有实际应用价值。
...
TL;DR: 提出了一种基于深度图的6D位姿估计方法，通过多视角深度图融合和稀疏TSDF表示，结合场景自适应注意力机制和密度感知稀疏Transformer，在密集工业分拣场景中实现高精度位姿估计。


<details>
  <summary>Details</summary>
Motivation: 工业分拣环境中存在严重遮挡、反射和纹理缺失等问题，传统方法难以准确恢复6D位姿，需要一种能够处理密集杂乱场景的鲁棒性方法。

Method: 采用多视角深度图融合为精细3D点云或稀疏TSDF表示，引入场景自适应注意力机制引导计算聚焦前景区域，提出密度感知稀疏Transformer处理遮挡和非均匀数据分布，通过逐体素投票策略预测任意数量目标物体的6D位姿。

Result: 在IPD和MV-YCB多视角数据集上验证，在密集工业分拣和家庭分拣场景中表现出色，实现了高分辨率的体积表示，能够捕捉精细几何细节。

Conclusion: 该方法通过完全稀疏的操作框架，实现了在密集杂乱场景中的高效6D位姿估计，为近距离机器人应用提供了有效的解决方案，在工业分拣领域具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 手术分割对于场景理解至关重要，但面临标注稀缺和不同手术间语义不一致的挑战。现有方法通常只是对自然基础模型进行有限监督的微调，仅作为领域适配器而非真正的手术基础模型，难以泛化到多样化的手术目标。

Method: 提出分层概念演化预训练范式：1）通过分层掩码解码器和父子查询嵌入建立腹腔镜概念层次结构，统一不同实体（解剖结构、组织、器械）为可扩展的知识结构；2）提出置信度驱动的演化标注，基于分层一致性迭代生成和筛选伪标签，逐步将可靠样本纳入训练，构建了包含114K图像-掩码对的大规模基准数据集LapBench-114K。

Result: 大量实验表明，LapFM显著优于最先进的方法，为通用腹腔镜分割中的粒度自适应泛化建立了新标准。

Conclusion: LapFM是一个专门设计用于腹腔镜手术分割的基础模型，通过创新的分层概念演化预训练方法，能够从大规模无标注手术图像中演化出强大的分割能力，解决了现有方法的泛化局限性问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的快照多光谱成像系统虽然能比传统RGB传感器捕获更丰富的频谱信息，但大多数现有方法将色彩校正流程分为多个独立阶段，往往在早期就丢弃了多光谱数据，导致信息损失。

Method: 提出一个统一的端到端学习框架，将整个色彩校正流程集成在单一模型中，联合利用高分辨率RGB传感器和辅助低分辨率多光谱传感器的数据。通过重构两种不同的最先进的图像到图像架构来展示框架的灵活性和通用性。构建专用数据集用于训练和评估，通过聚合和重新利用公开可用的光谱数据集，并在多种RGB相机灵敏度下进行渲染。

Result: 大量实验表明，该方法显著提高了色彩准确性和稳定性，与RGB-only和多光谱驱动基线相比，误差减少了高达50%。

Conclusion: 提出的统一端到端学习框架能够有效联合利用RGB和多光谱传感器数据，显著提升色彩校正性能，展示了框架的灵活性和通用性，为消费级和移动设备的色彩校正提供了有效解决方案。
...
### [43] [ContextDrag: Precise Drag-Based Image Editing via Context-Preserving Token Injection and Position-Consistent Attention](https://arxiv.org/abs/2512.08477)
*Huiguo He,Pengyu Yan,Ziqi Yi,Weizhi Zhong,Zheng Liu,Yejun Tang,Huan Yang,Kun Gai,Guanbin Li,Lianwen Jin*

Main category: cs.CV

TL;DR: ContextDrag是一种基于拖拽的图像编辑新范式，通过利用编辑模型的强大上下文建模能力，结合VAE编码的参考图像特征，无需微调或反转即可实现高保真度的图像编辑。
...
Method: 1. 引入Context-preserving Token Injection (CTI)，通过Latent-space Reverse Mapping (LRM)算法将无噪声的参考特征注入到正确目标位置；2. 采用Position-Consistent Attention (PCA)，对参考标记进行位置重编码并应用重叠感知掩码以消除不相关参考特征的干扰。
...
Method: 提出一个框架，将子模子集选择与分层梯度不确定性估计相结合。通过自适应权重扰动估计不确定性，并利用这些估计指导子模优化，确保多样化和信息丰富的子集选择。该方法不需要额外训练或辅助模型。

Result: 实证评估表明，该框架不仅缓解了现有方法在OOD场景下的弱点，还在ID设置中带来了改进。能够产生更稳健、保真度更高的解释。
...
Method: 基于WebGPU构建高效渲染器，支持每帧ONNX推理；引入标准化的高斯生成器合约，支持插件式算法动态生成或更新高斯；提供three.js插件库和TypeScript API。
...
TL;DR: 提出了一种轻量级SAM+CSRT足球运动员跟踪方法，结合SAM精确初始化、CSRT跟踪器和基于球衣颜色的外观模型，在资源受限条件下实现稳定跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 足球运动员跟踪面临频繁遮挡、相似外观和拥挤场景中快速运动等挑战。现有方法在资源受限环境下难以平衡精度与效率，需要结合深度学习和传统跟踪器的优势。

Method: 提出团队感知跟踪系统：1) 使用SAM进行精确初始化；2) CSRT跟踪器处理连续跟踪；3) 基于HSV直方图的球衣颜色外观模型进行重识别；4) 针对遮挡恢复设计专门机制。

Result: 在足球视频序列上达到7.6-7.7 FPS，内存使用稳定(~1880 MB)。轻度遮挡下保持100%跟踪成功率，拥挤禁区场景(5+球员)达90%。外观重识别恢复50%重度遮挡，但长期离帧重获取成功率仅8.66%。

Conclusion: SAM+CSRT组合在连续可见性场景表现稳定，但需要更强重识别机制处理长期离帧。结果为资源受限下足球跟踪系统部署提供实用指南，传统跟踪器结合领域特定线索可有效提升性能。
...
TL;DR: ReasonBreak是一个针对多模态大推理模型的对抗性框架，通过概念感知扰动破坏地理定位推理，相比现有方法显著提升隐私保护效果。


<details>
  <summary>Details</summary>
Motivation: 多模态大推理模型（MLRMs）通过分层思维链推理从个人图像中推断精确地理位置，对隐私构成严重威胁。现有的隐私保护技术主要针对感知模型设计，无法有效对抗MLRMs的复杂多步推理过程。

Method: 提出了ReasonBreak对抗框架，基于关键洞察：有效破坏地理推理需要与概念层次结构对齐的扰动，而非均匀噪声。该框架战略性地针对推理链中的关键概念依赖关系，生成使特定推理步骤失效并级联影响后续推理阶段的扰动。同时贡献了GeoPrivacy-6K数据集，包含6,341张超高分辨率图像和分层概念标注。

Result: 在七个最先进的MLRMs（包括GPT-o3、GPT-5、Gemini 2.5 Pro）上的广泛评估显示，ReasonBreak在区域级保护方面提升了14.4%（33.8% vs 19.4%），在街区级保护方面几乎翻倍（33.5% vs 16.8%）。

Conclusion: 这项工作为对抗基于推理的隐私威胁建立了新范式，证明了针对概念层次结构设计的对抗性扰动在保护地理隐私方面的有效性。
...
TL;DR: 提出PCI框架分析扩散模型中概念形成的动态过程，通过概念插入成功率研究噪声何时转化为特定概念并锁定去噪轨迹
...
Method: 提出PCI（Prompt-Conditioned Intervention）框架，这是一个无需训练、模型无关的方法。核心思想是分析概念插入成功率（CIS），定义为在给定时间步插入的概念在最终图像中被保留和反映的概率

Result: 应用于多个最先进的文本到图像扩散模型和广泛的概念分类，PCI揭示了不同扩散模型之间的多样化时间行为，轨迹的某些阶段对特定概念更有利，即使在同一概念类型内也是如此

Conclusion: 这些发现为文本驱动的图像编辑提供了可操作的见解，突出了何时干预最有效，无需访问模型内部或进行训练，相比强基线实现了语义准确性和内容保留的平衡，产生定量更强的编辑效果
...
TL;DR: 提出自动对齐多摄像头手术视频的方法，通过检测照明系统移动、重新对齐帧并选择遮挡最少的摄像头，生成固定视角的手术区域视频


<details>
  <summary>Details</summary>
Motivation: 手术视频对教育和研究很重要，但术中医生经常遮挡摄像头视野。现有方法使用多个摄像头但需要手动对齐，因为每次移动无影灯都会改变摄像头配置

Method: 识别照明系统移动的帧，重新对齐这些帧，选择遮挡最少的摄像头，生成固定视角的连续视频。还实现了多种合成选项供外科医生选择

Result: 用户研究表明，外科医生认为该方法生成的视频在确认手术区域和观看舒适度方面优于传统方法。视频质量也优于现有技术

Conclusion: 提出的方法能够自动对齐多摄像头手术视频，生成高质量、固定视角的手术区域视频，为手术教育和研究提供了更好的工具
...
Method: 提出分层相机初始化方案实现无需标定的相机间粗对齐，轻量级多相机束调整稳定轨迹，冗余消除的高斯采样策略和频率感知优化调度器减少高斯基元数量和优化迭代次数。

Result: 仅使用原始多相机视频流，在2分钟内重建数百米3D场景，实现了前所未有的速度、鲁棒性和保真度，支持实时性能的漂移自由轨迹估计。

Conclusion: 该框架首次实现了多相机阵列的实时3D高斯溅射重建，通过多相机融合、高效优化策略和冗余消除技术，在速度、覆盖范围和重建质量方面取得突破性进展。
...
TL;DR: CLIP模型在少样本学习中存在模板-样本相似性偏差，导致模型依赖模板而非真实类别对齐。本文提出使用空提示来捕获无偏模板特征，通过两阶段框架减少偏差，提高分类准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 研究发现CLIP模型中存在的模板-样本相似性偏差会导致模型过度依赖文本模板与图像的相似性，而非真实的样本-类别对齐关系，从而降低了少样本分类的准确性和鲁棒性。

Method: 提出两阶段框架：1）预训练阶段使用空提示（不含类别信息的文本输入）来揭示和减少CLIP编码器中的模板诱导偏差；2）少样本微调阶段通过偏差校准损失强制图像与其正确类别对齐，确保模型关注相关视觉特征。

Result: 在多个基准测试上的实验表明，该模板校正方法显著减少了由TSS引起的性能波动，获得了更高的分类准确性和更强的鲁棒性。

Conclusion: 通过使用空提示和偏差校准损失来解耦CLIP中的模板偏差，能够有效提高模型在少样本学习任务中的性能和稳定性。
...
TL;DR: 本文提出了一种仅使用单目RGB视觉和自然语言指令的无人机视觉语言导航统一框架，通过多任务学习联合优化空间感知、轨迹推理和动作预测，在单目RGB设置下取得了接近全景RGB-D方法的性能。
...
Method: 1. 将导航建模为下一个token预测问题；2. 通过提示引导的多任务学习联合优化空间感知、轨迹推理和动作预测；3. 提出关键帧选择策略以减少视觉冗余；4. 引入动作合并和标签重加权机制缓解长尾监督不平衡问题。

Result: 在Aerial VLN基准测试上的广泛实验验证了方法的有效性。在具有挑战性的单目RGB-only设置下，模型在已见和未见环境中都取得了强劲结果，显著优于现有的RGB-only基线方法，并缩小了与最先进全景RGB-D对应方法的性能差距。

Conclusion: 本文提出的统一框架仅使用单目RGB观测和自然语言指令就能实现有效的空中视觉语言导航，通过多任务学习和关键优化策略，在减少系统复杂度的同时保持了导航性能，为轻量级无人机的实际部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管条件扩散模型在文本到图像生成方面取得了成功，但错位和幻觉问题仍然常见。现有的对齐检测方法需要在完整生成后才能评估，计算成本高且无法实时干预。本研究假设文本/图像错位可以在去噪过程中早期检测，从而实现实时对齐评估。

Method: 提出NoisyCLIP方法，在反向扩散过程中使用双编码器测量噪声潜在空间中的语义对齐。这是首个探索和基准测试在图像生成过程中提示到潜在空间错位检测的方法，能够在去噪早期阶段评估对齐质量。

Result: NoisyCLIP在Best-of-N设置中减少了50%的计算成本，同时达到了98%的CLIP对齐性能。该方法实现了生成过程中的实时对齐评估，在不牺牲语义保真度的情况下显著降低了成本。

Conclusion: NoisyCLIP证明了文本/图像错位可以在去噪过程中早期检测，为实时对齐评估提供了有效解决方案，显著降低了计算成本，同时保持了高质量的对齐性能，为改进条件扩散模型的生成质量提供了新途径。
...
TL;DR: OCCDiff：一种基于潜在扩散和函数自编码器的建筑重建方法，通过占用函数空间处理不同分辨率的LiDAR点云数据


<details>
  <summary>Details</summary>
Motivation: 解决从LiDAR点云重建建筑时面临的主要挑战：在不同点密度和噪声干扰下准确捕捉建筑表面。需要灵活获取高质量3D建筑轮廓以适应不同分辨率需求。
...
Conclusion: OCCDiff能够有效处理不同分辨率和噪声的LiDAR点云数据，生成高质量的建筑表面重建结果，为建筑重建任务提供了灵活且鲁棒的解决方案。
...
TL;DR: 该研究提出了一个基于分布的个体公平性评估框架，用于皮肤病变分类，将肤色视为连续属性而非分类标签，通过核密度估计建模分布，比较12种统计距离度量，并提出基于距离的重新加权损失函数来纠正少数肤色的代表性不足问题。


<details>
  <summary>Details</summary>
Motivation: 肤色历来是歧视的焦点，但医学影像中的机器学习公平性研究通常依赖粗粒度的子组分类，忽视了个体层面的差异。这种基于群体的方法可能会掩盖子组内异常值面临的偏见。
...
TL;DR: 提出自精炼扩散框架，利用可解释AI技术检测生成图像中的伪影和不真实区域，通过前向过程增强噪声、反向过程聚焦修复来提升图像质量
...
Method: 提出自精炼扩散框架，使用基于可解释AI的缺陷高亮器生成缺陷激活图（FAMs），识别伪影和不真实区域。在前向过程中增强缺陷区域的噪声，在反向过程中聚焦修复这些区域。
...
TL;DR: MVP框架通过多视角推理解决GUI坐标预测不稳定性问题，无需训练即可提升模型性能


<details>
  <summary>Details</summary>
Motivation: 现有GUI坐标预测模型存在显著的不稳定性问题，微小的视觉扰动（如裁剪几个像素）就会导致预测结果在正确与错误之间剧烈波动，这种不稳定性严重影响了模型性能，特别是对于高分辨率和小UI元素的样本

Method: 提出MVP（多视角预测）框架，包含两个组件：1）注意力引导的视角提议，基于指令到图像的注意力分数生成多样化的裁剪视图；2）多坐标聚类，通过选择最密集空间簇的质心来集成预测结果
...
TL;DR: 提出一个统一的多模态油画生成与编辑框架，通过参考图像、手绘草图和自然语言提示实现精确控制，保持统一的油画风格。
...
Result: 广泛的实验表明，该交互系统能够在保持油画艺术品质的同时实现细粒度编辑，在风格化油画生成和编辑中实现了前所未有的想象力实现水平。

Conclusion: 提出的统一多模态框架通过三个关键技术进展实现了交互式油画创作，解决了数据稀缺问题并确保了风格一致性，为油画数字生成和编辑提供了创新解决方案。
...
TL;DR: 本文提出MatteViT框架，通过空间和频域信息消除文档阴影，同时保留文本边缘等高频细节，在公开基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 文档阴影去除对提高数字化文档清晰度至关重要。阴影常常会模糊或扭曲文本边缘和线条等高频细节结构，因此保留这些细节是文档阴影去除的关键挑战。

Method: 提出MatteViT框架，采用两种保护策略：1) 轻量级高频放大模块(HFAM)，分解并自适应放大高频分量；2) 基于连续亮度的阴影蒙版，使用自定义蒙版数据集和阴影蒙版生成器提供精确的空间指导。

Result: 在RDD和Kligler等公开基准测试中达到最先进性能，为实际文档阴影去除提供稳健实用解决方案。在下游任务如光学字符识别中能更好地保留文本级细节，提高识别性能。

Conclusion: MatteViT框架通过结合空间和频域信息，有效消除文档阴影同时保留精细结构细节，在文档阴影去除任务中表现出色，具有实际应用价值。
...
TL;DR: Photo3D是一个利用GPT-4o-Image生成图像数据驱动的高质量3D生成框架，通过结构对齐的多视角合成和细节增强技术，实现了照片级真实的3D生成效果。


<details>
  <summary>Details</summary>
Motivation: 当前3D原生生成器在几何结构上取得了进展，但在实现真实外观方面仍然不足。主要障碍在于缺乏多样化和高质量的具有丰富纹理细节的真实世界3D资产，因为捕获此类数据存在固有困难：场景尺度多样、物体非刚性运动以及3D扫描仪精度有限。

Method: 1. 利用GPT-4o-Image模型生成图像数据；2. 设计结构对齐的多视角合成流程，解决生成图像因缺乏多视角一致性而导致的3D结构扭曲问题；3. 构建细节增强的多视角数据集与3D几何配对；4. 提出真实细节增强方案，利用感知特征适应和语义结构匹配，在保持3D原生几何结构一致性的同时增强外观真实细节；5. 为不同3D原生生成器设计专门的训练策略，支持几何-纹理耦合和解耦的生成范式。

Result: 实验表明，Photo3D能够很好地泛化到不同的3D原生生成范式，并在照片级真实3D生成方面达到了最先进的性能。

Conclusion: Photo3D框架通过利用生成图像数据和创新的结构对齐、细节增强技术，成功解决了当前3D生成中外观真实性的瓶颈问题，为照片级真实3D生成提供了有效的解决方案。
...
TL;DR: Fast-ARDiff提出了一种统一的AR-扩散混合框架，通过熵感知推测解码和动态调度器，同时加速AR生成和扩散去噪过程，显著降低混合模型的推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有的自回归-扩散混合模型结合了AR的结构化建模能力和扩散模型的光照真实合成能力，但由于需要顺序AR生成和迭代去噪，导致推理延迟过高。需要解决这一瓶颈问题。

Method: 1. 熵感知推测策略：鼓励草稿模型生成与目标模型熵特征对齐的高熵表示，缓解熵不匹配和因草稿模型过度自信导致的高拒绝率。2. 动态调度器：将扩散解码集成到端到端框架中，优先优化AR部分以指导后续扩散步骤。3. 联合蒸馏框架：结合轨迹匹配和分布匹配，确保在极少数步骤下实现稳定训练和高质量合成。4. 推理时使用AR模块的浅层特征熵预过滤低熵草稿，避免冗余计算。

Result: 在ImageNet 256×256上，TransDiff实现了4.3倍无损加速；在文本条件生成任务上，NextStep-1实现了3倍加速，达到了最先进的加速性能。

Conclusion: Fast-ARDiff通过联合优化AR和扩散组件，有效解决了混合模型的延迟瓶颈，在保持生成质量的同时显著提升了推理速度，为AR-扩散混合模型的实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在领域变化时性能下降，或需要大量计算资源进行微调。需要一种更高效、鲁棒的适应方法来解决跨模态推理中的领域适应问题。

Method: 提出训练免费的双曲适配器(T-DHA)，在双曲空间而非传统欧几里得空间中建模视觉-语言关系的层次化树结构。利用双曲空间的指数体积增长特性，结合Poincaré球模型和负学习，以更少的特征维度实现更准确、鲁棒的分类。

Result: 在多个数据集上的实验结果表明，T-DHA方法在少样本图像识别和领域泛化任务中显著优于现有最先进方法。

Conclusion: 双曲空间建模视觉-语言层次关系是一种有效的策略，T-DHA方法无需训练即可提升视觉语言模型的适应能力，在计算效率和性能方面都有显著优势。
...
TL;DR: InfiniteVL是一种线性复杂度视觉语言模型架构，结合滑动窗口注意力与Gated DeltaNet，在有限训练数据下达到与Transformer模型相当的性能，同时实现3.6倍推理加速和恒定内存占用。
...
Method: 提出InfiniteVL架构，将滑动窗口注意力与Gated DeltaNet相结合，实现线性复杂度。采用三阶段训练策略：蒸馏预训练、指令调优和长序列SFT。使用不到领先VLM所需训练数据的2%。

Result: InfiniteVL不仅显著优于之前的线性复杂度VLM，还能与领先的基于Transformer的VLM性能相当，同时保持有效的长期记忆保留。相比使用FlashAttention-2加速的类似规模Transformer VLM，实现超过3.6倍的推理加速，保持恒定延迟和内存占用。在流式视频理解场景中，维持稳定的24 FPS实时预填充速度。

Conclusion: InfiniteVL成功解决了窗口注意力和线性注意力的局限性，在有限训练数据下实现了与Transformer模型竞争的性能，同时提供线性复杂度的优势，为高效视觉语言建模提供了有前景的解决方案。
...
Result: 在NOCAPS和MS-COCO两个文本-图像数据集上评估重建性能。与DDIM反演和其他基于固定点迭代的方法相比，该方法在不增加额外迭代或训练的情况下，在重建任务中取得了一致且更优的性能

Conclusion: 提出的无需迭代的固定点估计器解决了传统固定点迭代方法的高计算成本和超参数选择问题，通过误差近似方法实现了高效且准确的扩散反演
...
### [65] [SSCATeR: Sparse Scatter-Based Convolution Algorithm with Temporal Data Recycling for Real-Time 3D Object Detection in LiDAR Point Clouds](https://arxiv.org/abs/2512.08557)
*Alexander Dow,Manduhu Manduhu,Matheus Santos,Ben Bartlett,Gerard Dooly,James Riordan*

Main category: cs.CV

TL;DR: 提出SSCATeR算法，利用LiDAR连续扫描特性，通过时间窗口和卷积结果存储实现数据重用，仅处理点云变化区域，在保持准确性的同时将处理时间减少6.61倍。


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR目标检测方法需要对整个点云进行卷积计算，计算量大且效率低。作者希望利用LiDAR连续扫描的特性，通过只处理变化区域来减少不必要的计算。

Method: 采用滑动时间窗口和短步长策略，存储卷积结果以便在后续帧中重用。扩展了之前的散射卷积方法，提出SSCATeR算法，将LiDAR数据视为连续流，仅处理点云中变化的部分。

Result: 在保持与传统稀疏卷积相同特征图输出的前提下，处理时间最多减少6.61倍。测试结果显示特征图与传统方法完全相同，但计算效率大幅提升。

Conclusion: SSCATeR算法通过利用LiDAR数据的时空特性和数据重用机制，实现了高效的目标检测，在保持准确性的同时显著降低了计算成本。
...
TL;DR: VALOR是一个无需标注的视觉推理训练框架，通过AI验证器（LLM和VLM）提升推理和视觉定位能力，在多种空间推理任务中超越开源和专有模型。


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法存在局限性：语言链式思维方法需要大规模监督数据，程序合成方法存在逻辑缺陷和定位错误。需要一种既能提升推理能力又能改善视觉定位的标注免费方法。

Method: 提出AI验证器框架：LLM验证器通过强化学习优化推理，VLM验证器通过自动硬负例挖掘增强视觉定位，无需真实标注。结合语言模型的推理分解能力和视觉专家的定位能力。

Result: 在多种空间推理任务中，该方法提升了视觉推理能力，超越了开源和专有模型。改进的视觉定位模型进一步优于最近的纯文本视觉推理方法。

Conclusion: VALOR框架通过AI验证器实现了无需标注的视觉推理训练，有效结合了语言推理和视觉定位的优势，在多个任务中取得了优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 人类大脑如何表示视觉概念以及在哪些脑区编码这些表征仍然是一个长期挑战。现有研究通常规模小、依赖人工检查、关注特定区域和属性，且缺乏系统验证，需要更系统的大规模方法来理解复杂的脑信号和广阔的概念空间。

Method: 方法包含两个主要阶段：首先通过无监督数据驱动分解方法发现fMRI活动中的候选可解释模式；然后通过识别最能激发每个模式的自然图像集，并生成描述其共享视觉意义的自然语言描述来解释每个模式。引入自动化流程测试多个候选解释，分配定量可靠性分数，并为每个体素模式选择最一致的描述。

Result: 该框架揭示了数千个可解释模式，涵盖许多不同的视觉概念，包括以前未报告的细粒度表征。

Conclusion: 该研究提出了一个可扩展的自动化框架，能够系统性地发现和解释人类大脑皮层中的视觉表征，为理解大脑如何编码视觉概念提供了新的大规模方法。
...
TL;DR: Astra是一个交互式通用世界模型，能够为自动驾驶、机器人抓取等多样化场景生成真实世界的未来预测，支持精确的动作交互控制，实现交互式、一致且通用的长期视频预测。
...
<details>
  <summary>Details</summary>
Motivation: 现有连续测试时适应方法通常依赖固定或批处理级别的阈值，无法考虑不同类别和实例之间的难度差异，这在需要密集多类别预测的语义分割中尤为成问题。

Method: 提出了一种自适应调整伪标签的方法，反映每个图像内的置信度分布，并动态平衡学习，重点关注受域偏移影响最大的类别。

Result: 在八个CTTA和TTA场景（包括合成到真实和长期偏移）的广泛实验中，该方法始终优于最先进技术，为演化条件下的语义分割设定了新标准。

Conclusion: 这种细粒度的、类别和实例感知的适应方法能产生更可靠的监督，减轻连续适应过程中的错误积累，显著提升了语义分割在演化条件下的性能。
...
<details>
  <summary>Details</summary>
Motivation: 肿瘤微环境作为预后生物标志物的潜力巨大，但现有分析方法未能充分捕捉不同细胞类型间的复杂相互作用，需要更有效的方法来利用TME特征进行患者风险分层。

Method: 提出HiGINE分层图方法：1) 编码细胞邻域中的局部和全局相互关系；2) 整合细胞类型和形态学信息；3) 通过多模态融合将癌症分期与mIF衍生特征聚合；4) 预测患者生存期（短期vs长期）。

Result: 在两个公共数据集上验证显示：1) 改善了风险分层性能；2) 具有鲁棒性；3) 展现出良好的泛化能力；4) 多模态融合进一步提升了模型性能。

Conclusion: HiGINE能够有效利用肿瘤微环境的复杂相互作用信息，通过分层图表示和多模态融合，显著提高了肺癌患者生存预测和风险分层的准确性，具有临床应用潜力。
...
TL;DR: 使用深度学习技术改进和自动化花粉颗粒在光学和全息显微图像中的检测与分类，特别关注兽医细胞学应用。YOLOv8s用于检测，MobileNetV3L用于分类，在光学图像上取得良好效果，全息图像性能较低但通过数据增强技术得到改善。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决花粉颗粒在兽医细胞学中的自动化检测和分类问题，特别是探索如何将深度学习技术与低成本的无透镜数字全息显微设备结合，提高诊断效率和准确性。

Method: 采用YOLOv8s进行目标检测，MobileNetV3L进行分类任务。针对全息图像性能不足的问题，使用自动化标注和边界框区域扩大的数据集扩展技术来改善模型性能。

Result: 在光学图像上，检测达到91.3% mAP50，分类总体准确率97%。全息图像初始性能较低（检测2.49% mAP50，分类42%准确率），通过数据增强技术提升到检测13.3% mAP50，分类54%准确率。

Conclusion: 研究表明，至少在图像分类任务中，可以将深度学习技术与经济高效的无透镜数字全息显微设备结合使用，为兽医细胞学中的花粉分析提供自动化解决方案。
...
Method: 提出OpenMonoGS-SLAM框架，统一3D高斯溅射与开放集语义理解。利用视觉基础模型：MASt3R用于视觉几何，SAM和CLIP用于开放词汇语义。采用自监督学习目标，无需深度输入或3D语义标注。设计专门的内存机制管理高维语义特征，构建高斯语义特征图。

Result: 实验结果表明，该方法在封闭集和开放集分割任务中达到或超越现有基线性能，且不依赖深度图或语义标注等辅助传感器。

Conclusion: OpenMonoGS-SLAM是首个结合3D高斯溅射与开放集语义理解的单目SLAM框架，通过视觉基础模型和自监督学习，实现了开放世界环境下的准确相机跟踪、建图和语义理解，具有重要的实际应用价值。
...
TL;DR: 提出一种通过分析视频流模糊模式来估计深度的方法，结合光学设计和视觉语言模型，在手持拍摄场景中实现精确的深度估计和轨迹重建。


<details>
  <summary>Details</summary>
Motivation: 在无机械稳定器的情况下，相机拍摄时会产生旋转运动，导致透视模糊，特别是在长曝光场景中。这种模糊具有深度依赖性：不同空间位置的物体即使在同一成像设置下也会产生不同的模糊程度。

Method: 通过分析视频流的模糊模式来估计深度，结合光学设计算法。使用现成的视觉编码器和点跟踪器提取视频信息，通过窗口嵌入和多窗口聚合估计深度图，并使用视觉语言模型对光学算法的稀疏轨迹进行稠密化。

Result: 在多个深度数据集上的评估表明，该方法在大深度范围内表现出色，具有良好的泛化能力。在手持拍摄设置中，光学算法相对于真实轨迹具有更高的精度，稠密重建保持了较强的准确性。

Conclusion: 该方法通过分析视频模糊模式成功实现了深度估计，结合光学设计和视觉语言模型，在无稳定器的手持拍摄场景中取得了精确的深度估计和轨迹重建效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像生成模型内部过程不透明，限制了人类观察和干预，难以确保模型可靠性、安全性和控制性，且其非人类化工作流程难以解释。

Method: CoIG框架使用LLM将复杂提示分解为简单逐步指令，图像生成模型按此计划逐步生成和编辑图像，每个步骤专注于单个语义实体。

Result: CoIG显著增强了定量可监控性，同时在与基线模型相比时实现了具有竞争力的组合鲁棒性，并缓解了实体崩溃问题。

Conclusion: CoIG框架通过将图像生成重构为类似人类艺术创作的序列化语义过程，解决了当前图像生成模型的不透明性问题，提高了可监控性和控制性。
...
TL;DR: C-DIRA：一种用于车载摄像头驾驶员分心行为识别的轻量级框架，通过动态ROI路由和对抗学习实现高效、准确且鲁棒的识别


<details>
  <summary>Details</summary>
Motivation: 车载摄像头驾驶员分心行为识别需要在边缘设备上实时推理，但现有轻量级模型难以捕捉细粒度行为特征，ROI方法计算成本高，难以平衡效率与准确性

Method: 提出C-DIRA框架，结合显著性驱动的Top-K ROI池化和融合分类进行局部特征提取，动态ROI路由仅对高难度样本应用ROI推理，使用伪域标注和对抗学习学习领域不变特征

Result: 在State Farm Distracted Driver Detection Dataset上，C-DIRA在保持高准确率的同时显著减少FLOPs和延迟，在模糊和低光等视觉退化条件下表现鲁棒，在未见领域上性能稳定
...
Result: 在ImageNet-256上，Mname在40万步内达到FID 2.40的state-of-the-art性能，显著优于可比方法，实现更快的收敛和更优的生成质量。

Conclusion: Mname框架通过内存银行机制提供高质量负样本，消除了对外部预训练模型的依赖，不增加推理时的参数和计算成本，显著提升了去噪生成模型的训练效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的训练免费开放词汇语义分割方法在精确定位或复杂流程方面存在挑战，特别是在遥感场景中密集小目标众多。最近提出的SAM 3统一了分割和识别功能，本文探索其在遥感开放词汇语义分割中的应用潜力。

Method: 1. 实现掩码融合策略，结合SAM 3的语义分割头和Transformer解码器（实例头）输出，充分利用两个头的优势；2. 利用存在头的存在分数过滤场景中不存在的类别，减少误报。

Result: 在广泛的遥感数据集上进行评估，实验表明这种简单适配方法取得了有希望的性能，证明了SAM 3在遥感开放词汇语义分割中的潜力。

Conclusion: SAM 3在遥感开放词汇语义分割任务中具有良好潜力，无需训练即可获得有竞争力的性能，为遥感场景分析提供了新的解决方案。
...
TL;DR: 提出结合Blender 3D渲染与约束扩散模型的工作流，自动生成高质量、逼真的双孢蘑菇合成图像数据集，实现仅用合成数据训练即可在真实场景中达到SOTA分割性能。


<details>
  <summary>Details</summary>
Motivation: 工业蘑菇种植依赖计算机视觉进行监控和自动化收获，但开发准确的检测和分割模型需要大量精确标注的数据集，成本高昂。合成数据虽可扩展，但往往缺乏足够的真实感，难以泛化到真实场景。

Method: 提出新颖的工作流，整合Blender中的3D渲染与约束扩散模型，自动生成高质量、逼真的双孢蘑菇合成图像。该方法保持对3D场景配置和标注的完全控制，同时实现逼真效果，无需专门的计算机图形学专业知识。

Result: 发布两个合成数据集（各包含6000张图像，描绘超过25万个蘑菇实例），并在零样本设置下评估基于这些数据训练的Mask R-CNN模型。在两个独立的真实世界数据集（包括新收集的基准测试）上测试时，该方法实现了最先进的分割性能（在M18K上F1=0.859），尽管仅使用合成训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 高性能计算机视觉模型在医学影像中表现出色，但计算密集且模型庞大，不适合边缘设备部署；同时隐私约束阻碍集中式数据管理，需要联邦学习解决方案

Method: 提出偏度指导的剪枝方法，基于输出分布的统计偏度选择性剪枝多模态Swin Transformer的多头自注意力和多层感知机层，在水平联邦学习环境中验证

Result: 在紧凑型Swin Transformer上实现约36%的模型大小缩减，同时保持准确率不变，证明了在联邦学习环境中高效模型压缩的可行性

Conclusion: 该方法为边缘设备上的多模态医学AI提供了高效的模型压缩和隐私保护的分布式学习解决方案，平衡了性能、计算效率和隐私保护需求
...
Method: 核心思想是直接使原始条件特征具有运动感知能力来指导视频合成。首先用密集点轨迹表示物体运动，实现场景的细粒度控制；然后将轨迹投影到潜在空间，沿轨迹传播第一帧特征，生成对齐的时空特征图；该特征图作为更新的潜在条件，无缝集成到现成的图像到视频模型中作为运动指导。

Result: 通过规模化训练，Wan-Move能生成5秒、480p视频，其运动可控性在用户研究中与Kling 1.5 Pro的商业Motion Brush相当。在MoveBench基准测试和公共数据集上的实验一致显示其优越的运动质量。

Conclusion: Wan-Move框架实现了精确高质量的视频运动控制，无需辅助运动编码器，使基础模型微调易于扩展，为视频生成提供了实用的运动控制解决方案。
...
TL;DR: 论文探讨生成式与非生成式视觉模型在组合泛化能力上的差异，理论证明生成式方法通过解码器约束和反演能实现组合泛化，而非生成式方法难以实现。


<details>
  <summary>Details</summary>
Motivation: 研究生成式方法是否是人类级视觉感知的必要条件，探讨生成式与非生成式方法在组合泛化这一人类感知关键能力上的表现差异。

Method: 在组合数据生成过程下，形式化生成式（解码器）和非生成式（编码器）方法实现组合泛化所需的归纳偏置，理论分析编码器约束的不可行性，提出通过解码器约束和反演实现组合泛化的方法，包括在线梯度搜索和离线生成回放。

Result: 理论证明非生成式方法难以通过正则化或架构约束实现必要的归纳偏置，而生成式方法能直接实施这些偏置；实证研究显示非生成式方法常组合泛化失败，需要大规模预训练或额外监督，而生成式方法显著改善组合泛化能力。

Conclusion: 生成式方法通过解码器约束和反演能有效实现组合泛化，支持生成式方法对人类级视觉感知的重要性，为非生成式方法提供了理论限制和实证挑战。
...
TL;DR: Tri-Bench是一个紧凑的平面三角形几何推理基准，用于评估视觉语言模型在真实场景变化下的表现，发现模型在利用提示中的参考框架线索方面存在困难，主要依赖2D图像平面线索。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型具有令人印象深刻的能力，但在真实场景变化下经常失败。需要可验证的几何推理来构建可信和可控的智能体AI，因此开发了Tri-Bench基准来隔离相对几何推理能力。

Method: 创建Tri-Bench基准，包含平面三角形问题，重点测试两个部署关键因素：相机姿态（平面vs倾斜）和通过物体干扰的场景上下文（10个日常物体）。使用固定提示评估四个最近的VLM，提示中明确描述了周围的方形边框，通过单应性实现正确答案。

Result: 相对于3D地面真实值的总体准确率约为69%（最佳约75%，最差约64%）。与2D图像平面投影的对齐度更高，平均准确率约为72%。所有四个VLM在识别少数形状类别（等边、等腰、直角三角形）时都持续失败，准确率降至约0%。相机倾斜下总体VLM准确率下降约4.1%。物体干扰对VLM准确率没有显著影响。

Conclusion: 视觉语言模型未能正确利用提示中提供的明确参考框架提示，默认使用2D图像平面线索。这表明当前VLM在几何推理方面存在局限性，特别是在利用外部参考框架进行3D推理方面。
...
Result: 在多个遥感基准测试中取得了最先进的结果，视觉定位任务相对先前方法提升了24.8%的性能。

Conclusion: 将结构化空间推理整合到视觉语言模型中具有显著优势，为更可靠的卫星数据分析铺平了道路。
...
Result: 相比CUDNN，训练速度提升20-55%，能耗降低15-45%，精度与最先进的旋转不变方法相当。在8方向设置下，256×256输入获得45%加速和41%节能，1024×1024输入获得32%加速和23%节能。集成到U-Net分割模型中，相比非旋转感知基线精度提升达6%。

Conclusion: 该方法为现有旋转不变CNN框架提供了高效且有效的替代方案，在保持精度的同时显著提升了计算效率和能效，适用于无人机航拍图像等需要旋转不变性的分割任务。
...
TL;DR: UniLayDiff：首个统一的扩散Transformer模型，使用单一端到端可训练模型解决多种内容感知布局生成任务，将布局约束作为独立模态处理，通过多模态扩散Transformer框架实现背景图像、布局元素和多样约束的复杂交互。


<details>
  <summary>Details</summary>
Motivation: 内容感知布局生成在图形设计自动化中至关重要，但现实应用的多样性使得开发单一模型统一处理各种输入约束的子任务（如元素类型、尺寸或关系约束）极具挑战。现有方法要么只处理部分任务，要么需要为不同条件使用独立模型参数，缺乏真正的统一解决方案。
...
Conclusion: UniLayDiff首次实现了使用单一端到端可训练模型解决各种内容感知布局生成任务，不仅实现了统一条件生成，还提升了整体布局质量，为图形设计自动化提供了全面的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有单图像到3D生成器虽然能从单视图恢复合理的几何结构，但由于其面向对象的训练方式，难以泛化到复杂、大规模的场景，无法保证结构和纹理的忠实性。

Method: 采用三阶段迭代框架：1) 空间先验初始化；2) 视觉引导的3D场景网格生成；3) 空间引导的新视角生成。通过2D和3D域的交替优化，逐步改进结构和外观。

Result: 在多样化场景上的实验表明，EvoScene相比强基线方法，在几何稳定性、视角一致的纹理以及未见过区域的补全方面表现更优，能生成可直接用于实际应用的3D网格。

Conclusion: EvoScene通过结合现有模型的互补优势，实现了从单图像高质量重建3D场景，解决了现有方法在复杂场景泛化方面的局限性，为实际应用提供了可用的3D网格。
...
TL;DR: LiDAS是一种闭环主动照明系统，通过动态优化照明分布来提升夜间视觉感知性能，无需重新训练白天模型
...
Method: 结合现成的视觉感知模型和高清头灯，动态预测最优照明场，将光线从空区域重新分配到物体区域，实现零样本夜间泛化

Result: 在真实世界闭环驾驶场景中，相比标准近光灯提升18.7% mAP50和5.0% mIoU，在保持性能的同时减少40%能耗

Conclusion: LiDAS通过将现有头灯转变为主动视觉执行器，为鲁棒的夜间感知提供了经济有效的解决方案，补充了领域泛化方法
...
<details>
  <summary>Details</summary>
Motivation: 从视频中理解和重建动态场景的复杂几何和运动是计算机视觉中的一个重大挑战，需要高效的方法来解决4D重建任务。

Method: 采用统一的Transformer架构，通过新颖的查询机制独立灵活地探测时空任意点的3D位置，避免了密集逐帧解码的繁重计算和多任务特定解码器的复杂性。
...
<details>
  <summary>Details</summary>
Motivation: 传统NVS依赖显式3D归纳偏置和已知相机参数，而VGGT等视觉基础模型虽然能直接从无标定图像预测相机参数和3D表示，但其特征缺乏显式的多视角几何一致性，改进这种3D特征一致性对NVS和姿态估计都有益

Method: 提出Selfi自改进3D重建流水线，通过特征对齐将VGGT主干转换为高保真3D重建引擎。使用轻量级特征适配器，基于重投影一致性损失，将VGGT输出蒸馏到新的几何对齐特征空间，捕捉3D空间邻近性

Result: 在NVS和相机姿态估计任务中实现最先进的性能，证明特征对齐对下游3D推理是高度有益的一步

Conclusion: 特征对齐是提升3D推理性能的关键步骤，Selfi通过自改进流水线有效提升了VGGT特征的几何一致性，为无标定图像的3D重建提供了新思路
...
TL;DR: SEA是一个通用的字幕对齐框架，可将口语文本与手语视频对齐，支持多种语言和领域，无需特定数据集训练
...
Method: SEA框架包含三个步骤：1) 使用预训练模型将视频帧序列分割为单个手语；2) 将每个手语视频片段嵌入到与文本共享的潜在空间；3) 使用轻量级动态规划程序进行对齐

Result: 在四个手语数据集上的实验表明，SEA实现了最先进的对齐性能，即使在长达一小时的视频中也能在CPU上一分钟内完成对齐

Conclusion: SEA提供了一个灵活、高效的通用字幕对齐框架，能够生成高质量并行数据，推动手语处理技术的发展
...
TL;DR: 该论文提出了一种通用对抗后缀攻击方法，通过优化短令牌序列（4-10个令牌）来降低跨任务和跨模型的语言模型分类准确性。


<details>
  <summary>Details</summary>
Motivation: 现有对抗提示优化方法通常是任务或模型特定的，导致结果难以比较且可迁移性有限。需要一种通用的对抗攻击方法，能够在不同任务和模型之间有效转移。

Method: 使用Gumbel-Softmax松弛学习可微分的"软"后缀，然后离散化用于推理。训练时最大化标签区域的校准交叉熵，同时屏蔽黄金令牌以防止信息泄露，并加入熵正则化避免崩溃。

Result: 单个后缀在一个模型上训练后能有效迁移到其他模型，持续降低准确性和校准置信度。在情感分析、自然语言推理、释义检测、常识问答和物理推理等任务上，对Qwen2-1.5B、Phi-1.5和TinyLlama-1.1B模型均表现出一致的攻击效果。

Conclusion: 通用对抗后缀是一种有效的跨任务和跨模型攻击方法，揭示了语言模型在零样本/少样本分类设置下的脆弱性，为模型鲁棒性研究提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型容易受到短对抗性后缀的攻击，传统方法（梯度搜索或基于规则的方法）通常脆弱且局限于单一任务或模型，需要更通用、可迁移的攻击方法

Method: 采用强化学习框架，将对抗性后缀视为策略，使用近端策略优化（PPO）进行训练，以冻结模型作为奖励预测器；通过校准交叉熵进行奖励塑形，消除标签偏差并聚合不同表面形式以提高可迁移性

Result: 在五个不同的NLP基准数据集（涵盖情感分析、自然语言推理、释义、常识推理）和三个不同语言模型（Qwen2-1.5B Instruct、TinyLlama-1.1B Chat、Phi-1.5）上评估，RL训练的后缀能持续降低模型准确率，且比类似类型的先前对抗性触发器具有更好的跨任务和跨模型迁移效果

Conclusion: 强化学习框架为生成对抗性后缀提供了一种有效方法，相比传统方法具有更好的通用性和可迁移性，揭示了语言模型在面对精心设计的对抗性攻击时的脆弱性
...
<details>
  <summary>Details</summary>
Motivation: 提高临床试验数据的可访问性，为患者、临床医生、研究人员和政策制定者提供更便捷的循证医学信息访问途径

Method: 整合ClinicalTrials.gov数据，使用GPT-5.1和Gemini-3-Pro等大语言模型自动解析PubMed全文研究文章提取结构化试验信息，将用户查询转换为结构化数据库搜索，并提供基于证据的问答系统

Result: 相比仅使用ClinicalTrials.gov，系统将结构化临床试验数据访问量提高了83.8%，通过用户研究和自动评估验证了信息提取和问答能力

Conclusion: ClinicalTrialsHub平台通过整合和增强临床试验数据，显著提高了循证医学信息的可访问性，具有重要的临床应用价值
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在调查GLLM在标注任务中的偏差问题，通过概念性复现Boukes(2024)的人工标注研究，评估不同GLLM在不同提示下的标注表现

Method: 使用多种GLLM模型（Llama3.1:8b, Llama3.3:70b, GPT4o, Qwen2.5:72b）结合五种不同提示，对五个概念（政治内容、互动性、理性、不文明程度、意识形态）进行标注，并与人工标注结果对比

Result: GLLM在F1分数上表现尚可，但在分布频率上与人工标注存在显著差异，导致下游结果不同；GLLM之间的一致性高于与人工标注的一致性，显示系统性偏差；F1分数差异无法充分反映偏差程度

Conclusion: GLLM标注存在系统性偏差，不能简单依赖F1分数评估标注质量，需要更全面的偏差评估方法
...
TL;DR: 该研究通过对比解释和显著性归因分析，探索机器翻译模型中性别偏见的起源，而非仅仅测量偏见。研究使用性别模糊的自然源数据，分析源句中哪些上下文标记影响翻译模型选择特定性别屈折形式，并比较模型归因与人类感知的一致性。


<details>
  <summary>Details</summary>
Motivation: 当前可解释性研究主要关注理解黑盒模型决策，但在性别偏见这一重要问题上研究有限。研究旨在超越简单的偏见测量，探索性别偏见的起源，理解翻译模型在性别选择决策背后的机制。

Method: 使用性别模糊的自然源数据，采用对比解释和显著性归因计算方法。首先解决缺乏评分阈值的问题，分析不同归因水平下源词对模型性别决策的影响。将模型识别的显著源词与人类性别感知进行比较，并进行语言学分析。

Result: 研究发现人类感知与模型归因之间存在显著重叠，表明模型在性别决策时关注与人类相似的上下文线索。通过语言学分析揭示了影响性别决策的具体词汇特征。

Conclusion: 研究展示了理解模型翻译决策在性别方面的重要性，以及这与人类决策的对比。这些信息应被用于缓解性别偏见，为开发更公平的翻译系统提供基础。
...
TL;DR: 本研究提出一种软归纳偏置方法，通过明确定义推理视角来指导大型语言模型的推理过程，用于韩语不当言论检测，相比标准监督学习提升约3.89%的准确率。
...
Conclusion: 提出的软归纳偏置方法通过明确定义推理视角，能够有效指导大语言模型的推理过程，在不恰当言论检测任务中表现出色，为构建更安全的在线交流环境提供了有效技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型和多智能体系统在分解复杂任务方面有潜力，但在长时程推理任务上表现不佳且计算成本高昂，需要更有效的分布式推理方法。

Method: 采用分层多智能体架构，在64*64网格上分布轻量级智能体，配合选择性oracle支持；通过空间课程学习逐步扩展操作区域；使用负对数似然(NLL)作为置信度度量；采用Thompson Sampling课程管理器基于能力和NLL奖励信号自适应选择训练区域。

Result: 在空间接地的汉诺塔基准测试中，该方法表现出改进的稳定性、减少的oracle使用率，以及通过分布式智能体协作实现的更强的长程推理能力。

Conclusion: 提出的分层多智能体架构结合空间课程学习和置信度度量，能够有效解决长时程推理任务，在分布式协作中实现更稳定和高效的推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗NLP领域的综述要么忽略了一些重要任务（如隐私保护的合成数据生成、可解释性临床NLP），要么遗漏了重要方法（如检索增强生成、神经符号集成），因此需要提供一个更全面的入门教程。

Method: 采用三层层次结构组织教程内容：1) 数据/资源层（标注指南、伦理审批、治理、合成数据）；2) NLP评估层（NER、关系抽取、情感分析等任务，以及可解释性HealthAI方法）；3) 患者层（患者参与、健康素养、翻译简化、决策支持）。包含实践环节让听众使用HealthcareNLP应用。

Result: 提出了一个结构化的HealthcareNLP教程框架，涵盖从数据准备到患者应用的全流程，针对医疗NLP从业者、研究人员和学生设计，无需先验知识即可参与。

Conclusion: 该教程为医疗NLP领域提供了一个全面的入门指南，特别强调了当前研究忽略的重要方面，通过分层结构和实践环节帮助不同背景的参与者理解和应用HealthcareNLP技术。
...
<details>
  <summary>Details</summary>
Motivation: 为支持基于大语言模型的虚拟调查和标注任务，需要系统化的工具来生成问卷响应并评估不同方法对结果的影响，提高研究的可重复性和可靠性。

Method: 开发了QSTN开源框架，支持问卷呈现、提示扰动和响应生成方法的系统评估，提供无代码用户界面，使研究人员无需编程知识即可设置实验。

Result: 超过4000万次调查响应的评估显示，问题结构和响应生成方法对生成响应与人类答案的一致性有显著影响，且计算成本仅为一小部分。

Conclusion: QSTN框架有助于提高基于LLM研究的可重复性和可靠性，为研究人员提供了强大的实验工具，支持未来LLM研究的稳健发展。
...
Method: 构建3200篇巴斯克语C1水平作文数据集，每篇由专家标注包含正确性、丰富性、连贯性、衔接性和任务对齐等维度的评分及详细反馈；微调RoBERTa-EusCrawl和Latxa 8B/70B等开源模型进行评分和解释生成；提出结合自动一致性指标和专家验证的新型反馈生成评估方法

Result: 编码器模型在自动作文评分上保持高可靠性；监督微调的Latxa模型性能显著提升，在评分一致性和反馈质量上超越GPT-5和Claude Sonnet 4.5等最先进闭源系统；微调后的Latxa模型能生成标准对齐、教育学意义强的反馈，并能识别比专有模型更广泛的错误类型

Conclusion: 该资源和基准为巴斯克语等低资源语言的透明、可复现且教育基础扎实的NLP研究奠定了基础，展示了开源模型通过适当微调在特定语言任务上超越闭源系统的潜力
...
<details>
  <summary>Details</summary>
Motivation: 现有的偏好优化研究主要针对英语和中文，低资源语言缺乏母语者编写的数据集和能够生成流畅合成数据的语言模型，因此需要开发无需目标语言指令调优数据的流畅偏好对齐语言模型

Method: 采用基于策略的训练方法，与两种常见方法进行比较：基于机器翻译数据的监督微调和多语言微调，以挪威博克马尔语为案例进行研究

Result: 基于策略的训练方法在流畅性方面优于其他方法，且不依赖难以获取的数据，通过母语者评估验证了其有效性

Conclusion: 基于策略的训练方法对于低资源语言的偏好对齐至关重要，能够在缺乏目标语言指令数据的情况下实现流畅的语言模型对齐
...
TL;DR: 本文提出了一种在联邦学习环境中评估大型语言模型与多样化人类偏好对齐的框架，通过自适应聚合策略平衡对齐质量和公平性
...
Method: 引入综合评估框架，评估标准奖励聚合技术（最小、最大、平均值），并提出基于历史对齐性能动态调整偏好权重的自适应方案
...
<details>
  <summary>Details</summary>
Motivation: 虽然逐步增加Transformer深度训练（MIDAS）已被证明能降低训练成本并提高推理性能，但对其增益的机制理解仍然缺失。本研究旨在建立与"深度诅咒"现象（即非增长Transformer后半部分层对最终输出分布贡献较小）的联系，揭示深度增长训练的机制原理。

Method: 通过深度分析技术，研究逐步中间堆叠增长方法如何影响模型。具体包括：分析深度利用效率、残差流结构变化、可置换计算块形成机制。此外，提出了MIDAS的轻量级修改方案以进一步改进下游推理基准性能。

Result: 研究发现深度增长训练能更有效地利用模型深度，改变残差流结构，促进可置换计算块的形成。提出的MIDAS轻量级修改方案在下游推理基准测试中获得了进一步改进。深度增长训练克服了标准非增长模型中观察到的有限深度利用问题。

Conclusion: 逐步增加模型深度训练能形成独特的计算电路，克服标准非增长模型中的深度利用限制。这项工作强调了深度增长训练在改变计算结构和提高模型效率方面的重要作用，为理解Transformer深度训练的机制提供了新的视角。
...
TL;DR: RAGLens：基于稀疏自编码器和信息特征选择的轻量级RAG幻觉检测器，无需大规模训练数据或外部LLM查询，利用LLM内部表示准确识别不忠实生成


<details>
  <summary>Details</summary>
Motivation: RAG虽然通过检索证据提高了LLM的事实性，但生成内容与源信息矛盾或超出范围的"不忠实"问题仍然严重。现有幻觉检测方法要么需要大量标注数据进行训练，要么依赖外部LLM判断导致高推理成本，而基于LLM内部表示的方法准确率有限。

Method: 利用稀疏自编码器解耦LLM内部激活，识别RAG幻觉触发特征；构建基于信息特征选择和加性特征建模的系统化流程；开发轻量级幻觉检测器RAGLens，使用LLM内部表示标记不忠实RAG输出。

Result: RAGLens在检测性能上优于现有方法，同时提供可解释的决策依据，能够有效事后缓解不忠实的RAG生成；揭示了LLM中幻觉相关信号的分布新见解。

Conclusion: RAGLens通过机制可解释性方法成功识别了RAG幻觉的特征模式，实现了高效、准确的幻觉检测，为理解和缓解RAG不忠实问题提供了新工具和见解。
...
TL;DR: 该研究比较了数据导向设计(DOD)与传统面向对象设计(OOD)在多线程环境下的性能表现，通过A*搜索算法的四个版本测试发现DOD在多线程场景中具有显著性能优势，但细粒度任务中单线程版本优于多线程版本。
...
Conclusion: 即使在简单算法中性能差异看似细微，但DOD在关键指标上的持续优势凸显了其基础架构的优越性，表明对于复杂、大规模AI和并行计算任务，DOD是最大化硬件效率的更有效方法。
...
TL;DR: SkipKV是一种无需训练的KV缓存压缩方法，通过句子级选择和生成优化，在保持推理准确性的同时显著减少KV缓存开销和生成长度。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在链式思维推理过程中会产生线性增长的KV缓存开销，导致内存和吞吐量瓶颈。现有KV缓存淘汰方法在多批次设置中因不稳定的令牌级评分和填充令牌导致的KV预算减少而无法保持准确性，且往往生成更长的序列。

Method: SkipKV是一种无需训练的句子级KV压缩方法：1）引入句子评分指标识别并移除高度相似的句子，同时保持语义连贯性；2）动态调整引导向量更新推理期间的隐藏激活状态，强制模型生成简洁响应。

Result: 在多个推理基准测试中，SkipKV在相似压缩预算下相比替代方法保持高达26.7%的准确性提升，同时相比最先进方法减少1.6倍的生成长度，吞吐量提升高达1.7倍。

Conclusion: SkipKV通过句子级KV缓存压缩有效解决了大型推理模型在链式思维推理中的KV缓存开销问题，在保持准确性的同时显著提升了推理效率。
...
TL;DR: 开发了一个基于大语言模型的AI增强患者-临床试验匹配系统，解决传统人工筛选效率低的问题，通过结构化资格评估和可解释推理链支持专家审查。
...
Method: 使用开源、支持推理的大语言模型构建安全可扩展系统，整合异构电子健康记录数据，生成结构化资格评估和可解释推理链，支持人机协同审查。

Result: 系统能够超越简单的二元分类，将资格表示为动态状态而非固定判断，识别当前匹配并提供未来可能使患者符合资格的行动建议。

Conclusion: 该AI增强的患者-试验匹配系统有望减轻协调员负担，智能扩展每位患者考虑的试验范围，并保证所有AI生成输出的全面可审计性。
...
TL;DR: 本研究对ChatGPT和DeepSeek两大LLM在教育研究领域进行综合评估，比较了它们在文本生成、编程和专门问题解决方面的性能差异。


<details>
  <summary>Details</summary>
Motivation: 随着预训练大语言模型在教育研究领域的广泛应用，需要系统评估当前领先模型（ChatGPT和DeepSeek）的实际性能差异，为教育工作者和研究人员提供选择参考。

Method: 采用背景技术分析、实证实验和真实用户调查相结合的方法，评估模型在准确性、计算效率和用户体验方面的权衡，并在文本生成、编程和专门问题解决等任务上进行基准测试。

Result: ChatGPT在通用语言理解和文本生成方面表现更优，而DeepSeek凭借其效率导向的设计在编程任务中表现更佳；两种模型在医学诊断准确性和复杂数学问题解决方面均有良好表现。

Conclusion: 两种LLM在不同应用场景中各有优势，用户调查揭示了这些模型在教育研究中的实际价值与局限性，为领域发展提供了重要见解。
...
TL;DR: 开发了一个用于移动糖尿病预测应用的可扩展后端系统，采用水平扩展、数据库分片和异步消息队列，83%的功能满足性能目标，能处理1万并发用户。
...
Method: 采用水平扩展、数据库分片和通过消息队列（RabbitMQ）的异步通信架构来构建可扩展后端系统。

Result: 83%的系统功能（24个中的20个）满足性能目标（故障率低于5%，平均延迟低于1000ms），能处理1万并发用户，异步通信有效降低了计算密集型预测请求的错误率。

Conclusion: 该可扩展后端系统成功支持糖尿病预测应用，验证了架构设计的有效性，异步通信在确保系统可靠性方面发挥了关键作用。
...
### [110] [Beyond Traditional Diagnostics: Transforming Patient-Side Information into Predictive Insights with Knowledge Graphs and Prototypes](https://arxiv.org/abs/2512.08261)
*Yibowen Zhao,Yinan Zhang,Zhixiang Su,Lizhen Cui,Chunyan Miao*

Main category: cs.AI

TL;DR: 该论文提出了KPI框架，通过知识图谱增强、原型感知和可解释性方法，仅基于患者人口统计信息和自我报告症状来预测疾病，解决了现有方法中的不平衡分布和可解释性不足问题。


<details>
  <summary>Details</summary>
Motivation: 仅基于患者侧信息（如人口统计和自报症状）预测疾病具有重要价值，可以增强患者意识、促进早期医疗参与并提高医疗系统效率。然而，现有方法面临疾病分布不平衡和缺乏可解释性等关键挑战，导致预测存在偏见或不可靠。

Method: 提出KPI框架：1）将结构化可信医疗知识整合到统一疾病知识图谱中；2）构建具有临床意义的疾病原型；3）使用对比学习提高预测准确性，特别是对长尾疾病；4）利用大语言模型生成患者特定、医学相关的解释以提高可解释性和可靠性。

Result: 在真实世界数据集上的广泛实验表明，KPI在预测准确性方面优于最先进方法，并提供与患者叙述密切相关的临床有效解释，突显其在以患者为中心的医疗保健服务中的实用价值。

Conclusion: KPI框架通过整合医疗知识图谱、构建疾病原型、使用对比学习和LLM生成解释，有效解决了疾病预测中的不平衡分布和可解释性问题，为患者为中心的医疗保健提供了实用解决方案。
...
### [111] [Reasoning Models Ace the CFA Exams](https://arxiv.org/abs/2512.08270)
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明大语言模型在CFA考试中表现不佳，但近期推理模型在各种学科的研究生级别和专业考试中取得优异成绩，因此需要评估最新推理模型在CFA考试中的实际表现。

Method: 使用包含980道题目的模拟CFA考试数据集，涵盖三个Level I考试、两个Level II考试和三个Level III考试，采用与先前研究相同的通过/未通过标准进行评估。

Result: 大多数模型通过了所有三个级别考试。按总体表现排序，通过模型依次为：Gemini 3.0 Pro、Gemini 2.5 Pro、GPT-5、Grok 4、Claude Opus 4.1和DeepSeek-V3.1。Gemini 3.0 Pro在Level I创下97.6%的最高分记录；GPT-5在Level II以94.3%领先；在Level III，Gemini 2.5 Pro在选择题中获得86.4%最高分，而Gemini 3.0 Pro在构建回答题中获得92.0%最高分。

Conclusion: 最新推理模型在CFA考试中表现出色，多数模型能够通过所有三个级别，表明这些模型在专业金融知识评估方面已达到相当高的水平。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型通过强化学习后训练可以进化为推理语言模型，在思维链中表现出"顿悟"时刻和自我反思等高级推理策略。受此启发，作者希望开发一种机制让任何LLM都能获得这种推理能力。

Method: 提出强化策略注入机制(rSIM)，使用小型规划器作为领导者智能体，LLM作为跟随者智能体，基于领导者-跟随者框架和多智能体强化学习进行联合训练。规划器自适应地向LLM的思维链中注入推理策略，使用简单的基于规则的奖励。

Result: rSIM使Qwen2.5-0.5B模型转变为RLM，性能显著超越Qwen2.5-14B。规划器具有通用性：只需训练一次即可作为插件应用于现有LLM，大幅提升推理能力。规划器还支持跨任务持续学习，规划能力逐步提升并能泛化到更广泛的问题。

Conclusion: rSIM机制成功地将普通LLM转变为具有高级推理能力的RLM，通过小型规划器的策略注入显著提升了模型的推理性能，且该规划器具有通用性、可插拔性和持续学习能力。
...
TL;DR: 该研究开发了一个机器学习框架，使用土耳其382个土壤样本的物理化学特性来预测加州承载比(CBR)，随机森林回归器表现最佳，为岩土工程提供了一种高效的数据驱动替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统CBR实验室测试方法耗时、昂贵且不适用于大规模或多样化土壤剖面，需要更快速、精确的预测方法。人工智能和机器学习的发展为建模复杂土壤行为提供了数据驱动的新途径。
...
TL;DR: 该研究提出使用自动机器学习(AutoML)方法预测土壤压实参数(最优含水量OMC和最大干密度MDD)，相比传统实验室方法和现有ML模型，AutoML通过自动化算法选择和超参数优化，显著提高了预测精度和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统确定土壤压实参数的方法需要大量实验室工作，而现有经验回归模型和机器学习模型在不同土壤类型上的适用性和准确性有限，特别是在处理异质数据集时预测精度和泛化能力不足。

Method: 采用自动机器学习(AutoML)方法，自动化进行算法选择和超参数优化，以预测最优含水量(OMC)和最大干密度(MDD)。通过大量实验验证不同算法的性能。

Result: 研究发现极端梯度提升(XGBoost)算法表现最佳，在独立数据集上对MDD的R²达到80.4%，对OMC的R²达到89.1%。同时证实异质数据集对提升ML模型的泛化能力和性能至关重要。

Conclusion: AutoML方法能有效预测不同土壤类型的压实参数，为更高效可靠的施工实践提供支持。研究强调了异质数据集在提升机器学习模型性能方面的重要性。
...
TL;DR: DuSAR框架让单个冻结LLM通过双策略协调（高层整体规划与上下文局部策略）和轻量级反思机制实现自适应推理，在ALFWorld和Mind2Web上达到SOTA性能，同时大幅降低计算开销。
...
Method: 提出DuSAR框架：1）双策略协调：高层整体规划提供宏观指导，上下文局部策略处理具体情境；2）轻量级反思机制：通过策略适应度评分动态评估进展，在遇到困难时修订全局规划或在取得进展时细化规划；3）模仿人类元认知行为。

Result: 在ALFWorld上达到37.1%成功率（Llama3.1-70B），比之前最佳结果13.0%提高一倍多；在Mind2Web上达到4.02%，同样翻倍；每步token消耗减少3-9倍；消融研究证实双策略协调的必要性；可选集成专家演示可进一步提升性能。

Conclusion: DuSAR通过双策略协调和反思机制实现了高效的自适应推理，无需依赖外部演示，显著提升了LLM智能体的性能、泛化能力和计算效率，同时保持与外部知识集成的灵活性。
...
TL;DR: DeepFeature：首个基于大语言模型的情境感知特征生成框架，用于可穿戴生物信号，通过多源特征生成、迭代特征精炼和鲁棒代码转换，在8个任务上平均AUROC提升4.21-9.67%


<details>
  <summary>Details</summary>
Motivation: 当前可穿戴设备生物信号的特征提取方法存在三个主要问题：缺乏任务特定的情境知识、难以在高维特征空间中找到最优特征提取设置、容易产生代码生成和自动化错误。这些限制影响了医疗健康应用中机器学习模型的性能。

Method: DeepFeature框架包含三个核心组件：1）多源特征生成机制，整合专家知识和任务设置；2）迭代特征精炼过程，基于特征评估反馈进行特征重选；3）鲁棒的多层过滤和验证方法，确保特征到代码的可靠转换，避免程序崩溃。

Result: 在八个不同任务上的实验评估显示，DeepFeature相比基线方法平均AUROC提升了4.21-9.67%。在五个任务上超越了最先进的方法，在其余任务上保持了相当的性能。

Conclusion: DeepFeature是首个基于大语言模型的情境感知特征生成框架，能够有效解决现有特征提取方法的局限性，显著提升可穿戴生物信号分析任务的性能，为医疗健康应用提供了更可靠的特征工程解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究在没有外部反馈的情况下人类如何掌握高性能技能（如花样滑冰、投球、拉花艺术），通过可控的物理实验系统来研究技能获取过程。

Method: 使用通用强化学习智能体与桌面循环水槽中的旋转圆柱体直接交互，目标是最小化或最大化阻力。系统具有丰富的物理交互和复杂动力学特性，难以精确建模。

Result: 高维流场反馈让智能体在几分钟的真实交互中发现高性能阻力控制策略；执行时不需要反馈也能获得几乎相同的性能；没有流场反馈时，最大化阻力任务完全失败，最小化阻力任务虽能成功但更慢且不可靠。
...
Method: 提出Principles2Plan交互式研究原型，让领域专家提供规划领域、问题细节和高级伦理原则（如仁慈、隐私），系统生成可操作的伦理规则，用户可审查、优先排序并供给规划器生成伦理导向的计划。

Result: 开发了首个支持用户在经典规划情境中生成基于原则的伦理规则的系统，展示了人类-LLM协作使伦理自动化规划更实用可行的潜力。

Conclusion: Principles2Plan证明了人类与大型语言模型协作在生成情境敏感伦理规则和指导自动化规划方面的潜力，为解决机器人伦理规划的实际问题提供了新途径。
...
TL;DR: 本文提出CogMCTS框架，通过将LLM的认知引导机制与MCTS紧密结合，实现高效的自动启发式优化，在稳定性、效率和解决方案质量方面优于现有基于LLM的AHD方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的进化方法依赖种群策略且易陷入局部最优，而LLM与MCTS结合的方法在多轮认知整合和搜索多样性方面存在局限。需要克服这些限制，实现更高效的自动启发式设计。
...
TL;DR: 提出基于Transformer的蛋白质二级结构预测模型，利用注意力机制处理氨基酸序列，通过滑动窗口数据增强技术提升CB513数据集训练效果，能够有效捕捉局部和长程残基相互作用。


<details>
  <summary>Details</summary>
Motivation: 蛋白质二级结构（如α螺旋、β折叠、无规则卷曲）的预测对于理解蛋白质功能至关重要。传统方法在捕捉长程残基相互作用方面存在局限，需要更先进的模型来提升预测准确性。

Method: 采用基于Transformer的模型，应用注意力机制处理蛋白质序列数据。使用滑动窗口数据增强技术在CB513数据集上扩展训练样本，使模型能够处理可变长度序列并有效捕捉局部和长程残基相互作用。

Result: Transformer模型展现出强大的泛化能力，能够处理不同长度的蛋白质序列，同时有效捕捉残基间的局部和长程相互作用，在蛋白质二级结构预测任务中表现优异。

Conclusion: 基于Transformer的模型为蛋白质二级结构预测提供了有效的解决方案，其注意力机制能够更好地捕捉序列中的复杂模式，为蛋白质功能研究提供了有力工具。
...
### [121] [See-Control: A Multimodal Agent Framework for Smartphone Interaction with a Robotic Arm](https://arxiv.org/abs/2512.08629)
...
Method: 提出See-Control框架，包含三个关键组件：1) ESO基准测试（155个任务和评估指标）；2) 基于MLLM的具身智能体，无需ADB或系统后端访问即可生成机器人控制命令；3) 丰富标注的操作事件数据集。

Result: 通过低自由度机械臂直接物理交互实现智能手机操作，提供了平台无关的解决方案，为未来研究提供了有价值的资源。

Conclusion: See-Control弥合了数字智能体与物理世界之间的差距，为实现家庭机器人在真实环境中执行依赖智能手机的任务迈出了具体一步。
...
### [122] [Multi-Agent Intelligence for Multidisciplinary Decision-Making in Gastrointestinal Oncology](https://arxiv.org/abs/2512.08674)
...
<details>
  <summary>Details</summary>
Motivation: 胃肠肿瘤多模态临床推理需要整合内镜影像、放射学数据和生化标志物，但现有多模态大语言模型在处理复杂异质医疗数据时面临上下文稀释和幻觉问题，需要更可靠的自动化决策支持

Method: 提出分层多智能体框架，模拟人类多学科团队协作工作流程，通过智能体间的协作来解决多模态医疗数据整合问题

Result: 系统获得4.60/5.00的专家评估分数，相比单体基线有显著提升，智能体架构在推理逻辑和医学准确性方面改善最为明显

Conclusion: 模拟人类协作的智能体架构为肿瘤学自动化决策支持提供了可扩展、可解释且临床稳健的范式
...
### [123] [Towards Foundation Models with Native Multi-Agent Intelligence](https://arxiv.org/abs/2512.08743)
*Shuyue Hu,Haoyang Yan,Yiqun Zhang,Yang Chen,Dongzhan Zhou,Lei Bai*

Main category: cs.AI

TL;DR: 论文指出基础模型作为AI智能体的"大脑"，当前研究主要关注单智能体能力，但多智能体智能是下一个前沿，需要专门的研究和训练。


<details>
  <summary>Details</summary>
Motivation: 虽然基础模型正成为AI智能体的核心，但现有研究主要关注单智能体能力（如GUI交互、工具使用），而多智能体智能尚未得到足够重视。作者认为这是下一个重要研究方向。

Method: 识别了基础模型在多智能体环境中的四个核心能力：理解、规划、高效通信和适应。通过41个大语言模型的实证研究，验证了强单智能体性能不能自动转化为稳健的多智能体智能。

Result: 实证研究表明，强大的单智能体性能并不能自动带来稳健的多智能体智能，这揭示了当前基础模型在多智能体场景中的局限性。
...
### [124] [A Practical Guide for Designing, Developing, and Deploying Production-Grade Agentic AI Workflows](https://arxiv.org/abs/2512.08769)
...
TL;DR: 本文提供了一套从设计到部署生产级智能体AI工作流的端到端工程指南，介绍了结构化生命周期、九个核心最佳实践，并通过多模态新闻分析案例进行演示。
...
Method: 提出结构化工程生命周期，包括工作流分解、多智能体设计模式、模型上下文协议(MCP)、工具集成、确定性编排、负责任AI考虑和环境感知部署策略。介绍了九个核心最佳实践，包括工具优先设计、纯函数调用、单一职责智能体、外部化提示管理等。

Result: 通过多模态新闻分析和媒体生成工作流的案例研究，展示了这些原则的实际应用，提供了一个全面的生产级智能体AI工作流实施框架。

Conclusion: 本文提供了构建健壮、可扩展、生产就绪的智能体AI工作流的基础参考，结合了架构指导、操作模式和实践实施见解，为工程化生产级智能体AI系统提供了实用指南。
...
### [125] [CARLoS: Retrieval via Concise Assessment Representation of LoRAs at Scale](https://arxiv.org/abs/2512.08826)
...
<details>
  <summary>Details</summary>
Motivation: 生成式组件（如LoRA）的快速扩散创建了一个庞大但无结构的生态系统。现有的发现方法依赖于不可靠的用户描述或有偏见的流行度指标，阻碍了可用性。需要一种不依赖额外元数据就能表征LoRA的方法。

Method: 分析650多个LoRA模型，在各种提示和种子下进行图像生成以评估其行为。使用CLIP嵌入及其与基础模型生成的差异，定义了三个部分的表示：方向（定义语义偏移）、强度（量化效果显著性）和一致性（量化效果稳定性）。基于这些表示开发了高效的检索框架。

Result: CARLoS框架在自动和人工评估中优于文本基线，能够语义匹配文本查询到相关LoRA，同时过滤过强或不稳定的模型。该表示还支持分析强度、一致性与版权法中实质性、意志性等法律概念的关联。

Conclusion: CARLoS提供了一个实用的系统，不仅改进了LoRA检索，其表示方法还具有更广泛的LoRA分析相关性，包括与版权法律概念的连接。
...
### [126] [Interpolation in Knowledge Representation](https://arxiv.org/abs/2512.08833)
...
<details>
  <summary>Details</summary>
Motivation: Craig插值和一致插值在知识表示中有重要应用，包括可解释性、遗忘、模块化与重用以及学习等领域，但许多相关形式化方法通常缺乏这些插值特性，且实际计算插值具有挑战性

Method: 深入分析两种主要知识表示形式化方法（描述逻辑和逻辑编程），讨论计算插值的理论结果和实用方法

Result: 论文综述了描述逻辑和逻辑编程中插值问题的理论特性和计算方法，为实际应用提供指导

Conclusion: Craig插值和一致插值在知识表示中具有重要价值，但实际计算面临挑战，需要进一步研究理论特性和实用算法
...
TL;DR: REST和REST+基准测试用于系统评估多模态大语言模型的跨模态不一致性，发现当前SOTA模型在不同模态（图像、文本、混合）中无法保持一致的推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型虽然训练在同一个嵌入空间中表示视觉和语言信息，但无法在不同模态中执行相同的任务，存在跨模态不一致性问题，需要系统性的评估方法。

Method: 提出REST和REST+两个基准测试，包含相同语义信息在三种模态（图像、文本、混合）中的样本，评估了15个MLLM模型，分析OCR、视觉特征（颜色、分辨率、字体）和视觉token数量对性能的影响。

Result: 发现不同MLLM的模态不一致程度差异很大，即使OCR正确，文本颜色、分辨率和视觉token数量仍影响模型性能，渲染文本为图像或图像为文本都无法解决不一致性问题。

Conclusion: 一致性得分与文本和图像之间的模态差距相关，揭示了跨模态不一致MLLM的机制解释，为改进多模态模型的一致性提供了重要见解。
