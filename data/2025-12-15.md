...
TL;DR: 本文提出利用文本引导方法增强人脸图像性别分类算法公平性的新方法，通过图像-文本匹配指导和图像-文本融合策略，在无需人口统计标签的情况下减少性别和种族偏见。


<details>
  <summary>Details</summary>
Motivation: 当前人脸图像性别分类算法存在人口统计偏见问题，特别是在不同种族群体中表现不公平。研究旨在开发无需人口统计标签的公平性增强方法，通过利用文本语义信息来减少算法偏见。

Method: 提出两种核心方法：1) 图像-文本匹配指导，训练模型识别图像和文本之间的细粒度对齐，获得增强的多模态表示；2) 图像-文本融合，将两种模态结合成综合表示以提升公平性。方法不依赖人口统计标签，具有应用无关性。

Result: 在基准数据集上的广泛实验表明，相比现有方法，这些方法能有效减轻偏见，提高跨性别和种族群体的分类准确性。文本引导的集成提供了可解释和直观的训练范式。

Conclusion: 通过利用文本语义信息，本文提出的方法为开发更公平的人脸分析算法提供了有价值的见解，为解决人脸图像性别分类中的人口统计偏见这一关键挑战做出了贡献。
...
TL;DR: 提出首个足球视觉基础模型SoccerMaster，通过多任务预训练统一处理从细粒度感知到语义推理的多样化足球视觉理解任务，构建了自动化数据标注管道和综合数据集SoccerFactory，在多个下游任务上超越任务专用模型。
...
Method: 提出SoccerMaster足球视觉基础模型，通过监督多任务预训练统一处理多样化任务；开发自动化数据标注管道生成可扩展的空间标注；整合现有足球视频数据集构建SoccerFactory预训练数据资源。

Result: SoccerMaster在多个下游任务上持续超越任务专用专家模型，展示了其广度和优越性；数据、代码和模型将公开可用。

Conclusion: SoccerMaster作为首个足球专用视觉基础模型，通过统一框架和多任务预训练有效处理多样化足球视觉理解任务，为足球分析领域提供了强大的基础模型解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 结核病是全球主要死因之一，特别是在资源有限国家。胸部X光成像虽然成本低廉且易于获取，但需要专家解读，而专家资源往往不足。现有机器学习模型在结核病分类中表现良好，但依赖虚假相关性且泛化能力差。此外，构建具有高质量标注的医学图像数据集需要大量资源和专家投入，成本高昂。

Method: 采用知识蒸馏技术，通过教师-学生框架训练CNN模型。使用ResNet50架构，在TBX11k数据集上进行训练，无需边界框标注即可定位结核病相关异常。该方法旨在减少模型对虚假相关性的依赖。

Result: 提出的方法在TBX11k数据集上取得了0.2428 mIOU的优异分数。实验结果显示，学生模型持续优于教师模型，表明其具有更好的鲁棒性，在多样化临床环境中具有更广泛的部署潜力。

Conclusion: 知识蒸馏技术能有效训练CNN模型减少虚假相关性并定位结核病异常，无需昂贵的边界框标注。学生模型优于教师模型，显示出更好的鲁棒性和临床部署潜力，为资源有限环境下的结核病诊断提供了有前景的解决方案。
...
TL;DR: 该论文提出了Synthetic Vasculature Reasoning (SVR)框架，通过可控合成视网膜血管图像和对应文本，解决了医学领域缺乏大规模图像-文本配对数据的问题，并创建了OCTA-100K-SVR数据集，显著提升了视觉语言模型在糖尿病视网膜病变诊断中的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLMs)在可解释的医学诊断方面具有潜力，但训练需要大规模图像-文本数据集。在眼科OCTA图像等专业领域，包含精确病理描述的文本数据稀缺甚至不存在，这成为模型训练的主要瓶颈。

Method: 提出了Synthetic Vasculature Reasoning (SVR)框架，能够可控合成包含糖尿病视网膜病变特征（毛细血管脱落、微动脉瘤、新生血管、血管迂曲）的视网膜血管图像，并自动生成细粒度的推理文本。基于此方法创建了包含10万对数据的OCTA-100K-SVR数据集。

Result: 在OCTA-100K-SVR数据集上训练的通用视觉语言模型(Qwen3-VL-8b)在真实OCTA图像上实现了89.67%的零样本平衡分类准确率，优于监督基线方法。通过专家评估证实，该模型显著提升了临床数据的解释质量和病理定位能力。

Conclusion: SVR框架通过合成数据生成有效解决了医学专业领域数据稀缺问题，为训练高性能、可解释的视觉语言模型提供了可行方案，在糖尿病视网膜病变诊断等医疗应用中展现出显著优势。
...
TL;DR: 该论文综述了基于事件相机在理解城市动态研究中的应用，分析了其优势、挑战及机器学习应用，并提出了多传感器融合方案。


<details>
  <summary>Details</summary>
Motivation: 传统城市监测方法（人工观察、摄像头等）存在局限性，需要更好的技术来理解城市动态。基于事件相机具有独特优势，如低光工作能力，可能成为城市动态研究的新媒介。

Method: 通过文献综述方法，分析基于事件相机的工作原理、应用场景、优势与挑战，以及机器学习应用。提出将基于事件相机与其他传感器（红外、LiDAR、振动传感器）融合的多传感器方案。

Result: 基于事件相机能够捕捉光强度变化而非RGB值，具有低光工作能力、隐私保护等优势，适合城市动态监测。多传感器融合可以增强其能力并克服现有挑战。

Conclusion: 基于事件相机是研究城市动态的有前景的媒介，结合多传感器融合可以进一步提升城市监测能力，为理解城市动态提供新方法。
...
TL;DR: VLM-IRIS：一种零样本框架，通过将红外图像预处理为RGB兼容输入，使视觉语言模型能够理解红外数据，用于工业红外传感应用
...
Method: 提出VLM-IRIS框架，将FLIR Boson传感器捕获的红外图像预处理为RGB兼容输入，转换为岩浆表示，并应用质心提示集成与CLIP ViT-B/32编码器，实现零样本预测

Result: 在3D打印机床上成功演示了零样本工件存在检测，利用构建板和工件之间的温度差异，通过热成像实现高精度检测，无需模型重新训练

Conclusion: 提出的VLM改进可以有效地扩展到热应用，实现无标签监控，为工业红外传感提供了实用的零样本解决方案
...
TL;DR: VGent提出了一种模块化编码器-解码器架构，通过解耦高级推理和低级边界框预测来解决视觉定位问题，避免了自回归解码的缺点，实现了快速推理和SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉定位模型存在两个主要问题：基于MLLM的自回归解码速度慢且容易产生幻觉；通过重新对齐LLM学习新特殊标记会损害预训练推理能力。需要一种既能保持强大推理能力又能快速准确预测边界框的解决方案。

Method: VGent采用模块化编码器-解码器架构：冻结的MLLM作为编码器保持推理能力；解码器以检测器提出的高质量边界框作为查询，通过交叉注意力在编码器隐藏状态中选择目标框。还引入了QuadThinker增强多目标推理能力、mask-aware标签解决检测-分割歧义、全局目标识别改进目标识别。

Result: 在多目标视觉定位基准测试中，VGent实现了新的SOTA，F1分数比先前方法提升+20.6%，在视觉参考挑战下gIoU提升+8.2%、cIoU提升+5.8%，同时保持恒定快速的推理延迟。

Conclusion: VGent通过解耦推理和定位任务，充分利用了目标检测和MLLM的进展，避免了自回归解码的缺陷，实现了高效准确的视觉定位，支持模块化升级，为视觉定位任务提供了新的解决方案。
...
TL;DR: 该研究提出了一种基于信息驱动的智能融合策略，用于整合多个病理学基础模型，通过相关性引导的特征剪枝来减少冗余，提升癌症分级和分期任务的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在病理学任务中表现出色，但对其互补性、嵌入空间冗余以及特征生物学解释的理解仍然有限，需要开发有效的多模型融合方法来提升性能。

Method: 研究提出智能融合策略，基于相关性引导的特征剪枝来整合多个病理学基础模型（包括tile级和slide级模型），在肾癌、前列腺癌和直肠癌的H&E全切片图像上进行评估，比较了多数投票集成、简单特征拼接和智能融合三种方法。

Result: 智能融合在所有三种癌症的分类性能上都优于最佳单模型和简单融合方法；全局相似性度量显示模型嵌入空间高度对齐，但局部邻域一致性较低，表明模型间存在互补的细粒度信息；注意力图显示智能融合能集中关注肿瘤区域，减少对良性区域的错误关注。
...
TL;DR: LEGO是一个三阶段后训练领域自适应框架，通过使用生成式oracle创建伪监督数据，使预训练图像修复模型能够适应未见过的真实世界退化域，无需配对数据或架构修改。


<details>
  <summary>Details</summary>
Motivation: 预训练图像修复模型在真实世界、分布外退化上表现不佳，因为存在显著的领域差距。适应这些未见领域具有挑战性，因为分布外数据缺乏真实标签，传统适应方法通常需要复杂的架构更改。

Method: 提出LEGO三阶段框架：1) 从预训练模型获取初始修复结果；2) 利用冻结的大规模生成式oracle将这些估计细化为高质量伪真实标签；3) 使用混合监督策略（结合分布内数据和新的伪配对）微调原始模型。

Result: 实验表明LEGO能有效弥合领域差距，在多样化的真实世界基准测试中显著提升性能，同时不牺牲原始模型的鲁棒性或需要架构修改。

Conclusion: LEGO将无监督挑战转化为可处理的伪监督问题，为预训练图像修复模型的后训练领域自适应提供了一种实用框架，无需配对数据即可适应新的分布，同时保持原始模型的优势。
...
TL;DR: Fast-FoundationStereo通过知识蒸馏、神经架构搜索和结构化剪枝等技术，在保持强零样本泛化能力的同时实现实时帧率，比FoundationStereo快10倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有的立体视觉基础模型虽然零样本泛化能力强但计算成本高，而高效的立体架构虽然速度快但需要昂贵的领域微调且牺牲了鲁棒性。需要一种既能保持强零样本泛化能力又能实时运行的方法。

Method: 采用分而治之的加速策略：1) 知识蒸馏将混合主干压缩为单个高效学生模型；2) 块级神经架构搜索自动发现最优成本过滤设计；3) 结构化剪枝消除迭代细化模块中的冗余。此外，引入自动伪标注管道收集140万真实世界立体图像对补充合成训练数据。

Result: 模型运行速度比FoundationStereo快10倍以上，同时零样本精度与之接近，在实时方法中建立了新的最先进水平。

Conclusion: Fast-FoundationStereo首次实现了在实时帧率下保持强零样本泛化能力，填补了基础模型与高效架构之间的性能差距。
...
TL;DR: ItemizedCLIP：一个从项目化文本监督中学习完整且可解释视觉表示的框架，特别适用于医学影像和遥感等非以对象为中心领域


<details>
  <summary>Details</summary>
Motivation: 许多视觉领域（如医学影像、遥感）包含项目化文本标注：单个图像中有多个描述不同且语义独立发现的文本项目。这种监督不同于标准的多标题监督，需要专门的方法来处理这种非冗余、非重叠的文本项目

Method: 使用交叉注意力模块生成文本项目条件化的视觉嵌入，并设计定制化目标函数，共同强制执行项目独立性（不同项目对应不同区域）和表示完整性（覆盖所有项目）

Result: 在四个具有自然项目化文本监督的领域（脑MRI、头CT、胸CT、遥感）和一个合成项目化数据集上，ItemizedCLIP在零样本性能和细粒度可解释性方面显著优于基线方法
...
TL;DR: 该研究对Monkey视觉语言模型进行了复现和批判性分析，验证了图像分块策略对恢复细节的有效性，同时研究了全局上下文的影响，发现结果偏差与任务类型和分块粒度密切相关。
...
Method: 使用开源检查点复现Monkey VLM策略，重新实现训练流程，验证图像分块恢复细节的关键发现，并进一步研究全局上下文包含的影响。

Result: 确认了原始Monkey VLM工作的关键发现：分块策略能有效恢复局部细节。同时发现结果存在偏差，这些偏差的大小严重依赖于任务类型和分块粒度。全局上下文的研究为未来高分辨率多模态建模提供了实用见解。

Conclusion: 图像分块是恢复高分辨率图像细节的有效策略，但模型性能受任务类型和分块粒度影响显著。全局上下文的纳入为优化多模态模型提供了重要方向，强调了可复现性和透明实现细节在复杂模型研究中的重要性。
...
TL;DR: 提出基于视频编解码器的轻量级3D高斯溅射压缩方法，通过两阶段Morton扫描和MiniPLAS优化，在保持高质量的同时大幅降低计算时间和编码时间。


<details>
  <summary>Details</summary>
Motivation: 现有基于视频的GS压缩方法使用PLAS转换3D GS到2D映射，计算成本高且耗时，限制了在轻量设备上的应用。
...
Conclusion: LGSCV方法在保持高质量压缩的同时，显著降低了计算复杂度和时间消耗，使3D高斯溅射技术更适合轻量级设备应用。
...
<details>
  <summary>Details</summary>
Motivation: 传统CAD建模耗时耗力，现有3D生成方法主要关注单部件生成，不适用于需要多部件组装的实际应用场景。

Method: 提出CADKnitter框架，采用几何引导的扩散采样策略，能够根据给定CAD模型的几何约束和设计文本的语义约束生成互补部件。

Result: 创建了包含31万样本的KnitCAD数据集，实验表明该方法明显优于现有最先进基线方法。

Conclusion: CADKnitter能够生成符合几何和语义约束的互补CAD部件，解决了多部件组装的实际应用需求。
...
TL;DR: AutoRefiner是一种专为自回归视频扩散模型设计的噪声优化器，通过路径化噪声优化和反射KV缓存机制，在推理时提升样本质量，无需更新模型参数。


<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型在实时交互应用中表现出色，但样本保真度仍有提升空间。现有的推理时对齐方法计算成本高，而文本到图像模型的噪声优化器无法直接应用于视频模型。

Method: 提出AutoRefiner，包含两个关键设计：1) 路径化噪声优化 - 沿着随机去噪路径优化噪声；2) 反射KV缓存 - 提高优化效率。该方法作为即插即用模块，无需训练模型参数。

Result: 实验证明AutoRefiner能有效提升自回归视频扩散模型的样本保真度，相比传统优化方法计算效率更高，可作为高效的即插即用增强模块。

Conclusion: AutoRefiner成功解决了文本到图像噪声优化器无法直接应用于视频模型的问题，为自回归视频扩散模型提供了一种高效的推理时样本质量提升方案。
...
TL;DR: 该研究提出了SmokeBench基准测试，用于评估多模态大语言模型在识别和定位野火烟雾方面的能力，发现现有模型在烟雾分类上表现尚可，但在早期烟雾定位方面存在严重不足。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个极端：基于像素的视频生成方法计算成本高且难以直接用于决策；而基于VFM特征的确定性回归方法虽然计算高效，但会平均多个可能的未来，无法捕捉不确定性，影响预测准确性。

Method: 提出生成式预测器，在VFM特征空间中进行自回归流匹配。关键创新是将VFM特征编码到适合扩散的紧凑潜在空间，该空间比之前使用的PCA方法能更有效地保留信息。潜在预测可以解码为多种有用输出模态。
...
TL;DR: FutureX：一个基于思维链的自动驾驶端到端规划器，通过未来场景潜在推理和轨迹优化来提升复杂运动规划能力
...
Method: 提出FutureX框架，包含自动思维开关、潜在世界模型和总结器模块。自动思维开关评估当前场景是否需要额外推理；需要时进入思维模式，潜在世界模型进行思维链引导的推演预测未来场景表示，总结器模块进一步优化运动规划；否则进入即时模式直接生成规划。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的说话头生成方法存在推理速度慢和非自回归范式的问题，严重限制了其在实际应用中的使用。需要开发能够实时生成且保持时间一致性的流式生成框架。

Method: 1) 通过高时空VAE压缩学习紧凑视频潜在空间；2) 引入ID-Context缓存机制，结合ID-Sink和Context-Cache原则进行键值缓存，以保持长时间流式生成中的时间一致性和身份一致性；3) 提出异步流式蒸馏训练策略，利用非流式教师模型和异步噪声调度来监督流式学生模型的训练，减少自回归生成中的误差累积。

Result: 实验结果表明，REST在生成速度和整体性能上都优于最先进的方法，成功弥合了自回归和基于扩散方法之间的差距。

Conclusion: REST是首个基于扩散模型的实时端到端流式音频驱动说话头生成框架，通过创新的缓存机制和训练策略，实现了实时生成并保持高质量的时间一致性和身份一致性，对需要实时说话头生成的应用具有重要价值。
...
TL;DR: RoomPilot是一个统一框架，能够将文本描述或CAD平面图等多样化多模态输入解析为室内领域特定语言，用于生成结构化室内场景，具有强可控性和交互语义。


<details>
  <summary>Details</summary>
Motivation: 现有室内场景生成方法要么处理输入模态范围有限，要么依赖随机过程导致可控性差。需要一种能够解析多种输入模态并生成具有交互语义的室内场景的统一框架。

Method: 引入RoomPilot框架，将文本描述或CAD平面图等多模态输入解析为室内领域特定语言，作为共享语义表示。利用带交互标注的资产数据集合成具有真实物体行为的环境，区别于传统仅生成视觉合理但功能惰性布局的方法。

Result: 实验验证了RoomPilot具有强大的多模态理解能力、细粒度场景生成可控性，以及优越的物理一致性和视觉保真度，标志着向通用可控3D室内场景生成迈出重要一步。

Conclusion: RoomPilot通过室内领域特定语言作为共享语义表示，实现了从单一模态生成连贯高质量室内场景的能力，同时保持交互语义，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 不完全多模态情感识别(IMER)旨在通过探索部分观测的多源数据来理解人类意图和情感。虽然多模态数据有望提供更丰富的信息，但在实践中，性能差距和模态欠优化问题阻碍了有效的多模态学习，并且在面对缺失数据时这些问题会进一步加剧。

Method: 设计了Cross-modal Prompting (ComP)方法，包括：1）带有动态梯度调制器的渐进提示生成模块，用于产生简洁一致的模态语义线索；2）跨模态知识传播，通过传递的提示选择性地放大模态特征中的一致信息以增强模态特定输出的区分度；3）协调器设计，动态重新加权模态输出作为平衡策略的补充以提高模型效能。
...
TL;DR: PersonaLive是一个面向实时直播场景的肖像动画框架，通过混合隐式信号、外观蒸馏和自回归微块流式生成，实现了7-22倍的加速，解决了现有扩散模型生成延迟高的问题。
...
Method: 1. 采用混合隐式信号（隐式面部表示和3D隐式关键点）实现表达性图像级运动控制；2. 提出较少步数的外观蒸馏策略消除去噪过程中的外观冗余，大幅提高推理效率；3. 引入自回归微块流式生成范式，配备滑动训练策略和历史关键帧机制，实现低延迟和稳定的长期视频生成。

Result: 实验表明PersonaLive实现了最先进的性能，相比之前的基于扩散的肖像动画模型实现了7-22倍的加速。

Conclusion: PersonaLive是一个创新的扩散框架，通过多阶段训练方案实现了流式实时肖像动画，解决了现有方法在直播场景中的延迟问题，具有显著的性能提升。
...
TL;DR: 本研究评估了多时相Sentinel-2卫星影像在景观尺度上监测入侵物种锯齿丛草的效果，发现其性能略优于航空影像，为大规模监测提供了成本效益更高的替代方案。


<details>
  <summary>Details</summary>
Motivation: 锯齿丛草是澳大利亚维多利亚州的主要入侵草种，对生态系统和农业造成严重威胁。当前的地面调查和小尺度管理方法无法满足景观尺度监测需求，而高分辨率航空影像成本过高。需要寻找成本效益更高、可扩展的监测方案。

Method: 使用多时相Sentinel-2卫星影像，结合光谱波段、纹理特征、植被指数和季节性数据，开发了11个不同特征组合的模型。采用随机森林分类器进行入侵物种分类。

Result: 最佳Sentinel-2模型（M76*）的总体准确率为68%，总体Kappa系数为0.55，略优于最佳航空影像模型的67%准确率和0.52 Kappa系数。这表明卫星影像在景观尺度监测中具有可比性。

Conclusion: 多时相特征增强的卫星模型为入侵物种的大规模监测提供了可行且成本效益高的替代方案，特别适合景观尺度的生态管理需求。
...
TL;DR: FilmWeaver是一个用于生成任意长度多镜头视频的新框架，通过自回归扩散范式解决多镜头视频生成中的一致性问题，采用双级缓存机制分离镜头间一致性和镜头内连贯性。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在单镜头合成上表现良好，但在多镜头视频生成中面临重大挑战：难以保持角色和背景在不同镜头间的一致性，以及灵活生成任意长度和镜头数量的视频。

Method: 1. 采用自回归扩散范式实现任意长度视频生成；2. 通过双级缓存机制分离一致性问题：镜头记忆缓存前序镜头关键帧以保持角色和场景身份，时间记忆保留当前镜头帧历史以确保平滑连续运动；3. 支持灵活的多轮用户交互；4. 开发高质量多镜头视频数据集训练流程。

Result: 实验结果表明，该方法在一致性和美学质量指标上均超越现有方法，支持多概念注入和视频扩展等下游任务，为创建更一致、可控和叙事驱动的视频内容开辟新可能性。

Conclusion: FilmWeaver通过创新的双级缓存机制有效解决了多镜头视频生成中的一致性问题，实现了任意长度多镜头视频的生成，在一致性和美学质量方面表现优异，具有高灵活性和多功能性。
...
TL;DR: 提出端到端可训练的任务自适应视频关键帧选择框架，解决传统独立评分方法导致的帧冗余和静态伪标签问题，通过链式思维、连续集级目标函数和师生互学习实现动态优化。


<details>
  <summary>Details</summary>
Motivation: 传统视频关键帧选择方法存在两个主要问题：1）独立评分方法（top-K）无法整体优化选择，导致选择的时间聚类和视觉冗余帧；2）使用离线生成的MLLM伪标签训练轻量级选择器，监督信号无法动态适应任务目标。

Method: 1）使用链式思维方法引导小型语言模型生成任务特定的隐式查询向量，与多模态特征结合实现动态帧评分；2）定义包含相关性、覆盖度和冗余度的连续集级目标函数，通过Gumbel-Softmax实现可微分优化；3）采用师生互学习，通过KL散度对齐学生选择器（SLM）和教师推理器（MLLM）的帧重要性分布，结合交叉熵损失实现端到端优化。

Result: 在Video-MME、LongVideoBench、MLVU和NExT-QA等多个基准测试中，该方法显著优于现有方法。

Conclusion: 提出的端到端可训练任务自适应框架有效解决了传统关键帧选择方法的局限性，通过动态查询生成、集级优化和师生互学习实现了更优的帧选择性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法主要针对纯文本LLM，而MLLM的遗忘研究仍处于早期阶段。需要开发专门的方法来选择性擦除MLLM中的目标视觉知识，同时保持文本知识完整性。

Method: 提出视觉知识蒸馏（VKD）方案，利用MLLM内部的中间视觉表示作为监督信号，仅微调模型的视觉组件。通过解耦MLLM中嵌入的视觉和文本知识，选择性擦除目标视觉知识。

Result: 实验表明该方法在有效性和效率方面优于现有最先进的遗忘方法。首次评估了MLLM遗忘对重新学习攻击的鲁棒性，证明了方法的优越性。

Conclusion: 该方法为MLLM提供了一种高效、有效的选择性视觉知识遗忘方案，通过视觉知识蒸馏和仅微调视觉组件的设计，在保持模型实用性的同时实现目标知识擦除。
...
TL;DR: ARVAE是一种自回归视频自编码器，通过解耦时空表示来提升视频重建质量，支持任意长度视频处理，在轻量模型和小规模数据下实现优异性能。
...
Conclusion: ARVAE通过自回归框架和时空解耦表示，有效解决了现有视频自编码器的局限性，为高质量视频重建和生成提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在牙科领域存在两个主要问题：1）难以捕捉细粒度的牙科视觉细节；2）缺乏进行精确诊断所需的充分推理能力。这些限制阻碍了牙科自动化医疗保健的发展。

Method: 1）构建了迄今为止最大的牙科多模态标注数据集，包含超过12万张牙科图像及其详细描述；2）通过高质量领域知识注入训练增强模型对牙科视觉特征的理解；3）采用强化学习阶段进一步强化多模态复杂推理能力。

Result: DentalGPT在口腔内和全景影像基准测试以及牙科医学VQA基准测试子集上表现出卓越性能，在疾病分类和牙科视觉问答任务中优于许多最先进的多模态大语言模型，尽管只有70亿参数。

Conclusion: 高质量牙科数据结合分阶段适应为构建能力强、领域专用的牙科多模态大语言模型提供了有效途径，证明了领域专业化模型在牙科医疗保健中的价值。
...
TL;DR: 本文提出了一种基于Transformer架构的交通事故检测模型，通过整合RGB特征和光流运动线索，在综合平衡的数据集上实现了88.3%的准确率。


<details>
  <summary>Details</summary>
Motivation: 交通事故是全球主要死亡原因，传统计算机视觉方法在时空理解和跨域泛化方面存在局限。现有Transformer模型应用受限于小规模、非多样化数据集，且多数研究忽视了对动态场景理解至关重要的运动线索整合。

Method: 1) 构建了涵盖广泛交通环境、事故类型和情境变化的综合平衡数据集；2) 提出基于Transformer架构的事故检测模型，使用预提取的空间视频特征；3) 采用卷积层提取帧内局部相关性，利用Transformer捕获特征间的序列-时间依赖；4) 评估多种运动线索整合方法，最终采用RGB特征与光流拼接策略。

Result: RGB特征与光流拼接的方法取得了最高准确率88.3%。与GPT、Gemini、LLaVA-NeXT-Video等视觉语言模型相比，提出的方法表现出更好的效果。

Conclusion: 提出的基于Transformer的交通事故检测模型通过整合运动线索，在综合数据集上实现了高精度检测，为开发鲁棒、可泛化的自动化交通监控系统提供了有效解决方案。
...
TL;DR: Flowception是一个非自回归、可变长度的视频生成框架，通过交替离散帧插入和连续帧去噪来学习概率路径，相比自回归方法减少误差累积，相比全序列流方法降低计算成本。
...
Method: Flowception学习一个概率路径，交替进行离散帧插入和连续帧去噪。帧插入机制在采样过程中作为高效的压缩机制处理长时上下文，同时联合学习视频长度和内容，支持局部注意力变体。

Result: 相比自回归和全序列基线方法，Flowception在FVD和VBench指标上表现更好，训练FLOPs减少三倍，并能无缝集成图像到视频生成和视频插值等不同任务。

Conclusion: Flowception通过创新的离散-连续混合概率路径设计，在视频生成任务中实现了更好的性能、更高的计算效率和更强的任务集成能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的重建数据集主要关注静态多视角或单视角自我中心设置，缺乏用于动态场景重建的多视角自我中心数据集，而这类数据在全息社交互动记录中具有重要应用价值。

Method: 设计了基于硬件的采集系统和处理流程，实现了跨视角的亚毫秒级时间同步，并提供精确的姿态标注。数据集包含5个典型社交互动场景（会议、表演、演示），每个场景由参与者佩戴AR眼镜捕获5个真实自我中心视频。

Result: 实验验证表明该数据集在自由视点视频（FVV）应用中具有实际效用和有效性，为推进多视角自我中心动态场景重建研究建立了基础资源。

Conclusion: MultiEgo填补了多视角自我中心动态场景重建数据集的空白，为相关研究提供了首个基础数据集，在社交互动全息记录等应用中具有重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统高精地图构建面临车载传感器能力有限和遮挡问题，导致输入数据质量低、地图构建不完整。卫星图像虽然提供广域视角，但受阴影和植被建筑遮挡影响，现有特征提取和融合方法效果有限。

Method: 提出SATMapTR模型，包含两个核心组件：1) 门控特征精炼模块，通过整合高层语义与低层结构线索自适应过滤卫星图像特征；2) 几何感知融合模块，在网格级别一致融合卫星和BEV特征，最小化无关区域和低质量输入的干扰。

Result: 在nuScenes数据集上，SATMapTR达到73.8的mAP，比现有卫星增强模型提升高达14.2 mAP。在恶劣天气和传感器故障下表现出更低的mAP退化，在扩展感知范围内达到近3倍的mAP提升。

Conclusion: SATMapTR通过有效的卫星图像融合机制，显著提升了在线地图构建的准确性和鲁棒性，特别是在复杂场景和远距离感知方面表现优异，为自动驾驶提供了更可靠的地图支持。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉理解方面表现出色，能够解析复杂布局、属性和知识密集型场景，但在图像或视频生成方面，这些强大的核心模型通常只被用作扩散模型的全局文本编码器，大部分推理和规划能力未被充分利用，导致生成无法达到同等精确和结构化控制水平。

Method: 提出MetaCanvas框架，让多模态大语言模型直接在空间和时空潜在空间中进行推理和规划，并与扩散生成器紧密接口。该框架在三种不同的扩散模型骨干上实现，并在六个任务上进行评估。

Result: MetaCanvas在六个任务上（包括文本到图像生成、文本/图像到视频生成、图像/视频编辑和上下文视频生成）始终优于全局条件化基线方法，这些任务都需要精确布局、鲁棒属性绑定和推理密集型控制。

Conclusion: 将多模态大语言模型视为潜在空间规划器是缩小多模态理解与生成之间差距的有前景方向，MetaCanvas框架通过让MLLMs直接在潜在空间中进行推理和规划，实现了更精确和结构化的视觉生成控制。
...
TL;DR: KeyframeFace是一个用于文本到动画研究的大规模多模态数据集，通过关键帧级监督提供表情脚本、视频、ARKit系数等数据，并提出首个利用LLM先验进行可解释面部运动合成的文本到动画框架。
...
Method: 1) 构建KeyframeFace数据集，包含2100个表情脚本、单目视频、逐帧ARKit系数、上下文背景、复杂情感、手动定义关键帧以及基于LLM/MLLM的多视角标注；2) 提出首个利用LLM先验的文本到动画框架，将LLM的语义理解能力与ARKit系数的可解释结构对齐。

Result: KeyframeFace数据集为文本到动画研究提供了大规模多模态资源，提出的LLM-based框架实现了高保真度的表情动画生成，为可解释、关键帧引导、上下文感知的文本到动画建立了新基础。

Conclusion: KeyframeFace数据集和LLM-based框架共同为可解释、关键帧引导、上下文感知的文本到动画研究提供了新的基础，代码和数据已开源。
...
TL;DR: 提出了一种处理多帧卫星图像时间序列的方法，通过并行处理并交换时间信息来稳定预测，改进了冰川崩解前缘的自动检测精度


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习模型在自动检测SAR图像中冰川崩解前缘时，难以准确分类受季节性条件（如冰混合物或雪覆盖表面）影响的区域，需要改进模型的稳定性

Method: 提出并行处理同一冰川卫星图像时间序列的多帧图像，并在对应特征图之间交换时间信息以稳定每个预测，将方法集成到当前最先进的Tyrion架构中

Result: 在CaFFe基准数据集上实现了新的最先进性能：平均距离误差184.4米，平均交并比83.6

Conclusion: 通过处理多帧时间序列图像并交换时间信息，能够显著提高冰川崩解前缘检测的稳定性和准确性，特别是在处理受季节性条件影响的区域时表现更好
...
TL;DR: FreqDINO是一个频率引导的超声图像分割框架，通过多尺度频率提取对齐、频率引导边界细化和多任务边界引导解码器，增强边界感知和结构一致性，在超声图像分割中超越现有方法。
...
Method: 1. 多尺度频率提取与对齐(MFEA)：分离低频结构特征和多尺度高频边界细节，并通过可学习注意力对齐；2. 频率引导边界细化(FGBR)：从高频分量提取边界原型并细化空间特征；3. 多任务边界引导解码器(MBGD)：确保边界预测和语义预测之间的空间一致性。

Result: 大量实验表明FreqDINO超越了最先进的方法，具有卓越的泛化能力。

Conclusion: FreqDINO通过频率引导的方法有效解决了超声图像分割中的边界感知问题，提高了分割性能并展现出强大的泛化能力。
...
TL;DR: UFVideo是首个具备统一多粒度协同理解能力的视频大语言模型，通过统一的视觉-语言引导对齐机制，在单一模型中实现全局、像素和时间尺度的视频理解，支持文本响应、时间定位和掩码生成。
...
Method: 设计了统一的视觉-语言引导对齐机制，能够灵活处理全局、像素和时间尺度的视频理解。模型动态编码不同任务的视觉和文本输入，生成文本响应、时间定位或掩码。构建了UFVideo-Bench基准测试来评估多粒度视频理解任务。

Result: UFVideo在UFVideo-Bench的三个协作任务中展现出优于GPT-4o的灵活性和优势。在9个公共基准测试中验证了模型在各种常见视频理解任务中的有效性。

Conclusion: UFVideo是首个统一多粒度协同理解的视频大语言模型，为未来视频大语言模型的发展提供了有价值的见解，展示了在单一模型中实现全面视频理解的可行性。
...
TL;DR: 该研究首次系统评估了AI生成图像模型指纹检测技术的安全性，发现现有方法在对抗攻击下存在严重脆弱性，揭示了准确性与鲁棒性之间的权衡关系。
...
Method: 研究形式化了威胁模型（白盒和黑盒访问），设定了两种攻击目标：指纹移除（消除识别痕迹）和指纹伪造（导致错误归因）。实现了五种攻击策略，在12个最先进的图像生成器上评估了14种代表性的指纹检测方法，涵盖RGB、频率和学习特征三个领域。

Result: 实验揭示了干净环境和对抗环境下的显著性能差距。移除攻击效果显著，白盒设置下成功率常超过80%，受限黑盒访问下也超过50%。伪造攻击虽更具挑战性，但针对不同目标模型的成功率差异显著。研究发现准确率最高的方法往往最易受攻击，没有一种方法能在所有威胁模型中同时实现高鲁棒性和高准确性。

Conclusion: 现有模型指纹检测技术在对抗攻击下存在严重安全漏洞，需要在准确性和鲁棒性之间取得平衡。研究结果为开发更安全的指纹检测技术指明了方向，并识别了最有前景的改进方法。
...
TL;DR: 提出一个完全自动化的单目RGB视频动态场景重建流水线，通过增强动态高斯溅射的先验来提升重建质量


<details>
  <summary>Details</summary>
Motivation: 从随意拍摄的单目RGB视频中自动重建动态场景，无需设计新的场景表示方法，而是通过改进先验来提升现有动态高斯溅射方法的性能

Method: 1) 视频分割结合极线误差图生成物体级掩码；2) 掩码指导物体深度损失函数锐化深度图；3) 基于骨架的采样和掩码引导的重识别产生可靠的2D轨迹；4) 虚拟视角深度损失去除漂浮物；5) 脚手架投影损失将运动节点与轨迹绑定

Result: 系统超越了之前的单目动态场景重建方法，在渲染质量上取得显著提升

Conclusion: 通过增强先验并嵌入到重建阶段的动态高斯溅射方法，能够从单目RGB视频中自动重建出高质量的动态场景
...
<details>
  <summary>Details</summary>
Motivation: 无人机高效作物检测对精准农业规模化至关重要，但由于目标尺寸小和环境变化大，检测仍具挑战性。本文针对稻田水稻幼苗检测问题，旨在解决高分辨率航拍图像中小目标检测的特定困难。

Method: 采用基于迁移学习的Faster R-CNN架构，构建了大规模的无人机数据集进行训练，并在三个不同时间间隔获取的测试集上严格评估模型的泛化能力，以验证对图像采集条件变化的鲁棒性。

Result: 实证结果表明，迁移学习不仅促进了农业场景中目标检测模型的快速收敛，而且在图像采集领域偏移的情况下仍能保持一致的性能表现。

Conclusion: 该方法为无人机高效作物检测提供了一种有效的解决方案，特别是在处理小目标和环境变化方面表现出良好的鲁棒性和泛化能力。
...
TL;DR: 提出ARNet-v2模型解决伪装目标检测中的两个关键问题：通过通道信息交互模块增强同层特征跨通道交互，通过协同解码架构结合边界和区域信息，在多个数据集上达到SOTA性能。
...
Method: 提出通道信息交互模块（CIIM）实现通道维度上的水平-垂直整合机制；构建先验知识引导的协同解码架构，包含边界提取（BE）和区域提取（RE）模块生成先验，使用混合注意力协同校准解码特征；引入多尺度增强（MSE）模块丰富上下文特征表示。

Result: 在四个COD基准数据集上验证了模型的有效性和最先进性能；将模型迁移到显著目标检测（SOD）任务，并展示了在息肉分割、透明目标检测、工业和道路缺陷检测等下游任务中的适应性。
...
TL;DR: 论文提出N-Body问题：如何让N个人并行执行单视角视频中的任务，最大化加速比，同时避免物理冲突。通过结构化提示策略指导视觉语言模型进行3D环境推理，在EPIC-Kitchens数据集上实现45%动作覆盖提升和显著冲突降低。


<details>
  <summary>Details</summary>
Motivation: 人类能直观并行化复杂活动，但模型能否从观察单个人中学习这种能力？研究探索如何让N个人并行执行单视角视频中的任务，同时解决物理约束冲突问题。

Method: 形式化N-Body问题，提出评估指标（性能：加速比、任务覆盖；可行性：空间碰撞、物体冲突、因果约束）。引入结构化提示策略，指导视觉语言模型进行3D环境、物体使用和时间依赖推理，生成可行的并行执行方案。

Result: 在EPIC-Kitchens和HD-EPIC的100个视频上，N=2时，相比Gemini 2.5 Pro基线提示，动作覆盖提升45%，同时碰撞率、物体冲突和因果冲突分别降低55%、45%和55%。

Conclusion: 提出的结构化提示策略能有效指导视觉语言模型推理物理约束，生成可行的多人并行执行方案，显著提升任务覆盖并减少冲突。
...
TL;DR: FlowDC提出了一种并行解耦的复杂图像编辑方法，将多目标编辑分解为多个子编辑效果并行叠加，并通过速度分解提升源图像一致性。


<details>
  <summary>Details</summary>
Motivation: 当前复杂图像编辑方法存在局限性：单轮编辑受限于长文本跟随能力，多轮编辑存在累积不一致问题，难以在语义对齐和源图像一致性之间取得平衡。

Method: 1. 将复杂编辑解耦为多个子编辑效果，在编辑过程中并行叠加；2. 观察到与编辑位移正交的速度分量会损害源结构保持，因此分解速度并衰减正交部分以提升源一致性。

Result: 在构建的复杂编辑基准Complex-PIE-Bench和另一个基准上，FlowDC相比现有方法表现出优越的结果，并通过消融实验验证了模块设计的有效性。

Conclusion: FlowDC通过并行解耦编辑和速度分解机制，有效解决了复杂图像编辑中语义对齐与源一致性的平衡问题，为多目标编辑提供了有效解决方案。
...
TL;DR: JoyAvatar：一种音频驱动的自回归模型，能够实时推理和生成无限长度视频，通过渐进步长引导、运动条件注入和缓存重置的无界RoPE来解决现有方法的计算开销大和长视频生成问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于DiT的音频驱动虚拟人生成方法虽然取得进展，但受限于高计算开销和无法生成长时视频。自回归方法通过分块自回归扩散解决此问题，但存在误差累积和质量下降问题。
...
Conclusion: JoyAvatar通过创新的渐进步长引导、运动条件注入和动态位置编码技术，实现了实时推理和无限长度视频生成，解决了现有音频驱动虚拟人生成方法的局限性。
...
TL;DR: 提出半自动标注流水线改进YawDD数据集质量，提升疲劳驾驶检测中打哈欠识别性能，在边缘设备上实现实时监测


<details>
  <summary>Details</summary>
Motivation: 驾驶员疲劳是道路事故的主要原因，打哈欠是疲劳的早期行为指标，但现有机器学习方法面临视频标注数据集引入的粗粒度时间标注系统噪声问题

Method: 开发半自动标注流水线，结合人在回路验证，应用于YawDD数据集；训练MNasNet分类器和YOLOv11检测器架构

Result: YawDD+数据集上帧准确率提升6%，mAP提升5%，达到99.34%分类准确率和95.69%检测mAP；在NVIDIA Jetson Nano上实现59.8 FPS实时性能

Conclusion: 仅通过提升数据质量即可支持无需服务器端计算的边缘设备打哈欠监测，证实了高质量标注对疲劳驾驶检测的重要性
...
TL;DR: DOS是一个自监督学习框架，通过仅在可观测点蒸馏语义相关性软图来学习3D点云表示，避免了掩码区域的信息泄露，并在无监督设置中通过Zipfian原型和Zipf-Sinkhorn算法解决语义不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 3D点云的自监督学习面临几何不规则、重建易走捷径和语义分布不平衡等挑战。现有方法存在信息泄露问题，且离散的token-to-prototype分配提供的监督信息有限。

Method: 提出DOS框架：1）仅在可观测点蒸馏语义相关性软图，避免掩码区域信息泄露；2）引入Zipfian原型，通过Zipf-Sinkhorn算法（改进的Sinkhorn-Knopp算法）强制原型使用遵循幂律分布，调节目标软图的锐度。

Result: 在多个基准测试（nuScenes、Waymo、SemanticKITTI、ScanNet、ScanNet200）上，DOS在语义分割和3D目标检测任务中超越了当前最先进方法，且不依赖额外数据或标注。
...
TL;DR: CADMorph：基于预训练领域基础模型的几何驱动参数化CAD编辑框架，通过规划-生成-验证三阶段迭代，无需编辑数据三元组即可实现结构保持、语义有效和高形状保真度的CAD模型编辑。


<details>
  <summary>Details</summary>
Motivation: 在CAD设计迭代过程中，几何形状调整需要同步编辑底层参数序列，这面临三大挑战：保持原始序列结构、确保编辑语义有效性、维持高形状保真度，且缺乏编辑数据三元组。

Method: 提出CADMorph框架，包含规划-生成-验证三阶段：1) 规划阶段使用参数到形状(P2S)扩散模型的交叉注意力图定位需修改段并提供编辑掩码；2) 生成阶段使用掩码参数预测(MPP)模型填充语义有效编辑；3) 验证阶段通过P2S模型在形状潜在空间测量候选序列与目标形状距离并选择最佳。

Result: CADMorph超越GPT-4o和专用CAD基线，支持迭代编辑和逆向工程增强等下游应用。P2S和MPP模型均无需三元组数据训练，克服了数据稀缺瓶颈。

Conclusion: CADMorph框架通过协调预训练领域基础模型，有效解决了几何驱动参数化CAD编辑中的结构保持、语义有效性和形状保真度三大挑战，为CAD设计迭代提供了高效解决方案。
...
TL;DR: 提出两种方法将事件相机数据与多模态大语言模型结合：FRT（基于帧的重建与标记化）和ART（自适应重建与标记化），并创建首个客观真实世界基准EvQA进行评测。


<details>
  <summary>Details</summary>
Motivation: 事件相机在挑战性视觉条件下具有优势，但需要平衡保留事件数据独特优势与兼容基于帧的模型之间的矛盾，以实现在困难视觉条件下的通用场景理解。

Method: 提出两种方法：1）FRT：基于帧的重建与标记化方法；2）ART：利用事件稀疏性的自适应重建与标记化方法。同时创建EvQA基准，包含来自22个公共数据集的1000个事件-QA对。

Result: 实验表明，所提方法在EvQA基准上达到最先进性能，证明了多模态大语言模型在事件视觉中的巨大潜力。

Conclusion: 通过重建作为桥梁，成功将事件相机数据与多模态大语言模型结合，在保持事件数据优势的同时实现与基于帧模型的兼容，为事件视觉的通用场景理解开辟了新途径。
...
Method: 提出THREASURE-Net框架，使用Sentinel-2时间序列数据，基于LiDAR HD数据在多个空间分辨率下训练，生成年度树高地图。模型有三个变体，分别产生2.5米、5米和10米分辨率的预测。

Result: THREASURE-Net在2.5米、5米和10米分辨率下的平均绝对误差分别为2.62米、2.72米和2.88米。模型性能优于基于Sentinel数据的现有方法，与基于超高分辨率图像的方法相当。

Conclusion: THREASURE-Net展示了仅使用免费卫星数据进行温带森林结构监测的潜力和成本效益，能够生成高精度的年度冠层高度地图，具有可扩展性。
...
Result: Infinity-8B在两个基准测试中都实现了最强的整体组合对齐能力，而Infinity-2B在多个类别中也匹配或超过了更大的扩散模型，显示出良好的效率-性能权衡。相比之下，SDXL和PixArt-α在属性敏感和空间任务中表现出持续的弱点。
...
TL;DR: SSL-MedSAM2是一个创新的半监督学习框架，结合了基于预训练大模型SAM2的训练无关少样本学习分支和基于nnUNet的迭代全监督学习分支，用于医学图像分割，在肝脏分割任务上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像标注耗时且成本高，限制了深度学习模型在临床中的应用。虽然全监督学习方法在医学图像分割中表现出色，但需要大规模标注数据集。半监督学习成为减少标注成本的吸引人策略。

Method: 提出SSL-MedSAM2框架，包含两个分支：1) TFFS-MedSAM2分支：基于预训练大基础模型Segment Anything Model 2 (SAM2)的训练无关少样本学习分支，用于生成伪标签；2) FSL-nnUNet分支：基于nnUNet的迭代全监督学习分支，用于伪标签细化。两个分支协同工作，实现半监督学习。

Result: 在MICCAI2025挑战赛CARE-LiSeg（肝脏分割）任务中表现出色。在测试集上，GED4和T1 MRI的平均Dice分数分别为0.9710和0.9648，Hausdorff距离分别为20.07和21.97，优于其他方法。

Conclusion: SSL-MedSAM2框架有效解决了医学图像标注成本高的问题，通过结合预训练大模型和迭代全监督学习，在有限标注数据下实现了优异的肝脏分割性能，具有临床应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 3D牙齿分割是数字牙科中的关键但具有挑战性的任务，由于真实世界牙列的复杂性，需要准确识别牙齿实例并进行语义分类。现有方法面临精度和效率的挑战。

Method: 1. 将3D牙齿模型从预定义视角渲染为2D图像；2. 使用SAM2进行2D分割；3. 通过2D-3D投影重建3D结果；4. 引入三个轻量级可学习模块：提示嵌入生成器、掩码精炼器和掩码分类器；5. 在SAM2图像编码器中加入可变形全局注意力插件（DGAP）提升精度和训练速度。

Result: 在3DTeethSeg基准测试中，高分辨率3D牙齿网格上达到91.90%的IoU，创下了该领域新的最先进水平。

Conclusion: 3DTeethSAM成功将SAM2基础模型适配到3D牙齿分割任务，通过创新的多视图渲染和轻量级模块设计，显著提升了分割精度，为数字牙科提供了有效的解决方案。
...
TL;DR: 该论文提出了一个无需微调的3D场景理解基准测试，用于评估基础模型在密集视觉特征质量方面的内在3D推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常依赖下游任务的微调，难以隔离预训练编码器的内在3D推理能力，而3D空间理解对于机器人和自动驾驶等实际应用至关重要。
...
Conclusion: 提出了一个无需微调的3D场景理解基准，能够直接评估预训练编码器的密集视觉特征质量，为3D空间理解提供了更准确的评估工具。
...
TL;DR: 本文提出ContextSeisNet，一种基于上下文学习的模型，用于地震多次波压制处理，通过利用空间相关的示例对实现用户可控和空间一致的结果。


<details>
  <summary>Details</summary>
Motivation: 传统地震处理方法面临噪声数据、手动参数调优等挑战，现有深度学习方法存在空间不一致性和缺乏用户控制的问题。需要一种既能提供空间一致性又能让用户灵活控制的地震处理解决方案。

Method: 提出ContextSeisNet，采用上下文学习方法，在推理时通过观察空间相关的示例对（相邻共深度点道集及其对应标签）来学习任务特定的处理行为，无需重新训练。该方法利用用户定义的示例提供灵活性，并改善地震测线的横向一致性。

Result: 在合成数据上，ContextSeisNet在定量指标上优于U-Net基线，并显示出增强的相邻道集空间相干性。在野外数据上，相比传统Radon多次波压制和U-Net基线，模型实现了更好的横向一致性。相对于U-Net，ContextSeisNet还提供了改进的近偏移距性能和更完整的多次波去除。值得注意的是，ContextSeisNet在训练数据减少90%的情况下仍能达到相当的野外数据性能，显示出显著的数据效率。

Conclusion: ContextSeisNet为空间一致的地震多次波压制提供了一种实用方法，并具有扩展到其他地震处理任务的潜力。该方法通过上下文学习实现了用户控制和空间一致性的平衡，同时展现出优秀的数据效率。
...
TL;DR: 本文首次提出"具身图像压缩"这一科学问题，针对具身AI在多智能体系统中的通信约束和实时任务执行需求，建立了标准化基准EmbodiedComp，并在超低比特率条件下验证了现有视觉-语言-动作模型在压缩后的性能下降问题。
...
Method: 建立了标准化基准EmbodiedComp，在闭环设置下进行超低比特率条件的系统评估。通过在模拟和真实环境中的广泛实证研究，评估现有视觉-语言-动作模型在压缩后的性能表现。

Result: 实证研究表明，当压缩比特率低于具身比特率阈值时，现有的视觉-语言-动作模型甚至无法可靠地执行简单的操作任务，显示出当前压缩技术对具身AI应用的局限性。

Conclusion: EmbodiedComp基准将推动针对具身智能体的领域特定压缩技术的发展，从而加速具身AI在真实世界中的部署应用。具身图像压缩是视觉数据压缩领域的重要研究方向。
...
TL;DR: 提出基于高斯显式表示的快速3D医学图像重建方法，替代传统隐式神经表示，通过解析解实现5-10倍加速


<details>
  <summary>Details</summary>
Motivation: 在胎儿MRI等医学影像中，从低分辨率2D图像重建高分辨率3D图像对神经发育诊断至关重要。现有基于隐式神经表示的方法存在计算瓶颈，需要昂贵的蒙特卡洛采样来模拟点扩散函数，限制了临床实时应用。

Method: 将高分辨率3D图像参数化为各向异性高斯基元的场，利用高斯函数在卷积下闭合的特性，推导出前向模型的闭式解析解。该方法将原本难以处理的采集积分简化为精确的协方差加法，避免了计算密集的随机采样，同时确保精确的梯度传播。

Result: 在新生儿和胎儿数据上，该方法达到了自监督最先进切片到体积重建框架的重建质量，同时实现了5-10倍的加速。收敛时间通常在30秒以内，为实时胎儿3D MRI进入临床常规应用铺平了道路。

Conclusion: 基于高斯显式表示的方法通过解析解显著加速了3D医学图像重建，在保持重建质量的同时大幅提升计算效率，有望推动实时胎儿MRI在临床中的实际应用。
...
Method: 使用预训练图像编码器从驱动视频提取面部表情潜在表示作为控制信号；提出表情控制器将潜在表示注入视频扩散变换器；使用Plücker射线图和法线图控制摄像机和头部姿态；构建大规模合成数据集进行训练。

Result: 方法在真实性、表现力、控制精度和视角一致性方面优于现有方法，能够从单张肖像图像生成逼真的动画，同时控制面部表情、头部运动和摄像机视角。
...
TL;DR: KineMIC是一个迁移学习框架，通过文本编码空间的语义对应关系，将通用的文本到动作生成模型适配到人体动作识别领域，实现少样本动作合成，显著提升分类器性能。


<details>
  <summary>Details</summary>
Motivation: 大规模标注动作数据集的获取成本是骨骼动作识别的主要瓶颈。虽然文本到动作生成模型提供了可扩展的合成数据源，但其训练目标和数据集结构与动作识别对运动学精确、类别区分性动作的需求存在显著差异，导致领域差距。
...
Result: 使用HumanML3D作为源数据集，NTU RGB+D 120子集作为目标领域，每类仅随机选择10个样本。该方法生成的动作更加连贯，作为数据增强源使准确率提升23.1个百分点。

Conclusion: KineMIC成功弥合了通用文本到动作生成模型与动作识别需求之间的领域差距，通过少样本学习实现高质量动作合成，显著提升动作识别性能，为数据稀缺问题提供了有效解决方案。
...
TL;DR: CLV-Net是一个用于遥感图像理解的跨模态视觉提示引导多模态模型，通过简单的视觉提示（边界框）来引导生成相关的分割掩码和描述，解决现有方法在通用文本提示下难以定位用户相关区域的问题。
...
Method: 提出CLV-Net模型，允许用户提供简单的视觉提示（边界框）来指示感兴趣区域，并利用该提示引导模型生成相关的分割掩码和描述。核心设计包括：1）上下文感知掩码解码器，建模和整合物体间关系以增强目标表示；2）语义和关系对齐模块，包含跨模态语义一致性损失（增强视觉相似目标的细粒度区分）和关系一致性损失（强制文本关系与视觉交互的对齐）。

Result: 在两个基准数据集上的综合实验表明，CLV-Net优于现有方法，建立了新的最先进结果。模型能有效捕捉用户意图，并产生精确、意图对齐的多模态输出。

Conclusion: CLV-Net通过视觉提示引导和跨模态上下文感知学习，成功解决了遥感图像理解中用户意图捕捉和视觉相似目标识别的挑战，为多模态遥感图像分析提供了有效的解决方案。
...
TL;DR: Depth Copy Paste：一种多模态深度感知的数据增强框架，通过复制完整人体实例并粘贴到语义兼容的场景中，生成多样且物理一致的人脸检测训练样本。


<details>
  <summary>Details</summary>
Motivation: 传统复制粘贴增强方法由于前景提取不准确、场景几何不一致和背景语义不匹配，常常产生不真实的合成图像。需要解决这些限制，为具有挑战性条件下的人脸检测系统提供更鲁棒的训练数据。
...
Conclusion: Depth Copy Paste框架通过多模态语义匹配、高质量前景分割和深度感知的几何对齐，生成了物理一致且视觉逼真的合成图像，有效提升了人脸检测系统的鲁棒性。
...
Method: 采用四阶段处理流程：1)图像采集与预处理；2)使用DBNet++模型检测文本元素；3)使用BART模型对检测到的文本元素进行分类；4)通过Python和PyQt5编写的用户界面展示结果。支持图库模式和实时模式两种工作方式。

Result: 在Total-Text数据集上进行了10小时测试，文本识别率达到94.62%。该系统能够有效处理各种问题条件下的图像，包括高分辨率图像和多种挑战性场景。

Conclusion: 提出的方法在实际应用中有效，即使在非受控成像条件下也能实现混合来源的文本分类，系统设计解决了实际应用中的多种挑战性问题。
...
TL;DR: EditMGT：首个基于掩码生成Transformer的图像编辑框架，通过局部化解码范式实现精确区域编辑，避免非目标区域修改，比扩散模型更快更准确。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像编辑中虽然视觉质量优秀，但其全局去噪机制会将局部编辑目标与全图上下文混淆，导致非目标区域出现意外修改。需要寻找替代方法来解决这一挑战。

Method: 提出EditMGT框架：1) 利用MGT的交叉注意力图提供定位信号，设计多层注意力整合方案实现细粒度精确定位；2) 引入区域保持采样，限制在低注意力区域进行token翻转以抑制虚假编辑；3) 构建CrispEdit-2M高分辨率数据集；4) 通过注意力注入将预训练文本到图像MGT适配为图像编辑模型。

Result: 在四个标准基准测试中，仅用不到10亿参数就实现了相似性能，编辑速度提升6倍。在风格变化和风格迁移任务上分别提升3.6%和17.6%，编辑质量相当或更优。

Conclusion: MGT的局部化解码范式为图像编辑提供了新方向，EditMGT框架通过精确区域定位和区域保持采样，有效解决了扩散模型中非目标区域修改问题，实现了高效高质量的局部图像编辑。
...
TL;DR: 该论文提出了一种新的音乐到舞蹈生成方法，将2D姿态序列编码为单热图像，使用预训练图像VAE压缩，并用DiT风格主干建模，从而更好地捕捉高方差姿态分布。
...
Method: 将音乐到舞蹈生成重新定义为音乐令牌条件多通道图像合成问题：将2D姿态序列编码为单热图像，用预训练图像VAE压缩，用DiT风格主干建模；引入时间共享的时间索引方案显式同步音乐令牌和姿态潜在表示，以及参考姿态条件策略保持主体特定身体比例和屏幕尺度。

Result: 在大型野外2D舞蹈语料库和校准的AIST++2D基准测试中，相比代表性音乐到舞蹈方法，在姿态和视频空间指标以及人类偏好方面均显示一致改进，消融实验验证了表示、时间索引和参考条件策略的贡献。

Conclusion: 通过将音乐到舞蹈生成重新构建为图像合成问题，并引入时间同步和参考条件策略，该方法能够更好地生成时间一致、节奏对齐的舞蹈姿态，在复杂野外分布下表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 解决多路循环免疫荧光全玻片图像中重叠细胞核分割的挑战，特别是在生产环境中需要全自动处理且无法进行人工视觉校对的情况下，需要开发能够适应新仪器和新成像协议的方法。

Method: 采用弱到强泛化方法，结合多头Mask-RCNN扩展和高效通道注意力机制，通过伪标签校正和覆盖范围扩展实现泛化，能够自动学习分割新类别图像，无需人工标注。

Result: 与五种当前广泛使用的方法相比，该方法显示出显著改进，提供了代码、样本全玻片图像和高分辨率分割结果供社区采用和适配。

Conclusion: 该方法实现了全自动的细胞核分割，能够适应新仪器和新成像协议，在生产环境中无需人工校对，为社区提供了可用的开源工具和评估指标。
...
TL;DR: SVG-T2I是一个基于视觉基础模型表示空间的文本到图像生成框架，通过扩展SVG框架实现，在VFM特征域直接进行高质量图像合成，性能达到GenEval 0.75和DPG-Bench 85.78。


<details>
  <summary>Details</summary>
Motivation: 尽管基于视觉基础模型表示的可视化生成为整合视觉理解、感知和生成提供了统一途径，但在VFM表示空间中完全训练大规模文本到图像扩散模型仍未被充分探索。

Method: 扩展SVG框架，提出SVG-T2I，在VFM特征域直接支持高质量文本到图像合成，利用标准的文本到图像扩散流程。

Result: SVG-T2I达到竞争性性能：GenEval 0.75和DPG-Bench 85.78，验证了VFM在生成任务中的内在表示能力。

Conclusion: 该研究完全开源项目，包括自编码器、生成模型及其训练、推理、评估流程和预训练权重，以促进表示驱动的视觉生成研究。
...
<details>
  <summary>Details</summary>
Motivation: 在标签稀缺环境下，生成逼真的合成显微图像对训练深度学习模型至关重要。传统域适应方法在处理合成图像缺乏真实样本复杂纹理和视觉模式时存在困难，需要一种有效的方法来弥合合成与真实显微图像之间的域差距。

Method: 采用Inversion-Based Style Transfer (InST)框架，结合潜在空间自适应实例归一化和扩散模型中的随机反转技术，将真实荧光显微图像的风格转移到合成图像上，同时弱化保留内容结构。

Result: 使用InST合成图像训练的模型在细胞计数任务上表现优异：相比硬编码合成数据训练的模型，MAE降低37%；相比Cell200-s数据集训练的模型，MAE降低52%（从53.70降至25.95）；甚至优于仅使用真实数据训练的模型（25.95 vs 27.74 MAE）。结合DACS与CutMix等轻量级域适应技术可进一步提升性能。

Conclusion: InST风格迁移方法能有效减少合成与真实显微数据之间的域差距，为增强细胞计数性能提供可扩展的解决方案，同时最小化人工标注工作量。该方法在生物医学图像分析领域具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频遮罩数据集规模有限且真实性不足，利用分割数据虽能提升语义稳定性，但缺乏有效边界监督常导致遮罩细节不足

Method: 1) 提出学习型遮罩质量评估器MQE，无需真值即可评估语义和边界质量；2) 构建大规模真实视频遮罩数据集VMReal；3) 引入参考帧训练策略处理长视频外观变化；4) 开发MatAnyone 2模型

Result: 构建了包含28K视频片段和2.4M帧的VMReal数据集，MatAnyone 2在合成和真实世界基准测试中均超越先前所有方法，在所有指标上达到最先进性能

Conclusion: MQE评估器能有效提升视频遮罩质量，结合大规模真实数据集和参考帧训练策略，显著提升了视频遮罩的性能和实用性
...
TL;DR: 提出一个可信赖的频率感知分割框架，用于临床照片中白癜风范围的准确量化，结合数据高效训练策略、架构改进和临床信任机制，在专家标注数据集上取得优异性能。
...
Method: 基于三个协同支柱：1) 数据高效训练策略，结合ISIC 2019数据集上的领域自适应预训练和ROI约束的双任务损失；2) 架构改进，使用ConvNeXt V2编码器增强高频谱门控模块和stem-skip连接；3) 临床信任机制，采用K折集成和测试时间增强生成像素级不确定性图。

Result: 在专家标注的临床队列上验证，获得85.05%的Dice分数，显著降低边界误差（95% Hausdorff距离从44.79像素改善到29.95像素），优于CNN和Transformer基线，零灾难性失败，提供可解释的熵图。

Conclusion: 该框架为自动化白癜风评估建立了稳健可靠的标准，具有高可靠性并提供可解释的不确定性信息供临床医生审查。
...
TL;DR: SAM2VideoX：从自回归视频跟踪模型（SAM2）中提取结构保持运动先验到双向视频扩散模型（CogVideoX），通过双向特征融合模块和局部Gram流损失实现更真实的运动生成


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在保持物体结构（特别是人类和动物等关节可变形物体）的逼真运动方面存在挑战，仅靠扩大训练数据无法解决物理上不合理的过渡问题，现有方法依赖有噪声的运动表示（如光流或骨架）

Method: 提出SAM2VideoX方法：1）双向特征融合模块，从SAM2等循环模型中提取全局结构保持运动先验；2）局部Gram流损失，对齐局部特征如何一起移动

Result: 在VBench上获得95.51%得分，比REPA（92.91%）提升2.60%；FVD降低到360.57，比REPA和LoRA微调分别改善21.20%和22.46%；人类偏好研究中获得71.4%的偏好率

Conclusion: 通过从自回归视频跟踪模型中提取结构保持运动先验到双向扩散模型，显著提升了视频生成中运动的结构保持能力和物理合理性
...
<details>
  <summary>Details</summary>
Motivation: 现有的大规模视频生成模型在建模真实世界场景的光照交互方面表现出色，但缺乏一个能够联合理解场景本征属性（如反照率、法线、材质、辐照度）、利用这些属性进行视频合成，并支持可编辑本征表示的闭环框架。

Method: V-RGBX采用交错条件机制，统一了三个关键能力：1）视频逆向渲染为本征通道；2）从这些本征表示进行真实感视频合成；3）基于本征通道的关键帧视频编辑。通过用户选择的关键帧实现直观、物理基础的视频编辑。

Result: V-RGBX能够生成时间一致、真实感强的视频，并以物理合理的方式将关键帧编辑传播到整个序列。在物体外观编辑和场景级重光照等多样化应用中表现出色，超越了先前方法的性能。

Conclusion: V-RGBX是首个端到端的本征感知视频编辑框架，通过统一逆向渲染、视频合成和关键帧编辑能力，实现了物理基础的可编辑视频生成，为视频编辑提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术虽然实现了快速优化和实时渲染，但其依赖简化的alpha混合和粗糙的密度积分近似，限制了渲染复杂重叠半透明物体的能力。现有方法在物理准确性方面存在不足。

Method: 基于矩的序无关透明度方法，通过分析推导和计算每个像素的统计矩来表征相机光线上的密度分布，构建连续透射率函数，并在每个高斯内部独立采样，避免光线追踪或像素级排序。

Result: 该方法在光栅化渲染中实现了物理准确性，显著提高了复杂半透明介质中光衰减的建模能力，改善了整体重建和渲染质量。

Conclusion: 通过矩基透射率计算方法，成功弥合了光栅化渲染与物理准确性之间的差距，为3D高斯表示提供了高保真的透明度渲染能力。
...
TL;DR: 该研究对印度医疗环境中多语言ASR系统进行了首次系统性评估，比较了8个主流模型在卡纳达语、印地语和印度英语临床访谈数据上的表现，揭示了模型性能差异、语言混合问题以及基于说话者角色和性别的系统性性能差距。


<details>
  <summary>Details</summary>
Motivation: ASR在临床记录中应用日益广泛，但其在印度多语言和人口多样化医疗环境中的可靠性尚不清楚。需要评估ASR系统在真实临床数据上的表现，特别关注公平性和包容性问题。

Method: 使用真实世界临床访谈数据（涵盖卡纳达语、印地语和印度英语），比较了8个ASR模型（Indic Whisper、Whisper、Sarvam、Google speech to text、Gemma3n、Omnilingual、Vaani和Gemini）。评估了跨语言、说话者和人口亚组的转录准确性，特别关注患者与临床医生之间的错误模式以及基于性别或交叉性的差异。

Result: 结果显示不同模型和语言之间存在显著性能差异：某些系统在印度英语上表现良好，但在代码混合或方言语音上表现不佳。发现了与说话者角色和性别相关的系统性性能差距，引发了在临床环境中公平部署的担忧。
...
TL;DR: 本研究系统评估了四种最先进的ASR模型在13种非洲语言上的表现，通过在不同数据量（1-400小时）下微调，揭示了各模型在不同资源条件下的优势和适用场景。


<details>
  <summary>Details</summary>
Motivation: 非洲语言的自动语音识别面临标注数据有限、缺乏系统化模型选择指导的问题。虽然存在Whisper、XLS-R、MMS、W2v-BERT等大型预训练系统，但它们在非洲低资源环境中的比较行为尚未得到统一系统研究。

Method: 在13种非洲语言上对四种最先进的ASR模型进行基准测试，使用1到400小时不等的转录数据子集进行微调。除了报告错误率外，还分析模型在不同条件下的行为差异，并评估外部语言模型解码的效果。

Result: MMS和W2v-BERT在极低资源条件下数据效率更高；XLS-R在数据增加时扩展性更好；Whisper在中资源条件下表现优势。外部语言模型解码的效果取决于声学和文本资源的对齐程度，有时会带来改进，有时会达到平台期或引入额外错误。

Conclusion: 通过揭示预训练覆盖度、模型架构、数据集领域和资源可用性之间的相互作用，本研究为代表性不足语言的ASR系统设计提供了实用见解，有助于指导模型选择、数据扩展和解码策略。
...
TL;DR: MedBioRAG是一个结合语义和词法搜索、文档检索和监督微调的检索增强生成模型，在生物医学问答任务中显著优于现有最佳模型和GPT-4o。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成技术显著提升了大型语言模型在复杂问答任务中的能力，但在生物医学领域需要更专业的解决方案来处理该领域的复杂性和专业性。

Method: MedBioRAG采用语义和词法搜索相结合的方法进行文档检索和排序，通过监督微调实现上下文感知的响应生成，在NFCorpus、TREC-COVID、MedQA、PubMedQA和BioASQ等基准数据集上进行评估。

Result: MedBioRAG在所有评估任务中都优于之前的最先进模型和GPT-4o基础模型，在文档检索方面提升了NDCG和MRR分数，在封闭式问答中获得了更高准确率，在长格式问答中获得了更好的ROUGE分数。
...
TL;DR: KBQA-R1：通过强化学习将知识库问答从文本模仿转向交互优化，引入Group Relative Policy Optimization和Referenced Rejection Sampling方法，在多个数据集上实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 当前KBQA方法存在两种失败模式：要么生成没有验证模式存在的幻觉查询，要么采用僵化的模板推理模仿合成轨迹而缺乏对环境的真正理解。需要解决LLM在KBQA中的这些局限性。

Method: 提出KBQA-R1框架，将KBQA视为多轮决策过程，使用强化学习（Group Relative Policy Optimization）让模型学习在知识库中导航。同时引入Referenced Rejection Sampling数据合成方法，解决冷启动问题，严格对齐推理轨迹与真实动作序列。

Result: 在WebQSP、GrailQA和GraphQuestions等数据集上的广泛实验表明，KBQA-R1实现了最先进的性能，有效将LLM推理基于可验证的执行。
...
TL;DR: MultiScript30k扩展了Multi30k数据集，支持阿拉伯语、西班牙语、乌克兰语和简体/繁体中文，解决了原数据集仅支持欧洲语言的限制。
...
Result: 相似性分析显示，除繁体中文外，所有语言的余弦相似度均大于0.8，对称KL散度小于0.000251，与之前的扩展相当。COMETKiwi评估显示混合结果：阿拉伯语版本与ArEnMulti30k相当，但乌克兰语版本比Multi30k-Uk低6.4%。

Conclusion: MultiScript30k成功扩展了Multi30k数据集，支持更多全球语言和文字，为多模态机器翻译研究提供了更广泛的语言资源，尽管在某些语言上的翻译质量仍有提升空间。
...
TL;DR: 本文介绍了一个iMessage文本消息分析器，通过分析本地存储的iMessage数据文件，研究话题建模、响应时间、不情愿评分和情感分析等五个主要研究问题。


<details>
  <summary>Details</summary>
Motivation: 随着社会越来越依赖短格式电子通信，用户很少考虑公司基于其选择的通信平台可以收集哪些信息。虽然大多数公司将其数据严密保护，但苹果为Mac上的iMessage用户提供了一个存储所有消息和元数据的本地文件，这为分析个人消息数据创造了机会。

Method: 开发了一个iMessage文本消息分析器，利用苹果提供的本地存储文件，对iMessage数据进行探索性分析。该方法包括话题建模、响应时间分析、不情愿评分和情感分析等技术。

Result: 通过探索性数据分析展示了如何回答五个主要研究问题，包括话题识别、响应模式分析、沟通不情愿程度评估和情感倾向分析。

Conclusion: 该iMessage分析器能够有效分析个人消息数据，展示了其在未来iMessage数据研究中的潜力，为用户提供了理解自己通信模式的新工具。
...
TL;DR: 该研究通过模型和方法无关的评估框架，系统分析了特征归因方法（如集成梯度）在语言模型解释中存在的词汇和位置偏差问题，发现不同方法在相同输入上会产生不一致的解释，且偏差结构不平衡。


<details>
  <summary>Details</summary>
Motivation: 现有特征归因方法（如集成梯度）在相同输入上会产生不一致的解释，用户可能因此不信任这些解释或过度信任，需要超越表面不一致性，系统分析这些方法的底层偏差。

Method: 提出模型和方法无关的评估框架，包含三个评估指标，系统评估词汇偏差和位置偏差（输入中的内容和位置）。首先在人工数据的伪随机分类任务中进行受控实验，然后在自然数据的半受控因果关系检测任务中进行实验。

Result: 发现词汇偏差和位置偏差在模型比较中结构不平衡，在一个类型上得分高的模型在另一个类型上得分低。还发现产生异常解释的方法更可能自身存在偏差。

Conclusion: 特征归因方法存在系统性偏差，需要更全面的评估框架来理解不同解释方法的可靠性，这对提高语言模型解释的可信度具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注单实体事实和单语言数据，缺乏对多语言环境下多实体事实知识的系统性评估，因此需要开发FIBER基准来填补这一空白。

Method: 创建包含英语、意大利语和土耳其语的多语言数据集，设计句子补全、问答和对象计数预测三种任务类型，评估提示语言是否诱导实体选择推理偏差，并比较模型在单实体与多实体问题上的表现。

Result: 提示语言会影响模型输出，特别是与语言对应国家相关的实体；31%的主题显示推理偏差分数大于0.5；土耳其语提示比意大利语在83%的主题中表现出更高偏差；模型处理多实体问题比单实体问题更困难；英语表现最佳，土耳其语和意大利语得分较低；更大模型（Llama-3.1-8B和Qwen-2.5-7B）表现优于较小的3B-4B模型。

Conclusion: FIBER基准揭示了语言模型在多语言事实知识评估中的局限性，提示语言会诱导推理偏差，模型在处理多实体问题时面临更大挑战，且性能存在语言和模型规模依赖性，为改进多语言事实知识评估提供了重要基准。
...
Method: 使用开源工具构建可扩展的数据集生成管道，创建了两个数据集分割：一个经过整理的英文分割（1000万+文献）和一个多语言的未过滤TEI XML分割（3500万+文献）。基于该数据集预训练RoBERTa模型。

Result: 成功构建了SciLaD数据集，包含超过4500万篇科学文献。基于该数据集训练的RoBERTa模型在综合基准测试中表现出色，性能与类似规模的其他科学语言模型相当，验证了数据集的质量和实用性。

Conclusion: SciLaD数据集展示了开源工具在大规模科学数据整理中的有效性，数据集和评估管道的发布将促进科学自然语言处理领域的可重复性、透明度和进一步研究。
...
Result: 通过系统性综述，提供了多意图SLU领域的全面概览，总结了不同方法的性能表现和特点，为研究者提供了有价值的参考。

Conclusion: 本文讨论了当前面临的挑战，并展望了未来研究的有前景方向，希望为推进多意图SLU研究提供有价值的见解和参考。
...
TL;DR: 提出两阶段动态少样本学习方法，利用低成本和高性能LLM结合，提高系统综述标题摘要筛选效率，在10个系统综述中验证了方法的泛化性和成本效益。


<details>
  <summary>Details</summary>
Motivation: 系统综述是循证医学关键组成部分，但随着研究出版物快速增长，标题摘要筛选成为最耗时耗力的步骤之一，需要提高效率。

Method: 设计两阶段动态少样本学习方法：第一阶段使用低成本LLM进行初步筛选，第二阶段对低置信度实例使用高性能LLM重新评估，在控制计算成本的同时提升筛选性能。

Result: 在10个系统综述中评估该方法，结果显示其具有强大的泛化能力和成本效益，能够减少人工筛选负担并加速系统综述过程。

Conclusion: 两阶段动态少样本学习方法能有效提高系统综述标题摘要筛选的效率和性能，在实际应用中具有减少人工负担和加速综述过程的潜力。
...
TL;DR: 该论文提出了一种使用强化学习来改进大型语言模型推理能力的方法，通过GRPO优化策略，结合工具调用准确性和答案正确性作为奖励，相比传统监督微调方法取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 监督微调(SFT)在数据分布变化时泛化能力有限，而高质量推理轨迹的标注成本高、主观性强且难以扩展。最近的研究表明推理能力对模型泛化和可靠性至关重要，但缺乏有效的训练方法。

Method: 提出基于强化学习的管道，让LLMs生成推理步骤来指导工具调用和最终答案生成。采用Group Relative Policy Optimization (GRPO)方法，设计围绕工具准确性和答案正确性的奖励机制，使模型能够迭代优化推理和行动策略。

Result: 实验结果显示，该方法在推理质量和工具调用精度方面均有提升，相比无显式推理的SFT模型获得1.5%的相对改进，相比基础Qwen3-1.7B模型实现了40%的性能增益。

Conclusion: 通过强化学习统一推理和行动学习，能够构建更强大、更可泛化的对话智能体，展示了RL在提升LLMs推理能力方面的潜力。
...
TL;DR: AdaSD是一种自适应推测解码方法，通过动态调整生成长度和接受标准来加速大语言模型推理，无需额外训练或超参数调优，实现最高49%的加速效果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型参数规模增大导致推理速度显著下降。现有推测解码方法需要额外训练、大量超参数调优或部署前对模型和任务进行分析，限制了实际应用。

Method: 提出自适应推测解码(AdaSD)，引入两个自适应阈值：一个决定何时停止候选标记生成，另一个决定标记接受标准。两个阈值都基于标记熵和Jensen-Shannon距离实时更新，无需预分析或微调。

Result: 在基准数据集上的实验表明，AdaSD相比标准推测解码实现了最高49%的加速，同时将准确率下降控制在2%以内。

Conclusion: AdaSD提供了一种实用、高效且自适应的LLM推理解决方案，无需额外训练或超参数调优，兼容现成模型，显著提升推理效率。
...
TL;DR: CIP是一个轻量级即插即用的因果提示框架，通过构建实体、动作和事件之间的因果关系序列来指导LLM推理，减少幻觉并提高事实基础和可解释性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理长且嘈杂的检索上下文时经常产生幻觉，因为它们依赖虚假相关性而非真正的因果关系。需要一种方法来缓解输入阶段的幻觉问题。

Method: 提出CIP因果提示框架，构建实体、动作和事件之间的因果关系序列，并将其注入提示中以指导推理。通过因果干预和反事实推理来抑制非因果推理路径。

Result: 在7个主流语言模型（包括GPT-4o、Gemini 2.0 Flash和Llama 3.1）上的实验显示，CIP持续提升推理质量和可靠性：归因率提高2.6点，因果一致性得分提高0.38，有效信息密度提高四倍。API级分析显示，CIP加速上下文理解，端到端响应延迟降低达55.1%。
...
TL;DR: LegalRikai是一个由法律专业人士创建的日本企业法律实践基准测试，包含4个复杂任务和100个需要结构化长文本输出的样本，通过人类和自动评估验证了自动评估在专家资源有限时的实用性。


<details>
  <summary>Details</summary>
Motivation: 创建面向实际法律实践的基准测试，弥补传统短文本任务在文档级编辑能力评估上的不足，促进法律领域更实用的研究。

Method: 由法律专业人士在律师监督下创建包含4个复杂任务的基准测试，包含100个需要长文本结构化输出的样本。使用GPT-5、Gemini 2.5 Pro和Claude Opus 4.1等领先LLM进行人类和自动评估，评估多个实践标准。

Result: 人类评估发现抽象指令会引发不必要的修改，揭示模型在文档级编辑方面的弱点；自动评估在具有明确语言基础的标准上与人类判断一致，但结构一致性评估仍具挑战；自动评估可作为专家资源有限时的筛选工具。

Conclusion: 提出了数据集评估框架以促进法律领域更实践导向的研究，证明了自动评估在专家资源有限时的实用性，同时指出了结构一致性评估的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在专业任务部署中需要领域特定的参数高效微调（LoRA），但现有LoRA融合方法存在局限性：静态权重方法对所有参与LoRA赋予相同相关性，而监督训练方法需要大量数据和为每个可能的LoRA组合进行训练。有效融合适配器以处理复杂的多领域复合查询仍然是一个关键挑战。

Method: 提出qa-FLoRA方法，这是一种无需数据和训练的查询自适应LoRA融合方法。通过测量基础模型与相应适配器之间的分布差异，动态计算层级融合权重。该方法无需复合训练数据或领域代表性样本，可直接应用于现有适配器集合。

Result: 在9个多语言复合任务（数学、编程、医疗领域）上的广泛实验表明：qa-FLoRA在LLaMA-2上比静态融合提升约5%，在LLaMA-3上提升约6%；相比无训练基线，在LLaMA-2上提升约7%，在LLaMA-3上提升约10%。同时显著缩小了与监督基线的差距。层级融合权重分析揭示了可解释的融合模式。

Conclusion: qa-FLoRA提供了一种有效且无需训练的多领域适配方法，通过动态计算层级融合权重显著提升了复合查询性能，同时保持了方法的可解释性和实用性。
...
TL;DR: 本研究开发了自动化方法，使用自然语言处理技术分析捷克最高法院判决中的司法推理，挑战了关于中欧和东欧形式主义司法的传统观点。
...
Method: 创建MADON数据集（272个捷克最高法院判决，包含9,183个段落的专家标注，8种论证类型和整体形式主义标签）；使用30万捷克法院判决语料库，通过持续预训练调整transformer LLMs以适应捷克法律领域；采用非对称损失和类别加权等方法处理数据集不平衡；开发三阶段管道（结合ModernBERT、Llama 3.1和传统基于特征的机器学习）。

Result: 最佳模型成功检测论证段落（82.6% macro-F1）、分类传统法律论证类型（77.5% macro-F1）、将判决分类为形式主义/非形式主义（83.2% macro-F1）；三阶段管道在决策分类方面取得有希望的结果，同时降低计算成本并提高可解释性。

Conclusion: 法律论证挖掘能够实现可靠的司法哲学分类，并展示了其在计算法律研究中其他重要任务中的潜力；方法论易于在不同司法管辖区复制；所有管道、数据集、指南、模型和源代码均已公开。
...
<details>
  <summary>Details</summary>
Motivation: 研究数据选择对机器翻译微调的影响，特别是在开放大语言模型背景下，探索不同数据选择方法对模型性能的敏感度

Method: 使用日英双语语料库，比较五种数据选择器：TF-IDF、COMET Kiwi、QuRate、FD-Score和随机选择，在受控训练条件下进行实验

Result: 语义选择器在性能上持续优于词汇和几何启发式方法；即使所选数据差异小于3%，对模型性能的影响仍然显著，表明微调对数据质量高度敏感

Conclusion: 数据选择对机器翻译微调至关重要，语义选择方法优于传统启发式方法，微调过程对数据质量极其敏感，即使是微小的数据差异也会产生显著影响
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型处理长视频能力增强，重要视觉信息容易在长上下文中丢失，需要设计成本效益高的长视频分析工具
...
Conclusion: 提出的关键片段选择方法能够有效识别长视频中的重要视觉信息，为构建高质量多模态摘要提供了一种计算效率高的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前语言模型主要基于高资源语言训练，难以处理中低资源语言的医疗查询多样性，缺乏可靠的多语言医疗可信度评估，阻碍了语言模型在全球医疗环境中的实际应用。

Method: 开发了CLINIC基准测试，系统评估语言模型在5个可信度维度（真实性、公平性、安全性、鲁棒性、隐私性），通过18个多样化任务，覆盖15种语言（涵盖各大洲），涉及疾病状况、预防措施、诊断测试、治疗、手术、药物等关键医疗主题。

Result: 评估显示语言模型存在事实准确性不足、跨人口统计和语言群体的偏见、易受隐私泄露和对抗攻击等问题，突显了当前模型在多语言医疗环境中的局限性。

Conclusion: CLINIC基准测试为增强语言模型在全球医疗领域的覆盖范围和安全性的研究奠定了基础，有助于推动语言模型在多样化语言环境中的可信应用。
...
TL;DR: 提出了一个新的希伯来语医学语言模型，用于从电子健康记录中提取结构化临床时间线，构建患者旅程。模型基于DictaBERT 2.0，在超过500万条去识别化医院记录上持续预训练，在两个新数据集上表现优异。
...
Method: 基于DictaBERT 2.0模型，在超过500万条去识别化医院记录上进行持续预训练，引入词汇适应技术提高标记效率，创建了两个新的希伯来语医学数据集用于评估。

Result: 模型在两个新数据集（内科/急诊科和肿瘤科）上都取得了强劲性能，词汇适应提高了标记效率，去识别化不影响下游任务性能，支持隐私保护的模型开发。

Conclusion: 成功开发了一个有效的希伯来语医学语言模型，能够从电子健康记录中提取临床时间线，证明了去识别化数据训练的可行性，模型在伦理限制下可供研究使用。
...
Method: 研究调查了三种幻觉减少技术：验证链（CoVe）、层对比解码（DoLa）和检索增强生成（RAG）对LLM创造力的影响。评估了多个模型家族（LLaMA、Qwen、Mistral）在不同规模（1B-70B参数）上，使用两个创造力基准测试（NeoCoder和CS4）。

Result: 研究发现这些方法对发散性创造力有相反的影响：CoVe增强发散思维，DoLa抑制发散思维，RAG影响最小。为科学应用中在事实准确性和创造性探索之间平衡时选择适当的幻觉减少方法提供了指导。

Conclusion: 在需要平衡事实准确性和创造性探索的科学应用中，应根据具体需求选择合适的幻觉减少方法：需要增强创造力时选择CoVe，需要严格控制事实准确性时选择DoLa，RAG则对创造力影响较小。
...
TL;DR: 提出DBSA方法，用于审计黑盒大语言模型，分析每个输入token对输出的敏感性，无需模型内部信息或梯度计算。


<details>
  <summary>Details</summary>
Motivation: 现有LLM审计方法通常关注特定方面（如偏见检测），缺乏对黑盒模型中输入token如何影响输出的系统性理解。在医疗、法律等高风险领域，需要工具来理解不可访问API端点中LLM的行为可靠性。

Method: 提出分布敏感性分析（DBSA），这是一种轻量级、模型无关的方法，通过分析输出分布来评估每个输入token的敏感性，无需对LLM做分布假设，也不需要计算梯度。

Result: DBSA能够快速、即插即用地可视化探索LLM对特定输入token的依赖程度，通过示例展示了该方法可以发现现有可解释性方法可能忽略的敏感性。

Conclusion: DBSA为实践者提供了一个实用的黑盒LLM审计工具，能够帮助理解模型输出如何依赖于输入token，特别是在高风险应用场景中。
...
TL;DR: 提出基于Merlin-Arthur协议的RAG训练框架，将检索器与生成器视为交互式证明系统，通过对抗性训练提升模型对检索证据的验证能力，减少幻觉并改善拒绝行为。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统将检索视为弱启发式而非可验证证据，导致LLM在缺乏支持时回答、在上下文不完整或误导时产生幻觉、依赖虚假证据。需要建立可验证的证据处理机制。

Method: 采用Merlin-Arthur协议框架：Arthur（生成器LLM）训练未知来源的问题，Merlin提供有用证据，Morgana注入对抗性误导上下文。使用线性时间XAI方法识别和修改对Arthur影响最大的证据。训练模型：(i) 在上下文支持时回答，(ii) 证据不足时拒绝，(iii) 依赖真正支撑答案的具体上下文片段。

Result: 在三个RAG数据集和两种不同规模的模型家族上，M/A训练的LLM显示出改进的groundedness、完整性、合理性、拒绝行为，以及减少的幻觉，无需手动标注不可回答的问题。检索器通过自动生成的M/A硬正负样本也提高了召回率和MRR。

Conclusion: 自主交互式证明风格监督为可靠RAG系统提供了原则性和实用路径，使检索文档不再仅仅是建议，而是可验证的证据。提出的EIF指标能够解耦解释保真度与基线预测误差，衡量认证互信息保证。
...
TL;DR: 使用BERTopic分析历史报纸档案中核能核安全话语，克服传统主题建模方法在捕捉历史文本动态复杂性方面的不足


<details>
  <summary>Details</summary>
Motivation: 传统主题建模方法（如LDA）难以有效处理历史报纸档案中的主题演化、OCR噪声和大规模文本，无法充分捕捉历史话语的复杂性和动态性

Method: 采用BERTopic神经主题建模方法，利用基于transformer的嵌入来提取和分类主题，分析1955-2018年间关于核能和核安全的文章，追踪主题分布的时间演化

Result: BERTopic能够有效识别核能与核武器相关主题的共现模式，揭示主题重要性随时间的变化趋势，展现出比传统方法更好的可扩展性和上下文敏感性

Conclusion: BERTopic为历史研究提供了比传统方法更丰富的历史话语洞察，有助于核能、历史和社会科学研究，同时指出了当前局限性和未来研究方向
...
TL;DR: 该论文建立了确定性推测生成算法的首个紧下界，通过将令牌生成过程与分支随机游走类比，证明了每个推测迭代成功预测的令牌数存在理论上限，为推测解码系统的设计提供了理论指导。


<details>
  <summary>Details</summary>
Motivation: 推测生成技术通过并行验证多个草稿令牌来加速大型语言模型的推理，但目前对可实现的加速上限缺乏理论理解。需要建立推测生成算法的基本性能界限。

Method: 将令牌生成过程与分支随机游走建立平行关系，分析最优草稿树选择问题。在基本假设下，通过数学证明建立了推测生成算法的运行时下界。
...
Method: 基于Amazon 2023评论数据集构建高质量数据管道，采用两阶段对齐流程：1）通过非对称知识蒸馏进行角色感知的监督微调；2）使用偏好估计器进行强化学习与AI反馈，捕捉细粒度角色相关信号
...
TL;DR: 论文提出了CORL框架，使用强化学习端到端微调混合整数线性规划方案，以最大化实际运营性能，将MILP求解过程转化为可微分的随机策略。


<details>
  <summary>Details</summary>
Motivation: 传统MILP方法难以准确建模现实世界中的随机问题，导致实际性能不佳。现有机器学习方法通常依赖监督学习，需要真实最优决策，并使用MILP梯度的替代方法。

Method: 提出CORL框架，将分支定界算法求解的MILP转化为可微分的随机策略，与强化学习兼容，使用真实世界数据进行端到端微调。

Result: 在简单的组合顺序决策示例中验证了CORL方法的有效性。

Conclusion: CORL框架为使用强化学习优化MILP模型的实际性能提供了概念验证，避免了传统方法对监督学习和真实最优决策的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时计算扩展技术（如重复采样、自验证、自反思）在单智能体场景中有效，但缺乏在多智能体系统中分配计算以促进协作的原则性机制，特别是在明确预算约束下。

Method: 提出FutureWeaver框架：1）模块化协作，通过自玩反思从历史轨迹中抽象可重用多智能体工作流；2）双级规划架构，基于当前任务状态和未来步骤推测优化计算分配。

Result: 在复杂智能体基准测试中，FutureWeaver在不同预算设置下始终优于基线方法，验证了其在推理时优化中促进多智能体协作的有效性。

Conclusion: FutureWeaver为多智能体系统中的测试时计算分配提供了原则性框架，通过模块化协作和前瞻性规划，在固定预算下显著提升了协作性能。
...
TL;DR: TriFlow是一个用于真实世界旅行规划的多智能体框架，通过检索、规划和治理三阶段流程，将开放式用户请求转化为可执行的行程，在空间、时间和预算约束下满足用户偏好。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的智能体在约束满足、工具协调和效率方面存在困难，经常产生不可行或成本过高的计划。需要解决这些限制，实现更好的旅行规划。

Method: TriFlow采用渐进式多智能体框架，通过检索、规划和治理三阶段流程，结合结构化推理和语言灵活性，使用规则-LLM协作组装约束一致的行程，并进行有界迭代优化。

Result: 在TravelPlanner和TripTailor基准测试中取得最先进结果，分别达到91.1%和97%的最终通过率，相比当前SOTA实现了超过10倍的运行效率提升。

Conclusion: TriFlow通过渐进式多智能体框架有效解决了真实世界旅行规划中的约束满足和效率问题，实现了高质量的个性化行程规划。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉验证码的基准测试存在局限性，无法全面覆盖所有验证码类型，且缺乏专门针对LVLMs的基准测试。先前的研究在设计基准和数据集时都是根据特定研究目标定制的，导致这些基准无法全面评估LVLMs的性能。

Method: 提出了CAPTURE（CAPTCHA for Testing Under Real-world Experiments）基准测试，该基准包含4种主要验证码类型和25种子类型，来自31个不同供应商。基准具有广泛的类别多样性、大规模数据以及专门为LVLMs定制的标签。

Result: 使用该基准测试评估当前LVLMs时，发现它们在解决验证码方面表现不佳。基准测试填补了先前研究在数据全面性和标签针对性方面的空白。

Conclusion: CAPTURE基准测试首次为LVLMs提供了全面的验证码评估框架，揭示了当前LVLMs在解决验证码任务上的局限性，为未来研究提供了重要的评估工具。
...
Result: 智能体在与环境交互过程中，这些组件协同演化，产生可信赖的行为。

Conclusion: 该框架能够使LLM智能体在强化学习形式化的环境中按照明确的行为指导行动，提高任务完成的可靠性和可验证性。
...
TL;DR: AgentBalance是一个在明确token成本和延迟预算约束下构建成本效益多智能体系统的框架，采用"先骨干后拓扑"的设计方法，相比现有方法在相同预算下能获得10-22%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的多智能体系统在构建时很少在明确的token成本和延迟预算约束下进行建模和优化，这导致当预算成为约束时，现有的拓扑优先设计方法往往成本效益不佳，无法满足大规模部署的实际需求。

Method: AgentBalance采用"先骨干后拓扑"的两阶段设计：1) 骨干导向的智能体生成：通过LLM池构建、池选择和角色-骨干匹配构建异构骨干的智能体；2) 自适应MAS拓扑生成：通过智能体表示学习、门控和延迟感知的拓扑合成来指导智能体间通信。

Result: 在包含14个候选LLM骨干的基准测试中，AgentBalance在匹配的token成本预算下获得高达10%的性能提升，在匹配的延迟预算下获得高达22%的性能提升，并在性能与预算曲线上展现出强大的AUC表现。

Conclusion: AgentBalance是一个有效的框架，能够在明确的预算约束下构建成本效益的多智能体系统，可作为现有MAS的插件使用，并能很好地泛化到未见过的LLM，实现实用的预算感知部署。
...
TL;DR: 该论文指出当前XAI中基于基线的保真度评估指标存在问题，不同基线选择会偏向不同的归因方法，甚至导致矛盾结果。作者提出基线应满足两个理想属性：移除信息和不产生过度分布外图像，并引入一种新的模型依赖基线来改善这一权衡。


<details>
  <summary>Details</summary>
Motivation: 当前可解释人工智能中广泛使用的归因方法评估指标（如插入和删除指标）依赖于基线选择，但不同基线会偏向不同的归因方法，甚至导致相互矛盾的结果。这引发了一个关键问题：应该使用哪个基线？作者旨在研究基线选择对归因方法评估的影响，并提出更合理的基线设计标准。

Method: 作者首先分析了现有基线的问题，提出了基线应满足的两个理想属性：(1) 移除信息，(2) 不产生过度分布外图像。通过实验验证现有基线无法同时满足这两个标准，存在权衡关系。最后，作者利用特征可视化技术，提出了一种新的模型依赖基线，该基线能够移除信息而不产生过度分布外图像。

Result: 实验表明，现有基线都无法同时满足移除信息和不产生过度分布外图像两个标准，存在明显的权衡关系。作者提出的新型模型依赖基线在权衡方面优于现有基线，能够更好地平衡这两个属性，从而提供更公平的归因方法评估。

Conclusion: 基线选择对归因方法评估有重大影响，当前评估指标存在系统性偏差。作者提出的基线设计标准（移除信息且不产生过度分布外图像）为基线选择提供了理论指导，而新提出的模型依赖基线在权衡方面优于现有方法，有助于更公平地评估归因方法。
...
<details>
  <summary>Details</summary>
Motivation: No-Three-In-Line问题是组合几何中的著名问题，经典方法如整数线性规划(ILP)虽然能保证最优解，但随着网格规模增大会面临指数级复杂度。机器学习方法为模式近似提供了有前景的替代方案，需要系统比较经典优化与AI方法在该问题上的性能。

Method: 首次将PatternBoost变换器学习和强化学习(PPO)应用于该问题，并与传统ILP方法进行比较。PatternBoost使用变换器架构学习模式，PPO使用策略优化方法，ILP作为基准提供最优解。
...
TL;DR: BAID框架系统评估AI文本检测器偏见，发现对弱势群体文本检测性能存在显著差异


<details>
  <summary>Details</summary>
Motivation: 现有AI文本检测器在教育和工作场景中被广泛使用，但之前研究仅发现孤立偏见案例（特别是对英语学习者），缺乏基于更广泛社会语言因素的系统性评估

Method: 提出BAID评估框架，包含7大类（人口统计、年龄、教育等级、方言、正式程度、政治倾向、主题）超过20万样本，并为每个样本生成保留原始内容但反映特定子群体写作风格的合成版本，评估4个开源SOTA AI文本检测器

Result: 发现检测性能存在一致差异，特别是对来自代表性不足群体的文本召回率较低
...
TL;DR: 该研究首次实证证实大型语言模型在处理临床信息时表现出类似代谢功能障碍的特征，提出了"AI-MASLD"创新概念，警告当前LLMs在医疗应用中存在功能缺陷，需在人类专家监督下作为辅助工具使用。


<details>
  <summary>Details</summary>
Motivation: 模拟真实临床场景，系统评估大型语言模型从含噪声和冗余的患者主诉中提取核心医学信息的能力，并验证其是否表现出类似代谢功能障碍相关脂肪性肝病的功能衰退。

Method: 采用基于标准化医学探针的横断面分析设计，选取GPT-4o、Gemini 2.5、DeepSeek 3.1和Qwen3-Max四种主流LLMs作为研究对象，使用包含20个医学探针、涵盖5个核心维度的评估系统模拟真实临床沟通环境，所有探针均有临床专家定义的金标准答案，并由两名独立临床医生通过双盲逆向评分量表进行评估。

Result: 所有测试模型均表现出不同程度的功能缺陷，Qwen3-Max整体表现最佳，Gemini 2.5最差；在极端噪声条件下，大多数模型出现功能崩溃；GPT-4o在深静脉血栓继发肺栓塞风险评估中做出严重误判。

Conclusion: 首次实证证实LLMs在处理临床信息时表现出类似代谢功能障碍的特征，提出"AI-MASLD"创新概念，为AI在医疗保健领域的应用提供重要安全警告，强调当前LLMs必须在人类专家监督下作为辅助工具使用，其理论知识与实际临床应用之间仍存在显著差距。
...
TL;DR: 论文提出AI基准测试需要从传统静态基准转向动态自适应基准框架，以解决模型记忆静态基准、基准结果与现实性能脱节的问题，并倡导建立AI基准测试专业教育体系。


<details>
  <summary>Details</summary>
Motivation: 当前AI基准测试面临多重挑战：模型架构快速演进、规模扩大、数据集更新、部署环境多样化，导致评估成为移动目标；大语言模型容易记忆静态基准，造成基准结果与实际性能脱节；现有基准过度强调顶级硬件上的峰值性能，对多样化现实场景指导有限。

Method: 提出动态自适应基准测试框架，需要纳入不断演进的模型、更新的数据和异构平台，同时保持透明度、可复现性和可解释性。倡导建立"AI基准测试专业教育体系"，通过技术革新和系统化教育培养基准设计和使用的专业能力。

Result: 识别出关键障碍包括：高资源需求、专用硬件访问有限、基准设计专业知识缺乏、结果与应用领域关联不确定性。通过MLCommons、DOE万亿参数联盟等实践经验，强调社区协作的重要性。

Conclusion: 基准测试必须动态化、包容化，支持应用相关的比较，确保评估能跟上AI发展步伐，促进负责任、可复现、可访问的AI部署。社区努力可为AI基准测试专业教育提供基础。
...
TL;DR: 该论文提出了一种基于结构因果模型的能源需求预测方法，通过分析天气和日历因素之间的因果关系，构建贝叶斯模型，在测试集上实现了3.84%的平均绝对百分比误差。
...
Result: 1. 因果模型揭示了能源需求对温度波动的响应具有季节依赖性敏感性；2. 发现冬季能源需求方差较低，这是由于温度变化与日常活动模式之间的解耦效应；3. 贝叶斯模型在测试集上实现了3.84%的MAPE；4. 跨两年数据的交叉验证平均MAPE为3.88%，显示出强鲁棒性。
