...
TL;DR: 提出了一种基于视频扩散模型的单张模糊图像后处理重聚焦方法，能够生成感知准确的焦堆栈视频序列，实现交互式重聚焦


<details>
  <summary>Details</summary>
Motivation: 自动对焦系统经常无法捕捉到预期的拍摄主体，用户希望在拍摄后调整焦点，但现有方法存在局限性

Method: 使用视频扩散模型从单张模糊图像生成感知准确的焦堆栈，表示为视频序列，支持交互式重聚焦

Result: 方法在感知质量和鲁棒性方面一致优于现有方法，特别是在具有挑战性的场景中表现优异

Conclusion: 该方法为日常摄影中更先进的焦点编辑功能铺平了道路，并发布了大规模真实世界智能手机条件下获取的焦堆栈数据集
...
<details>
  <summary>Details</summary>
Motivation: 重新审视几何模型拟合中的评分函数问题，这是RANSAC鲁棒几何拟合的关键组件。现有方法如MAGSAC++在基准测试中表现最佳，但其理论基础存在问题，需要重新评估。

Method: 1. 扩展几何误差到球面噪声模型；2. 在鲁棒设置中考虑均匀分布离群值的混合模型；3. 分析MAGSAC++方法的推导问题；4. 提出基于大验证集或小随机验证集的实验评估方法。

Result: 1. 发现MAGSAC++的推导不符合合理原则，其评分函数在数值上等价于简单的高斯-均匀似然模型；2. 所有评分函数（包括使用学习的内点分布）在实验中表现相同；3. MAGSAC++评分既不优于简单竞争者，也不对阈值超参数选择更不敏感。

Conclusion: 本文全面重新审视了最先进的方法，发现MAGSAC++的理论基础存在问题，所有评分函数在实际应用中表现相似，这对未来改进方法或应用于其他鲁棒拟合问题具有重要意义。
...
TL;DR: HyGE-Occ提出了一种用于3D全景占用预测的新框架，通过混合视图变换分支结合3D高斯和边缘先验，增强了几何一致性和边界感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在保持精确几何结构和捕捉3D实例精确空间范围方面存在困难，这对于稳健的全景分离至关重要。需要改进几何一致性和边界感知能力。
...
Conclusion: HyGE-Occ通过结合高斯表示和边缘先验，有效提升了3D全景占用预测的几何一致性和边界感知，为密集3D场景理解提供了更精确的解决方案。
...
TL;DR: 该研究提出Widget2Code任务，针对紧凑、无上下文的小部件界面生成代码，开发了WidgetFactory系统，通过组件组装、图标检索和自适应渲染提高视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 现有UI2Code研究主要关注网页和移动界面，而小部件（widget）作为紧凑、无上下文的微界面，具有密集布局和图标化特点，且缺乏可访问的标记数据，需要专门的研究方法。

Method: 开发了WidgetFactory端到端基础设施，包括：1）遵循小部件设计原则将原子组件组装成完整布局；2）图标检索和可重用可视化模块；3）框架无关的小部件特定领域语言WidgetDSL；4）将WidgetDSL编译为多种前端实现的编译器；5）自适应渲染模块优化空间维度以满足紧凑性约束。

Result: 创建了仅基于图像的小部件基准测试，包含细粒度、多维度的评估指标。基准测试显示，尽管通用多模态大语言模型优于专门的UI2Code方法，但仍产生不可靠和视觉不一致的代码。提出的WidgetFactory系统显著提高了视觉保真度。

Conclusion: 该研究为Widget2Code任务建立了强大的基准和统一的基础设施，通过联合推进感知理解和结构化代码生成，解决了小部件界面代码生成的独特挑战，为未来研究奠定了基础。
...
TL;DR: NeurAlign是一个深度学习框架，通过统一的体积-表面表示联合配准3D脑MRI图像，在球坐标空间中桥接表面拓扑与体积解剖结构，实现更快更准确的皮质和皮质下区域配准。


<details>
  <summary>Details</summary>
Motivation: 传统方法将脑MRI的体积配准和表面配准分开处理，导致不一致性，限制了后续分析。需要一种能够同时配准皮质和皮质下区域的统一方法。

Method: 提出NeurAlign深度学习框架，通过中间球坐标空间桥接解剖表面拓扑与体积解剖结构，将球面配准整合到学习中，确保体积和表面域之间的几何一致性。

Result: 在域内和域外数据集上，NeurAlign始终优于经典和基于机器学习的配准方法，Dice分数提高高达7个点，同时保持规则的变形场。比标准方法快几个数量级，且只需MRI扫描无需额外输入。

Conclusion: NeurAlign以其卓越的准确性、快速推理和易用性，为联合皮质和皮质下配准设定了新标准。
...
TL;DR: SE360：一种用于360度全景图像多条件引导对象编辑的新型框架，通过粗到细的自主数据生成流程和两阶段数据精炼策略，实现了语义和几何一致的对象编辑。


<details>
  <summary>Details</summary>
Motivation: 现有的基于指令的图像编辑方法扩展到360度全景图像时面临挑战，在等距柱状投影和透视视图中经常产生不合理的编辑结果，需要专门针对全景图像特性的编辑解决方案。

Method: 1. 提出粗到细的自主数据生成流程，利用视觉语言模型和自适应投影调整进行层次分析，确保对象及其物理环境的整体分割；2. 引入两阶段数据精炼策略提高数据真实感并减少模型过拟合；3. 基于构建的数据集训练基于Transformer的扩散模型，支持文本、掩码或参考图像等多种条件引导的360度全景对象编辑。

Result: 实验表明，该方法在视觉质量和语义准确性方面均优于现有方法，能够生成语义有意义且几何一致的编辑结果，即使是从未标记的全景图像中提取的数据也能保持一致性。

Conclusion: SE360框架通过创新的数据生成和精炼策略，成功解决了360度全景图像编辑中的挑战，为多条件引导的对象编辑提供了有效的解决方案，在保持几何一致性的同时实现了高质量的语义编辑。
...
TL;DR: 该研究提出了首个模型无关框架，用于量化视频基础模型（VidFMs）的3D理解能力，通过浅层读出器从模型特征中估计多种3D属性，发现最先进的视频生成模型展现出强大的3D理解能力，甚至超过专门训练的3D专家模型。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索视频基础模型在大量视频数据预训练后，是否自然获得了对3D世界的全局理解。视频是3D世界的连续2D投影，研究者想知道这种3D理解能力是否会在视频训练中自发涌现。

Method: 提出了首个模型无关框架，通过浅层读出器（shallow read-outs）从各种视频基础模型的特征中估计多种3D属性，从而量化它们的3D感知能力。该方法不依赖于特定模型架构，能够统一评估不同VidFMs的3D理解水平。

Result: 研究发现最先进的视频生成模型展现出对3D物体和场景的强烈理解能力，尽管它们从未接受过任何3D数据训练。这种3D理解能力甚至超过了专门为3D任务训练的大型专家模型。研究还对主要视频基础模型进行了全面的3D基准测试。

Conclusion: 该研究为构建可扩展的3D模型提供了有价值的观察，表明通过大规模视频数据训练可以获得强大的3D理解能力，这为未来3D建模方法的发展提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 全切片图像的高通量"病理组学"分析为研究组织特征和生物标志物发现提供了新机会，但缺乏能够测量个体结构特征空间相互作用及其与临床参数关联的工具，限制了组织特征在微环境和宏观环境层面的临床相关性。

Method: HistoWAS框架包含：(1) 特征空间：在传统指标基础上增加30个来自地理信息系统点模式分析的拓扑和空间特征，用于量化组织微结构；(2) 关联研究引擎：受表型全关联研究启发，对每个特征进行大规模单变量回归分析并进行统计校正。

Result: 作为概念验证，将HistoWAS应用于肾脏精准医学项目的385张PAS染色全切片图像（来自206名参与者），分析了总共102个特征（72个传统对象级特征和30个空间特征）。代码和数据已在GitHub上发布。

Conclusion: HistoWAS是一个能够将组织空间组织与临床结果关联的计算框架，通过引入空间特征和关联分析引擎，为病理图像分析提供了新的工具和方法。
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉国作为重要农业国家，番石榴是其重要热带水果。炭疽病和果蝇感染会降低番石榴的质量和产量，早期检测系统可以减少损失并保护收成。需要开发适合本地种植品种的疾病检测模型。

Method: 使用CNN与传统机器学习技术结合的方法，提出CNN-ML级联框架，并采用CNN与梯度提升机（Gradient Boosting Machine）的集成模型。使用来自孟加拉国Rajshahi和Pabna种植园的GFDD24数据集，包含健康、果蝇感染和炭疽病三类图像。

Result: 提出的集成模型在番石榴数据集上实现了约99.99%的最高分类准确率。CNN-ML级联框架表现出强大、高精度的番石榴疾病检测能力。

Conclusion: 该方法适用于实时农业监测系统，能够有效识别孟加拉国本地种植番石榴的疾病，有助于提高番石榴生产和农业实践，对经济发展具有重要意义。
...
TL;DR: DDTM提出了一种双分支弱监督框架，通过扩散分支细化局部语义和Transformer分支增强全局上下文一致性，解决跨分辨率土地覆盖制图中的分辨率不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 跨分辨率土地覆盖制图需要从粗分辨率监督中生成高分辨率语义预测，但严重的分辨率不匹配导致现有弱监督方法难以对齐细粒度空间结构与粗标签，造成噪声监督和精度下降。
...
<details>
  <summary>Details</summary>
Motivation: 现有路面缺陷检测数据集缺乏标准化，在标注风格、缺陷类型定义和格式上各不相同，导致难以整合用于统一训练，限制了模型在多样化真实世界条件下的泛化能力。

Method: 整合多个公开可用的数据源，创建标准化基准数据集，统一了13种缺陷类型的定义和标注格式，数据集包含来自7个国家的52747张图像和135277个边界框标注，涵盖了图像质量、分辨率、视角和天气条件的广泛变化。

Result: 使用YOLOv8-YOLOv12、Faster R-CNN和DETR等最先进的目标检测模型进行基准测试，模型在多样化场景中取得了有竞争力的性能，并展示了向新环境的零样本迁移能力。

Conclusion: 该标准化数据集提供了第一个全球代表性的路面缺陷检测基准，实现了模型间的公平比较，解决了数据集不一致问题，促进了路面缺陷检测技术的发展。
...
TL;DR: 提出LaSeRS数据集和SegEarth-R2模型，用于解决遥感图像中复杂语言指令引导分割的挑战，包括层次粒度、多目标、推理需求和语言变异性四个维度。


<details>
  <summary>Details</summary>
Motivation: 当前模型只能处理简单的单目标指令，但在复杂地理空间场景（如多粒度分割、多目标指令、隐含用户意图）中表现不佳。现有数据集过于简化，导致模型在真实世界中敏感易错。

Method: 1. 创建LaSeRS数据集，涵盖语言引导分割的四个关键维度：层次粒度、目标多重性、推理需求和语言变异性。2. 提出SegEarth-R2 MLLM架构，包含空间注意力监督机制（处理小目标定位）和灵活高效的分割查询机制（处理单目标和多目标场景）。

Result: SegEarth-R2在LaSeRS和其他基准测试中表现出色，为新一代地理空间分割建立了强大的基线。

Conclusion: LaSeRS数据集填补了现有数据集过于简化的关键空白，SegEarth-R2模型通过专门的空间注意力监督和灵活的分割查询机制，有效解决了复杂语言引导遥感图像分割的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有分心驾驶检测模型主要依赖驾驶员视角，忽略了影响驾驶行为的重要环境上下文。本研究旨在探索结合道路视角是否能改善自然驾驶条件下的分心检测准确性。

Method: 使用真实世界驾驶中的同步双摄像头记录，对三种领先的时空动作识别架构（SlowFast-R50、X3D-M和SlowOnly-R50）进行基准测试。每种模型在两种输入配置下评估：仅驾驶员视角和堆叠双视角。

Result: 结果显示，上下文输入在某些模型中能改善检测，但性能提升强烈依赖于底层架构。单路径SlowOnly模型在双视角输入下实现了9.8%的改进，而双路径SlowFast模型由于表征冲突导致准确性下降了7.2%。

Conclusion: 简单地添加视觉上下文并不足够，除非架构专门设计用于支持多视角集成，否则可能导致干扰。这项研究强调了融合感知设计对未来多模态驾驶员监控系统的重要性。
...
TL;DR: H2em：一种用于组合零样本学习的双曲几何框架，通过层次双曲嵌入解决现有方法在大规模分类体系中的泛化问题


<details>
  <summary>Details</summary>
Motivation: 现有CZSL方法在欧几里得空间中建模层次结构时，无法适应现实世界大规模分类体系的指数级结构，导致泛化能力受限。双曲几何天生适合嵌入树状结构，但简单的双曲映射可能面临层次塌陷和细粒度区分不足的问题。

Method: 提出H2em框架：1）利用双曲几何嵌入层次结构；2）设计双重层次蕴含损失，使用双曲蕴含锥强制预定义层次关系；3）设计判别对齐损失，通过难负样本挖掘在语义相似组合间建立大地距离；4）提出双曲跨模态注意力机制，在双曲几何中实现实例感知的跨模态融合。

Result: 在三个基准测试上的广泛消融实验表明，H2em在封闭世界和开放世界场景中都建立了新的最先进性能。

Conclusion: H2em通过双曲几何有效建模CZSL中的层次结构，解决了大规模分类体系下的泛化问题，为组合零样本学习提供了更强大的框架。
...
TL;DR: VALLR-Pin是一个针对普通话唇语识别的两阶段框架，通过联合预测汉字和拼音，并利用大语言模型结合语音上下文来纠正同音字错误，提升识别准确率。


<details>
  <summary>Details</summary>
Motivation: 普通话唇语识别面临两个主要挑战：1）视觉音素高度模糊，2）同音字普遍存在。现有的英语唇语识别方法难以直接应用于普通话，需要专门设计来解决这些语言特性带来的问题。

Method: 1）两阶段框架：共享视频编码器输入双解码器，联合预测汉字序列和标准拼音；2）多任务学习：通过同时学习字符和语音输出，增强视觉语义表示；3）推理阶段：文本解码器生成多个候选转录，将拼音输出与候选汉字序列拼接作为提示，输入大语言模型解决歧义；4）微调LLM：使用中间检查点生成的含噪拼音-文本对创建指令-响应对，训练LLM识别特定错误模式。

Result: 该方法通过结合视觉特征与语音和语言上下文，提高了普通话唇语识别的性能。具体来说，多任务学习增强了视觉语义表示，而基于大语言模型的纠错机制有效解决了同音字引起的错误。

Conclusion: VALLR-Pin成功地将英语唇语识别架构扩展到普通话，通过协同利用视觉特征、语音上下文和语言模型，为普通话唇语识别提供了一种有效的解决方案，特别针对同音字和视觉音素模糊的问题进行了优化。
...
<details>
  <summary>Details</summary>
Motivation: 现有唇部同步系统通常需要复杂的掩码处理或依赖GANs/扩散模型，导致计算开销大、推理速度慢。FlashLips旨在实现实时高质量的唇部同步，同时避免使用掩码和复杂生成模型。

Method: 采用两阶段架构：第一阶段是紧凑的一步潜在空间编辑器，使用参考身份、掩码目标帧和低维唇部姿态向量重建图像，仅使用重建损失训练；通过自监督生成嘴部变化变体作为伪真值进行微调，实现无掩码推理。第二阶段是基于流匹配目标的音频到姿态变换器，从语音预测唇部姿态向量。

Result: 系统在单个GPU上实现超过100FPS的实时性能，匹配最先进大模型的视觉质量。结合确定性重建与鲁棒的音频控制，提供高感知质量和超实时速度。

Conclusion: FlashLips通过解耦唇部控制与渲染的两阶段架构，实现了无需掩码、实时高质量的唇部同步，为实际应用提供了简单稳定的解决方案。
...
Conclusion: 提出的多模态管道通过整合外部文本知识有效增强了图像描述的上下文深度，为需要更丰富信息描述的实际应用提供了有前景的解决方案。
...
TL;DR: 本文提出了一个新的基于自然语言的文档图像检索（NL-DIR）基准测试，使用自然语言描述作为细粒度语义查询，包含41K真实文档图像，每个图像配有5个高质量查询，通过LLM生成并人工验证。
...
Method: 1. 构建NL-DIR数据集：41K真实文档图像，每个图像配有5个高质量细粒度语义查询，通过大语言模型生成并结合人工验证；2. 对现有主流对比视觉语言模型和OCR-free视觉文档理解模型进行零样本和微调评估；3. 研究两阶段检索方法以提高性能，同时实现时间和空间效率。

Result: 提出了新的NL-DIR基准测试和相应评估指标，包含大规模高质量数据集，为视觉文档理解社区带来新的研究机会。

Conclusion: NL-DIR基准测试能够促进视觉文档理解领域的研究，填补了现有文档图像检索方法在处理细粒度语义文本查询方面的空白。
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在HAR中广泛应用，但激活函数和优化器的组合对模型性能的影响尚未得到充分分析，特别是在实际医疗场景中。现有研究多关注架构设计，而AF与MO的相互作用相对未被探索。
...
Result: ConvLSTM在所有数据集上始终优于BiLSTM。ConvLSTM结合Adam或RMSprop达到99.00%的准确率，表现出强大的时空学习能力和稳定性能。BiLSTM在UCF101上表现良好（约98.00%），但在HMDB51上降至约60.00%，表明其跨数据集鲁棒性有限且对AF和MO变化不敏感。
...
TL;DR: LiDARDraft：一种利用3D布局作为桥梁，从文本、图像等多样化输入生成可控高质量LiDAR点云的方法


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成真实多样的LiDAR点云时，难以同时实现高质量结果和多样化控制能力，主要原因是复杂的LiDAR点云分布与简单控制信号之间的不平衡

Method: 将文本、图像和点云统一表示为3D布局，转换为语义和深度控制信号，然后使用基于rangemap的ControlNet引导LiDAR点云生成

Result: 该方法在可控LiDAR点云生成方面表现出色，实现了"从零开始模拟"，能够从任意文本描述、图像和草图创建自动驾驶环境

Conclusion: LiDARDraft通过3D布局桥接多样化条件信号与LiDAR点云，解决了控制信号与复杂点云分布之间的不平衡问题，实现了高质量可控的LiDAR点云生成
...
TL;DR: DDAVS提出了一种解耦音频语义和延迟双向对齐框架，通过音频原型记忆库提取音频语义，并使用延迟模态交互的双重交叉注意力机制，解决了多源纠缠和视听错位问题，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有音频视觉分割方法存在多源纠缠和视听错位问题，导致模型偏向于更大或更响的物体，而忽略了较弱、较小或同时发生的声源。

Method: DDAVS框架包含两个核心组件：1) 使用可学习查询从音频原型记忆库中提取音频语义，并通过对比学习优化；2) 引入具有延迟模态交互的双重交叉注意力机制，改善多模态对齐的鲁棒性。

Result: 在AVS-Objects和VPO基准测试上的广泛实验表明，DDAVS在单源、多源和多实例场景中均优于现有方法，验证了其有效性和泛化能力。

Conclusion: DDAVS通过解耦音频语义和延迟双向对齐，有效解决了音频视觉分割中的多源纠缠和视听错位问题，在具有挑战性的真实世界条件下表现出强大的性能。
...
TL;DR: HEART-ViT提出了一种基于Hessian引导的高效动态注意力机制和token剪枝框架，首次统一了token和注意力头的剪枝，通过二阶优化实现更好的精度-效率权衡，在保持精度的同时显著减少计算量和延迟。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers虽然精度高，但其二次注意力计算成本和冗余计算严重阻碍了在延迟和资源受限平台上的部署。现有剪枝方法要么单独处理token或注意力头，依赖启发式或一阶信号，往往牺牲精度或无法跨输入泛化。

Method: HEART-ViT是一个Hessian引导的高效动态注意力和token剪枝框架，使用高效的Hessian-向量乘积估计token和注意力头的曲率加权敏感性，在显式损失预算下实现原则性剪枝决策。该方法揭示了token剪枝主导计算节省，而注意力头剪枝提供细粒度冗余移除的重要结构洞察。

Result: 在ImageNet-100和ImageNet-1K数据集上，使用ViT-B/16和DeiT-B/16模型，HEART-ViT实现了高达49.4%的FLOPs减少、36%的延迟降低和46%的吞吐量提升，同时在微调后始终匹配甚至超越基线精度（例如在40% token剪枝时恢复4.7%精度）。在实际边缘设备部署中也验证了推理速度和能效的实际提升。
...
TL;DR: 提出基于3D高斯溅射重建果园场景的苹果姿态估计标注新流程，将手动标注需求减少99.6%，通过3D重建自动生成大量训练标签


<details>
  <summary>Details</summary>
Motivation: 果园自动化任务面临环境变化大和遮挡严重的挑战，苹果姿态估计中关键点（如花萼）经常被遮挡。现有姿态估计方法虽然不再依赖这些关键点，但仍需要它们进行标注，导致标注困难且耗时。由于遮挡问题，不同图像中同一水果的标注可能存在冲突或缺失。

Method: 提出新颖的流程：1）使用3D高斯溅射重建果园场景；2）简化标注过程；3）自动将标注投影到图像；4）训练和评估姿态估计方法。通过3D重建，仅需105个手动标注即可获得28,191个训练标签。

Result: 使用遮挡程度≤95%的水果标签训练时获得最佳性能，原始图像上F1分数为0.927，渲染图像上为0.970。训练数据集大小调整对模型性能影响较小。最少遮挡的水果位置估计最好，随着遮挡增加而变差。测试的姿态估计方法未能正确学习苹果的方向估计。

Conclusion: 提出的3D重建标注流程显著减少了手动标注工作量，在苹果姿态估计任务中表现出良好性能，但方向估计仍需改进。该方法为果园自动化中的姿态估计提供了高效的标注解决方案。
...
TL;DR: 本文系统研究了视觉基础模型的多教师蒸馏方法，提出了AMoE框架，通过非对称关系知识蒸馏、令牌平衡批处理和分层聚类采样等技术，显著降低了训练成本并提高了数据效率。


<details>
  <summary>Details</summary>
Motivation: 尽管通过多教师蒸馏训练的视觉基础模型在统一视觉表示方面具有潜力，但其学习动态和数据效率尚未得到充分探索。本文旨在系统研究多教师蒸馏方法，识别关键因素以实现更低计算成本的训练。

Method: 提出了Agglomerative Mixture-of-Experts Vision Foundation Models (AMoE)，同时从SigLIP2和DINOv3两个教师模型中蒸馏知识到混合专家学生模型。关键技术包括：1) 非对称关系知识蒸馏损失，保留每个教师的几何特性同时实现有效知识转移；2) 令牌平衡批处理，将不同分辨率图像打包成具有统一令牌预算的序列；3) 分层聚类和采样训练数据，显著提高多教师蒸馏的样本效率。

Result: 通过结合这些发现，构建了OpenLVD200M数据集（包含2亿张图像），该数据集在多教师蒸馏中表现出卓越的效率。在混合专家模型中实例化了这些方法，并发布了OpenLVD200M数据集和蒸馏模型。

Conclusion: 本文系统研究了多教师蒸馏的关键因素，提出了有效的技术组合，显著提高了视觉基础模型蒸馏的数据效率和计算效率，为构建更高效的统一视觉表示提供了实用解决方案。
...
Method: 1. 在生成式VQ-VAE的潜在空间进行变换编码；2. 引入分类超模块减少超信息比特成本；3. 基于代码预测的监督增强语义一致性。

Result: 1. 自然图像小于0.04 bpp，人脸图像小于0.01 bpp时仍保持高视觉质量；2. 在CLIC2020测试集上，以比MS-ILLM少45%的比特数达到相同FID；3. 支持图像修复和风格迁移等应用。

Conclusion: 生成式潜在空间具有更好的稀疏性、更丰富的语义和更好的人类感知对齐特性，为低比特率图像压缩提供了有效解决方案，并支持多种下游应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有大规模文档布局数据集如PubLayNet和DocBank缺乏细粒度区域标签和多语言多样性，而人工标注数据集如M6Doc和D4LA虽然标签更丰富但规模太小且多语言覆盖不足。印度语言文档尤其缺乏代表性，限制了文档布局分析的发展。

Method: 引入IndicDLP数据集，涵盖11种代表性印度语言和英语，覆盖12个常见文档领域。同时创建UED-mini数据集（基于DocLayNet和M6Doc）用于预训练，为印度语言布局模型提供基础。

Result: 实验表明，在IndicDLP上微调现有英语模型能显著提升性能，验证了数据集的有效性。在IndicDLP上训练的模型能够很好地泛化到非印度语言布局，表明该数据集对文档数字化具有广泛价值。
...
TL;DR: 提出DAMP框架，通过设计空间-光谱退化度量作为退化提示，无需依赖预定义退化先验，实现统一高光谱图像恢复


<details>
  <summary>Details</summary>
Motivation: 现有统一高光谱图像恢复方法依赖显式退化先验（如退化标签）作为提示，但在现实复杂混合退化场景中难以获取。需要一种不依赖预定义退化先验的方法。

Method: 提出退化感知度量提示（DAMP）框架：1）设计空间-光谱退化度量连续量化多维退化，作为退化提示；2）引入空间-光谱自适应模块（SSAM）动态调制空间和光谱特征提取；3）将SSAM作为专家集成到混合专家架构中，使用退化提示作为门控路由器。

Result: 在自然和遥感高光谱数据集上的广泛实验表明，DAMP实现了最先进的性能，并展示了卓越的泛化能力，能够适应多样、混合或未见过的退化情况。

Conclusion: DAMP框架通过退化感知度量提示和自适应特征调制，实现了无需预定义退化先验的统一高光谱图像恢复，在性能和泛化能力方面优于现有方法。
...
TL;DR: 提出BiCoR-Seg框架，通过双向协同精化解决高分辨率遥感图像语义分割中的类间相似度高、类内变化大问题，提升边界清晰度和分类准确性。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像语义分割面临类间相似度高、类内变化大的挑战，现有方法难以将抽象且强判别性的语义知识有效注入像素级特征学习，导致复杂场景中边界模糊和类别混淆。

Method: 提出BiCoR-Seg框架，包含三个核心组件：1) 热图驱动的双向信息协同模块(HBIS)，通过生成类别级热图建立特征图和类别嵌入之间的双向信息流；2) 分层监督策略，将每个HBIS模块生成的可解释热图直接用作低分辨率分割预测进行监督；3) 跨层类别嵌入Fisher判别损失，增强嵌入表示的判别能力。

Result: 在LoveDA、Vaihingen和Potsdam数据集上的大量实验表明，BiCoR-Seg实现了出色的分割性能，同时提供了更强的可解释性。

Conclusion: BiCoR-Seg通过双向协同精化框架有效解决了高分辨率遥感图像语义分割中的关键挑战，在提升分割精度的同时增强了模型的可解释性。
...
TL;DR: LADLE-MM是一个在有限标注数据和计算资源下工作的多模态虚假信息检测器，通过模型集成初始化，使用双单模态分支加多模态分支的架构，在减少60.3%可训练参数的情况下，在DGM4和VERITE数据集上达到或超越现有方法性能。


<details>
  <summary>Details</summary>
Motivation: 随着多媒体内容生成和操纵工具的普及，跨多模态的逼真合成篡改已成为广泛威胁，被用于扭曲重要事件叙事和传播虚假信息。现有检测方法通常依赖计算密集型架构或需要大量标注数据，难以在资源受限环境下应用。

Method: 提出LADLE-MM模型：采用模型集成初始化，包含两个单模态分支（图像和文本）和一个多模态分支，后者使用BLIP提取的多模态嵌入作为固定参考空间来增强图像和文本表示。设计为在有限标注设置和受限训练资源下工作。

Result: 在DGM4基准测试中，LADLE-MM在二元和多标签分类任务上达到竞争性性能，比先前最先进模型减少60.3%可训练参数，在无基础标注训练时优于现有方法。在VERITE数据集上，超越使用更复杂大型视觉语言模型的现有方法，显示出在开放集设置中的有效泛化能力和对单模态偏差的强鲁棒性。

Conclusion: LADLE-MM证明了在有限标注和计算资源下实现高效多模态虚假信息检测的可能性，通过轻量级架构和模型集成方法，在减少参数的同时保持或超越复杂模型的性能，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有WSCOD方法性能远落后于全监督方法，主要因为两个限制：1）通用分割模型生成的伪标签不可靠，缺乏COD任务特定的语义理解；2）忽略涂鸦标注的固有偏差，阻碍模型捕捉伪装目标的全局结构。

Method: 提出两阶段框架：第一阶段采用自适应熵驱动点采样和多智能体辩论机制增强SAM在COD中的能力；第二阶段设计FADeNet，渐进融合多级频率感知特征，平衡全局语义与局部细节，动态重加权监督强度以缓解涂鸦偏差。

Result: D³ETOR在多个基准测试中取得了最先进的性能，显著缩小了弱监督与全监督COD之间的差距。
...
TL;DR: 提出一个结合Dirichlet后验采样和Dempster-Shafer理论的框架，用于量化医学影像中SHAP解释的不确定性，解决SHAP在存在认知和随机不确定性时的不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习在医学影像领域广泛应用，但复杂模型（如ResNets、Vision Transformers等）的可解释性差。SHAP作为主流解释方法，在存在认知不确定性和随机不确定性时，其解释结果不稳定且不可靠，这在医学影像应用中尤其成问题。

Method: 使用Dirichlet后验采样和Dempster-Shafer理论来量化SHAP解释中的不确定性。框架采用信念图、似然图和融合图方法，结合统计定量分析，对SHAP解释的不确定性进行量化。

Result: 在三个具有不同类别分布、图像质量和模态类型的医学影像数据集上评估了该框架，这些数据集涵盖病理学、眼科学和放射学领域，引入了显著的认知不确定性。

Conclusion: 提出的框架能够有效量化医学影像应用中SHAP解释的不确定性，提高了模型解释的可靠性和稳定性，特别是在存在复杂不确定性的医学影像场景中。
...
TL;DR: TAVID是一个统一的框架，能够从文本和参考图像同步生成交互式面部视频和对话语音，解决现有研究中音频和视觉模态分离的问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将说话头生成、倾听头生成和对话语音生成分开研究，忽视了人类对话中紧密耦合的音频-视觉交互特性，无法构建真正类人的对话系统。

Method: TAVID通过两个跨模态映射器（运动映射器和说话者映射器）集成面部和语音生成流程，实现音频和视觉模态之间互补信息的双向交换。

Result: 在四个维度（说话面部真实感、倾听头响应性、二元交互流畅性和语音质量）的广泛实验表明，该方法在所有方面都表现出有效性。

Conclusion: TAVID提供了一个统一的框架，能够同步生成交互式面部和对话语音，更好地模拟人类对话中的多模态交互特性。
...
TL;DR: KeyTailor提出了一种基于关键帧的细节注入策略，结合ViT-HD高清数据集，解决了视频虚拟试衣中服装动态细节捕捉和背景完整性问题，同时避免了传统方法对DiT架构的修改和额外计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散变换器的视频虚拟试衣方法存在三个主要问题：1）难以捕捉细粒度服装动态细节；2）无法保持视频帧间背景完整性；3）由于在DiT中引入额外交互模块导致计算成本高；4）现有公共数据集规模和质量有限限制了模型泛化能力。

Method: KeyTailor采用关键帧驱动的细节注入策略，包括：1）指令引导的关键帧采样策略从输入视频中筛选信息丰富的帧；2）服装细节增强模块从关键帧中提取服装动态信息到服装相关潜在空间；3）协作背景优化模块优化背景潜在空间的完整性；4）将增强的细节与姿态、掩码和噪声潜在变量一起注入标准DiT块中。

Result: 实验表明KeyTailor在动态和静态场景下，在服装保真度和背景完整性方面均优于现有最先进基线方法。同时提出的ViT-HD数据集包含15,070个810*1080分辨率的高质量视频样本，覆盖多样化服装类型。

Conclusion: KeyTailor通过关键帧驱动的细节注入策略，在不修改DiT架构的情况下实现了高效、真实的视频虚拟试衣合成，解决了现有方法在服装动态细节捕捉和背景完整性方面的挑战，同时避免了额外计算复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 全球超过一半人口是双语者，人们经常在多语言场景下交流。现有的人脸-语音关联方法在训练和测试语言相同时表现良好，但在实际多语言环境中，测试语言可能与训练语言不同，这带来了新的挑战。

Method: FAME 2026挑战赛通过设置训练和测试语言不同的场景，鼓励研究者开发语言无关或语言鲁棒的人脸-语音关联方法。挑战赛的具体技术方法未在摘要中详细说明，但重点是解决跨语言泛化问题。

Result: 摘要中未提供具体实验结果，但该挑战赛在ICASSP 2026会议上举办，旨在推动跨语言人脸-语音关联技术的发展。

Conclusion: FAME 2026挑战赛强调了在多语言环境中开发鲁棒的人脸-语音关联方法的重要性，为解决实际应用中的跨语言泛化问题提供了平台和方向。
...
TL;DR: SmartSplat是一种基于高斯泼溅的图像压缩框架，通过梯度-颜色引导采样和尺度自适应颜色采样，在超高分辨率图像压缩中实现高质量重建和强压缩比。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI快速发展，超高分辨率视觉内容的产生对端设备上的高效压缩和实时解码提出了重大挑战。现有2D高斯图像模型在超高分辨率场景下难以平衡压缩比和重建质量。

Method: 提出SmartSplat框架，采用梯度-颜色引导变分采样和基于排除的均匀采样策略，提高高斯基元在像素空间中的非重叠覆盖；提出尺度自适应高斯颜色采样方法优化颜色初始化；通过空间布局、尺度和颜色初始化的联合优化，用有限数量的高斯基元有效捕捉局部结构和全局纹理。

Result: 在DIV8K和新构建的16K数据集上的实验表明，SmartSplat在可比压缩比下持续优于现有最先进方法，并突破了它们的压缩极限，展现出强大的可扩展性和实际应用价值。

Conclusion: SmartSplat通过图像感知特征和自适应采样策略，在超高分辨率图像压缩中实现了高质量重建与高效压缩的良好平衡，为解决生成式AI时代的高分辨率内容压缩挑战提供了有效方案。
...
TL;DR: DETACH：一种用于外中心视频与环境传感器对齐的分解时空框架，通过显式分解保留局部细节，解决传统全局对齐方法在捕捉细微动作和区分相似时间模式不同语义上下文方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于可穿戴传感器的自我中心视频对齐方法存在用户不适、隐私问题和可扩展性限制。外中心视频与环境传感器作为非侵入性替代方案，但传统全局对齐方法在此设置下存在两个问题：无法捕捉局部细节（如细微动作），以及过度依赖模态不变的时间模式导致不同语义上下文的动作错误对齐。

Method: 提出DETACH分解时空框架：1）显式分解保留局部细节；2）通过在线聚类发现传感器-空间特征提供语义基础；3）两阶段对齐：首先通过相互监督建立空间对应关系，然后通过空间-时间加权对比损失进行时间对齐，自适应处理简单负样本、困难负样本和假负样本。

Result: 在Opportunity++和HWU-USP数据集的下游任务综合实验中，相比适应的自我中心-可穿戴基线方法，DETACH取得了显著改进。

Conclusion: DETACH框架有效解决了外中心视频与环境传感器对齐中的局部细节捕捉和语义上下文区分问题，为非侵入式、可扩展的人类动作识别提供了有效解决方案。
...
TL;DR: 提出Chain-of-Anomaly-Thoughts (CoAT)框架，通过引入归纳性犯罪偏见的异常分类层，显著提升大视觉语言模型在视频监控中的异常检测性能


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型在视频监控中存在对正常性的固有偏见，难以有效检测犯罪异常；而思维链推理策略缺乏归纳性异常偏见，进一步导致模型偏向正常解释

Method: 提出Chain-of-Anomaly-Thoughts (CoAT)多智能体推理框架，在推理过程中通过最终的异常聚焦分类层引入归纳性犯罪偏见

Result: 在低分辨率视频中异常检测F1分数提升11.8个百分点，在高分辨率视频中异常分类提升3.78个百分点

Conclusion: CoAT框架通过引入归纳性异常偏见，有效解决了大视觉语言模型在视频监控中的正常性偏见问题，显著提升了异常检测和分类性能
...
TL;DR: TransFuser v6通过解决专家演示与学生观察之间的不对齐问题，在CARLA驾驶基准测试中实现了新的SOTA性能


<details>
  <summary>Details</summary>
Motivation: 模拟器可以生成无限驾驶数据，但模仿学习策略在模拟中仍难以实现鲁棒的闭环性能。研究发现专家演示与学生观察之间存在严重不对齐：专家具有更高的可见性（忽略遮挡）和更低的感知不确定性（知道其他车辆动作），且导航意图在测试时仅通过单个目标点指定，导致模仿学习效果受限

Method: 通过缩小专家与学生之间的差距来解决不对齐问题，包括改进TransFuser架构（TFv6），将感知监督集成到共享的模拟到真实流水线中，并处理导航意图的模糊性问题

Result: TFv6在CARLA所有主要公开闭环基准测试中达到新的SOTA：Bench2Drive上95 DS，Longest6~v2和Town13上性能翻倍。在NAVSIM和Waymo视觉端到端驾驶基准测试中也获得一致提升

Conclusion: 通过解决专家与学生之间的不对齐问题，可以显著提升模仿学习在自动驾驶中的性能，TFv6展示了缩小这种差距的有效方法，并在多个基准测试中取得了突破性进展
...
TL;DR: 本文提出了一种纹理基集成平滑分解方法，用于纹理图像中的异常检测，相比传统方法具有更低的误识别率、更小的训练数据集需求和更好的检测性能。
...
Method: 提出纹理基集成平滑分解方法，包括两个主要过程：首先学习纹理基函数以提取准周期纹理模式，然后利用该纹理基作为先验知识进行异常检测，避免纹理误识别并高精度捕获潜在异常。
...
TL;DR: 该研究探讨了将物理推断的关节作用力纳入人体运动理解任务中，通过在步态识别、动作识别和视频描述三个主要任务上的系统评估，证明了力信号能显著提升模型性能，特别是在动态、遮挡或外观变化等挑战性条件下。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉的人体运动理解方法大多忽略了关节作用力等物理线索，而这些线索在生物力学中是基础性的。本研究旨在探索物理推断的力信号是否以及何时能增强运动理解能力。

Method: 将力信号整合到现有的运动理解流程中，在8个基准数据集上系统评估其对三个主要任务的影响：步态识别、动作识别和细粒度视频描述。

Result: 在CASIA-B步态识别数据集上，Rank-1准确率从89.52%提升至90.39%（+0.87%），在穿外套和侧视等挑战性条件下提升更显著（+2.7%和+3.0%）。Gait3D数据集性能从46.0%提升至47.3%（+1.3%）。在Penn Action动作识别任务中，CTR-GCN模型提升+2.00%，高用力类如击打/拍打提升+6.96%。视频描述任务中，Qwen2.5-VL的ROUGE-L分数从0.310提升至0.339（+0.029）。

Conclusion: 物理推断的力信号能显著补充视觉和运动学特征，特别是在动态、遮挡或外观变化条件下，为人体运动理解提供了有价值的补充信息。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的文本到图像模型在视觉内容生成方面表现出色，但在小规模排版和非拉丁文字（如中文）的文本渲染性能有限，特别是在图形设计中需要高精度文本编辑和生成的场景下存在不足。

Method: 提出UTDesign统一框架：1）从零开始训练基于DiT的文本风格迁移模型，在合成数据集上生成透明RGBA文本前景；2）扩展为条件文本生成框架，通过多模态条件编码器处理背景图像、提示和布局规范；3）集成预训练文本到图像模型和基于MLLM的布局规划器，构建全自动文本到设计流程。

Result: 在广泛实验中，UTDesign在开源方法中实现了最先进的性能，在风格一致性和文本准确性方面表现优异，与专有商业方法相比也展现出独特优势。

Conclusion: UTDesign为AI辅助图形设计提供了一个有效的统一框架，特别解决了非拉丁文字和小规模排版的问题，通过高精度文本编辑和条件生成能力，推动了自动化设计工具的发展。
...
TL;DR: 研究评估了自适应RGB和LWIR融合技术用于无人机系统检测地表地雷，通过热对比度提升特征提取，YOLOv11在特定参数下表现最佳，但存在准确率与效率的权衡。


<details>
  <summary>Details</summary>
Motivation: 地雷仍然是持续的人道主义威胁，全球有1.1亿枚活跃地雷分布在60个国家，每年造成2.6万人伤亡。需要开发有效的检测技术来应对这一挑战。
...
Result: YOLOv11达到最佳性能（86.8% mAP），10-30%热融合在5-10米高度为最优检测参数。RF-DETR准确率最高（69.2% mAP），但YOLOv11训练速度快17.7倍。聚合多时相训练数据集比季节特定方法表现更好（提升1.8-9.6%）。反坦克地雷检测准确率61.9%，反人员地雷仅19.2%。

Conclusion: 自适应RGB-LWIR融合结合YOLOv11架构为无人机地雷检测提供了有效解决方案，但存在准确率与计算效率的权衡。多时相训练能提升模型鲁棒性，不同地雷类型检测性能差异显著。未来需要研究不同埋藏深度和土壤类型下的热对比度效应。
...
TL;DR: 该论文系统研究了多模态对齐、翻译、融合和迁移技术，通过五个章节分别解决空间关系翻译、医学文本定位、知识图谱构建、动作识别融合和知识迁移等核心问题，提升机器对复杂多模态输入的理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前机器在处理复杂多模态输入时面临诸多挑战，包括空间语言理解、医学文本定位、知识图谱构建、动作识别和计算效率等问题。作者旨在通过系统性的多模态学习方法，提升计算系统处理多样化复杂输入的能力。

Method: 论文采用分章节的系统方法：第3章提出Spatial-Reasoning Bert将文本空间关系翻译为2D布局；第4章利用空间共现损失函数将医学文本映射到3D解剖图谱；第5章建立基准将自然语言链接到知识图谱实体；第6章融合视频帧和物体检测进行动作识别；第7章通过多模态知识蒸馏实现RGB模型模仿多模态融合能力。

Result: 研究实现了空间语言到视觉布局的有效解码，医学文本在解剖图谱中的精确定位，知识图谱与自然语言的清晰链接，动作识别准确性的提升，以及RGB模型在保持性能的同时显著降低计算需求。

Conclusion: 该工作系统性地推进了多模态机器学习方法，在空间语言理解、医学文本解释、知识图谱丰富和动作识别等多个领域取得重要进展，显著增强了计算系统处理复杂多模态输入的能力，为跨领域应用提供了有效解决方案。
...
Method: 提出AlignPose方法：1) 从多个外参标定RGB视角聚合信息；2) 设计了专门针对物体姿态的多视角特征度量细化模块，通过优化一致的世界坐标系物体姿态，最小化实时渲染物体特征与观测图像特征在所有视角上的差异。

Result: 在四个数据集(YCB-V, T-LESS, ITODD-MV, HouseCat6D)上使用BOP基准评估，AlignPose优于其他已发表方法，特别是在多视角实际可用的工业数据集上表现突出。

Conclusion: AlignPose通过多视角特征聚合和专门设计的细化优化，有效解决了单视角姿态估计的局限性，在无需物体特定训练的情况下实现了优异的泛化性能，特别适用于工业应用场景。
...
TL;DR: MTIF提出了一种多粒度文本引导的图像融合新范式，通过分层跨模态调制模块，利用细粒度文本描述指导图像融合，在多重曝光和多重焦点融合任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用粗粒度文本描述作为辅助指导，但这种方法难以理解细粒度细节，且跨模态对齐精度不足。需要更精细的文本引导机制来提升图像融合质量。

Method: 提出MTIF框架：1）引入多粒度文本描述，分别捕捉细节、结构线索和语义内容；2）通过分层跨模态调制模块指导图像融合；3）在每个粒度级别添加监督信号促进视觉-文本特征对齐；4）采用显著性驱动增强模块丰富训练数据的密集语义内容。
...
Method: 提出FlashVLM框架：1) 计算投影图像token与标准化文本嵌入在语言模型空间中的显式跨模态相似度；2) 使用对数域加权和温度控制锐化将外在相关性与内在视觉显著性融合；3) 通过多样性保持分区保留最小但具有代表性的背景token以维持全局上下文。

Result: 在相同token预算和评估协议下，FlashVLM实现了超越无损压缩的效果：在LLaVA 1.5上剪枝高达77.8%的视觉token，在94.4%压缩率下仍保持92.8%准确率，在14个图像和视频基准测试中达到最先进的效率-性能平衡。

Conclusion: FlashVLM通过文本引导的视觉token选择，在保持强鲁棒性和泛化能力的同时，为主流VLM提供了卓越的效率-性能权衡，实现了超越基线模型的压缩效果。
...
TL;DR: FedPOD是一种用于优化联邦学习效率和通信成本的新方法，通过包含被排除的异常值参与者、消除对先前轮次学习信息的依赖，以及应用每轮验证损失计算方法来解决FedPIDAvg的局限性。


<details>
  <summary>Details</summary>
Motivation: FedPIDAvg虽然通过PID控制器和泊松分布建模提高了性能和通信效率，但存在两个主要问题：1) 基于泊松分布排除异常值参与者限制了数据利用；2) PID控制器需要在整个联邦学习过程中保持相同的参与者，因为它使用先前轮次的学习信息。FedPOD旨在解决这些问题。

Method: FedPOD通过三个关键改进：1) 包含被排除为异常值的参与者；2) 消除对先前轮次学习信息的依赖；3) 应用每轮验证损失计算方法。该方法从Kubernetes的最小计算单元POD获得灵感，设计为与Kubernetes自动扩展兼容，将轮次任务扩展到POD单元，实现类似Kubernetes自动扩展的灵活设计。

Result: FedPOD在Dice分数指标上与FedPIDAvg表现相当：WT为0.78，ET为0.71，TC为0.72（平均值）。在投影收敛分数方面，平均为0.74。这表明FedPOD在保持性能的同时解决了FedPIDAvg的局限性。

Conclusion: FedPOD展示了通过提高效率、灵活性和性能指标来增强联邦学习的潜力。其与Kubernetes自动扩展的兼容性为联邦学习系统提供了更灵活的设计可能性，能够更好地适应动态变化的计算环境。
...
TL;DR: L-IVA是一个评估生成环境中目标导向规划的任务和基准，ORCA是首个实现视频化身主动智能的框架，通过内部世界模型和OTAR闭环循环实现自主多步任务完成。


<details>
  <summary>Details</summary>
Motivation: 当前视频化身生成方法在身份保持和运动对齐方面表现良好，但缺乏真正的自主性，无法通过自适应环境交互来追求长期目标。需要从被动动画转向主动、目标导向的行为。
...
Result: 广泛实验表明，ORCA在任务成功率和行为一致性方面显著优于开环和非反思基线，验证了内部世界模型启发的设计能够有效提升视频化身的智能水平。

Conclusion: ORCA框架成功将视频化身智能从被动动画推进到主动、目标导向的行为，通过内部世界模型能力和闭环OTAR循环实现了在开放域场景中的自主多步任务完成。
...
TL;DR: SemanticGen提出在语义空间而非VAE潜在空间生成视频，通过两阶段扩散模型实现：首先生成紧凑的语义特征进行全局规划，再基于语义特征生成VAE潜在表示，最终解码为像素视频。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在VAE潜在空间学习视频分布存在收敛慢、计算成本高的问题，特别是在生成长视频时。视频固有的冗余性表明，生成过程应从紧凑的高层语义空间开始进行全局规划，再添加高频细节，而不是直接建模大量低层视频token。

Method: 采用两阶段生成过程：第一阶段使用扩散模型生成紧凑的语义视频特征，定义视频的全局布局；第二阶段使用另一个扩散模型，基于这些语义特征生成VAE潜在表示，最终通过VAE解码器生成像素视频。

Result: 在语义空间生成比在VAE潜在空间生成收敛更快，且扩展到长视频生成时仍然有效且计算高效。大量实验表明SemanticGen能生成高质量视频，优于现有最先进方法和强基线。

Conclusion: SemanticGen通过在语义空间进行视频生成，解决了传统VAE潜在空间方法的收敛慢和计算成本高的问题，实现了更高效、高质量的视频生成，特别适用于长视频生成场景。
...
TL;DR: 大型语言模型在未经定制的情况下对真实课堂转录进行教学行为分类的能力有限，少量样本提示能显著提升性能但存在可靠性限制


<details>
  <summary>Details</summary>
Motivation: 随着LLM在教育技术中的广泛应用，了解其未经定制的"开箱即用"能力对于设定期望和基准测试越来越重要，特别是在解释真实教育场景方面

Method: 比较了6个LLM在课堂转录中分类教学行为的基线性能，评估了零样本、单样本和少量样本三种典型提示方法

Result: 零样本表现中等，少量样本提示显著提升了最先进模型的性能，最佳配置达到Cohen's Kappa=0.58；但性能因教学行为类型而异，召回率提升常伴随假阳性增加

Conclusion: 基础模型在解释教学话语方面表现出有意义但有限的能力，提示设计有助于展现其能力但不能消除基本的可靠性限制
...
TL;DR: 本文提出了一种基于LLM的反事实框架来量化机器学习论文中的修辞风格，通过生成不同修辞风格的对比文本并评估，发现愿景式修辞能显著预测论文关注度，且2023年后修辞强度因LLM写作辅助而大幅提升。


<details>
  <summary>Details</summary>
Motivation: AI领域的快速发展引发了人们对机器学习论文中"炒作"现象的担忧，但现有方法难以将修辞风格与实质性内容分离。由于大胆的语言可能源于强实证结果或仅仅是修辞风格，区分二者一直是个挑战。

Method: 提出基于LLM的反事实框架：1）多个LLM修辞角色从相同实质性内容生成不同修辞风格的对比文本；2）LLM评委通过成对比较评估这些文本；3）使用Bradley-Terry模型聚合评估结果。该方法应用于2017-2025年的8,485篇ICLR投稿，生成超过25万篇对比文本。

Result: 1）愿景式修辞能显著预测下游关注度（引用和媒体关注），即使控制同行评审评价后依然成立；2）2023年后修辞强度急剧上升，实证证据表明这主要由LLM写作辅助工具驱动；3）框架可靠性得到验证：对角色选择具有鲁棒性，LLM判断与人工标注高度相关。

Conclusion: LLM可以作为测量和改进科学评估的工具，能够有效量化论文的修辞风格，为理解科学传播中的修辞效应提供了新方法，并揭示了LLM写作辅助对科学写作风格的影响。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在对话系统中的应用日益广泛，但其回应常带有微妙语调偏见（如过度礼貌、欢快或谨慎），这些偏见会影响用户对信任、同理心和公平性的感知，需要系统性地检测和衡量这些隐藏的行为特征。

Method: 1. 创建两个合成对话数据集：一个来自中性提示，另一个明确引导产生积极或消极语调；2. 使用预训练的DistilBERT模型进行弱监督标注；3. 训练多个分类器检测语调模式；4. 采用集成模型提高检测效果。

Result: 1. 即使中性数据集也显示出持续的语调偏差，表明偏见源于模型的底层对话风格；2. 集成模型实现了高达0.92的宏观F1分数，证明语调偏见是系统性、可测量的；3. 该方法能够有效检测对话AI中的语调偏见模式。

Conclusion: 语调偏见是大语言模型的隐藏行为特征，通过可控对话合成与语调分类模型的集成方法，可以系统性地检测和衡量这些偏见，这对于设计公平可信的对话AI系统具有重要意义。
...
TL;DR: ThinkARM框架使用Schoenfeld的Episode理论分析LLM推理过程，将推理轨迹抽象为功能步骤（分析、探索、实施、验证等），揭示数学问题解决中的思维动态和结构差异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然展示推理轨迹，但其底层认知结构和步骤难以识别和分析，需要超越表面统计的中间尺度分析框架来理解推理过程。

Method: 引入ThinkARM框架，基于Schoenfeld的Episode理论，将推理轨迹抽象为功能推理步骤（分析、探索、实施、验证等），应用于不同模型的数学问题解决过程分析。

Result: 该抽象揭示了推理模型与非推理模型之间可复现的思维动态和结构差异，探索步骤作为关键分支步骤与正确性相关，效率导向方法选择性地抑制评估反馈步骤而非均匀缩短响应。

Conclusion: 片段级表示使推理步骤显式化，能够系统分析现代语言模型中推理的结构、稳定性和变化方式，为理解模型推理过程提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 随着序列决策任务长度增加，在上下文中保持完整交互历史变得计算上不切实际，需要一种方法来压缩上下文同时保持决策性能

Method: 提出ABBEL框架：用信念状态（自然语言摘要）替代完整交互历史；每一步先更新先验信念形成后验信念，然后仅基于后验信念选择行动；通过RL训练改进，包括信念质量评分和长度惩罚

Result: ABBEL在六个多样化多步环境中保持恒定内存使用并生成可解释信念；但瓶颈方法易受错误传播影响，性能低于完整上下文设置；RL训练后ABBEL性能超越完整上下文设置，同时使用更少内存

Conclusion: ABBEL框架通过语言表达的信念瓶颈有效压缩多步决策上下文，结合RL训练可以超越完整上下文设置的性能，为长序列决策任务提供了内存高效且可解释的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态RAG在音频-视觉领域面临两大挑战：1）现有多模态知识图谱的模态覆盖有限且多跳连接不足；2）仅基于共享嵌入空间相似度的检索方法无法过滤无关或冗余知识。这些问题限制了多模态大语言模型的推理深度和答案忠实度。

Method: 提出M³KG-RAG框架，包含两个核心组件：1）轻量级多智能体流水线构建多跳多模态知识图谱（M³KG），包含上下文丰富的多模态实体三元组，支持基于输入查询的模态化检索；2）GRASP机制（Grounded Retrieval And Selective Pruning），确保实体与查询的精确对齐，评估答案支持相关性，并剪枝冗余上下文，仅保留生成响应所需的关键知识。

Result: 在多个多模态基准测试上的广泛实验表明，M³KG-RAG显著提升了多模态大语言模型的多模态推理和基础能力，优于现有方法。

Conclusion: M³KG-RAG通过构建多跳多模态知识图谱和引入GRASP机制，有效解决了音频-视觉领域多模态RAG的局限性，显著提升了多模态大语言模型的推理深度和答案忠实度，为多模态知识检索和增强生成提供了有效解决方案。
...
TL;DR: 本文提出了一种名为早期知识对齐（EKA）的模块，用于改进迭代式检索增强生成系统。该模块通过在规划前将大语言模型与检索到的相关知识对齐，显著提升了检索精度和推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有的迭代式RAG系统在规划问题分解时通常不考虑可用的检索语料库信息，导致检索效率低下和推理链级联错误，最终影响性能。需要一种方法在规划前让模型更好地理解检索到的知识。

Method: 提出早期知识对齐（EKA）模块，在迭代式RAG系统的规划阶段之前，将大语言模型与检索到的上下文相关知识对齐。这是一种无需训练、可扩展的推理策略。

Result: 在六个标准RAG数据集上的实验表明，EKA显著提高了检索精度，减少了级联错误，提升了性能和效率。从熵的角度分析显示，早期知识减少了推理过程中的不必要探索。

Conclusion: EKA通过建立更强的推理基础，推进了迭代式RAG系统的最新技术水平，同时阐明了强化学习增强框架中结构化推理与高效探索之间的关键相互作用。
...
TL;DR: RetroPrompt是一种新的提示学习方法，通过检索机制从知识库中获取相关上下文信息，减少对死记硬背的依赖，提高模型在零样本和少样本场景下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的PFMs提示学习方法仍遵循参数化学习范式，存在过度依赖记忆和浅层模式的问题，特别是在数据有限的情况下难以充分利用非典型实例，泛化稳定性不足。

Method: RetroPrompt通过解耦知识和记忆，利用训练数据生成的公开知识库，在输入、训练和推理阶段引入检索机制，主动从语料库中检索相关上下文信息来增强可用线索。

Result: 在自然语言处理和计算机视觉任务的多数据集实验中，RetroPrompt在零样本和少样本场景下表现出优越性能，通过记忆模式分析显示能有效减少对死记硬背的依赖。

Conclusion: RetroPrompt通过检索增强的提示学习方法，在记忆和泛化之间取得了更好的平衡，提高了PFMs在数据有限情况下的性能表现。
...
TL;DR: Fun-Audio-Chat是一个大型音频语言模型，通过双分辨率语音表示和核心鸡尾酒训练解决了语音-文本模型中的时间分辨率不匹配和灾难性遗忘问题，在语音理解、推理和生成方面表现出色。
...
Method: 1）双分辨率语音表示（DRSR）：共享LLM以高效的5Hz处理音频（通过标记分组），而语音精炼头以25Hz生成高质量标记，平衡效率和质量；2）核心鸡尾酒训练：两阶段微调与中间合并，缓解灾难性遗忘；3）多任务DPO训练：增强鲁棒性、音频理解、指令跟随和语音共情能力。

Result: Fun-Audio-Chat 8B和MoE 30B-A3B在语音转文本和语音转语音任务上表现优异，在口语问答基准测试中在同类规模模型中排名靠前。在音频理解、语音功能调用、指令跟随和语音共情方面也达到竞争性甚至更优的性能。还开发了全双工变体Fun-Audio-Chat-Duplex。

Conclusion: Fun-Audio-Chat通过创新的双分辨率架构和多阶段训练方法，有效解决了现有语音-文本模型的局限性，在保持文本LLM知识的同时获得了强大的音频理解、推理和生成能力，无需大规模音频-文本预训练，仅利用预训练模型和广泛的后训练即可实现。
...
TL;DR: 研究发现LLMs在简历筛选等应用中存在对抗性指令攻击漏洞，攻击成功率超80%，提出FIDS防御方法显著降低攻击率


<details>
  <summary>Details</summary>
Motivation: LLMs在代码审查、内容审核等任务中表现出色，但研究发现它们容易受到隐藏在输入数据（如简历、代码）中的"对抗性指令"攻击，导致偏离预期任务。虽然代码审查等成熟领域可能有防御措施，但简历筛选、同行评审等常见应用往往缺乏相应防护。

Method: 论文引入了一个基准测试来评估简历筛选中的这种漏洞，并评估了两种防御机制：基于提示的防御和提出的FIDS（通过分离检测外部指令）。FIDS使用LoRA适配技术，结合两种方法形成综合防御策略。

Result: 基准测试显示某些攻击类型的成功率超过80%。基于提示的防御实现了10.1%的攻击减少，但误拒率增加12.5%；FIDS实现了15.4%的攻击减少，误拒率增加10.4%。组合方法提供26.3%的攻击减少，表明训练时防御在安全性和效用保持方面优于推理时缓解措施。

Conclusion: LLMs在简历筛选等应用中存在对抗性指令攻击的重大安全漏洞，需要专门的防御机制。提出的FIDS方法结合LoRA适配技术能有效降低攻击成功率，训练时防御策略比推理时缓解措施更有效，为实际应用中的LLM安全提供了重要解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在大语言模型的实际应用中，如检索增强生成和摘要，识别输出中是否包含忠实性幻觉至关重要，需要提高模型的可信度。

Method: 1. 使用先进LLM合成带解释的训练数据，并通过数据过滤策略确保标签正确性、解释质量和数据多样性；2. 在精心策划的训练数据上进行微调作为冷启动；3. 使用基于规则的强化学习进一步优化，奖励预测正确性和解释质量。

Result: 在12个多样化任务上，8B参数的FaithLens超越了GPT-4.1和o3等先进模型，并能生成高质量的解释，实现了可信度、效率和效果之间的独特平衡。
...
TL;DR: 该研究创建了一个跨语言对话语料库，用于评估自动语音翻译系统，并探索了跨语言会议中误解的自动检测方法。


<details>
  <summary>Details</summary>
Motivation: 为了评估在无共同语言个体会议中使用的自动语音翻译系统，需要一个真实且多功能的评估语料库。同时，跨语言交流中误解的检测也是一个重要研究问题。

Method: 创建了包含12种原始语言的5小时语音记录语料库，包含ASR转录、黄金转录、自动翻译和人工修正的英文翻译。为研究跨语言摘要，还包含了会议纪要。同时，手动标注误解并测试大型语言模型（特别是Gemini模型）的自动检测能力。

Result: 建立了包含多语言语音数据和翻译的语料库。在误解检测方面，Gemini模型能够以77%的召回率和47%的精确度识别包含误解的文本片段。

Conclusion: 该研究提供了一个有价值的跨语言对话评估资源，并展示了大型语言模型在检测跨语言交流误解方面的潜力，尽管精确度仍有提升空间。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动幻灯片生成方法通常假设统一标准，但用户偏好差异很大，导致生成结果与个体需求不匹配。需要一种能够根据用户特定偏好定制幻灯片的方法。

Method: 提出SlideTailor框架：1) 通过论文-幻灯片示例对和视觉模板隐式学习用户偏好；2) 采用渐进式生成可编辑幻灯片；3) 引入chain-of-speech机制使幻灯片内容与口头叙述计划对齐；4) 构建包含多样化用户偏好的基准数据集

Result: 实验证明该框架能有效提取和泛化用户偏好，显著提升生成幻灯片质量，并支持视频演示等下游应用

Conclusion: SlideTailor通过自然易提供的输入（示例对和模板）隐式学习用户偏好，实现了用户对齐的个性化幻灯片生成，解决了现有方法个性化不足的问题
...
TL;DR: LLMs在精神疾病诊断中表现优于人类专家，但在自恋型人格障碍诊断上存在严重偏差，显示出对价值负载术语的回避倾向。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在精神科自我评估中的使用增加，需要评估其解释定性患者叙述的能力，特别是与心理健康专业人员的比较。

Method: 使用波兰语第一人称自传体叙述，对最先进的LLMs和心理健康专业人员进行边缘型人格障碍和自恋型人格障碍诊断的直接比较。

Result: Gemini Pro模型在总体诊断准确率上比人类专业人员高出21.91个百分点（65.48% vs. 43.57%）。模型在识别边缘型人格障碍方面表现出色，但在自恋型人格障碍诊断上严重不足（F1=6.7 vs. 50.0），显示出对"自恋"这一价值负载术语的回避。
...
TL;DR: SpidR是一种自监督语音表示模型，通过掩码预测、自蒸馏和在线聚类学习具有丰富语音信息的表示，在文本无关的语音语言建模任务上优于现有方法，且训练时间大幅缩短。


<details>
  <summary>Details</summary>
Motivation: 随着语言建模和语音表示学习的并行发展，直接从语音中学习语言而无需文本中间体成为可能，这需要从语音中直接提取语义表示。

Method: SpidR在原始波形上训练，使用掩码预测目标结合自蒸馏和在线聚类。学生模型的中间层学习预测来自教师模型中间层的分配，这种学习目标稳定了在线聚类过程，产生更高质量的码本。

Result: SpidR在下游语言建模基准测试（sWUGGY, sBLIMP, tSC）上优于wav2vec 2.0、HuBERT、WavLM和DinoSR。验证了语音单元质量与语言建模性能之间的相关性。相比HuBERT，训练时间从一周缩短到一天（16个GPU）。

Conclusion: SpidR为文本无关的语音语言建模提供了高质量的语音表示，训练效率显著提升，相关代码和模型已开源。
...
TL;DR: BanglaRiddleEval是一个包含1,244个孟加拉语传统谜语的基准测试，包含四个任务类型，评估LLM在低资源比喻推理方面的能力。结果显示当前LLM表现远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在标准NLP基准上表现出色，但在比喻性、文化基础和低资源环境下的推理能力尚未充分探索。研究针对孟加拉语填补这一空白，评估LLM在文化特定比喻推理方面的能力。

Method: 创建BanglaRiddleEval基准，包含1,244个传统孟加拉语谜语，实例化为四个任务（共4,976个谜语-任务实例）。使用LLM管道生成思维链解释、语义连贯的干扰项和细粒度歧义标注，评估开源和闭源模型在不同提示策略下的表现。

Result: 模型在生成式QA上达到中等语义重叠但正确率低，多项选择题准确率最高仅约56%（人类基线为83%），歧义解决率在26%到68%之间，高质量解释仅限于最强模型。当前LLM能捕捉一些孟加拉语谜语推理线索，但远未达到人类水平。

Conclusion: BanglaRiddleEval为低资源比喻推理建立了一个具有挑战性的新基准，显示当前LLM在文化特定比喻推理方面仍有很大提升空间。所有数据、代码和评估脚本已开源。
...
TL;DR: 提出一个情感感知的文本摘要框架，针对社交媒体等非结构化用户生成内容，将情感信号嵌入到抽取式和生成式摘要方法中，以提升情感细微差别和主题相关性的捕捉能力。


<details>
  <summary>Details</summary>
Motivation: 随着社交媒体、评论和论坛中非结构化数据的快速增长，文本挖掘在信息系统（IS）中变得至关重要。现有摘要方法主要针对结构化新闻优化，难以处理嘈杂、非正式的用户生成内容。情感线索对于品牌监控和市场分析等IS任务至关重要，但很少有研究将情感建模整合到短用户生成文本的摘要中。
...
Conclusion: 提出的情感感知摘要框架能够有效处理非结构化用户生成内容，通过整合情感建模提升摘要质量，为信息系统中的品牌监控、市场分析等任务提供更好的支持。
...
### [68] [Step-DeepResearch Technical Report](https://arxiv.org/abs/2512.20491)
...
TL;DR: Step-DeepResearch：一种用于深度研究任务的成本效益型端到端智能体，通过原子能力数据合成策略和渐进式训练路径，在中文领域ADR-Bench上表现优异，32B模型在Scale AI Research Rubrics上达到61.4%得分。


<details>
  <summary>Details</summary>
Motivation: 现有学术基准如BrowseComp无法满足现实世界对开放式深度研究的需求，这些需求包括意图识别、长时程决策和跨源验证等能力。同时中文领域缺乏深度研究评估基准。

Method: 1. 提出基于原子能力的数据合成策略来增强规划和报告撰写能力；2. 采用从智能体中期训练到SFT和RL的渐进式训练路径；3. 引入清单式评判器提升鲁棒性；4. 建立中文领域ADR-Bench评估基准。

Result: Step-DeepResearch（32B）在Scale AI Research Rubrics上获得61.4%得分。在ADR-Bench上显著优于同类模型，并与OpenAI和Gemini DeepResearch等闭源SOTA模型表现相当。

Conclusion: 精细化训练使中等规模模型能够以行业领先的成本效益实现专家级能力，证明了在深度研究任务上成本效益型解决方案的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 将预训练的softmax注意力Transformer蒸馏为混合架构（交替使用softmax和线性注意力层）可以显著提高LLM的推理效率，而无需昂贵的从头预训练。关键挑战在于如何选择要转换的层。

Method: 使用通用文本数据的少量训练来获取层重要性评分，基于这些评分选择要转换为线性注意力的层。然后采用RADLADS蒸馏流程，包括注意力权重转移、隐藏状态对齐、KL分布匹配和少量微调。

Result: 该方法比现有层选择方法更有效，包括基于固定比例均匀交错线性注意力的启发式方法，以及依赖专门诊断数据集的更复杂方法。

Conclusion: 基于层重要性评分的简单方法为将预训练Transformer蒸馏为高效混合架构提供了有效的层选择策略，优于现有方法。
...
TL;DR: Gnosis是一种轻量级自感知机制，让冻结的LLM通过解码隐藏状态和注意力模式进行内在自我验证，以预测自身错误，仅增加约500万参数且与序列长度无关。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能生成流畅复杂的输出，但往往无法识别自己的错误和幻觉。现有方法依赖外部评判、多样本一致性或基于文本的自我批评，这些方法要么增加额外计算成本，要么与真实正确性相关性较弱。

Method: Gnosis通过被动观察内部痕迹（隐藏状态和注意力模式），将其压缩为固定预算的描述符，然后预测正确性。该方法仅增加约500万参数，推理成本可忽略，且独立于序列长度。

Result: 在数学推理、开放域问答和学术知识基准测试中，针对1.7B到20B参数的冻结骨干模型，Gnosis在准确性和校准方面始终优于强大的内部基线和大型外部评判器。此外，它能零样本泛化到部分生成，实现早期错误检测和计算感知控制。

Conclusion: 可靠的正确定性线索内在于生成过程，无需外部监督即可高效提取。这表明LLM可以通过内部状态进行有效的自我验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在空间和序列推理方面的评估缺乏系统性的基准测试，需要开发一个紧凑、可复现的测试工具来评估模型在复杂空间任务中的表现。

Method: 创建Cube Bench基准，包含五个技能维度：从图像和文本重建魔方面、选择最优下一步、预测候选移动结果、执行多步计划并恢复错误、检测和修正自身错误。使用共享的魔方状态集、相同提示和解析器，以及单一解决距离度量来比较不同MLLMs。

Result: 测试七个MLLMs显示：准确率随魔方复杂度深度急剧下降；一旦轨迹停滞或偏离，模型很少能恢复；高面重建准确率不能保证有效的动作选择或多步执行；闭源模型在单步感知和多步控制任务上都领先，开源模型在困难设置下接近随机水平；即使最佳MLLM在更高复杂度下也会退化；简单的自我修正能带来适度改进但也可能引入过度思考。

Conclusion: Cube Bench提供了一个紧凑、可复现的MLLMs序列空间推理能力探测工具，揭示了当前MLLMs在复杂空间推理任务中的局限性，特别是在多步执行和错误恢复方面的不足。
...
TL;DR: MoE-DiffuSeq是一个基于专家混合的框架，用于增强扩散模型在长文档生成中的性能，通过稀疏注意力和专家混合架构解决现有扩散文本生成模型的计算成本和内存开销问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的文本生成模型（如DiffuSeq）在处理长序列时存在高计算成本和内存开销的问题，这限制了它们在长文档生成场景中的实际应用。

Method: 1. 集成稀疏注意力与专家混合架构，实现高效可扩展的长序列建模；2. 引入定制的稀疏注意力机制，降低计算复杂度同时保持文本质量和连贯性；3. 在扩散过程中加入软吸收状态，加速序列重建并提高生成精度。

Result: MoE-DiffuSeq显著提高了训练效率和采样速度，特别是在长文档场景中（包括科学文章生成、代码库建模和长篇对话生成），在效率、速度、准确性和表达能力方面都有改进。

Conclusion: MoE-DiffuSeq通过稀疏注意力和专家混合架构，提高了扩散模型在高质量长文本生成中的实际适用性，为长文档生成任务提供了有效的解决方案。
...
TL;DR: 该研究提出了一种双目标优化方法，用于灾后救援物资配送的车辆路径规划和有限资源分配，同时考虑效率（总旅行时间最小化）和公平性（基于基尼系数的不满足需求不平等最小化）。


<details>
  <summary>Details</summary>
Motivation: 重大灾害中，预置的救援物资往往无法满足所有需求，需要在有限资源下平衡配送效率和公平性，确保救援物资能够及时且公平地分配到各个避难所。

Method: 建立混合整数规划模型，采用ε-约束方法处理双目标优化。通过推导最优解的数学特性，引入有效不等式，设计给定可行车辆路径下的最优配送分配算法，并开发分支定价算法进行高效求解。

Result: 在土耳其凡城地震和伊斯坦布尔卡塔尔地区的实际数据集上测试表明，分支定价算法显著优于商业MIP求解器。双目标方法在不影响效率的情况下，将援助分配不平等降低了34%。

Conclusion: 当时间约束非常宽松或严格时，优先考虑需求覆盖的词典序优化是有效的；对于中等限制的时间约束，平衡方法对于避免不公平结果是必要的。该研究为灾后人道主义物流提供了有效的决策支持工具。
...
TL;DR: 本文提出了一种用于PlantClef 2025挑战赛的方法，通过从训练数据集中提取类别原型作为指导，训练分割Vision Transformer在测试集图像上进行多标签物种识别。


<details>
  <summary>Details</summary>
Motivation: PlantClef 2025挑战赛需要从高分辨率图像中进行细粒度多标签物种识别。为了解决这一挑战，作者提出了一种方法，能够从多类别单物种识别适应到高分辨率植被地块的多标签分类。

Method: 方法包括：1) 从训练数据集图像中提取特征并应用K-Means聚类创建类别原型；2) 构建定制化的窄Vision Transformer，用预训练的DinoV2替换补丁嵌入层；3) 训练模型从测试数据集图像重建训练数据集的类别原型；4) 使用注意力分数识别和定位感兴趣区域，从而指导分类过程。

Result: 该方法在PlantCLEF 2025挑战赛的私有排行榜上获得第五名，F1分数为0.33331。与最佳提交相比仅低0.03分，表明在基准任务中具有竞争力。

Conclusion: 提出的方法成功实现了从多类别单物种识别到高分辨率植被地块多标签分类的领域适应，在PlantCLEF 2025挑战赛中取得了有竞争力的性能。
...
TL;DR: 该论文提出了一种通过类内聚类分配分类来学习细粒度特征的新方法，以缓解细粒度视觉分类任务中的类内变异问题，特别是在类别样本不足的情况下。
...
Result: 在PlantNet300k数据集上的初步实验显示，该方法即使在某些组件尚未完全优化的情况下，仍然达到了最先进的性能水平。

Conclusion: 该方法通过类内聚类和分层分类有效缓解了细粒度视觉分类中的类内变异问题，为未来研究提供了关键方向，代码已开源供进一步研究。
...
TL;DR: 提出LieFlow方法，通过李群上的流匹配直接从数据中学习对称性，无需先验知识，能够发现离散群等复杂对称结构
...
Method: 提出LieFlow方法，将对称性发现定义为在李群上学习分布，使学习到的分布与数据中观察到的对称性匹配。使用流匹配技术，并针对对称性发现中的"最后一分钟收敛"问题提出了新的插值方案

Result: 在2D和3D点云数据上成功发现了离散群，包括通过复数域上的流匹配发现的反射对称性。新方法比先前工作更灵活，需要更少的假设

Conclusion: LieFlow能够直接从数据中学习对称性，为物理系统理解和机器学习应用提供了有效的对称性发现工具，解决了对称性发现中的关键挑战
...
TL;DR: SOF框架从无动作视频中学习基于光流的潜在技能表示，连接视频生成模型与机器人动作执行，实现高层次规划


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型难以转化为低级动作，而潜在动作模型缺乏高层次规划能力，需要弥合视频学习与机器人动作执行之间的鸿沟

Method: 提出SOF框架，通过光流作为中间表示学习潜在技能空间，该表示能同时捕捉视频动态和机器人动作信息，实现从视频到技能的抽象

Result: 实验表明SOF在多任务和长时域设置中持续提升性能，能够直接从原始视觉数据中获取和组合技能

Conclusion: 通过光流表示的技能抽象有效连接了视频学习与机器人动作执行，为从视频数据中学习通用机器人技能提供了可行路径
...
<details>
  <summary>Details</summary>
Motivation: 气候变化导致极端天气灾害频发，快速准确的灾害位置识别对应急响应和资源分配至关重要，需要提高地理定位的准确性和可解释性

Method: 提出ProbGLC概率交叉视角地理定位框架，将概率模型和确定性模型统一，通过不确定性量化和局部化评分增强模型可解释性，支持多种灾害类型

Result: 在两个灾害数据集上实验，达到0.86的Acc@1km和0.97的Acc@25km定位精度，同时提供概率分布和局部化评分增强可解释性

Conclusion: ProbGLC在灾害地理定位中表现出优越性能，具有快速灾害响应的潜力，通过生成式交叉视角方法促进位置感知能力
...
TL;DR: 该研究系统评估了使用强化学习训练大语言模型进行内容审核的方法，发现RL在需要复杂政策推理的任务上表现优异，且数据效率比监督微调高100倍。


<details>
  <summary>Details</summary>
Motivation: 当前数字生态系统中，大规模内容审核面临严峻挑战，需要处理数十亿用户和AI生成的内容。虽然大语言模型在政策驱动的审核方面显示出潜力，但在真实场景中训练这些系统达到专家级准确度仍存在挑战，特别是在标签稀疏、政策定义不断演变、需要超越浅层模式匹配的细致推理等情况下。

Method: 通过全面的实证研究，系统评估了多种RL训练方案和奖励塑造策略，包括可验证奖励和LLM作为评判框架，将通用语言模型转化为专门的政策对齐分类器。在三个真实世界内容审核任务上进行实验。

Result: RL表现出S型扩展行为，性能随训练数据、rollouts和优化步骤增加而平滑提升后逐渐饱和。RL在需要复杂政策推理的任务上显著提升性能，数据效率比监督微调高100倍，特别适用于专家标注稀缺或成本高的领域。

Conclusion: 强化学习为工业级内容审核系统提供了可行的解决方案，特别是在需要复杂政策推理且标注数据有限的场景中，RL训练方法具有显著优势。
...
Result: 在三个医疗数据集上评估，包括专有分诊数据集和公共生物医学QA数据集。Reason2Decide在不同模型规模下均优于其他微调基线，在预测（F1）和解释保真度（BERTScore、BLEU、LLM-as-a-Judge）方面表现优异。在分诊任务中，对LLM生成、护士撰写和护士后处理的解释均具有鲁棒性。

Conclusion: Reason2Decide使用比当代基础模型小40倍的模型取得了显著性能提升，使临床推理在资源受限环境中更加可访问，同时提供可解释的决策支持。LLM生成的解释适合用于模型预训练，减少对人类标注的依赖。
...
TL;DR: MolAct是一个基于智能体强化学习的分子编辑与优化框架，通过两阶段训练让LLM智能体学习使用化学工具进行多步分子设计，在编辑和优化任务上均表现出色。


<details>
  <summary>Details</summary>
Motivation: 分子编辑和优化是多步骤问题，需要在保持化学有效性和结构相似性的同时迭代改进性质。传统方法难以同时处理这些复杂约束，需要一种能够结合推理、工具使用和分子优化的智能方法。

Method: 提出MolAct框架，将分子设计形式化为智能体强化学习问题。采用两阶段训练：第一阶段建立编辑能力，第二阶段重用学习到的编辑行为进行性质优化。智能体通过多轮交互调用化学工具进行有效性检查、性质评估和相似性控制，并根据反馈细化后续编辑。

Result: 在分子编辑任务中，MolEditAgent-7B在添加、删除和替换编辑上分别达到100、95和98的有效性，优于DeepSeek-R1等基线；MolEditAgent-3B性能接近Qwen3-32B-think等更大模型。在分子优化任务中，MolOptAgent-7B在LogP上超越Claude 3.7等基线，在溶解度上保持竞争力，并在其他目标上保持平衡性能。

Conclusion: 将分子设计视为多步骤、工具增强的过程是实现可靠且可解释改进的关键。MolAct框架展示了智能体强化学习在分子设计中的潜力，为复杂化学优化问题提供了新思路。
...
TL;DR: 该论文提出通过在时间序列数据token化前注入噪声的方法，提升冻结大语言模型在零样本时间序列预测中的性能，无需微调即可增强模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在利用冻结大语言模型进行时间序列预测时，性能对输入数据的文本表示非常敏感，因为模型参数无法适应分布变化。需要一种简单有效的方法来克服这种脆弱性。

Method: 在原始时间序列数据token化前注入噪声，作为一种推理时数据增强手段，迫使冻结大语言模型基于稳健的底层时间模式而非表面数值伪影进行外推。

Result: 该方法在多个基准测试中表现出有效性。特别地，为了完全消除LLM预训练中数据污染可能带来的偏差，作者引入了两个新颖的时间序列数据集，在这些数据集上一致观察到性能提升。

Conclusion: 噪声注入是一种简单有效的策略，能够提升冻结大语言模型在时间序列预测中的鲁棒性，为直接利用现成大语言模型进行时间序列预测提供了进一步的研究方向。
...
### [83] [A Bidirectional Gated Recurrent Unit Model for PUE Prediction in Data Centers](https://arxiv.org/abs/2512.20161)
*Dhivya Dharshini Kannan,Anupam Trivedi,Dipti Srinivasan*

Main category: cs.AI

TL;DR: 该研究开发了基于双向门控循环单元(BiGRU)的数据中心PUE预测模型，并与GRU模型进行性能对比，旨在优化数据中心能源管理。


<details>
  <summary>Details</summary>
Motivation: 数据中心能耗巨大且碳足迹显著，边缘计算和AI发展推动存储容量增长。能源效率是应对气候变化、降低成本、提升竞争力和促进可持续发展的关键。PUE作为数据中心运营效率指标，其预测有助于理解各特征对能耗的影响，从而针对性地改进能源效率。

Method: 1. 使用EnergyPlus模拟新加坡数据中心，获得52,560个样本和117个特征的数据集；2. 采用递归特征消除与交叉验证(RFECV)算法选择最相关特征集；3. 开发BiGRU模型并优化超参数配置；4. 与GRU模型对比，使用均方误差(MSE)、平均绝对误差(MAE)和R平方指标评估性能。

Result: 通过RFECV算法筛选出不同参数设置下的最优特征集，用于训练和优化BiGRU模型。研究比较了优化后的BiGRU模型与GRU模型在PUE预测上的性能表现。

Conclusion: BiGRU模型在PUE预测方面表现出优于GRU的性能，为数据中心能源管理优化提供了有效的预测工具，有助于通过针对性改进关键特征来提升能源效率。
...
### [84] [Concept Generalization in Humans and Large Language Models: Insights from the Number Game](https://arxiv.org/abs/2512.20162)
*Arghavan Bazigaran,Hansem Sohn*

Main category: cs.AI

TL;DR: 比较人类与大型语言模型在数字游戏概念推理任务中的泛化能力差异，发现人类更灵活地结合规则与相似性推理，而LLM更依赖数学规则，且人类能从单样本泛化而LLM需要更多样本。
...
Method: 使用数字游戏作为概念推理任务，以贝叶斯模型为分析框架，比较人类和LLM的归纳偏置和推理策略，评估两者在规则推理和相似性推理方面的表现。

Result: 贝叶斯模型能更好地捕捉人类行为：人类能灵活推断基于规则和基于相似性的概念，而LLM更依赖数学规则；人类能从单个示例实现少样本泛化，而LLM需要更多样本才能泛化。

Conclusion: 人类与大型语言模型在数学概念推断和泛化方面存在根本性差异，人类展现出更灵活的推理能力和更强的少样本学习能力。
...
### [85] [Offline Safe Policy Optimization From Heterogeneous Feedback](https://arxiv.org/abs/2512.20173)
*Ze Gong,Pradeep Varakantham,Akshat Kumar*

Main category: cs.AI

TL;DR: 本文提出PreSa方法，通过直接学习策略而非间接学习奖励和成本模型，解决离线偏好强化学习中的安全问题，在连续控制任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 离线偏好强化学习(PbRL)无需人工标注和奖励工程，但确保安全性仍是关键挑战。现有基于人类反馈的安全RL方法先学习奖励和成本模型，再用约束RL优化策略，在长时域连续控制任务中误差会累积，影响性能。

Method: 提出PreSa框架：1) 直接基于奖励偏好和轨迹段安全性的二元标签学习策略；2) 结合偏好学习模块和安全对齐的约束优化问题；3) 在拉格朗日框架内求解，直接学习奖励最大化且安全的策略，无需显式学习奖励和成本模型。

Result: 在连续控制任务中，使用合成和真实人类反馈进行评估，PreSa方法成功学习到高奖励的安全策略，优于最先进的基线方法，甚至优于使用真实奖励和成本的离线安全RL方法。

Conclusion: PreSa方法通过直接学习策略而非间接学习奖励和成本模型，有效解决了离线偏好强化学习中的安全对齐问题，在连续控制任务中表现出色，避免了传统方法中误差累积的问题。
...
### [86] [TongSIM: A General Platform for Simulating Intelligent Machines](https://arxiv.org/abs/2512.20206)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI特别是多模态大语言模型的快速发展，研究重点正从单模态文本处理转向更复杂的多模态和具身AI领域。现有模拟平台大多设计狭窄，针对特定任务，缺乏能够支持从低级具身导航到高级复合活动（如多智能体社会模拟和人机协作）的通用训练环境。

Method: 开发了TongSIM平台，提供100多个多样化、多房间的室内场景以及开放式、交互丰富的户外城镇模拟。平台具有定制场景、任务自适应保真度、多样化智能体类型和动态环境模拟等特性，提供全面的评估框架和基准测试。

Result: TongSIM为研究人员提供了灵活可扩展的统一平台，能够精确评估智能体的感知、认知、决策、人机协作以及空间和社会推理等能力，加速了通用具身智能的训练、评估和进步。

Conclusion: TongSIM填补了现有模拟平台的空白，作为一个高保真、通用型平台，为具身智能研究提供了统一的训练和评估环境，支持从低级到高级的各种任务，推动了通用具身智能的发展。
...
### [87] [ActionFlow: A Pipelined Action Acceleration for Vision Language Models on Edge](https://arxiv.org/abs/2512.20276)
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在边缘设备上推理延迟高（仅3-5Hz），无法满足机器人交互所需的20-30Hz实时性要求，而现有优化方法要么需要大量重新训练，要么会牺牲模型精度。
...
Conclusion: ActionFlow通过创新的系统级优化方法，有效解决了VLA模型在边缘设备上的高延迟问题，为实现实时机器人控制提供了可行方案。
...
### [88] [A DeepSeek-Powered AI System for Automated Chest Radiograph Interpretation in Clinical Practice](https://arxiv.org/abs/2512.20344)
...
TL;DR: Janus-Pro-CXR是基于DeepSeek Janus-Pro开发的胸部X光解读系统，通过多中心前瞻性临床试验验证，在报告生成质量、关键放射学发现检测和工作流程效率方面优于现有模型，特别适合资源有限环境。
...
Method: 开发基于DeepSeek Janus-Pro模型的Janus-Pro-CXR（1B参数）胸部X光解读系统，通过多中心前瞻性临床试验（NCT07117266）进行严格验证，包括回顾性和前瞻性评估。

Result: 系统在自动化报告生成方面优于最先进的X光报告生成模型，包括ChatGPT 4o（200B参数）；可靠检测六种临床关键放射学发现；前瞻性临床部署中，AI辅助显著提高报告质量评分，减少18.3%解读时间（P<0.001），54.3%案例中专家更偏好AI辅助结果。

Conclusion: 通过轻量级架构和领域特定优化，Janus-Pro-CXR提高了诊断可靠性和工作流程效率，特别适合资源有限环境。模型架构和实施框架将开源，促进AI辅助放射学解决方案的临床转化。
...
### [89] [Benchmarking LLMs for Predictive Applications in the Intensive Care Units](https://arxiv.org/abs/2512.20520)
*Chehak Malhotra,Mehak Gopal,Akshaya Devadiga,Pradeep Singh,Ridam Pal,Ritwik Kashyap,Tavpritesh Sethi*

Main category: cs.AI

TL;DR: 该研究比较了大型语言模型（LLMs）和特定领域语言模型（SLMs）在预测危重患者休克方面的性能，发现尽管GatorTron-Base表现最佳，但LLMs在预测临床事件方面并不固有优于SLMs。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs的发展，其在自然语言处理领域的应用日益广泛，但在预测性任务中的应用研究相对较少。及时预测休克能够实现早期干预，从而改善患者预后，因此需要评估不同语言模型在临床预测任务中的表现。

Method: 研究比较了GatorTron-Base（基于临床数据训练）、Llama 8B和Mistral 7B等大型语言模型，以及BioBERT、DocBERT、BioClinicalBERT、Word2Vec和Doc2Vec等特定领域模型。使用MIMIC III数据库中17,294例ICU住院患者的文本数据，筛选出住院时间>24小时且休克指数(SI)>0.7的患者，得到355例正常SI和87例异常SI患者。在微调过程中使用焦点损失和交叉熵损失来处理类别不平衡问题。

Result: GatorTron-Base获得了最高的加权召回率80.5%，但整体性能指标显示SLMs和LLMs之间的表现相当。这表明尽管LLMs在基于文本的任务上表现出色，但在预测未来临床事件方面并不固有优于SLMs。

Conclusion: 为了实现有意义的临床结果，未来训练LLMs的努力应优先开发能够预测临床轨迹的模型，而不是专注于命名实体识别或表型分析等较简单的任务。LLMs在临床预测任务中并不固有优于专门训练的SLMs。
...
### [90] [Advancing Multimodal Teacher Sentiment Analysis:The Large-Scale T-MED Dataset & The Effective AAM-TSA Model](https://arxiv.org/abs/2512.20548)
...
<details>
  <summary>Details</summary>
Motivation: 教师情感状态在教育场景中至关重要，但现有研究往往因教师的表演性质而无法准确捕捉其情感，且忽视了教学信息对情感表达的关键影响。

Method: 1. 构建T-MED数据集：包含14,938个教师情感数据实例，来自250个真实课堂，涵盖11个学科，整合了文本、音频、视频和教学信息等多模态数据，采用人机协作标注流程确保标注质量。2. 提出AAM-TSA模型：引入非对称注意力机制和分层门控单元，实现差异化的跨模态特征融合和精确的情感分类。

Result: 实验结果表明，AAM-TSA模型在T-MED数据集上的准确性和可解释性方面显著优于现有的最先进方法。

Conclusion: 本研究通过构建大规模教师多模态情感分析数据集和提出创新的非对称注意力模型，为准确分析教师情感状态提供了有效的解决方案，有助于提升教学效果和学生参与度。
...
### [91] [Automated stereotactic radiosurgery planning using a human-in-the-loop reasoning large language model agent](https://arxiv.org/abs/2512.20586)
...
TL;DR: 研究测试了思维链推理是否能改善立体定向放射外科（SRS）的自动计划质量，开发了SAGE智能体，发现推理模型在保持计划质量的同时降低了耳蜗剂量，并展现出可审计的推理过程。


<details>
  <summary>Details</summary>
Motivation: 立体定向放射外科需要精确的剂量分布，但黑盒AI系统由于不透明性而临床采用有限。研究旨在探索思维链推理是否能提高自动计划系统的透明度和性能。

Method: 开发了基于大语言模型的SAGE自动计划智能体，在41例脑转移瘤患者的回顾性队列中测试。比较了推理模型和非推理模型两种变体，分析计划质量、剂量参数和推理过程。

Result: 推理模型在主要终点（靶区覆盖率、最大剂量、适形指数、梯度指数）上与人类计划者相当（p>0.21），同时将耳蜗剂量降低至人类基线以下（p=0.022）。推理模型展现出系统性的规划行为，包括前瞻性约束验证和权衡考量。

Conclusion: 思维链推理不仅改善了自动计划的质量，还提供了可审计的优化痕迹，为实现透明、可信的自动放射外科计划开辟了道路。
...
### [92] [LongVideoAgent: Multi-Agent Reasoning with Long Videos](https://arxiv.org/abs/2512.20618)
*Runtao Liu,Ziyi Liu,Jiaqi Tang,Yue Ma,Renjie Pi,Jipeng Zhang,Qifeng Chen*

Main category: cs.AI

TL;DR: 提出多智能体框架解决长视频问答问题，通过主LLM协调定位智能体和视觉智能体，在长视频数据集上显著优于基线方法
...
Method: 提出多智能体框架：主LLM协调定位智能体定位问题相关片段，视觉智能体提取目标文本观察；主智能体使用步数限制进行规划，并通过强化学习训练以实现简洁、正确、高效的多智能体协作

Result: 在提出的LongTVQA和LongTVQA+数据集上，多智能体系统显著优于强基线方法；强化学习进一步增强了训练智能体的推理和规划能力

Conclusion: 多智能体框架通过定位智能体关注相关片段、视觉智能体补充字幕细节，产生可解释的轨迹，有效解决了长视频问答中的挑战
