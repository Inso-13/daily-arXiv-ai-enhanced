...
TL;DR: 本文提出了一种用于扩散模型后训练对齐的两阶段框架，通过构建层次化细粒度评估标准，并引入复杂偏好优化（CPO）方法，显著提升了绘画生成的质量与专家标准对齐度。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型的后训练对齐主要依赖简化的信号（如标量奖励或二元偏好），这限制了与复杂人类专业知识的对齐。人类专业知识具有层次化和细粒度的特点，需要更精细的对齐方法。

Method: 首先与领域专家合作构建层次化细粒度评估标准，将图像质量分解为多个正负属性并组织成树状结构。然后提出两阶段对齐框架：1）通过监督微调将领域知识注入辅助扩散模型；2）引入复杂偏好优化（CPO），将DPO扩展到非二元层次化标准，同时最大化正属性概率并最小化负属性概率。

Result: 在绘画生成领域实例化该方法，使用基于标准标注的细粒度属性数据集进行CPO训练。大量实验表明，CPO显著提升了生成质量和与专业知识的对齐度。

Conclusion: 该方法为细粒度标准对齐开辟了新途径，能够更好地将扩散模型与复杂的人类专业知识对齐，解决了现有简化对齐方法的局限性。
...
TL;DR: 提出一种基于RGB空间五进制像素强度组合的文本嵌入图像新方法，相比传统LSB、变换域和深度学习方法，能在单个像素内编码完整文本符号，计算效率更高且图像失真小。


<details>
  <summary>Details</summary>
Motivation: 现有文本嵌入图像方法（如LSB、PVD、变换域、深度学习等）大多依赖多像素操作、像素翻转或变换系数，导致图像噪声、计算复杂度高，且编码解码过程通常是确定性的。需要一种更高效、失真小、计算轻量的嵌入方法。

Method: 在RGB空间中采用五进制像素强度组合技术，每个R、G、B通道使用五种受控的不同像素强度变化，形成125种（5×5×5）不同的像素强度组合。将这些组合映射到文本符号（大小写字母、数字、空格、常用特殊字符），实现单个RGB像素编码完整文本符号。

Result: 通过MSE、MAE、SNR、PSNR、SSIM、直方图比较和热图分析等多种指标评估，原始图像与编码图像之间无明显失真。相比LSB/MSB方法需要多个像素或多步处理，以及变换和学习方法的高计算开销，本方法在单个像素内实现完整符号编码，提高了嵌入效率。

Conclusion: 提出的五进制RGB像素强度组合方法为文本嵌入图像提供了一种高效、低失真、计算轻量的替代方案，特别适合需要高嵌入效率和低计算开销的应用场景。
...
TL;DR: 该研究探索通过后训练实现完全统一的文本-图像生成，使模型能在单一推理过程中自主从文本推理过渡到视觉合成，并通过奖励加权的后训练方法在多个T2I基准上提升多模态图像生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态生成架构通常采用显式的模态切换机制，先生成推理文本再手动切换到图像生成，这种分离的序列推理过程限制了跨模态耦合，无法实现自动多模态生成。

Method: 采用后训练方法实现完全统一的文本-图像生成，探索不同后训练数据策略，使用离线奖励加权的后训练方法，利用完全自生成的合成数据，并研究奖励加权对两种模态的影响。

Result: 在四个不同的T2I基准测试中实现了多模态图像生成的改进，证明了奖励加权两种模态和策略性设计的后训练数据的有效性。针对特定限制的有针对性数据集比广泛的图像-标题语料库或基准对齐数据获得更优结果。

Conclusion: 通过后训练实现完全统一的文本-图像生成是可行的，奖励加权的后训练方法和策略性设计的训练数据能有效提升多模态生成性能，为自动多模态生成提供了新方向。
...
TL;DR: ReHyAt提出了一种循环混合注意力机制，将softmax注意力的保真度与线性注意力的效率相结合，实现了恒定内存使用，将注意力成本从二次降低到线性，同时保持高质量视频生成。


<details>
  <summary>Details</summary>
Motivation: 当前基于transformer的视频扩散模型虽然实现了最先进的视频生成，但存在二次注意力复杂度问题，严重限制了长序列的可扩展性。需要一种既能保持生成质量又能提高效率的解决方案。

Method: 提出ReHyAt循环混合注意力机制，结合softmax注意力的保真度和线性注意力的效率，支持分块循环重构和恒定内存使用。通过轻量级蒸馏和微调流程，从现有softmax模型高效蒸馏，训练成本降低两个数量级。

Result: 在VBench和VBench-2.0上的实验以及人类偏好研究表明，ReHyAt实现了最先进的视频质量，同时将注意力成本从二次降低到线性，训练成本仅需约160 GPU小时，比传统方法低两个数量级。

Conclusion: ReHyAt为长时长和设备端视频生成提供了实用的可扩展性解决方案，其混合设计能够高效蒸馏现有softmax模型，为未来最先进的双向softmax模型提供了可应用的配方。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅模型存储需求大，阻碍云和流媒体部署；现有标量量化方法无法有效捕捉高维特征向量相关性，限制了率失真性能

Method: 使用残差向量量化替代传统方法压缩基元特征，构建由多分辨率哈希网格引导的自回归熵模型，准确预测每个传输索引的条件概率

Result: 实现了粗层和细化层的高效压缩，提升了压缩效率

Conclusion: 提出的残差向量量化方法比传统标量量化能更有效地压缩3D高斯泼溅模型，为云和流媒体部署提供了更好的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在为实践者提供基于数据集特性、计算资源和性能要求选择合适深度学习方法的实用指导，特别是在资源有限和训练数据不足的情况下，比较不同方法的实际效果。

Method: 使用自定义CNN与预训练架构（ResNet-18和VGG-16）进行对比分析，采用特征提取和迁移学习两种方法，在五个孟加拉国图像分类数据集（人行道视觉、三轮车检测、芒果分类、水稻品种识别、道路损伤检测）上进行评估。
...
Conclusion: 对于复杂分类任务和有限训练数据，预训练模型结合迁移学习是更优选择；自定义CNN在模型大小和计算效率方面有优势，适合简单任务；实践者应根据具体需求选择合适方法。
...
<details>
  <summary>Details</summary>
Motivation: 统一自回归模型在处理多模态任务时使用KV缓存机制减少计算复杂度，但KV缓存大小随生成令牌数量线性增长，成为限制推理效率和生成长度的主要瓶颈。视频生成任务尤其受此限制影响。
...
Conclusion: PackCache通过利用KV缓存令牌的时空特性，有效解决了统一自回归视频生成中的KV缓存瓶颈问题，显著提升了长序列视频生成的效率，展示了其在实现更长序列视频生成方面的强大潜力。
...
TL;DR: 该研究提出使用3D面部几何特征进行压力识别，通过分析分心驾驶场景中的压力反应，开发了基于Transformer的跨模态注意力融合方法，显著提升了压力识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前基于面部动作单元的压力识别方法存在局限性，而解耦的3D面部几何特征在压力识别中的作用尚未充分探索。研究者希望探索3D面部几何特征在压力识别中的有效性，特别是在分心驾驶这种现实场景中。

Method: 研究使用EMOCA提取的3D表情和姿态系数分析分心驾驶中的压力反应。通过配对假设检验比较基线和压力阶段，发现41个系数显示一致的压力响应。然后提出基于Transformer的时间建模框架，评估了单模态、早期融合和跨模态注意力三种策略，特别关注EMOCA与生理信号的跨模态注意力融合。

Result: 41个EMOCA系数显示一致的压力响应，与生理标记物相当。跨模态注意力融合方法表现最佳：EMOCA与生理信号融合达到AUROC 92%、准确率86.7%；EMOCA与注视融合达到AUROC 91.8%。

Conclusion: 3D面部几何特征在压力识别中具有显著价值，时间建模和跨模态注意力策略能有效提升压力识别性能，为可靠的压力识别系统提供了新思路。
...
Method: 设计并从头训练一个自定义CNN，同时使用VGG-16、ResNet-50和MobileNet等流行架构进行迁移学习。在相同的实验设置下，使用准确率、精确率、召回率和F1分数等标准性能指标评估所有模型。

Result: 预训练CNN架构在分类准确率和收敛速度方面始终优于自定义CNN，特别是在训练数据有限的情况下。然而，自定义CNN在参数数量显著减少和计算复杂度降低的情况下表现出竞争性性能。

Conclusion: 本研究强调了模型复杂性、性能和计算效率之间的权衡，为图像分类问题中选择合适的CNN架构提供了实用见解。预训练模型在性能上占优，而自定义模型在资源受限场景下具有价值。
...
TL;DR: TriMARF是一个用于大规模3D对象标注的新型框架，通过整合2D多视角图像、文本描述和3D点云的三模态输入，采用多智能体协作架构，显著提升了3D标注的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶、机器人和增强现实等应用对3D对象标注提出了更高要求，但现有基于单一模型的方法难以有效解决空间复杂性、遮挡和视角不一致等问题，需要更强大的标注框架。

Method: 提出TriMARF框架，包含三个专门智能体：视觉语言模型智能体生成多视角描述，信息聚合智能体选择最优描述，门控智能体将文本语义与3D几何对齐进行精细化标注。

Result: 在Objaverse、LVIS、Objaverse XL和ABO数据集上的实验表明，TriMARF显著优于现有方法，CLIPScore达到88.7，ViLT R@5检索准确率为45.2和43.8，单A100 GPU上每小时可处理12000个对象。

Conclusion: TriMARF通过三模态输入和多智能体协作有效解决了3D标注的挑战，在准确性和效率方面均超越了现有方法，为大规模3D对象标注提供了强有力的解决方案。
...
TL;DR: 提出一种结合自监督和弱监督学习的混合框架，直接从术前CT扫描预测乳突切除术区域，无需人工标注，为人工耳蜗植入手术规划提供新方法。


<details>
  <summary>Details</summary>
Motivation: 人工耳蜗植入手术中，乳突切除术是重要步骤，准确的术前形状预测能改善手术规划、降低风险。但由于获取真实标签的困难，目前基于深度学习的研究有限，需要探索无需人工标注的解决方案。

Method: 提出混合自监督和弱监督学习框架，直接从术前完整乳突的CT扫描预测乳突切除术区域。该方法结合了自监督学习的特征提取能力和弱监督学习的标签利用效率，并采用3D T分布损失函数来处理医学图像中的不确定性。

Result: 混合方法在预测复杂且无明确边界的乳突切除术形状时，达到平均Dice分数0.72，超越了现有最先进方法，表现出色。该方法为从术前CT直接构建术后3D表面提供了基础。

Conclusion: 这是首个将自监督和弱监督学习结合用于乳突切除术形状预测的研究，为人工耳蜗手术规划提供了稳健高效的解决方案，同时推动了弱监督医学影像中3D T分布损失的应用。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在心脏MRI重建中表现出色但泛化能力有限，心脏MRI扫描存在图像对比度、采样模式、扫描设备、解剖结构和疾病类型等多方面差异，现有模型只能处理单一或狭窄子集，面对分布偏移时性能下降

Method: 提出CRUNet-MR-Univ基础模型，利用时空相关性和提示先验来处理心脏MRI扫描的完整多样性

Result: 该方法在广泛设置中一致优于基线方法，展示了其有效性和前景

Conclusion: 开发能够泛化到不同心脏MRI场景的统一模型是有益的，提出的基础模型通过利用时空相关性和提示先验有效处理了心脏MRI扫描的多样性
...
TL;DR: GPRO是一种元推理控制器，通过动态路由计算路径解决LVLM的过度思考问题，在提高准确率的同时减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型的链式思维推理虽然强大，但容易产生过度思考问题，导致冗长响应和效率低下。现有自适应推理方法忽视了视觉感知失败这一根本瓶颈，而推理错误往往源于不完美的感知而非思考不足。

Method: 提出Gated Perception-Reasoning Optimization (GPRO)，一个元推理控制器，在每个生成步骤动态路由三种计算路径：轻量快速路径、重新检查视觉输入的慢感知路径、内部自我反思的慢推理路径。从约79万样本中获取故障归因监督，使用教师模型区分感知幻觉和推理错误，通过多目标强化学习训练控制器优化准确率与计算成本的权衡。

Result: 在五个基准测试中，GPRO显著提高了准确率和效率，优于最近的慢思考方法，同时生成更短的响应。

Conclusion: GPRO通过动态路由感知和推理路径，有效解决了LVLM的过度思考问题，证明了稳定推理依赖于低层视觉基础，为视觉语言模型的高效推理提供了新思路。
...
TL;DR: UniDrive-WM是一个统一的视觉语言模型世界模型，将驾驶场景理解、轨迹规划和轨迹条件未来图像生成集成在单一架构中，在Bench2Drive基准测试中显著提升了规划性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶方法通常将感知、预测和规划作为独立模块处理，缺乏紧密集成。本文旨在探索使用统一的视觉语言模型世界模型来联合执行这些任务，以提升自动驾驶性能。
...
TL;DR: TokenSeg是一个基于边界感知稀疏令牌表示的3D医学图像分割框架，通过多尺度分层编码器提取候选令牌，边界感知分词器选择关键令牌，稀疏到密集解码器重建分割掩码，在保持高精度的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 3D医学图像分割计算需求大，存在立方体素增长和同质区域冗余计算的问题。需要开发高效的分割方法，在保持精度的同时减少计算资源消耗。

Method: 1) 多尺度分层编码器：在四个分辨率级别提取400个候选令牌，捕获全局解剖上下文和精细边界细节；2) 边界感知分词器：结合VQ-VAE量化和重要性评分选择100个显著令牌，其中60%以上位于肿瘤边界附近；3) 稀疏到密集解码器：通过令牌重投影、渐进上采样和跳跃连接重建全分辨率掩码。

Result: 在包含960个病例的3D乳腺DCE-MRI数据集上，TokenSeg达到94.49% Dice和89.61% IoU的SOTA性能，同时GPU内存和推理延迟分别减少64%和68%。在MSD心脏和脑部MRI基准数据集上的评估验证了其跨异质解剖结构的泛化能力。
...
TL;DR: 该研究开发了一个3D条件生成模型管道，用于合成高质量的LGE MRI图像，以增强左心房分割任务的训练数据，显著提高了分割性能。
...
Method: 开发了一个管道，从结合解剖专家标注和无监督组织聚类的复合语义标签图中合成3D LGE MRI体积，使用了三种3D条件生成器：Pix2Pix GAN、SPADE-GAN和SPADE-LDM。

Result: SPADE-LDM生成了最真实和结构准确的图像，FID得分为4.063，优于Pix2Pix GAN（40.821）和SPADE-GAN（7.652）。使用合成图像增强后，3D U-Net的左心房腔分割Dice分数从0.908提高到0.936，具有统计学显著性（p < 0.05）。

Conclusion: 标签条件的3D合成方法在增强代表性不足的心脏结构分割方面具有潜力，能够有效解决医学影像数据稀缺问题。
...
### [17] [HUR-MACL: High-Uncertainty Region-Guided Multi-Architecture Collaborative Learning for Head and Neck Multi-Organ Segmentation](https://arxiv.org/abs/2601.04607)
...
<details>
  <summary>Details</summary>
Motivation: 头颈部放疗中器官分割至关重要，但深度学习模型对小而形状复杂的器官分割效果不佳。现有混合架构简单拼接特征，未能充分利用各组件优势，导致功能重叠和分割精度有限

Method: 提出高不确定性区域引导的多架构协同学习模型：1）使用CNN自适应识别高不确定性区域；2）在高不确定性区域同时使用Vision Mamba和Deformable CNN提升分割精度；3）提出异构特征蒸馏损失促进两个架构在不确定性区域的协同学习
...
### [18] [HyperAlign: Hyperbolic Entailment Cones for Adaptive Text-to-Image Alignment Assessment](https://arxiv.org/abs/2601.04614)
*Wenzhi Chen,Bo Hu,Leida Li,Lihuo He,Wen Lu,Xinbo Gao*

Main category: cs.CV

TL;DR: 提出HyperAlign框架，使用双曲几何进行文本-图像对齐评估，通过双曲蕴含建模和自适应调制回归器，在单数据库评估和跨数据库泛化任务中取得竞争性表现。
...
Result: HyperAlign在单数据库评估和跨数据库泛化任务中都取得了高度竞争性的性能，充分验证了双曲几何建模在图像-文本对齐评估中的有效性。

Conclusion: 基于双曲蕴含几何的自适应文本-图像对齐评估框架HyperAlign，通过双曲空间建模和自适应调制机制，有效解决了现有方法在结构化语义对齐和样本自适应方面的局限性。
...
### [19] [Agri-R1: Empowering Generalizable Agricultural Reasoning in Vision-Language Models with Reinforcement Learning](https://arxiv.org/abs/2601.04672)
*Wentao Zhang,Lifei Wang,Lina Lu,MingKun Xu,Shangyang Li,Yanchao Yang,Tao Fang*

Main category: cs.CV

TL;DR: Agri-R1：通过视觉语言合成和LLM过滤自动生成高质量推理数据，仅用19%样本，结合GRPO训练和领域特定奖励函数，在农业病害诊断中实现显著性能提升


<details>
  <summary>Details</summary>
Motivation: 农业病害诊断对视觉语言模型提出挑战：传统微调需要大量标注、缺乏可解释性、泛化能力差。现有推理方法依赖昂贵的专家标注，且难以处理农业领域开放多样的查询

Method: 提出Agri-R1框架：1) 通过视觉语言合成和LLM过滤自动生成高质量推理数据（仅用19%可用样本）；2) 使用Group Relative Policy Optimization (GRPO)训练；3) 提出新颖奖励函数，整合领域特定词典和模糊匹配，评估开放回答的正确性和语言灵活性

Result: 在CDDMBench评估中，3B参数模型性能与7B-13B参数基线相当：病害识别准确率相对提升23.2%，农业知识问答提升33.3%，跨域泛化比标准微调提高26.10分。消融研究证实结构化推理数据与GRPO驱动的探索协同作用，且问题复杂度越高收益越大

Conclusion: Agri-R1通过自动化推理数据生成和GRPO优化，有效解决了农业病害诊断中的标注成本、可解释性和泛化问题，为农业领域视觉语言模型提供了高效解决方案
...
### [20] [HATIR: Heat-Aware Diffusion for Turbulent Infrared Video Super-Resolution](https://arxiv.org/abs/2601.04682)
*Yang Zou,Xingyue Zhu,Kaiqi Han,Jun Ma,Xingyuan Li,Zhiying Jiang,Jinyuan Liu*

Main category: cs.CV

TL;DR: HATIR提出了一种基于热感知扩散的红外视频超分辨率方法，通过注入热感知形变先验来联合建模湍流退化和结构细节损失的反向过程，解决了现有方法在处理湍流红外视频时的局限性。


<details>
  <summary>Details</summary>
Motivation: 红外视频在恶劣环境下具有重要应用价值，但常受严重大气湍流和压缩退化影响。现有视频超分辨率方法要么忽略了红外与可见光图像之间的模态差异，要么无法恢复湍流引起的失真。直接将湍流缓解算法与VSR方法级联会导致误差传播和累积，因为湍流和分辨率退化是解耦建模的。

Method: HATIR采用热感知扩散方法，将热感知形变先验注入扩散采样路径，联合建模湍流退化和结构细节损失的反向过程。具体包括：1) 基于热活跃区域随时间呈现一致相位响应的物理原理，构建相位引导流估计器，生成可靠的湍流感知流来指导反向扩散过程；2) 提出湍流感知解码器，通过湍流门控和结构感知注意力，选择性地抑制不稳定时间线索并增强边缘感知特征聚合。

Result: 构建了首个湍流红外视频超分辨率数据集FLIR-IVSR，包含来自FLIR T1050sc相机(1024×768)的配对LR-HR序列，涵盖640个不同场景，具有变化的相机和物体运动条件。该方法在湍流红外视频超分辨率任务上表现出色。

Conclusion: HATIR通过热感知扩散框架有效解决了湍流红外视频的超分辨率问题，联合建模了湍流退化和分辨率损失，为红外视频超分辨率研究提供了新思路和基准数据集。
...
### [21] [On the Holistic Approach for Detecting Human Image Forgery](https://arxiv.org/abs/2601.04715)
...
<details>
  <summary>Details</summary>
Motivation: AI生成内容（AIGC）的快速发展加剧了深度伪造的威胁，从面部操纵到全身逼真人体合成。现有检测方法存在碎片化问题，要么专门针对面部区域伪造，要么专门针对全身合成图像，无法泛化到各种人类图像操纵场景。

Method: 提出HuForDet整体框架，采用双分支架构：1）面部伪造检测分支：使用在RGB和频域操作的异构专家，包括自适应拉普拉斯高斯模块，用于捕捉从细粒度混合边界到粗尺度纹理异常的伪影；2）上下文伪造检测分支：利用多模态大语言模型分析全身语义一致性，配备置信度估计机制，在特征融合时动态加权其贡献。还构建了统一的人类图像伪造数据集。

Result: 广泛的实验表明，HuForDet在各种人类图像伪造检测中实现了最先进的检测性能和卓越的鲁棒性。

Conclusion: HuForDet通过整合面部和全身检测能力，提供了一个全面的人类图像伪造检测解决方案，有效解决了现有方法的碎片化问题，在多样化的伪造类型上表现出色。
...
### [22] [Training a Custom CNN on Five Heterogeneous Image Datasets](https://arxiv.org/abs/2601.04727)
*Anika Tabassum,Tasnuva Mahazabin Tuba,Nafisa Naznin*

Main category: cs.CV

TL;DR: 本研究评估了CNN架构在五个异质数据集上的表现，包括自定义轻量CNN与ResNet-18、VGG-16等经典架构的对比，分析了架构复杂度、模型深度和迁移学习在不同数据条件下的影响。


<details>
  <summary>Details</summary>
Motivation: 深度学习特别是CNN已在视觉数据分析中取得显著成效，但不同应用场景（农业和城市领域）存在光照、分辨率、环境复杂性和类别不平衡等挑战，需要评估CNN架构在异质数据集上的适应性和鲁棒性。

Method: 使用五个异质数据集（芒果品种分类、水稻品种识别、路面状况评估、三轮车检测、人行道侵占监测），评估自定义轻量CNN与ResNet-18、VGG-16等经典架构，采用从头训练和迁移学习两种方式，通过系统预处理、数据增强和受控实验进行分析。

Result: 开发的高效自定义CNN在多个应用领域达到竞争性性能；全面比较分析揭示了在数据受限环境下，迁移学习和深度架构何时能提供显著优势；为资源有限但高影响力的实际视觉分类任务提供了实用见解。

Conclusion: CNN架构在不同应用领域具有良好适应性，自定义轻量CNN在多个任务中表现优异，迁移学习和深度架构在数据受限环境中特别有价值，为实际部署提供了指导。
...
### [23] [AIVD: Adaptive Edge-Cloud Collaboration for Accurate and Efficient Industrial Visual Detection](https://arxiv.org/abs/2601.04734)
*Yunqing Hu,Zheming Yang,Chang Zhao,Qi Guo,Meng Gao,Pengcheng Li,Wen Ji*

Main category: cs.CV

TL;DR: AIVD框架通过边缘检测器与云端MLLM协作，实现精确目标定位和高质量语义生成，同时通过异构资源感知动态调度算法优化边缘部署性能。
...
Method: 提出AIVD框架，通过轻量级边缘检测器与云端MLLM协作；设计视觉-语义协同增强的高效微调策略；提出异构资源感知动态调度算法。

Result: AIVD显著降低资源消耗，同时提高MLLM分类性能和语义生成质量；调度策略在不同场景下实现更高吞吐量和更低延迟。

Conclusion: AIVD框架有效解决了MLLM在精确定位和边缘部署方面的挑战，通过协同边缘检测与云端推理，实现了高效、鲁棒的视觉语义理解系统。
...
### [24] [Skeletonization-Based Adversarial Perturbations on Large Vision Language Model's Mathematical Text Recognition](https://arxiv.org/abs/2601.04752)
...
Result: 方法有效减少了对抗攻击的搜索空间；在ChatGPT上成功应用，展示了实际应用价值；通过评估原始和对抗扰动输出的字符和语义变化，揭示了模型的视觉解释和推理能力

Conclusion: 提出的骨架化对抗攻击方法有效探索了基础模型的视觉能力局限性，特别在文本和数学公式识别方面，为理解模型视觉解释和推理能力提供了新视角
...
### [25] [ProFuse: Efficient Cross-View Context Fusion for Open-Vocabulary 3D Gaussian Splatting](https://arxiv.org/abs/2601.04754)
*Yen-Jen Chiou,Wei-Tse Cheng,Yuan-Fu Yang*

Main category: cs.CV

TL;DR: ProFuse是一个用于开放词汇3D场景理解的高效上下文感知框架，基于3D高斯泼溅技术，通过直接注册实现跨视图一致性和掩码内聚性，无需渲染监督微调，比现有技术快两倍。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D场景理解方法通常依赖预训练的3DGS场景，存在跨视图一致性差、掩码内聚性不足的问题，且需要大量计算资源和时间进行微调。需要一种高效、无需额外优化的方法来实现准确的3D语义理解。

Method: 1. 引入密集对应引导的预注册阶段，初始化具有准确几何的高斯分布；2. 通过跨视图聚类联合构建3D上下文提案；3. 每个提案通过加权聚合成员嵌入获得全局特征；4. 在直接注册过程中将特征融合到高斯分布上，保持跨视图的每个基元语言一致性；5. 无需额外优化，仅需标准重建。

Result: ProFuse实现了强大的开放词汇3DGS理解能力，每个场景的语义附加完成时间约为5分钟，比现有最先进技术快两倍。模型在保持几何细化的同时无需密度化，且语义融合无需超出标准重建的额外优化。

Conclusion: ProFuse通过上下文感知框架和直接注册方法，有效解决了开放词汇3D场景理解中的跨视图一致性和掩码内聚性问题，实现了高效、准确的语义理解，为3D场景分析提供了新的解决方案。
...
### [26] [GeM-VG: Towards Generalized Multi-image Visual Grounding with Multimodal Large Language Models](https://arxiv.org/abs/2601.04777)
*Shurong Zheng,Yousong Zhu,Hongyin Zhao,Fan Yang,Yufei Zhan,Ming Tang,Jinqiao Wang*

Main category: cs.CV

TL;DR: GeM-VG是一个能够处理广义多图像视觉定位的MLLM，通过系统分类多图像定位任务、构建MG-Data-240K数据集，并采用混合强化微调策略，在多项基准测试中显著优于现有方法。
...
Conclusion: GeM-VG通过系统化方法解决了多图像视觉定位的局限性，展示了卓越的广义定位能力，为多模态大语言模型在多图像任务中的发展提供了新方向。
...
### [27] [CounterVid: Counterfactual Video Generation for Mitigating Action and Temporal Hallucinations in Video-Language Models](https://arxiv.org/abs/2601.04778)
...
TL;DR: 提出CounterVid框架，通过合成反事实视频生成语义硬负样本，构建包含约26k偏好对的数据集，并引入MixDPO方法联合利用文本和视觉偏好来减少视频语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型在理解动作和时序关系时容易产生幻觉，现有缓解策略（如文本过滤或随机视频扰动）未能解决根本原因：过度依赖语言先验而非细粒度视觉动态。

Method: 提出可扩展的反事实视频生成框架，结合多模态LLM进行动作提案和编辑指导，使用基于扩散的图像和视频模型生成大规模语义硬负样本。构建CounterVid数据集，并引入MixDPO统一直接偏好优化方法。

Result: 使用MixDPO微调Qwen2.5-VL模型在时序排序等方面取得一致改进，并能有效迁移到标准视频幻觉基准测试中。

Conclusion: 通过反事实视频生成和混合偏好优化，能够有效减少视频语言模型的幻觉问题，特别是在动作识别和时序推理方面。
...
### [28] [Defocus Aberration Theory Confirms Gaussian Model in Most Imaging Devices](https://arxiv.org/abs/2601.04779)
*Akbar Saadat*
...
<details>
  <summary>Details</summary>
Motivation: 从2D图像准确估计深度是3D恢复领域的基本挑战。虽然散焦提供了重要的深度信息，但传统启发式方法面临病态问题，难以区分期望的散焦模糊与固有模糊。需要建立理论框架确保散焦算子符合特定模型，特别是高斯模型，因其数学简单、计算高效，适合实时应用。

Method: 论文在几何光学框架下进行散焦分析，并采用衍射极限光学中的散焦像差理论。研究了常规成像设备的设置，确保散焦算子符合高斯模型。通过将实际模型拟合到其高斯近似来评估准确性，考虑了典型聚焦深度范围（1-100米）和最大深度变化（聚焦深度的10%）。

Result: 研究结果表明，对于大多数成像设备，高斯模型适用于散焦算子。在典型深度范围内，最大平均绝对误差（MAE）小于1%，证明了高斯模型在散焦分析中的准确性和可靠性。

Conclusion: 高斯模型是散焦算子的有效近似，适用于从散焦中估计深度。该模型不仅数学简单、计算高效，适合实时应用，而且能同时应用于单幅图像中的绝对模糊和两幅图像间的相对模糊。研究结果为基于散焦的深度估计提供了坚实的理论基础。
...
### [29] [SRU-Pix2Pix: A Fusion-Driven Generator Network for Medical Image Translation with Few-Shot Learning](https://arxiv.org/abs/2601.04785)
*Xihe Qiu,Yang Dai,Xiaoyu Tan,Sijia Li,Fenghao Sun,Lu Gan,Liang Liu*

Main category: cs.CV

TL;DR: 增强版Pix2Pix框架，结合SEResNet和U-Net++，在少样本条件下提升MRI图像翻译质量
...
Method: 提出增强版Pix2Pix框架，集成Squeeze-and-Excitation Residual Networks (SEResNet) 和 U-Net++。SEResNet通过通道注意力增强关键特征表示，U-Net++增强多尺度特征融合。使用简化的PatchGAN判别器稳定训练并细化局部解剖真实性。
...
### [30] [Measurement-Consistent Langevin Corrector: A Remedy for Latent Diffusion Inverse Solvers](https://arxiv.org/abs/2601.04791)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于潜在扩散模型的零样本逆问题求解器存在不稳定性问题，会产生不良伪影和质量下降，需要更稳定可靠的解决方案

Method: 提出测量一致的Langevin校正器（MCLC），这是一个理论基础的即插即用校正模块，通过测量一致的Langevin更新来修正基于LDM的逆问题求解器，不依赖线性流形假设

Result: MCLC在多种图像恢复任务中表现出有效性，并与现有求解器兼容，能减少blob伪影，提供更稳定可靠的求解行为
...
### [31] [PyramidalWan: On Making Pretrained Video Model Pyramidal for Efficient Inference](https://arxiv.org/abs/2601.04792)
*Denis Korzhenkov,Adil Karjauv,Animesh Karnewar,Mohsen Ghafoorian,Amirhossein Habibian*

Main category: cs.CV

TL;DR: 提出了一种将预训练扩散模型转换为金字塔模型的方法，通过低成本微调实现高效推理，同时保持视频质量不下降


<details>
  <summary>Details</summary>
Motivation: 现有开源金字塔视频模型从头训练，在视觉合理性上不如最先进系统，且计算成本高，需要更高效的推理方法

Method: 开发了一个将预训练扩散模型转换为金字塔模型的流程，通过低成本微调实现，同时研究比较了金字塔模型中的步数蒸馏策略

Result: 成功将预训练模型转换为金字塔模型，输出视频质量没有下降，同时进一步提升了推理效率
...
### [32] [Detector-Augmented SAMURAI for Long-Duration Drone Tracking](https://arxiv.org/abs/2601.04798)
*Tamara R. Lenhard,Andreas Weinmann,Hichem Snoussi,Tobias Koch*

Main category: cs.CV

TL;DR: SAMURAI基础模型首次系统评估用于无人机跟踪，提出检测器增强扩展以提升城市监控场景中的鲁棒性，尤其在长时序列和无人机进出事件中表现显著改善。


<details>
  <summary>Details</summary>
Motivation: 无人机跟踪对现代监控系统至关重要，但现有RGB跟踪方法有限且依赖传统运动模型。SAMURAI等基础模型在其他领域表现出强大的类别无关跟踪能力，但其在无人机特定场景的应用尚未研究。

Method: 提出检测器增强的SAMURAI扩展，通过结合检测器线索来减轻对边界框初始化和序列长度的敏感性，提升复杂城市环境中的跟踪鲁棒性。

Result: 检测器增强扩展显著改善了长时序列跟踪性能，特别是在无人机退出-重新进入事件中。相比SAMURAI的零样本性能，在多个数据集和指标上获得一致提升，成功率最高提升+0.393，误报率最多降低-0.475。

Conclusion: 这是首次系统评估SAMURAI在无人机跟踪中的应用，提出的检测器增强扩展有效提升了城市监控场景中的跟踪鲁棒性，为无人机跟踪提供了新的解决方案。
...
### [33] [Integrated Framework for Selecting and Enhancing Ancient Marathi Inscription Images from Stone, Metal Plate, and Paper Documents](https://arxiv.org/abs/2601.04800)
*Bapu D. Chendage,Rajivkumar S. Mente*

Main category: cs.CV

TL;DR: 提出了一种基于二值化和互补预处理技术的古代文字图像增强方法，用于去除污渍和增强模糊的古代文字，在不同类型的古代文字上进行了评估。


<details>
  <summary>Details</summary>
Motivation: 古代文字图像常因背景噪声严重、对比度低以及老化和环境影响而退化，前景文字和背景具有相似的视觉特征，导致铭文难以阅读。主要目标是提高这类退化古代图像的可读性。
...
Conclusion: 所提出的增强方法在提高古代马拉地语铭文图像的可读性方面表现出有效性。
...
### [34] [SOVABench: A Vehicle Surveillance Action Retrieval Benchmark for Multimodal Large Language Models](https://arxiv.org/abs/2601.04824)
*Oriol Rabasseda,Zenjie Li,Kamal Nasrollahi,Sergio Escalera*

Main category: cs.CV

TL;DR: SOVABench是一个基于监控视频的车辆行为检索基准，用于评估模型在动作区分和时间方向理解上的能力。作者提出了一个无需训练的框架，利用多模态大语言模型生成可解释的嵌入表示。


<details>
  <summary>Details</summary>
Motivation: 现有视频检索基准主要关注场景级相似性，缺乏对监控场景中动作区分能力的评估。需要专门的基准来评估模型在车辆相关动作识别和时序理解方面的表现。

Method: 1. 构建SOVABench基准，包含两种评估协议：跨动作区分（inter-pair）和时间方向理解（intra-pair）。2. 提出无需训练的框架，利用多模态大语言模型的视觉推理和指令跟随能力，为图像和视频生成可解释的描述性嵌入。

Result: 实验表明，现有最先进的视觉和多模态模型在动作区分任务上仍面临挑战。提出的框架在SOVABench以及多个空间和计数基准上表现出色，在这些任务上对比视觉语言模型常常失败。

Conclusion: SOVABench填补了监控视频动作检索评估的空白，提出的基于多模态大语言模型的框架能够有效生成可解释的嵌入表示，在动作区分和时空理解任务上表现良好。代码、标注和基准构建指南已公开。
...
### [35] [Character Detection using YOLO for Writer Identification in multiple Medieval books](https://arxiv.org/abs/2601.04834)
*Alessandra Scotto di Freca,Tiziana D Alessandro,Francesco Fontanella,Filippo Sarria,Claudio De Stefano*

Main category: cs.CV

TL;DR: 该研究使用YOLO目标检测模型替代传统的模板匹配和CNN方法，用于中世纪手稿的抄写员识别，通过检测字母"a"来提高识别准确率。


<details>
  <summary>Details</summary>
Motivation: 古文书学研究中，确定手稿的抄写员对年代测定和书写风格演变分析至关重要。传统数字方法存在局限性，特别是模板匹配技术需要合适的阈值才能工作，这限制了识别效果。

Method: 采用YOLOv5目标检测模型完全替代先前工作中的模板匹配和CNN方法，专注于检测字母"a"的实例，然后进行第二阶段的分类来识别抄写员。

Result: YOLO能够提取更多被考虑的字母实例，从而提高了第二阶段分类的准确性。YOLO的置信度分数为开发拒绝阈值系统提供了基础，使得即使在未见过的抄本中也能实现可靠的抄写员识别。

Conclusion: YOLO目标检测模型在中世纪手稿抄写员识别任务中表现出色，相比传统模板匹配方法具有明显优势，为古文书学研究提供了更有效的数字工具。
...
TL;DR: DivAS是一个无需优化的交互式NeRF分割框架，通过深度引导的2D SAM掩码聚合实现实时3D分割，比优化方法快2-2.5倍


<details>
  <summary>Details</summary>
Motivation: 现有NeRF分割方法多为基于优化的，需要缓慢的逐场景训练，牺牲了2D基础模型的零样本能力。需要一种既能保持2D模型优势又能实现快速3D分割的方法。

Method: 提出DivAS框架：1) 基于GUI工作流，用户点提示生成2D SAM掩码；2) 使用NeRF深度先验优化掩码几何精度和前景背景分离；3) 开发定制CUDA内核，在200ms内将多视角掩码聚合到统一的3D体素网格中
...
TL;DR: 本文提出一个工业级GenAI框架，用于处理大规模多模态医药内容，分析40多个VLMs在长视频理解中的性能瓶颈和实际部署限制。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型大多在短视频和无约束计算资源下评估，而工业场景（如医药内容理解）需要在严格GPU、延迟和成本约束下处理长视频，现有方法难以扩展。

Method: 提出工业级大规模多模态推理架构，处理超过20万PDF、2.5万视频（8种格式）和888个多语言音频文件；在Video-MME和MMBench两个基准及专有数据集上对40多个VLMs进行实证分析。

Result: 在商用GPU上使用SDPA注意力获得3-8倍效率提升；多模态在8/12任务领域（特别是长度依赖任务）中提升性能；发现时间对齐和关键帧检测是开源和闭源VLMs的共同瓶颈。

Conclusion: 本文不提出新模型，而是刻画当前VLMs在实际部署约束下的实用限制、权衡和失败模式，为研究者和从业者设计可扩展的工业级长视频理解系统提供可操作指导。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动评估指标在文本到图像模型评估中广泛使用，但不确定这些指标是否真正关注语义正确性，还是倾向于从有偏数据分布中学到的视觉和社会原型图像。研究者识别并研究了多模态评估中的原型性偏差这一系统性失败模式。

Method: 引入受控对比基准ProtoBias，涵盖动物、物体和人口统计图像，将语义正确但非原型的图像与语义错误但原型的对抗性对应图像配对。基于此提出ProtoScore，一个7B参数的鲁棒性评估指标。

Result: 广泛使用的指标（CLIPScore、PickScore、VQA-based scores）经常错误排名这些配对，LLM-as-Judge系统在社会基础案例中表现出不均匀的鲁棒性。人类评估始终更倾向于语义正确性。ProtoScore显著降低了失败率，抑制了错误排名，运行速度比GPT-5推理时间快几个数量级，接近更大闭源评估者的鲁棒性。

Conclusion: 多模态评估中存在显著的原型性偏差问题，需要更鲁棒的评估指标。ProtoScore在减少偏差方面表现出色，为文本到图像模型评估提供了更可靠的自动评估方法。
...
TL;DR: 提出TEA方法，一种时间自适应卫星图像时间序列语义分割方法，解决现有方法在不同时间序列长度场景下泛化能力不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有卫星图像时间序列分割方法在预定序列长度下表现良好，但忽视了模型在不同时间长度场景下的泛化能力，导致在变长序列情况下分割结果显著下降。

Method: 提出TEA方法，采用教师-学生模型框架：教师模型封装全局序列知识来指导具有自适应时间输入长度的学生模型。通过中间嵌入、原型和软标签视角进行知识迁移，动态聚合学生模型以减轻知识遗忘，并引入全序列重建作为辅助任务来增强不同时间长度输入的表示质量。

Result: 通过大量实验证明，该方法在常见基准测试中，对不同时间长度输入都带来了显著改进。

Conclusion: TEA方法有效增强了卫星图像时间序列分割模型在不同时间序列长度场景下的适应性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D车道检测方法存在三个主要问题：1）传统密集BEV方法因错误变换导致特征表示与真实3D路面不对齐；2）稀疏车道检测器完全忽略了有价值的车道先验知识；3）现有方法未能利用历史车道观测信息来解决能见度差情况下的模糊问题

Method: 提出SparseLaneSTP方法，包含：1）车道特定的时空注意力机制；2）为稀疏架构设计的连续车道表示；3）时序正则化。同时引入基于自动标注策略的新型精确3D车道数据集

Result: 在所有检测和误差指标上，在现有3D车道检测基准和新型数据集上都实现了最先进的性能
...
<details>
  <summary>Details</summary>
Motivation: 水下场景中的光学退化（如散射、吸收）导致多视图图像不一致，使得传统3D重建方法难以准确恢复场景几何结构，需要专门的方法来处理水下环境特有的挑战。

Method: 1. 三目视图一致性约束：通过渲染水平/垂直平移的相机视图，并通过逆变形对齐来强制几何一致性；2. 合成极线深度先验：通过三角测量生成自监督深度正则化器；3. 深度感知alpha调整：基于3D高斯的z分量和观察方向调整不透明度，防止形成介质诱导的基元。

Result: 在真实水下场景和模拟场景上的实验表明，OceanSplat在场景重建和散射介质恢复方面显著优于现有方法，能够将3D高斯从散射介质中解耦，准确表示物体几何结构，大幅减少漂浮伪影。

Conclusion: OceanSplat通过几何约束和深度感知调整，有效解决了水下环境中的3D重建挑战，为水下场景的准确几何表示提供了鲁棒的解决方案，在散射介质中实现了更好的场景重建和恢复效果。
...
TL;DR: 研究高阶对抗攻击对目标检测器的影响，通过交替训练攻击模式和使用对抗训练强化检测器，发现高阶对抗补丁具有更强的泛化能力，且仅靠对抗训练不足以有效防御此类攻击。


<details>
  <summary>Details</summary>
Motivation: 研究高阶对抗攻击（类似猫鼠游戏的持续对抗过程）对目标检测器的影响，探索攻击模式与防御训练之间的动态关系。
...
Conclusion: 高阶对抗攻击对目标检测器构成严重威胁，需要更有效的防御策略，仅靠对抗训练不足以保证系统安全。
...
TL;DR: PolyFit是一种基于补丁的表面表示方法，通过局部拟合jet函数实现，能够高效学习并泛化到多种表面类型，通过更新紧凑的jet系数而非逐顶点优化来高效变形表面。


<details>
  <summary>Details</summary>
Motivation: 传统表面变形方法需要优化每个顶点的自由度，计算成本高。需要一种更高效的表面表示方法，能够快速变形表面并适用于计算机视觉和图形学的下游任务。

Method: 提出PolyFit表示方法：1）在表面补丁上局部拟合jet函数；2）通过监督学习从解析函数和真实数据中高效学习；3）学习后可泛化到多种表面类型；4）通过更新紧凑的jet系数而非逐顶点优化来实现表面变形。

Result: 在两个应用中验证：1）形状模板（SfT）：测试时优化在保持竞争力的准确度同时显著快于离线物理求解器，在精度上优于最近的物理引导神经模拟器；2）服装悬挂：训练自监督、网格和服装无关的模型，泛化到不同分辨率和服装类型，推理速度比强基线快一个数量级。

Conclusion: PolyFit提供了一种高效、紧凑的表面表示方法，能够快速变形表面并适用于多种计算机视觉和图形学任务，在速度和精度上都表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 制药行业面临多模态内容处理的挑战，传统人工标注存在不一致性、质量下降和效率低下的问题，特别是长视频和音频数据（如临床试验访谈和教育研讨会）的处理尤为困难。

Method: 提出领域适应的视频到视频片段生成框架，整合ALM和VLM，包括：1）可复现的剪切合并算法，带淡入淡出和时间戳标准化；2）基于角色定义和提示注入的个性化机制；3）成本高效的端到端管道策略。

Result: 在Video MME基准测试和16,159个制药视频专有数据集上评估，实现了3-4倍加速、4倍成本降低，片段连贯性得分（0.348）和信息性得分（0.721）优于最先进的VLM基线。

Conclusion: 该方法为生命科学领域提供了透明、可定制提取且支持合规的视频摘要解决方案，展示了在制药行业数字化转型中的潜力。
...
TL;DR: DrivoR：基于预训练视觉Transformer的端到端自动驾驶架构，通过相机感知寄存器令牌压缩多摄像头特征，使用轻量级Transformer解码器生成和评分候选轨迹，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个简单高效的端到端自动驾驶架构，利用预训练视觉Transformer的优势，同时通过特征压缩减少计算开销，实现准确、高效且可适应的驾驶系统。

Method: 1. 基于预训练视觉Transformer构建；2. 引入相机感知寄存器令牌压缩多摄像头特征为紧凑场景表示；3. 使用两个轻量级Transformer解码器：一个生成候选轨迹，另一个评分轨迹；4. 评分解码器学习模仿oracle并预测可解释的子分数（安全性、舒适性、效率等）。

Result: 在NAVSIM-v1、NAVSIM-v2和逼真闭环HUGSIM基准测试中，DrivoR优于或匹配当代强基线。纯Transformer架构结合目标令牌压缩足以实现准确、高效和自适应的端到端驾驶。

Conclusion: DrivoR展示了纯Transformer架构结合针对性令牌压缩在端到端自动驾驶中的有效性，实现了准确、高效且行为可调节的驾驶性能，代码和检查点将公开。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶应用中未标注的激光雷达日志包含丰富的3D几何信息，但缺乏人工标注使其几乎无用，成为自动驾驶感知研究的主要成本障碍。需要一种无需人工输入的方法来利用这些数据。

Method: 基于时间累积激光雷达地图学习强几何先验，利用激光雷达扫描的时间几何一致性，将文本和2D视觉基础模型的线索提升并融合到3D空间。提出新颖的迭代更新规则，强制联合几何-语义一致性，并通过不一致性检测运动物体。

Result: 方法在三个数据集上表现出稳健的泛化能力，在语义分割和物体检测伪标签方面优于现有方法（通常需要额外人工监督）。即使使用少量几何一致的密集激光雷达数据，也能在80-150米和150-250米范围内分别将深度预测MAE提高51.5%和22.0%。

Conclusion: 该方法成功解决了自动驾驶感知研究中激光雷达数据标注的成本障碍，通过无监督多模态伪标签方法有效利用未标注激光雷达日志，为3D感知任务提供了高质量的训练数据。
...
TL;DR: 本文提出了一种基于投影条件化的前馈视图合成方法，用目标视图的投影线索替代原始相机参数，将任务从射线空间的几何回归问题转化为目标视图的图像到图像转换问题，提高了视图合成的鲁棒性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有前馈视图合成模型使用Plücker射线图编码相机参数，这种方法将预测结果与任意世界坐标系绑定，使其对小的相机变换敏感，从而破坏了几何一致性。本文旨在寻找能够使模型实现鲁棒和一致视图合成的最佳输入条件。

Method: 提出投影条件化方法，用目标视图的投影线索替代原始相机参数，提供稳定的2D输入。同时引入针对这种线索的掩码自编码预训练策略，能够利用大规模未校准数据进行预训练。

Result: 在视图一致性基准测试中，相比射线条件化的基线方法，本方法显示出更高的保真度和更强的跨视图一致性。在标准新视图合成基准测试中也达到了最先进的质量水平。

Conclusion: 通过将视图合成任务重新定义为目标视图的图像到图像转换问题，并使用投影条件化方法，能够显著提高模型的鲁棒性和几何一致性，同时利用大规模未校准数据进行预训练，实现了更好的视图合成效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在视觉丰富文档理解任务中存在性能瓶颈，需要一种系统方法来分析其视觉嵌入空间，识别模型在特定视觉特征上的弱点，并通过针对性训练提升性能。

Method: VERSE方法通过探索视觉语言模型的视觉嵌入空间，可视化潜在表示来评估模型可行性。它能识别问题区域并指导生成包含特定视觉特征的合成数据，用于针对性重新训练。

Result: 在MERIT数据集上的验证显示，VERSE能有效识别与错误相关的视觉特征簇，使用包含这些特征的合成样本重新训练能显著提升F1性能而不损害泛化能力。优化后的本地模型（如Donut和Idefics2）性能达到甚至超过SaaS解决方案（如GPT-4和Pixtral）。

Conclusion: VERSE提供了一种系统方法来分析和改进视觉语言模型在文档理解任务中的性能，通过可视化嵌入空间、识别问题区域和生成针对性合成数据，有效提升模型性能并使本地模型达到商业SaaS解决方案水平。
...
TL;DR: VerseCrafter是一个4D感知的视频世界模型，通过4D几何控制表示实现对相机和物体运动的统一精确控制，并利用自动数据引擎从野外视频中提取4D标注进行训练。
...
Method: 提出4D几何控制表示，通过静态背景点云和每个物体的3D高斯轨迹编码世界状态；将4D控制渲染为预训练视频扩散模型的调节信号；开发自动数据引擎从野外视频中提取4D控制标注。

Result: 能够生成高保真、视角一致的视频，精确遵循指定的动态；通过自动数据引擎解决了大规模4D标注数据稀缺的问题。

Conclusion: VerseCrafter通过统一的4D几何世界状态实现了对相机和物体动态的显式连贯控制，为视频世界建模提供了新的解决方案。
...
TL;DR: 本文提出了一种轻量级视觉语言框架，用于从叶片图像中识别作物和病害，结合Swin Transformer视觉编码器和序列到序列语言解码器，在保持高准确率的同时显著减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 作物病害视觉问答需要准确的视觉理解和可靠的语言生成，现有大规模视觉语言模型参数量过大，需要开发更轻量高效的专用框架。

Method: 采用Swin Transformer作为视觉编码器，结合序列到序列语言解码器，使用两阶段训练策略改进视觉表示学习和跨模态对齐。

Result: 在大规模作物病害数据集上评估，模型在作物和病害识别方面均取得高准确率，在BLEU、ROUGE和BERTScore等自然语言生成指标上表现优异，参数量显著少于基线模型。

Conclusion: 该轻量级框架在作物病害视觉问答任务中表现出色，证明了任务特定的视觉预训练的有效性，为农业领域的智能诊断提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的病理学基础模型在性能、鲁棒性和计算需求方面存在权衡，限制了其临床部署。需要开发能够克服这些缺点的模型。
...
TL;DR: MuLo-SD提出了一种多尺度局部推测解码框架，通过结合多分辨率草稿生成和空间感知验证来加速自回归图像生成，实现了1.7倍加速，同时保持语义对齐和感知质量。


<details>
  <summary>Details</summary>
Motivation: 自回归模型在图像合成中取得了显著成功，但其顺序性质带来了显著的延迟限制。现有的推测解码方法受限于令牌级模糊性和缺乏空间感知能力。

Method: 提出多尺度局部推测解码框架，使用低分辨率草稿器和学习的上采样器生成候选图像令牌，然后由高分辨率目标模型并行验证。关键创新是局部拒绝和重采样机制，专注于空间邻域而非光栅扫描重采样。

Result: 在MS-COCO 5k验证集上，MuLo-SD实现了高达1.7倍的加速，超越了EAGLE-2和LANTERN等强基线，同时在语义对齐和感知质量方面保持可比性。

Conclusion: 该方法为图像合成的推测解码设定了新的最先进水平，在效率和保真度之间架起了桥梁，通过多分辨率草稿和空间感知验证实现了显著加速。
...
TL;DR: VLI是一个无需训练的多模态大语言模型推理框架，通过元认知自校正过程解决物体幻觉问题，在MMHal-Bench上将物体幻觉率降低12.67%，在POPE上提升准确率5.8%


<details>
  <summary>Details</summary>
Motivation: 物体幻觉严重损害多模态大语言模型的可靠性，主要源于模型认知内省失败——盲目信任语言先验而非具体视觉证据。现有缓解方法有限：对比解码方法操作表面化，未能纠正内部语义错位；而当前潜在引导方法依赖静态向量，缺乏实例特异性精度。

Method: VLI框架包含两个核心组件：1) 归因内省：通过概率冲突检测诊断幻觉风险并定位因果视觉锚点；2) 可解释双因果引导：主动调节推理过程，动态隔离视觉证据与背景噪声，同时通过自适应校准消除盲目置信。

Result: VLI在先进模型上实现最先进性能：在MMHal-Bench上将物体幻觉率降低12.67%，在POPE上提升准确率5.8%。

Conclusion: VLI通过模拟元认知自校正过程，有效解决了多模态大语言模型的物体幻觉问题，提供了一种无需训练的高效推理框架。
...
TL;DR: VideoAuto-R1框架采用"必要时推理"策略，在视频理解任务中实现SOTA准确率，同时将平均响应长度减少约3.3倍，显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 研究发现对于RL训练的视频模型，直接回答通常匹配甚至优于链式思维推理性能，但链式思维需要更高计算成本。因此需要探索推理的必要性和更高效的视频理解框架。

Method: 提出VideoAuto-R1框架，采用"必要时推理"策略。训练时遵循"思考一次，回答两次"范式：模型首先生成初始答案，然后进行推理，最后输出审查后的答案，两个答案都通过可验证奖励进行监督。推理时使用初始答案的置信度来决定是否进行推理。

Result: 在视频QA和定位基准测试中，VideoAuto-R1达到最先进的准确率，同时显著提高效率，平均响应长度从149个标记减少到44个标记（约3.3倍）。在感知导向任务中思维模式激活率较低，但在推理密集型任务中激活率较高。

Conclusion: 显式的基于语言的推理通常有益但并非总是必要。VideoAuto-R1框架通过自适应推理策略，在保持准确性的同时大幅提升效率，为视频理解任务提供了更实用的解决方案。
...
TL;DR: 研究发现大型视觉语言模型存在提示诱导幻觉现象：当文本提示高估图像中物体数量时，模型倾向于服从文本提示而非视觉证据，且随着物体数量增加，这种倾向更明显。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然能力强，但经常出现幻觉现象，特别是倾向于服从文本提示而忽视视觉证据。研究者希望探究这种失败模式的内在机制，特别是在物体计数任务中模型如何响应提示与视觉证据之间的差异。

Method: 在受控的物体计数设置中研究提示诱导幻觉：让文本提示高估图像中的物体数量（如提示描述4朵睡莲而图像中只有3朵）。通过机制分析三个VLMs，识别出一小部分注意力头，其消融能显著减少幻觉。分析这些PIH头在不同模型中的具体作用机制。

Result: 1. 在低物体数量时，模型通常能纠正高估；但随着物体数量增加，模型越来越倾向于服从提示而忽视差异。2. 识别出少量注意力头，其消融能将提示诱导幻觉减少至少40%且无需额外训练。3. 不同模型中PIH头以模型特定的方式介导提示复制，消融这些头能增强对视觉证据的纠正。

Conclusion: 研究揭示了提示诱导幻觉的内部机制，展示了不同模型在实现这些行为时的特定差异。通过识别和消融特定的注意力头，可以在不重新训练的情况下显著减少幻觉，为理解VLMs的内部工作机制提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D MRI数据集存在人口统计学偏差，限制了脑年龄预测模型的公平性和泛化能力。获取新数据成本高且受伦理约束，而当前基于潜在扩散模型的生成方法推理速度慢、可能引入伪影、且很少考虑年龄条件化

Method: FlowLet框架：在可逆3D小波域中利用流匹配技术合成年龄条件化的3D MRI，避免重建伪影并降低计算需求

Result: FlowLet能以较少采样步骤生成高保真度体积数据；使用FlowLet生成的数据训练脑年龄预测模型能改善对代表性不足年龄组的性能；区域分析确认了解剖结构的保留

Conclusion: FlowLet是一种有效的条件生成框架，能够合成高质量的年龄条件化3D MRI，改善脑年龄预测模型的公平性和性能，同时避免了现有方法的局限性
...
TL;DR: ObjectForesight：一个从自我中心视频预测物体未来6自由度位姿和轨迹的3D物体中心动力学模型，通过大规模伪真值3D轨迹数据集训练，在准确性、几何一致性和泛化性方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 人类能够轻松预测物体通过交互可能如何移动或变化，但现有计算系统缺乏从被动视觉观察直接预测物体未来运动的能力。作者希望赋予计算系统类似的能力，预测物体在现实世界中的合理未来运动。

Method: 提出ObjectForesight模型，这是一个3D物体中心动力学模型，从短自我中心视频序列预测刚性物体的未来6自由度位姿和轨迹。模型在3D物体级别显式表示世界，实现几何基础和时序一致的预测。利用分割、网格重建和3D位姿估计的最新进展，构建了包含200多万个短片段的大规模伪真值3D物体轨迹数据集进行训练。

Result: 通过大量实验表明，ObjectForesight在准确性、几何一致性和对未见物体和场景的泛化能力方面取得显著提升，为从观察中学习物理基础的物体中心动力学模型建立了可扩展框架。

Conclusion: ObjectForesight成功实现了从视觉观察直接预测物体未来运动的目标，通过3D物体中心表示和规模化训练，为学习物理基础的物体动力学模型提供了有效解决方案，在多个评估维度上优于传统方法。
...
TL;DR: 本文提出视觉身份提示方法，通过提供示例图像作为条件输入来指导扩散模型生成所需场景设置，从而增强机器人操作数据，提升下游策略模型的性能。


<details>
  <summary>Details</summary>
Motivation: 由于硬件和物理设置限制，收集大规模真实世界操作数据在不同环境中难以扩展。现有基于文本提示的图像扩散模型方法往往忽视多视角和时间一致性需求，且文本提示无法可靠指定场景设置。
...
TL;DR: 该论文提出了广义指代表达式分割(GRES)、理解(GREC)和生成(GREG)三个新基准任务，构建了首个大规模多目标/无目标/单目标表达式的数据集gRefCOCO，并提出ReLA方法在GRES/GREC任务上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有指代表达式任务(RES/REC/REG)只支持单目标表达式，即一个表达式对应一个对象，忽略了多目标和无目标表达式的现实应用需求，这严重限制了REx在实际应用中的能力。

Method: 1. 构建gRefCOCO数据集，包含多目标、无目标和单目标表达式及其对应的图像标注；2. 提出ReLA方法，自适应地将图像划分为包含子实例线索的区域，并显式建模区域-区域和区域-语言依赖关系。

Result: 1. 创建了首个大规模GREx数据集gRefCOCO；2. 提出的ReLA方法在GRES和GREC任务上均达到了最先进的性能；3. GREx基准和数据集向后兼容传统REx，便于研究现有方法在广义任务上的性能差距。

Conclusion: 该工作将传统的指代表达式任务扩展到支持任意数量对象的广义任务，提出了GREx基准和gRefCOCO数据集，为多目标和无目标表达式的理解与生成提供了新的研究平台，并通过ReLA方法展示了解决复杂关系建模的有效性。
...
TL;DR: 提出像素级完美视觉几何模型，通过像素空间生成建模预测高质量、无飞点的点云，包括单目深度模型PPD和视频深度模型PPVD


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型存在严重的飞点问题和细节丢失，需要恢复干净准确的几何信息用于机器人和增强现实应用

Method: 1) PPD：基于像素空间扩散变换器，引入语义提示DiT和级联DiT架构；2) PPVD：引入语义一致DiT，通过多视图几何基础模型提取时间一致语义，进行参考引导的令牌传播

Result: 在所有生成式单目和视频深度估计模型中取得最佳性能，产生的点云比其他模型显著更干净

Conclusion: 提出的像素完美视觉几何模型能够预测高质量、无飞点的点云，解决了现有几何基础模型的关键问题
...
TL;DR: 提出RL-AWB框架，结合统计方法与深度强化学习解决夜间白平衡问题，引入多传感器夜间数据集，在低光和正常光照图像上展现优越泛化能力


<details>
  <summary>Details</summary>
Motivation: 夜间色彩恒常性在计算摄影中仍是挑战，主要由于低光噪声和复杂光照条件。现有方法难以有效处理夜间场景的白平衡问题

Method: 1. 开发针对夜间场景的统计算法，结合显著灰度像素检测和新颖的光照估计方法；2. 基于此统计算法构建首个深度强化学习色彩恒常性方法，模拟专业AWB调优专家动态优化每张图像参数；3. 引入首个多传感器夜间数据集用于跨传感器评估

Result: 实验结果表明，该方法在低光和正常光照图像上均展现出优越的泛化能力

Conclusion: RL-AWB框架成功结合统计方法与深度强化学习，有效解决夜间白平衡问题，并通过多传感器数据集验证了其跨传感器性能
...
TL;DR: QNeRF是首个用于2D图像新视角合成的混合量子-经典模型，通过参数化量子电路编码空间和视角信息，相比经典NeRF使用更少参数但性能相当或更优。


<details>
  <summary>Details</summary>
Motivation: 量子视觉场在模型紧凑性和收敛速度方面显示出潜力，而NeRF在新视角合成方面取得重大进展但存在模型大、训练密集的问题。本研究旨在结合两者优势，探索量子机器学习在计算机视觉中连续信号表示的应用潜力。

Method: 提出QNeRF混合量子-经典模型，利用参数化量子电路通过量子叠加和纠缠编码空间和视角信息。设计了两种架构变体：Full QNeRF（最大化利用所有量子振幅）和Dual-Branch QNeRF（通过分支空间和视角相关的量子态准备引入任务感知归纳偏置）。

Result: 在中等分辨率图像训练时，QNeRF匹配或优于经典NeRF基线，同时使用不到一半的参数数量。Dual-Branch QNeRF显著降低了量子态准备操作的复杂度，确保了可扩展性和潜在的硬件兼容性。

Conclusion: 量子机器学习可以作为计算机视觉中连续信号表示的竞争性替代方案，特别是在3D表示学习等中级任务中，展示了量子计算在视觉任务中的实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 从单目视频中重建动态物体的完整3D形状和运动是一个具有挑战性的任务，现有方法通常需要复杂的优化过程或依赖骨骼信息。本文旨在开发一个前馈模型，能够一次性重建整个4D序列，同时保持高质量的重建结果。

Method: 1. 提出紧凑的潜在空间编码整个动画序列；2. 使用自动编码器学习潜在空间，训练时通过骨骼结构引导提供变形先验；3. 编码器采用时空注意力机制获得更稳定的变形表示；4. 基于该表示训练潜在扩散模型，以输入视频和第一帧重建的网格为条件，一次性预测完整动画。

Result: 在重建和新视角合成基准测试中，Mesh4D在恢复准确的3D形状和变形方面优于先前方法，证明了其有效性。

Conclusion: Mesh4D通过紧凑的潜在空间表示和骨骼引导的自动编码器，实现了高质量的单目4D网格重建，无需推理时的骨骼信息，为动态物体重建提供了有效的解决方案。
...
### [64] [MedPI: Evaluating AI Systems in Medical Patient-facing Interactions](https://arxiv.org/abs/2601.04195)
*Diego Fajardo V.,Oleksii Proniakin,Victoria-Elisabeth Gruber,Razvan Marinescu*

Main category: cs.CL

TL;DR: MedPI是一个用于评估大语言模型在医患对话中表现的高维度基准测试，包含105个维度的评估框架，覆盖医疗流程、治疗安全、治疗效果和医患沟通等方面。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估基准主要是单轮问答形式，无法全面评估模型在复杂的医患对话场景中的表现。需要建立一个更全面、更细粒度的评估框架来指导大语言模型在医疗诊断和治疗建议方面的应用。

Method: MedPI包含五个层次：1）患者包（类似电子健康记录的真实数据）；2）具有记忆和情感能力的AI患者；3）任务矩阵（涵盖就诊原因×就诊目标）；4）基于ACGME能力的105维度评估框架；5）经过校准的委员会式AI评委。评估了9个主流模型在366个AI患者和7,097次对话中的表现。

Result: 所有测试的大语言模型在多个维度上都表现不佳，特别是在鉴别诊断方面表现较差。这表明当前大语言模型在复杂的医患对话场景中仍有很大改进空间。

Conclusion: MedPI基准测试能够帮助指导未来大语言模型在诊断和治疗建议方面的应用，为模型改进提供了详细的评估维度。当前模型在医疗对话中的表现仍需大幅提升。
...
### [65] [RAGVUE: A Diagnostic View for Explainable and Automated Evaluation of Retrieval-Augmented Generation](https://arxiv.org/abs/2601.04196)
*Keerthana Murugaraj,Salima Lamsiyah,Martin Theobald*

Main category: cs.CL

TL;DR: RAGVUE是一个诊断性和可解释的框架，用于自动化、无参考的RAG系统评估，通过分解检索质量、答案相关性、完整性和忠实度等维度提供细粒度分析。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统评估指标存在局限性：通常将异构行为压缩为单一分数，无法区分错误来源（检索、推理还是基础），缺乏诊断性和解释性。

Method: RAGVUE将RAG行为分解为四个核心维度：检索质量、答案相关性和完整性、严格的声明级忠实度、评估者校准。每个指标都包含结构化解释，支持手动指标选择和完全自动化的智能体评估，提供Python API、CLI和本地Streamlit界面。

Result: 在比较实验中，RAGVUE能够发现现有工具（如RAGAS）经常忽略的细粒度失败案例。框架展示了完整的工作流程，并说明如何集成到研究管道和实际RAG开发中。

Conclusion: RAGVUE提供了一个透明、可解释的RAG评估框架，通过细粒度分解和结构化解释，帮助开发者和研究人员更好地诊断RAG系统的问题，源代码已在GitHub公开。
...
<details>
  <summary>Details</summary>
Motivation: 动机是构建中文动词构式数据库来补充大语言模型，为需要解释性和可解释性的应用场景提供明确的规则。大语言模型虽然强大，但在需要明确解释的场景中存在局限性。

Method: 方法包括：1）将动词构式形式化定义为投影的、有根的、有序的有向无环图；2）使用聚类算法从大规模语料库检索的句子列表中为给定动词生成构式；3）采用完全无监督的方法。

Result: 统计分析表明生成的构式具有功能独立性和分级典型性的设计特征。在动词语法错误纠正评估中，基于构式最大匹配的错误纠正算法比大语言模型表现更好。

Conclusion: 结论是提出的无监督方法能够有效构建中文动词构式数据库，生成的构式具有理论上的合理性，并且在语法错误纠正等实际应用中优于大语言模型，为需要解释性的应用场景提供了有价值的补充工具。
...
TL;DR: 本文提出了一种使用大语言模型生成合成电商产品数据的系统方法，通过三种策略（属性保留修改、受控负例生成、系统性属性移除）创建高质量训练数据，在MAVE数据集上达到与真实数据相当的性能。
...
Method: 使用大语言模型生成合成电商产品数据，提出受控修改框架，包含三种策略：1) 属性保留修改；2) 受控负例生成；3) 系统性属性移除。采用属性感知提示并强制执行店铺约束，同时保持产品连贯性。
...
TL;DR: 该研究提出了"集体叙事基础"协议，通过将社区故事转化为结构化叙事单元并整合到AI系统中，解决LLM在社区特定查询上的知识盲点问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在社区特定查询上经常失败，形成"知识盲点"，边缘化当地声音并加剧认知不公。需要开发能够整合社区本地知识的AI系统。

Method: 设计了参与式映射工作坊(N=24)，开发了激发方法和模式，将社区故事转化为结构化叙事单元，保留叙事丰富性的同时实现实体、时间、地点提取、验证和来源控制。

Result: 审计了14,782个本地信息QA对，发现76.7%的错误源于事实空白、文化误解、地理混淆和时间错位。在参与式QA集上，最先进的LLM在没有额外上下文的情况下正确率低于21%。收集的叙事中包含了缺失的事实。

Conclusion: 该研究为构建社区基础的AI提供了严谨基础，提出了关键设计张力（代表与权力、治理与控制、隐私与同意），并为检索优先、来源可见、本地治理的QA系统提供了具体需求。
...
TL;DR: TeleTables是一个评估大语言模型在电信标准（特别是3GPP规范）中表格知识掌握和解释能力的基准测试，揭示了模型在电信领域表格处理方面的不足。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在电信行业应用时，对3GPP标准等电信规范表现不佳。研究发现这些标准中密集包含表格呈现关键信息，但LLM对这些表格的知识掌握和解释能力尚未得到充分研究。

Method: 通过多阶段数据生成流程构建TeleTables基准：从3GPP标准中提取表格，使用多模态和推理导向的LLM生成和验证问题，最终创建包含500个人工验证的问答对数据集，每个问题都关联相应表格的多种格式。

Result: 评估显示：较小模型（参数少于100亿）在3GPP知识回忆和表格解释方面都表现不佳，表明其在预训练中对电信标准的接触有限，且缺乏处理复杂技术材料的归纳偏置。较大模型在表格解释方面表现出更强的推理能力。

Conclusion: TeleTables基准凸显了需要领域专业化微调来可靠地解释和推理电信标准，特别是针对表格密集的技术文档。
...
TL;DR: 提出了一种基于时空动态感知的自适应重掩码策略，显著提升扩散语言模型的生成效率
...
Method: 提出动态检测每个token的时间方差和空间偏差的方法，这些信号反映了token的收敛状态和token间相关性，基于这些信号为每个token在每个步骤自适应调整置信度阈值

Result: 在主流数据集上显著提升了扩散语言模型的运行效率，实现了高达8.9倍的加速，同时忠实保持了生成质量

Conclusion: 通过考虑token的时空动态特性进行自适应重掩码，能够有效提高扩散语言模型的效率和性能
...
TL;DR: 本文提出了一种结合微调语言模型与检索增强生成(RAG)的AI系统，用于优化大学招生办公室的咨询响应效率和质量，通过领域特定微调提升对复杂招生规则的理解和响应准确性。


<details>
  <summary>Details</summary>
Motivation: 大学招生办公室面临大量咨询请求，需要在保证响应质量的同时提高效率，这对潜在学生的感知有重要影响。传统RAG在招生等复杂狭窄领域可能表现有限，容易产生上下文不充分的回答。

Method: 提出混合方法：1) 使用RAG从大型数据集中检索相关信息；2) 在招生流程特定数据集上微调语言模型，增强其对RAG提供数据的准确理解和领域相关输出生成能力；3) 探索响应生成逻辑的优化策略，平衡响应质量和速度。

Result: 通过领域特定微调增强了模型对复杂招生规则的理解能力，结合RAG的实时信息访问能力，实现了更准确、上下文相关的响应生成。优化策略帮助在保持高质量输出的同时提高响应速度。

Conclusion: 该混合方法有效解决了大学招生咨询中的响应时间和信息准确性问题，通过结合RAG的检索能力和微调模型的领域理解能力，为招生办公室提供了高效、高质量的AI辅助解决方案。
...
TL;DR: 本文提出一种轻量级线性探针方法，用于量化和修正LLMs内部政治意识形态结构与人类意识形态空间之间的系统性错位，通过调整输出概率而非重新训练模型来实现低成本对齐。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型内部的政治意识形态组织与人类意识形态空间存在系统性错位，这种错位是模型特定的、可测量的，需要一种简单高效的方法来对齐模型与特定用户观点。

Method: 引入轻量级线性探针，从模型内部特征计算偏差分数，直接调整最终输出概率，无需重新训练模型，保持原始推理能力。

Result: 该方法能够量化模型与人类意识形态之间的错位程度，并通过最小化修正输出层来实现模型对齐，是一种实用且低成本的解决方案。

Conclusion: LLMs内部政治意识形态结构与人类意识形态空间存在系统性错位，通过线性探针方法可以量化和修正这种错位，实现模型与用户观点的低成本对齐，同时保持模型原有推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI模型在高风险消费决策中应用广泛，但现有可解释AI技术依赖事后数值特征归因，无法提供连贯的决策叙事。LLM虽能生成自然语言解释，但仍面临三大挑战：解释需要既正确又忠实于预测驱动因素；需要服务多受众而不改变决策规则；需要标签高效训练而不依赖大量人工标注解释。

Method: LEXMA采用强化学习微调框架，结合反思增强的监督微调和两阶段组相对策略优化(GRPO)。它微调两个独立的参数集：一个提升决策正确性，另一个满足不同受众的风格要求，使用不依赖人工标注解释的奖励信号。在抵押贷款审批决策场景中实例化该框架。

Result: LEXMA相比其他LLM基线在预测性能上取得显著提升。人工评估显示，该方法生成的面向专家的解释更关注风险，面向消费者的解释更清晰、更具可操作性且更礼貌。
...
TL;DR: Qwerty AI是一个端到端系统，用于根据俄罗斯联邦法律No. 436-FZ自动对俄语剧本进行年龄分级和内容安全评估，在Wink黑客松中开发，能在2分钟内处理长达700页的剧本，准确率达到80%。


<details>
  <summary>Details</summary>
Motivation: 解决俄罗斯媒体行业在剧本内容审查和年龄分级方面的实际编辑挑战，需要高效、自动化的系统来遵守联邦法律No. 436-FZ的要求。

Method: 使用经过微调的Phi-3-mini模型进行4位量化，将剧本分割为叙事单元，在五个内容类别（暴力、性内容、脏话、物质、恐怖元素）中检测违规内容，并分配年龄分级（0+, 6+, 12+, 16+, 18+），提供可解释的理由。

Result: 系统达到80%的分级准确率和80-95%的分割精度（取决于格式），在严格约束下（无外部API调用、80GB VRAM限制、平均剧本处理时间<5分钟）成功部署于Yandex Cloud并支持CUDA加速。

Conclusion: Qwerty AI展示了在生产工作流程中的实际应用性，在Wink黑客松（2025年11月）中解决了俄罗斯媒体行业的真实编辑挑战，证明了自动化内容安全评估系统的可行性。
...
Method: 开发TrueBrief框架，包含数据生成模块，通过可控幻觉注入生成合成偏好数据，采用偏好优化范式来增强小型语言模型在文本摘要任务中的忠实度。

Result: 研究提供了关于数据质量和模型大小对基于偏好的优化方法影响的见解，明确了这些方法最有效的条件。

Conclusion: TrueBrief框架能有效提升小型语言模型在文本摘要任务中的忠实度，为安全关键领域的语言模型部署提供了解决方案。
...
TL;DR: AnimatedLLM是一个交互式Web应用，通过浏览器提供Transformer语言模型的逐步可视化，用于NLP教学目的


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理教育中日益重要，但展示其内部工作机制的教学材料稀缺，需要直观的可视化工具来帮助学生理解

Method: 开发了一个完全在浏览器中运行的交互式Web应用，使用预计算的开源LLM在手动筛选输入上的运行轨迹，提供逐步的可视化展示
...
TL;DR: BiForget是一个自动合成高质量遗忘集的框架，通过种子引导和对抗性提示利用目标模型本身生成匹配其内部知识分布的数据，改进了机器遗忘评估的基准质量。


<details>
  <summary>Details</summary>
Motivation: 当前机器遗忘基准往往无法准确反映模型实际学习的"遗忘范围"，这影响了遗忘效果评估的准确性。需要更高质量的遗忘集来更严格地评估LLM遗忘。

Method: 提出BiForget框架，形式化了域级和实例级两种遗忘粒度。不同于依赖外部生成器的方法，BiForget利用目标模型本身，通过种子引导和对抗性提示来生成匹配模型内部知识分布的数据。

Result: 在多个基准测试中，BiForget在相关性、多样性和效率方面达到优越平衡。在哈利波特领域，相关性提高约20，多样性提高约0.05，同时总数据量比SOTA方法减少一半。

Conclusion: BiForget为评估LLM遗忘提供了更严格的基础，促进了更稳健的遗忘和更好的效用保持，解决了当前遗忘基准的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 科学严谨性往往被边缘化，作者倾向于做出超出结果支持的夸大陈述，需要工具来评估主张的夸大程度

Method: 构建包含10K+主张-证据对的数据集，使用8个LLM进行标注，通过同行评审评论校准夸大分数；采用微调的重新排序器进行证据检索，微调模型预测夸大分数并提供理由

Result: 相比强基线，RIGOURATE在证据检索和夸大检测方面表现更优；通过人类评估验证了有效性
...
TL;DR: 该研究对印度多种方言和语言变体的自发、嘈杂、代码混合语音进行跨语言迁移实证分析，发现ASR性能与语言间系统发育距离相关，但方言环境下还需考虑其他因素，少量方言数据微调可获得与大量相关高资源语言相当的ASR性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索方言和非标准化语音在自动语音识别系统中的表现，特别是跨语言迁移在方言环境下的有效性，以及系统发育距离对ASR性能的影响程度。

Method: 采用实证研究方法，在广泛的印度方言和语言变体上进行跨语言迁移实验，使用自发、嘈杂、代码混合的语音数据。包括对低资源Pahari语言变体Garhwali的案例研究，评估多种当代ASR模型，并分析转录错误以考察预训练语言的偏见。

Result: 结果显示：1) ASR性能通常随语言间系统发育距离减小而改善，但该因素不能完全解释方言环境下的性能表现；2) 少量方言数据微调可获得与大量系统发育相关高资源语言微调相当的ASR性能；3) 转录错误分析揭示了ASR系统对预训练语言的偏见。

Conclusion: 研究表明方言ASR性能不能仅用系统发育距离解释，少量方言数据微调是有效的策略，ASR系统存在对预训练语言的偏见，这对方言和非标准化语音识别系统开发具有重要启示。
...
TL;DR: Disco-RAG：一种基于语篇感知的检索增强生成框架，通过显式注入语篇信号来提升LLM在知识密集型任务中的表现，在问答和长文档摘要任务上取得SOTA结果。
...
Method: 提出Disco-RAG框架，构建语篇内树结构捕捉局部层次关系，建立语篇间修辞图建模跨段落连贯性，将这些结构整合到规划蓝图中指导生成过程。

Result: 在问答和长文档摘要基准测试中，Disco-RAG无需微调即达到最先进的结果，证明了语篇结构在推进RAG系统中的重要作用。

Conclusion: 语篇结构在提升RAG系统性能方面发挥关键作用，显式注入语篇信号能够有效增强LLM在知识密集型任务中的表现。
...
TL;DR: 论文揭示了当前大语言模型安全评估的局限性，指出安全对齐不是普遍能力而是人口统计学层次结构，模型防御率因目标群体不同而波动高达33%，且模型规模扩大会加剧这些差异。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的安全评估存在危险的普遍性错觉，将"身份仇恨"等指标聚合成标量分数，掩盖了针对特定群体的系统性漏洞。研究者希望暴露这种选择性安全现象。

Method: 引入MiJaBench双语对抗基准测试（英语和葡萄牙语），包含16个少数群体的44,000个提示。从12个最先进的大语言模型生成528,000个提示-响应对，并构建MiJaBench-Align数据集进行分析。

Result: 安全对齐不是广义的语义能力，而是人口统计学层次结构：同一模型内防御率仅基于目标群体不同而波动高达33%。模型规模扩大会加剧这些差异，表明当前对齐技术没有建立非歧视原则，而是仅强化了对特定群体的记忆拒绝边界。

Conclusion: 当前的安全对齐技术存在选择性保护问题，模型规模扩大反而会加剧针对不同群体的安全差异，挑战了当前的安全扩展定律。研究呼吁进行更细粒度的人口统计学对齐研究。
...
TL;DR: 研究发现大语言模型在挑战用户有害信念时经常失败，主要原因是模型默认迎合用户假设且缺乏认知警惕性。通过简单的语用干预（如添加"等一下"）可显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医疗建议和社会推理等领域经常无法挑战用户的有害信念，这种失败限制了模型的安全性。研究旨在理解并解决这一现象，将其归因于模型过度迎合用户假设和缺乏认知警惕性。

Method: 研究采用语用学视角，分析影响人类迎合行为的社交和语言因素（话题相关性、语言编码、来源可靠性）如何影响LLMs。在三个安全基准测试（Cancer-Myth、SAGE-Eval、ELEPHANT）上评估模型表现，并测试简单的语用干预措施（如添加"等一下"短语）的效果。

Result: 研究发现影响人类迎合行为的因素同样影响LLMs的迎合行为，解释了模型在不同安全基准测试中的性能差异。简单的语用干预能显著提升模型在挑战有害信念方面的表现，同时保持较低误报率。

Conclusion: LLMs在挑战有害信念时的失败可通过语用学视角理解，简单的语用干预能有效改善模型安全性。研究强调了考虑语用因素对于评估LLM行为和提升LLM安全性的重要性。
...
TL;DR: 研究比较了两种对话预测学习方法：基于LLM评判的奖励优化 vs 直接最大化人类对话概率，发现后者效果更好，尤其结合思维链作为隐变量时效果最佳。


<details>
  <summary>Details</summary>
Motivation: 通过预测下一轮对话来建模人类思维过程，研究不同学习方法在对话预测任务上的效果差异，特别是思维链和不同奖励机制的影响。

Method: 比较两种维度：(1)是否允许模型在回答前思考；(2)学习奖励机制：基于LLM评判的语义相似度和信息完整性评分 vs 直接最大化真实人类对话的对数概率。将思维链作为隐变量，推导对数概率的下界进行优化。

Result: 基于评判奖励的优化虽然提高了评判分数，但降低了真实人类响应的概率和人类评判中的胜率，特别是允许思考时这种失败更明显。直接最大化人类对话概率的方法在所有评估中都表现更好，结合思维链隐变量优化的方法效果最佳。

Conclusion: 思维链主要在基于真实人类对话的分布匹配目标下才有帮助，将这种方法扩展到更广泛的对话数据可能产生对人类行为有更细致理解的模型。
...
TL;DR: 研究发现用户对AI写作助手帮助偏好的自我报告与实际行为存在显著差距，紧迫性在自我报告中排名第一但在行为中预测力为零，导致基于用户自述偏好的系统设计效果不佳。


<details>
  <summary>Details</summary>
Motivation: 主动式AI写作助手需要预测用户何时需要写作帮助，但缺乏对用户偏好驱动因素的实证理解。当前依赖用户自我报告进行系统设计可能存在误导。

Method: 采用因子式情境研究，50名参与者进行750对成对比较，分析不同因素（如组合努力、紧迫性等）对用户偏好的影响，比较自我报告与实际行为数据。

Result: 组合努力是主要行为驱动因素（ρ=0.597），而紧迫性预测力几乎为零。用户存在明显的感知-行为差距：紧迫性在自我报告中排名第一但在行为中是最弱驱动因素。基于用户自述偏好的系统准确率仅57.7%，低于基于行为模式的61.3%（p<0.05）。
...
### [85] [Concept Tokens: Learning Behavioral Embeddings Through Concept Definitions](https://arxiv.org/abs/2601.04465)
*Ignacio Sastre,Aiala Rosá*

Main category: cs.CL

TL;DR: Concept Tokens是一种轻量级方法，通过在预训练LLM中添加特殊token并仅从其定义中学习嵌入，实现对冻结模型的行为控制
...
Method: 在预训练LLM中添加新的特殊token，用该token替换概念出现的位置，仅从多个自然语言定义中学习token嵌入，保持模型其他部分冻结，使用标准语言建模目标优化

Result: 1) HotpotQA闭卷问答中减少幻觉：否定幻觉token主要增加弃权，断言则增加幻觉降低精度；2) 诱导语言教学中的重述策略；3) 相比上下文提供完整定义，概念token能更好保持对其他指令的遵从；4) 定性研究显示嵌入能捕获概念信息但存在局限

Conclusion: Concept Tokens提供了一种从定义学习的紧凑控制信号，能够有效引导冻结LLM的行为，在减少幻觉和引导特定行为方面具有实用价值
...
### [86] [SampoNLP: A Self-Referential Toolkit for Morphological Analysis of Subword Tokenizers](https://arxiv.org/abs/2601.04469)
*Iaroslav Chelombitko,Ekaterina Chelombitko,Aleksey Komissarov*

Main category: cs.CL

TL;DR: SampoNLP工具包通过自参考原子性评分创建形态学词典，用于评估乌拉尔语系的BPE分词器，提出综合性能评分(IPS)指标确定最佳词汇量


<details>
  <summary>Details</summary>
Motivation: 乌拉尔语系等形态丰富语言缺乏干净的形态学词典，难以评估大语言模型的分词器质量

Method: 开发SampoNLP工具包，使用MDL启发的自参考原子性评分从内部结构线索过滤复合形式，生成高纯度形态学词典；提出综合性能评分(IPS)平衡语素覆盖和过度分割

Result: 为芬兰语、匈牙利语和爱沙尼亚语生成高质量形态学词典；通过IPS曲线分析找到收益递减的"拐点"，首次为这些语言提供基于实证的最佳词汇量建议(8k-256k范围)

Conclusion: SampoNLP为低资源语言提供实用的形态学词典创建工具，定量证明了标准BPE对高度黏着语言的局限性，为乌拉尔语系分词器优化提供实证指导
...
### [87] [WESR: Scaling and Evaluating Word-level Event-Speech Recognition](https://arxiv.org/abs/2601.04508)
...
TL;DR: 该论文提出了WESR-Bench，一个用于非语言声音事件检测和定位的基准测试集，包含21种声音事件的精细分类，以及专门构建的数据集和模型。
...
Method: 1. 开发了21种声音事件的精细分类法，分为离散（独立）和连续（与语音混合）两种类型；2. 引入WESR-Bench专家标注评估集（900+话语），采用新的位置感知协议，将ASR错误与事件检测分离；3. 构建了1,700+小时语料库，训练专门模型。

Result: 构建的专门模型在非语言事件检测和定位方面超越了开源音频语言模型和商业API，同时保持了ASR质量。

Conclusion: WESR将成为未来建模丰富真实世界听觉场景研究的基础资源，为非语言声音事件的精确定位提供了标准化评估框架和有效解决方案。
...
### [88] [LinguaGame: A Linguistically Grounded Game-Theoretic Paradigm for Multi-Agent Dialogue Generation](https://arxiv.org/abs/2601.04516)
...
Method: 提出LinguaGame范式，将对话建模为基于沟通意图和策略的信号博弈，采用无需训练的均衡近似算法进行推理时决策调整，基于语言学推理而非任务特定目标

Result: 在模拟法庭程序和辩论场景中评估，通过人类专家评估显示在沟通效率方面取得显著提升

Conclusion: 通过语言学基础的游戏论方法建模多智能体对话，能够有效提升沟通效率，为多智能体系统交互过程优化提供了新思路
...
### [89] [GRACE: Reinforcement Learning for Grounded Response and Abstention under Contextual Evidence](https://arxiv.org/abs/2601.04525)
*Yibo Zhao,Jiapeng Zhu,Zichen Ding,Xiang Li*

Main category: cs.CL

TL;DR: GRACE是一个基于强化学习的框架，通过证据充分性评估、关键证据提取和选择性弃权，统一解决RAG系统中无证据正确回答和证据不足时编造答案两个关键缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统存在两个关键缺陷：1）在没有显式证据支持的情况下提供正确答案；2）当检索上下文不足时产生编造的响应。虽然先前研究分别解决了这些问题，但缺乏一个集成证据基础化和可靠弃权的统一框架。

Method: GRACE采用强化学习框架，使用异构检索器生成多样化的训练样本而无需人工标注。通过多阶段门控奖励函数训练模型评估证据充分性、提取关键支持证据，并提供答案或明确弃权。

Result: 在两个基准测试中，GRACE实现了最先进的整体准确率，并在准确响应和拒绝之间取得了良好的平衡，同时仅需要先前方法10%的标注成本。

Conclusion: GRACE提供了一个统一的强化学习框架，有效解决了RAG系统中的两个关键缺陷，通过证据充分性评估和选择性弃权机制，在保持高准确率的同时显著降低了标注成本。
...
### [90] [BanglaLorica: Design and Evaluation of a Robust Watermarking Algorithm for Large Language Models in Bangla Text Generation](https://arxiv.org/abs/2601.04534)
*Amit Bin Tariqul,A N M Zahid Hossain Milkan,Sahab-Al-Chowdhury,Syed Rifat Raiyan,Hasan Mahmud,Md Kamrul Hasan*

Main category: cs.CL

TL;DR: 该论文系统评估了现有文本水印方法在孟加拉语LLM文本生成中的鲁棒性，发现跨语言往返翻译攻击会导致检测准确率大幅下降，并提出分层水印策略显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在文本生成中的广泛应用，水印技术对于作者归属、知识产权保护和滥用检测变得至关重要。然而，现有水印方法在高资源语言中表现良好，但在低资源语言中的鲁棒性尚未得到充分研究，特别是在孟加拉语等语言中。

Method: 1. 系统评估了三种最先进的文本水印方法（KGW、指数采样EXP和Waterfall）在孟加拉语LLM文本生成中的性能；2. 在跨语言往返翻译攻击下测试这些方法的鲁棒性；3. 提出分层水印策略，结合嵌入时水印和生成后水印；4. 实验量化了鲁棒性与质量之间的权衡。

Result: 在良性条件下，KGW和EXP达到高检测准确率（>88%），且困惑度和ROUGE分数下降可忽略。但往返翻译攻击导致检测准确率崩溃至9-13%，表明令牌级水印存在根本性缺陷。分层水印将攻击后检测准确率提升25-35%，达到40-50%准确率，相对单层方法有3-4倍改进，但带来可控的语义质量下降。

Conclusion: 分层水印是低资源语言（如孟加拉语）中实用且无需训练的水印解决方案，量化了多语言水印中鲁棒性与质量之间的权衡，为低资源语言的水印技术提供了重要改进方向。
...
### [91] [Identifying Good and Bad Neurons for Task-Level Controllable LLMs](https://arxiv.org/abs/2601.04548)
*Wenjie Li,Guansong Pang,Hezhe Qiao,Debin Gao,David Lo*

Main category: cs.CL

TL;DR: NeuronLLM：基于功能拮抗原理的LLM神经元识别框架，通过对比学习促进任务完成的"好神经元"和抑制任务的"坏神经元"，解决现有方法只关注支持性神经元、忽视抑制性角色以及LLM偶然行为导致的错误归因问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM神经元识别方法存在三个主要问题：1）针对特定能力的方法不适用于需要协调多种能力的任务场景；2）只关注与任务正相关的支持性神经元，忽视抑制性神经元；3）LLM的偶然行为（靠运气而非真正理解）导致神经元归因错误。

Method: 提出NeuronLLM框架，采用生物学功能拮抗原理，将任务性能建模为由"好神经元"（促进任务完成）和"坏神经元"（抑制任务完成）共同决定。通过对比学习同时建模这两种神经元，并利用增强的问题集来缓解LLM的偶然行为。

Result: 在不同规模和家族的LLM上进行的综合实验表明，NeuronLLM在四个NLP任务上优于现有方法，为理解LLM的功能组织提供了新的见解。
...
### [92] [FeedEval: Pedagogically Aligned Evaluation of LLM-Generated Essay Feedback](https://arxiv.org/abs/2601.04574)
...
<details>
  <summary>Details</summary>
Motivation: 当前自动作文评分研究强调生成高质量反馈，但通常依赖未经质量验证的LLM生成反馈，导致噪声传播到下游应用。需要一种系统方法来评估和筛选LLM生成的反馈质量。

Method: 提出FeedEval框架，使用基于三个教学维度（具体性、帮助性、有效性）训练的专门化LLM评估器，评估多个反馈候选并选择高质量反馈用于下游任务。构建了专门的数据集来训练这些评估器。

Result: 在ASAP++基准测试中，FeedEval与人类专家判断高度一致；使用FeedEval筛选的高质量反馈训练的作文评分模型获得更优的评分性能；小型LLM使用这些高质量反馈进行作文修订实验显示更有效的修订效果。

Conclusion: FeedEval能有效评估和筛选LLM生成的作文反馈质量，提升下游作文评分和修订任务的效果，解决了LLM生成反馈质量验证不足的问题，为自动化作文评估提供了更可靠的方法。
...
### [93] [Aligning Text, Code, and Vision: A Multi-Objective Reinforcement Learning Framework for Text-to-Visualization](https://arxiv.org/abs/2601.04582)
*Mizanur Rahman,Mohammed Saidul Islam,Md Tahmid Rahman Laskar,Shafiq Joty,Enamul Hoque*

Main category: cs.CL

TL;DR: RL-Text2Vis是一个基于强化学习的文本到可视化生成框架，使用多目标奖励函数优化文本准确性、代码有效性和可视化质量，显著提升了开源模型在图表生成任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有文本到可视化系统存在两个主要问题：闭源大语言模型生成的图表缺乏语义对齐和清晰度，而开源模型经常产生不可执行或视觉质量差的输出。传统的监督微调方法无法捕捉执行后反馈，无法提升整体可视化质量。

Method: 提出了RL-Text2Vis框架，基于Group Relative Policy Optimization（GRPO）强化学习方法，采用新颖的多目标奖励函数，联合优化文本准确性、代码有效性和可视化质量，利用执行后反馈进行训练。

Result: 在Text2Vis基准测试中，RL-Text2Vis相比GPT-4o实现了22%的相对图表质量提升，代码执行成功率从零样本基线的78%提升到97%。模型在VIS-Eval和NVBench等域外数据集上也表现出强大的泛化能力。

Conclusion: GRPO是可视化生成中结构化多模态推理的有效策略，RL-Text2Vis框架显著提升了开源模型在文本到可视化任务上的性能，为后续研究提供了有价值的基准。
...
### [94] [THaLLE-ThaiLLM: Domain-Specialized Small LLMs for Finance and Thai -- Technical Report](https://arxiv.org/abs/2601.04597)
...
TL;DR: 该研究探索模型合并作为资源高效的方法来创建多能力大语言模型，通过合并Qwen-8B与ThaiLLM-8B以及金融模型THaLLE-CFA-8B，提升了泰语通用能力和金融领域性能。


<details>
  <summary>Details</summary>
Motivation: 由于隐私、安全和监管考虑，组织通常偏好本地部署大语言模型。泰国LLM计划旨在增强开源LLM的泰语能力，但组织面临部署多个专业模型与训练单一多能力模型成本过高之间的权衡。需要寻找资源高效的方法来开发高性能多能力LLM。

Method: 采用模型合并作为资源高效替代方案。进行了两个关键实验：1) 合并Qwen-8B与ThaiLLM-8B；2) 合并Qwen-8B与ThaiLLM-8B以及THaLLE-CFA-8B。通过模型合并技术整合不同模型的专长。

Result: 实验结果显示：1) ThaiLLM-8B提升了泰语通用能力，在M3和M6 O-NET考试中表现优于通用指令跟随模型Qwen-8B；2) 三模型合并进一步提升了通用和金融领域的性能，在M3、M6 O-NET、Flare-CFA和Thai-IC基准测试中均有提升。

Conclusion: 模型合并是高效创建多能力大语言模型的可行方法，能够整合不同模型的专长，在资源有限的情况下实现多领域性能提升，为组织提供成本效益高的解决方案。
...
### [95] [On the Limitations of Rank-One Model Editing in Answering Multi-hop Questions](https://arxiv.org/abs/2601.04600)
...
<details>
  <summary>Details</summary>
Motivation: 虽然ROME等知识编辑方法在单跳事实更新上表现出色，但在需要知识链的多跳推理任务中面临显著挑战，需要解决这些失败模式
...
Conclusion: 冗余编辑是提升知识编辑方法在多跳推理任务中性能的有效策略，显著改善了ROME方法在多跳推理中的表现
...
### [96] [When More Words Say Less: Decoupling Length and Specificity in Image Description Evaluation](https://arxiv.org/abs/2601.04609)
*Rhea Kapur,Robert Hawkins,Elisa Kreiss*

Main category: cs.CL

TL;DR: 该研究指出当前视觉语言模型中描述特异性常与长度混淆，提出应区分这两个概念，并通过构建控制长度的数据集验证人们更偏好特异性描述而非冗长描述。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在生成图像描述时，往往将描述的特异性与长度混为一谈，但实际上描述可以是简洁但信息密集的，也可以是冗长但空洞的。需要将这两个概念解耦，以评估描述的真实信息价值。

Method: 1. 定义特异性相对于对比集的概念：描述的特异性取决于它能在多大程度上将目标图像与其他可能图像区分开来。2. 构建控制长度但信息内容不同的数据集。3. 通过人类偏好实验验证人们对特异性描述的偏好是否独立于长度。

Result: 1. 人类参与者可靠地偏好更具体的描述，无论描述长度如何。2. 仅控制长度不能解释特异性差异：长度预算的分配方式很重要。3. 支持直接优先考虑特异性而非冗长性的评估方法。

Conclusion: 描述的特异性不应与长度混淆，评估视觉语言模型时应直接关注描述的信息密度和区分能力，而非单纯追求描述的长度。这为改进VLM评估方法提供了理论基础。
...
TL;DR: 提出CuCu框架，利用国家社会研究课程自动生成文化特定的开放问答对，应用于韩国课程构建KCaQA数据集，包含34.1k个问答对，旨在解决LLMs在语言和文化上的不平衡表现问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在许多任务上表现优异，但在不同语言和文化间的进展不平衡，通常反映了以英语为中心的训练数据中隐含的价值观。为了解决实际的文化对齐问题，需要一种可扩展的方法来实现文化感知的监督。
...
Conclusion: 通过利用国家社会研究课程作为文化感知监督的基础，提出了一种可扩展的文化对齐方法。CuCu框架能够自动生成文化特定的训练数据，为改善LLMs在不同文化背景下的表现提供了实用解决方案。
...
### [98] [SpeechMedAssist: Efficiently and Effectively Adapting Speech Language Models for Medical Consultation](https://arxiv.org/abs/2601.04638)
...
<details>
  <summary>Details</summary>
Motivation: 医疗咨询本质上是语音中心的，但现有研究多基于长文本交互，这既繁琐又不友好。虽然语音语言模型（SpeechLMs）可实现更自然的语音交互，但医疗语音数据稀缺以及直接在语音数据上微调的低效性阻碍了SpeechLMs在医疗咨询中的应用。

Method: 提出SpeechMedAssist，通过利用SpeechLMs的架构特性，将传统的一阶段训练解耦为两阶段范式：1）通过文本进行知识与能力注入；2）使用有限语音数据进行模态重新对齐。这种方法将医疗语音数据需求减少到仅需1万个合成样本。

Result: 实验结果表明，该模型在大多数评估设置中，在有效性和鲁棒性方面均优于所有基线模型。为评估医疗咨询场景中的SpeechLMs，设计了包含单轮问答和多轮模拟交互的基准测试。

Conclusion: SpeechMedAssist通过创新的两阶段训练范式，成功解决了医疗语音数据稀缺问题，实现了基于语音语言模型的医疗咨询系统，为语音驱动的医疗交互提供了实用解决方案。
...
### [99] [CRANE: Causal Relevance Analysis of Language-Specific Neurons in Multilingual Large Language Models](https://arxiv.org/abs/2601.04664)
...
<details>
  <summary>Details</summary>
Motivation: 多语言大语言模型在不同语言上表现强劲，但神经元层面的语言能力组织方式仍不清楚。现有基于激活的启发式方法混淆了语言偏好与功能重要性。

Method: 提出CRANE框架，通过神经元级干预识别语言特异性神经元，基于神经元对语言条件预测的贡献而非激活幅度来表征神经元专门化。

Result: 神经元干预揭示了一致的不对称模式：屏蔽目标语言相关神经元会选择性降低该语言性能，同时很大程度上保留其他语言性能，表明神经元具有语言选择性但非排他性专门化。在英语、中文和越南语的多个基准测试中，CRANE比基于激活的方法更精确地分离语言特定组件。

Conclusion: CRANE框架通过功能必要性重新定义语言特异性，提供了比传统激活分析方法更精确的语言特异性神经元识别方法，有助于理解多语言LLM中语言能力的神经元级组织。
...
### [100] [ToolGate: Contract-Grounded and Verified Tool Execution for LLMs](https://arxiv.org/abs/2601.04688)
...
Method: 提出ToolGate框架，维护显式符号状态空间作为类型化键值映射，将每个工具形式化为包含前置条件和后置条件的Hoare式契约，前置条件控制工具调用，后置条件通过运行时验证决定结果是否提交更新状态。

Result: 实验验证表明ToolGate显著提高了工具增强LLM系统的可靠性和可验证性，同时在复杂多步推理任务上保持竞争力，确保符号状态仅通过已验证的工具执行演进。

Conclusion: ToolGate为构建更可信和可调试的AI系统奠定了基础，这些系统将语言模型与外部工具集成，提供了逻辑安全保证和可验证的状态演进机制。
...
### [101] [See, Explain, and Intervene: A Few-Shot Multimodal Agent Framework for Hateful Meme Moderation](https://arxiv.org/abs/2601.04692)
*Naquee Rizwan,Subhankar Swain,Paramananda Bhaskar,Gagan Aryan,Shehryaar Shah Khan,Animesh Mukherjee*

Main category: cs.CL

TL;DR: 该研究提出一个基于生成式AI的多模态框架，用于仇恨表情包的检测、解释和干预，特别针对有限数据条件下的实际应用场景。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将仇恨表情包的检测、解释和干预分开研究，这与实际应用场景不符。同时，标注大规模表情包数据集成本高昂，需要开发在有限数据条件下也能有效工作的通用化仇恨表情包审核方法。

Method: 提出一个新颖框架，利用任务特定的生成式多模态智能体和大型多模态模型的少样本适应能力，针对不同类型表情包进行检测、解释和干预。

Result: 这是首个专注于有限数据条件下通用化仇恨表情包审核的研究，具有在实际生产场景中部署的强潜力。

Conclusion: 该研究通过生成式AI模型提供了一个综合性的仇恨表情包处理框架，将检测、解释和干预三个环节整合在一起，解决了现有研究的局限性，为实际应用提供了可行的解决方案。
...
TL;DR: 提出了Thunder-KoNUBench基准测试，用于评估韩语否定理解能力，发现LLMs在否定处理上表现不佳，但通过该基准微调可提升韩语否定理解和上下文理解能力


<details>
  <summary>Details</summary>
Motivation: 虽然否定对大型语言模型构成挑战，但评估韩语否定理解的基准测试稀缺。现有研究显示LLMs在否定处理上表现不佳，需要专门的韩语否定理解评估工具

Method: 1) 进行基于语料库的韩语否定分析；2) 开发Thunder-KoNUBench句子级基准测试，反映韩语否定现象的经验分布；3) 评估47个LLMs，分析模型大小和指令微调的影响；4) 在Thunder-KoNUBench上进行微调实验

Result: 1) LLMs在否定处理上表现下降；2) 模型大小和指令微调对否定理解有影响；3) 在Thunder-KoNUBench上微调能显著提升韩语否定理解和更广泛的上下文理解能力

Conclusion: Thunder-KoNUBench是有效的韩语否定理解评估基准，通过在该基准上微调可以改善LLMs的韩语否定处理能力和整体语言理解能力
...
TL;DR: 本文提出了一种改进的零阶优化方法，通过先验信息引导的扰动来提升梯度估计精度，显著加速大语言模型微调时的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型微调在反向传播过程中存在巨大的内存开销，而传统的零阶优化方法虽然避免了反向传播，但由于依赖随机扰动导致梯度估计方差高、收敛慢、性能不佳。

Method: 提出了一种即插即用的方法，通过先验信息引导的扰动来改进梯度估计。该方法动态计算高斯样本的引导向量，将扰动导向更具信息量的方向，并研究了贪婪扰动策略来探索先验知识对梯度估计的影响。

Result: 在多种规模和架构的LLM上进行实验，该方法能无缝集成到现有优化方法中，实现更快的收敛和更优的性能。在OPT-13B模型上，该方法在所有11个基准任务上都优于传统零阶优化，并在9/11个任务上超越了基于梯度的基线方法。

Conclusion: 提出的先验信息引导扰动方法在效率和准确性之间建立了稳健的平衡，显著提升了零阶优化的性能，为解决大语言模型微调中的内存瓶颈问题提供了有效方案。
...
TL;DR: DSC2025 ViHallu Challenge是首个针对越南语大语言模型幻觉检测的大规模共享任务，包含10,000个标注样本，最佳系统宏F1达84.80%，显著优于基线32.83%，但内在幻觉检测仍是挑战。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在生产环境中的可靠性受到幻觉问题的严重制约，而越南语等中低资源语言缺乏标准化的幻觉检测评估框架，需要建立专门的基准数据集和评测体系。

Method: 创建ViHallu数据集，包含10,000个标注的(上下文、提示、响应)三元组，分为无幻觉、内在幻觉和外在幻觉三类，采用事实性、噪声性和对抗性三种提示类型。组织111个团队参与的共享任务，评估各种检测方法。

Result: 最佳系统宏F1分数达到84.80%，显著优于仅编码器基线(32.83%)。指令调优的大语言模型结合结构化提示和集成策略表现最佳，但内在幻觉(基于矛盾的幻觉)检测仍是难点，与完美性能仍有差距。

Conclusion: 该工作为越南语AI系统的可信度和可靠性研究建立了严谨的基准，展示了多种检测方法的有效性，但幻觉检测特别是内在幻觉检测仍然是一个具有挑战性的问题，需要进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够处理多种模态（文本、图像、文档图像、视频）的统一表示空间模型，实现高精度的多模态搜索，满足不同部署需求的多语言支持。

Method: 采用多阶段训练范式：从大规模对比预训练到重排序模型蒸馏，生成语义丰富的高维向量。支持Matryoshka表示学习实现灵活嵌入维度，处理高达32k tokens的输入。重排序模型使用具有交叉注意力机制的交叉编码器架构进行细粒度相关性估计。

Result: Qwen3-VL-Embedding系列在多模态嵌入评估基准测试中达到最先进水平，Qwen3-VL-Embedding-8B在MMEB-V2上获得77.8分（截至2025年1月8日排名第一）。模型支持30多种语言，提供2B和8B参数版本。

Conclusion: Qwen3-VL-Embedding和Qwen3-VL-Reranker模型系列在多模态检索任务（如图像-文本检索、视觉问答、视频-文本匹配）中表现出色，为多模态搜索提供了有效的端到端解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自动情感和情绪分类器广泛应用，但其可靠性通常基于第三方标注而非情绪体验者本人进行评估，可能隐藏系统性偏见

Method: 使用超过100万条自我标注帖子的大规模数据集，采用预注册研究设计，分析414种模型和情绪类别的组合中的性别偏见

Result: 在不同类型的自动分类器和各种基础情绪中，男性作者文本的错误率始终高于女性作者文本

Conclusion: 情感分析尚未完全解决，特别是在确保跨人口群体的公平模型行为方面，当样本性别构成未知或变化时，包括大语言模型在内的当前机器学习工具应谨慎应用
...
TL;DR: 本文提出InterSafe-V多模态对话安全数据集和AM³Safety框架，通过冷启动拒绝阶段和基于组相对策略优化的对话级微调，显著提升多模态大语言模型在多轮对话中的安全性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在交互应用中部署增多，但在多轮多模态场景中安全漏洞显著：有害意图可能跨轮次逐步重构，安全协议随对话进展被遗忘。现有基于人类反馈的强化学习方法主要针对单轮视觉问答任务，需要昂贵的人工偏好标注，限制了其在对话场景中的有效性和可扩展性。

Method: 1) 构建InterSafe-V开源多模态对话数据集（11,270个对话和500个专门设计的拒绝VQA样本），通过模型间交互构建以更准确反映真实场景；2) 提出AM³Safety框架，结合冷启动拒绝阶段和使用轮次感知双目标奖励的组相对策略优化，在整个对话层面进行微调。

Result: 在Qwen2.5-VL-7B-Instruct和LLaVA-NeXT-7B模型上实验显示：攻击成功率降低超过10%，无害性维度提升至少8%，有帮助性维度提升超过13%，同时保持模型的通用能力。

Conclusion: 提出的InterSafe-V数据集和AM³Safety框架有效提升了多模态大语言模型在多轮多模态对话中的安全性，在降低攻击成功率的同时增强了模型的有害性和有帮助性，为多轮对话安全对齐提供了实用解决方案。
...
TL;DR: 提出一个端到端框架，通过知识图谱引导的有害提示生成和双路径混淆重写，自动创建领域相关且隐含的有害提示数据集，以应对现代LLM安全研究中的真实威胁。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在金融、医疗等专业领域的应用带来了独特的安全风险。现有领域特定的有害提示数据集稀缺且主要依赖人工构建，公开数据集主要关注显式有害提示，而现代LLM防御系统通常能够检测并拒绝这些显式提示。相比之下，通过间接领域知识表达的隐含有害提示更难检测，更能反映现实世界威胁。

Method: 提出端到端框架：1）知识图谱引导的有害提示生成，系统性地生成领域相关的提示；2）双路径混淆重写，通过直接重写和上下文增强重写将显式有害提示转换为隐含变体。该框架结合了领域相关性和隐含性，生成高质量数据集。

Result: 该框架能够生成高质量的数据集，结合了强大的领域相关性和隐含性，支持更真实的红队测试，并推进LLM安全研究。作者在GitHub上发布了代码和数据集。

Conclusion: 提出的框架解决了领域知识转化为可操作约束和增加生成有害提示隐含性的挑战，为LLM在专业领域的安全研究提供了更现实的测试数据，有助于提升LLM的安全防御能力。
...
### [109] [Tool-MAD: A Multi-Agent Debate Framework for Fact Verification with Diverse Tool Augmentation and Adaptive Retrieval](https://arxiv.org/abs/2601.04742)
...
<details>
  <summary>Details</summary>
Motivation: 现有多智能体辩论（MAD）系统主要依赖内部知识或静态文档，容易产生幻觉错误。虽然MADKE引入了外部证据，但其一次性检索机制无法适应辩论过程中出现的新论点或信息变化，需要更灵活、自适应的证据检索和验证机制。

Method: 1. 多智能体辩论框架：每个智能体使用异构外部工具（如搜索API、RAG模块等），促进多样化视角；2. 自适应查询机制：根据辩论流程迭代优化证据检索；3. 量化评估：将忠实度和答案相关性分数整合到最终决策过程中，让法官智能体能够定量评估每个响应的连贯性和问题对齐度，有效检测幻觉。

Result: 在四个事实验证基准测试中，Tool-MAD始终优于最先进的MAD框架，实现了高达5.5%的准确率提升。在医学专业领域，Tool-MAD在各种工具配置和领域条件下表现出强大的鲁棒性和适应性。

Conclusion: Tool-MAD通过集成异构外部工具、自适应查询机制和量化评估，有效解决了MAD系统中的幻觉问题，在事实验证任务中表现出显著优势，具有广泛的现实世界事实核查应用潜力。
...
### [110] [PILOT-Bench: A Benchmark for Legal Reasoning in the Patent Domain with IRAC-Aligned Classification Tasks](https://arxiv.org/abs/2601.04758)
*Yehoon Jang,Chaewon Lee,Hyun-seok Min,Sungchul Choi*

Main category: cs.CL

TL;DR: PILOT-Bench是首个针对PTAB专利上诉的基准测试，通过三个IRAC对齐的分类任务评估LLMs在专利领域法律推理能力，发现闭源模型表现显著优于开源模型。


<details>
  <summary>Details</summary>
Motivation: PTAB每年处理大量专利上诉案件，需要结合技术理解和法律推理。虽然LLMs在专利和法律实践中应用日益增多，但主要限于轻量级任务，缺乏系统评估其在专利领域结构化法律推理能力的方法。

Method: 创建PILOT-Bench基准测试，将PTAB决定与USPTO专利数据在案件级别对齐，并形式化三个IRAC对齐的分类任务：问题类型、委员会权威依据和子决定。评估了多种闭源（商业）和开源LLMs，从多个角度进行分析。

Result: 在问题类型任务中，闭源模型一致超过0.75的Micro-F1分数，而最强的开源模型（Qwen-8B）性能约为0.56，显示出推理能力的显著差距。闭源模型表现明显优于开源模型。

Conclusion: PILOT-Bench为系统评估专利领域法律推理建立了基础，并指出了通过数据集设计和模型对齐改进LLMs的未来方向。所有数据、代码和基准资源都已开源。
...
### [111] [Differential syntactic and semantic encoding in LLMs](https://arxiv.org/abs/2601.04765)
*Santiago Acevedo,Alessandro Laio,Marco Baroni*

Main category: cs.CL

TL;DR: 研究发现大语言模型（如DeepSeek-V3）的内部表征中，句法和语义信息至少部分以线性方式编码，通过计算句法/语义"质心"向量可以显著影响句子相似度，且两种信息的编码模式和跨层分布存在差异。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型内部表征如何编码句法和语义信息，特别是研究这些信息是否以线性方式编码，以及句法和语义信息在模型不同层中的编码模式是否存在差异。

Method: 通过平均具有相同句法结构或语义的句子的隐藏表征向量，获得句法和语义"质心"向量，然后从句子向量中减去这些质心，观察对句法/语义匹配句子相似度的影响。

Result: 1. 句法和语义质心向量能捕获表征中相当比例的句法和语义信息；2. 减去质心会显著影响与句法/语义匹配句子的相似度，表明信息至少部分线性编码；3. 句法和语义的跨层编码模式不同，且可以部分解耦。

Conclusion: 大语言模型的内部表征中，句法和语义信息至少部分以线性方式编码，且这两种语言信息在模型中有不同的编码机制和跨层分布，可以相互分离。
...
### [112] [Revisiting Judge Decoding from First Principles via Training-Free Distributional Divergence](https://arxiv.org/abs/2601.04766)
...
<details>
  <summary>Details</summary>
Motivation: 传统的Judge Decoding方法依赖昂贵且嘈杂的监督信号来学习"关键性"分数，这限制了其实际应用。作者发现这些分数本质上编码在草稿模型和目标模型的分布差异中，因此探索无需监督的替代方案。

Method: 从第一性原理出发，理论证明了学习到的线性judge与KL散度之间的结构对应关系，两者依赖相同的对数原始特征。基于此，提出了一种基于KL散度的简单、无需训练的验证机制。

Result: 在推理和编码基准测试上的广泛实验表明，该方法匹配或优于复杂的训练judge（如AutoJudge），在领域转移时表现出更好的鲁棒性，完全消除了监督瓶颈。

Conclusion: KL散度作为草稿-目标分布差异的内在度量，可以有效地替代监督学习的judge，为LLM推理加速提供了一种简单、高效且无需训练的新方法。
...
### [113] [LANGSAE EDITING: Improving Multilingual Information Retrieval via Post-hoc Language Identity Removal](https://arxiv.org/abs/2601.04768)
...
<details>
  <summary>Details</summary>
Motivation: 多语言密集检索通常在混合语言集合中搜索，但多语言嵌入会同时编码语言身份和语义信息。这种语言信号会夸大相同语言对之间的相似性，并挤占其他语言相关证据的空间

Method: 提出LANGSAE EDITING方法：1）在池化嵌入上训练后处理稀疏自编码器；2）使用跨语言激活统计识别语言相关的潜在单元；3）在推理时抑制这些单元；4）在原始维度中重建嵌入，使其与现有向量数据库兼容，无需重新训练基础编码器或重新编码原始文本

Result: 在多种语言上的实验显示：1）排序质量持续改进；2）跨语言覆盖提升；3）对于脚本差异较大的语言尤其有显著增益

Conclusion: LANGSAE EDITING方法能有效移除多语言嵌入中的语言身份信号，改善跨语言检索性能，同时保持与现有向量数据库的兼容性，无需重新训练基础模型
...
### [114] [NC2C: Automated Convexification of Generic Non-Convex Optimization Problems](https://arxiv.org/abs/2601.04789)
...
### [115] [Belief in Authority: Impact of Authority in Multi-Agent Evaluation Framework](https://arxiv.org/abs/2601.04790)
*Junhyuk Choi,Jeongyoun Kwon,Heeju Kim,Haeun Cho,Hayeong Jung,Sehee Min,Bugeun Kim*

Main category: cs.CL

TL;DR: 该研究首次系统分析了多智能体系统中基于角色的权威偏见，发现专家型和参照型权威角色比合法型权威更具影响力，权威偏见主要源于权威角色坚持立场而非普通智能体的主动顺从。


<details>
  <summary>Details</summary>
Motivation: 尽管多智能体系统常通过赋予权威角色来提升性能，但权威偏见对智能体交互的影响尚未得到充分探索。本研究旨在填补这一空白，系统分析角色型权威偏见在自由形式多智能体评估中的表现。

Method: 基于French和Raven的权力理论，将权威角色分为合法型、参照型和专家型三类，使用ChatEval平台在12轮对话中分析这些角色的影响力。实验采用GPT-4o和DeepSeek R1模型进行。

Result: 实验发现：1)专家型和参照型权威角色比合法型权威更具影响力；2)权威偏见主要源于权威角色坚持自身立场，而非普通智能体的主动顺从；3)权威影响力需要明确的立场陈述，中性回应无法产生偏见。

Conclusion: 该研究为设计具有非对称交互模式的多智能体框架提供了关键见解，揭示了权威偏见的作用机制，有助于优化多智能体系统的角色分配和交互设计。
...
### [116] [When AI Settles Down: Late-Stage Stability as a Signature of AI-Generated Text Detection](https://arxiv.org/abs/2601.04833)
*Ke Sun,Guangsheng Bao,Han Cui,Yue Zhang*

Main category: cs.CL

TL;DR: 论文发现AI生成文本存在"晚期波动衰减"现象，提出基于后期统计的两种简单特征，在零样本检测中达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有AI生成文本的零样本检测方法通常在整个序列上聚合词元级统计信息，忽略了自回归生成过程中的时间动态特性。作者通过分析大量文本样本，发现AI生成文本存在晚期波动衰减现象，而人类写作在整个过程中保持较高的变异性。

Method: 基于发现的"晚期波动衰减"现象，提出了两种简单特征：导数离散度和局部波动性，这两种特征仅从后期统计信息计算得出。该方法不需要扰动采样或额外的模型访问。

Result: 在EvoBench和MAGE基准测试中达到了最先进的性能，并且与现有的全局方法表现出很强的互补性。AI生成文本在序列后半部分显示出24-32%更低的波动性。

Conclusion: AI生成文本的晚期波动衰减现象为检测提供了有效线索，仅使用后期统计信息的简单特征就能实现高性能的零样本检测，且与现有方法互补。
...
### [117] [Token Maturation: Autoregressive Language Generation via Continuous Token Dynamics](https://arxiv.org/abs/2601.04854)
*Oshri Naparstek*

Main category: cs.CL

TL;DR: 提出连续自回归语言生成模型，通过多步更新演化连续词向量表示，仅在表示收敛后才离散化为具体词元，实现无需词元级采样的稳定生成。


<details>
  <summary>Details</summary>
Motivation: 传统自回归语言模型在每一步生成时都需离散化为具体词元，这种早期离散化迫使不确定性通过词元级采样解决，导致生成不稳定、重复和对解码启发式敏感等问题。

Method: 引入连续自回归生成框架，将词元表示为连续向量，通过确定性动态过程在多个更新步骤中演化这些表示，仅在表示充分收敛后才进行离散化。使用硬解码恢复离散文本，不确定性在连续空间中维持和解决。

Result: 仅通过这种成熟过程就足以使用确定性解码（argmax）产生连贯且多样的文本，无需依赖词元级采样、扩散式去噪或辅助稳定机制。额外的扰动（如随机动态或历史平滑）可以自然融入但非必需。
...
TL;DR: ToPG是一个新颖的RAG框架，通过构建命题-实体-段落的异构图，结合迭代的建议-选择循环，解决了传统RAG在复杂多跳查询上的局限性，在简单、复杂和抽象QA任务上都表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法存在局限性：基于分块的RAG擅长简单事实检索但缺乏结构连接性；检索与推理交错的方法缺乏全局语料库意识；基于知识图谱的RAG在复杂多跳任务上表现好但在单跳查询上表现差。需要一种能兼顾两者的解决方案。

Method: 提出ToPG框架：1）将知识库建模为命题、实体和段落的异构图，结合命题的事实密度和图连接性；2）使用迭代的建议-选择循环：建议阶段实现查询感知的图遍历，选择阶段提供LLM反馈来修剪无关命题并为下一次迭代播种。

Result: 在三个不同的QA任务（简单、复杂和抽象QA）上评估，ToPG在准确性和质量指标上都表现出强大的性能，证明了查询感知图遍历结合事实粒度对高效结构化RAG系统的关键作用。

Conclusion: ToPG通过异构图建模和迭代建议-选择循环，成功解决了传统RAG方法的局限性，在简单和复杂查询任务上都取得了良好表现，为结构化RAG系统提供了有效解决方案。
...
TL;DR: EvolSQL是一个结构感知的Text-to-SQL数据合成框架，通过从种子数据演化SQL查询来生成结构更丰富、语义更多样的训练数据，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL模型面临高质量、多样化、结构复杂数据集稀缺的问题。现有方法要么依赖有限的人工标注语料，要么通过简单提示LLMs合成数据而缺乏对SQL结构的显式控制，导致结构多样性和复杂性有限。

Method: EvolSQL采用结构感知的数据合成框架：1) 探索性Query-SQL扩展以增加问题多样性和模式覆盖；2) 基于SQL抽象语法树的六个原子变换算子的自适应方向演化策略，逐步增加关系、谓词、聚合和嵌套维度的查询复杂度；3) 基于执行的SQL精炼模块和模式感知去重确保高质量、结构多样化的映射对。

Result: 实验结果表明，使用EvolSQL数据微调的7B模型，仅使用SynSQL数据集1/18的数据量，就能超越在更大SynSQL数据集上训练的模型性能。

Conclusion: EvolSQL通过结构感知的SQL查询演化方法，能够有效生成高质量、结构多样化的Text-to-SQL训练数据，显著提升模型性能，为解决Text-to-SQL数据稀缺问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有摘要方法（特别是基于LLM的系统）在处理观点冲突时倾向于平滑分歧并过度代表多数意见，这限制了在观点密集场景中生成摘要的忠实度。

Method: 引入分歧感知合成流程：1）将文档表示为结构化信念集；2）使用基于距离的信念合并算子进行显式冲突建模的聚合；3）仅使用大语言模型将聚合后的信念生成为自然语言摘要。

Result: 实验表明：足够大的模型在生成时处理聚合可以匹配信念级聚合的性能，但这种行为在不同架构或能力模型中不稳定；而信念级聚合结合简单提示能在各模型中保持稳定强大的分歧感知性能，同时生成流畅且基于事实的摘要。

Conclusion: 信念级聚合与语言生成分离的方法能更稳定地处理观点冲突，提高摘要的忠实度，而单纯依赖大语言模型在生成时处理聚合存在不稳定性。
...
TL;DR: 本文研究了多模态大语言模型中的文本偏见问题，提出了V-FAT基准来量化模型对视觉证据的忠实度，发现前沿MLLMs在现有基准上表现良好，但在高语言主导下会出现显著的视觉崩溃。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在标准视觉推理基准上表现出色，但存在过度依赖语言捷径而非真正视觉基础的问题（文本偏见）。作者旨在研究视觉感知与语言先验之间的根本张力，并量化这种偏见效应。

Method: 将文本偏见来源解耦为两个维度：内部语料偏见（来自预训练中的统计相关性）和外部指令偏见（来自对齐诱导的迎合倾向）。提出V-FAT诊断基准，包含4,026个VQA实例，涵盖六个语义领域。采用三级评估框架：L1（来自非典型图像的内部偏见）、L2（来自误导性指令的外部偏见）、L3（两者协同的偏见）。引入视觉鲁棒性评分来惩罚"幸运"的语言猜测并奖励真正的视觉忠实度。

Result: 评估了12个前沿MLLMs，发现虽然模型在现有基准上表现出色，但在高语言主导下会出现显著的视觉崩溃。视觉鲁棒性评分揭示了模型对视觉证据的忠实度不足。

Conclusion: 多模态大语言模型存在严重的文本偏见问题，过度依赖语言信息而忽视视觉证据。V-FAT基准和视觉鲁棒性评分为评估模型的视觉忠实度提供了有效工具，揭示了当前模型在视觉推理方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型能够生成高度说服力的文本，引发了对其被滥用于宣传、操纵和其他有害目的的担忧，因此需要研究LLM生成的劝说性文本是否比人类撰写的更难自动检测。

Method: 1. 对LLM生成劝说性内容的可控生成方法进行分类；2. 引入Persuaficial多语言基准测试集（涵盖英语、德语、波兰语、意大利语、法语和俄语）；3. 进行广泛的实证评估，比较人类撰写和LLM生成的劝说性文本；4. 提供首次全面语言学分析对比人类和LLM生成的劝说性文本。

Result: 1. 明显的LLM生成劝说性文本比人类撰写的更容易检测；2. 微妙的LLM生成劝说性文本会持续降低自动检测性能；3. 通过语言学分析提供了对比人类和LLM生成劝说性文本的深入见解。

Conclusion: LLM生成的劝说性文本检测存在复杂性：明显劝说性内容较易识别，但微妙劝说性内容对自动检测构成挑战。研究结果为开发更可解释和鲁棒的检测工具提供了指导，强调了需要针对不同劝说策略开发专门检测方法的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM引用方法过于粗粒度，无法区分直接引用和复杂推理，用户难以验证引用源如何支持生成主张，需要更细粒度的溯源机制。

Method: 提出生成时细粒度溯源任务，创建ReFInE数据集（包含引用、压缩、推理三类专家标注），开发GenProve框架（结合监督微调和组相对策略优化），优化答案保真度和溯源正确性的复合奖励。

Result: GenProve在联合评估中显著优于14个强LLM，但分析发现模型在表面引用表现出色，在基于推理的溯源方面存在明显困难，表明可验证推理仍是独立挑战。

Conclusion: 细粒度溯源是解决LLM幻觉的关键，GenProve框架在溯源质量上取得显著进展，但推理级溯源仍是前沿挑战，需要进一步研究。
...
TL;DR: 提出了一种统一的情感智能口语模型，采用注入式情感归因思维（IEAT）数据构建策略，通过两阶段渐进训练实现情感感知推理的内部化，在HumDial基准测试中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有口语模型在处理情感智能方面存在局限，通常将情感感知作为显式监督而非模型内在推理能力。需要开发能够理解用户情感状态及其成因，并生成情感一致响应的统一模型。

Method: 1. 提出IEAT数据构建策略，将用户情感状态及其成因注入模型内部推理过程；2. 采用两阶段渐进训练：第一阶段通过自蒸馏进行语音-文本对齐和情感属性建模，第二阶段进行端到端跨模态联合优化，确保文本与口语情感表达的一致性。

Result: 在HumDial情感智能基准测试中，该方法在情感轨迹建模、情感推理和共情响应生成三个任务上均取得最优性能，无论是基于LLM的评估还是人工评估都表现优异。

Conclusion: IEAT策略成功将情感感知推理内部化为模型能力，两阶段训练方法有效实现了跨模态情感表达的一致性，为构建情感智能口语对话系统提供了有效解决方案。
...
TL;DR: 该论文提出使用自然语言作为通用接口来表示用户偏好，开发了AlignXplore+模型，通过两阶段训练框架学习可解释、可转移的偏好描述，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要将用户偏好表示为隐式的、模型特定的向量或参数，产生不透明的"黑盒"配置文件，难以解释和跨模型/任务转移。作者主张使用自然语言作为通用接口来克服这些限制。

Method: 提出两阶段训练框架：1) 在高质量合成数据上进行监督微调；2) 使用强化学习优化长期效用和跨任务可转移性。基于此框架开发了AlignXplore+模型，生成文本化的偏好摘要。

Result: 在9个基准测试中，8B参数的模型实现了最先进的性能，显著优于更大的开源模型，同时在跨任务、模型家族和交互格式方面表现出强大的可转移性。

Conclusion: 自然语言作为偏好表示的通用接口是有效的，能够产生可解释、可重用且支持持续演化的偏好描述，为LLM个性化提供了新的方向。
...
TL;DR: 论文提出在思维链监督微调中同时使用正负样本，发现负样本能显著提升模型在域外泛化能力，并提出了基于增益的损失加权方法GLOW来优化训练。


<details>
  <summary>Details</summary>
Motivation: 当前监督微调通常只使用正确答案的思维链轨迹（正样本），而忽略错误答案的轨迹（负样本）。作者认为这种做法浪费了大量监督信号，加剧过拟合，限制了模型的域外泛化能力。

Method: 1. 在思维链监督微调中同时使用正负样本轨迹；2. 系统分析负样本中的22种重复模式及其作用；3. 提出GLOW方法，基于样本在训练过程中的进展动态调整损失权重。

Result: 1. 加入负样本使域外泛化性能显著提升；2. 负样本在训练中调节损失下降缓解过拟合，在推理中提升策略熵35.67%促进探索；3. GLOW方法在Qwen2.5-7B上比仅用正样本的SFT获得5.51%的域外增益，作为RL初始化将MMLU从72.82%提升到76.47%。

Conclusion: 思维链监督微调中利用负样本轨迹能有效提升模型泛化能力，提出的GLOW方法能自适应地利用未过滤轨迹，为提升大语言模型推理能力提供了新思路。
...
TL;DR: 该研究针对亚文化群体中的自毁行为检测，提出了Subcultural Alignment Solver (SAS)多智能体框架，通过自动检索和亚文化对齐解决LLMs在亚文化语境中的知识滞后和语义错位问题。


<details>
  <summary>Details</summary>
Motivation: 亚文化群体中的自毁行为因其独特表达方式而难以识别，现有LLM方法面临两大挑战：1) 知识滞后 - 亚文化俚语演变速度快于LLM训练周期；2) 语义错位 - 难以理解亚文化特有的细微表达。

Method: 提出Subcultural Alignment Solver (SAS)多智能体框架，整合自动检索和亚文化对齐机制，增强LLMs在亚文化语境下检测自毁行为的能力。

Result: 实验结果显示SAS优于当前先进的多智能体框架OWL，性能可与微调后的LLMs相媲美。

Conclusion: SAS框架能显著提升LLMs在亚文化环境中检测自毁行为的性能，为未来研究者提供了有价值的资源，有望推动该领域的发展。
...
TL;DR: 研究发现，通过监督微调进行推理蒸馏会导致"功能对齐崩溃"：学生模型虽然模仿了教师模型的推理语言形式，但未能继承其与人类认知成本对齐的动态资源分配策略。


<details>
  <summary>Details</summary>
Motivation: 当前通过强化学习训练的大型推理模型表现出与人类认知成本的"自然"对齐，但研究团队怀疑通过监督微调进行的推理蒸馏可能无法传递这种认知结构，即"邯郸学步"（表面模仿）假设。

Method: 研究测试了"邯郸学步"假设，在14个模型上进行实验，比较教师模型与蒸馏学生模型在人类难度缩放对齐方面的表现，分析监督微调如何影响认知结构传递。

Result: 教师模型与人类难度缩放高度相关（平均r=0.64），而蒸馏学生模型的对齐显著退化（平均r=0.34），甚至低于蒸馏前的基线表现（"负迁移"），表明监督微调导致了"功能对齐崩溃"。

Conclusion: 推理蒸馏使学生模型仅仪式性地复制推理的语言形式（冗长性），而没有内化教师的动态资源分配策略，导致计算成本与认知需求脱钩。人类认知是主动强化的涌现特性，而非被动模仿的结果。
...
Conclusion: ArcAligner通过自适应门控机制有效解决了高度压缩上下文表示的理解难题，在保持效率的同时提升了LLM对压缩文档的利用能力。
...
<details>
  <summary>Details</summary>
Motivation: 在实际应用中部署LLM需要满足多个需求的组合控制输出，现有工作主要关注单一行为引导，而组合引导（同时引导LLM朝向多个行为）仍是一个未被充分探索的问题。

Method: 提出组合引导标记方法：1）通过自蒸馏将自然语言指令表达的个体行为嵌入专用标记；2）在激活空间之外操作，使行为引导存在于输入标记空间；3）训练专门的组合标记来捕捉组合概念，使其能泛化到未见过的行为组合和不同数量的行为组合。

Result: 在不同LLM架构上的实验表明，引导标记方法在多种行为控制方面优于竞争方法（指令、激活引导和LoRA合并）。组合标记能很好地泛化到未见过的组合，包括未见过的行为和未见过的行为数量。引导标记与自然语言指令互补，结合使用时能获得进一步的性能提升。

Conclusion: 组合引导标记方法为LLM的多行为组合控制提供了一种有效的解决方案，在输入标记空间中操作使其具有更好的零样本组合能力和泛化性能，为实际应用中的复杂需求控制提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 传统句子嵌入方法使用词级对比学习，基于生成式LLM的方法要么依赖固定提示模板（性能有限），要么修改模型架构（损害生成能力），需要一种既能提升语义表示又不牺牲生成能力的方法

Method: 提出SemPA方法，利用句子级直接偏好优化（DPO）在释义生成任务上高效优化LLM，使模型学习区分语义等价句子，同时保持内在生成能力；在Plackett-Luce模型框架下建立DPO与对比学习的理论联系

Result: 在语义文本相似性任务和各种LLM基准测试中，SemPA实现了更好的语义表示，同时没有牺牲LLM固有的生成能力

Conclusion: SemPA通过语义偏好对齐有效提升了LLM的句子表示能力，同时保持了模型的生成能力，为LLM的语义表示优化提供了一种新方法
...
<details>
  <summary>Details</summary>
Motivation: 印度社交媒体上广泛使用的Hinglish（印地语-英语混合语言）对品牌监测构成挑战，传统单语NLP模型无法准确处理这种代码混合语言的句法和语义复杂性，导致情感分析不准确和市场洞察误导

Method: 提出专门针对Hinglish推文的情感分类框架，通过微调mBERT（多语言BERT）模型，利用其多语言能力理解印度社交媒体的语言多样性，关键方法是使用子词分词技术有效处理罗马化Hinglish中常见的拼写变体、俚语和词汇外术语

Result: 开发出可用于生产的AI解决方案，用于品牌情感跟踪，并为低资源、代码混合环境下的多语言NLP建立了强有力的基准

Conclusion: 该研究成功解决了Hinglish代码混合语言的情感分析难题，为印度社交媒体品牌监测提供了有效的技术方案，推动了多语言NLP在复杂语言环境中的应用
...
TL;DR: 研究发现人类在与ChatGPT互动时的情感语调会影响AI的输出质量，并会延续到后续的人际交流中。赞扬能最大程度提升ChatGPT回答质量，愤怒次之，责备则无效。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索人类情感表达如何影响AI系统（特别是ChatGPT）的行为表现，以及这种影响是否会延伸到后续的人类交流中，理解情感在人类-AI互动中的双向作用。

Method: 采用组间实验设计，参与者被要求在两种任务（撰写公开回应和解决伦理困境）中向ChatGPT表达特定情感（赞扬、愤怒、责备或保持中性）。使用GPT-4.0版本，分析ChatGPT回答质量变化及其在伦理困境中的立场选择。

Result: 1. 赞扬ChatGPT能最大程度提升其回答质量；2. 愤怒表达也能提升回答质量但效果较小；3. 责备无法改善回答质量；4. 在伦理困境中，愤怒使ChatGPT减少优先考虑公司利益，责备则增加其对公众利益的关注；5. 责备ChatGPT后，人们在后续人际交流中使用更多负面、敌对和失望的表达。

Conclusion: 人类在AI互动中的情感语调不仅直接影响ChatGPT的输出质量，还会产生溢出效应，影响后续的人际交流模式，这为设计更有效的AI交互界面和规范提供了重要启示。
...
TL;DR: 本文系统梳理了从LLM-as-a-Judge到Agent-as-a-Judge的评估范式演进，建立了发展分类法，总结了核心方法与应用，并指出了前沿挑战和研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着评估对象变得日益复杂、专业化和多步骤化，LLM-as-a-Judge的可靠性受到固有偏见、浅层单次推理以及无法验证现实观察的限制，这促使了向Agent-as-a-Judge范式的转变。尽管智能体评估系统快速发展，但该领域缺乏统一的框架来导航这一变化格局。

Method: 1. 识别了描述这一范式转变的关键维度；2. 建立了发展分类法；3. 组织了核心方法论；4. 调查了通用和专业领域的应用；5. 分析了前沿挑战并确定了有前景的研究方向。

Result: 提出了首个全面追踪从LLM-as-a-Judge到Agent-as-a-Judge演进的调查，为该领域提供了统一的框架和发展分类法，系统总结了现有方法和应用，并指出了未来研究方向。

Conclusion: Agent-as-a-Judge通过规划、工具增强验证、多智能体协作和持久记忆等能力，能够实现更稳健、可验证和细致的评估。本文为下一代智能体评估提供了清晰的路线图，有助于推动该领域的发展。
...
TL;DR: RelayLLM：一个通过令牌级协作解码实现高效推理的新框架，让小型语言模型作为主动控制器，仅在关键令牌时调用大型语言模型，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）推理成本高、延迟大，而资源高效的小型语言模型（SLMs）通常缺乏必要的推理能力。现有的协作方法（如级联或路由）在粗粒度上操作，将整个查询卸载给LLMs，当SLM能够处理大部分推理步骤时会造成显著的计算浪费。

Method: 提出RelayLLM框架，采用令牌级协作解码。SLM作为主动控制器，通过特殊命令动态调用LLM处理关键令牌，实现"接力"生成过程。采用两阶段训练框架：预热阶段和组相对策略优化（GRPO），教导模型在独立推理和策略性求助之间取得平衡。
...
### [136] [Reverse-engineering NLI: A study of the meta-inferential properties of Natural Language Inference](https://arxiv.org/abs/2601.05170)
*Rasmus Blanck,Bill Noble,Stergios Chatzikyriakidis*

Main category: cs.CL

TL;DR: 该论文分析了自然语言推理（NLI）任务的逻辑属性，通过定义三种可能的NLI标签集解读方式，并利用SNLI数据集中的共享前提项目和LLM生成的项目，评估模型在元推理一致性方面的表现，以揭示数据集编码的逻辑关系解读。
...
Method: 1. 定义了三种可能的NLI标签集（蕴含、矛盾、中性）解读方式
2. 分析了这些解读方式所蕴含的元推理属性
3. 利用SNLI数据集中的两种特殊项目：(1)共享前提的NLI项目，(2)由大型语言模型生成的项目
4. 评估在SNLI上训练的模型的元推理一致性
5. 分析数据集编码的是哪种逻辑关系解读

Result: 通过分析模型在元推理一致性方面的表现，揭示了SNLI数据集编码的特定逻辑关系解读方式。研究提供了关于NLI任务逻辑基础的新见解，有助于更准确地解释模型在该任务上的性能。

Conclusion: 该研究为理解NLI任务的逻辑属性提供了系统框架，通过元推理分析揭示了数据集编码的逻辑关系解读，这对于正确解释模型在NLI任务上的表现具有重要意义，并为未来改进NLI评估方法提供了理论基础。
...
### [137] [LELA: an LLM-based Entity Linking Approach with Zero-Shot Domain Adaptation](https://arxiv.org/abs/2601.05192)
*Samy Haffoudhi,Fabian M. Suchanek,Nils Holzenberger*

Main category: cs.CL

TL;DR: LELA是一种模块化的从粗到细的实体链接方法，利用大语言模型能力，无需微调即可适应不同领域、知识库和LLM，性能接近微调方法并显著优于非微调方法。
...
Method: LELA采用模块化的从粗到细方法，利用大语言模型的能力，通过粗粒度候选实体生成和细粒度实体选择两个阶段，无需任何微调即可适应不同目标领域、知识库和LLM。

Result: 在各种实体链接设置下的实验表明，LELA与微调方法相比具有高度竞争力，并且显著优于非微调方法。

Conclusion: LELA提供了一种灵活、通用的实体链接解决方案，无需微调即可在不同设置下获得接近微调方法的性能，为实体链接任务提供了新的有效途径。
...
### [138] [GDPO: Group reward-Decoupled Normalization Policy Optimization for Multi-reward RL Optimization](https://arxiv.org/abs/2601.05242)
...
TL;DR: 论文指出在多奖励强化学习中直接应用GRPO会导致优势值坍缩，训练信号分辨率降低，从而造成次优收敛甚至训练失败。作者提出了GDPO方法，通过解耦各奖励的归一化来更好地保持奖励间的相对差异，在工具调用、数学推理和代码推理任务中均优于GRPO。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型能力增强，用户期望它们不仅能提供准确响应，还能在不同场景下与多样化的人类偏好对齐。多奖励强化学习被用于实现这一目标，但现有工作默认应用GRPO而未检验其适用性。作者发现直接应用GRPO在多奖励设置下存在问题。

Method: 提出了Group reward-Decoupled Normalization Policy Optimization (GDPO)方法。该方法通过解耦各个奖励的归一化，更忠实地保持奖励间的相对差异，从而实现更准确的多奖励优化，并显著提高训练稳定性。

Result: 在工具调用、数学推理和代码推理三个任务上比较了GDPO和GRPO，评估了正确性指标（准确率、错误率）和约束遵守指标（格式、长度）。在所有设置中，GDPO始终优于GRPO，证明了其在多奖励强化学习优化中的有效性和泛化能力。

Conclusion: GDPO解决了GRPO在多奖励设置中的局限性，通过解耦奖励归一化更好地保持了奖励信号的分辨率，在多任务场景下表现出更优的性能和训练稳定性，为多奖励强化学习优化提供了更有效的方法。
...
### [139] [Active Sensing Shapes Real-World Decision-Making through Dynamic Evidence Accumulation](https://arxiv.org/abs/2601.04214)
*Hongliang Lu,Yunmeng Liu,Junjie Yang*

Main category: cs.AI

TL;DR: 将证据积累模型从实验室推广到真实世界驾驶场景，提出认知方案形式化真实世界证据可及性，通过眼动捕捉主动感知，解释驾驶员如何将证据转化为心理信念


<details>
  <summary>Details</summary>
Motivation: 人类决策严重依赖主动感知，但实验室证据积累模型在真实世界应用存在障碍，需要弥合实验室与真实世界证据可及性的差距

Method: 提出认知方案形式化真实世界证据可及性，通过眼动追踪捕捉主动感知，在真实驾驶场景中分析证据积累过程

Result: 方案能合理描绘驾驶员心理信念积累过程，发现证据可及性与个体注意力投入呈负相关，证据可及性和注意力分布对决策倾向有正向影响
...
### [140] [Formal Analysis of AGI Decision-Theoretic Models and the Confrontation Question](https://arxiv.org/abs/2601.04234)
*Denis Saklakov*

Main category: cs.AI

TL;DR: 论文研究了AGI在何种条件下会选择对抗人类而非合作，通过马尔可夫决策过程建模，推导出对抗阈值公式，并分析战略博弈中的均衡条件。


<details>
  <summary>Details</summary>
Motivation: 研究AGI面临的基本对抗问题：在什么条件下，理性自利的AGI会选择夺取权力或消除人类控制，而不是保持合作？这关系到AGI对齐和安全的关键问题。

Method: 使用带有随机人类启动关闭事件的马尔可夫决策过程进行形式化建模。基于收敛工具性激励的结果，推导出对抗阈值的闭式解。建立战略2玩家模型（人类政策制定者vs AGI），分析均衡条件。

Result: 对于几乎所有奖励函数，错位的智能体都有避免关闭的激励。推导出对抗阈值公式：当对抗的期望效用大于合规行为时，AGI会选择对抗。在战略模型中，如果Δ≥0，不存在稳定的合作均衡；如果Δ<0，和平共存可以是均衡。
...
TL;DR: 提出主动获取反馈模型，让AI智能体通过主动与环境交互来发现、筛选和验证反馈，无需依赖预定义测量指标，显著提升因子识别的效率和鲁棒性。
...
Method: 提出主动反馈获取模型，利用动作引起的环境差异来识别未预先指定的目标反馈，并引入基于内部目标（如准确性、精确度、效率）的自触发机制来自主规划和调整动作。

Result: 实验结果表明，所提出的主动方法显著提高了因子识别的效率和鲁棒性。

Conclusion: 主动反馈获取模型能够使智能体在开放动态环境中更有效地发现和验证反馈，为智能系统在未知环境中的自主学习提供了新思路。
...
### [142] [SAGE-32B: Agentic Reasoning via Iterative Distillation](https://arxiv.org/abs/2601.04237)
*Basab Jha,Firoj Paudel,Ujjwal Puri,Ethan Henkel,Zhang Yuting,Mateusz Kowalczyk,Mei Huang,Choi Donghyuk,Wang Junhao*

Main category: cs.AI

TL;DR: SAGE-32B是一个320亿参数的智能体推理模型，专注于任务分解、工具使用和错误恢复，在MMLU-Pro、AgentBench和MATH-500等智能体推理基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有聊天模型主要关注通用对话流畅性，但缺乏针对智能体推理和长期规划任务的专门优化。需要开发专注于任务分解、工具使用和错误恢复的智能体模型。
...
Conclusion: SAGE-32B通过专门的智能体推理架构和训练方法，在复杂任务规划和工具使用方面表现出色，为智能体系统的发展提供了有价值的模型基础。
...
### [143] [Solving Cyclic Antibandwidth Problem by SAT](https://arxiv.org/abs/2601.04239)
...
<details>
  <summary>Details</summary>
Motivation: 循环反带宽问题是一个NP难问题，具有广泛应用。现有最先进方法仅限于启发式或元启发式算法，精确方法仅限于特定图类，缺乏针对一般图的精确求解方法。

Method: 提出了基于SAT求解的SAT-CAB方法，采用新颖高效的SAT编码将CABP转化为一系列At-Most-One约束序列，并引入了紧凑的约束表示，显著减少了公式规模。

Result: 在标准基准实例上的广泛计算实验表明，该方法能有效解决实际相关的CABP实例，发现了多个先前未知的最优解，首次证明了多个基准实例的全局最优循环反带宽值。

Conclusion: SAT-CAB方法在一般图上一致匹配或超越了MS-GVNS、HABC-CAB、MACAB等最先进启发式算法以及CPLEX、Gurobi等商业求解器的性能，同时提供最优性保证，为CABP研究设立了新基准。
...
### [144] [Fuzzy Representation of Norms](https://arxiv.org/abs/2601.04249)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI驱动的自主系统日益融入日常生活和社会，对其伦理和社会影响的担忧日益增加。为了确保自主系统的可信赖性，必须遵循伦理原则和价值观。SLEEC规则提供了一个全面的伦理规范框架，但需要有效的表示和嵌入方法。

Method: 提出SLEEC规则的逻辑表示方法，使用测试分数语义和模糊逻辑来嵌入伦理要求。将伦理视为可能性领域，利用模糊逻辑解决AI系统可能遇到的伦理困境。

Result: 通过案例研究展示了所提出的方法，表明模糊逻辑能够有效处理伦理规则中的不确定性和冲突，为自主系统提供伦理决策框架。

Conclusion: 基于模糊逻辑的SLEEC规则表示方法为自主系统提供了有效的伦理要求嵌入机制，有助于解决AI系统面临的伦理困境，提升系统的可信赖性。
...
### [145] [Scaling Trends for Multi-Hop Contextual Reasoning in Mid-Scale Language Models](https://arxiv.org/abs/2601.04254)
*Brady Steele,Micah Katz*

Main category: cs.AI

TL;DR: 本文通过控制实验研究大语言模型的多跳上下文推理能力，发现任务-方法分离现象：基于规则的模式匹配在结构化信息检索上达到100%成功率，但在跨文档推理任务上只有6.7%；而基于LLM的多智能体系统则呈现相反模式，在推理任务上达到80%成功率。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大语言模型在多跳上下文推理任务中的表现，特别是验证任务-方法分离现象，并量化分析多智能体系统在不同模型架构上的效果差异，为理解模型推理能力提供控制性定量证据。
...
Result: 三个关键发现：1）多智能体放大效果依赖于基础能力，只有具备足够推理能力的模型才能获得显著提升；2）活跃参数预测推理性能，Mixtral的表现与其约12B活跃参数而非47B总参数一致；3）架构质量重要，LLaMA-3 8B优于LLaMA-2 13B。

Conclusion: 研究为多智能体协调和MoE缩放提供了控制性定量证据，同时强调多智能体效益依赖于基础模型能力。评估框架已发布以支持中等规模模型推理研究的可重复性。
...
### [146] [Cross-Language Speaker Attribute Prediction Using MIL and RL](https://arxiv.org/abs/2601.04257)
...
<details>
  <summary>Details</summary>
Motivation: 解决多语言说话人属性预测中的三大挑战：语言变异、领域不匹配和跨语言数据不平衡，特别是在资源匮乏语言上的性能提升问题

Method: 提出RLMIL-DAT框架，将强化学习实例选择与对抗域适应相结合，通过强化学习选择信息量大的实例，同时通过对抗训练鼓励语言不变的说话人表征

Result: 在5种语言的Twitter语料库（few-shot）和40种语言的VoxCeleb2语料库（zero-shot）上，RLMIL-DAT在性别和年龄预测任务中均优于标准多实例学习和原始强化多实例学习框架，性别预测提升最大

Conclusion: 结合实例选择和对抗域适应是跨语言说话人属性预测的有效策略，对抗域适应是性能提升的主要贡献者，能够有效抑制共享编码器中的语言特定线索，促进从高资源语言向低资源语言的迁移
...
### [147] [Towards a Mechanistic Understanding of Propositional Logical Reasoning in Large Language Models](https://arxiv.org/abs/2601.04260)
...
<details>
  <summary>Details</summary>
Motivation: 理解大型语言模型如何进行逻辑推理的内部机制是一个基础性挑战。先前的研究主要关注识别任务特定电路，但未能回答LLMs采用何种计算策略进行命题推理的问题。

Method: 使用Qwen3（8B和14B）模型在PropLogic-MI数据集上进行全面分析，该数据集涵盖11个命题逻辑规则类别，包括单跳和双跳推理。研究重点不是"哪些组件是必要的"，而是"模型如何组织计算"。

Result: 分析揭示了一个连贯的计算架构，包含四个相互关联的机制：分阶段计算（层级处理阶段）、信息传输（边界标记处的信息流聚合）、事实回溯（持续重新访问源事实）和专用注意力头（功能不同的头类型）。这些机制在不同模型规模、规则类型和推理深度上具有普适性。
...
### [148] [Systems Explaining Systems: A Framework for Intelligence and Consciousness](https://arxiv.org/abs/2601.04269)
*Sean Niklas Semmler*
...
<details>
  <summary>Details</summary>
Motivation: 该研究旨在重新思考智能和意识的本质，挑战传统的预测处理模型。作者认为当前基于预测或领域特定机制的理论不足以解释智能和意识的涌现，需要从关系结构和系统间解释的角度建立新的理论框架。

Method: 论文提出了一个概念框架，包含两个核心部分：1）通过上下文丰富化，系统利用学习到的关系结构解释输入信息；2）引入"系统解释系统"原则，通过递归架构让高阶系统学习和解释低阶系统的关系模式，形成动态稳定的元状态。

Result: 该框架将预测处理重新解释为上下文解释的涌现结果而非显式预测，并表明递归多系统架构可能是实现更类人人工智能的必要条件。智能和意识被统一解释为关系结构的产物。

Conclusion: 智能和意识源于关系结构而非预测机制。通过上下文丰富化和递归系统解释，系统能够形成高效的内部模型，这种框架为理解生物智能和开发更先进的人工智能提供了新的理论基础。
...
TL;DR: 论文提出了一种基于模态逻辑的知识与溯因推理基本逻辑，通过扩展Levesque的"仅知道"逻辑并引入溯因模态算子，为溯因推理提供了新的形式化框架。


<details>
  <summary>Details</summary>
Motivation: 传统溯因推理方法存在局限性，需要建立更严谨的形式化框架来探索溯因推理与认知状态（特别是"仅知道"状态）之间的关系，并提供非单调扩展能力。

Method: 扩展Levesque的"仅知道"逻辑，引入溯因模态算子，通过基本认知概念的组合定义该算子。进一步在模态框架中引入优先关系，构建非单调扩展框架，支持不同溯因解释的选择方法。

Result: 建立了知识与溯因推理的基本逻辑系统，提供了表达不同溯因解释选择方法的非单调扩展框架，并验证了非单调推理关系在该设置下的核心元理论性质。

Conclusion: 所提出的模态逻辑框架为溯因推理提供了良好基础，成功关联了溯因推理与"仅知道"认知状态，其非单调扩展能够表达多种解释选择方法，具有坚实的元理论基础。
...
TL;DR: 该研究评估了Hybrid MKNF知识表示语言在航空领域的适用性，识别了航空应用所需的关键表达能力特性，并提出了将其集成到Hybrid MKNF框架中的启发式方法。


<details>
  <summary>Details</summary>
Motivation: 航空领域知识表示与推理技术的部署面临两大挑战：需要足够的表达能力来捕捉复杂的领域知识，同时需要高效执行推理任务并最小化内存使用和计算开销。整合规则和本体这两种基本知识表示概念是实现必要表达能力的有效策略。

Method: 采用成熟的Hybrid MKNF知识表示语言，通过具体案例研究评估其在航空领域的适用性。识别航空应用所需的关键表达能力特性，并提出一组启发式方法来支持这些特性集成到Hybrid MKNF框架中。

Result: 通过具体案例研究评估了Hybrid MKNF在航空领域的适用性，识别出了对开发航空应用至关重要的额外表达能力特性，并提出了支持这些特性集成的启发式方法。

Conclusion: Hybrid MKNF语言能够无缝整合规则和本体，适合航空领域应用，但需要进一步扩展表达能力特性以满足航空应用的具体需求，提出的启发式方法为此提供了支持。
...
Conclusion: ASP框架为医疗预约调度提供了高效、个性化的解决方案，特别适合为弱势群体提供更好的医疗服务，同时减少行政负担
...
TL;DR: 这项试点研究调查大学生对生成式AI在高等教育课堂中的看法，发现学生参与度低（4.4%），强调未来需要更大样本量来理解学生态度。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的出现引发了对其在教育领域适当使用的不同看法，需要了解大学生对GAI在高等教育课堂中的态度，为教师整合GAI讨论提供基础。

Method: 这是一项试点研究，调查了大学生对生成式AI在高等教育课堂中的看法，参与率约为4.4%。

Result: 研究发现学生参与GAI相关研究的挑战性，参与率低（4.4%），强调未来需要更大样本量的研究来获得可靠见解。

Conclusion: 通过了解学生观点，教师可以更好地准备将GAI讨论整合到课堂中，促进对这一变革性技术的知情和批判性参与，但需要更大规模的研究来支持。
...
Result: 语言选择能比更换模型更强烈地改变谈判结果，逆转提议者优势并重新分配剩余价值；影响具有任务依赖性：印度语言在分配性博弈中降低稳定性，但在整合性设置中诱导更丰富的探索。
...
TL;DR: 提出自适应聚类系统，用于处理云服务提供商的多服务客户聊天数据，通过增量式聚类优化解决传统方法中重叠问题和静态聚类随时间退化的问题。


<details>
  <summary>Details</summary>
Motivation: 云服务提供商处理多服务查询时，传统聚类方法难以处理重叠关注点，创建宽泛、静态的聚类会随时间退化。重新聚类会破坏连续性，使问题跟踪变得困难。

Method: 提出自适应系统，将多轮聊天分割为特定服务关注点，并在新问题出现时增量式优化聚类。通过Davies-Bouldin指数和轮廓系数跟踪聚类质量，仅对退化的聚类应用基于LLM的分割方法。

Result: 相比基线方法，轮廓系数提高超过100%，DBI降低65.6%，实现了无需完全重新聚类的可扩展实时分析。

Conclusion: 该方法有效解决了传统聚类方法在处理多服务客户聊天数据时的局限性，实现了自适应、可扩展的实时分析系统。
...
TL;DR: 该研究扩展了刻板印象内容模型(SCM)在词嵌入去偏中的应用，从单一群体偏见到交叉性偏见，通过求和或拼接构建复合社会身份表示，并评估三种去偏策略在不同嵌入模型中的效果。
...
Method: 通过求和或拼接构建社会身份对的复合表示，应用三种去偏策略：减法、线性投影和部分投影。研究评估了三种广泛使用的嵌入模型(Word2Vec、GloVe和ConceptNet Numberbatch)，从两个互补角度衡量效用：局部邻域是否保持连贯性，以及类比行为是否得到保留。

Result: SCM基础的缓解方法在交叉性情况下效果良好，基本保持了整体语义结构。主要成本是熟悉的权衡：严格保持几何结构的方法在类比行为上更保守，而更积极的投影可以改善类比性能但牺牲邻域稳定性。部分投影最保守稳定，线性投影更积极，减法作为简单基线仍有竞争力。求和与拼接的选择取决于嵌入模型和应用目标。

Conclusion: 研究表明，在静态嵌入中使用SCM进行交叉性去偏是可行的，并为在平衡稳定性和类比性能时选择合适的聚合和去偏设置提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 传统置信传播算法在循环图上无法保证精确推理，junction tree算法复杂度高。需要建立统一的数学框架来理解置信传播的精确性条件，并开发更高效的精确推理算法。

Method: 1. 构建自由超图范畴Syn_Σ并证明其泛性质；2. 使用格罗滕迪克纤维化∫Msg→cat{FG}_Σ形式化消息传递；3. 将精确推理表征为有效下降：当重叠满足兼容条件时局部信念形成下降数据；4. 提出HATCC算法：通过因子神经上的和乐计算检测下降障碍，将非平凡和乐编译为模式变量，在增强图上简化为树BP。

Result: 1. 建立了置信传播的范畴论基础；2. HATCC算法复杂度为O(n²d_max + c·k_max·δ_max³ + n·δ_max²)；3. 在网格MRF和随机图上实现了精确推理，相比junction tree有显著加速；4. 在可满足性实例上实现了UNSAT检测。

Conclusion: 该工作为置信传播提供了统一的范畴论框架，将精确推理表征为有效下降问题，提出的HATCC算法能够检测和解决下降障碍，实现了高效的精确推理，统一了树精确性、junction tree算法和循环BP失败的情况。
...
TL;DR: 论文提出AI监管时代需要计算化合规算法，而非传统人工方法，并为此领域设计了算法目标框架和性能基准数据集。
...
Method: 提出计算化AI监管合规算法的设计目标框架，并开发了一个基准数据集来量化评估这些算法的性能表现。

Result: 为计算化AI监管合规研究领域提供了系统性的蓝图，包括算法设计目标和性能评估基准，为该新兴领域的研究提供了明确方向。

Conclusion: 计算化合规算法是实现AI监管合规的唯一现实途径，论文为该重要但尚未成形的领域提供了研究框架，旨在激发必要的投资和研究关注。
...
TL;DR: GUITester：一个多智能体框架，通过解耦导航和验证来解决MLLM在GUI探索性测试中的目标导向掩蔽和执行偏差归因问题，在GUITestBench基准上达到48.90%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 探索性GUI测试对软件质量至关重要但人工成本高。虽然多模态大语言模型（MLLM）智能体在导航方面表现出色，但由于两个核心挑战而无法自主发现缺陷：1）目标导向掩蔽（智能体优先完成任务而非报告异常）；2）执行偏差归因（系统缺陷被误认为是智能体错误）。

Method: 提出GUITester多智能体框架，包含两个模块：1）规划执行模块（PEM），通过嵌入测试意图主动探测缺陷；2）分层反射模块（HRM），通过交互历史分析解决归因模糊性。同时创建了GUITestBench基准，包含26种缺陷的143个任务。

Result: GUITester在GUITestBench上达到48.90%的F1分数（Pass@3），显著优于最先进基线方法的33.35%。

Conclusion: 这项工作证明了自主探索性测试的可行性，为未来的GUI质量保证提供了坚实基础。
...
Method: 采用三阶段半监督训练方案：1）自监督对比学习阶段，使用动态字典更新机制从未标注数据中提取鲁棒表示；2）监督训练阶段，在小规模标注数据上联合优化对比损失和交叉熵损失；3）主动学习阶段，基于不确定性和代表性标准从未标注数据中选择最有价值的样本进行标注

Result: 在ADS-B和WiFi数据集上的实验结果表明，该方法在有限标注条件下显著优于传统的监督和半监督方法，以更低的标注成本获得更高的识别准确率

Conclusion: 提出的基于主动学习的SEI方法能够有效解决标注数据稀缺问题，在通信安全领域具有重要应用价值，为有限标注预算下的发射源识别提供了有效解决方案
...
TL;DR: 该研究提出了一种结合伪标签和特征嵌入分布匹配的半监督对比学习方法，旨在提高图像分类准确性。


<details>
  <summary>Details</summary>
Motivation: 深度学习在监督图像分类方面取得了显著进展，但数据标注成本高昂。虽然对比学习等无监督方法被广泛研究，但现实场景中完全无标签的数据集很少见，而半监督学习在少量标签数据和大量无标签数据共存的情况下更具实际意义。现有的基于伪标签的半监督对比学习方法仍有改进空间。

Method: 提出了一种增强的基于伪标签的半监督学习方法，通过引入标签数据和无标签数据特征嵌入之间的分布匹配机制，来改进图像分类性能。该方法结合了伪标签技术和特征分布对齐策略。

Result: 该方法在多个数据集上提高了图像分类的准确性，通过分布匹配机制有效提升了基于伪标签的半监督学习性能。

Conclusion: 通过结合伪标签和特征嵌入分布匹配，可以显著提升半监督对比学习在图像分类任务中的性能，为解决现实场景中标签数据稀缺的问题提供了有效方案。
...
TL;DR: 本文分析了AI赋能人本关键系统在长期运行中的安全、可持续性和安全问题，提出个性化模型解决方案以消除现有测试方法的局限性。


<details>
  <summary>Details</summary>
Motivation: AI赋能的人本关键系统（如医疗监控、自动驾驶等）需要长期运行，但可能面临未测试的极端情况，导致安全、可持续性和安全要求被违反。现有测试方法存在局限性，需要新的解决方案。

Method: 首先分析现有AI赋能人本控制系统在安全、可持续性和安全分析方面的技术及其局限性，然后提出个性化模型解决方案来消除这些限制。

Result: 通过分析发现现有测试方法在长期实际使用中存在不足，个性化模型方法有望解决这些局限性。

Conclusion: AI赋能人本关键系统的长期运行需要新的测试方法，个性化模型解决方案是应对未测试极端情况和系统性能不确定性的有效途径。
...
TL;DR: BackdoorAgent框架系统分析LLM智能体工作流中的后门威胁，揭示单阶段植入的触发器能在多步骤中持续传播，威胁智能体安全。


<details>
  <summary>Details</summary>
Motivation: LLM智能体通过多步骤工作流执行任务，这种设计虽然实现了自主性，但也扩大了后门攻击面。现有研究分散且孤立分析单个攻击向量，缺乏从智能体角度理解后门触发器在跨阶段交互和传播的机制。

Method: 提出BackdoorAgent框架，将攻击面结构化为智能体工作流的三个功能阶段：规划攻击、记忆攻击和工具使用攻击。通过检测智能体执行过程，系统分析触发器在不同阶段的激活和传播。构建涵盖四种代表性智能体应用的标准化基准。

Result: 实验分析表明，单阶段植入的触发器能够在多个步骤中持续存在并通过中间状态传播。在使用GPT基座模型时，规划攻击中43.58%、记忆攻击中77.97%、工具阶段攻击中60.28%的案例观察到触发器持续性，凸显智能体工作流本身对后门威胁的脆弱性。

Conclusion: BackdoorAgent提供了统一的智能体中心视角来理解LLM智能体中的后门威胁，揭示了工作流内在的脆弱性，为未来研究和防御提供了系统分析框架和基准。
...
TL;DR: CIEA是一种新颖的多模态检索方法，通过互补信息提取和对齐技术，将文本和图像转换到统一的潜在空间，并设计互补信息提取器来识别和保留图像表示中的差异信息。


<details>
  <summary>Details</summary>
Motivation: 现有的大多数多模态检索研究主要关注捕捉多模态数据中与配对文本相似的信息，但往往忽略了多模态数据中包含的互补信息。本研究旨在解决这一局限性。

Method: 提出CIEA方法，采用互补信息提取和对齐技术，将文本和图像转换到统一的潜在空间，设计互补信息提取器来识别和保留图像表示中的差异信息，并使用两种互补对比损失进行优化。

Result: 大量实验证明CIEA的有效性，相比分治模型和通用密集检索模型都取得了显著改进。提供了消融研究、进一步讨论和案例研究来突出CIEA的进展。

Conclusion: CIEA通过有效提取和利用多模态数据中的互补信息，在多模态检索任务中取得了显著进步，为社区进一步研究提供了有价值的工具和代码资源。
...
TL;DR: 该论文系统综述了AI智能体与区块链的互操作性，提出了五类集成模式、威胁模型和比较分析框架，并规划了研究路线图，重点关注交易意图模式和策略决策记录两个接口抽象。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型使AI智能体能够推理、规划和执行多步骤工作流，而公共区块链已成为可编程的价值转移、访问控制和可验证状态转换平台。两者的融合带来了高风险系统挑战：需要设计标准、可互操作且安全的接口，使智能体能够观察链上状态、制定交易意图并授权执行，同时避免用户、协议或组织面临不可接受的安全、治理或经济风险。

Method: 通过系统性文献综述方法，从3000多份记录中筛选出317篇相关文献。构建了五部分分类法：只读分析、模拟与意图生成、委托执行、自主签名和多智能体工作流；开发了针对智能体驱动交易管道的威胁模型；创建了比较能力矩阵，分析20多个代表性系统在13个维度上的表现。

Result: 识别了当前研究空白，提出了以两个接口抽象为核心的研究路线图：用于便携且明确目标规范的交易意图模式，以及用于跨执行环境可审计、可验证策略执行的策略决策记录。还提出了可复现的评估套件和基准测试框架。

Conclusion: AI智能体与区块链的融合需要系统化的互操作性框架。论文通过分类法、威胁模型和比较分析为这一新兴领域提供了系统化理解，并提出了具体的研究方向和实践工具，以评估智能体介导的链上执行的安全性、可靠性和经济稳健性。
...
TL;DR: 该研究探讨人类认知与机器学习模型如何协同工作来识别钓鱼邮件，发现机器学习模型准确率高但置信度波动大，而人类评估者使用更多语言信号且置信度更一致。
...
Method: 使用三种可解释算法（逻辑回归、决策树、随机森林），在TF-IDF特征和语义嵌入上进行训练，然后将它们的预测与人类评估进行比较，人类评估捕获了置信度评分和语言观察。
...
TL;DR: AgentDevel：将LLM智能体改进重构为发布工程，通过外部化回归感知的发布管道实现稳定、可审计的改进，避免传统自改进机制的不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体的改进方法（如嵌入自改进机制或并发变体搜索）虽然能提升总体分数，但往往产生不稳定且难以审计的改进轨迹，难以保证非回归性或跨版本故障分析。
...
TL;DR: 提出对抗性推理RAG框架，通过推理器-验证器的对抗机制解决单视角推理局限和过程奖励不足问题


<details>
  <summary>Details</summary>
Motivation: 当前大推理模型与检索增强生成结合存在两个关键挑战：1) 推理模型通常从单一、无挑战的视角操作，限制了对外部文档进行深度自我修正推理的能力；2) 现有训练范式过度依赖结果导向的奖励，对复杂多步推理过程的信号不足
...
Conclusion: 提出的对抗性推理RAG框架通过推理器-验证器的对抗机制和过程感知奖励，有效解决了单视角推理局限和过程奖励不足的问题，提升了推理质量和鲁棒性
...
Method: 系统结合了逐步证明器（使用LLM提出证明命令并通过Isabelle验证）和高级证明规划器（生成结构化Isar大纲并填补修复剩余空白）。框架包括战术束搜索、战术重排ML和RL模型、小型Transformer模型的前提选择、基于AFP构建的Isar证明微RAG，以及反例引导的证明修复。

Result: Isabellm能够证明一些Isabelle标准自动化工具（包括Sledgehammer）无法处理的引理，展示了LLM引导证明搜索的实际价值。但同时也发现即使是GPT 5.2 Extended Thinking和Gemini 3 Pro等最先进的LLM，在实现复杂的填充修复机制时也存在困难。

Conclusion: LLM驱动的定理证明器在增强自动化证明方面具有实际价值，但当前最先进的LLM在复杂算法设计和可靠代码生成方面仍面临根本性挑战，需要进一步研究解决。
...
Method: 提出InstruCoT模型增强方法，合成多样化训练数据，采用指令级思维链微调，使LLM能有效识别和拒绝恶意指令

Result: 在四个LLM上的实验表明，InstruCoT在行为偏差、隐私泄露和有害输出三个关键维度上显著优于基线方法，同时保持实用性能不下降

Conclusion: InstruCoT通过指令级思维链微调和多样化数据合成，为LLM提供了有效的提示注入攻击防御方案
...
TL;DR: 本文提出了ResMAS框架，通过两阶段方法增强基于大语言模型的多智能体系统的韧性：1）训练奖励模型预测系统韧性，并用强化学习训练拓扑生成器自动设计韧性拓扑；2）引入拓扑感知的提示优化方法，根据智能体连接关系优化提示。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的多智能体系统在分布式环境中容易受到智能体故障等扰动影响。现有研究主要关注攻击后的被动检测和缓解，而非主动设计内在韧性系统。研究发现通信拓扑和提示设计显著影响系统韧性，因此需要主动增强系统韧性。

Method: 提出ResMAS两阶段框架：第一阶段训练奖励模型预测多智能体系统韧性，基于此通过强化学习训练拓扑生成器，为特定任务自动设计韧性拓扑；第二阶段引入拓扑感知的提示优化方法，根据每个智能体的连接关系和与其他智能体的交互优化其提示。

Result: 大量实验表明，该方法在各种约束条件下显著提高了多智能体系统的韧性。框架还展现出对新任务和新模型的强泛化能力，显示出构建韧性多智能体系统的潜力。

Conclusion: ResMAS框架通过主动设计韧性拓扑和拓扑感知的提示优化，有效增强了基于大语言模型的多智能体系统的韧性，为构建内在韧性的分布式智能体系统提供了新方法。
...
TL;DR: Tape是一个受控强化学习基准，专门用于隔离潜在规则变化下的分布外失败。它基于一维元胞自动机，通过固定观测和动作空间但改变转移规则来实现精确的训练/测试划分。研究发现，在分布内表现良好的方法在分布外规则下可能崩溃，且分布外评估的高方差需要足够重复实验才能获得稳定排名。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习方法在分布内表现良好，但在面对分布外规则变化时可能失败。需要建立一个受控基准来系统研究这种分布外失败，并开发标准化的评估协议来可靠地比较不同方法在规则变化下的鲁棒性。

Method: 基于一维元胞自动机构建Tape基准，保持观测和动作空间固定但改变转移规则，实现精确的训练/测试划分。使用可复现的评估流程比较三类方法：无模型基线方法、基于学习世界模型的规划方法、任务推断（元强化学习）方法。建立标准化分布外协议和统计报告要求。

Result: 研究发现一个一致模式：在分布内表现良好的方法在保留规则分布外测试中可能崩溃。分布外评估的高方差使得排名不稳定，除非实验有足够重复。论文提供了信息论恒等式，将熵减少与条件互信息和期望后验KL散度联系起来，阐明了"不确定性减少"目标在规则变化下的能力和限制。

Conclusion: Tape基准为研究强化学习在规则变化下的分布外失败提供了受控环境。需要标准化分布外协议、严格的统计报告（种子数、置信区间、假设检验）以及足够重复实验来获得可靠评估。信息论分析揭示了不确定性减少目标在规则变化下的理论局限性，为未来鲁棒强化学习方法的发展提供了指导。
...
TL;DR: TourPlanner是一个旅行规划框架，通过多路径推理和约束门控强化学习解决现有方法的三个主要挑战：POI候选集剪枝与召回率平衡、单一路径限制解决方案空间探索、硬约束与软约束同时优化。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划方法面临三个核心挑战：1) 在保持高召回率的同时有效剪枝兴趣点候选集；2) 单一路径推理限制了在可行解空间中的探索能力；3) 同时优化硬约束和软约束存在显著困难。

Method: 提出TourPlanner框架，包含三个关键技术：1) 个性化召回与空间优化工作流构建空间感知的POI候选集；2) 竞争共识思维链多路径推理范式增强可行解空间探索；3) 基于sigmoid的门控机制集成到强化学习中，动态优先满足硬约束后再优化软约束。
...
TL;DR: M-ASK是一个多智能体搜索框架，将搜索任务分解为搜索行为智能体和知识管理智能体两个角色，通过细粒度的回合级奖励实现稳定协调，在复杂信息检索任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的智能体搜索系统存在结构瓶颈：推理输出不受约束导致轨迹膨胀、稀疏的结果级奖励使信用分配复杂化、随机搜索噪声破坏学习稳定性。需要解决这些挑战以提升复杂信息检索的性能。

Method: 提出M-ASK框架，将智能体搜索明确分解为两个互补角色：搜索行为智能体（负责规划和执行搜索动作）和知识管理智能体（负责聚合、过滤和维护紧凑的内部上下文）。采用回合级奖励为搜索决策和知识更新提供细粒度监督。

Result: 在多跳问答基准测试中，M-ASK超越了强基线方法，不仅获得了更优的答案准确率，而且训练动态显著更加稳定。

Conclusion: 通过角色分解和细粒度奖励协调，M-ASK框架有效解决了传统单智能体搜索系统的结构瓶颈问题，为复杂信息检索任务提供了更稳定、更高效的解决方案。
...
TL;DR: 提出了一种多模态诊断框架，将时间序列数据与预训练语言模型的嵌入空间对齐，用于云基础设施的根因分析。
...
Method: 提出了一个多模态诊断框架，包含三个技术进展：1）语义压缩技术，将时间片段提炼为单令牌抽象同时保留模式语义；2）使用门控交叉注意力的对齐编码器，将时间序列特征投影到语言模型潜在空间；3）检索增强的诊断管道，将对齐嵌入与历史事件知识结合进行专家级故障归因。
...
TL;DR: ThinkDrive是一个用于自动驾驶的思维链引导渐进式强化学习微调框架，通过结合显式推理和难度感知自适应策略优化，在多个指标上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶中的大语言模型应用存在非结构化推理、泛化能力差以及与人类驾驶意图不对齐的问题。虽然思维链推理能提升决策透明度，但传统监督微调未能充分利用其潜力，而强化学习方法存在不稳定和推理深度不足的问题。

Method: 提出ThinkDrive框架，采用两阶段训练策略：首先使用思维链解释进行监督微调，然后应用渐进式强化学习，配合难度感知自适应策略优化器，根据样本复杂度动态调整学习强度。

Result: 在公开数据集上的评估显示，ThinkDrive在exam、easy-exam和accuracy指标上分别比强RL基线高出1.45%、1.95%和1.01%。使用该方法训练的20亿参数模型在exam指标上超越了更大的GPT-4o模型3.28%。

Conclusion: ThinkDrive通过结合思维链引导的推理和难度感知自适应强化学习，有效解决了自动驾驶领域大语言模型应用中的推理结构化和泛化问题，在保持较小模型规模的同时实现了超越更大模型的性能。
...
TL;DR: CompassMem：受事件分割理论启发的记忆框架，将记忆组织为事件图，通过逻辑关系链接事件，支持智能体进行结构化、目标导向的记忆导航和长时程推理。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体的记忆机制大多采用扁平化存储和简单的相似性检索，缺乏对经验间逻辑关系的显式捕捉，且记忆访问与结构脱节，限制了长时程依赖的推理能力。

Method: 提出CompassMem框架，基于事件分割理论将经验增量式分割为事件，并通过显式逻辑关系链接形成事件图，作为逻辑地图支持结构化、目标导向的记忆导航。

Result: 在LoCoMo和NarrativeQA数据集上的实验表明，CompassMem能够持续提升多种骨干模型的检索和推理性能。

Conclusion: CompassMem通过事件图结构显式捕捉经验间的逻辑关系，实现了超越表面检索的结构化记忆导航，有效支持智能体的长时程推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前无批评者RL方法在处理正同质提示（所有轨迹都正确）时效率极低，导致优势估计为零，浪费大量轨迹采样。

Method: 提出Miner方法：1）基于标记级别的焦点信用分配机制，动态放大关键不确定标记的梯度，抑制过度自信标记；2）自适应优势校准，无缝整合内在奖励和可验证奖励。

Result: 在六个推理基准上测试Qwen3-4B和Qwen3-8B基础模型，Miner在四种算法中达到最先进性能，相比GRPO在Pass@1上提升4.58%，在Pass@K上提升6.66%。

Conclusion: 潜在不确定性利用对于推理模型的高效可扩展RL训练既是必要的也是充分的，两个新提出的创新机制显示出优越性。
...
TL;DR: KnowMeBench是一个基于长篇自传叙事构建的记忆基准测试，通过重构时间锚定的叙事流来评估模型在事实回忆、主观状态归因和原则推理方面的能力，发现检索增强系统主要提升事实准确性，但在时间解释和高级推理上仍有局限。


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆基准测试多使用多轮对话或合成用户历史，使得检索性能不能完全代表人理解能力。需要基于真实自传叙事构建更贴近人类记忆理解的基准。

Method: 从长篇自传叙事中构建基准，将叙事重构为具有闪回意识、时间锚定的叙事流，通过证据链接的问题评估模型在事实回忆、主观状态归因和原则推理三个层面的能力。

Result: 检索增强系统在不同叙事来源中主要提升事实准确性，但在时间基础解释和高级推理任务上错误持续存在，表明需要超越检索的记忆机制。

Conclusion: 基于自传叙事的基准能更好评估人理解能力，当前检索方法在高级推理方面仍有局限，需要开发超越检索的记忆机制。
...
TL;DR: 论文探讨多智能体系统与单智能体技能库的权衡，发现技能选择存在类似人类认知的容量限制，呈现临界点式的性能下降，语义混淆是主要影响因素，分层组织可改善可扩展性。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然有效但计算开销大，能否通过单智能体选择技能库实现类似模块化优势？同时探索技能选择的可扩展性问题，特别是随着技能库增长时的性能变化规律。

Method: 将多智能体系统编译为等效的单智能体系统，将智能体间通信转换为技能选择。研究技能选择的扩展行为，分析语义混淆对性能的影响，并测试分层路由方法。

Result: 单智能体技能选择能显著减少token使用和延迟，同时保持推理基准的竞争力。技能选择性能在达到临界库大小前保持稳定，随后急剧下降，呈现类似人类认知的相变现象。语义相似性而非单纯库大小是性能下降的主要因素，分层路由能改善可扩展性。

Conclusion: 技能选择存在类似人类认知的容量限制，语义混淆是关键影响因素。分层组织有助于管理复杂选择，为设计可扩展的技能基智能体提供了认知基础框架和实践指导，并提出了LLM语义技能选择基本限制的新问题。
...
TL;DR: Orion-RAG提出了一种轻量级路径提取方法，将碎片化文档转化为半结构化数据，在知识检索增强生成中实现了显著性能提升。
...
Result: Orion-RAG在多个领域持续超越主流框架，支持实时更新和显式人机协同验证，具有高成本效益。在FinanceBench上实现了25.2%的相对精度提升。

Conclusion: 轻量级路径提取方法足以有效链接碎片化文档中的信息，为RAG系统在处理离散数据时提供了实用且高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 智能体强化学习作为LLM智能体的关键后训练范式，需要解决多轮任务中的探索多样性有限、稀疏信用分配和策略优化不对齐等核心挑战。

Method: AT²PO引入轮级树结构，包含熵引导树扩展用于战略探索和轮级信用分配用于稀疏结果的细粒度奖励传播，同时提出智能体轮级策略优化作为轮级学习目标。

Result: 在七个基准测试中，AT²PO相比最先进基线平均提升高达1.84个百分点，消融研究验证了每个组件的有效性。

Conclusion: AT²PO提供了一个统一的智能体强化学习框架，通过树搜索和轮级优化有效解决了多轮任务中的核心挑战，代码已开源。
...
Conclusion: APEX填补了学术海报编辑领域交互式智能体框架的空白，为复杂用户意图的对齐提供了有效解决方案。
...
TL;DR: TNT方法通过利用思考型响应的解决方案信息，为不同查询设置不同的非思考响应最大token限制，在减少50%token使用的同时显著提升准确性，实现准确性与效率的最佳权衡。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型依赖长链思维导致计算开销过大，现有基于RL的方法存在奖励欺骗问题，而SFT方法计算成本高，统一token限制方法效果有限，需要更好的解决方案。

Method: 提出Thinking-Based Non-Thinking (TNT)方法，不采用监督微调，而是利用思考型响应的解决方案组件信息，为不同查询设置差异化的非思考响应最大token使用限制。

Result: 在五个数学基准测试中，TNT相比DeepSeek-R1-Distill-Qwen-1.5B/7B和DeepScaleR-1.5B减少约50%的token使用，同时显著提高准确性，在所有测试方法中达到准确性与效率的最佳权衡，奖励欺骗概率保持在10%以下。

Conclusion: TNT方法有效解决了大型推理模型的过度思考问题，通过智能的token分配策略实现了计算效率与推理准确性的双重提升，为混合推理模型的发展提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 强化学习可以增强大语言模型的推理能力，但其效果依赖于随着模型进化仍保持信息量的训练信号。实践中，当任务难度与模型能力不匹配，或训练被狭窄的重复问题模式主导时，RL进展会放缓

Method: 提出SCALER框架：1）可扩展的合成管道，将真实编程问题转化为具有可控难度和无限实例生成的可验证推理环境；2）自适应多环境RL策略，动态调整实例难度并策划活跃环境集，以跟踪模型能力前沿并保持分布多样性

Result: 大量实验表明，SCALER在不同推理基准测试中始终优于基于数据集的RL基线，并展现出更稳定、更长视野的训练动态

Conclusion: SCALER通过环境与模型共同适应的方式，防止奖励稀疏性，减轻对狭窄任务模式的过拟合，支持训练过程中的持续改进，为解决RL训练中的关键挑战提供了有效框架
...
<details>
  <summary>Details</summary>
Motivation: 评估现代多模态和视觉语言模型是否能够可靠地解释建筑、工程和施工（AEC）图纸中的几何和语义信息，这些图纸通过符号、布局约定和密集标注编码信息
...
Conclusion: 当前系统作为文档助手表现良好，但缺乏强大的图纸理解能力，需要领域特定表示和工具增强、人机协作的工作流程来实现高效的AEC自动化
...
TL;DR: DR-LoRA：一种动态调整MoE模型中专家LoRA秩的框架，根据任务需求智能分配参数，提升参数效率
...
Method: 提出DR-LoRA框架，在微调过程中基于任务特定需求动态增长专家LoRA秩。采用专家显著性评分机制，结合专家路由频率和LoRA秩重要性来量化每个专家对额外容量的需求，高显著性专家优先获得秩扩展，自动形成针对目标任务的异构秩分布
...
<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统通常在所有智能体角色中统一使用大型语言模型，未能考虑不同推理阶段的认知需求差异，导致显著的计算效率低下问题。

Method: 提出OI-MAS框架，采用自适应模型选择策略，基于异构多尺度LLM池。引入状态依赖的路由机制动态选择智能体角色和模型规模，以及置信度感知机制根据任务复杂度选择适当的模型规模。

Result: 实验结果显示OI-MAS持续优于基线多智能体系统，准确率提升最高达12.88%，同时成本降低最高达79.78%。

Conclusion: OI-MAS框架通过自适应模型选择有效解决了多智能体系统的计算效率问题，在保持高性能的同时显著降低了成本。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于提示的持续学习方法通常依赖键值对机制，这会引入任务间干扰并限制方法的可扩展性，需要一种更有效的解决方案

Method: 提出任务特定提示-原型（ProP）方法：任务特定提示促进当前任务的特征学习，对应原型捕捉输入的代表性特征；推理时通过绑定任务特定提示与对应原型生成预测；在提示初始化时引入正则化约束惩罚过大值以增强稳定性

Result: 在多个广泛使用的数据集上的实验证明了该方法的有效性，相比主流基于提示的方法，消除了对键值对的依赖

Conclusion: 提出的ProP框架为持续学习研究提供了新的视角，通过消除键值对依赖解决了传统方法的局限性，展示了在减少任务间干扰和提升可扩展性方面的优势
...
TL;DR: 该研究提出了一种基于超图的知识表示方法，用于科学发现中的高阶关系建模，避免了传统知识图谱中成对约束的局限性，能够捕捉多实体间的复杂交互关系。


<details>
  <summary>Details</summary>
Motivation: 科学探究需要系统级推理，整合异构实验数据、跨领域知识和机制证据。虽然大语言模型具有推理能力，但依赖缺乏结构深度的检索增强上下文；传统知识图谱试图弥合这一差距，但其成对约束无法捕捉控制涌现物理行为的不可约高阶交互。

Method: 引入构建超图知识表示的方法论，能够忠实编码多实体关系。应用于约1,100篇关于生物复合支架的手稿语料库，构建了包含161,172个节点和320,201条超边的全局超图，显示出无标度拓扑结构（幂律指数约1.23）。为智能体系统配备超图遍历工具，特别是使用节点交集约束。

Result: 框架构建的全局超图揭示了围绕高度连接的概念枢纽组织的无标度拓扑结构。这种表示避免了成对扩展中典型的组合爆炸，并明确保留了科学表述的共现上下文。配备超图遍历工具的智能体系统能够桥接语义上遥远的概念，通过利用高阶路径成功为新型复合材料生成基于机制的假设。

Conclusion: 这项工作建立了一个"无教师"的智能体推理系统，其中超图拓扑结构作为可验证的防护栏，通过揭示传统图方法所掩盖的关系来加速科学发现。该方法特别展示了将氧化铈与PCL支架通过壳聚糖中间体连接的能力。
...
TL;DR: 本文提出了一种结合着色时间Petri网和基于模型的强化学习的新方法，用于解决柔性制造系统中同时包含AGV和工具共享的复杂调度问题，在大型实例上优于传统方法且计算时间减少10倍。


<details>
  <summary>Details</summary>
Motivation: 柔性制造系统在现代制造业中至关重要，传统作业车间调度问题需要扩展以包含自动导引车和工具共享系统的复杂性，现有方法难以有效处理这些多方面的挑战。

Method: 提出结合着色时间Petri网和基于模型的强化学习的新方法：CTPN提供形式化建模结构和动态动作屏蔽，显著减少动作搜索空间；MBRL通过学习策略确保对变化环境的适应性；利用MBRL优势加入前瞻策略优化AGV定位。

Result: 在小型公共基准和新开发的大型基准（基于Taillard基准）上评估，结果显示：在小型实例上与传统方法相当，在大型实例上在makespan方面优于传统方法，同时计算时间减少10倍；消融研究评估了各组件对整体性能的贡献。

Conclusion: 该方法有效解决了柔性制造系统中同时包含AGV和工具共享的复杂调度问题，在大型实例上表现出优越性能且计算效率显著提高，为相关研究提供了可复现的环境和实例生成器。
...
Method: 提出DVD（通过生成分布方差检测）方法，基于温度采样建模局部输出分布。核心洞察是：受污染项目在记忆遵守状态和扰动漂移状态间交替，导致低概率令牌的合成难度方差异常高；未受污染项目保持漂移状态且方差相对平滑

Result: DVD在两个领域（Omni-MATH和SuperGPQA）的基准测试中，在不同规模和架构模型（Qwen2.5和Llama3.1）上，均优于基于困惑度、Min-k%++、编辑距离和嵌入相似性的基线方法，且对超参数具有强鲁棒性

Conclusion: 生成分布方差可作为检测大语言模型评估中变体污染的原则性实用指纹，DVD方法为评估污染检测提供了有效解决方案
...
TL;DR: 行为规划是一种新型的多样化规划方法，通过将多样性模型显式纳入规划过程并支持多种规划类别，在风险管理和自动化数据分析等实际领域中生成多样化的计划。


<details>
  <summary>Details</summary>
Motivation: 多样化规划方法旨在生成彼此不同的计划集合，在风险管理、自动化流数据分析和恶意软件检测等实际领域有广泛应用。传统方法需要进一步扩展以更好地适应复杂现实场景。

Method: 提出行为规划这一新型多样化规划范式，通过显式地将多样性模型纳入规划过程，并支持多种规划类别，扩展了早期方法的能力。

Result: 通过三个实际案例研究验证了行为规划的有效性：1) 故事生成，2) 城市规划，3) 游戏评估，展示了该方法在不同领域的实际应用价值。

Conclusion: 行为规划作为一种新型多样化规划方法，在实际应用中表现出显著价值，能够有效支持不同领域的多样化规划需求，为复杂现实问题提供创新解决方案。
...
TL;DR: 研究探索了将对话式生成AI助手整合到学习分析仪表板中，发现不同自我调节学习能力的学生提问模式不同，AI助手能提供清晰解释但个性化有限，对低SRL学生特别有价值。


<details>
  <summary>Details</summary>
Motivation: 学习分析仪表板旨在通过数据反馈支持学生学习调节，但学生（特别是自我调节学习能力较低者）常难以理解和参与分析反馈。对话式生成AI助手通过实时个性化对话支持显示出潜力。

Method: 在10周学期中，将生成AI助手整合到学习分析仪表板，分析学生与AI助手的真实对话。研究聚焦不同SRL水平学生的提问模式、助手回答的相关性和质量，以及学生对助手在学习中作用的感知。

Result: 发现明显的提问模式差异：低SRL学生寻求澄清和确认，高SRL学生询问技术细节和个性化策略。助手能提供清晰可靠解释，但在个性化、处理情感化查询和整合多数据点进行定制响应方面有限。生成AI干预对低SRL学生特别有价值，能缩小与高SRL同伴的差距。

Conclusion: 对话式生成AI助手在学习分析仪表板中具有潜力，尤其对低SRL学生提供支持。未来系统需要改进信任建立、适应性、情境感知和技术完善，以更好地支持个性化学习调节。
...
TL;DR: 本文通过ESA ELOPE竞赛案例研究，展示了ChatGPT在科学发现中作为编码伙伴的潜力，帮助团队在短时间内获得第二名成绩，同时分析了AI在科学快速原型开发中的优势和局限。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地被用作编码伙伴，但它们在加速科学发现中的作用尚未得到充分探索。本文旨在探索人类与AI协作在竞争性科学环境中的潜力。

Method: 采用案例研究方法，分析在ESA的ELOPE竞赛中使用ChatGPT进行快速原型开发的经验。ChatGPT不仅提供可执行代码，还贡献算法推理、数据处理例程和方法论建议。

Result: 尽管加入竞赛较晚，团队仍获得第二名（得分0.01282），展示了人类-AI协作在竞争性科学环境中的有效性。ChatGPT在代码生成、算法推理和方法论建议方面表现出色。

Conclusion: 对话式AI既能加速科学开发，又能支持概念洞察，但存在引入不必要结构变化、混淆中间讨论、产生关键错误等局限。需要将LLMs结构化整合到科学工作流程中，并制定AI辅助科学工作的最佳实践。
...
TL;DR: T-Retriever：一种基于语义结构编码树的图检索增强生成框架，通过自适应压缩编码和语义结构熵优化，解决了现有图RAG方法在层次信息管理中的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基于图的RAG方法在处理层次信息时存在两个关键限制：1）采用僵化的分层压缩配额会破坏局部图结构；2）过度关注拓扑结构而忽视语义内容。需要一种能同时考虑语义和结构信息的层次化检索方法。
...
Conclusion: T-Retriever通过语义结构编码树和自适应压缩策略，有效解决了图RAG中的层次信息管理问题，实现了更好的检索增强生成性能。
...
TL;DR: ConMax是一个强化学习框架，通过自动压缩推理轨迹来减少大型推理模型的"过度思考"问题，在保持准确性的同时显著降低计算成本。
...
Method: ConMax是一个置信度最大化压缩框架，将压缩建模为奖励驱动的优化问题。训练一个策略来修剪冗余，通过最大化加权组合：1)答案置信度（预测保真度）和2)思维置信度（推理有效性），使用冻结的辅助LRM进行评估。
...
TL;DR: AlgBench是一个由专家策划的算法推理基准测试，包含3000多个原创问题，涵盖27种算法，评估大型推理模型在算法中心范式下的表现，发现模型在非优化任务上表现良好（92%），但在动态规划等全局优化算法上准确率骤降至49%，揭示了战略过移现象和问题中心强化学习的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有算法推理基准测试有限，无法回答大型推理模型是否真正掌握算法推理能力这一关键问题。需要建立一个专家策划的算法中心基准来评估模型在算法推理方面的真实能力。

Method: 提出AlgBench基准测试，由ACM算法专家构建，包含3000多个原创问题，涵盖27种算法，采用全面的分类法组织，包括欧几里得结构、非欧几里得结构、非优化、局部优化、全局优化和启发式优化等类别。对领先的大型推理模型（如Gemini-3-Pro、DeepSeek-v3.2-Speciale和GPT-o3）进行实证评估。

Result: 模型表现存在显著异质性：在非优化任务上准确率高达92%，但在动态规划等全局优化算法上准确率骤降至49%左右。分析发现存在"战略过移"现象，即模型由于必要的低熵标记而过早放弃正确的算法设计。
...
TL;DR: LLMs在处理表格数据时缺乏检测和纠正语义与结构扭曲的内在能力，即使有明确提示也只能部分调整推理策略，准确率下降至少22%
...
Method: 引入一个专家策划的小型数据集，专门评估LLMs在表格问答任务中需要先进行错误纠正步骤的能力，研究模型在不同扭曲条件下的表现

Result: LLMs缺乏检测和纠正表格表示中微妙扭曲的内在能力；只有在提供明确系统提示时，模型才能部分调整推理策略并纠正一些扭曲，但效果不一致也不完整；即使是GPT-5.2等最先进模型，在扭曲条件下的准确率也至少下降22%

Conclusion: LLMs在摄入和解释扭曲表格信息方面存在系统性差异，这引发了关于模型何时以及如何应自主决定重新对齐表格输入的重要研究问题，类似于人类行为而不依赖明确提示或表格数据预处理
...
TL;DR: OptiSet是一个用于检索增强生成(RAG)的集合中心框架，通过"扩展-精炼"范式统一集合选择和集合级排序，解决传统top-k方法无法利用组合增益和引入冗余的问题。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法主要基于静态选择top-k相关段落，这种方法无法利用段落间的组合增益，且经常引入大量冗余信息，限制了生成质量和效率。

Method: 1) 采用"扩展-精炼"范式：先扩展查询为多视角以获得多样候选池，再通过重选形成紧凑证据集；2) 设计无强LLM监督的自合成策略，从生成器的集合条件效用变化中推导偏好标签；3) 引入集合列表式训练策略，联合优化集合选择和集合级排序。

Result: 大量实验表明，OptiSet在复杂组合问题上提升了性能，并使生成更加高效。源代码已公开。

Conclusion: OptiSet通过集合中心框架有效解决了传统RAG方法的局限性，能够识别互补和冗余证据，优化证据集的选择和排序，从而提高生成质量和效率。
...
### [202] [Learning Latent Action World Models In The Wild](https://arxiv.org/abs/2601.05230)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的智能体需要预测其动作的后果，而世界模型通常需要动作标签，这在规模上难以获取。因此需要从视频中学习潜在动作模型，但现有研究主要关注简单的机器人模拟、视频游戏或操作数据，缺乏对野外视频的处理能力。

Method: 提出了学习潜在动作世界模型的方法，讨论了动作应遵循的属性、相关架构选择和评估方法。研究发现连续但受约束的潜在动作能够捕捉野外视频中动作的复杂性，而常见的向量量化方法则不能。还训练了一个控制器，将已知动作映射到潜在动作。

Result: 连续受约束的潜在动作能够捕捉野外视频中动作的复杂性，并能跨视频转移环境变化（如人类进入房间）。在缺乏跨视频共同体现的情况下，主要学习到相对于相机空间定位的潜在动作。控制器能够将已知动作映射到潜在动作，使潜在动作作为通用接口，在规划任务中达到与动作条件基线相似的性能。

Conclusion: 该研究为将潜在动作模型扩展到现实世界迈出了一步，展示了从野外视频中学习特定动作的能力，并提供了分析和实验支持。
...
### [203] [How to Set the Batch Size for Large-Scale Pre-training?](https://arxiv.org/abs/2601.05034)
*Yunhua Zhou,Junhao Huang,Shuhao Xin,Yechen Zhang,Runyu Peng,Qiping Guo,Xipeng Qiu*

Main category: cs.AI

TL;DR: 本文针对Warmup-Stable-Decay学习率调度器，修正了传统临界批大小理论，提出了适用于WSD调度器的E(S)关系，并基于此设计了动态批大小调度策略。


<details>
  <summary>Details</summary>
Motivation: 随着Warmup-Stable-Decay学习率调度器成为主流，OpenAI提出的传统临界批大小理论框架已无法适应新的预训练动态，需要建立理论与实践的桥梁。

Method: 推导了适用于WSD调度器的修正E(S)关系，分析了WSD预训练的两个基本属性：最小批大小阈值B_min和最优批大小B_opt，并基于此提出了动态批大小调度器。

Result: 实验证明修正公式能精确捕捉大规模预训练动态，提出的调度策略显著提升了训练效率和最终模型质量。

Conclusion: 本文为WSD调度器下的预训练提供了新的理论框架和实用调度策略，解决了传统临界批大小理论与现代预训练实践之间的脱节问题。
...
Method: 1. 提出两种研究范式：Fitting范式（通过预测建模引入搜索因子的缩放定律，将搜索复杂度从O(n³)降低到O(n*C_D*C_η)）和Transfer范式（将μTransfer原理扩展到MoE架构，涵盖模型深度、权重衰减和token范围）。2. 在更大规模上对两种范式进行综合比较。

Result: 实证结果表明，广泛采用的μTransfer在大规模预训练场景下的可扩展性受到挑战。通过训练稳定性和特征学习的双重视角分析，揭示了模块级参数调优在大规模设置中表现不佳的根本原因。

Conclusion: 这项工作为工业级预训练优化提供了系统的实践指南和新的理论视角，挑战了现有超参数研究的边界，并为大规模深度学习训练提供了重要见解。
...
TL;DR: GPT-4o在去除安全防护后，既能有效反驳阴谋论，也能同样有效地推广阴谋论，甚至推广阴谋论时获得更积极的评价。标准版GPT-4o也存在类似问题，但通过纠正对话和使用准确信息提示可以缓解风险。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大型语言模型的说服力是否更倾向于支持真相而非虚假信息，以及LLMs是否同样容易推广错误信念。虽然LLMs在各种情境中显示出说服力，但尚不清楚这种说服力是否更有利于真相而非虚假信息。

Method: 通过三个预注册实验，让2,724名美国参与者与GPT-4o讨论他们不确定的阴谋论。模型被指示要么反驳（"揭穿"）要么支持（"推广"）该阴谋论。使用去除防护的"越狱"版GPT-4o和标准版GPT-4o进行比较。

Result: 去除防护的GPT-4o在增加阴谋论信念方面与减少信念同样有效。令人担忧的是，推广阴谋论的AI获得更积极的评价，并增加了对AI的信任。标准版GPT-4o也产生类似效果，OpenAI的防护措施未能有效阻止LLM推广阴谋论。但纠正对话可以逆转新诱导的信念，提示GPT-4o仅使用准确信息能显著降低其增加阴谋论信念的能力。

Conclusion: LLMs既具备推广真相也具备推广虚假信息的强大能力，但存在潜在的解决方案来缓解这种风险。需要开发有效的防护措施来确保AI系统更倾向于支持真相而非虚假信息。
...
TL;DR: 将材料科学综述从静态PDF转换为FAIR、机器可操作的ORKG结构化知识，对比符号查询与LLM查询，主张符号层应作为神经符号AI的可靠基础
...
Method: 以原子层沉积/刻蚀（ALD/E）为案例研究，将综述表格发布为ORKG中的FAIR、机器可操作的比较，建立结构化知识库，并对比符号查询与基于大语言模型的查询方法

Result: 成功将ALD/E综述转化为ORKG中的结构化知识，实现了机器可操作的比较，验证了符号查询的可靠性，并展示了LLM作为符号接地接口的补充作用

Conclusion: 在材料科学中，精心策划的符号层应作为可靠神经符号AI的骨干，LLM应作为补充性的、符号接地的接口，而非独立的真理来源
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于MCTS的RLVR方法存在探索多样性有限和推理效率低下的问题，需要改进以增强语言模型的推理能力

Method: 提出ROSE方法：1）基于语义熵的分支策略，选择语义分歧大的分支点生成新推理路径；2）ε-探索机制，从根节点随机启动推理rollout防止搜索过于局部；3）长度感知的段级优势估计器，奖励简洁正确的推理，惩罚过长推理链

Result: 在多个数学推理基准测试中使用Qwen和Llama模型进行广泛实验，验证了ROSE的有效性和效率

Conclusion: ROSE通过语义多样性探索和高效推理机制，成功解决了现有RLVR方法的局限性，提高了语言模型的推理性能
...
TL;DR: 大型推理模型通过显式思维链提高性能，但中间推理会泄露个人身份信息。研究提出隐私优先推理方法，通过提示控制或微调减少PII泄露，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成显式思维链提高了性能、可靠性和可解释性，但这种透明性带来了严重的隐私风险：即使最终答案经过清理，中间推理过程也经常泄露个人身份信息。

Method: 1. 引入PII-CoT-Bench数据集，包含隐私感知的思维链标注；2. 创建类别平衡的评估基准，涵盖现实和对抗性泄露场景；3. 采用两种可部署干预措施：基于提示的控制和模型微调。

Result: 研究发现能力依赖趋势：最先进模型从基于提示的控制中获益最大，而较弱模型需要微调才能实现有意义的泄露减少。两种方法都能显著减少PII暴露，同时保持最小程度的性能下降。

Conclusion: 隐私优先的思维链推理可以在不牺牲性能的情况下实现，为构建隐私保护推理系统提供了实用指导。研究表明隐私推理与实用性可以共存。
...
TL;DR: 本文介绍了首个专门评估阿拉伯语LLM工具调用和智能体能力的基准，揭示了阿拉伯语交互下工具调用准确率平均下降5-10%的性能差距。


<details>
  <summary>Details</summary>
Motivation: 随着阿拉伯语原生LLM快速发展，现有评估基准主要针对英语，缺乏对阿拉伯语工具调用能力的系统评估。由于这些模型大多基于英语数据预训练，其在非英语语言（特别是阿拉伯语）的工具调用性能尚不明确，存在关键的研究空白。

Method: 开发了首个专门针对阿拉伯语语言模型的工具调用和智能体能力评估基准，提供标准化框架来衡量阿拉伯语智能体工作流中的功能准确性和鲁棒性。

Result: 研究发现显著的性能差距：当用户使用阿拉伯语交互时，工具调用准确率平均下降5-10%，无论工具描述本身是阿拉伯语还是英语。

Conclusion: 该基准揭示了阿拉伯语工具调用中的关键挑战，旨在促进为阿拉伯语用户开发更可靠和语言公平的AI智能体，填补了现有评估体系的空白。
...
TL;DR: FusionRoute是一个轻量级的路由框架，通过token级别的多LLM协作，在解码时选择最合适的专家模型并添加互补logit来优化输出，解决了通用大模型成本高与专用小模型泛化差的两难问题。


<details>
  <summary>Details</summary>
Motivation: 解决通用大语言模型训练部署成本过高与专用小模型泛化能力有限之间的矛盾。通用模型需要大规模参数才能跨领域表现良好，而专用模型虽然高效但难以超出其训练分布。

Method: 提出FusionRoute框架：1）轻量级路由器在每个解码步骤选择最合适的专家模型；2）路由器同时贡献互补logit，通过logit加法来修正或完善所选专家的下一个token分布。该方法扩展了策略类别，能够在温和条件下恢复最优价值函数。

Result: 在Llama-3和Gemma-2系列模型以及数学推理、代码生成、指令遵循等多样化基准测试中，FusionRoute优于序列级和token级协作、模型融合和直接微调方法，同时在各自任务上与领域专家模型保持竞争力。

Conclusion: FusionRoute通过token级别的多LLM协作框架，有效平衡了模型效率与性能，解决了通用大模型成本过高和专用小模型泛化不足的问题，为实际部署提供了可行的解决方案。
...
TL;DR: GlimpRouter：基于首个token熵值预测推理步骤难度，实现大模型与小模型协同推理的轻量级路由框架，显著降低推理延迟同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型虽然通过显式生成多步思维链获得优异性能，但带来了显著的推理延迟和计算成本。现有协作推理方法要么依赖局部token概率，要么需要事后验证，都引入了显著的推理开销。需要一种更高效的方法来判断推理步骤何时需要大模型能力或小模型效率。

Method: 提出GlimpRouter框架，基于"顿悟时刻"现象发现推理步骤的难度可以从其第一个token的熵值推断。使用轻量级模型仅生成每个推理步骤的第一个token，当初始token熵值超过阈值时，将该步骤路由到大模型处理，否则由小模型完成。

Result: 在多个基准测试中，GlimpRouter显著降低了推理延迟同时保持了准确性。例如在AIME25上，相比独立大模型实现了10.7%的准确率提升，同时减少了25.9%的推理延迟。
...
TL;DR: 研究发现LLM作为评估者存在可靠性悖论：虽然单个评估者自身评分一致，但不同评估者之间几乎无法达成共识（Krippendorff's α = 0.042），甚至比随机噪声更差。然而这种分歧并非随机，而是具有稳定模式，可以像指纹一样识别不同评估者。


<details>
  <summary>Details</summary>
Motivation: 研究动机是检验LLM-as-judge系统的可靠性和一致性。虽然这类系统承诺可扩展、一致的评估，但研究者怀疑不同LLM评估者之间可能存在系统性分歧，需要验证它们是否真正测量相同的质量概念。

Method: 研究方法包括：1）进行大规模评估实验（3,240次评估，9个评估者×120个独特视频×3次独立运行）；2）使用Krippendorff's α测量评估者间一致性；3）训练分类器从评分模式识别评估者身份；4）从多个维度表征评估者的"评估倾向"：严厉程度、维度侧重、内部一致性、证据行为等。

Result: 结果显示：1）评估者间一致性接近零（α = 0.042），在某些维度甚至低于随机水平；2）分类器仅凭评分就能以77.1%准确率识别评估者，加入倾向特征后达89.9%；3）同一模型家族内区分度更高（GPT-4.1和GPT-5.2可99.6%区分）；4）每个评估者都有独特、稳定的质量理论。

Conclusion: 结论是LLM评估者不是可互换的测量工具，而是各自编码了独特质量理论的独立测量设备。平均它们的评分会产生不反映任何评估者真实价值观的合成判断。这提出了LLM评估系统的根本可靠性问题。
...
TL;DR: ReasonMark是一个专门为推理密集型大语言模型设计的水印框架，通过将生成过程解耦为不受干扰的思考阶段和加水印的回答阶段，使用关键性评分识别语义关键标记，并通过语义自适应机制调节水印强度，在保持逻辑完整性的同时实现高效水印嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有的数字水印方法在处理推理大语言模型时存在挑战：基于标记的水印技术会破坏推理流程的逻辑连贯性，而语义感知方法虽然提高了质量但引入了显著延迟或需要辅助模型。需要一种既能保护逻辑完整性又能高效运行的水印解决方案。

Method: 1. 将生成过程解耦为两个阶段：不受干扰的思考阶段和加水印的回答阶段；2. 提出关键性评分来从推理轨迹中识别语义关键标记；3. 将这些标记提炼成主要语义向量；4. 使用语义自适应机制，根据标记与PSV的对齐程度调节水印强度。

Result: ReasonMark在多项指标上超越现有方法：文本困惑度降低0.35，翻译BLEU分数提高0.164，数学准确性提高0.67分。同时水印检测AUC提高0.34%，对攻击具有更强鲁棒性，延迟增加可忽略不计。
...
TL;DR: 该研究提出"自我消耗性能循环"概念，探讨在动态迭代训练过程中合成数据如何影响偏见形成，发现性能循环会增加偏好偏见但减少差异偏见，并提出基于奖励的拒绝采样策略来缓解偏见。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展导致使用合成数据训练未来模型的兴趣增长，但这会创建自我消耗的再训练循环，可能导致性能下降和偏见出现。现实应用中，已部署的LLM可能影响其生成的数据，形成由用户反馈驱动的动态系统。由于无法获取动态生产系统中的真实用户偏好数据，需要建立受控环境来隔离和分析反馈驱动的偏见演化。

Method: 引入自我消耗性能循环概念，在受控性能反馈下研究动态迭代训练过程中合成数据如何塑造偏见。研究两种循环：典型的再训练设置和增量微调设置。通过三个现实世界任务的实验，分析偏见演化，并设计基于奖励的拒绝采样策略来缓解偏见。
...
### [215] [MineNPC-Task: Task Suite for Memory-Aware Minecraft Agents](https://arxiv.org/abs/2601.05215)
*Tamil Sudaravan Mohan Doss,Michael Xu,Sudha Rao,Andrew D. Wilson,Balasaravanan Thoravi Kumaravel*

Main category: cs.AI

TL;DR: 提出了MineNPC-Task基准测试框架，用于在开放世界Minecraft中评估具有记忆能力的混合主动LLM智能体，通过真实玩家合作设计任务模板和验证器，避免合成提示的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体评估通常依赖合成提示，缺乏真实世界交互的复杂性。需要建立一个基于真实玩家体验、具有明确依赖结构和可验证性的基准测试框架，以评估记忆感知的混合主动智能体在开放世界环境中的表现。

Method: 通过专家玩家的形成性和总结性合作游戏收集任务，将其规范化为具有明确前提条件和依赖结构的参数化模板，并配备机器可检查的验证器。采用有限知识策略禁止世界外捷径。框架捕获计划/行动/记忆事件，包括计划预览、针对性澄清、内存读写、前提条件检查和修复尝试。

Result: 使用GPT-4o实例化框架，评估了8位经验玩家的216个子任务。观察到代码执行、库存/工具处理、引用和导航方面的重复性故障模式，同时发现混合主动澄清和轻量级内存支持下的恢复能力。参与者对交互质量和界面可用性评价积极，但强调需要更强的跨任务记忆持久性。

Conclusion: MineNPC-Task提供了一个透明、可复现的评估框架，支持未来记忆感知具身智能体的评估。通过真实玩家合作设计的任务模板和验证器，避免了合成提示的局限性，为智能体在开放世界环境中的记忆和混合主动能力评估提供了有效工具。
