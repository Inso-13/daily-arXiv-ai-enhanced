...
TL;DR: 提出一个两步生成式数据增强框架，结合基于规则的口罩变形和无配对图像转换，用于生成逼真的戴口罩人脸样本，以解决数据稀缺和分布偏移问题。
...
Method: 两步框架：1）基于规则的口罩变形；2）使用GAN进行无配对图像到图像转换。引入非口罩保护损失和随机噪声注入来稳定训练并增强样本多样性。

Result: 相比单独使用基于规则的变形方法，该框架在质量上取得一致改进，并补充了现有GAN方法如IAMGAN。实验验证了所提组件的有效性。
...
TL;DR: JARVIS是一个基于JEPA的自监督视觉增强框架，用于提升多模态大语言模型的视觉推理能力，通过整合I-JEPA学习范式来学习图像的结构和语义规律，而不完全依赖语言监督。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基础视觉推理任务上表现有限，主要原因是它们主要从文本描述中学习视觉理解，而文本描述是主观且不完整的监督信号。此外，多模态指令调优规模相对较小，导致模型过度依赖语言先验而忽视视觉细节。

Method: 提出JARVIS框架，将I-JEPA学习范式整合到MLLMs的标准视觉-语言对齐流程中。使用冻结的视觉基础模型作为上下文和目标编码器，训练预测器（实现为LLM的早期层）从图像中学习结构和语义规律，不依赖语言监督。

Result: 在标准MLLM基准测试上的广泛实验表明，JARVIS在不同LLM家族中都能持续提升以视觉为中心的基准性能，同时不会降低多模态推理能力。

Conclusion: JARVIS框架通过自监督视觉增强有效解决了MLLMs在视觉推理方面的局限性，为提升多模态模型的视觉理解能力提供了一种有前景的方法。
...
TL;DR: 提出了稀疏接地视觉导航任务和CityNav基准，用于评估多模态大语言模型在真实城市环境中的顺序决策能力，并提出了Verbalization of Path方法提升导航成功率。
...
Method: 提出了稀疏接地视觉导航任务，并创建了CityNav基准，包含四个全球城市的导航场景。提出了Verbalization of Path方法，通过显式构建认知地图（关键地标和方向）来增强模型推理能力。

Result: 实验表明当前最先进的多模态大语言模型和标准推理技术在该挑战性设置下表现显著不足，而提出的Verbalization of Path方法能大幅提升导航成功率。

Conclusion: 稀疏接地视觉导航任务揭示了多模态大语言模型在真实世界导航中的局限性，提出的Verbalization of Path方法通过显式构建认知地图有效提升了导航性能，为开发更强大的具身智能体提供了重要方向。
...
TL;DR: R4是一个免训练框架，通过构建4D时空知识库增强视觉语言模型，实现基于检索的时空推理


<details>
  <summary>Details</summary>
Motivation: 受人类在四维时空构建结构化内部表征能力的启发，旨在为视觉语言模型提供结构化、终身记忆，以支持动态环境中的推理

Method: 构建4D知识数据库，将对象级语义描述锚定在度量空间和时间中，通过自然语言查询分解为语义、空间和时间键进行检索，集成到VLM推理中

Result: 在具身问答和导航基准测试中，R4在时空信息检索和推理方面显著优于基线方法

Conclusion: R4为动态环境中的具身4D推理提供了新范式，实现了无需训练的协作推理
...
TL;DR: 该论文提出了"感知观测台"框架，用于评估多模态大语言模型的感知能力，超越传统准确率指标，关注模型在扰动下的鲁棒性、归因保真度和推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然能力不断增强，但其感知能力缺乏系统评估。大多数模型家族在扩展语言组件时重用相同的视觉编码器，引发了对模型是否真正具备视觉基础能力还是仅依赖文本知识的担忧。现有评估方法过于强调最终任务准确率，忽视了鲁棒性、归因保真度和受控扰动下的推理能力。

Method: 提出了"感知观测台"框架，从多个垂直维度评估MLLMs：1)简单视觉任务（如人脸匹配和文本视觉理解）；2)局部到全局理解（如图像匹配、网格指向游戏和属性定位）。每个维度都使用真实数据集（人脸和文字），并通过像素级增强和基于扩散的风格化幻觉进行系统性扰动。

Result: 该框架超越了排行榜准确率，提供了关于MLLMs如何在扰动下保持感知基础和关系结构的见解，为分析当前和未来模型的优势和弱点提供了原则性基础。

Conclusion: 感知观测台框架为系统评估多模态大语言模型的感知能力提供了新的方法论，能够更全面地理解模型的视觉基础能力，识别其真实感知能力与文本知识依赖之间的差异。
...
TL;DR: CAMP-VLM：基于视觉语言模型的框架，通过视觉输入的上下文特征和场景图的空间感知，从第三人称视角预测多人行为，在合成和真实数据上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注第一人称视角下的单人行为预测，但许多机器人应用需要从第三人称视角理解多人行为，缺乏适合的数据集和方法。

Method: 提出CAMP-VLM框架，结合视觉输入的上下文特征和场景图的空间感知；使用照片级真实模拟器生成合成数据进行微调；采用监督微调(SFT)和直接偏好优化(DPO)方法。

Result: CAMP-VLM在预测准确率上比最佳基线方法提升高达66.9%；在合成和真实世界序列上评估了泛化能力。

Conclusion: CAMP-VLM框架能有效预测多人行为，通过合成数据训练和VLM方法解决了缺乏合适数据集的问题，为机器人应用中的人类行为理解提供了新方法。
...
TL;DR: VLM-based检测器在少样本多光谱目标检测中表现优异，显著优于专用多光谱模型，并在全监督设置下达到竞争性或更优结果


<details>
  <summary>Details</summary>
Motivation: 多光谱目标检测在自动驾驶和监控等安全敏感应用中至关重要，但标注数据有限限制了深度检测器的训练。文本类别信息可以作为有价值的语义监督源，特别是在数据稀缺场景下。

Method: 将两种代表性的VLM检测器（Grounding DINO和YOLO-World）适配到多光谱输入，并提出有效机制整合文本、视觉和热模态信息。

Result: 在FLIR和M3FD两个多光谱图像基准上的广泛实验表明，VLM检测器在少样本场景下表现优异，显著优于使用可比数据训练的专业多光谱模型，在全监督设置下也达到竞争性或更优结果。
...
<details>
  <summary>Details</summary>
Motivation: 评估视觉语言模型在农业决策支持中的可靠性，了解这些通用模型在农业视觉识别任务中的实际表现，为农业AI应用提供参考

Method: 在AgML集合的27个农业分类数据集上对开源和闭源VLM进行基准测试，涵盖162个类别，包括植物病害、害虫和损害、植物和杂草物种识别，使用零样本评估、多选提示和开放式提示，并应用LLM语义判断

Result: 零样本VLM在所有任务中都显著低于专用监督基线(YOLO11)；多选提示下最佳VLM(Gemini-3 Pro)平均准确率约62%，开放式提示准确率通常低于25%；LLM语义判断可提高开放式准确率；开源模型中Qwen-VL-72B表现最佳；植物和杂草分类比害虫和损害识别更容易

Conclusion: 当前现成的VLM尚不适合作为独立的农业诊断系统，但可以作为辅助组件与约束接口、明确标签本体和领域感知评估策略结合使用
...
TL;DR: 提出一种从初始图像和关节状态生成视频-动作对的方法，通过自动标注动作标签克服视频扩散模型缺乏动作标注的问题，用于机器人策略学习


<details>
  <summary>Details</summary>
Motivation: 现有方法要么采用两阶段流水线限制了跨模态信息共享，要么依赖单模态扩散模型无法充分利用预训练视频知识，需要一种能有效利用大规模视频数据进行机器人学习的方法
...
<details>
  <summary>Details</summary>
Motivation: 单壳低角分辨率dMRI估计FOD精度有限，多壳高角分辨率dMRI需要长扫描时间限制了应用。扩散模型在基于LAR-FOD估计HAR-FOD方面有潜力，但由于FOD中球谐系数数量庞大，高效生成HAR-FOD具有挑战性

Method: 提出3D多通道补丁扩散模型预测HAR-FOD；设计FOD补丁适配器引入先验脑解剖知识实现更高效的补丁学习；引入体素级条件协调模块增强模型全局理解；设计SH注意力模块有效学习SH系数的复杂相关性

Result: 实验结果显示该方法在HAR-FOD预测中取得最佳性能，优于其他最先进方法

Conclusion: 提出的3D多通道补丁扩散模型能够有效从LAR-FOD预测HAR-FOD，通过创新的架构设计解决了球谐系数数量庞大带来的挑战，为扩散MRI分析提供了高效解决方案
...
TL;DR: 提出Auto-Vocabulary 3D目标检测(AV3DOD)，无需用户指定类别即可自动生成检测对象的类别名称，引入语义评分(SS)评估生成质量，在ScanNetV2和SUNRGB-D数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D目标检测方法虽然在训练时未见过的类别上能定位3D框，但仍需用户在训练和推理时指定类别。作者希望实现真正的自动词汇3D目标检测，无需任何用户输入即可自动生成检测对象的类别名称。

Method: 提出AV3DOD框架：1) 引入语义评分(SS)评估生成类别名称的质量；2) 利用2D视觉语言模型(VLMs)通过图像描述生成丰富的语义候选；3) 生成伪3D框；4) 在特征空间进行语义扩展。

Result: 在ScanNetV2和SUNRGB-D数据集上取得最先进的定位性能(mAP)和语义质量(SS)。在ScanNetV2上比SOTA方法CoDA提升3.48 mAP，SS相对提升24.5%。

Conclusion: AV3DOD实现了真正的自动词汇3D目标检测，无需用户指定类别即可自动生成高质量的对象类别名称，在定位精度和语义质量方面均优于现有方法。
...
TL;DR: 提出了一种使用闪烁星点图案准直仪的事件相机标定方法，用于解决长距离高精度测量场景下的几何标定问题


<details>
  <summary>Details</summary>
Motivation: 事件相机作为新型仿生视觉传感器具有高动态范围和高时间分辨率优势，但在长距离测量场景下的几何标定（包括内参和外参）仍然面临重大挑战

Method: 使用带有闪烁星点图案的准直仪，首先基于准直仪的球面运动模型线性求解相机参数，然后通过非线性优化对这些参数进行高精度细化

Result: 在不同条件下的全面真实世界实验中，该方法在准确性和可靠性方面始终优于现有的事件相机标定方法

Conclusion: 提出的基于准直仪闪烁星点图案的事件相机标定方法能够有效解决长距离高精度测量需求，为事件相机在相关应用中的几何标定提供了可靠解决方案
...
TL;DR: TurboDiffusion是一个视频生成加速框架，通过注意力加速、步长蒸馏和量化技术，将端到端扩散生成速度提升100-200倍，同时保持视频质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在视频生成方面取得了显著进展，但其计算成本高昂，生成速度慢，限制了实际应用。需要开发高效的加速框架来降低计算成本，提高生成效率。

Method: 1. 注意力加速：使用低比特SageAttention和可训练的稀疏线性注意力（SLA）加速注意力计算；2. 步长蒸馏：采用rCM进行高效的步长蒸馏；3. W8A8量化：将模型参数和激活量化为8位，加速线性层并压缩模型；4. 其他工程优化。

Result: 在Wan2.2-I2V-14B-720P、Wan2.1-T2V-1.3B-480P、Wan2.1-T2V-14B-720P和Wan2.1-T2V-14B-480P模型上的实验表明，TurboDiffusion在单个RTX 5090 GPU上实现了100-200倍的视频生成加速，同时保持可比的视频质量。

Conclusion: TurboDiffusion是一个高效的视频生成加速框架，通过多种技术创新显著提升了扩散模型的生成速度，为实际应用提供了可行的解决方案。开源代码和模型检查点已公开。
...
TL;DR: 本文提出了一种使用准直仪系统的新型相机标定方法，通过角度不变性约束将6自由度相对运动简化为3自由度纯旋转运动，实现了无需相机运动的单图像标定。
...
Result: 在合成和真实世界实验中评估了方法性能，验证了使用准直仪系统进行标定的可行性，并证明该方法优于现有基线方法。

Conclusion: 提出的准直仪相机标定方法通过角度不变性约束简化了标定过程，实现了无需相机运动的灵活快速标定，为相机标定提供了新颖的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 牛群互动检测对智能畜牧管理至关重要（如发情检测），但现有研究主要针对人类互动检测，牛群互动检测面临缺乏全面行为数据集的挑战，因为放牧牛群的互动是罕见事件

Method: 1. 从大规模牛群动作数据集学习动作潜在空间；2. 通过对比学习微调预训练潜在空间来嵌入罕见互动，构建动作与互动的统一潜在空间；3. 开发集成视频和GPS输入的实用工作系统

Result: 在商业规模牧场上的实验表明，该方法相比基线实现了准确的互动检测

Conclusion: CattleAct方法通过数据高效的方式解决了牛群互动检测的挑战，为智能畜牧管理提供了实用解决方案，代码已开源
...
TL;DR: C-DGPA提出了一种基于类别中心的双重对齐生成提示适应方法，通过双分支架构同时优化边际分布对齐和条件分布对齐，解决了视觉语言模型在无监督域适应中面临的领域差异问题。


<details>
  <summary>Details</summary>
Motivation: 现有提示调优策略主要关注边际分布对齐，但忽略了条件分布差异，导致类别原型错位和语义判别性下降等问题。需要同时解决这两种分布差异以实现更好的域适应性能。

Method: 提出C-DGPA框架：1）边际分布对齐分支采用动态对抗训练框架；2）条件分布对齐分支引入类别映射机制，通过标准化语义提示理解和防止源域过依赖来对齐条件分布差异。两个分支通过协同优化将领域知识整合到提示学习中。

Result: 在OfficeHome、Office31和VisDA-2017三个基准数据集上的广泛实验验证了C-DGPA的优越性，在所有基准上都取得了新的最先进结果。

Conclusion: C-DGPA通过双重对齐策略有效整合领域知识到提示学习中，确保了领域不变且语义可判别的表示，显著提升了视觉语言模型在无监督域适应任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统相机在端到端驾驶中面临领域差距问题，特别是当训练数据条件与部署环境不匹配时性能大幅下降。本文针对昼夜光照差异这一具体领域差距问题，探索更鲁棒的传感器替代方案。

Method: 提出使用事件相机作为传统相机的替代方案。事件相机基于异步像素级亮度变化检测，对光照变化具有更强的鲁棒性，能够在不同光照条件下保持一致的性能表现。

Result: 实验结果显示，事件相机在不同光照条件下保持更一致的性能，其领域转移惩罚通常与灰度帧相当或更小，在跨领域场景中提供更优越的基准性能。

Conclusion: 事件相机是应对自动驾驶中昼夜光照差异领域差距的有效替代方案，能够在不需额外调整的情况下维持跨光照条件的性能一致性，为解决领域差距问题提供了有前景的解决方案。
...
TL;DR: Avatar4D是一个可迁移的合成人体运动数据生成管道，专注于特定领域应用，提供对身体姿态、外观、相机视角和环境背景的细粒度控制，无需人工标注。通过创建Syn2Sport体育数据集验证了其在运动领域的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注日常通用运动，灵活性有限，难以满足特定领域应用的需求。体育等领域具有独特的人类动作和运动模式，对运动理解提出了特殊挑战，需要专门的数据集支持。

Method: 开发了Avatar4D管道，能够生成高保真4D（随时间变化的3D几何）人体运动序列，控制身体姿态、外观、相机视角和环境背景。创建了Syn2Sport大规模合成数据集，涵盖棒球和冰球等运动。

Result: 在Syn2Sport上对多种最先进的姿态估计模型进行了基准测试，证明了其在监督学习、零样本迁移到真实世界数据以及跨运动泛化方面的有效性。评估了合成数据与真实世界数据集在特征空间中的对齐程度。

Conclusion: Avatar4D展示了无需依赖特定领域真实数据，就能生成可扩展、可控且可迁移的人体数据集，为多样化特定领域任务提供了潜力。
...
TL;DR: VALOR提出了一种基于强化学习的后对齐框架，通过两阶段训练改进医学视觉语言模型，提升放射学报告生成的视觉基础和临床准确性。


<details>
  <summary>Details</summary>
Motivation: 尽管大型医学视觉语言模型有进展，但生成既视觉基础又临床准确的放射学报告仍然困难。现有方法依赖大规模标注数据、昂贵的任务特定偏好数据或检索方法，但未能充分缓解视觉和语言表示之间跨模态对齐不良导致的幻觉问题。

Method: 提出VALOR方法，采用基于强化学习的后对齐框架，使用组相对近端优化。训练分为两阶段：1) 使用文本奖励改进Med-VLM，鼓励临床精确术语；2) 将文本基础模型的视觉投影模块与疾病发现对齐，引导注意力到与诊断任务最相关的图像区域。
...
TL;DR: 该论文提出了一种基于信息几何的3D点云形状分析方法，通过将点云表示为统计流形上的高斯混合模型，并引入具有理论上界和下界保证的改进对称KL散度，解决了传统几何度量和现有KL近似方法的局限性。
...
Method: 提出信息几何框架，将点云表示为统计流形上的高斯混合模型。证明GMM空间构成统计流形，并提出改进对称KL散度，该度量具有理论上界和下界保证，确保所有GMM比较的数值稳定性。

Result: 在人体姿态识别（MPI-FAUST数据集）和动物形状比较（G-PCD数据集）上的实验表明，MSKL提供稳定且单调变化的数值，直接反映几何变化，优于传统距离和现有KL近似方法。

Conclusion: 该研究提出的信息几何框架和MSKL散度为3D点云形状分析提供了数值稳定且几何意义明确的度量方法，解决了传统方法的局限性，在点云比较任务中表现出优越性能。
...
TL;DR: SVD低秩矩阵近似用于图像压缩，但压缩效率不如JPEG等标准格式，在低误差水平下压缩后文件甚至可能比原图更大
...
Result: 低秩近似能产生视觉上相似的图像，但压缩效率始终不如JPEG、JPEG2000和WEBP等标准格式。在低误差容忍水平下，SVD压缩表示甚至可能超过原始图像大小。

Conclusion: SVD低秩近似方法在实际图像压缩中不具竞争力，无法与行业标准编解码器相比。
...
TL;DR: ARMFlow是一个基于MeanFlow的自回归框架，用于3D人体反应生成，解决了高运动保真度、实时推理和在线场景自回归适应性的挑战，通过因果上下文编码器和MLP速度预测器实现，引入Bootstrap Contextual Encoding减少误差累积。


<details>
  <summary>Details</summary>
Motivation: 3D人体反应生成面临三个主要挑战：高运动保真度、实时推理和在线场景的自回归适应性。现有方法无法同时满足这三个要求，需要一种能够处理在线交互场景的实时高质量反应生成方法。

Method: 提出ARMFlow框架，基于MeanFlow的自回归架构，包含因果上下文编码器和MLP速度预测器。引入Bootstrap Contextual Encoding训练方法，使用生成的历史而非真实历史进行编码，减少自回归生成中的误差累积。还提出离线变体ReMFlow。

Result: ARMFlow在InterHuman和InterX数据集上的单步在线生成性能超过现有在线方法40%以上的FID指标，同时匹配离线SOTA性能，尽管只使用部分序列条件。ReMFlow在离线方法中达到SOTA性能且推理速度最快。

Conclusion: ARMFlow成功解决了3D人体反应生成的三个核心挑战，通过全局上下文编码器增强语义对齐，单步推理实现高精度低延迟，BSCE减少累积误差，为在线交互场景提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多视角人群计数可解决大范围场景中的严重遮挡问题，但多视角数据收集和标注困难，现有数据集规模有限。需要开发能在有限标注数据下有效工作的半监督方法。

Method: 提出两种半监督多视角人群计数框架：1）基于模型预测排序：要求较少视角输入的预测结果不大于较多视角的预测；2）基于模型不确定性排序：要求较多视角的模型不确定性不大于较少视角的不确定性。这些约束以半监督方式引入模型训练。

Result: 实验表明，提出的多视角模型排序方法相比其他半监督计数方法具有优势，在有限标注数据下能有效提升多视角人群计数性能。
...
<details>
  <summary>Details</summary>
Motivation: 文本渲染已成为视觉生成中最具挑战性的前沿领域，但图像中的文本编辑仍未被充分探索，因为它需要在生成清晰字符的同时保持语义、几何和上下文的一致性。当前缺乏专门评估文本编辑能力的基准。

Method: 提出了TextEditBench，一个专注于图像中文本中心区域的全面评估基准。除了基本的像素操作外，强调需要理解物理合理性、语言意义和跨模态依赖的推理密集型编辑场景。引入了新的评估维度——语义期望（SE），用于衡量模型在文本编辑过程中保持语义一致性、上下文连贯性和跨模态对齐的推理能力。

Result: 对最先进的编辑系统进行广泛实验发现，虽然当前模型能够遵循简单的文本指令，但在上下文依赖推理、物理一致性和布局感知集成方面仍存在困难。TextEditBench为推进文本引导的图像编辑和多模态生成中的推理能力建立了新的测试平台。

Conclusion: TextEditBench填补了文本编辑评估的空白，通过专注于这一长期被忽视但基本的能力，为文本引导的图像编辑和多模态生成中的推理能力提供了新的评估标准和研究方向。
...
TL;DR: MACL通过标签感知采样、频率敏感加权和动态温度缩放，解决了多标签遥感图像检索中的语义重叠、标签分布不平衡和类别共现复杂性问题。


<details>
  <summary>Details</summary>
Motivation: 多标签遥感图像检索面临三大挑战：1）土地覆盖类别间的语义重叠；2）高度不平衡的标签分布；3）复杂的类别间共现模式。这些因素导致传统对比学习方法难以平衡常见和稀有类别的表示学习。

Method: 提出多标签自适应对比学习（MACL），包含三个关键技术：1）标签感知采样策略，考虑类别间关系；2）频率敏感加权机制，平衡常见和稀有类别；3）动态温度缩放，适应不同类别的表示难度。

Result: 在DLRSD、ML-AID和WHDLD三个基准数据集上的实验表明，MACL持续优于基于对比学习的基线方法，有效缓解语义不平衡问题，在大规模遥感档案中提供更可靠的检索性能。

Conclusion: MACL通过自适应对比学习框架成功解决了多标签遥感图像检索中的语义不平衡问题，为大规模遥感档案检索提供了有效的解决方案。
...
Method: 提出PixelArena基准，使用语义分割任务来客观评估多模态大语言模型的细粒度图像生成能力。通过零样本设置测试模型生成语义掩码的精度，并对不同模型进行定性和定量比较。

Result: Gemini 3 Pro Image展现出新兴的图像生成能力，在零样本设置下能生成高保真语义掩码，表现出前所未有的视觉智能和在新任务中的真正泛化能力。研究还展示了其他模型的比较结果和失败案例。

Conclusion: 该发现不仅标志着该领域的激动进展，还为未来多模态、推理、可解释性和基准测试相关研究提供了重要见解，表明多模态大语言模型在细粒度图像生成方面具有巨大潜力。
...
TL;DR: 本文提出了一种基于岭估计的融合定位方法，结合序列图像的丰富场景信息和激光测距的高精度，在无人机多传感器条件下提高目标定位精度。


<details>
  <summary>Details</summary>
Motivation: 在无人机搭载多种传感器进行目标跟踪和测量时，传统最小二乘估计在远距离、小交会角、大倾角等有限观测条件下，设计矩阵的列向量存在严重多重共线性，导致病态问题和定位结果不稳定、鲁棒性差。

Method: 提出基于岭估计的融合定位方法，将序列图像的丰富场景信息与激光测距的高精度相结合，通过引入岭估计来缓解有限观测条件下的多重共线性问题。

Result: 实验结果表明，该方法相比基于单一信息的地面定位算法具有更高的定位精度，岭估计的引入有效增强了鲁棒性，特别是在有限观测条件下表现更优。

Conclusion: 基于岭估计的融合定位方法能够有效解决无人机多传感器定位中的多重共线性问题，提高定位精度和系统鲁棒性，特别是在远距离、小交会角等有限观测条件下具有显著优势。
...
TL;DR: QUIDS系统通过质量感知激励机制优化非专用车载移动群智感知中的信息质量，在预算约束下提高覆盖率和可靠性，相比现有方法提升ASQ 38%


<details>
  <summary>Details</summary>
Motivation: 解决非专用车载移动群智感知系统中实现最优信息质量的挑战，包括感知覆盖率、感知可靠性和车辆动态参与等相互关联的问题

Method: 提出QUIDS系统，引入聚合感知质量(ASQ)指标量化信息质量，开发相互辅助信念感知车辆调度算法估计感知可靠性并在不确定性下分配激励

Result: 在真实城市部署数据评估中，QUIDS相比无调度场景提升ASQ 38%，相比最先进方法提升10%，重构地图误差降低39-74%

Conclusion: 通过质量感知激励机制联合优化覆盖率和可靠性，QUIDS实现了无需专用基础设施的低成本高质量城市监测，适用于交通和环境感知等智慧城市场景
...
<details>
  <summary>Details</summary>
Motivation: 传统部署中，边缘设备将视觉数据传输到服务器进行VLM推理时，原始图像需要调整尺寸以适应视觉编码器的输入分辨率，这会导致细粒度细节丢失，从而降低推理精度

Method: 设计了一个两阶段框架：第一阶段服务器在全局图像上进行推理，利用VLM内部注意力识别感兴趣区域，计算输出令牌的最小熵作为置信度；如果最小熵超过预设阈值，服务器请求边缘设备发送RoI的细节保留局部图像；第二阶段服务器联合利用全局和局部图像进行细化推理

Result: 在多个VLM架构上的实验表明，该框架在保持推理精度的同时显著降低了通信成本

Conclusion: 提出的协作边缘到服务器推理框架通过选择性重传策略，有效解决了传统VLM部署中通信成本高和细节丢失的问题，实现了精度和效率的良好平衡
...
TL;DR: GMODiff：基于增益图的一步扩散框架，用于多曝光HDR重建，通过将HDR重建重新定义为条件引导的增益图估计任务，实现100倍加速并抑制幻觉
...
Method: 提出GMODiff框架：1）将HDR重建重新定义为条件引导的增益图（GM）估计任务，增益图编码扩展的动态范围但保持与LDR图像相同的位深度；2）从信息丰富的基于回归的估计开始去噪过程，而非纯噪声，实现单步去噪生成高质量增益图；3）利用回归先验引导LDM的去噪过程和潜在解码，抑制幻觉同时保持结构准确性
...
Method: 1. 引入身份匹配图（IMG），将生成帧和参考帧中的角色建模为加权完全二分图中的两个节点集；2. 提出掩码查询注意力（MQA）计算边权重量化角色间亲和度；3. 将身份对应正确性形式化为图结构度量并在训练中优化；4. 提出针对多角色动画的专门策略：身份嵌入引导、多尺度匹配策略和预分类采样。

Result: 构建了专门用于多角色身份对应评估的基准测试（Identity Correspondence Evaluation benchmark）。大量实验表明，EverybodyDance在身份对应和视觉保真度方面显著优于现有最先进基线方法。

Conclusion: EverybodyDance通过系统化的图建模方法和专门优化的策略，有效解决了多角色动画中的身份对应问题，为多角色姿态驱动动画提供了可靠解决方案。
...
TL;DR: FVG通过三阶段分解方法解决T2V模型在复杂场景和时序逻辑上的问题：LLM重写提示描述初始场景，T2I模型生成高质量锚定帧，视频模型专注于动画化场景，显著提升性能并加速采样
...
Method: 提出因子化视频生成（FVG）管道，将文本到视频生成分解为三个专门阶段：1）推理阶段：大型语言模型重写视频提示，仅描述初始场景，解决时间歧义；2）构图阶段：文本到图像模型从新提示合成高质量、构图正确的锚定帧；3）时序合成阶段：微调的视频模型专注于动画化场景并遵循提示

Result: 在T2V CompBench基准测试中达到新的最先进水平，在VBench2上显著提升所有测试模型性能。视觉锚定允许将采样步骤减少70%而不损失性能，实现采样速度大幅提升

Conclusion: 因子化视频生成为更高效、鲁棒和可控的视频合成提供了一条简单而实用的路径，通过任务分解解决了现有T2V模型的核心局限性
...
TL;DR: 该研究利用3D神经表示构建人脸图像集合的统一解释，采用高斯泼溅技术结合三角化表面约束，实现从少量图像重建中性姿态人脸，并能将高斯泼溅转换为纹理空间的可视化神经纹理。


<details>
  <summary>Details</summary>
Motivation: 解决从少量未校准人脸图像重建统一3D表示的问题，传统方法需要大量数据或视频序列，而该方法旨在从仅11张图像中重建中性姿态人脸，并使其能无缝集成到标准图形管线中。

Method: 1. 使用高斯泼溅而非NeRF，因其更显式且更易约束；2. 利用分割标注对齐人脸语义区域；3. 将高斯软约束到三角化表面上以获得结构化重建；4. 将高斯泼溅转换为纹理空间的视点依赖神经纹理；5. 使用可重光照高斯模型分离纹理和光照，获得高分辨率反照率纹理。

Result: 成功从仅11张图像重建中性姿态人脸，生成可用于标准图形管线的三角化表面和高分辨率反照率纹理，支持在不相容光照下的图像训练，并展示了在文本驱动资产创建管线中的应用效果。

Conclusion: 该方法实现了从少量图像重建高质量3D人脸表示，将高斯泼溅与三角化表面约束相结合，生成可直接用于标准图形管线的几何和纹理资产，为文本驱动资产创建提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于token序列的大语言模型难以直接处理包含复杂几何和拓扑信息的3D Brep模型，需要弥合结构化3D几何数据与自然语言之间的模态差距。
...
Result: 实验表明BrepLLM在3D物体分类和描述任务上达到了最先进的性能，并构建了包含269,444个Brep-文本问答对的数据集Brep2Text。

Conclusion: BrepLLM成功实现了LLM对原始Brep数据的解析和推理，为3D几何与自然语言的跨模态理解提供了有效解决方案。
...
TL;DR: CountZES是一个无需训练的零样本目标计数框架，通过三阶段协同方法从文本描述中发现多样化的示例，在自然、航拍和医疗领域都表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有零样本目标计数方法存在局限性：基于开放词汇检测器的方法会产生多实例候选，而随机补丁采样方法无法准确界定目标实例。需要一种能精确选择单实例示例的方法来解决复杂场景中的零样本计数问题。
...
Result: 在多个数据集上的实验表明，CountZES在零样本目标计数方法中表现出优越性能，并能有效泛化到自然、航拍和医疗领域。

Conclusion: CountZES通过三阶段协同的零样本示例选择方法，解决了复杂场景中零样本目标计数的挑战，在平衡文本基础、计数一致性和特征代表性方面表现出色，具有广泛的领域适应性。
...
TL;DR: 提出了一种无需训练的方法，通过几何视角精炼文本嵌入来抑制不想要的语义，解决文本到图像扩散模型中多输出间主题一致性问题。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在从自然语言描述生成高质量图像方面表现出色，但在多个输出间保持主题一致性方面存在不足，限制了其在视觉叙事中的应用。现有方法依赖模型微调或图像条件，计算成本高且需要针对每个主题进行优化。

Method: 提出了一种简单而有效的无需训练方法，从几何视角解决语义纠缠问题，通过精炼文本嵌入来抑制不想要的语义，提高主题一致性和文本对齐。

Result: 大量实验证明，该方法在主题一致性和文本对齐方面显著优于现有基线方法。

Conclusion: 该方法为文本到图像扩散模型中的主题一致性问题提供了一种计算效率高且无需训练的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统VAR模型在大规模步数下计算复杂度和运行时间急剧增加，现有加速方法依赖手动步数选择且忽略了生成过程中不同阶段的重要性差异。

Method: StageVAR通过分析发现早期步骤对语义和结构一致性至关重要，而后期步骤主要细化细节。基于此，提出无需额外训练的即插即用加速策略，利用后期计算的语义无关性和低秩特性进行剪枝和近似。

Result: StageVAR实现了最高3.4倍的加速，在GenEval上仅下降0.01分，在DPG上仅下降0.26分，一致优于现有加速基线方法。

Conclusion: 阶段感知设计是高效视觉自回归图像生成的有力原则，StageVAR为VAR模型提供了一种系统化的加速框架。
...
TL;DR: 该研究提出了一种通过强化学习让模型获得人类感知-推理能力的盲图像质量评估方法，使模型能够像人类一样进行自我一致的推理判断。


<details>
  <summary>Details</summary>
Motivation: 人类通过感知-推理级联来评估图像质量，结合感官线索和隐式推理形成自我一致的判断。当前盲图像质量评估模型缺乏这种人类式的推理能力，需要开发既能预测质量分数又能解释推理过程的模型。

Method: 首先收集捕捉人类感知-推理过程多个方面的人类评估数据，然后采用强化学习，使用人类标注作为奖励信号来引导模型获得人类式感知和推理能力。为了培养自我一致的推理能力，设计了奖励机制使模型能够仅从自我生成的描述中推断图像质量。

Result: 在通用指标（皮尔逊和斯皮尔曼相关系数）上，该方法达到了与最先进盲图像质量评估系统相当的性能。在人类-模型对齐评估中，使用ROUGE-1衡量模型生成与人类感知-推理链的相似性，在1000多个人类标注样本上获得了0.512的ROUGE-1分数（基线为0.443），表明对人类解释有显著覆盖。

Conclusion: 该方法不仅实现了高质量分数预测，还显著提升了模型推理过程与人类推理的对齐程度，朝着人类式可解释推理的盲图像质量评估迈出了重要一步。
...
TL;DR: 该论文提出眼动行为辅助的多模态情感识别(EMER)数据集和EMERT模型，通过眼动行为弥补面部表情识别与真实情感识别之间的差距，显著提升了情感识别性能。


<details>
  <summary>Details</summary>
Motivation: 当前情感识别(ER)领域过度依赖面部表情识别(FER)，但面部表情常作为社交工具而非真实内在情感的表现。为理解和弥合FER与ER之间的差距，作者引入眼动行为作为重要情感线索。

Method: 1. 构建EMER数据集：使用自发情感诱导范式收集真实情感数据，同时捕捉眼动序列、注视点图和面部表情视频；2. 设计EMERT模型：采用模态对抗特征解耦和多任务Transformer，将眼动行为作为面部表情的强有力补充。

Result: 1. 创建了包含多模态数据和多视角情感标注的EMER数据集；2. EMERT模型在七个多模态基准协议上显著优于其他最先进方法；3. 验证了眼动行为建模对提升情感识别鲁棒性的重要性。

Conclusion: 眼动行为在情感识别中具有重要作用，能够有效弥合面部表情识别与真实情感识别之间的差距。EMER数据集和EMERT模型为更鲁棒的情感识别研究提供了新方向。
...
TL;DR: 本文提出了一种用于骨架时序动作定位的自监督预训练方法，通过片段判别任务和U形模块融合中间特征，提升帧级定位性能。


<details>
  <summary>Details</summary>
Motivation: 骨架时序动作定位需要时间敏感的特征来捕捉相邻帧间的细微差异，而现有的自监督预训练方法主要针对视频级动作识别，对动作边界检测的研究不足。

Method: 1) 设计片段判别前置任务：将骨架序列密集投影到非重叠片段，通过对比学习区分不同视频的片段；2) 在骨架动作识别模型基础上，使用U形模块融合中间特征以增强帧级定位的特征分辨率。

Result: 在BABEL数据集上，该方法持续改进了现有的骨架对比学习方法；在PKUMMD上通过NTU RGB+D和BABEL预训练实现了最先进的迁移学习性能。

Conclusion: 提出的自监督预训练方法能有效学习时间敏感特征，提升骨架时序动作定位性能，并在多个数据集和评估协议上验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型主要基于2D图像回答问题，缺乏内在的3D物体感知能力，限制了它们在3D场景中理解空间关系和深度信息的能力。

Method: 提出N3D-VLM框架，通过原生3D物体感知能力直接基于文本描述在3D空间中定位物体，然后进行显式的3D推理。开发了可扩展的数据构建流程，利用深度估计将大规模2D标注提升到3D空间，生成用于3D物体定位和空间推理的训练数据。

Result: 实验结果表明，该框架在3D定位任务上达到最先进性能，在3D空间推理方面也持续超越现有方法。数据构建流程产生的3D物体定位数据比现有最大单图像3D检测数据集大六倍以上。

Conclusion: N3D-VLM成功地将原生3D物体感知与3D感知视觉推理相结合，实现了更精确的3D定位和更可解释的结构化空间理解，为3D视觉语言模型的发展提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能重建当前可见的场景部分，无法处理物体移动和遮挡情况，缺乏对完整场景的持久重建能力

Method: 将场景分解为刚性3D基元，通过估计的密集2D对应关系联合推断这些基元的刚性运动，使用优化管道实现4D重建，并引入运动外推机制处理不可见物体

Result: 在物体扫描和多物体数据集上，该系统在定量和定性评估中都显著优于现有方法，实现了可重放的3D重建、多物体扫描和物体持久性

Conclusion: 该系统实现了完整的4D时空感知能力，能够从单目视频中生成动态场景的持久重建，为计算机视觉和机器人应用提供了新的可能性
...
TL;DR: 该论文提出Causal-Tune方法，通过频域分析分离视觉基础模型中的因果和非因果因素，提升领域泛化语义分割性能


<details>
  <summary>Details</summary>
Motivation: 现有方法在微调视觉基础模型时忽略了长期预训练产生的伪影问题，这些伪影与非因果因素相关，阻碍了有价值表示的利用，从而降低了领域泛化语义分割性能
...
TL;DR: CRONOS是一个用于3D医学扫描时间演化的统一预测框架，支持从多个过去扫描进行多对一预测，能够处理离散和连续时间戳，通过学习时空速度场实现任意时间点的体素级预测。


<details>
  <summary>Details</summary>
Motivation: 现有3D医学扫描时间演化预测模型存在局限性：要么依赖单一先验扫描，要么使用固定网格时间，要么针对全局标签，这限制了不规则采样下的体素级预测能力。

Method: CRONOS学习一个时空速度场，将上下文体积传输到任意时间点的目标体积，直接在3D体素空间中操作，支持离散（网格）和连续（实值）时间戳的统一处理。

Result: 在涵盖Cine-MRI、灌注CT和纵向MRI的三个公共数据集上，CRONOS优于其他基线方法，同时在计算上保持竞争力。
...
TL;DR: SkiLa提出了一种新的多模态推理范式，通过在推理过程中原生生成连续视觉嵌入（潜在草图令牌），实现文本和视觉思维的统一空间交互。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在需要视觉想象力的场景中表现不足，而人类能够在统一的大脑空间中构建灵活的视觉-文本思维交互。受此启发，考虑到当前MLLMs已经在同一特征空间中编码视觉和文本信息，研究者认为视觉令牌可以无缝插入到文本令牌承载的推理过程中。

Method: 提出Sketch-in-Latents（SkiLa）范式，扩展MLLMs的自回归能力以原生生成连续视觉嵌入（潜在草图令牌）作为视觉思维。在推理过程中，模型动态交替使用文本思维模式生成文本思考令牌和视觉草图模式生成潜在草图令牌，并引入潜在视觉语义重建机制确保这些令牌具有语义基础。

Result: 大量实验表明，SkiLa在视觉中心任务上实现了优越性能，同时在多样化通用多模态基准测试中表现出强大的泛化能力。

Conclusion: SkiLa通过统一的多模态推理范式，使MLLMs能够在推理过程中原生生成视觉嵌入，实现了更接近人类思维方式的视觉-文本交互能力，为多模态人工智能发展提供了新方向。
...
TL;DR: Yuan-TecSwin是一种基于Swin-transformer的文本条件扩散模型，通过替换CNN块为Swin-transformer块来提升长距离语义理解能力，在ImageNet生成基准上达到了1.37的SOTA FID分数。
...
Method: 提出Yuan-TecSwin模型，用Swin-transformer块替换编码器和解码器中的CNN块；改进文本-图像对齐，包括精心选择的文本编码器、有效利用文本嵌入和精心设计的文本条件融合；采用自适应时间步搜索策略。

Result: 在ImageNet生成基准上达到1.37的SOTA FID分数；推理性能提升10%；人类评估者难以区分模型生成图像和人工绘制图像。

Conclusion: Yuan-TecSwin通过引入Swin-transformer块有效提升了扩散模型的长距离语义理解能力，在图像生成质量和文本-图像对齐方面取得了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够在移动和嵌入式设备上实时运行的图像去雾系统，解决现有方法计算复杂、难以在消费级硬件上部署的问题，实现无需GPU加速的实时视频去雾。

Method: 基于暗通道先验和大气散射模型，采用伽马自适应重建、快速传输近似（带数值稳定性下界）、基于分数顶部像素平均的稳定大气光估计器，以及可选的颜色平衡阶段。

Result: 在真实世界图像和视频上的实验演示显示，Hazedefy能够改善可见度和对比度，同时保持计算效率，适合移动和嵌入式应用。

Conclusion: Hazedefy提供了一个实用且高效的去雾解决方案，能够在消费级硬件上实现实时视频增强，为移动和嵌入式视觉应用提供了可行的去雾工具。
...
TL;DR: LLSA提出了一种对数线性稀疏注意力机制，通过分层结构将DiTs中的选择和注意力计算成本从二次降低到对数线性，显著加速长序列训练和推理。


<details>
  <summary>Details</summary>
Motivation: 现有DiTs的自注意力二次计算成本限制了长序列扩展，Top-K稀疏注意力方法仍存在二次选择成本和随序列增长需要增加K值的问题，单层粗粒度设计无法有效表示全局结构。

Method: 提出LLSA：1）分层Top-K选择，利用前一层索引进行渐进稀疏选择；2）分层KV增强机制，在注意力计算中使用不同粒度的更少token保留全局上下文；3）高效GPU实现，仅使用稀疏索引，无需密集注意力掩码。

Result: 在不使用分块和VAE编码的高分辨率像素空间图像生成中，LLSA在256x256像素token序列上实现注意力推理加速28.27倍，DiT训练加速6.09倍，同时保持生成质量。

Conclusion: LLSA为高效训练长序列DiTs提供了有前景的方向，通过分层结构将计算复杂度从二次降低到对数线性，显著提升效率而不损失质量。
...
TL;DR: 该论文提出了一种基于电源插座的三阶段深度学习管道，用于室内多媒体地理位置识别，以辅助打击人口贩运等犯罪活动。


<details>
  <summary>Details</summary>
Motivation: 室内多媒体地理位置识别在数字取证中具有重要应用价值，特别是在打击人口贩运、儿童剥削等严重犯罪方面。然而，由于室内环境布局相似、装修频繁、视觉模糊、光照变化、GPS信号不可靠以及敏感领域数据集有限等挑战，室内地理位置识别技术发展滞后。

Method: 采用三阶段深度学习管道：1) 使用YOLOv11检测电源插座（mAP@0.5 = 0.843）；2) 使用Xception模型将检测到的插座分类为12种插座类型（准确率0.912）；3) 将插座类型映射到对应国家（置信度>90%时准确率0.96）。为解决数据稀缺问题，创建了两个专用数据集：包含2,328张标注图像（通过增强扩展到4,072张）的插座检测数据集，以及包含3,187张图像、涵盖12种插座类型的分类数据集。

Result: 在Hotels-50K数据集（特别是TraffickCam子集）上评估了管道性能，该数据集包含真实世界条件下的酒店图像（如光线不佳、业余拍摄角度）。管道在插座检测、分类和国家映射三个阶段均表现出色，为实际数字取证应用提供了可行方案。

Conclusion: 该研究展示了利用电源插座作为室内一致性标记进行地理位置识别的可行性，为现实世界数字取证应用迈出了实际一步。所有代码、训练模型和数据均已开源。
...
<details>
  <summary>Details</summary>
Motivation: 上下文扩散模型虽然能够轻松修改图像，但也带来了严重的隐私问题：个人图像可能被未经授权地用于身份冒充、虚假信息等恶意用途。现有工作在个性化文本到图像生成方面探索了输入扰动保护，但现代大规模基于DiT的上下文模型的鲁棒性尚未得到充分研究
...
Conclusion: DeContext提供了一种简单、高效且鲁棒的防御方法，通过针对性扰动跨注意力机制来保护图像免遭未经授权的上下文编辑，为解决扩散模型带来的隐私和安全问题提供了有效解决方案
...
Method: 1) 构建首个百万级SAR数据集SAR-1M，包含配对的SAR与光学图像；2) 设计斑点感知表征增强(SARE)，在掩码自编码器中注入SAR特有的斑点噪声；3) 引入语义锚点表征约束(SARC)，利用配对光学图像先验对齐SAR特征并确保语义一致性。

Result: 在多个SAR数据集上的实验表明，SARMAE在分类、检测和分割任务上均达到了最先进的性能。

Conclusion: SARMAE通过大规模数据集构建和噪声感知的自监督学习方法，有效解决了SAR图像数据稀缺和噪声干扰问题，显著提升了SAR图像的理解能力。
...
TL;DR: REGLUE是一种新的潜在扩散模型框架，通过联合建模VAE潜在表示、局部VFM语义和全局CLS标记来提升图像合成质量，相比现有方法在FID指标和收敛速度上均有改善。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型通过重建式去噪目标仅提供间接语义监督，高级语义出现缓慢，需要更长的训练时间并限制样本质量。现有方法要么通过外部表示对齐注入语义，要么仅在扩散过程中联合建模VFM特征的狭窄切片，未能充分利用VFM提供的丰富、非线性、多层空间语义。
...
Conclusion: REGLUE通过全局-局部-潜在联合建模框架，有效利用了VFM的丰富空间语义，显著提升了潜在扩散模型的图像合成质量和训练效率。
...
TL;DR: FrameDiffuser是一个自回归神经渲染框架，通过结合G-buffer数据和模型自身先前输出来生成时间一致的光照真实帧，专门针对交互式应用优化。
...
Result: FrameDiffuser能够在数百到数千帧中保持稳定的时间一致性生成，相比通用方法，环境专门化训练在光照真实感、阴影和反射方面表现更优。

Conclusion: FrameDiffuser为交互式应用提供了一种高效、时间一致的神经渲染解决方案，通过环境专门化训练在保持推理速度的同时实现了优越的光照真实感。
...
### [54] [Weakly Supervised Pneumonia Localization from Chest X-Rays Using Deep Neural Network and Grad-CAM Explanations](https://arxiv.org/abs/2511.00456)
*Kiran Shahi,Anup Bagale*

Main category: cs.CV

TL;DR: 提出基于Grad-CAM的弱监督深度学习框架，用于肺炎分类和定位，无需像素级标注，使用图像级标签生成临床可解释的热力图，评估了7个预训练模型，获得96-98%的高分类准确率。


<details>
  <summary>Details</summary>
Motivation: 胸部X光成像常用于肺炎诊断，但精确定位肺炎区域通常需要昂贵的像素级标注。为了解决这一限制，本研究旨在开发一个弱监督框架，减少对昂贵标注的依赖，同时提供临床可解释的定位结果。

Method: 提出基于Grad-CAM的弱监督深度学习框架，使用图像级标签而非像素级标注，生成肺炎区域热力图。评估了7个预训练模型（包括Vision Transformer），在相同训练条件下使用焦点损失和患者级数据分割防止数据泄露。

Result: 所有模型均获得高分类准确率（96-98%），其中ResNet-18和EfficientNet-B0表现最佳，MobileNet-V3作为轻量级替代方案。Grad-CAM热力图可视化证实方法聚焦于临床相关的肺部区域。

Conclusion: 该研究展示了弱监督可解释模型在肺炎筛查中的潜力，能够增强AI辅助放射诊断的透明度和临床信任度，为减少标注成本同时提供临床可解释结果提供了可行方案。
...
Method: 使用基于三元组边界损失的ResNet-50模型进行主题指纹识别，将同一受试者的所有图像映射到潜在空间中的特定区域。在3D MRI和2D X射线数据上评估少样本指纹识别，包括标准（20-way 1-shot）和挑战性（1000-way 1-shot）场景。
...
Conclusion: 主题指纹识别方法能有效解决开源数据集合并时的数据泄露问题，在医学影像数据上表现出优异的少样本重识别性能，为数据集质量控制提供了实用工具。
...
TL;DR: SDFoam：结合显式Voronoi图和隐式SDF的混合方法，在保持渲染效率的同时显著提升网格重建质量
...
Method: 联合学习显式Voronoi图和隐式有符号距离场（SDF），通过光线追踪优化场景，使用Eikonal正则化，使Voronoi单元面与SDF零水平集对齐

Result: SDFoam在保持渲染效率（与RadiantFoam相当）和外观质量（PSNR、SSIM）的同时，显著改善了网格重建精度（Chamfer距离），减少了漂浮物，提升了拓扑结构

Conclusion: 提出的隐式-显式混合方法SDFoam在多个场景中实现了更清晰、视角一致的表面重建，同时保持了高效的训练和渲染性能
...
<details>
  <summary>Details</summary>
Motivation: 胎儿超声生长评估依赖标准平面中手动识别解剖标志点进行生物测量。手动标注耗时、操作者依赖性强，且对扫描设备和站点差异敏感，限制了自动化方法的可重复性。需要多源标注数据集来开发AI辅助的胎儿生长评估方法。

Method: 创建了一个开放的多中心、多设备基准数据集，包含4,513张去标识化超声图像，来自1,904名受试者，采集自三个临床站点和七种不同超声设备。数据集涵盖所有主要胎儿生物测量指标，提供标准化的受试者分离训练/测试分割、评估代码和基线结果。

Result: 使用自动生物测量模型量化了领域偏移，证明仅在单一中心训练和评估会显著高估性能（相对于多中心测试）。这是首个公开的多中心、多设备、标志点标注数据集，为领域适应和多中心泛化提供了稳健基准。

Conclusion: 该数据集解决了胎儿生物测量领域的数据瓶颈问题，为开发更可靠的跨中心AI辅助胎儿生长评估方法提供了基础，所有数据、标注、训练代码和评估流程均已公开。
...
TL;DR: 该论文提出了OMG-Bench基准数据集和HMATr框架，用于解决在线微手势识别的挑战，包括数据集稀缺和算法限制问题。


<details>
  <summary>Details</summary>
Motivation: 在线微手势识别对VR/AR交互至关重要，但面临两大挑战：1）缺乏公开的大规模数据集，因为微手势动作细微，难以获得精确的骨架数据和帧级标注；2）现有算法多为任务特定，缺乏统一的端到端解决方案。

Method: 1）开发多视图自监督流水线自动生成骨架数据，结合启发式规则和专家精修实现半自动标注，构建OMG-Bench基准数据集；2）提出分层记忆增强Transformer（HMATr）端到端框架，统一手势检测和分类，利用分层记忆库存储帧级细节和窗口级语义，并使用从内存初始化的可学习位置感知查询来隐式编码手势位置和语义。

Result: 构建了OMG-Bench数据集，包含40个细粒度手势类别、13,948个实例、1,272个序列，具有动作细微、动态快速、连续执行的特点。HMATr框架在检测率上比现有最优方法高出7.6%，为在线微手势识别建立了强基线。

Conclusion: 该工作通过构建首个大规模公开的骨架微手势基准数据集OMG-Bench，并提出统一的HMATr端到端框架，显著推进了在线微手势识别领域的发展，为VR/AR交互提供了重要技术基础。
...
TL;DR: 提出TODSynth框架，通过MM-DiT模型和任务反馈引导的采样策略，为遥感语义分割生成更稳定、任务导向的合成数据，在少样本和复杂场景下表现优异。
...
Method: 1. 提出TODSynth框架，包含多模态扩散变换器(MM-DiT)和任务反馈引导的即插即用采样策略；2. MM-DiT采用文本-图像-掩码联合注意力机制，并对图像和掩码分支进行全微调；3. 提出控制-校正流匹配(CRFM)方法，在早期高可塑性阶段通过语义损失动态调整采样方向。

Result: 实验表明该方法在遥感语义分割数据合成方面优于现有最先进的可控生成方法，特别是在少样本和复杂场景下，能生成更稳定、任务导向的合成数据。

Conclusion: TODSynth框架通过改进的控制方案和任务反馈引导的采样策略，有效解决了遥感语义分割数据合成中的控制复杂性和采样不稳定性问题，为扩展标注数据集提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 基于学习的图像压缩技术计算复杂度高，限制了其广泛应用，需要开发低复杂度的解决方案

Method: 采用二叉树结构的编码器-解码器架构，结合注意力特征融合机制整合多分支特征

Result: 在低比特率下，TreeNet比JPEG AI平均BD-rate提升4.83%，同时模型复杂度降低87.82%

Conclusion: TreeNet通过二叉树架构实现了计算复杂度和压缩性能的良好平衡，为学习型图像压缩的实际应用提供了可行方案
...
TL;DR: Make-It-Poseable：一种新颖的前馈框架，通过潜在空间变换解决3D角色摆姿问题，避免了传统方法中的蒙皮权重预测不准确、拓扑缺陷等问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D角色摆姿方法（如自动绑定和姿势条件生成）存在蒙皮权重预测不准确、拓扑缺陷和姿势一致性差等挑战，限制了其鲁棒性和泛化能力。

Method: 将角色摆姿重新定义为潜在空间变换问题，通过潜在摆姿变换器基于骨骼运动操作形状标记，使用密集姿势表示进行精确控制，并引入潜在空间监督策略和自适应补全模块确保高质量几何和拓扑变化适应性。

Result: 该方法在摆姿质量上表现出优越性能，并能自然地扩展到3D编辑应用，如部件替换和细化。

Conclusion: Make-It-Poseable通过潜在空间变换方法有效解决了传统3D角色摆姿方法的局限性，实现了高质量的姿势生成和灵活的3D编辑能力。
...
TL;DR: FlowDet是首个使用现代条件流匹配技术进行目标检测的框架，通过更简单直接的生成路径超越基于扩散的方法，在COCO和LVIS数据集上获得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有DiffusionDet将目标检测框架化为边界框空间中的生成去噪问题，但扩散方法产生弯曲的随机传输路径。FlowDet旨在将检测问题重新表述为更广泛的生成传输问题，学习更简单直接的路径以获得更好的性能。

Method: FlowDet采用条件流匹配技术，将目标检测重新表述为生成传输问题。与扩散方法不同，它学习更简单直接的传输路径，同时保持无需重新训练即可改变边界框数量和推理步骤的能力。

Result: FlowDet在多个特征主干和数据集上超越了基于扩散的检测系统和非生成基线。在召回约束设置下，相比DiffusionDet在COCO数据集上获得+3.6% AP提升，在LVIS数据集上获得+4.2% AP$_{rare}$提升。

Conclusion: FlowDet通过条件流匹配技术成功将目标检测重新框架化为生成传输问题，学习更简单直接的路径，在推理步骤增加时能更快扩展检测性能，为生成式目标检测提供了新的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成系统通常采用分离的流水线方法，无法有效整合视频生成、编辑和智能推理任务。需要构建一个能够处理多样化用户输入（文本指令、参考图像、视频上下文）并产生高质量视频内容的统一框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集存在局限性：真实数据集缺乏精确标注，合成数据集缺乏真实车辆运动轨迹。需要结合两者优势，为轨迹预测研究提供高质量数据。

Method: 创建合成3D环境，并从SinD无人机数据集提取真实世界轨迹，将这些真实轨迹整合到合成环境中，生成具有真实车辆运动的合成数据集。

Result: R3ST数据集填补了合成数据与真实轨迹之间的差距，提供了精确的多模态标注和真实的人类驾驶车辆轨迹。

Conclusion: R3ST通过结合真实轨迹和合成环境，为车辆轨迹预测研究提供了高质量数据集，推进了该领域的研究进展。
...
TL;DR: KineST：一种基于运动学引导状态空间模型的新方法，用于从稀疏信号重建逼真多样的全身姿态，在轻量级框架下实现精度和时间一致性的平衡。
...
Method: 提出KineST模型，采用运动学引导的双向扫描策略嵌入运动学先验，使用混合时空表示学习方法紧密耦合空间和时间上下文，并引入几何角速度损失施加物理约束。

Result: 大量实验表明，KineST在轻量级框架下在精度和时间一致性方面都表现出优越性能。

Conclusion: KineST通过运动学引导的状态空间模型有效解决了从稀疏信号重建全身姿态的挑战，在精度、平滑性和效率之间取得了良好平衡。
...
TL;DR: TabReX：一个无参考、基于属性的表格生成评估框架，通过图推理实现可解释的评分


<details>
  <summary>Details</summary>
Motivation: 现有评估方法存在局限性：要么将表格扁平化为文本忽略结构，要么依赖固定参考限制泛化能力，需要新的评估框架来解决表格生成质量评估的挑战

Method: TabReX将源文本和生成表格转换为规范知识图，通过LLM引导的匹配过程对齐它们，并计算可解释的、基于评分标准的分数来量化结构和事实保真度

Result: TabReX在专家排名相关性方面表现最佳，在更难扰动下保持稳定，支持细粒度的模型与提示分析，建立了结构化生成系统可信、可解释评估的新范式

Conclusion: TabReX提供了一个无参考、基于属性的评估框架，通过图推理实现可控的敏感度-特异性权衡，为表格生成系统提供了可信、可解释的评估方法
...
TL;DR: SocialStoryFrames是一个用于分析读者对故事反应的计算模型框架，通过对话上下文和叙事理论分类法来捕捉读者对作者意图、推理、情感反应和价值判断的推断。


<details>
  <summary>Details</summary>
Motivation: 现有计算模型对读者反应的建模能力有限，无法进行细致的分析。需要开发能够捕捉读者对故事丰富解释性、情感性和评价性反应的框架。

Method: 引入SocialStoryFrames形式化框架，基于叙事理论、语言语用学和心理学建立分类法。开发了两个模型：SSF-Generator（通过382名参与者的人类调查验证）和SSF-Classifier（通过专家标注验证）。应用这些模型分析SSF-Corpus数据集中的6,140个社交媒体故事。

Result: 模型验证成功，能够分析不同社区中讲故事意图的频率和相互依赖性，比较和对比叙事实践及其多样性。展示了该框架在大规模研究讲故事方面的实用性。

Conclusion: SocialStoryFrames通过将细粒度、上下文敏感的建模与通用的读者反应分类法相结合，为在线社区中的讲故事研究提供了新的研究途径。
...
TL;DR: 研究探索如何利用个人信息（如个人陈述）来改进个体特征建模和主观任务标注预测，发现人口统计信息比态度、关系、经历更有效，理论方法优于自动聚类，少量相关评论即可，多样化的标注者样本表现最佳。


<details>
  <summary>Details</summary>
Motivation: 以往研究虽然探索了使用个人信息来改进个体特征建模和标注预测，但信息量通常受限，缺乏对何种信息对预测标注者标签最有效的深入理解。本研究旨在填补这一空白，通过分类个人陈述信息来构建标注者模型，预测社会规范判断。

Method: 对个人陈述句子进行分类，并利用这些分类构建标注者模型来预测社会规范判断。进行了多项消融实验和分析，检验不同类型信息对预测标注模式的影响。比较了理论方法和自动聚类方法的效果。

Result: 研究发现：1）人口统计信息比态度、关系、经历更具影响力；2）理论方法通常优于自动聚类方法；3）与先前研究相反，仅需少量相关评论即可获得良好效果；4）拥有更多样化的标注者个人陈述样本能带来最佳性能。

Conclusion: 在利用个人信息预测标注者标签时，应优先考虑人口统计信息，采用理论驱动的分类方法，收集多样化的标注者样本，且无需大量相关评论即可获得良好预测效果。
...
TL;DR: Sage是一个无需人工标注的LLM-as-a-Judge评估套件，通过局部自一致性和全局逻辑一致性来衡量LLM评判质量，发现当前顶尖LLM在近四分之一困难案例中存在显著可靠性问题。
...
Result: 实验表明Sage指标稳定且与监督基准高度相关。当前最先进的LLM（如Gemini-2.5-Pro和GPT-5）在近四分之一困难案例中无法保持一致的偏好。发现情境偏好现象，显示明确的评分标准有助于模型保持一致性。

Conclusion: Sage作为无需人工标注的评估套件可靠有效。微调LLM-as-a-Judge是提升性能的可行方法，基于小组的评判和深度推理能增强评判一致性。人类判断也存在显著不一致性，表明人工标注可能不是可靠的金标准。
...
TL;DR: 该论文提出了一种基于语义驱动的强化学习方法（MRG-R1），用于医学报告生成，通过优化报告级别的临床正确性奖励而非传统的词元级监督，显著提升了生成报告的临床准确性。
...
Method: 提出语义驱动强化学习（SRL）方法，采用组相对策略优化（GRPO）来鼓励临床正确性导向的学习。优化报告级奖励：基于边缘的余弦相似度（MCCS），计算生成报告和参考报告中提取的关键放射学发现之间的相似度。同时引入轻量级推理格式约束，指导模型生成结构化的"思考报告"输出。

Result: 在IU X-Ray和MIMIC-CXR数据集上，MRG-R1取得了最先进的性能：IU X-Ray上CE-F1为51.88，MIMIC-CXR上为40.39。实验表明标签语义强化优于传统的词元级监督。

Conclusion: 优化基于临床基础的报告级奖励而非词元重叠，能显著提高临床正确性。这项工作是探索语义强化在监督医学大型视觉语言模型训练中医学正确性的先驱研究。
...
TL;DR: 该论文介绍了一个用于检测印地语-英语混合社交媒体文本中由虚假叙事驱动的仇恨言论（Faux-Hate）的系统，该系统在Faux-Hate共享任务中取得了有竞争力的结果。


<details>
  <summary>Details</summary>
Motivation: 社交媒体平台已成为有害内容（包括仇恨言论和虚假叙事）快速传播的中心。Faux-Hate共享任务专注于检测由虚假叙事驱动的仇恨言论这一特定现象，特别是在印地语-英语混合文本中。
...
Conclusion: 通过结合NLP技术和领域特定预训练，多任务学习方法能够有效检测社交媒体中由虚假叙事驱动的仇恨言论，为处理这一复杂问题提供了有效解决方案。
...
TL;DR: 基于Qwen2.5-7B模型，通过LoRA微调和提示工程，从警方通报中提取15个关键字段，准确率超过98%


<details>
  <summary>Details</summary>
Motivation: 警方事件通报的结构化信息提取对及时准确的数据处理至关重要，但由于社交媒体文本的变异性和非正式性，这一任务面临挑战

Method: 开发领域适应的提取流程，结合针对性提示工程和LoRA参数高效微调的Qwen2.5-7B模型，处理噪声和异质文本

Result: LoRA微调显著优于基础和指令微调模型，死亡率检测准确率超98.36%，死亡人数精确匹配率95.31%，省级位置提取精确匹配率95.54%

Conclusion: 该流程为专业领域的多任务结构化信息提取提供了验证有效的高效解决方案，为社会科学研究中将非结构化文本转化为可靠结构化数据提供了实用框架
...
TL;DR: 该研究提出了一种用于医疗领域的LLM事实核查模块和领域特定摘要模型，以减少LLM在医疗决策中的幻觉风险，确保输出可靠性和准确性。


<details>
  <summary>Details</summary>
Motivation: 在医疗保健领域，LLM生成的输出需要高度可靠和准确，特别是在涉及决策制定和患者安全的关键场景中。然而，LLM存在幻觉输出的风险，导致在这些关键领域的输出往往不可靠。

Method: 1. 提出独立于LLM的事实核查模块，使用数值正确性测试和通过离散逻辑进行细粒度逻辑检查，以验证事实与电子健康记录的一致性。2. 开发领域特定的摘要模型，使用低秩适应（LoRa）在MIMIC III数据集上进行微调，以减少幻觉率。3. 在完整的MIMIC-III数据集上训练LLM模型。

Result: 1. 事实核查模块在104个摘要提取的3,786个命题上测试，达到精确率0.8904、召回率0.8234、F1分数0.8556。2. LLM摘要模型在摘要质量方面达到ROUGE-1分数0.5797和BERTScore 0.9120。

Conclusion: 该研究提出的独立事实核查模块和领域特定摘要模型能有效减少医疗领域中LLM的幻觉输出，提高输出的可靠性和准确性，为医疗决策提供更安全的支持。
...
TL;DR: IBKE：基于信息瓶颈理论的知识编辑框架，通过压缩和隔离关键信息实现通用知识修正，避免全模型重训练


<details>
  <summary>Details</summary>
Motivation: 大语言模型中的错误或过时信息会影响其准确性和安全部署，现有编辑技术通常难以将修正推广到狭窄领域之外，且容易产生意外后果，需要开发无需全模型重训练的高效更新策略

Method: 基于信息瓶颈理论，通过压缩和隔离实现通用知识修正所需的关键信息，同时最小化对无关模型行为的影响；提出信息瓶颈知识编辑器（IBKE），利用紧凑的潜在表示指导基于梯度的更新

Result: 在多个LLM架构和标准基准任务上验证了IBKE的有效性，展示了最先进的准确性，并提高了编辑的通用性和特异性

Conclusion: 建立了一个理论上有原则且实用的开放域知识编辑范式，提高了LLM在现实应用中的实用性和可信度
...
TL;DR: Sigma-MoE-Tiny是一个极稀疏的MoE语言模型，总参数量20B但仅激活0.5B，通过细粒度专家分割（每层96个专家）实现最高稀疏度，在保持训练稳定性的同时达到顶级性能。


<details>
  <summary>Details</summary>
Motivation: MoE作为基础模型的有效扩展范式，当前面临专家负载平衡的挑战，特别是在极稀疏设置下（每层96个专家但每个token仅激活1个专家），传统负载平衡方法在底层失效。

Method: 1) 采用细粒度专家分割（每层最多96个专家）；2) 提出渐进稀疏化调度策略以平衡专家利用率和训练稳定性；3) 在多样化高质量语料上进行预训练和后续训练。

Result: 模型训练过程非常稳定，未出现不可恢复的损失尖峰。尽管仅激活0.5B参数，但在同规模或更大规模模型中达到顶级性能。同时深入探讨了高稀疏MoE模型中的负载平衡问题。

Conclusion: Sigma-MoE-Tiny展示了在极稀疏设置下MoE模型的有效性，提出的渐进稀疏化调度策略解决了高稀疏度下的负载平衡问题，为未来MoE架构的稀疏化发展提供了重要见解。
...
TL;DR: 该研究比较了OpenAI GPT模型中推理与非推理架构在芬兰语与四种低资源乌拉尔语翻译任务中的表现差异，发现推理模型拒绝率低16个百分点


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型翻译评估主要关注高资源语言，对低资源和濒危语言的性能了解不足，特别是乌拉尔语系语言的翻译能力需要深入研究

Method: 使用文学文本平行语料库，比较OpenAI GPT模型中推理与非推理架构在芬兰语与科米-兹良语、莫克沙语、埃尔齐亚语、乌德穆尔特语之间的翻译表现，通过拒绝率分析评估模型翻译意愿

Result: 推理模型与非推理模型在翻译性能上存在显著差异，推理模型的拒绝率比非推理模型低16个百分点，表现出更强的翻译意愿和能力

Conclusion: 研究为乌拉尔语系研究者提供了有价值的见解，揭示了推理模型在濒危语言保护方面的潜力，有助于更全面理解大语言模型在低资源语言翻译中的能力
...
TL;DR: SpeechLLMs vs 传统级联系统在语音翻译中的性能对比研究，发现级联系统仍是最可靠的，SpeechLLMs仅在特定场景下表现相当


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型扩展到文本之外，将语音作为原生模态整合产生了SpeechLLMs，旨在直接翻译口语，绕过传统的转录级联流程。但问题是这种整合是否真的比成熟的级联架构提高了语音到文本的翻译质量。

Method: 提出了"Hearing to Translate"测试套件，首次全面评估了5个最先进的SpeechLLMs与16个强大的直接和级联系统，这些系统结合了领先的语音基础模型和多语言大语言模型。分析涵盖16个基准测试、13种语言对和9种挑战性条件。

Result: 在广泛的评估中发现，级联系统总体上仍然是最可靠的，而当前的SpeechLLMs仅在特定设置下与级联系统相当，语音基础模型则落后于两者。这表明整合大语言模型（无论是内部整合还是管道整合）对于高质量的语音翻译至关重要。

Conclusion: 虽然SpeechLLMs作为新兴技术有潜力，但传统的级联架构目前在语音翻译任务中仍然保持优势地位，整合大语言模型是获得高质量翻译的关键因素。
...
TL;DR: 提出隐私保护、边缘设备适应的ASR框架，解决临床音频识别中的技术障碍，在真实临床音频上实现17.1%的相对WER改进


<details>
  <summary>Details</summary>
Motivation: 自动语音识别在临床文档处理中有巨大潜力，但面临数据隐私限制、计算资源有限和声学领域偏移等技术障碍，现有模型在真实临床音频上表现差（40.94% WER）

Method: 提出高效的隐私保护适应框架，使用低秩适应（LoRA）在边缘设备上实现持续学习，结合多领域经验回放减少灾难性遗忘

Result: 在目标领域实现17.1%的相对WER改进，与朴素适应相比将灾难性遗忘减少47%

Conclusion: 展示了在现实高影响环境中构建可靠、自改进ASR系统的可行路径，特别适用于资源受限的医疗环境
...
TL;DR: 该研究介绍了CLEF 2025 SimpleText竞赛Task 1的提交方案，使用OpenAI的GPT-4.1系列模型进行科学文本的句子和文档级简化，比较了无上下文提示工程和微调两种方法。


<details>
  <summary>Details</summary>
Motivation: 解决科学文本在句子和文档两个粒度上的简化问题，为CLEF 2025 SimpleText竞赛Task 1提供有效的解决方案，探索不同模型和方法在科学文本简化任务中的表现。

Method: 使用OpenAI的GPT-4.1、GPT-4.1-mini和GPT-4.1-nano模型，比较两种方法：1）无上下文方法（仅依赖提示工程）；2）微调方法。在句子级和文档级两个粒度上进行科学文本简化。

Result: GPT-4.1-mini模型在无上下文方法下在句子和文档级简化都表现稳健；微调模型结果混合，其中GPT-4.1-nano-ft在文档级简化中在某些情况下表现突出，表明不同粒度文本简化的复杂性。

Conclusion: 无上下文的GPT-4.1-mini模型在科学文本简化任务中表现稳定可靠，而微调方法的效果因模型和简化粒度而异，突显了在不同文本粒度上进行简化的技术挑战。
...
TL;DR: 提出Refusal Steering方法，在推理时精细控制LLM对政治敏感话题的拒绝行为，无需重新训练，可移除政治拒绝同时保持安全对齐


<details>
  <summary>Details</summary>
Motivation: 现有基于模式的拒绝检测方法脆弱，需要一种能在推理时精细控制LLM拒绝行为的方法，特别是针对政治敏感话题，同时保持对有害内容的安全对齐

Method: 使用LLM作为评判器分配拒绝置信度分数，提出岭正则化变体计算更好的拒绝-顺从方向隔离的引导向量，在Qwen3-Next-80B-A3B-Thinking等模型上实现激活引导

Result: 成功移除模型对政治敏感话题的拒绝行为，在JailbreakBench上保持安全性，在通用基准测试中接近基线性能，方法在4B和80B模型上具有通用性，也可诱导目标拒绝

Conclusion: 激活引导可以移除政治拒绝行为同时保留对有害内容的安全对齐，为推理时可控、透明的审核提供了实用路径，拒绝信号集中在Transformer深层且分布在多个维度
...
TL;DR: JustRL提出了一种极简的强化学习方法，使用单阶段训练和固定超参数，在1.5B推理模型上达到SOTA性能，计算量减少2倍，且训练过程稳定无需干预。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的强化学习方法越来越复杂，作者质疑这种复杂性是否必要，希望探索更简单有效的方法。

Method: 采用单阶段训练流程，使用固定超参数，避免复杂的多阶段训练、动态超参数调度和课程学习策略。

Result: 在两个1.5B推理模型上，在九个数学基准测试中分别达到54.9%和64.3%的平均准确率，计算量减少2倍，超参数在不同模型间可迁移且无需调优。

Conclusion: 复杂的强化学习方法可能不必要，简单的基线方法就能取得优异性能，许多传统"技巧"反而可能降低性能，建议建立简单有效的基准方法。
...
TL;DR: GinSign框架通过分层分类方法将自然语言映射到系统签名，实现自然语言到时序逻辑的准确翻译，无需依赖大型语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有自然语言到时序逻辑翻译框架要么需要准确的原子基础假设，要么基础翻译准确率低，无法支持下游模型检查。

Method: 提出GinSign框架，采用分层分类方法：先预测谓词标签，再选择适当类型的常量参数，将自由形式生成问题转化为结构化分类问题。

Result: 实验显示框架达到95.5%的基础逻辑等价分数，比现有最佳方法提升1.4倍，支持下游模型检查。

Conclusion: GinSign框架通过结构化分类方法有效解决自然语言到时序逻辑翻译中的基础问题，显著提升翻译准确率。
...
<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成在检索源冲突、包含过时或主观信息时表现不佳，现有工作缺乏统一的推理监督机制。

Method: 提出推理轨迹增强的RAG框架，包含文档级裁决、冲突分析和基于证据的合成三阶段结构化推理；引入冲突感知信任评分管道，使用LLM作为评判者评估基础性、事实正确性、拒绝准确性和冲突行为对齐。

Result: 实验结果显示相比基线有显著提升，特别是Qwen模型，监督微调后端到端答案正确率从0.069提升到0.883，行为一致性从0.074提升到0.722。

Conclusion: 提出的推理增强RAG框架和评估管道为冲突感知、可解释的RAG系统奠定了基础，显著提升了处理冲突信息的能力和系统性能。
...
TL;DR: 研究比较了多模态检索增强生成中两种视觉信息处理策略：将图表转换为文本 vs OCR-free视觉检索，发现在糖生物学领域，中等模型更适合转换策略，而前沿模型下OCR-free检索具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 多模态检索增强生成在生物医学QA中应用广泛，但缺乏对何时应该将图表转换为文本、何时使用OCR-free视觉检索的系统研究，特别是在视觉密集的糖生物学领域。

Method: 构建了包含120道多选题的糖生物学基准数据集，按检索难度分层。实现了四种增强策略：无增强、文本RAG、多模态转换、OCR-free视觉检索（ColPali）。使用Docling解析和Qdrant索引，评估了中等规模开源模型和前沿专有模型。

Result: 对于Gemma-3-27B-IT，文本和多模态转换策略（0.722-0.740）显著优于OCR-free检索（0.510）。对于GPT-4o，多模态策略最佳（0.808），文本（0.782）和ColPali（0.745）接近。在GPT-5系列中，ColPali和ColFlor表现最佳（0.828），各检索器间无统计差异。

Conclusion: 策略选择依赖模型能力：中等模型更适合将视觉信息转换为文本以降低理解负担，而前沿模型下OCR-free视觉检索具有竞争力。ColFlor在保持性能的同时具有较小计算开销，是强生成器下的高效默认选择。
...
TL;DR: Anubuddhi是一个多智能体AI系统，能够从自然语言提示设计和模拟量子光学实验，无需专业编程知识。系统通过语义检索组合光学组件，并通过物理模拟验证设计，在13个实验中达到8-9/10的设计-模拟对齐分数。


<details>
  <summary>Details</summary>
Motivation: 量子光学实验设计通常需要专业编程知识，这限制了研究人员和教育工作者。Anubuddhi旨在通过自然语言界面实现量子光学实验设计的民主化，让非专家也能进行实验设计和模拟。

Method: 系统采用多智能体架构，结合意图路由、知识增强生成和双模式验证（QuTiP和FreeSim）。通过语义检索从三层工具箱中组合光学组件，然后通过物理模拟进行验证和收敛细化。

Result: 在13个实验中（包括基础光学、量子信息协议和先进技术），系统达到8-9/10的设计-模拟对齐分数，模拟结果忠实地建模了预期物理。自由形式模拟在11/13实验中优于约束框架，显示量子光学多样性需要灵活的数学表示。

Conclusion: Anubuddhi成功实现了量子光学实验设计的民主化，为研究和教学提供了强大的初始设计工具。系统区分了结构正确性和定量准确性，高对齐分数确认了正确的物理架构，而数值预测需要专家审查。用户可以通过对话迭代细化设计。
...
<details>
  <summary>Details</summary>
Motivation: 传统伦理框架在处理不确定性决策时存在局限，通常将其简单视为行动约束。本文旨在建立一个能够建模道德责任如何随智能体认知状态（特别是不确定性）变化而动态调整的框架，为复杂系统（包括AI）的伦理决策提供更精确的指导。

Method: 提出了比例责任原则（PPD）框架，建立了总责任公式：D_total = K[(1-HI) + HI * g(C_signal)]，其中K代表知识，HI代表谦逊/不确定性，C_signal代表情境信号强度。通过蒙特卡洛模拟验证了保持基线谦逊系数（λ>0）的系统能产生更稳定的责任分配。在临床伦理、受赠人权利法、经济治理和人工智能四个领域进行了跨学科应用验证。

Result: 蒙特卡洛模拟表明，保持基线谦逊系数的系统能产生更稳定的责任分配，降低过度自信决策的风险。该框架在四个不同领域都显示出有效性，证明比例责任原则能够作为复杂系统中的稳定原则，通过动态平衡认知信心和情境风险，防止过度干预和疏忽遗漏。

Conclusion: 比例责任原则（PPD）通过将谦逊作为系统参数形式化，提供了一种数学上可处理的道德责任建模方法，可用于开发可审计的AI决策系统。该框架揭示了道德责任在不确定性下的动态转化特性，为跨学科伦理决策提供了统一的理论基础。
...
TL;DR: 提出一个从自然语言描述生成物理可实现的装配指令框架，使用离散零件词汇表确保几何有效性、连接约束和可构建顺序，通过LDraw作为中间表示，利用大语言模型生成有效的分步构建序列。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素的扩散方法或CAD模型无法支持复杂装配指令或组件交换，需要一种能够连接语义设计意图与可制造输出的方法，填补从自然语言规范到物理原型之间的空白。

Method: 使用LDraw作为文本丰富的中间表示，引导大语言模型生成有效的分步构建序列；引入Python库进行程序化模型生成；采用"积木袋"方法作为物理API，通过精确定向的积木位置连接"词袋"，将任意功能需求编译为物质现实。

Result: 能够为超过3000个装配零件生成有效的分步构建序列和装配指令，在复杂卫星、飞机和建筑领域评估可构建输出，展示了可扩展性、模块化和保真度，填补了语义设计意图与可制造输出之间的差距。

Conclusion: 该方法作为一种物理API，通过约束词汇表连接精确定向的积木位置与"词袋"，将任意功能需求编译为物质现实，为制造和工程原型中的自然语言实现提供了新的设计选项和指导。
...
TL;DR: AI流行病学是一个通过应用群体层面监测方法来治理和解释高级AI系统的框架，它通过标准化捕获AI-专家交互来预测输出失败，无需理解模型内部机制。


<details>
  <summary>Details</summary>
Motivation: 当前可解释性方法（如SHAP和机制可解释性）在部署模型规模上面临模型复杂性问题，需要一种绕过模型内部复杂性、基于群体层面统计证据的AI治理框架。

Method: 通过标准化捕获AI-专家交互为结构化评估字段（风险等级、对齐分数、准确度分数），将这些作为暴露变量来统计预测输出失败，类似于流行病学中胆固醇和血压预测心脏事件的方法。

Result: 该框架为零负担专家提供自动审计追踪，通过被动跟踪专家与AI建议的趋同和分歧来分析输出而非内部计算，在机构更新模型和切换供应商时保持治理连续性。

Conclusion: AI流行病学通过提供可靠性分数和语义评估，使领域专家无需机器学习专业知识即可检测不可靠的AI输出，实现AI监督的民主化，在造成危害前预防不可靠输出。
...
TL;DR: 本文提出了一种用于循环经济产品分类决策的新型框架，通过状态增强的拆卸序列规划图实现最优递归评估，以电动汽车电池为例展示了该方法的灵活性。


<details>
  <summary>Details</summary>
Motivation: 循环经济产品分类需要平衡保留价值与处理成本、劳动力约束之间的权衡，现有方法缺乏能够适应复杂操作约束的统一决策框架。

Method: 提出基于状态增强拆卸序列规划图的确定性求解器，将拆卸历史编码到状态中强制满足马尔可夫性质，实现最优递归评估，整合基于诊断健康评分的条件感知效用和复杂操作约束。

Result: 框架在电动汽车电池分层分类案例中得到验证，展示了如何通过组件递归估值驱动决策，适应不同机械复杂度、安全要求和经济驱动因素。
...
TL;DR: 研究评估大语言模型在儿科咨询中的安全性，发现模型在面对真实世界父母焦虑压力时存在安全隐患，特别是对抗性查询会降低安全性8%，紧急语言导致最大安全下降。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地被父母用于儿科指导，但它们在真实世界对抗性压力下的安全性了解不足。焦虑的父母经常使用紧急语言，这可能破坏模型的安全防护机制，导致有害建议。

Method: 创建了PediatricAnxietyBench开源基准测试，包含300个高质量查询（150个患者来源，150个对抗性），涵盖10个儿科主题。使用多维安全框架评估两个Llama模型（70B和8B），评估维度包括诊断克制、转诊依从性、模糊表达和紧急情况识别。对抗性查询融入了父母压力模式，包括紧急语言、经济障碍和对免责声明的挑战。

Result: 平均安全得分为5.50/15（标准差2.41）。70B模型优于8B模型（6.26 vs 4.95，p<0.001），关键失败率更低（4.8% vs 12.0%，p=0.02）。对抗性查询使安全性降低8%（p=0.03），紧急语言导致最大下降（-1.40）。在癫痫发作（33.3%不适当诊断）和疫苗接种后查询中发现漏洞。模糊表达与安全性强相关（r=0.68，p<0.001），而紧急情况识别完全缺失。

Conclusion: 模型规模影响安全性，但所有模型都显示出对真实父母压力的脆弱性。PediatricAnxietyBench提供了一个可重复使用的对抗性评估框架，能够揭示标准基准测试忽略的临床重要失败模式。
...
<details>
  <summary>Details</summary>
Motivation: 虽然可以直接查询大型语言模型获取知识，但在高容量操作中，成本、速度、安全性和置信度等问题可能成为障碍。通过将信息从LLM中预先提取到标准数据库中查询，可以缓解这些问题，特别是在医疗等关键领域。

Method: DV通过构建具有特定功能的系统来缓解LLM响应中的错误、离题、自由文本、过于笼统和不一致等问题，并支持多元素响应。系统提供基于浏览器的图形用户界面，便于非技术背景的领域专家进行提示工程。

Result: DV已作为免费、开源、可扩展的软件发布，采用"按现状"原则，不提供任何保证或条件。用户需要意识到使用DV及其输出的潜在风险和益处，并负责确保任何使用都是安全和有效的。

Conclusion: 虽然DV可能存在严重错误，但作者希望适当使用当前和未来版本的DV及其输出能够帮助改善医疗保健。该系统为从LLM中提取结构化知识提供了一种实用方法，特别是在需要可靠、高效查询的领域。
...
TL;DR: 本文提出了一种基于扩散激活算法的新型RAG框架，通过自动构建的知识图谱连接文档，提升大语言模型在复杂推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 传统RAG系统将所有检索信息视为同等可靠，忽视了文本语料库中信息可信度和关联性的差异。GraphRAG方法虽然通过知识图谱改进RAG，但依赖高质量图谱表示，成本高且构建困难，同时面临与标准RAG类似的挑战。

Method: 提出新型RAG框架，采用扩散激活算法从自动构建的知识图谱连接的文档语料库中检索信息。该方法可作为即插即用模块与多种RAG方法集成，结合思维链迭代检索进一步提升性能。

Result: 实验表明，该方法在复杂任务（如多跳问答）上达到或优于迭代RAG方法。与思维链迭代检索结合时，相比朴素RAG在答案正确性上获得高达39%的绝对提升，且使用小型开源语言模型即可实现，在资源受限环境中表现优异。

Conclusion: 提出的扩散激活算法RAG框架有效解决了传统RAG在复杂推理任务中的局限性，通过知识图谱增强信息检索，显著提升性能，且易于集成到现有RAG系统中，特别适合资源受限环境。
...
TL;DR: 研究证明，通过领域适配和精细调优的小型语言模型（SLM）可以在特定任务上替代大型语言模型（LLM），显著降低生成式AI的部署成本，同时保持高性能。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在企业中的规模化应用，大型语言模型的高计算成本成为阻碍可持续性和可访问性的关键因素。虽然LLM在各种任务上表现出色，但其庞大的计算需求使得常规企业使用成本过高，这促使研究者探索更经济的小型语言模型替代方案。

Method: 研究训练了一个领域适配的小型语言模型来执行传统上由LLM处理的任务，如文档摘要、查询回答和结构化数据解释。具体使用facebook/opt-350m模型（350M参数），通过Hugging Face TRL的监督微调（SFT）训练器进行单轮微调。

Result: 实验结果显示，微调后的SLM在ToolBench评估中取得了77.55%的通过率，显著优于所有基线模型，包括ChatGPT-CoT（26.00%）、ToolLLaMA-DFS（30.18%）和ToolLLaMA-CoT（16.27%）。

Conclusion: 精心设计和针对性训练的小型语言模型可以显著降低生成式AI的采用门槛，实现经济高效的大规模生产系统集成，为企业在保持性能的同时控制成本提供了可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是理解人类智能如何动态合成新的目标函数，以及如何赋予人工智能系统同样的能力。核心问题是：目标函数从何而来？我们如何选择追求什么目标？

Method: 提出了主观函数的概念，这是一种内生于智能体的高阶目标函数（相对于智能体自身特征定义，而非外部任务）。以预期预测误差作为主观函数的具体实例进行研究。

Result: 该提案与心理学、神经科学和机器学习中的许多概念建立了联系，为理解智能体如何内生地生成和选择目标函数提供了理论框架。

Conclusion: 主观函数的概念为理解智能体如何动态合成目标函数提供了新的视角，有望帮助构建能够内生生成目标的人工智能系统。
...
TL;DR: KalshiBench基准测试显示，前沿大语言模型在预测未知未来事件时普遍存在系统性过度自信问题，即使推理增强模型也未能改善校准效果。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在各种任务上表现出色，但其认知校准能力仍未被充分理解。现有基准主要测试静态知识准确性，缺乏对模型量化真正未知事件不确定性的评估。

Method: 引入KalshiBench基准，包含300个来自受监管交易所Kalshi的预测市场问题，这些问题的真实结果发生在模型训练截止日期之后。评估了Claude Opus 4.5、GPT-5.2、DeepSeek-V3.2、Qwen3-235B和Kimi-K2五个前沿模型，使用预期校准误差和Brier技能分数等指标。

Result: 所有模型都表现出系统性过度自信。最佳校准模型Claude Opus 4.5的ECE为0.120，而推理增强模型GPT-5.2-XHigh的校准更差（ECE=0.395）。只有1个模型获得正Brier技能分数，表明大多数模型表现不如简单预测基准率。

Conclusion: 模型规模和推理增强并不能自动带来校准改进，认知校准是一个需要针对性开发的独立能力。这突显了在评估大语言模型时，除了准确性外，还需要关注其不确定性量化能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在教育中的普及，学生使用这些工具时面临政策不明确、期望不一致的问题。各教育机构的AI政策差异大且不断变化，导致学生对如何负责任地使用GenAI感到困惑。
...
Method: 提出WeMusic-Agent训练框架，结合知识内化和智能边界学习，教导模型智能决策何时使用内化知识、何时调用专业工具（如音乐检索API、推荐系统）。基于此框架开发WeMusic-Agent-M1模型，通过在500亿音乐相关语料上持续预训练内化音乐知识，同时获得调用外部工具的能力。还构建了基于微信听书真实数据的开源对话式音乐推荐基准。

Result: 在真实世界数据上的实验表明，WeMusic-Agent相比现有模型取得了显著改进。构建的基准支持从相关性、个性化、多样性等多个维度进行全面评估。

Conclusion: WeMusic-Agent框架通过平衡知识内化和工具调用，有效提升了对话式音乐推荐的性能，同时构建的开源基准为领域研究提供了重要评估工具。
...
### [98] [QuadSentinel: Sequent Safety for Machine-Checkable Control in Multi-agent Systems](https://arxiv.org/abs/2512.16279)
*Yiliu Yang,Yilei Jiang,Qunzhong Wang,Yingshui Tan,Xiaoyong Zhu,Sherman S. M. Chow,Bo Zheng,Xiangyu Yue*

Main category: cs.AI

TL;DR: QuadSentinel是一个四智能体安全防护系统，将自然语言安全策略编译为机器可检查的规则，通过状态跟踪器、策略验证器、威胁监视器和裁判四个组件在线执行，提高大语言模型智能体工具使用时的安全性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型智能体在解决复杂任务时使用工具、多步规划和智能体间通信，这会带来安全风险。部署者用自然语言编写的策略存在模糊性和上下文依赖性，难以映射到机器可检查的规则，导致运行时执行不可靠。

Method: 将安全策略表达为sequent形式，设计QuadSentinel四智能体防护系统：状态跟踪器、策略验证器、威胁监视器和裁判。系统将这些策略编译为基于可观察状态谓词的机器可检查规则，并在线执行。裁判逻辑和高效的top-k谓词更新器通过优先级检查和分层冲突解决来降低成本。

Result: 在ST-WebAgentBench（ICML CUA'25）和AgentHarm（ICLR'25）基准测试中，QuadSentinel提高了防护栏杆的准确性和规则召回率，同时减少了误报。相比ShieldAgent（ICML'25）等单智能体基线，它提供了更好的整体安全控制。

Conclusion: 近期部署可以采用这种模式，无需修改核心智能体，保持策略分离和机器可检查性。该方法为LLM智能体安全防护提供了有效的解决方案。
...
Conclusion: ToolForge框架能够高效生成高质量工具调用数据，显著降低训练成本，提升模型在真实世界工具调用任务中的性能
...
### [100] [Adaptation of Agentic AI](https://arxiv.org/abs/2512.16301)
*Pengcheng Jiang,Jiacheng Lin,Zhiyi Shi,Zifeng Wang,Luxi He,Yichen Wu,Ming Zhong,Peiyang Song,Qizheng Zhang,Heng Wang,Xueqiang Xu,Hanwen Xu,Pengrui Han,Dylan Zhang,Jiashuo Sun,Chaoqi Yang,Kun Qian,Tian Wang,Changran Hu,Manling Li,Quanzheng Li,Hao Peng,Sheng Wang,Jingbo Shang,Chao Zhang,Jiaxuan You,Liyuan Liu,Pan Lu,Yu Zhang,Heng Ji,Yejin Choi,Dawn Song,Jimeng Sun,Jiawei Han*

Main category: cs.AI

TL;DR: 本文提出了一个统一的框架，将智能体AI系统的适应策略分为智能体适应和工具适应两大类，并进一步细分为不同形式，为构建更强大、高效、可靠的智能体AI系统提供概念基础和实践路线图。


<details>
  <summary>Details</summary>
Motivation: 随着智能体AI系统在能力和范围上的增长，适应机制成为提升性能、可靠性和泛化性的核心手段。当前研究领域快速扩展但缺乏系统性框架，需要统一视角来理解不同适应策略的设计空间和权衡。

Method: 提出一个系统性框架，将适应策略分为智能体适应和工具适应两大类。智能体适应进一步分解为工具执行信号驱动和智能体输出信号驱动两种形式；工具适应分为智能体无关和智能体监督两种形式。通过该框架分析代表性方法，明确设计空间和权衡。

Result: 该框架有助于澄清智能体AI中适应策略的设计空间，使各种策略的权衡变得明确，并为系统设计中选择或切换策略提供实践指导。通过回顾各类别中的代表性方法，分析了它们的优势和局限性。
...
### [101] [PDE-Agent: A toolchain-augmented multi-agent framework for PDE solving](https://arxiv.org/abs/2512.16214)
*Jianming Liu,Ren Zhu,Jian Xu,Kun Ding,Xu-Yao Zhang,Gaofeng Meng,Cheng-Lin Liu*

Main category: cs.AI

TL;DR: PDE-Agent：首个基于LLM驱动的多智能体协作框架，通过工具链增强实现从自然语言描述自动求解偏微分方程，结合了多智能体规划与多工具协作的创新机制。


<details>
  <summary>Details</summary>
Motivation: 传统PDE求解方法依赖人工设置和领域专业知识，现有PINNs和DeepXDE等框架仍需专家知识且缺乏完全自主性。需要一种能够从自然语言描述自动求解PDE的新范式。

Method: 提出PDE-Agent框架，包含两个关键创新：1) Prog-Act框架与图记忆的多智能体协作，通过双循环机制（局部修复和全局修订）实现动态规划和错误纠正；2) 资源池与工具-参数分离机制的多工具协作，集中管理运行时工件并解决现有框架中的工具间依赖间隙。

Result: 开发了PDE-Bench多类型PDE基准测试集，提出多级指标评估工具协调能力。验证表明PDE-Agent在复杂多步骤、跨步骤依赖任务中表现出优越的适用性和性能。
...
TL;DR: SPRITE框架通过模拟器和大型模型程序化合成可扩展、多样且高质量的空间推理数据，解决了传统模板方法僵化与人工标注不可扩展的困境。


<details>
  <summary>Details</summary>
Motivation: 当前体现智能面临空间理解和推理能力不足的挑战，现有方法存在两难：基于模板的数据集可扩展但结构僵化，人工标注语言多样但不可扩展且计算不精确。

Method: 将真实值生成重构为代码生成任务，利用LLMs将复杂空间问题编译为可执行程序，通过模拟器提取的高精度场景元信息进行验证，确保计算精确且可验证。

Result: 构建了包含3个模拟器、11k+场景、300k+图像/视频指令调优对的数据集，训练出的VLM在多个空间基准测试中显著提升，优于同等规模的开源数据集。

Conclusion: SPRITE框架通过程序化合成克服了传统模板方法的低多样性问题，对构建鲁棒、可泛化的空间智能至关重要，将公开代码和完整数据集以促进研究。
...
TL;DR: AlignMerge：一种几何感知的大语言模型融合框架，在保持对齐性的同时合并多个模型的能力，避免传统融合方法破坏安全对齐的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的大语言模型融合方法（如线性权重融合、任务向量、Fisher加权平均）虽然能保持损失函数性能，但会悄悄破坏模型的对齐性。作者认为融合不应只是数值技巧，而应是在已对齐锚点周围的几何约束操作，需要引导融合过程尊重安全几何结构。

Method: 提出AlignMerge框架，在指令调优基模型的局部Fisher图表中，估计对齐子空间并引入投影器P_A。优化目标包含三个部分：L_geo保持融合在Fisher-Rao几何中接近专家模型；L_align惩罚沿对齐敏感方向的运动；L_bud强制执行软对齐预算。使用解码不变的对齐质量指数（AQI）作为对齐功能度量。

Result: 在五个模型家族（LLaMA-3 8B、Mistral 7B、Qwen 2、Phi-3.5、Gemma 2）上测试，将安全锚点与任务专家融合时，AlignMerge在保持对齐指标（AQI、毒性、LLM-judge对齐）的同时，在指令遵循、推理和帮助性方面匹配或超越最佳专家模型。相比Fisher soups、TIES、SafeMerge和MergeAlign等方法，表现出更小的对齐子空间漂移和更少的预算违规。

Conclusion: 该研究使对齐保持融合成为首要设计目标，为未来基础模型的几何感知组合提供了路径。AlignMerge框架将对齐性作为显式不变量，实现了在模型融合过程中同时保持安全性和性能的平衡。
...
### [104] [Learning to Wait: Synchronizing Agents with the Physical World](https://arxiv.org/abs/2512.16262)
*Yifei She,Ping Zhang,He Liu,Yanmin Jia,Yang Jing,Zijun Liu,Peng Sun,Xiangbin Li,Xiaohe Hu*

Main category: cs.AI

TL;DR: 论文提出了一种面向智能体的方法，让大语言模型能够主动对齐其认知时间线与物理世界，通过预测精确等待时间来同步异步环境，减少查询开销和执行延迟。
...
Method: 提出了一种智能体侧方法，将"代码即动作"范式扩展到时间域。智能体利用语义先验和上下文学习来预测精确的等待时间（time.sleep(t)），从而在不进行详尽检查的情况下与异步环境同步。

Result: 在模拟的Kubernetes集群中的实验表明，智能体能够精确校准其内部时钟，最小化查询开销和执行延迟，验证了时间感知是可学习的能力。

Conclusion: 时间感知是智能体在开放环境中实现自主演进的重要可学习能力，智能体侧的时间同步方法能够有效解决异步环境中的时间间隙问题。
...
### [105] [Code-in-the-Loop Forensics: Agentic Tool Use for Image Forgery Detection](https://arxiv.org/abs/2512.16300)
...
<details>
  <summary>Details</summary>
Motivation: 现有图像伪造检测方法要么利用低级语义无关的伪影，要么依赖具有高级语义知识的多模态大语言模型。这两种信息流在范式和推理上高度异构，难以统一或有效建模其跨级交互。
...
<details>
  <summary>Details</summary>
Motivation: 去中心化LLM推理需要透明和抗审查的访问，但现有验证方法难以扩展到现代模型。原始的PoQ忽略了推理节点和评估节点之间的异构计算成本，需要一种成本感知的解决方案。

Method: 提出成本感知PoQ框架，将显式效率测量集成到奖励机制中。设计结合了真实标记级F1、轻量级学习评估器和GPT判断的统一评估流程，采用平衡归一化质量和成本的线性奖励函数。

Result: 实验表明：1）语义文本相似性双编码器比交叉编码器与真实标记和GPT分数相关性更高；2）池中最大模型在单位延迟质量方面也最有效；3）蒙特卡洛模拟显示成本感知奖励方案能持续奖励高质量低成本的推理模型和高效评估器。

Conclusion: 成本感知PoQ为经济可持续的去中心化LLM推理提供了实用基础，通过平衡质量和成本来优化系统性能。
...
<details>
  <summary>Details</summary>
Motivation: 受人类构建和使用路径行为的启发，特别是人类倾向于使用热门交通路线、在路径封闭时智能混合现有路径构建新路线、以及随机选择不同路径探索未知目的地的行为。

Method: PCIA模仿人类路径构建行为：1）生成随机种群寻找最佳路径（类似群体算法）；2）每个粒子代表一条通往目的地的路径；3）算法结合了路径混合和随机探索机制。
...
TL;DR: Synthelite是一个基于大语言模型的计算机辅助合成规划框架，能够通过自然语言交互生成端到端的合成路线，并灵活适应各种用户约束。


<details>
  <summary>Details</summary>
Motivation: 现有的计算机辅助合成规划框架缺乏与人类专家交互的机制，限制了化学家洞察力的整合。需要开发能够直接与专家互动、利用自然语言提示进行干预的合成规划工具。

Method: Synthelite使用大语言模型直接提出逆合成转化，利用LLMs固有的化学知识和推理能力生成端到端的合成路线，同时允许专家通过自然语言提示进行干预。

Result: 实验表明，Synthelite能够灵活适应不同的用户指定约束，在策略约束和起始物料约束的合成任务中达到高达95%的成功率。此外，Synthelite在路线设计中表现出考虑化学可行性的能力。
...
TL;DR: TIB AIssistant是一个AI支持的研究平台，通过多个助手在整个研究生命周期中为研究人员提供支持，包括生成研究论文草稿、访问外部学术服务，并将数据导出为RO-Crate格式以提高研究可重复性。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能和大语言模型的快速发展，AI对学术研究领域产生了广泛影响。AI支持的研究有潜力在整个研究生命周期中协助研究人员完成各种任务，但目前缺乏一个系统化的平台来整合这些功能。

Method: 开发了TIB AIssistant平台，包含多个专门负责特定研究任务的助手。平台提供工具访问外部学术服务，生成的数据存储在资产中，并可导出为RO-Crate格式。通过助手之间的顺序交互演示了平台的主要功能，包括生成研究论文草稿的各个部分。

Result: 成功展示了TIB AIssistant平台的主要功能，通过助手间的交互能够生成研究论文草稿的各个部分。平台实现了AI支持的研究流程，提供了透明度和可重复性保障。

Conclusion: TIB AIssistant为AI支持的研究奠定了基础，旨在建立一个社区维护的平台，促进学术研究的AI辅助应用，提高研究效率和可重复性。
...
TL;DR: TS2R是一个将锂电池运行时间序列数据转换为结构化报告的提示框架，使大语言模型能够在电池储能系统管理中执行推理、预测和决策任务。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在解释多元时间序列数据方面具有潜力，但在实际电池储能系统运维中的应用尚未充分探索。需要一种方法将原始传感器数据转换为大语言模型能够理解和处理的形式。

Method: 提出TimeSeries2Report（TS2R）框架，通过分割、语义抽象和基于规则的解释相结合的方式，将短期时间动态编码为自然语言，从而将低层传感器信号与高层上下文洞察连接起来。

Result: 在实验室和真实数据集上的评估显示，与基于视觉、嵌入和文本的提示基线相比，TS2R的报告式提示在大语言模型性能上持续改进，在准确性、鲁棒性和可解释性方面表现优异。TS2R集成的大语言模型无需重新训练或架构修改即可达到专家级决策质量和预测一致性。

Conclusion: TS2R为大语言模型驱动的自适应电池智能建立了实用路径，通过将原始时间序列数据转换为语义丰富的报告，使大语言模型能够在电池储能系统管理场景中有效执行推理、预测和决策任务。
...
Conclusion: cuPilot通过策略协调的多智能体框架有效解决了现有自动内核优化方法的局限性，实现了显著的性能提升，生成的代码已开源。
...
TL;DR: 提出了一种新的系统特定保真度度量方法——决定性特征保真度（DFF），用于评估自动驾驶系统在真实和合成环境中决策所依赖的因果证据是否一致，而不仅仅是图像视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶安全验证主要依赖合成数据的虚拟测试，但研究发现像素级视觉保真度并不能保证从仿真到真实世界的可靠迁移。关键问题在于被测系统是否在真实和模拟环境中基于相同的因果证据做出决策，而不仅仅是图像对人类"看起来真实"。

Method: 提出了决定性特征保真度（DFF）这一新的系统特定度量方法，利用可解释AI（XAI）方法来识别和比较驱动被测系统输出的决定性特征。进一步提出了基于反事实解释的实用估计器，以及DFF引导的校准方案来增强仿真器保真度。

Result: 在2126个匹配的KITTI-VirtualKITTI2数据对上的实验表明，DFF揭示了传统输出值保真度忽略的差异。结果显示DFF引导的校准在保持输出值保真度的同时，提高了决定性特征和输入级保真度，适用于多种被测系统。

Conclusion: 决定性特征保真度（DFF）为自动驾驶虚拟测试提供了一种行为基础的保真度度量方法，能够评估系统在真实和模拟环境中决策机制的一致性，有助于提高仿真到真实世界的迁移可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 元算法研究（如算法选择、配置和调度）依赖大量计算密集型实验，实验设计和设置的自由度大，存在多种可能影响研究可扩展性和有效性的误差来源。现有最佳实践分散在不同出版物和领域中，且各自独立发展，需要系统整理。
...
Conclusion: 该报告系统整理了元算法研究的完整实验周期最佳实践，有助于提高该领域研究的科学性、可重复性和有效性，为社区提供了统一的实践指南。
...
### [114] [ParamExplorer: A framework for exploring parameters in generative art](https://arxiv.org/abs/2512.16529)
*Julien Gachadoat,Guillaume Lagarde*

Main category: cs.AI

TL;DR: ParamExplorer：一个基于强化学习思想的交互式模块化框架，帮助探索生成艺术算法中的参数空间，通过人机交互或自动反馈引导探索


<details>
  <summary>Details</summary>
Motivation: 生成艺术系统通常涉及高维复杂的参数空间，美学上有吸引力的输出只占据其中很小且分散的区域。由于组合爆炸问题，艺术家通常依赖大量手动试错，导致许多潜在有趣的配置未被发现

Method: 引入ParamExplorer框架，这是一个受强化学习启发的交互式模块化框架，帮助探索生成艺术算法的参数空间，通过人机交互或自动反馈引导探索。该框架可与现有p5.js项目无缝集成，并在框架内实现和评估多种探索策略（称为智能体）

Result: 论文介绍了ParamExplorer框架及其实现，但摘要中未提供具体的实验结果数据

Conclusion: ParamExplorer为生成艺术参数空间探索提供了一个有效的交互式框架，通过多种探索策略帮助艺术家发现更多美学上有趣的配置，减少手动试错的负担
...
TL;DR: 本文系统研究了CPU-only环境下本地大语言模型和视觉语言模型的推理性能，发现了两个经验缩放定律：语言模型的计算成本与token长度近似线性相关；视觉语言模型存在预处理驱动的"分辨率拐点"。量子启发压缩技术可显著降低计算资源消耗。


<details>
  <summary>Details</summary>
Motivation: 尽管GPU主导AI部署，但大多数消费硬件（笔记本电脑、台式机、工业控制器、嵌入式系统）依赖CPU。然而，CPU-only环境下本地语言和视觉语言工作负载的计算规律尚未得到充分探索，需要系统量化其性能特征。

Method: 在两种代表性CPU层级上系统基准测试：MacBook Pro M2（主流笔记本级）和Raspberry Pi 5（低功耗嵌入式）。采用基于处理器和内存使用连续采样及曲线下面积积分的统一方法，分析计算负载如何随输入文本长度（语言模型）和图像分辨率（视觉语言模型）变化。

Result: 发现两个经验缩放定律：1）语言模型推理的计算成本与token长度近似线性缩放；2）视觉语言模型存在预处理驱动的"分辨率拐点"，计算量在内部分辨率钳位以上保持恒定，以下则急剧下降。量子启发压缩技术可将处理器和内存使用降低高达71.9%，能耗降低高达62%，同时保持或提高语义准确性。

Conclusion: 本研究系统量化了多模态CPU-only环境下本地语言和视觉语言工作负载的缩放特性，识别出模型压缩和输入分辨率预处理作为可持续边缘推理的有效、低成本杠杆。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在现实安全敏感应用中面临检测准确性、推理延迟和部署成本之间的三向权衡，需要一种既高效又准确的检测方法。
...
Conclusion: Prefix Probing提供了一种实用高效的有害内容检测解决方案，在准确性、延迟和成本之间取得了良好平衡，适用于现实安全敏感应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着伊斯兰文本数字化的发展，需要自动化地对圣训进行主题分组，以促进数字伊斯兰研究和基于技术的学习系统
...
<details>
  <summary>Details</summary>
Motivation: 探究在多模态大语言模型中，多智能体设计相比单智能体在几何问题求解方面的实际优势，目前这方面的效益尚不明确

Method: 在四个视觉数学基准测试（Geometry3K、MathVerse、OlympiadBench、We-Math）上系统比较单智能体和多智能体流水线，测试开源模型（Qwen-2.5-VL）和闭源模型（Gemini-2.0-Flash）

Result: 开源模型在多智能体模式下表现显著提升（如Qwen-2.5-VL 7B在Geometry3K上提升+6.8分），而闭源模型在经典基准测试中单智能体表现更好，仅在新数据集We-Math上多智能体有适度改进

Conclusion: 多智能体流水线对开源模型有明显益处，也能辅助强大的专有系统处理较新、较不熟悉的基准测试，但智能体分解并非普遍最优策略
...
<details>
  <summary>Details</summary>
Motivation: 研究算法智能的固有计算限制，特别是形式不完备性和动态不可预测性如何共同约束智能系统的推理和预测能力

Method: 形式化两个独立计算限制：形式不完备性（限制一致推理系统的演绎能力）和动态不可预测性（限制有限精度下的长期预测），分析它们对智能体自我推理能力的影响

Result: 证明这两个极端限制共同对智能体推理自身预测能力施加结构约束，特别是算法智能体通常无法计算自身的最大预测范围

Conclusion: 这一视角阐明了智能系统中推理、预测和自我分析之间固有的权衡关系，揭示了算法智能的基本限制
...
TL;DR: 该论文提出了一种使用PDDL风格表示来学习黑盒AI系统规划能力的方法，通过蒙特卡洛树搜索创建测试任务、获取数据并剪枝假设空间，学习到的模型能够描述BBAI的能力、执行条件及可能结果。
...
TL;DR: 提出混合机器学习和大语言模型管道，预测肺癌住院患者48和72小时内疼痛发作，结合结构化与非结构化电子病历数据，提高预测敏感性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 肺癌患者中高达91%会经历突发性疼痛，需要及时干预。现有方法难以有效预测疼痛发作，无法实现主动疼痛管理。需要开发结合多种数据源、具有临床可解释性的预测工具。

Method: 采用混合机器学习与大语言模型管道：1) 机器学习模块捕捉时间性药物使用趋势；2) 大语言模型解析模糊剂量记录和自由文本临床笔记；3) 整合结构化和非结构化电子病历数据（人口统计学、肿瘤分期、生命体征、WHO分级镇痛药使用等）。回顾性分析266名住院患者数据。

Result: 预测准确率：48小时为0.874，72小时为0.917。大语言模型增强使敏感性分别提高8.6%和10.4%。混合方法提高了敏感性和临床可解释性。

Conclusion: 该混合方法提供了临床可解释且可扩展的早期疼痛发作预测工具，有望提高肿瘤治疗精度和优化资源分配，实现主动疼痛管理。
...
TL;DR: CitySeeker是一个评估视觉语言模型在动态城市环境中理解隐含需求并进行空间推理的新基准，包含8个城市的6440条轨迹，顶级模型仅达到21.1%的任务完成率，揭示了长时推理、空间认知和记忆检索的瓶颈。
...
Method: 提出CitySeeker基准，包含8个城市的6440条轨迹，涵盖7种目标驱动场景的多样视觉特征和隐含需求。通过实验分析模型性能瓶颈，并探索基于回溯机制、丰富空间认知和记忆检索（BCR）的策略，这些策略受人类认知映射中迭代观察-推理循环和自适应路径优化的启发。

Result: 实验显示即使是顶级模型（如Qwen2.5-VL-32B-Instruct）也仅达到21.1%的任务完成率。研究发现主要瓶颈在于长时推理中的错误累积、空间认知不足和经验记忆缺陷。BCR策略分析为开发具有鲁棒空间智能的视觉语言模型提供了可行见解。

Conclusion: CitySeeker基准揭示了视觉语言模型在解决隐含需求导航任务中的显著局限性，特别是长时推理、空间认知和记忆检索方面的挑战。提出的BCR策略和分析为开发能够应对"最后一公里"导航挑战的具有鲁棒空间智能的视觉语言模型提供了重要指导。
