...
TL;DR: 提出了一种基于Depth Anything V2架构的单目深度估计方法，通过动态向量低秩适应技术将合成数据先验知识高效迁移到医疗领域，在手术内窥镜环境中显著提升了深度估计精度。


<details>
  <summary>Details</summary>
Motivation: 传统自监督单目深度估计方法在具有镜面反射和充满液体的内窥镜手术环境中表现脆弱，特别是在薄手术器械和透明表面边界处容易崩溃。现有方法通常依赖带噪声真实世界伪标签训练的基础模型，无法有效处理手术环境中的挑战性条件。

Method: 1. 利用Depth Anything V2架构的高保真合成先验知识，该架构能捕捉薄结构的精确几何细节；2. 采用动态向量低秩适应技术高效地将这些先验知识迁移到医疗领域，最小化参数预算同时弥合合成到真实的差距；3. 在SCARED数据集上引入物理分层评估协议，严格量化高镜面反射区域的性能。

Result: 在SCARED数据集上建立了新的最先进性能：准确度（<1.25）达到98.1%，平方相对误差比现有基线降低了超过17%，在不利手术光照条件下表现出卓越的鲁棒性。

Conclusion: 通过结合高保真合成先验和高效的领域适应技术，该方法显著提升了手术内窥镜环境中的单目深度估计性能，特别是在处理薄手术器械和镜面反射表面方面表现出色，为机器人手术提供了更可靠的深度感知能力。
...
TL;DR: 提出基于视频分析的自动评估系统，用于城市作战训练中的"进入并清理房间"演练，通过计算机视觉提取骨骼、视线和轨迹数据，开发特定任务指标来评估心理运动流畅性、情境意识和团队协作。


<details>
  <summary>Details</summary>
Motivation: 城市作战训练需要情境意识和肌肉记忆，传统评估方法依赖昂贵传感器或主观观察，限制了可扩展性和准确性，需要自动化的客观评估系统。

Method: 使用计算机视觉模型从训练视频中提取2D骨骼、视线向量和运动轨迹，开发特定任务指标衡量心理运动流畅性、情境意识和团队协调，通过加权组合的扩展认知任务分析层次结构生成总体性能分数。

Result: 通过真实世界ECR演练的案例研究，展示了该方法能够提供可操作的领域特定指标，捕捉个人和团队表现，支持行动后审查和交互式仪表板。

Conclusion: 视频分析为合成训练环境中的可扩展评估提供了可行方法，未来工作包括扩展到3D视频数据，并利用视频分析实现STE内的可扩展评估。
...
TL;DR: 该论文提出了终身领域自适应3D人体姿态估计的新任务，通过创新的GAN框架解决非平稳目标姿态数据集的问题，有效缓解领域偏移和灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 3D人体姿态估计在多种应用中很重要，但现有方法依赖受控环境标注数据，难以泛化到真实场景。现有领域自适应范式忽略了非平稳目标姿态数据集问题，需要解决终身适应中的领域偏移和灾难性遗忘挑战。

Method: 提出创新的GAN框架，包含3D姿态生成器、2D姿态判别器和3D姿态估计器。构建新颖的3D姿态生成器范式，集成姿态感知、时间感知和领域感知知识，增强当前领域适应并减轻对先前领域的灾难性遗忘。

Result: 在多样化的领域自适应3D人体姿态估计数据集上进行广泛实验，证明了该方法具有优越性能。

Conclusion: 首次将终身领域自适应引入3D人体姿态估计任务，提出的框架能有效解决领域偏移和灾难性遗忘问题，为3D HPE在真实场景中的应用提供了新思路。
...
TL;DR: 该研究提出了一种深度学习驱动的流程，可将0.2-2岁儿童的T1加权MRI转换为合成CT，实现颅骨分割、缝合线概率热图生成和直接缝合线分割。


<details>
  <summary>Details</summary>
Motivation: 量化儿童颅骨发育和缝合线骨化对诊断和治疗颅骨生长障碍至关重要。CT虽能评估颅骨和缝合线畸形，但其电离辐射对无明显异常的儿童是禁忌。MRI无辐射且软组织对比度好，但无法显示颅骨缝合线、评估骨密度或颅骨生长。因此需要一种非侵入性方法来获取CT级别的颅骨信息。

Method: 提出深度学习驱动的流程：使用特定领域的变分自编码器，将儿童T1加权MRI转换为合成CT；预测详细的颅骨分割；生成缝合线概率热图；从热图直接获得缝合线分割。

Result: 合成CT与真实CT的结构相似度达99%，Frechet起始距离为1.01；颅骨分割在七个颅骨上平均Dice系数达85%；缝合线分割Dice系数达80%；通过双侧等价性检验确认了合成CT与真实CT在颅骨和缝合线分割上的等价性。
...
TL;DR: 提出MGML框架解决多模态MRI数据缺失下的脑肿瘤分割问题，通过元参数化自适应模态融合和一致性正则化模块，在不改变原始模型架构的情况下提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 临床实践中多模态MRI数据常常不完整，如何充分利用不完整的多模态信息是重要研究挑战。需要开发能够有效整合不完整多模态信息的脑肿瘤分割方法。

Method: 提出元引导多模态学习（MGML）框架，包含两个核心组件：1）元参数化自适应模态融合（Meta-AMF），根据可用模态生成自适应软标签监督信号，促进更一致的多模态融合；2）一致性正则化模块，增强分割性能并隐式提升框架的鲁棒性和泛化能力。

Result: 在BraTS2020和BraTS2023数据集上进行了广泛实验。在BraTS2020上，针对15种缺失模态组合的平均Dice分数：全肿瘤（WT）87.55，肿瘤核心（TC）79.36，增强肿瘤（ET）62.67，优于多种最先进方法。

Conclusion: MGML框架能够有效处理不完整多模态MRI数据，在不改变原始模型架构的情况下显著提升脑肿瘤分割性能，具有实用性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 神经外科微血管吻合术需要精细操作技能，传统评估依赖专家主观评分，存在评分者间变异大、耗时等问题，需要自动化、可扩展的客观评估方案

Method: 三模块框架：1)基于YOLO和DeepSORT的器械尖端跟踪定位；2)基于自相似矩阵的动作边界检测和无监督聚类的动作分割；3)用于评估手术手势熟练度的监督分类模块

Result: 在58个专家评分的微血管吻合视频数据集上验证，帧级动作分割准确率达92.4%，技能分类准确率达85.5%，能有效复现专家评估

Conclusion: 该方法能为显微外科教育提供客观、实时反馈，实现更标准化、数据驱动的训练协议，推进高风险手术环境中的能力评估
...
Method: 提出两种攻击方法：T2VAttack-S通过贪婪搜索识别提示中语义或时间关键词语并用同义词替换；T2VAttack-I迭代插入优化词语以最小化提示扰动。同时定义语义目标（评估视频-文本对齐）和时间目标（评估时间动态性）。

Result: 在多个最先进的T2V模型（ModelScope、CogVideoX、Open-Sora、HunyuanVideo）上进行评估，实验表明即使单个词语的替换或插入也会导致语义保真度和时间动态性的显著下降，揭示了当前T2V扩散模型的关键脆弱性。

Conclusion: 当前文本到视频扩散模型对对抗攻击高度脆弱，即使是微小的提示修改也能显著影响生成视频的质量。这凸显了开发更鲁棒的T2V模型的重要性，并为未来防御机制的研究提供了基础。
...
TL;DR: 提出一种用于自动驾驶场景视图外推的有效方法，仅需图像和可选相机位姿，无需昂贵的传感器或人工标注，通过4D高斯模型和视频扩散模型的迭代优化实现高质量新视角生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖激光雷达点云、3D边界框和车道标注等先验信息，需要昂贵传感器或人工标注，限制了实际部署应用。需要一种更轻量、更实用的视图外推解决方案。

Method: 首先从图像和可选相机位姿估计全局静态点云和每帧动态点云，融合为统一表示。采用可变形4D高斯框架重建场景，用初始训练的4D高斯模型渲染降质伪图像训练视频扩散模型。然后通过扩散模型迭代精炼渐进偏移的高斯渲染结果，并将增强结果作为训练数据反馈给4DGS，直到外推到目标视角。

Result: 与基线方法相比，该方法在新外推视角下生成更高质量的图像，验证了方法的有效性。

Conclusion: 提出了一种仅需图像和可选相机位姿的自动驾驶场景视图外推方法，通过4D高斯模型和视频扩散模型的协同优化，实现了高质量的新视角生成，避免了昂贵传感器和人工标注的需求。
...
TL;DR: 本文提出了一种基于深度学习的卫星影像地表变化检测方法，通过训练一个修复模型来预测时间序列中的最后一帧图像，然后通过预测与观测之间的差异来检测异常变化。


<details>
  <summary>Details</summary>
Motivation: 卫星影像地表变化检测对于灾害快速响应和环境监测至关重要，但由于大气噪声、季节变化和传感器伪影的复杂相互作用，传统方法面临挑战。需要一种能够利用卫星时间序列的时间冗余性，以前所未有的灵敏度检测异常变化的方法。

Method: 基于SATLAS基础模型构建修复模型，使用全球分布的训练数据（涵盖不同气候带和土地覆盖类型）训练模型，使其能够从Sentinel-2时间序列的前期采集数据中重建最后一帧图像。当应用于发生突然地表变化的区域时，预测与观测之间的差异揭示了传统变化检测方法可能遗漏的异常。

Result: 在2023年土耳其-叙利亚地震序列引发的地表破裂检测中，该方法在Tepehan地区检测到断裂特征，比时间中值法或Reed-Xiaoli异常检测器具有更高的灵敏度和特异性。检测阈值比基线方法低约三倍。

Conclusion: 该方法为利用免费的多光谱卫星数据实现自动化、全球尺度的地表变化监测提供了一条可行路径，能够以前所未有的灵敏度检测异常变化。
...
<details>
  <summary>Details</summary>
Motivation: 传统U-Net在建模长距离上下文依赖方面存在局限，特别是在多器官场景和低对比度区域；Transformer虽然能解决此问题但计算资源需求高，不利于临床部署。需要一种既高效又能捕捉全局依赖的解决方案。

Method: 提出轻量级即插即用的分组坐标注意力模块，将通道上下文建模解耦为多个组以处理语义异质性，集成方向感知坐标编码来捕捉水平和垂直轴的结构化空间依赖。
...
TL;DR: GASeg是一个通过拓扑信息桥接外观与几何的自监督语义分割框架，使用可微分盒计数模块和多目标损失来增强对模糊外观的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 自监督语义分割方法在面对外观模糊性时经常失败，这是因为过度依赖不稳定的外观特征（如阴影、眩光、局部纹理）。需要引入稳定的拓扑信息来桥接外观和几何特征。

Method: 提出GASeg框架，核心是可微分盒计数模块，从几何特征流和外观特征流中量化多尺度拓扑统计。使用拓扑增强策略模拟真实世界模糊性，通过多目标损失强制跨模态对齐。
...
TL;DR: 提出一种利用3D高斯泼溅技术从RPO图像序列重建未知航天器3D结构的新方法，通过整合太阳位置先验知识改善光照适应性，提升几何和光度精度


<details>
  <summary>Details</summary>
Motivation: 在空间交会对接操作中，从动态光照条件下的图像序列重建未知目标航天器的3D结构面临挑战，传统3DGS方法假设静态场景，无法适应空间快速变化的光照条件

Method: 使用3D高斯泼溅表示目标几何和外观，将服务航天器估计的太阳位置先验知识整合到训练流程中，通过光度优化提升渲染图像质量，支持相机姿态估计

Result: 实验证明该方法有效，3DGS模型能够适应空间快速变化的光照条件，准确反映全局阴影和自遮挡效应，提升了下游姿态估计任务的性能

Conclusion: 通过整合太阳位置先验知识，提出的方法成功解决了空间动态光照条件下的3D重建问题，为RPO过程中的精确姿态估计提供了可靠基础
...
TL;DR: 提出CVC框架解决流式扩散编辑中的速度误差累积问题，通过双视角速度转换机制和基于经验贝叶斯推断的后验一致性更新，实现稳定的潜在轨迹和语义一致的编辑效果。


<details>
  <summary>Details</summary>
Motivation: 现有流式扩散编辑方法在源图像和目标图像分布之间进行直接转换时，潜在轨迹存在累积速度误差，导致语义不一致和结构保真度损失。
...
Conclusion: CVC框架通过数学上严谨的速度校正机制，有效解决了流式扩散编辑中的轨迹漂移问题，为分布转换提供了稳定可靠的解决方案。
...
TL;DR: FitControler是一个可学习的插件，能够集成到现代虚拟试穿模型中，实现定制化的服装合身度控制，解决了传统方法忽视服装合身度对整体风格影响的问题。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法主要关注服装细节的忠实渲染，但忽视了塑造整体风格的关键因素——服装合身度。合身度定义了服装如何与穿着者的身体对齐，是时尚设计的基本要素，对整体风格协调至关重要。

Method: 提出FitControler插件，包含两个核心组件：1) 合身度感知布局生成器，基于精心处理的服装无关表示重新绘制身体-服装布局；2) 多尺度合身度注入器，将布局线索传递给VTON模型实现布局驱动的虚拟试穿。同时构建了包含13,000个不同合身度的身体-服装对的Fit4Men数据集。

Result: 实验表明FitControler能够与各种VTON模型协同工作，实现准确的合身度控制。提出了两种合身度一致性指标来评估生成结果，并在包含不同合身度、上衣和下装、不同相机距离和身体姿势的数据集上进行了验证。

Conclusion: FitControler成功解决了虚拟试穿中的合身度控制问题，通过可学习的插件架构实现了定制化的服装合身度调整，填补了现有方法在风格协调方面的空白，为虚拟试穿技术提供了更全面的解决方案。
...
TL;DR: 提出结构引导的2D高斯泼溅分配方法，通过结构感知初始化、自适应比特量化、几何一致正则化，提升2DGS在低比特率下的率失真性能，同时保持毫秒级解码速度。


<details>
  <summary>Details</summary>
Motivation: 现有2DGS方法在分配表示容量和参数精度时忽略了图像结构信息，导致在低比特率下的率失真效率受限。需要一种能够结合图像结构指导表示分配的方法。

Method: 1. 结构引导初始化：基于自然图像的空间结构先验分配2D高斯分布，获得局部化和语义有意义的分布；2. 量化感知微调：对协方差参数进行自适应比特宽度量化，复杂区域的小尺度高斯给予更高精度；3. 几何一致正则化：对齐高斯方向与局部梯度方向以保持结构细节。

Result: 方法显著提升了2DGS的表示能力和率失真性能，同时保持超过1000 FPS的解码速度。相比基线GSImage，在Kodak数据集上降低BD-rate 43.44%，在DIV2K数据集上降低29.91%。

Conclusion: 提出的结构引导分配原则通过显式耦合图像结构与表示容量和量化精度，有效提升了2D高斯泼溅的率失真效率，同时保持了其原有的快速解码优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在遥感领域面临挑战，因为遥感图像与自然图像存在本质差异。现有遥感VLM难以提取细粒度视觉特征，并且在深度语言处理过程中容易出现视觉遗忘问题。

Method: 提出MF-RSVLM模型，通过多尺度视觉表示学习，结合全局上下文和局部细节，改进对遥感场景中复杂结构的捕捉。采用循环视觉特征注入方案，确保语言模型在生成过程中保持视觉证据的关联，减少视觉遗忘。
...
Method: 提出RSAgent智能体MLLM，通过多轮工具调用交替进行推理和分割行动：查询分割工具箱、观察视觉反馈、利用历史观察修正空间假设、重新定位目标并迭代优化掩码。构建多轮推理分割轨迹合成数据管道，采用两阶段训练框架：冷启动监督微调，然后进行具有细粒度任务特定奖励的智能体强化学习。

Result: 在ReasonSeg测试集上达到66.5% gIoU的零样本性能，比Seg-Zero-7B提升9%；在RefCOCOg上达到81.5% cIoU，在领域内和领域外基准测试中都展示了最先进的性能。

Conclusion: RSAgent通过智能体式的多轮推理-行动交互机制，有效解决了传统一次性分割方法的局限性，在文本引导对象分割任务中实现了显著的性能提升。
...
TL;DR: 本文提出了一种基于强化学习的可训练各向异性扩散框架，用于图像去噪。通过将去噪过程建模为由深度Q学习决定顺序的一系列扩散动作，该方法在多种噪声类型上超越了传统扩散方法，并与深度CNN方法竞争。


<details>
  <summary>Details</summary>
Motivation: 传统各向异性扩散方法使用显式扩散算子，难以适应复杂的图像结构，性能受限。而学习基方法（如深度CNN）虽然效果好，但缺乏扩散方法的理论优势。因此需要结合两者优势，开发具有自适应性的可训练扩散框架。

Method: 提出基于强化学习的可训练各向异性扩散框架：1）将去噪过程建模为一系列朴素扩散动作的序列；2）使用深度Q学习来学习扩散动作的顺序；3）不同迭代中选择的扩散动作构成一个具有强自适应性的随机各向异性扩散过程。

Result: 实验表明：1）该方法在三种常见噪声类型上去噪效果优于现有扩散基方法；2）与代表性的深度CNN方法竞争；3）深度Q学习在不同迭代中选择的扩散动作确实构成了对图像结构具有强自适应性的随机各向异性扩散过程。

Conclusion: 通过将强化学习与传统扩散方法结合，提出了一种有效的可训练各向异性扩散去噪器。该方法既保留了扩散方法的理论优势，又通过学习获得了对复杂图像结构的强适应性，在性能上超越了传统扩散方法，并与现代学习基方法相当。
...
Method: 提出NIRNL框架：1) 跨模态边界保持(CMP)调整正负样本对相对距离；2) 邻居感知实例精炼(NIR)通过跨模态邻居共识识别纯子集、硬子集和噪声子集；3) 针对细粒度划分构建定制化优化策略
...
TL;DR: 本文提出了一种平衡层次对比损失与解耦学习策略，用于改进细粒度遥感目标检测中的层次标签结构学习问题。


<details>
  <summary>Details</summary>
Motivation: 细粒度遥感数据集通常使用层次标签结构来从粗到细地区分对象，但将这种语义层次结构嵌入表示学习空间以改进检测性能仍然具有挑战性。先前的研究在不同层次级别应用监督对比学习，但忽视了两个关键问题：(1) 标签层次中数据分布不平衡导致高频类别主导学习过程；(2) 类别间语义关系学习干扰了与类别无关的定位任务。

Method: 提出了一种平衡层次对比损失与解耦学习策略，结合检测变换器(DETR)框架。该损失引入可学习的类别原型，并在每个层次级别平衡不同类别贡献的梯度，确保每个层次类别在每个小批次中对损失计算贡献相等。解耦策略将DETR的对象查询分为分类和定位两组，实现任务特定的特征提取和优化。

Result: 在三个具有层次标注的细粒度数据集上的实验表明，该方法优于最先进的方法。

Conclusion: 通过平衡层次对比损失和解耦学习策略，有效解决了细粒度遥感目标检测中的层次标签学习问题，提高了检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 扩散Transformer模型在视频和图像生成任务中计算成本极高，限制了实际应用。现有稀疏注意力方法存在稀疏模式预测开销大和硬件通用性不足的问题，大多数方法仅针对GPU设计。

Method: 1) 利用块状均值作为代表性token进行稀疏掩码预测；2) 实现时空感知的token排列；3) 为视频生成场景引入首帧下沉机制。这些技术共同构成了在线自适应、硬件高效的稀疏注意力机制。

Result: RainFusion2.0能够实现80%的稀疏度，在保持视频质量的同时获得1.5-1.8倍的端到端加速。该方法在各种生成模型上有效，并在不同硬件平台上验证了其泛化能力。

Conclusion: RainFusion2.0成功解决了现有稀疏注意力方法的局限性，提供了一种低开销、硬件通用的解决方案，能够显著加速视频和图像生成模型，具有广泛的实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频语言模型在事件级感知的时间定位方面存在困难，且时间定位和文本响应这两个任务通常耦合处理，缺乏清晰的逻辑层次结构，导致优化目标不理想

Method: 提出D²VLM框架，采用"先定位后回答并引用证据"的范式，引入证据标记进行证据定位；开发因子化偏好优化算法，将概率时间定位建模显式纳入优化目标；构建合成数据集支持因子化偏好学习

Result: 在各种任务上的实验表明该方法具有明显优势

Conclusion: 通过解耦时间定位和文本响应的学习，并强调它们的内在依赖关系，能够显著提升视频语言模型的事件级感知能力
...
TL;DR: 提出Latent Motion Reasoning (LMR)框架，通过两阶段"思考-行动"决策过程解决文本到动作生成中的语义-运动学阻抗失配问题，显著提升语义对齐和物理合理性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到动作生成方法将问题视为直接翻译，面临语义-运动学阻抗失配的根本理论瓶颈：难以在单次映射中将语义密集的离散语言意图转换为运动学密集的高频动作数据。

Method: 提出Latent Motion Reasoning (LMR)框架，采用两阶段"思考-行动"决策过程。核心是新颖的双粒度分词器，将动作解耦为两个不同的流形：用于规划全局拓扑的压缩语义丰富的推理潜空间，以及用于保持物理保真度的高频执行潜空间。

Result: 在T2M-GPT（离散）和MotionStreamer（连续）两个代表性基线上实现LMR，实验显示在语义对齐和物理合理性方面均获得非平凡改进，验证了最优动作规划基底是学习的动作对齐概念空间而非自然语言。

Conclusion: 通过向潜在系统2推理的架构转变，LMR有效弥合了语言与物理之间的不可言说性鸿沟，证明了将生成重新表述为两阶段决策过程是解决文本到动作生成中语义-运动学阻抗失配问题的有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 单目深度估计是自动驾驶系统的核心感知模块，但容易受到对抗攻击。现有物理攻击主要依赖纹理贴片，存在放置限制严格、真实性有限的问题，在复杂驾驶环境中效果不佳。
...
Conclusion: 该框架能够生成物理上合理的对抗物体，诱导显著的对抗深度偏移，为自动驾驶系统的安全评估提供了更有效的攻击方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在几何推理评估中存在三个主要问题：1）基于教科书的基准测试存在测试数据污染风险；2）过度强调最终答案而非推理过程；3）诊断粒度不足。需要更全面的评估框架来系统分析几何问题解决能力。

Method: 提出GeoBench分层基准测试，包含四个推理级别：视觉感知、目标导向规划、严格定理应用和自我反思回溯。通过TrustGeoGen生成六个正式验证的任务，系统评估从属性提取到逻辑错误纠正的能力范围。

Result: 实验显示：1）推理模型（如OpenAI-o3）优于通用MLLMs；2）性能随任务复杂度增加显著下降；3）子目标分解和无关前提过滤对最终问题解决准确性至关重要；4）在某些任务中，思维链提示意外地降低了性能。
...
TL;DR: 该研究系统探索了使用大语言模型（LLM）进行计算机视觉架构自动设计，提出了Few-Shot Architecture Prompting（FSAP）方法和Whitespace-Normalized Hash Validation去重技术，在7个视觉基准上生成了1900个独特架构。


<details>
  <summary>Details</summary>
Motivation: 传统神经架构搜索（NAS）计算成本高昂，而大语言模型为架构设计提供了有前景的替代方案。然而，在计算机视觉领域中，LLM在架构生成方面的应用尚未得到系统研究，特别是在提示工程和验证策略方面存在空白。

Method: 1. 提出Few-Shot Architecture Prompting（FSAP）：系统研究了支持示例数量（n=1-6）对LLM架构生成的影响；2. 引入Whitespace-Normalized Hash Validation：轻量级去重方法，比AST解析快100倍；3. 基于NNGPT/LEMUR框架，在7个计算机视觉基准上生成架构；4. 采用数据集平衡评估方法解决异构视觉任务比较的挑战。

Result: 1. 发现n=3个示例在架构多样性和上下文聚焦之间达到最佳平衡；2. 去重方法仅需不到1毫秒，防止了重复架构的冗余训练；3. 在MNIST、CIFAR-10、CIFAR-100、CelebA、ImageNette、SVHN、Places365等基准上生成了1900个独特架构；4. 建立了LLM在计算机视觉架构搜索中的实用指南和严格评估实践。

Conclusion: 该研究为计算机视觉中的LLM架构搜索提供了可操作的指导方针，建立了严格的评估实践，使计算资源有限的研究人员能够更便捷地进行自动化设计。FSAP方法和去重技术显著提高了架构生成效率和多样性。
...
TL;DR: 本文提出D²-Align框架，通过方向性解耦对齐解决文本到图像扩散模型在人类反馈强化学习中出现的偏好模式崩溃问题，在保持生成多样性的同时提升与人类偏好的对齐度。


<details>
  <summary>Details</summary>
Motivation: 现有基于人类反馈强化学习的文本到图像扩散模型虽然能在自动化奖励指标上获得高分，但往往导致偏好模式崩溃（PMC）——模型收敛到狭窄的高分输出模式（如单一风格或普遍过曝），严重损害生成多样性。

Method: 提出方向性解耦对齐（D²-Align）框架：1）在奖励模型的嵌入空间中学习方向性校正，同时保持模型冻结；2）在优化过程中将校正应用于奖励信号，防止模型崩溃到特定模式，从而保持多样性。

Result: 通过结合定性分析和质量与多样性的定量指标的综合评估表明，D²-Align在保持生成多样性的同时，实现了与人类偏好的更优对齐。

Conclusion: D²-Align框架有效缓解了文本到图像扩散模型在人类反馈强化学习中的偏好模式崩溃问题，在维持生成多样性的前提下提升了与人类偏好的对齐质量。
...
TL;DR: IMDD-1M是首个大规模工业多模态缺陷数据集，包含100万个对齐的图像-文本对，用于制造和质量检测的多模态学习。基于此数据集训练了扩散式视觉语言基础模型，通过轻量微调可高效适应专业领域。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏大规模、高质量的工业缺陷多模态数据集，限制了制造和质量检测领域多模态学习的发展。需要构建专门针对工业场景的数据集和基础模型，以支持分类、分割、检索、生成等多种应用。

Method: 1. 构建IMDD-1M数据集：包含100万个对齐的图像-文本对，涵盖60多种材料类别和400多种缺陷类型，每对数据都有专家验证的标注和细粒度文本描述。
2. 训练扩散式视觉语言基础模型：从零开始训练专门针对工业场景的扩散式视觉语言基础模型。
3. 轻量微调适配：通过少量任务特定数据（少于5%）对基础模型进行微调，适应专业领域需求。

Result: 1. 成功构建了首个大规模工业多模态缺陷数据集IMDD-1M。
2. 训练出的基础模型具有良好泛化能力，可作为通用基础。
3. 仅需少于5%的任务特定数据，就能达到专用专家模型的性能水平。
4. 展示了数据高效的基础模型适配在工业检测和生成任务中的潜力。

Conclusion: IMDD-1M数据集和相应的扩散式视觉语言基础模型为工业制造智能提供了可扩展、领域自适应和知识基础的新途径。该方法通过数据高效的基础模型适配，显著降低了工业应用中对大量标注数据的需求，推动了制造智能的发展。
...
TL;DR: DiffThinker：一种基于扩散的生成式多模态推理框架，将多模态推理重新定义为原生的图像到图像生成任务，在视觉中心任务中显著优于现有MLLMs模型。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）在多模态推理中取得进展，但其推理过程仍以文本为中心，在复杂长视野、视觉中心任务中表现不佳，需要新的推理范式。

Method: 提出生成式多模态推理范式，引入DiffThinker——基于扩散的推理框架，将多模态推理重新定义为原生的图像到图像生成任务，确保逻辑一致性和空间精度。

Result: 在四个领域（顺序规划、组合优化、约束满足、空间配置）的广泛实验中，DiffThinker显著优于领先的闭源模型：GPT-5（+314.2%）、Gemini-3-Flash（+111.6%）和微调的Qwen3-VL-32B基线（+39.0%）。

Conclusion: 生成式多模态推理是视觉中心推理的有前景方法，DiffThinker框架展现出效率、可控性、原生并行性和协作性四个核心特性，为多模态推理提供了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱图像分析面临计算瓶颈，数据量超过可用内存。现有预训练基础模型在特定领域（如近距离农业监测）中迁移效果不佳，因为光谱特征、空间尺度和语义目标存在根本差异。

Method: 提出深度全局聚类（DGC）框架，通过重叠区域的小块操作学习全局聚类结构，强制一致性，保持恒定内存使用，无需预训练。

Result: 在叶片病害数据集上，DGC实现了背景-组织分离（平均IoU 0.925），并通过可导航的语义粒度展示了无监督病害检测能力。但存在优化不稳定问题，源于多目标损失平衡。

Conclusion: DGC的设计理念有价值，但稳定实现需要动态损失平衡的原则性方法。该工作被视为智力脚手架，为进一步研究提供基础。
...
TL;DR: 本文提出了一种名为内部指导（IG）的简单有效策略，通过在训练过程中对中间层引入辅助监督，并在采样过程中外推中间层和深层输出，显著提升了扩散模型的训练效率和生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能捕捉完整的数据分布，但由于缺乏足够的训练和数据来覆盖低概率区域，模型在这些区域无法生成高质量图像。现有的指导策略如CFG会导致样本过度简化或失真，而其他基于"坏版本"的指导方法则需要精心设计的退化策略、额外训练和采样步骤。

Method: 提出内部指导（IG）策略：1）在训练过程中对中间层引入辅助监督；2）在采样过程中外推中间层和深层输出以获得生成结果。这种方法简单且无需额外训练或复杂采样步骤。

Result: 在各种基准测试中取得显著改进：在ImageNet 256x256上，SiT-XL/2+IG在80和800轮分别达到FID=5.31和1.75；LightningDiT-XL/1+IG达到FID=1.34，大幅领先其他方法；结合CFG后达到当前最先进的FID=1.19。

Conclusion: 内部指导（IG）是一种简单而有效的策略，通过引入中间层监督和外推机制，显著提升了扩散模型的训练效率和生成质量，在多个基准测试中取得了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: DiT模型在视觉生成方面表现出色，但其迭代去噪过程结合大容量导致推理成本高昂。现有研究表明DiT模型的迭代去噪过程存在大量跨步骤的冗余计算。

Method: 提出CorGi框架：1）基于贡献度评估识别低贡献transformer块；2）在间隔内缓存这些块的输出并在后续步骤中重用；3）对于文本到图像任务，提出CorGi+，利用跨注意力图识别重要token并应用部分注意力更新以保护对象细节。
...
TL;DR: ARM是一个轻量级可学习模块，通过自适应融合CLIP的层次特征来提升开放词汇语义分割性能，采用"一次训练，随处使用"范式
...
Method: 提出注意力精炼模块（ARM），使用语义引导的交叉注意力块，用鲁棒的深层特征（K，V）选择和精炼细节丰富的浅层特征（Q），然后通过自注意力块进一步处理

Result: ARM在多个基准测试中一致提升基线性能，推理开销可忽略不计，为训练免费的开放词汇语义分割建立了高效有效的范式

Conclusion: ARM作为通用即插即用后处理器，通过"一次训练，随处使用"的范式，有效解锁和精炼了CLIP的内部潜力，解决了现有训练免费方法的局限性
...
Method: 基于文本到视频扩散先验，注入预训练2D编码器的时序无关潜在特征到3D解码器以恢复细节；引入两阶段数据对齐策略（粗粒度3D对齐和细粒度2D精炼）解决分布不匹配问题。

Result: 在多样化编辑场景中实现了高真实感和时间一致性，并能泛化到其他视频到视频转换任务，为未来研究提供了可靠基准。
...
TL;DR: MambaSeg：一种使用并行Mamba编码器进行RGB和事件数据双分支语义分割的新型框架，通过双维度交互模块实现空间和时间维度的细粒度融合，在降低计算成本的同时达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: RGB相机在快速运动、低光照或高动态范围条件下性能下降，而事件相机虽然具有高时间分辨率和低延迟，但缺乏颜色和纹理信息。现有多模态融合方法计算昂贵且主要关注空间融合，忽略了事件流的时间动态特性。

Method: 提出MambaSeg双分支语义分割框架，使用并行Mamba编码器分别建模RGB图像和事件流。引入双维度交互模块（DDIM），包含跨空间交互模块（CSIM）和跨时间交互模块（CTIM），在空间和时间维度上进行细粒度融合，提高跨模态对齐并减少模糊性。

Result: 在DDD17和DSEC数据集上的广泛实验表明，MambaSeg实现了最先进的语义分割性能，同时显著降低了计算成本，展示了其在高效、可扩展和鲁棒的多模态感知方面的潜力。

Conclusion: MambaSeg通过创新的双分支Mamba编码器架构和双维度交互模块，有效融合RGB和事件数据的互补特性，在保持计算效率的同时提升了语义分割性能，为多模态感知系统提供了有前景的解决方案。
...
TL;DR: PGMP框架通过物理基础流形投影解决牙科CBCT金属伪影问题，结合解剖自适应物理模拟、DMP-Former确定性重建和语义结构对齐模块，实现高效、可靠的伪影去除。


<details>
  <summary>Details</summary>
Motivation: 牙科CBCT中的金属伪影严重遮挡解剖结构，影响诊断。现有深度学习方法存在局限：监督方法因"回归到均值"导致光谱模糊，无监督方法存在结构幻觉风险，而扩散模型虽然真实但采样缓慢，不适合临床应用。

Method: 提出物理基础流形投影(PGMP)框架：1) 解剖自适应物理模拟(AAPS)通过蒙特卡洛光谱建模和患者特异性数字孪生合成高保真训练对；2) DMP-Former采用直接x预测范式，将恢复重构为确定性流形投影，单次前向传播即可恢复干净解剖结构；3) 语义结构对齐(SSA)模块利用医学基础模型(MedDINOv3)的先验锚定解决方案。

Result: 在合成和多中心临床数据集上的实验表明，PGMP在未见解剖结构上优于现有最先进方法，在效率和诊断可靠性方面设定了新的基准。

Conclusion: PGMP框架成功解决了牙科CBCT金属伪影去除的关键挑战，通过物理基础模拟、确定性重建和语义对齐的结合，实现了高效、可靠的临床应用。
...
TL;DR: DualityForge框架通过可控扩散视频编辑合成反事实视频数据，结合DNA-Train训练方法，显著减少多模态大语言模型在视频理解中的幻觉问题。
...
Conclusion: DualityForge框架通过合成反事实视频数据和DNA-Train训练方法，有效解决了MLLM在视频理解中的幻觉问题，提高了模型的鲁棒性和泛化能力。作者承诺开源数据集和代码。
...
TL;DR: 提出了一种空间提升的全景拼接框架，将2D图像提升为3D点云表示，通过统一投影中心生成几何一致的360°全景图，有效减少视差和遮挡场景中的几何失真和重影。


<details>
  <summary>Details</summary>
Motivation: 传统基于2D单应性变换和网格变形的图像拼接方法在处理真实3D场景（多深度层和遮挡）时，在非重叠区域会产生重影、结构弯曲和拉伸失真，特别是在多视角累积和360°闭环拼接场景中问题更加严重。

Method: 1. 将输入图像提升为统一坐标系中的密集3D点表示，通过置信度指标进行全局跨视图融合；2. 在3D空间中建立统一投影中心，采用等距柱面投影将融合数据映射到单一全景流形；3. 在画布域进行空洞填充以处理视角转换揭示的未知区域，恢复连续纹理和语义一致性。
...
TL;DR: EndoRare是一个一次性、无需重新训练的生成框架，能够从单个参考图像合成多样化的高保真罕见胃肠道病变图像，用于AI模型训练和临床教育。
...
Method: 通过语言引导的概念解耦，将病变特征与非诊断属性分离，将前者编码为可学习的原型嵌入，同时变化后者以确保多样性。

Result: 合成图像被专家判断为临床可信，用于数据增强时显著提升下游AI分类器性能，新手内镜医生接触EndoRare生成病例后召回率提高0.400，精确率提高0.267。
...
TL;DR: 开发了Virtual-Eyes CT质量控制流程，显著提升通用基础模型在低剂量CT肺癌筛查中的性能，但对专业模型有负面影响


<details>
  <summary>Details</summary>
Motivation: 深度学习在低剂量CT肺癌筛查中很少量化鲁棒预处理的影响，需要开发临床导向的质量控制流程来评估其对不同类型模型的影响

Method: 开发Virtual-Eyes 16位CT质量控制流程：强制512x512平面分辨率、筛选诊断性序列、提取连续肺块（使用亨氏单位过滤和双侧肺覆盖评分）。使用765名NLST患者数据，评估RAD-DINO、Merlin、Sybil和ResNet-18模型在原始输入与Virtual-Eyes处理输入下的性能

Result: Virtual-Eyes显著提升RAD-DINO性能：切片级AUC从0.576提升至0.610，患者级AUC从0.646提升至0.683（平均池化）和0.619提升至0.735（最大池化）。但Sybil和ResNet-18性能下降（Sybil AUC从0.886降至0.837），Merlin转移性有限（AUC约0.507-0.567）

Conclusion: 解剖学导向的质量控制可以稳定和改进通用基础模型工作流程，但可能破坏适应原始临床环境的专业模型，表明预处理策略需要根据模型类型进行定制
...
Method: 提出UniAct两阶段框架：1）集成微调的多模态大语言模型（MLLM）；2）采用因果流式处理管道。通过FSQ共享离散码本统一输入，确保跨模态对齐，同时将动作约束在物理可行的流形上。

Result: 在UniMoCap（20小时人形动作基准）上验证，实现了低于500毫秒的延迟，零样本跟踪不完美参考动作的成功率提升了19%，在多样化现实场景中展现出鲁棒的泛化能力。

Conclusion: UniAct标志着向响应式、通用人形助手迈出了关键一步，通过统一的感知和控制实现了无缝交互，为人形机器人的多模态指令执行提供了有效解决方案。
...
TL;DR: LVLDrive是一个将LiDAR点云作为额外输入模态的框架，旨在增强现有视觉语言模型在自动驾驶中的3D度量空间理解能力，通过渐进融合机制避免对预训练模型的知识干扰。


<details>
  <summary>Details</summary>
Motivation: 当前基于图像的视觉语言模型在自动驾驶中存在瓶颈，主要问题是缺乏准确的度量空间推理和几何推断能力，导致驾驶策略不可靠，需要引入3D度量数据来构建更可信的系统。

Method: 提出LVLDrive框架，通过渐进融合Q-Former逐步注入LiDAR特征，避免对预训练VLM造成灾难性干扰；同时开发空间感知问答数据集来显式教授模型高级3D感知和推理能力。

Result: 在多个驾驶基准测试中，LVLDrive相比纯视觉方法在场景理解、度量空间感知和可靠驾驶决策方面都表现出优越性能。

Conclusion: 研究表明，显式的3D度量数据对于构建可信赖的基于VLM的自动驾驶系统是必要的，LVLDrive通过融合LiDAR点云有效提升了模型的3D空间理解能力。
...
TL;DR: DermaVQA-DAS扩展了DermaVQA数据集，引入皮肤病评估框架DAS，支持封闭式问答和皮肤病变分割任务，提供多模态模型基准测试结果。
...
Method: 开发了皮肤病评估框架DAS，包含36个高级和27个细粒度评估问题，提供专家标注的封闭式问答和分割数据集，并评估了多种多模态模型和提示策略。

Result: 分割任务中，不同提示策略影响性能：默认提示在Mean-of-Max和Mean-of-Mean评估中表现最佳，增强提示在多数投票评估中Jaccard指数达0.395，Dice得分0.566。封闭式问答中，o3模型准确率最高（0.798），GPT-4.1（0.796）和Gemini-1.5-Pro（0.783）表现优异。

Conclusion: DermaVQA-DAS填补了患者中心皮肤病视觉语言建模的空白，通过标准化评估框架和公开数据集支持未来研究。
...
### [44] [The Mechanics of CNN Filtering with Rectification](https://arxiv.org/abs/2512.24338)
*Liam Frija-Altrac,Matthew Toews*

Main category: cs.CV

TL;DR: 该论文提出了一种基于信息力学的新模型，将卷积滤波的机械特性与相对论和量子力学中的能量-动量关系联系起来，通过分析核函数的奇偶分量来解释信息传播的基本模式。


<details>
  <summary>Details</summary>
Motivation: 受狭义相对论和量子力学物理理论的启发，作者希望建立一个新的模型来理解卷积滤波与整流操作的机械特性，探索通用CNN中信息处理与物理学基本原理之间的联系。

Method: 将卷积核分解为正交的偶分量和奇分量，偶分量导致图像内容各向同性扩散并保持质心，奇分量导致质心方向性位移。通过离散余弦变换在频域分析奇偶特性，特别关注小卷积滤波器（如3×3像素）中由低频基（DC Σ和梯度分量∇）主导的结构。

Result: 发现信息位移速度与奇分量与总核能量的比值呈线性关系，小卷积滤波器的结构主要由低频基主导，这些基定义了信息传播的基本模式。首次证明了通用CNN中信息处理与相对论物理中能量-动量关系之间的联系。

Conclusion: 该研究提出了基本信息力学作为理解卷积滤波机械特性的新框架，建立了深度学习与物理学基本原理之间的桥梁，为理解CNN中的信息传播机制提供了新的视角。
...
TL;DR: 该论文提出了一个用于多模态预训练的综合框架，旨在解决自动驾驶系统中摄像头和LiDAR等多传感器数据融合的挑战，构建统一的空间智能理解。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆和无人机等自主系统的快速发展，迫切需要从多模态车载传感器数据中构建真正的空间智能。虽然基础模型在单模态场景中表现出色，但将摄像头、LiDAR等不同传感器的能力整合起来创建统一理解仍然是一个巨大挑战。

Method: 提出了一个多模态预训练的综合框架，分析了基础传感器特性与学习策略之间的相互作用，评估了平台特定数据集的作用。核心贡献是制定了预训练范式的统一分类法：从单模态基线到学习整体表示的复杂统一框架，用于3D目标检测和语义占用预测等高级任务。还研究了文本输入和占用表示的集成，以促进开放世界感知和规划。

Result: 论文识别了推动多模态预训练进展的核心技术集，并提出了一个统一的分类法来组织不同的预训练范式。通过分析传感器特性、学习策略和数据集的作用，为构建统一的空间智能理解提供了系统框架。

Conclusion: 论文确定了计算效率和模型可扩展性等关键瓶颈，并提出了实现通用多模态基础模型的路线图，这些模型能够实现稳健的空间智能，用于实际部署。为自动驾驶系统的多传感器融合和空间智能发展指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于分块的方法需要完整的非因果上下文窗口，引入显著延迟，无法满足实时对话中听者非语言反馈的即时性需求

Method: 采用流友好的自回归框架，结合流匹配头进行概率建模；提出因果编码器增强设计，通过前瞻模块引入短期未来上下文（如60ms）以提升质量同时保持低延迟

Result: 每帧生成时间34ms，系统总延迟低于100ms；在HDTF数据集上获得离线8.13和在线7.61的LipSync Confidence分数，达到最先进的唇同步质量

Conclusion: DyStream通过创新的因果编码器设计和流匹配自回归框架，成功实现了高质量、低延迟的双人对话视频实时生成，解决了现有方法延迟过高的问题
...
TL;DR: 提出基于AI的微血管吻合术自动评估框架，使用改进的TimeSformer视频Transformer架构，结合YOLO目标检测追踪，实现87.7%动作分割准确率和76%专家评估分类准确率。


<details>
  <summary>Details</summary>
Motivation: 传统手术技能评估依赖专家监督，存在主观性强、评估者间差异大、耗时耗力等问题，尤其在低收入国家难以规模化应用，需要客观、一致、可扩展的自动评估方法。

Method: 采用改进的TimeSformer视频Transformer架构，加入层次化时间注意力和加权空间注意力机制进行动作识别；使用YOLO目标检测和追踪方法提取精细运动特征；从五个技能维度评估微血管吻合术表现。

Result: 在58个专家标注视频数据集上验证，动作分割帧级准确率达87.7%（后处理提升至93.62%），在五个技能维度上复制专家评估的平均分类准确率为76%。

Conclusion: 该AI驱动框架能够提供客观、一致、可解释的反馈，有望实现更标准化、数据驱动的手术培训评估，解决传统方法的主观性和可扩展性问题。
...
TL;DR: 本文通过离散小波变换分析Vision Transformer编码器的组合性，发现单层DWT分解的基元在潜在空间中近似组合，为ViT信息组织提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 虽然对Transformer模型的理解主要来自语言任务分析，但本文旨在通过组合性视角研究Vision Transformer编码器学习到的表示，探索视觉任务中ViT的信息组织方式。

Method: 引入一个框架来测量ViT编码器中的组合性，关键工具是离散小波变换，用于获取视觉设置中依赖于输入的基元。通过检查组合表示重现原始图像表示的能力，实证测试表示空间中组合性的程度。

Result: 研究发现，单层DWT分解产生的基元在编码器表示中能够在潜在空间中近似组合，这表明ViT以某种组合方式组织视觉信息。

Conclusion: 这项工作为理解Vision Transformer如何结构化信息提供了新的视角，表明DWT基元在ViT表示空间中表现出组合性特征，有助于解释ViT在视觉任务中的工作机制。
...
Method: 包含两个互补模块：1) iSWGE（谱间窗口图嵌入），通过图节点表示光谱通道并学习边缘特征来显式建模谱间关系；2) WSGA-C（窗口空间图注意力与卷积块注意力），结合稀疏图注意力和卷积注意力来减少空间冗余并强调精细结构

Result: 在SDOML数据集六个极紫外通道上的评估显示：MSID降低20.15%，PSNR提升最高1.09%，log变换MS-SSIM增益1.62%，在相同比特率下提供更清晰且光谱保真的重建

Conclusion: 提出的学习图像压缩框架有效提升了多光谱太阳图像的压缩质量，在保持光谱保真度和空间细节方面优于现有方法
...
TL;DR: EVOL-SAM3：一个零样本推理分割框架，将推理分割重新定义为推理时进化搜索过程，通过"生成-评估-进化"循环迭代优化提示假设，显著超越现有方法。
...
Method: 提出EVOL-SAM3零样本框架，将推理分割重新定义为推理时进化搜索过程。维护提示假设种群，通过"生成-评估-进化"循环迭代优化：1)视觉竞技场进行无参考成对竞赛评估提示适应性；2)语义变异算子注入多样性并纠正语义错误；3)异构竞技场模块整合几何先验与语义推理确保鲁棒最终选择。

Result: 在挑战性的ReasonSeg基准测试中，EVOL-SAM3不仅显著超越静态基线方法，还在零样本设置下显著超过完全监督的最先进方法。

Conclusion: EVOL-SAM3通过将推理分割重新定义为进化搜索过程，解决了现有方法的局限性，实现了更强大的零样本推理分割能力，为复杂视觉语言任务提供了新的解决方案。
...
Method: 开发了一个集成YOLOv5和YOLOv8进行胸部X光图像异常检测的管道，将检测结果（边界框和类别标签）输入大型语言模型（GPT-4）生成自然语言放射学报告。比较了两个YOLO模型在检测精度、推理延迟方面的性能，并评估了生成文本的质量。

Result: AI生成报告与人工报告在语义相似性方面表现良好。人类评估显示GPT-4在清晰度方面得分很高（4.88/5），但在自然写作流畅性方面得分较低（2.81/5），表明当前系统能达到临床准确性，但在风格上与放射科医生撰写的文本仍有区别。

Conclusion: 该研究展示了结合目标检测模型和大型语言模型生成放射学报告的可行性，系统在临床准确性方面表现良好，但在文本风格的自然性方面仍需改进，为未来医学影像报告自动化提供了有前景的方向。
...
TL;DR: 提出一种通过遗憾最小化和无导数优化改进多元互信息估计的新方法，使基于2D视觉输入训练的跨模态系统能够在线适应3D场景中的物体遮挡和特征区分
...
Result: 该方法使现成的跨模态系统能够在线适应物体遮挡和区分特征，无需预训练或微调即可在多物体3D场景的跨模态任务中提升性能

Conclusion: 提出的算法成功解决了2D到3D的维度转换问题，通过改进的互信息估计和相机控制策略，实现了跨模态系统在复杂3D场景中的有效适应
...
TL;DR: 提出PhyGDPO框架，通过物理增强数据管道和基于物理的奖励优化，解决文本到视频生成中物理一致性不足的问题


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法虽然视觉质量不错，但难以遵循物理规律，主要受限于训练数据中物理交互的稀缺性和现有方法在复杂物理推理上的局限性

Method: 1. 提出PhyAugPipe物理增强数据管道，利用视觉语言模型和思维链推理构建大规模训练数据集PhyVidGen-135K；2. 设计PhyGDPO框架，基于分组Plackett-Luce概率模型进行物理感知的偏好优化；3. 引入物理引导奖励机制和LoRA-Switch Reference方案提高训练效率

Result: 在PhyGenBench和VideoPhy2基准测试中显著优于现有开源方法，证明了物理一致性生成的有效性

Conclusion: 通过物理增强数据构建和物理感知优化框架，成功提升了文本到视频生成的物理一致性，为物理可信的视频生成提供了有效解决方案
...
Conclusion: 该二阶优化算法有效解决了深度学习中的大规模优化问题，在视觉定位任务中展现出显著优势，为相关领域提供了高效的优化解决方案。
...
TL;DR: 该论文提出DARFT框架，通过决策模糊性引导的强化微调来解决CDVQA中的决策模糊问题，在少样本设置下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现CDVQA中许多失败案例并非源于明显错误预测，而是决策模糊性——模型对正确答案和强干扰项赋予相似置信度。这种决策模糊样本阻碍了模型判别能力和鲁棒性。

Method: 提出DARFT框架：1）使用SFT训练的参考策略挖掘决策模糊样本；2）在挖掘的子集上应用组相对策略优化，通过多样本解码和组内相对优势抑制强干扰项，锐化决策边界。

Result: 大量实验表明DARFT在CDVQA任务上相比SFT基线获得一致性能提升，特别是在少样本设置下效果显著。

Conclusion: 明确优化决策模糊样本对提升CDVQA模型判别性和鲁棒性至关重要，DARFT框架通过强化微调有效解决了这一问题，无需额外监督。
...
### [56] [HaineiFRDM: Explore Diffusion to Restore Defects in Fast-Movement Films](https://arxiv.org/abs/2512.24946)
*Rongji Xun,Junjie Yuan,Zhongjie Wang*

Main category: cs.CV

TL;DR: HaineiFRDM是一个基于扩散模型的电影修复框架，通过分块训练策略处理高分辨率影片，引入全局提示和帧融合模块，使用全局-局部频率模块保持纹理一致性，并采用全局残差减少块状伪影。
...
Method: 采用分块训练和测试策略使24GB显存GPU能处理高分辨率影片；设计位置感知的全局提示和帧融合模块；引入全局-局部频率模块重建不同分块间的一致性纹理；先修复低分辨率结果作为全局残差以减少分块伪影；构建包含真实修复影片和合成数据的电影修复数据集。

Result: 综合实验结果表明，该模型在缺陷修复能力上明显优于现有开源方法。

Conclusion: HaineiFRDM框架成功探索了扩散模型在电影修复中的应用，通过创新的分块策略和模块设计实现了高质量的高分辨率影片修复，代码和数据集将开源。
...
TL;DR: SliceLens：一个基于LLM/VLM的假设驱动框架，用于在多实例视觉任务中发现细粒度错误切片，并提出了首个针对实例级任务的细粒度切片发现基准FeSD。


<details>
  <summary>Details</summary>
Motivation: 现有切片发现方法主要针对图像分类任务，难以应用于检测、分割、姿态估计等多实例任务。真实场景中的错误切片通常涉及复杂视觉关系，现有实例级方法缺乏细粒度推理能力。当前基准测试要么针对特定算法，要么偏向图像分类，且使用人工标注的真实性不足。

Method: 提出SliceLens框架，利用LLM和VLM通过基于视觉的推理生成和验证多样化的失败假设，实现细粒度、可解释的错误切片识别。同时构建FeSD基准，包含专家标注的细粒度切片，精确关联到局部错误区域。

Result: 在现有基准和FeSD上的实验表明，SliceLens达到最先进性能，在FeSD上将Precision@10提高了0.42（0.73 vs. 0.31）。识别出的可解释切片有助于实际模型改进，模型修复实验验证了其有效性。

Conclusion: SliceLens通过假设驱动的视觉推理方法，有效解决了多实例视觉任务中的细粒度错误切片发现问题，为模型评估和修复提供了可靠工具。FeSD基准为未来研究提供了更真实的评估标准。
...
TL;DR: 该研究针对自然灾害后评估需求，构建了首个基于无人机影像的飓风灾害3D点云数据集，并测试了现有SOTA 3D语义分割模型在灾后场景中的性能表现，发现现有方法存在显著局限性。
...
Method: 1. 使用无人机采集飓风伊恩（2022）受灾区域的航拍影像
2. 采用SfM（运动恢复结构）和MVS（多视角立体）技术重建3D点云
3. 构建专门的3D灾后场景数据集
4. 在数据集上评估三种SOTA 3D语义分割模型：FPT、PTv3和OA-CNNs

Result: 现有最先进的3D语义分割模型在灾后场景数据集上表现出显著局限性，无法有效处理灾后环境的特殊特征和复杂性。

Conclusion: 迫切需要开发专门的3D基准数据集和推进3D分割技术进步，以提升灾后场景理解和响应能力，填补现有研究空白。
...
TL;DR: CPJ框架通过结构化图像描述和LLM评判模块，无需训练即可提升农业病虫害VQA性能，在疾病分类和问答得分上显著优于基线方法。
...
Method: 提出Caption-Prompt-Judge（CPJ）框架：1）使用大型视觉语言模型生成多角度图像描述；2）通过LLM-as-Judge模块迭代优化描述；3）基于优化后的描述进行双答案VQA，同时提供识别和管理建议。

Result: 在CDDMBench评估中，使用GPT-5-mini描述时，GPT-5-Nano在疾病分类上提升22.7个百分点，在QA得分上提升19.5分，显著优于无描述基线，且提供透明、基于证据的推理。

Conclusion: CPJ框架无需微调即可实现准确、可解释的农业病虫害诊断，通过结构化描述和LLM评判机制提升了模型在领域转移下的鲁棒性和透明度。
...
Method: 提出协作低秩适应(CLoRA)，包含基础空间共享和样本无关多样性增强(SADE)组件。基础空间共享让所有低秩模块共享一组下/上投影空间，SADE则通过正则化矩阵间相似性来鼓励多样化的表示学习。

Result: 在广泛使用的图像和点云数据集上的实验表明，CLoRA在学习性能和参数效率之间取得了更好的平衡，在点云分析中需要最少的GFLOPs。

Conclusion: CLoRA通过协作低秩适应机制，在保持参数效率的同时提升了学习能力，为预训练视觉Transformer的微调提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要在理想光照条件下评估视觉语言模型，但实际应用中需要模型在黑暗环境等视觉退化条件下也能稳定工作，这一需求被长期忽视。

Method: DarkEQA通过模拟物理真实的低光照条件（在RAW空间建模光照衰减和传感器噪声，配合ISP渲染流程），评估模型在可控视觉退化下的问答能力，从而分离感知瓶颈。

Result: 评估了多种最先进的视觉语言模型和低光照图像增强模型，系统揭示了这些模型在挑战性视觉条件下的局限性。

Conclusion: DarkEQA填补了视觉语言模型在黑暗环境评估方面的空白，为可归因的鲁棒性分析提供了工具，代码和数据集将在论文接受后发布。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位的研究主要集中在室内和室外驾驶场景，而户外监控场景由于缺乏路边基础设施传感器采集的配对点云-文本数据，一直未被探索。路边基础设施系统需要理解自然语言并在复杂交通环境中定位相关目标物体，这对智能交通系统至关重要。

Method: 提出了Moni3DVG端到端方法，该方法利用图像提供的丰富外观信息，以及点云提供的几何和光学信息，进行多模态特征学习和3D物体定位。同时构建了MoniRefer数据集，包含约136,018个物体和411,128个自然语言描述，数据来自真实世界复杂交通路口。

Result: 在提出的基准测试上进行了大量实验和消融研究，证明了所提方法的优越性和有效性。数据集包含高质量的人工验证的3D标签和语言描述。

Conclusion: 本文开创了户外监控场景的3D视觉定位新任务，提供了首个真实世界大规模多模态数据集，并提出了有效的端到端方法，为基础设施级别的交通场景理解提供了新途径，超越了传统的车辆视角。
...
Method: 1）构建新的FireRescue数据集，覆盖城市、山区、森林和水域多种救援场景，包含8个关键类别，共15,980张图像和32,000个边界框；2）提出FRS-YOLO改进模型，引入即插即用的多维协同增强注意力模块增强易混淆类别的判别表示，集成动态特征采样器强化高响应前景特征。

Result: 实验结果表明，消防救援场景中的目标检测具有高度挑战性，所提方法有效提升了YOLO系列模型在此场景下的检测性能。

Conclusion: 通过构建全面的消防救援数据集和提出针对性的改进模型，解决了现有研究在场景覆盖和目标类别方面的不足，为消防救援指挥决策提供了更有效的技术支持。
...
TL;DR: RadAR提出了一种基于径向拓扑的并行化自回归视觉生成框架，通过环形分组和嵌套注意力机制，在保持表示能力的同时显著提升推理效率。
...
Method: 提出RadAR框架：1) 采用径向拓扑组织生成过程，以初始token为中心，将所有其他token按空间距离分组到多个同心环中；2) 按环从内到外并行预测同一环内的所有token；3) 引入嵌套注意力机制，在前向传播过程中动态修正不合理输出，减轻错误累积。

Result: 该方法在保持视觉场景结构局部性和空间连贯性的同时，显著提高了并行化程度，从而大幅提升了生成效率。

Conclusion: 通过结合径向并行预测和动态输出修正，RadAR在保持自回归模型表示能力的同时，有效解决了传统自回归视觉生成的低效问题。
...
TL;DR: SpaceTimePilot是一个视频扩散模型，能够解耦空间和时间控制，实现可控制的生成式渲染。给定单目视频，它可以独立改变相机视角和运动序列，在生成过程中重新渲染场景，实现跨空间和时间的连续任意探索。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时独立控制视频生成中的空间（相机视角）和时间（运动序列）维度。缺乏同时提供连续时间变化和空间视角变化的数据集，限制了视频生成模型对时空解耦控制的能力。

Method: 1. 引入有效的动画时间嵌入机制，在扩散过程中显式控制输出视频相对于源视频的运动序列；2. 提出简单有效的时间扭曲训练方案，重新利用现有的多视角数据集模拟时间差异；3. 改进相机条件机制，允许从第一帧开始改变相机视角；4. 创建CamxTime数据集，第一个提供场景内完全自由时空视频轨迹的合成时空全覆盖渲染数据集。

Result: 在真实世界和合成数据上的评估表明，SpaceTimePilot实现了清晰的时空解耦，与先前工作相比表现出更强的结果。联合训练时间扭曲方案和CamxTime数据集产生了更精确的时间控制。

Conclusion: SpaceTimePilot通过创新的时间嵌入机制、时间扭曲训练方案和专门的CamxTime数据集，成功实现了视频生成中空间和时间的解耦控制，为可控生成式渲染提供了新的解决方案。
...
TL;DR: Splatwizard是一个专门用于评估3D高斯泼溅压缩模型的统一基准工具包，旨在解决现有基准测试缺乏标准化和全面评估指标的问题。


<details>
  <summary>Details</summary>
Motivation: 随着3D高斯泼溅技术的快速发展，基于3DGS的算法激增，但缺乏专门针对压缩任务的标准化和全面评估工具。现有基准测试往往缺少评估不同方法独特特征（如渲染速度、率失真权衡、内存效率和几何精度）所需的特定指标。

Method: Splatwizard提供了一个易于使用的框架，用于实现新的3DGS压缩模型并利用先前工作的最先进技术。该框架包含一个集成管道，可自动计算关键性能指标，包括基于图像的质量指标、重建网格的倒角距离、渲染帧率和计算资源消耗。

Result: 开发了一个统一的基准工具包，代码已在GitHub上开源提供。该工具包能够全面评估3DGS压缩模型的多个维度性能。

Conclusion: Splatwizard填补了3DGS压缩模型评估领域的空白，为研究人员提供了一个标准化、全面的基准测试框架，有助于推动该领域的发展。
...
TL;DR: 本文提出了一种统一框架，将3D高斯溅射与可学习特征嵌入相结合，通过"嵌入到标签"过程实现3D实例分割，解决了多视角2D标签不一致问题，并通过边界硬挖掘和线性层稳定化提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D实例分割方法面临多视角2D标签不一致的问题，导致3D预测质量差。现有方法要么使用两阶段方法（依赖超参数敏感的对比学习聚类），要么预处理标签以确保一致性，这些方法效率低下且性能有限。

Method: 提出统一框架：1) 为高斯基元引入可学习特征嵌入进行分割；2) 通过新颖的"嵌入到标签"过程高效解码为实例标签；3) 针对边界伪影问题，提出边界硬挖掘策略，并在计算三元组损失前对光栅化特征嵌入应用线性层以稳定训练。

Result: 在ScanNet、Replica3D和Messy-Rooms数据集上，该方法在定性和定量评估中均优于基线方法，减少了训练时间并提高了性能。

Conclusion: 提出的统一框架有效解决了多视角2D标签不一致问题，通过可学习特征嵌入和边界优化策略，实现了高效且高质量的3D实例分割，在多个数据集上展现了优越性能。
...
<details>
  <summary>Details</summary>
Motivation: Noise2Noise方法虽然无需干净目标图像，但存在主要限制：对噪声目标应用非线性函数会导致结果偏差。由于非线性函数在图像处理中很常见，这限制了可对噪声目标进行的预处理类型。特别是在高动态范围(HDR)图像去噪中，异常值会严重影响训练效果。

Method: 开发了一个理论框架来分析非线性函数的影响，描述了一类具有最小偏差的非线性函数。针对HDR图像去噪问题，研究了特定的损失函数和色调映射函数组合，这些组合可以减少异常值影响同时引入最小偏差。将此方法应用于现有的基于机器学习的蒙特卡洛去噪器。

Result: 该方法在蒙特卡洛渲染产生的HDR图像去噪中表现出色。虽然原始实现使用高样本数参考图像进行训练，但新方法仅使用噪声训练数据就能接近原始实现的性能，成功解决了HDR图像训练中的异常值问题。

Conclusion: 某些非线性函数可以在Noise2Noise训练中应用于噪声目标图像而不引入显著偏差。通过选择合适的损失函数和色调映射函数组合，可以有效处理HDR图像去噪中的异常值问题，扩展了Noise2Noise方法的应用范围。
...
TL;DR: CropTrack是一个用于农业环境多目标跟踪的新框架，通过结合外观和运动信息来解决农业场景中重复模式、相似外观和频繁遮挡的挑战，相比传统运动跟踪方法在身份保持方面表现更好。


<details>
  <summary>Details</summary>
Motivation: 农业环境中的多目标跟踪面临重复模式、相似物体外观、突然光照变化和频繁遮挡等主要挑战。当前农业跟踪器主要依赖运动信息而非外观进行关联，但在目标频繁遮挡时难以保持物体身份。由于物体外观高度相似，将外观关联整合到农业场景中具有挑战性。

Method: CropTrack提出一个新颖的多目标跟踪框架，结合外观和运动信息。主要方法包括：1）重新排序增强的外观关联；2）一对多关联与基于外观的冲突解决策略；3）指数移动平均原型特征库来改进基于外观的关联。

Result: 在公开可用的农业多目标跟踪数据集上评估，CropTrack展示了持续的身份保持能力，优于传统的基于运动的跟踪方法。与最先进方法相比，CropTrack在识别F1分数和关联准确率方面取得显著提升，同时身份切换数量更低。

Conclusion: CropTrack通过有效结合外观和运动信息，解决了农业环境中多目标跟踪的挑战，特别是在保持物体身份方面表现出色，为农业场景中的目标跟踪提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在各种视觉语言任务中表现出色，但作为具身智能体（需要多轮对话、空间推理和顺序动作预测）的性能仍需探索。本文旨在系统评估MLLMs在视觉语言导航任务中的零样本能力。

Method: 提出VLN-MME统一评估框架，将传统导航数据集标准化为基准测试。采用高度模块化和可访问的设计简化评估，支持跨不同MLLM架构、智能体设计和导航任务的结构化比较和组件级消融实验。

Result: 研究发现，在基线智能体上增强思维链推理和自我反思能力反而导致性能下降，这表明MLLMs在具身导航任务中上下文感知能力较差。虽然MLLMs能够遵循指令并结构化输出，但其3D空间推理保真度较低。
...
TL;DR: FinMMDocR是一个用于评估多模态大语言模型在真实世界金融数值推理任务上的双语多模态基准，包含1200个专家标注的问题，涵盖12种金融场景和9类文档类型，要求多步骤推理和跨页面证据检索。


<details>
  <summary>Details</summary>
Motivation: 现有基准在真实世界金融数值推理任务上存在不足，特别是在场景意识、文档理解和多步骤计算方面。金融领域需要处理复杂的多模态文档和专家级推理，而现有基准无法充分评估模型在这些复杂场景下的能力。

Method: 构建了包含1200个专家标注问题的双语多模态基准，其中57.9%的问题融入了12种隐式金融场景（如投资组合管理）。基准包含837份中英文文档，涵盖9种类型，平均每份50.8页。问题平均需要11步推理（5.3步信息提取+5.7步计算），65%的问题需要跨页面证据检索。

Result: 最佳性能的MLLM仅达到58.0%的准确率，不同的检索增强生成方法在该任务上表现出显著性能差异。这表明显有模型在处理复杂金融多模态推理任务方面仍有很大提升空间。

Conclusion: FinMMDocR基准填补了现有评估体系的空白，能够推动MLLM和推理增强方法在真实世界复杂多模态推理任务上的改进，特别是在金融领域需要专家级推理和复杂文档理解的场景。
...
Result: 该方法在域自适应任务中表现优于线性探测和现有SOTA域自适应技术，且仅需极少的标注预算。

Conclusion: 通过神经元激活模式选择代表性样本进行标注，结合持续学习技术，可以实现高效且有效的LiDAR域自适应，显著提升跨域性能。
...
TL;DR: ProDM是一种基于扩散模型的生成框架，能够从非门控胸部CT中恢复无运动伪影的钙化病灶，提高冠状动脉钙化评分的准确性。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉钙化评分是心血管疾病风险评估的重要工具，但常受心脏和呼吸运动伪影影响。虽然ECG门控心脏CT能减少伪影，但在人群筛查和常规成像中应用受限。非门控胸部CT虽更易获取，但运动伪影更严重，需要有效的方法来恢复无伪影的钙化病灶。

Method: ProDM包含三个关键组件：1）CAC运动模拟数据引擎，从心脏门控CT合成具有多样运动轨迹的非门控采集数据；2）属性感知学习策略，通过可微钙化一致性损失融入钙化特异性先验；3）渐进校正方案，在扩散步骤中逐步减少伪影以增强稳定性和钙化保真度。

Result: 在真实患者数据集上的实验表明，ProDM显著提高了CAC评分准确性、空间病灶保真度和风险分层性能。在真实非门控扫描上的读者研究进一步证实ProDM能抑制运动伪影并提高临床可用性。

Conclusion: ProDM展示了渐进式、属性感知框架在常规胸部CT成像中实现可靠CAC量化的潜力，为心血管疾病风险评估提供了更易获取的解决方案。
...
TL;DR: VIPER：首个面向生成式视频推理的过程感知评估基准，包含16个任务和新的POC@r指标，揭示当前视频模型存在约20%的过程-结果一致性，存在显著的结果作弊问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型展现出链式帧推理能力，但现有评估框架多基于单帧评估，容易导致结果作弊——模型通过错误过程得出正确结论。需要建立过程感知的评估范式来准确衡量生成式视频推理的真实能力。

Method: 提出VIPER基准，涵盖16个任务，涉及时间、结构、符号、空间、物理和规划推理。提出过程-结果一致性指标POC@r，使用VLM-as-Judge和分层评分标准来评估中间步骤有效性和最终结果。

Result: 实验显示最先进的视频模型仅达到约20%的POC@1.0，存在显著的结果作弊现象。测试时缩放和采样鲁棒性分析揭示了当前视频生成与真正广义视觉推理之间的巨大差距。

Conclusion: VIPER基准填补了生成式视频推理评估的空白，揭示了当前模型的局限性，为未来研究提供了过程感知的评估框架，有助于推动真正智能的视频推理系统发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统终身行人重识别方法需要为历史图库图像重新提取特征（重新索引），但这面临数据隐私问题和高昂的计算成本。特征不兼容问题严重影响了重识别性能，因此需要一种无需重新索引的解决方案。

Method: 提出双向连续兼容表示框架（Bi-C2R），通过持续更新旧模型提取的图库特征，使新旧模型输出的特征相互兼容，实现高效的终身行人重识别。
...
<details>
  <summary>Details</summary>
Motivation: 音频驱动的视觉配音面临缺乏理想训练数据的根本挑战：需要成对的视频，其中只有唇部运动不同而其他视觉条件完全相同。现有方法采用基于掩码的修复范式，导致视觉伪影、身份漂移和同步效果差

Method: 提出自引导框架：1) 使用Diffusion Transformer作为数据生成器，为每个真实样本合成唇部调整的伴生视频，形成视觉对齐的视频对；2) 基于DiT的音频驱动编辑器在这些对上端到端训练，利用完整对齐的输入视频帧专注于精确的音频驱动唇部修改；3) 引入时间步自适应多阶段学习策略，解耦扩散时间步中的冲突编辑目标

Result: 方法实现了高度准确的唇部同步、忠实的身份保持，并在具有挑战性的野外场景中表现出卓越的鲁棒性。还提出了ContextDubBench基准数据集用于多样化实际应用场景的鲁棒评估

Conclusion: 通过将视觉配音重新定义为条件良好的视频编辑问题，并利用自引导框架生成理想训练数据，该方法克服了现有方法的局限性，实现了高质量的音频驱动视觉配音效果
...
TL;DR: FineTec是一个用于时间损坏骨架序列细粒度动作识别的统一框架，通过上下文感知补全、空间分解和物理驱动估计来恢复细微运动线索，在多个基准测试中显著优于现有方法。
...
Method: 1) 使用上下文感知补全和多样化时间掩码从损坏输入中恢复基础骨架序列；2) 基于骨架的空间分解模块将骨架分为五个语义区域，根据运动方差进一步分为动态和静态子组，通过针对性扰动生成两个增强骨架序列；3) 物理驱动估计模块利用拉格朗日动力学估计关节加速度；4) 将融合的骨架位置序列和加速度序列共同输入基于GCN的动作识别头。
...
Conclusion: FineTec通过统一框架有效解决了时间损坏骨架序列的细粒度动作识别问题，展示了其鲁棒性和泛化能力，为真实世界场景中的动作识别提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的方法在稀疏视图3D重建中存在三个关键局限：已知视角外围覆盖不足、生成视图间几何不一致、计算流程昂贵。需要一种既能保持几何一致性又能高效扩展场景覆盖的方法。

Method: 提出GaMO框架，通过多视角外绘重新定义稀疏视图重建：从现有相机位姿扩展视野而非生成新视角，采用多视角条件和几何感知去噪策略，以零样本方式无需训练。

Result: 在Replica和ScanNet++数据集上，在3、6、9个输入视图下均达到SOTA重建质量（PSNR和LPIPS指标），相比SOTA扩散方法实现25倍加速，处理时间低于10分钟。

Conclusion: GaMO通过多视角外绘方法有效解决了稀疏视图重建中的几何一致性和覆盖范围问题，在保持高质量重建的同时显著提升了计算效率。
...
TL;DR: CAT框架通过一致性-准确率关系曲线和CORE指数，可视化评估LLMs在可控输入变化下的准确率与响应一致性之间的权衡关系


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估主要关注准确率或基准分数，而一致性对于高风险实际应用部署至关重要。虽然这两个维度需要独立评估，但它们的相互依赖性也需要考虑，以实现更细致的LLM评估。

Method: 提出CAT框架，核心是一致性-准确率关系曲线，可视化模型准确率如何随一致性要求增加而变化；提出最小一致性准确率指标和一致性导向的鲁棒性估计指数，结合CAR曲线的面积和形状量化准确率与一致性之间的权衡。

Result: 在多个通用和专业领域LLMs上演示了该框架，评估了多个选择题基准测试。框架可扩展到选择题之外的任务，支持通过适应性评分函数进行长格式、开放式评估。

Conclusion: CAT框架为LLMs提供了更全面的评估视角，不仅考虑准确率和一致性作为独立指标，还关注它们之间的相互作用，这对于高风险实际应用中的模型部署具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在英语到代码生成方面表现出色，但这一进展未能扩展到低资源语言。针对孟加拉语到Python代码生成的需求，需要开发专门的方法来克服语言资源不足的挑战

Method: 提出BanglaCodeAct框架，采用基于智能体的方法，利用多智能体提示和迭代自校正机制。框架使用开源多语言LLM，在Thought-Code-Observation循环中实现动态代码生成、测试和优化，无需任务特定的微调

Result: 在mHumanEval数据集上评估多个小参数开源LLM，Qwen3-8B结合BanglaCodeAct表现最佳，在开发集上达到94.0%的pass@1准确率，在盲测集上达到71.6%的准确率，为孟加拉语到Python翻译设立了新基准

Conclusion: BanglaCodeAct框架在低资源语言代码生成方面取得了显著进展，证明了基于智能体的推理方法在可靠代码生成方面的潜力，为其他低资源语言的类似任务提供了可借鉴的解决方案
...
TL;DR: PharmaShip是一个真实世界的中文药品运输单据数据集，用于测试预训练文本布局模型在噪声OCR和异构模板下的性能，包含三个任务：序列实体识别、关系抽取和阅读顺序预测。
...
Method: 创建PharmaShip数据集，包含扫描的药品运输单据，采用实体中心的评估协议，对五个代表性基线模型（LiLT、LayoutLMv3-base、GeoLayoutLM及其RORE增强变体）进行基准测试，标准化预处理、数据分割和优化过程。

Result: 像素信息和显式几何信息提供互补的归纳偏置，但单独使用都不够；注入阅读顺序导向的正则化能持续改进序列实体识别和实体链接，提供最鲁棒的配置；更长的位置覆盖能稳定页面后期预测并减少截断伪影；阅读顺序预测在词级别准确但在片段级别具有挑战性。
...
Method: 向初始生成状态注入随机噪声种子，在152个生成周期中观察语言行为的非线性转变，分析熵特征和相位转变。

Result: 发现了三种具有不同熵特征的稳定人格模式，外部噪声源能可靠地诱导反射性生成动力学的相位转变，人格保持一致性且模式间存在显著差异（p < 0.01）。
...
TL;DR: HarmTransform是一个多智能体辩论框架，用于将有害查询转化为更隐蔽的形式，以增强LLM安全对齐训练数据
...
Result: 实验表明HarmTransform在生成有效查询转化方面显著优于标准基线方法，同时分析发现辩论具有双刃剑效应：既能提升转化质量和隐蔽性，也可能引入主题偏移和不必要的复杂性

Conclusion: 多智能体辩论在生成全面安全训练数据方面既有前景也有局限性，需要平衡转化质量与复杂性之间的关系
...
TL;DR: LLMs在完美信息游戏中已展现世界建模能力，本研究将其扩展到不完全信息领域（以扑克为典型POMDP），发现模型能从扑克手牌历史数据中学习确定性和随机性特征，形成与理论信念状态相关的内部表征。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明LLMs能在完美信息游戏中发展涌现的世界模型，本研究旨在探索LLMs在不完全信息领域（部分可观察马尔可夫决策过程）中是否也能学习环境表征，以扑克作为典型POMDP案例进行研究。

Method: 在扑克手牌历史数据上预训练GPT风格模型，通过探测内部激活来分析模型学习的内容，主要使用非线性探针来解码内部表征。

Result: 模型在没有明确指导的情况下学习了确定性结构（如手牌等级）和随机性特征（如胜率），这些表征可通过非线性探针解码，并与理论信念状态相关，表明LLMs学习了德州扑克随机环境的自身表征。

Conclusion: LLMs能够在部分可观察环境中学习复杂的环境表征，包括确定性和随机性特征，这扩展了我们对LLMs世界建模能力的理解，表明它们能够在不完全信息领域中形成有效的内部表示。
...
TL;DR: 提出两阶段框架应对性别歧视内容检测中的三大挑战：代表性不足、数据噪声和概念模糊性，通过针对性训练和推理时协作专家判断实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 在线性别歧视内容日益隐蔽且依赖语境，传统检测方法难以应对。数据中存在标注不一致、标签稀缺和类别不平衡问题，导致模型决策边界不稳定，忽略更细微的伤害形式。

Method: 两阶段框架：1) 训练阶段使用类别平衡焦点损失、类别感知批处理和阈值校准处理标签不平衡和噪声监督；2) 推理阶段通过动态路由机制，高置信度案例直接分类，不确定案例交由协作专家判断模块处理，该模块使用多角色提示并通过法官模型整合推理。

Result: 在多个基准测试中取得最先进结果：EXIST 2025 Task 1.1上F1提升2.72%，EDOS Tasks A和B分别提升4.48%和1.30%。

Conclusion: 该框架通过统一针对性训练程序和选择性推理机制，有效解决了性别歧视内容检测中的代表性不足、噪声和概念模糊性问题，显著提升了检测性能。
...
TL;DR: EASD是一种无需训练的方法，通过动态熵惩罚机制改进推测解码，在保持效率的同时可能超越目标LLM的性能
...
Method: 在标准推测解码基础上引入动态熵惩罚机制，利用采样分布的熵量化模型不确定性，当两个模型都呈现高熵且前N个预测有显著重叠时，拒绝相应token并由目标LLM重新采样

Result: 在多个推理基准测试中，EASD始终优于现有推测解码方法，在大多数情况下甚至超越了目标LLM本身的性能，同时保持了与标准推测解码相当的效率

Conclusion: EASD通过熵感知机制和草稿模型验证，不仅改进了推测解码，还实现了超越目标模型固有性能的可能性，为LLM推理加速提供了新思路
...
TL;DR: MiMo-Audio通过将音频预训练数据扩展到超过1亿小时，实现了音频任务的少样本学习能力，在多项基准测试中达到开源模型SOTA水平，并能泛化到训练数据中未见的任务。


<details>
  <summary>Details</summary>
Motivation: 现有音频语言模型通常需要针对特定任务进行微调，而人类能够通过少量示例或简单指令泛化到新任务。GPT-3展示了文本领域通过扩展下一个标记预测预训练可以实现强大的泛化能力，作者认为这一范式同样适用于音频领域。

Method: 将MiMo-Audio的预训练数据扩展到超过1亿小时，观察到少样本学习能力的涌现。在后期训练阶段，策划了多样化的指令调优语料库，并在音频理解和生成中引入了思维机制。

Result: MiMo-Audio-7B-Base在语音智能和音频理解基准测试中达到开源模型SOTA性能，能够泛化到训练数据中未见的任务（如语音转换、风格迁移、语音编辑），并展现出强大的语音延续能力。MiMo-Audio-7B-Instruct在音频理解、口语对话和指令TTS评估中达到开源SOTA，接近或超越闭源模型。

Conclusion: 通过大规模音频预训练，可以实现类似GPT-3在文本领域的泛化能力，为音频任务提供强大的少样本学习能力。MiMo-Audio展示了音频语言模型在理解和生成任务上的卓越性能，为音频AI的发展提供了新范式。
...
TL;DR: 本研究通过SHAP解释性分析比较了两种基于Transformer的新闻偏见检测模型，发现领域自适应模型比标准偏见检测器产生更少的误报，且其归因模式与预测结果更一致。
...
Method: 采用比较可解释性研究方法，分析两种基于Transformer的偏见检测模型：在BABE数据集上微调的偏见检测器模型，以及在BABE数据集上微调的领域自适应预训练RoBERTa模型。使用SHAP解释技术分析正确和错误预测的词级归因，以表征不同模型架构如何操作语言偏见。

Result: 1. 两种模型都关注相似的评价语言类别，但在将这些信号整合到预测中的方式上存在显著差异；2. 偏见检测器模型对误报赋予比真报更强的内部证据，表明归因强度与预测正确性之间存在错位，导致系统性地过度标记中性新闻内容；3. 领域自适应模型展现出与预测结果更一致的归因模式，产生63%更少的误报；4. 模型错误源于不同的语言机制，误报主要由语篇层面的模糊性驱动，而非明确的偏见线索。

Conclusion: 研究强调了可解释性感知评估对偏见检测系统的重要性，表明架构和训练选择对模型可靠性和在新闻背景下的部署适用性具有关键影响。领域自适应方法能产生更可靠的偏见检测结果。
...
TL;DR: 本文研究LLM在检索增强问答中的应用，发现长上下文虽然能包含更多相关知识，但也引入了无关信息，影响模型性能。作者提出自适应提示策略，将检索信息分块处理，在性能和效率间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 随着LLM上下文窗口的扩大，检索增强生成中使用更广泛上下文的情况增多。虽然长上下文更容易包含目标知识，但也引入了更多无关信息，这会阻碍模型的生成过程并降低其性能。需要解决如何在利用相关信息和减少无关信息之间取得平衡的问题。
...
Conclusion: 自适应提示策略在保持性能的同时提高了效率。研究强调了需要进一步研究增强LLM在面对信息不足时有效拒绝请求的能力，这是提高检索增强问答系统可靠性的关键方向。
...
TL;DR: 利用注意力层token分布生成对抗样本，相比提示或梯度攻击更符合模型内部生成逻辑，在论证质量评估任务中验证了有效性但存在语法退化问题


<details>
  <summary>Details</summary>
Motivation: 利用机制可解释性研究发现中间注意力层编码token级假设的特性，探索从注意力层token分布直接生成对抗样本的可能性，以更符合模型内部生成过程的方式创建对抗扰动

Method: 从中间注意力层的token分布中提取token作为对抗扰动，在ArgQuality数据集上进行论证质量评估实验，使用LLaMA-3.1-Instruct-8B同时作为生成器和评估器

Result: 注意力层生成的对抗样本导致评估性能显著下降，同时保持与原始输入的语义相似性；但某些层和token位置的替换会引入语法退化，限制了实际有效性

Conclusion: 中间层表示作为对抗样本来源具有潜力但存在局限性，可用于压力测试基于LLM的评估流程，但需要解决语法退化问题以提高实用性
...
TL;DR: 该研究通过多检索器RAG系统结合最新LLM，解决了金融数值推理QA任务中因缺乏领域知识导致的错误，在FinQA基准上实现了SOTA性能提升7%以上。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型取得进展，但金融数值问题仍然具有挑战性，因为它们需要特定的金融领域知识和复杂的多步骤数值推理能力。

Method: 实现多检索器RAG系统，检索外部领域知识和内部问题上下文，利用最新LLM处理任务。通过SecBERT编码器进行领域特定训练，构建神经符号模型。

Result: 领域特定训练的神经符号模型超越了FinQA论文的基准模型；基于提示的LLM生成器实现了SOTA性能，提升超过7%，但仍低于人类专家水平。

Conclusion: 研究发现领域特定训练具有潜在优势；小模型存在幻觉损失与外部知识获取的权衡，而大模型的外部知识收益通常超过幻觉损失；最新LLM在少样本学习中的数值推理能力得到增强。
...
TL;DR: 论文提出一个分析框架，将学生回答的内容信号与评分者倾向分离，使用教师历史作为动态先验和文本嵌入，通过线性模型量化各信号贡献，提升评分透明度。


<details>
  <summary>Details</summary>
Motivation: 传统自动评分方法往往将学生回答内容与教师评分倾向混为一谈，缺乏透明度。需要一种能够分离内容信号与评分者倾向、使评分判断可见且可审计的分析框架。

Method: 使用ASSISTments数学回答数据，将教师评分历史建模为动态先验，从句子嵌入中提取文本表示，采用中心化和残差化处理以减少提示和教师混杂因素。使用时序验证的线性模型量化各信号贡献，并通过投影可视化模型分歧。

Result: 教师先验对评分预测影响显著；先验与内容嵌入结合时效果最佳（AUC~0.815），仅内容模型效果较弱但高于随机（AUC~0.626）。调整评分者效应能锐化残差内容表示，保留更多信息维度，揭示语义证据支持理解而非表面回答差异的情况。

Conclusion: 该框架将嵌入从简单特征转化为反思性学习分析工具，使教师和研究者能够检查评分实践与学生推理和学习证据的一致性（或冲突），为透明、可审计的评分分析提供实用流程。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统的记忆模块主要作为被动存储，仅积累孤立事实用于压缩输入和生成子查询，忽视了原始事实间的高阶相关性。这种静态设计限制了表示能力和对多步推理及知识演化的影响，导致碎片化推理和弱全局理解能力。

Method: 提出HGMem（超图记忆机制），将记忆表示为超图结构，其中超边对应不同的记忆单元。该机制能够渐进形成记忆内的高阶交互，围绕核心问题连接事实和思考，演化为集成的情境化知识结构，为后续步骤的深度推理提供有力支持。

Result: 在多个为全局理解设计的挑战性数据集上进行评估，广泛实验和深入分析表明，HGMem方法持续改进多步RAG性能，在多样化任务上显著优于强基线系统。

Conclusion: HGMem将记忆从简单存储扩展为动态、表达能力强的结构，能够支持复杂推理和全局理解，通过超图结构有效捕捉事实间的高阶相关性，提升多步RAG系统的性能。
...
Result: LoZA在长上下文场景中实现显著加速，LongCat-Flash-Exp模型能够高效处理高达100万个token，支持高效长期推理和长视野智能体能力

Conclusion: LoZA稀疏注意力方案为长上下文处理提供了有效的计算优化方法，使模型能够在有限计算预算下处理超长序列，提升推理和智能体任务的效率
...
TL;DR: CEC-Zero：一种无需监督的强化学习框架，通过让大语言模型自我纠错来解决中文拼写纠错问题，在9个基准测试中显著超越有监督基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型和有监督方法对新型错误缺乏鲁棒性，且依赖昂贵的标注数据，难以应对大规模中文拼写纠错的实际需求。

Method: CEC-Zero框架从干净文本合成错误输入，通过语义相似性和候选一致性计算聚类共识奖励，使用PPO算法优化策略，实现无需标注的强化学习。

Result: 在9个基准测试中，CEC-Zero比有监督基线方法提升10-13个F1分数点，比强LLM微调方法提升5-8个F1分数点，具有无偏奖励和收敛的理论保证。

Conclusion: CEC-Zero建立了无需标注的鲁棒、可扩展中文拼写纠错新范式，释放了大语言模型在噪声文本处理中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型的推理能力不断增强，但其推理过程的内部机制仍未被充分探索。现有方法通常依赖人工定义的概念（如过度思考、反思）在词级别进行分析，但这种方法有限，因为无法捕捉所有可能的推理行为，且许多行为难以在token空间中定义。

Method: 提出RISE框架：将思维链轨迹分割为句子级别的"步骤"，在步骤级激活上训练稀疏自编码器（SAEs），发现编码不同推理行为的推理向量（激活空间中的方向）。通过可视化和聚类分析这些行为在解码器列空间中的分布，并对SAE衍生向量进行针对性干预以控制推理行为。

Result: SAEs能够解耦出可解释的行为特征（如反思和回溯），这些行为在解码器列空间中占据可分离区域。通过干预SAE衍生向量可以可控地增强或抑制特定推理行为，改变推理轨迹而无需重新训练。SAEs还能捕捉结构特性（如响应长度），并发现超出人类监督的新行为（如识别置信度相关向量）。

Conclusion: 无监督潜在发现方法在解释和可控引导LLM推理方面具有巨大潜力，能够发现人类未预定义的推理行为，并为模型行为的理解和控制提供新途径。
...
<details>
  <summary>Details</summary>
Motivation: 区分假新闻与讽刺幽默内容具有挑战性，因为它们在语言特征上重叠但意图不同。研究旨在开发一个评估框架，比较轻量级模型在资源受限环境下的表现。

Method: 开发WISE框架，在Fakeddit的20,000个平衡数据集上评估8个轻量级Transformer模型和2个基线模型。使用分层5折交叉验证，评估包括准确率、精确率、召回率、F1分数、ROC-AUC、PR-AUC、MCC、Brier分数和预期校准误差在内的综合指标。通过配对t检验和McNemar检验进行统计显著性比较。

Result: MiniLM在所有模型中准确率最高(87.58%)，RoBERTa-base的ROC-AUC最高(95.42%)且准确率也很高(87.36%)。DistilBERT在效率与准确性间取得良好平衡(86.28%准确率，93.90% ROC-AUC)。统计检验确认模型间存在显著性能差异。

Conclusion: 轻量级模型能够匹配甚至超越基线模型性能，为在资源受限的真实环境中部署虚假信息检测系统提供了可行的解决方案。
...
TL;DR: iCLP框架让大语言模型在潜在空间中进行隐式规划，在语言空间进行推理，显著提升数学推理和代码生成任务的准确性和效率，同时保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于显式文本规划的方法存在幻觉问题且难以适应任务多样性，受人类潜意识认知启发，需要更紧凑、泛化的规划方式。

Method: iCLP框架：1) 从现有逐步推理轨迹中提取显式规划；2) 通过向量量化自编码器和码本学习离散表示；3) 在配对潜在规划和推理步骤上微调LLM，使其学会隐式规划。

Result: 实验显示iCLP在数学推理和代码生成任务上显著提升准确性和效率，具有强跨领域泛化能力，同时保持链式思维推理的可解释性。

Conclusion: iCLP通过让LLM在潜在空间进行隐式规划，在语言空间进行推理，有效解决了显式规划的局限性，实现了更高效、准确且可泛化的推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗、法律、金融等关键决策领域应用日益广泛，但其可靠性仍不确定。现有模型存在过度自信错误、输入变化下性能下降、缺乏清晰不确定性估计等问题，且现有评估方法分散，只关注孤立方面。

Method: 提出了复合可靠性评分（CRS）框架，将校准、鲁棒性和不确定性量化整合为单一可解释指标。在10个领先开源LLM和5个QA数据集上进行实验，评估基线性能、扰动下的表现和校准方法。

Result: CRS提供了稳定的模型排名，揭示了单一指标可能忽略的隐藏故障模式，并表明最可靠的系统能够平衡准确性、鲁棒性和校准后的不确定性。

Conclusion: CRS作为一个统一的可靠性评估框架，能够全面评估LLM在关键决策领域的可靠性，帮助识别最可靠的系统，这些系统需要在准确性、鲁棒性和不确定性校准之间取得平衡。
...
TL;DR: HY-MT1.5系列机器翻译模型通过多阶段训练框架，在参数效率上表现优异，1.8B模型超越多个大型开源模型和商业API，7B模型在其规模类别中达到新的SOTA。


<details>
  <summary>Details</summary>
Motivation: 开发高性能机器翻译模型，通过参数效率优化，在较小模型规模下达到接近甚至超越大型专有模型的翻译性能，同时支持高级翻译约束功能。

Method: 采用多阶段训练框架：通用与MT导向的预训练、监督微调、策略蒸馏和强化学习，支持术语干预、上下文感知翻译和格式保持等高级约束功能。

Result: HY-MT1.5-1.8B在标准中-外和英-外任务中超越Tower-Plus-72B、Qwen3-32B等大型开源模型及微软翻译、豆包翻译等商业API，达到Gemini-3.0-Pro约90%性能；HY-MT1.5-7B在其规模类别中达到SOTA，在Flores-200上达到Gemini-3.0-Pro 95%性能，在WMT25和普通话-少数民族语言测试集上超越Gemini-3.0-Pro。

Conclusion: HY-MT1.5系列模型通过创新的多阶段训练框架，在参数效率和翻译性能方面取得了显著突破，为通用和专业翻译任务提供了具有竞争力的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型开发主要由资源丰富的研究机构和行业伙伴主导，许多研究人员因缺乏本地计算资源而转向云服务，但云平台陡峭的学习曲线和分散的文档成为主要障碍

Method: 通过集中整理AWS SageMaker上训练Hugging Face模型所需的关键信息，提供从零开始的完整指南，填补现有文档的知识空白

Result: 创建了一个系统化的演示论文，将碎片化的云端训练知识整合为连贯的教程，帮助研究人员成功在AWS SageMaker上训练他们的第一个Hugging Face模型

Conclusion: 通过提供集中化的云端训练指南，能够降低云平台采用门槛，促进大语言模型研究的民主化，使更多研究人员能够利用云资源进行模型训练
...
TL;DR: 提出了一种用于掩码扩散语言模型的高效推理时控制框架，通过对比示例计算层间引导向量，无需模拟去噪轨迹即可实现属性调制


<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型通过迭代去噪过程生成文本，具有掩码并行解码和与自回归大模型竞争的性能，但其推理时控制和引导机制尚未充分探索

Method: 提出激活引导框架，通过对比示例在单次前向传播中计算层间引导向量，并在每个反向扩散步骤中应用这些方向，形成高效的推理时控制机制

Result: 在LLaDA-8B-Instruct上的实验表明能够可靠地调制高级属性，消融实验检验了引导在不同Transformer子模块和token范围（提示vs.响应）上的效果

Conclusion: 该框架为掩码扩散语言模型提供了有效的推理时控制机制，无需模拟去噪轨迹即可实现高效属性引导
...
TL;DR: 该论文提出了大型情感世界模型（LEWM），通过整合情感因素来增强世界模型对人类行为的理解和预测能力，特别是在情感驱动的社会行为方面。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型主要关注物理世界规律建模，缺乏对情感因素的系统探索。情感作为世界知识的重要组成部分，显著影响人类决策，但当前模型在情感理解方面存在不足。

Method: 首先通过实验证明情感信息对推理性能的重要性；然后基于心智理论提出LEWM模型，构建了整合情感的Emotion-Why-How数据集，该数据集将情感融入因果关系，支持推理行为原因和情感如何驱动未来世界状态；模型同时建模情感状态、视觉观察和动作，预测未来状态和情感转变。

Result: 实验结果显示，LEWM在情感驱动的社会行为预测方面更准确，同时在基础任务上保持与通用世界模型相当的性能。

Conclusion: 情感是世界理解的关键组成部分，LEWM通过系统整合情感因素，在保持基础能力的同时显著提升了情感驱动行为的预测准确性，为更全面的世界建模提供了新思路。
...
TL;DR: TeleChat3-MoE是一个基于混合专家架构的大语言模型系列，参数规模从1050亿到超过1万亿，在昇腾NPU集群上进行端到端训练。技术报告主要介绍了支持前沿模型规模可靠高效扩展的训练基础设施。


<details>
  <summary>Details</summary>
Motivation: 为了在昇腾NPU硬件生态系统中实现大规模语言模型的高效训练和扩展，需要建立可靠的基础设施来支持从1050亿到超过1万亿参数的混合专家架构模型训练。

Method: 1. 系统化的算子级和端到端数值精度验证方法，确保跨硬件平台和分布式并行策略的一致性；2. 性能优化套件：交错流水线调度、长序列训练的注意力感知数据调度、专家并行的分层重叠通信、DVM算子融合；3. 基于分析估计和整数线性规划的系统化并行化框架，优化多维并行配置；4. 集群级优化方法，解决大规模训练任务中的主机和设备瓶颈。

Result: 这些基础设施改进在包含数千个设备的集群上实现了显著的吞吐量提升和接近线性的扩展，为硬件生态系统上的大规模语言模型开发提供了坚实基础。

Conclusion: TeleChat3-MoE的训练基础设施通过系统化的精度验证、性能优化和并行化框架，成功支持了从1050亿到超过1万亿参数的混合专家架构模型在昇腾NPU集群上的高效训练和扩展。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在临床诊断中存在三个关键局限：1) 由于缺乏已验证知识基础而生成幻觉医学内容；2) 提出冗余或低效问题而非有助于诊断进展的区分性问题；3) 在多轮对话中失去连贯性，导致矛盾或不一致的结论。

Method: MedKGI框架整合医学知识图谱以将推理约束在已验证的医学本体中，基于信息增益选择问题以最大化诊断效率，采用OSCE格式的结构化状态来保持跨轮次的一致证据追踪。

Result: 在临床基准测试中，MedKGI在诊断准确性和询问效率方面均优于强大的LLM基线，平均提高对话效率30%，同时保持最先进的准确性。

Conclusion: MedKGI通过将临床实践原则整合到LLM推理中，有效解决了当前LLM在临床诊断中的关键局限，实现了更准确、高效的诊断对话系统。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯语自动作文评分研究因缺乏公开可用的数据集而受到限制，需要构建一个大规模、高质量的阿拉伯语作文评分数据集来支持相关研究。

Method: 设计了LAILA数据集，包含7,859篇阿拉伯语作文，标注了整体分数和七个维度的特质分数（相关性、组织结构、词汇、风格、发展、机制、语法）。详细描述了数据集的设计、收集和标注过程，并使用最先进的阿拉伯语和英语模型在特定提示和跨提示设置下提供了基准结果。

Result: LAILA成为迄今为止最大的公开阿拉伯语AES数据集，为阿拉伯语自动作文评分研究提供了重要资源。基准测试结果表明了现有模型在该数据集上的性能表现。

Conclusion: LAILA填补了阿拉伯语AES研究的关键需求，支持开发鲁棒的评分系统，将促进阿拉伯语自动作文评分领域的研究发展。
...
Method: 开发了Pat-SPECTER模型，即在SPECTER2模型基础上针对专利数据进行微调。通过马赛式评估比较了8种语言相似度模型预测专利-论文引用的能力，并在两个实际场景中测试模型性能。

Result: Pat-SPECTER模型在预测可信专利-论文引用方面表现最佳。验证了假设：由于诚信义务要求，美国专利引用的论文在语义上比其他主要司法管辖区的相似度更低。

Conclusion: Pat-SPECTER模型能有效处理专利和科学文献的相似度分析，为学术研究和实践应用提供了有价值的工具，并揭示了不同司法管辖区引用行为的差异。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法在万亿级token预训练数据集（如FineWeb和DCLM）上存在局限性：基于质量指标的选择在长期预训练中表现出严重的收益递减，而基于多样性指标的选择会移除过多高质量样本，两者都限制了预训练LLM的能力。

Method: DATAMASK将选择过程视为掩码学习问题，采用基于策略梯度的优化方法：迭代采样数据掩码，基于预定义目标计算策略梯度，更新掩码采样逻辑。通过多种加速增强技术，相比贪婪算法减少了98.9%的选择时间。

Result: 从15万亿token的FineWeb数据集中选择了约10%的子集（FineWeb-Mask）。在12个多样化任务上评估，1.5B密集模型性能提升3.2%，7B MoE模型性能提升1.9%。

Conclusion: DATAMASK是一个高效的大规模预训练数据选择框架，能够同时优化质量和多样性指标，显著减少选择时间并提升模型性能，为万亿级token数据集的数据选择提供了实用解决方案。
...
TL;DR: FIGR通过强化学习将视觉思维整合到多轮推理中，构建视觉表示来外化中间结构假设，在复杂数学推理任务上显著优于纯文本推理模型。


<details>
  <summary>Details</summary>
Motivation: 复杂推理问题常涉及隐式的空间、几何和结构关系，这些关系在文本中未明确编码。纯文本推理难以捕捉复杂场景中的全局结构约束，需要新的方法来增强推理的稳定性和可靠性。

Method: FIGR通过端到端强化学习将主动视觉思维整合到多轮推理中。系统在问题解决过程中构建视觉表示来外化中间结构假设，并自适应地调节何时以及如何调用视觉推理，从而更好地处理全局结构属性。

Result: 在具有挑战性的数学推理基准测试中，FIGR显著优于强大的纯文本思维链基线模型。具体来说，FIGR在AIME 2025上比基础模型提高了13.12%，在BeyondAIME上提高了11.00%。

Conclusion: 图形引导的多模态推理能有效增强复杂推理的稳定性和可靠性，通过整合视觉思维可以更好地处理文本难以捕捉的全局结构约束。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的模型在处理长文档（尤其是法律和医学领域）时存在困难，稀疏注意力机制资源消耗大且难以捕捉完整文档上下文，分层Transformer模型效率较好但无法清晰解释文档不同部分之间的关系。受人类略读文本、关注重要部分以理解整体信息的策略启发，需要开发更有效且高效的长文档表示方法。

Method: 提出新的自监督对比学习框架：1）随机掩码文档的一个段落；2）使用基于自然语言推理（NLI）的对比目标，将掩码段落与文档相关部分对齐，同时与不相关部分保持距离；3）模拟人类综合信息的方式，生成更丰富且计算效率更高的文档表示。

Result: 在法律和生物医学文本上的实验证实，该方法在准确性和效率方面都取得了显著提升。

Conclusion: 通过模拟人类略读策略的自监督对比学习框架，能够有效增强长文档表示，在保持计算效率的同时提高模型性能，特别适用于法律和医学等需要处理长文档的专业领域。
...
TL;DR: 本文比较了低资源语言文本摘要的多种方法，包括零样本提示大语言模型、微调小模型、数据增强、多语言迁移和翻译管道方法，发现微调的mT5模型在大多数指标上优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 自动文本摘要在英语等高资源语言中已取得高性能，但对低资源语言的摘要研究相对较少。本文旨在系统比较低资源语言文本摘要的各种方法，为这一领域提供实证分析。

Method: 1) 零样本提示不同规模的大语言模型；2) 微调mT5等小模型，结合三种数据增强方法；3) 多语言迁移学习；4) 翻译管道方法：将源语言翻译为英语、摘要、再翻译回源语言；5) 使用五种不同指标进行评估。

Result: 1) 相似参数规模的大语言模型性能存在差异；2) 多语言微调的mT5基线在大多数指标上优于包括零样本大语言模型在内的其他方法；3) 大语言模型作为评估者在低资源语言上可能不太可靠。

Conclusion: 对于低资源语言的文本摘要任务，微调的多语言模型（如mT5）在大多数情况下优于零样本大语言模型方法，而大语言模型在低资源语言评估中的可靠性需要进一步研究。
...
TL;DR: 开发了一个基于Transformer的雅思写作自动评分与反馈平台，通过设计研究循环从规则系统升级到DistilBERT模型，显著提升了评分准确性，并实现了有效的自适应反馈。


<details>
  <summary>Details</summary>
Motivation: 传统雅思写作备考方法缺乏针对雅思评分标准的个性化反馈，需要开发能够提供针对性指导的智能平台来弥补这一不足。

Method: 采用设计研究法进行迭代开发：早期使用规则系统，后期升级为基于DistilBERT的Transformer模型配合回归头进行评分，并实现自适应反馈功能。平台架构将对话指导与写作界面分离以降低认知负荷。

Result: DistilBERT模型显著提升了评分准确性（MAE=0.66，R²为正）。自适应反馈使写作分数平均提高0.060分（p=0.011，效应量d=0.504），但不同修订策略效果存在差异。
...
TL;DR: 该论文提出了语音转录本段落分割的新任务，建立了首个基准数据集，开发了保留原始文本的约束解码方法，并创建了高效的分割模型。


<details>
  <summary>Details</summary>
Motivation: 自动语音转录本通常是无结构的单词流，影响可读性和再利用。语音处理领域缺乏段落分割这一结构化步骤，文本分割领域也缺乏稳健的自然基准。

Method: 1) 建立TEDPara和YTSegPara两个基准数据集；2) 提出约束解码方法，让大语言模型插入段落分隔符同时保留原始转录本；3) 开发MiniSeg紧凑模型，并通过分层扩展实现章节和段落联合预测。

Result: 创建了首个语音段落分割基准数据集，提出的约束解码方法实现了忠实、句子对齐的评估，MiniSeg模型达到了最先进的准确率，且计算成本低。

Conclusion: 该研究的资源和方将段落分割确立为语音处理中标准化、实用的任务，填补了语音处理与文本分割交叉领域的空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有法律基准测试难以区分语言模型的推理能力与参数化知识，需要开发能够独立评估法律推理能力的基准测试。

Method: 创建KCL基准测试，包含KCL-MCQA（283个选择题，1,103个对齐判例）和KCL-Essay（169个开放式问题，550个对齐判例，2,739个实例级评分标准），提供问题级支持判例以实现推理能力与知识的分离。

Result: 对30多个模型的系统评估显示存在较大差距（特别是在KCL-Essay部分），专门用于推理的模型始终优于通用模型。

Conclusion: KCL基准测试能够有效评估语言模型的法律推理能力，揭示了当前模型的局限性，并表明推理专门化模型具有优势，所有资源已开源。
...
TL;DR: CREST是一种无需训练的方法，通过识别和干预LLM中的认知注意力头来优化推理过程，提高准确性同时减少计算成本


<details>
  <summary>Details</summary>
Motivation: 大型语言模型依赖长链式思维推理解决复杂任务，但这种方法效率低下，导致高延迟和不稳定推理（思考不足或过度思考），需要更高效的推理方法

Method: CREST包含两个组件：1）离线校准步骤识别认知注意力头并推导头特定的导向向量；2）推理时程序旋转隐藏表示以抑制这些向量方向上的分量，自适应抑制低效推理行为

Result: 在多样化的推理基准测试和模型中，CREST将准确率提高达17.5%，同时减少37.6%的token使用量，实现更快更可靠的LLM推理

Conclusion: CREST通过轻量级干预认知注意力头，有效引导模型远离低效推理模式，提供了一种简单有效的途径来实现更快更可靠的LLM推理
...
TL;DR: Youtu-LLM是一个1.96B参数的轻量级语言模型，通过从头预训练而非蒸馏，在保持计算效率的同时具备原生智能体能力，支持128k上下文，在STEM和智能体任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前小型模型通常依赖蒸馏技术，缺乏真正的推理和规划能力。研究旨在开发一个轻量级但具备原生智能体智能的模型，能够在有限计算资源下执行复杂的长时程推理和规划任务。

Method: 1. 采用密集多潜在注意力架构和STEM导向词汇表，支持128k上下文窗口；2. 设计"常识-STEM-智能体"三阶段课程学习策略，使用约11T token的语料库；3. 针对智能体任务采用可扩展的中期训练，通过多样化数据构建方案合成数学、编程和工具使用领域的轨迹。

Result: Youtu-LLM在2B以下模型中达到新的SOTA水平。在通用基准测试中与更大模型竞争，在智能体特定任务上显著超越现有基线，证明轻量级模型可以具备强大的内在智能体能力。

Conclusion: 通过系统化的架构设计和训练策略，轻量级语言模型可以同时实现高计算效率和原生智能体智能，为资源受限环境下的复杂推理和规划任务提供了可行解决方案。
...
TL;DR: R-Debater是一个基于论证记忆的智能体框架，用于生成多轮辩论，通过检索先前论据和案例证据来保持立场一致性、回应对手并支持主张。


<details>
  <summary>Details</summary>
Motivation: 当前辩论系统缺乏跨轮次的立场一致性和证据支持，需要结合修辞学和记忆研究，开发能够回忆和调整先前论据以维持辩论连贯性的系统。

Method: 整合辩论知识库（用于检索案例证据和先前辩论动作）与基于角色的智能体，构建连贯的跨轮次话语。在ORCHID标准化辩论数据集上评估，包含1,000项检索语料和32个跨7个领域的保留辩论。

Result: 相比强大的LLM基线，R-Debater在单轮和多轮评分中均获得更高分数。人类评估（20位经验辩手）进一步证实其一致性和证据使用能力，显示检索基础与结构化规划结合能产生更忠实、立场对齐且连贯的跨轮次辩论。

Conclusion: 结合检索基础与结构化规划的方法能够生成更忠实、立场一致且连贯的多轮辩论，验证了基于论证记忆的智能体框架在辩论生成中的有效性。
...
TL;DR: 本文提出MUSIC方法，通过多轮对话对比数据增强训练多轮奖励模型，提升对多轮对话质量的评估能力，同时保持单轮评估性能。


<details>
  <summary>Details</summary>
Motivation: 评估多轮对话质量对LLM发展至关重要，但现有方法依赖昂贵的人工评估。标准偏好数据集通常只对比最终轮次响应，无法捕捉多轮交互的细微差别，导致多轮奖励模型训练信号不足。

Method: 提出MUSIC（多步指令对比）无监督数据增强策略，合成跨多轮次展现差异的对比对话对。基于Skywork偏好数据集，使用Gemma-2-9B-Instruct模型训练多轮奖励模型。

Result: MUSIC增强的奖励模型在多项实验中表现优于基线方法，与先进专有LLM评估者的判断更一致，且不损害标准单轮奖励模型的基准性能。

Conclusion: 多轮对比对于构建稳健的多轮奖励模型至关重要，MUSIC方法能有效提升多轮对话评估质量，为LLM训练提供更可靠的可扩展信号。
...
TL;DR: BIOME-Bench：首个用于评估大语言模型在多组学分析中生物分子相互作用推断和通路机制阐明能力的标准化基准


<details>
  <summary>Details</summary>
Motivation: 当前多组学研究依赖通路富集分析，但这种方法受限于通路资源的固有缺陷（如更新滞后、功能冗余、对分子状态和干预敏感性有限）。虽然已有研究探索使用大语言模型改进通路富集分析，但缺乏端到端多组学通路机制阐明的标准化基准，限制了可重复性进展。

Method: 通过四阶段工作流程构建BIOME-Bench基准，评估大语言模型在多组学分析中的两个核心能力：1）生物分子相互作用推断；2）端到端多组学通路机制阐明。开发了针对这两个任务的评估协议，并在多个当代强大模型上进行了全面实验。
...
TL;DR: 论文对LLMs在推理任务中的计算效率进行测试时计算感知评估，发现MoE架构在性能与效率间取得良好平衡，并识别出推理计算存在饱和点。
...
Result: 发现MoE架构在评估设置中能良好平衡性能与效率；识别出推理计算存在饱和点，超过特定阈值后准确率增益会减弱；推导出每单位计算准确率增益的涌现趋势。

Conclusion: 虽然扩展推理能力有益，但无法克服模型在特定复杂性方面的内在限制，工业应用需要在性能与计算效率间做出权衡。
...
TL;DR: 介绍了一门NLP伦理课程的教学方法，通过主动学习、互动活动和"以教促学"等方式，在四年间跨机构、跨学科实施，并产生了可复用的教学材料和实际教育产品。
...
Method: 采用基于主动学习的教学方法，包括互动课程、实践活动和"以教促学"方法。课程在四年间不断优化，适应不同机构、教育水平和跨学科背景。

Result: 课程产生了许多可复用的产品，包括教学材料和学生为不同受众制作的实际教育产品。课程成功在不同教育环境中实施并持续改进。

Conclusion: 通过分享这一教学方法和经验，希望为寻求将社会影响考量融入课程的教育工作者提供启发，促进NLP伦理教育的普及和发展。
...
TL;DR: 提出因果三角验证方法，通过跨语言参考家族验证多语言模型的机制解释，过滤虚假电路


<details>
  <summary>Details</summary>
Motivation: 多语言模型在总体表现上很强，但在不同语言、文字和文化间的行为不可预测，需要满足因果标准的机制解释

Method: 提出参考家族作为谓词保持变体，引入三角验证方法：必要性（消融电路降低目标行为）、充分性（修补激活转移行为）、不变性（在参考家族中保持方向稳定和足够幅度）

Result: 三角验证为机制主张提供了可证伪标准，能够过滤通过单环境测试但在跨语言不变性上失败的虚假电路

Conclusion: 三角验证通过因果抽象框架，为多语言模型的机制解释提供了严格的验证标准，连接了实用可解释性议程
...
<details>
  <summary>Details</summary>
Motivation: 当前世界面临生态、意义和语言三重危机，这些危机正在汇聚成元危机。大型AI正在加速所有这些危机，语言工程师在其中扮演了关键角色，他们坚持着对人类失败的扩展性叙事，为富豪和贪腐者提供关键人才，并创造仿佛价值中立的新技术。迫切需要探索替代方案。

Method: 论文提出需要运用集体智慧，设计一个以人类在活生生的星球上繁荣为中心、肯定生命的NLP未来。这需要从根本上重新思考NLP的发展方向和价值取向。

Result: 论文没有提供具体实验结果，而是提出了一个批判性分析和转型呼吁。它指出了当前NLP发展模式的根本问题，并提出了替代方向。

Conclusion: NLP领域迫切需要从当前加速危机的模式转向以人类福祉和地球生命为中心的发展道路，这需要集体智慧和根本性的价值重定向。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型基准测试存在三个根本限制：1) 容易受到数据污染的影响；2) 仅限于单知识点评估；3) 依赖昂贵的领域专家标注。需要一种新的基准构建方法来克服这些限制。

Method: 从权威教材中提取独立的知识陈述作为基本单元，在测试时通过随机抽样动态组合成评估问题。每个问题聚合8-10个陈述进行综合多知识评估，标注者只需验证格式合规性而不需要领域专业知识。
...
Conclusion: Encyclo-K作为一个可扩展的框架，能够动态评估大语言模型对多个细粒度学科知识陈述的综合理解能力，解决了现有基准的主要局限性。
...
Method: 提出流形约束超连接（mHC）框架，将HC的残差连接空间投影到特定流形以恢复恒等映射特性，并进行基础设施优化确保效率

Result: mHC在大规模训练中有效，提供实质性性能改进和优越的可扩展性

Conclusion: mHC作为HC的灵活实用扩展，有助于深入理解拓扑架构设计，为基础模型演进指明方向
...
<details>
  <summary>Details</summary>
Motivation: 现有战略对话研究虽然能准确估计信念，但缺乏使用这些信念进行生成的原则性机制。本文旨在填补这一空白，通过将信念估计转化为生成约束来提升战略对话的可靠性。

Method: 提出BEDA框架，包含世界集合、信念估计器和条件生成器。首先形式化对抗和对齐两种核心对话行为，然后通过概率约束操作化这些行为，确保生成的话语与推断的信念一致。

Result: 在三种设置中均取得显著改进：在CKBG（对抗）中，成功率在不同骨干模型上至少提升5.0个百分点，使用GPT-4.1-nano时提升20.6个百分点；在Mutual Friends（合作）中平均提升9.3个百分点；在CaSiNo（谈判）中达到相对于所有基线的最优交易结果。
...
<details>
  <summary>Details</summary>
Motivation: 多步LLM管道能有效解决复杂任务，但其性能严重依赖每一步的提示。联合优化这些提示很困难，因为缺少步骤级监督和步骤间依赖关系。现有的端到端提示优化方法在这些条件下表现不佳，通常产生次优或不稳定的更新。
...
Conclusion: ADOPT通过显式建模步骤依赖关系、解耦梯度估计与更新、自适应资源分配，为多步LLM管道提供了一种有效的提示优化框架，解决了现有方法在缺少步骤级监督和复杂依赖关系下的局限性。
...
TL;DR: 基于DeBERTa V3和LSTM的法律文档分类器，使用48个随机选择的短文本块（最多128个词元）作为输入，解决了长文档处理问题，并采用Temporal构建了可靠的部署流水线。
...
Method: 提出了一种基于DeBERTa V3和LSTM的法律文档分类器，使用48个随机选择的短文本块（每个最多128个词元）作为输入。同时介绍了使用Temporal（一种持久执行解决方案）的部署流水线，确保处理工作流的可靠性和鲁棒性。

Result: 最佳模型的加权F分数为0.898，在CPU上运行的流水线处理100个文件的平均时间为498秒。

Conclusion: 该方法有效解决了长法律文档的分类问题，通过随机采样短文本块和结合DeBERTa V3与LSTM的架构，实现了良好的分类性能，并通过Temporal构建了可靠的生产部署流水线。
...
TL;DR: 该论文提出了RESTOREx资源库和MAMAMemeia框架，用于检测社交媒体表情包中的抑郁症状，基于LLM生成和人工标注的解释，性能比现有最佳方法提升7.55%


<details>
  <summary>Details</summary>
Motivation: 随着表情包从单纯的幽默表达演变为用户自由表达各种情绪（包括抑郁情绪）的媒介，需要开发有效的方法来识别社交媒体表情包中表现的抑郁症状，以进行心理健康监测和干预

Method: 1. 引入RESTOREx资源库，包含LLM生成和人工标注的解释，用于检测表情包中的抑郁症状；2. 提出MAMAMemeia框架，基于认知分析疗法（CAT）能力，采用协作式多智能体多方面讨论方法

Result: MAMAMemeia框架在宏观F1分数上比当前最先进方法提升了7.55%，在超过30种方法的比较中确立了新的基准性能

Conclusion: 该研究为社交媒体表情包中的抑郁症状检测提供了有效的解决方案，MAMAMemeia框架通过多智能体协作和临床心理学方法显著提升了检测性能，为心理健康监测提供了新的工具
...
<details>
  <summary>Details</summary>
Motivation: 传统Transformer主要依赖表层共现统计，缺乏对实体和事件的全局一致潜在表示，导致关系方向泛化能力差、上下文错误和数据效率低。受认知科学启发，人类理解语言时会将其转换为紧凑的事件表征并持久记忆。

Method: 提出Thought Gestalt模型，一种循环Transformer，在token和句子级"思想"状态两个抽象层次上建模语言。模型逐句生成token，同时交叉关注先前句子表征的记忆。使用同一组参数生成token和句子表征，通过单一目标（下一个token交叉熵）训练，梯度通过交叉注意力反向传播优化早期句子向量。

Result: 在扩展实验中，TG相比匹配的GPT-2基线持续提升效率，缩放拟合表明GPT-2需要多5-8%的数据和33-42%的参数才能达到TG的损失水平。TG在父子关系反转诅咒探针上减少了关系方向泛化错误。

Conclusion: Thought Gestalt模型通过双层次抽象和记忆机制，有效提升了语言模型的全局一致性、数据效率和关系方向泛化能力，验证了认知科学启发的语言建模方法的有效性。
...
TL;DR: AdaGReS是一个用于RAG的冗余感知上下文选择框架，通过结合查询-块相关性和集合内冗余惩罚的集合级目标，在token预算约束下进行贪婪选择，无需手动调参，并在理论和实验上验证了其有效性。
...
Method: 提出AdaGReS框架：1) 定义集合级目标函数，结合查询-块相关性和集合内冗余惩罚；2) 在token预算约束下使用边际增益进行贪婪选择；3) 引入闭式、实例自适应的校准机制，自动调整相关性与冗余权衡参数，无需手动调参；4) 提供理论分析，证明目标函数在实际嵌入相似性条件下具有epsilon-近似子模性，为贪婪选择提供近似最优性保证。

Result: 在开放域问答（Natural Questions）和高冗余生物医学（药物）语料库上的实验表明，AdaGReS在冗余控制和上下文质量方面持续改进，转化为更好的端到端答案质量和跨设置的鲁棒性。

Conclusion: AdaGReS通过系统性地处理冗余问题，提供了一种无需手动调参的token预算感知上下文选择方法，在理论和实验上都验证了其有效性，能够提升RAG系统的整体性能。
...
TL;DR: 论文提出DDFT评估框架，测量语言模型在语义压缩和对抗性攻击下的认知鲁棒性，发现模型鲁棒性与参数规模无关，而与错误检测能力相关。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估主要关注理想条件下的知识表现，但无法衡量模型在现实压力下的认知鲁棒性。现有静态基准测试无法区分模型是缺乏知识还是验证机制在信息退化或对抗攻击下崩溃。

Method: 提出Drill-Down and Fabricate Test (DDFT)协议，测量认知鲁棒性。采用两系统认知模型：语义系统生成流畅文本，认知验证器验证事实准确性。评估9个前沿模型在8个知识领域、5个压缩级别（共1800次轮级评估）。

Result: 认知鲁棒性与传统设计范式正交：参数数量（r=0.083, p=0.832）和架构类型（r=0.153, p=0.695）均不显著预测鲁棒性。错误检测能力强烈预测整体鲁棒性（rho=-0.817, p=0.007）。旗舰模型表现出脆弱性，而较小模型可获得鲁棒性能。

Conclusion: DDFT框架为关键应用部署前评估认知鲁棒性提供理论基础和实用工具。模型鲁棒性主要源于训练方法和验证机制，而非规模大小，挑战了模型规模与可靠性关系的传统假设。
...
TL;DR: CASCADE是一个自进化的LLM智能体框架，实现了从"LLM+工具使用"到"LLM+技能获取"的转变，通过持续学习和自我反思等元技能掌握复杂外部工具，在材料科学和化学研究任务中达到93.3%的成功率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体依赖预定义工具或脆弱的工具生成，限制了其在复杂科学任务中的能力和适应性。需要一种能够自主学习和掌握外部工具的框架来推动AI辅助科学研究。

Method: 提出CASCADE自进化智能体框架，包含两个核心元技能：1) 通过网页搜索和代码提取实现持续学习；2) 通过内省和知识图谱探索实现自我反思。框架支持人机协作和记忆巩固。

Result: 在包含116个材料科学和化学研究任务的SciSkillBench基准测试中，使用GPT-5的CASCADE达到93.3%的成功率，相比没有进化机制的35.4%有显著提升。展示了在计算分析、自主实验室实验和论文选择性复现等实际应用中的有效性。

Conclusion: CASCADE能够积累可执行技能，这些技能可以在智能体和科学家之间共享，为实现可扩展的AI辅助科学研究迈出了重要一步，代表了从工具使用到技能获取的范式转变。
...
<details>
  <summary>Details</summary>
Motivation: 准确的疾病预测对及时干预、有效治疗和减少医疗并发症至关重要。符号AI在医疗保健中应用有限，主要因为构建高质量知识库需要大量努力

Method: McCoy框架结合大型语言模型与答案集编程：1）使用LLM将医学文献翻译成ASP代码；2）将生成的ASP代码与患者数据结合；3）使用ASP求解器处理得到最终诊断

Result: 初步结果显示McCoy在小规模疾病诊断任务上表现出色，框架结合了两种范式的优势，提供了稳健且可解释的预测系统

Conclusion: McCoy框架通过结合LLM的文本理解能力和ASP的逻辑推理能力，克服了符号AI在医疗领域应用的知识库构建障碍，为疾病诊断提供了新的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 个性化搜索需要建模用户不断演化的多维信息需求，但现有系统受限于静态用户画像或单一检索流程，无法有效捕捉人类信息寻求行为的复杂性、流动性和情境敏感性。

Method: SPARK框架定义了基于角色、专业知识、任务上下文和领域的角色空间，引入角色协调器动态解释查询并激活相关专业智能体。每个智能体执行独立的检索增强生成过程，配备长短期记忆存储和情境感知推理模块。智能体间通过结构化通信协议（共享记忆库、迭代辩论、接力式知识传递）进行协作。

Result: 该框架产生了关于协调效率、个性化质量和认知负荷分布的可测试预测，同时包含自适应学习机制用于持续角色优化。SPARK展示了如何通过分布式智能体行为和最小协调规则产生涌现的个性化特性。

Conclusion: SPARK通过整合细粒度智能体专业化和协作检索，为下一代搜索系统提供了洞察，使其能够捕捉人类信息寻求行为的复杂性、流动性和情境敏感性，推动了认知架构、多智能体协调理论和信息检索的交叉研究。
...
### [136] [LoongFlow: Directed Evolutionary Search via a Cognitive Plan-Execute-Summarize Paradigm](https://arxiv.org/abs/2512.24077)
*Chunhui Wan,Xunan Dai,Zhuo Wang,Minglei Li,Yanpeng Wang,Yinan Mao,Yu Lan,Zhiwen Xiao*

Main category: cs.AI

TL;DR: LoongFlow是一个自进化智能体框架，通过将LLM集成到"计划-执行-总结"认知范式中，显著提高进化效率，在算法发现和机器学习管道优化中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统进化方法缺乏结构化推理，在从静态大语言模型向自改进智能体过渡时面临过早收敛和高维代码空间探索效率低的问题。

Method: 提出LoongFlow框架，将LLM集成到"计划-执行-总结"认知范式中，采用混合进化记忆系统，结合多岛模型、MAP-Elites和自适应玻尔兹曼选择来平衡探索与利用。

Result: 在AlphaEvolve基准测试和Kaggle竞赛中，LoongFlow比领先基线（如OpenEvolve、ShinkaEvolve）进化效率提高达60%，并能发现更优解决方案。

Conclusion: LoongFlow在自主科学发现方面迈出重要一步，能够以更低的计算成本生成专家级解决方案，显著推进了自进化智能体的发展。
...
### [137] [Graph-Based Exploration for ARC-AGI-3 Interactive Reasoning Tasks](https://arxiv.org/abs/2512.24156)
*Evgenii Rudakov,Jonathan Shock,Benjamin Ultan Cowley*

Main category: cs.AI

TL;DR: 提出一种无需训练的图基方法，用于解决ARC-AGI-3基准测试中的交互推理任务，通过视觉帧处理与系统化状态空间探索，在稀疏反馈环境中显著优于当前最先进的LLM。


<details>
  <summary>Details</summary>
Motivation: ARC-AGI-3基准测试包含类游戏任务，需要智能体通过有限交互推断任务机制并适应递增复杂度。当前最先进的LLM无法可靠解决这些任务，因此需要探索非学习方法的有效性。

Method: 结合视觉帧处理与系统化状态空间探索的图结构方法：1) 将视觉帧分割为有意义组件；2) 基于视觉显著性优先选择动作；3) 维护已探索状态和转移的有向图；4) 跟踪访问状态和测试动作，优先选择到达未测试状态-动作对的最短路径动作。

Result: 在ARC-AGI-3预览挑战中，该方法在6个游戏的52个关卡中解决了中位数30关，在私有排行榜上排名第3，显著优于前沿的LLM基智能体。

Conclusion: 即使无需学习，显式的图结构探索也能作为交互推理的强大基线，突显了在稀疏反馈环境中系统化状态跟踪和动作优先级的重要性，这些是当前LLM未能捕捉任务动态的关键能力。
...
### [138] [Constrained Language Model Policy Optimization via Risk-aware Stepwise Alignment](https://arxiv.org/abs/2512.24263)
...
<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法（如Safe RLHF和SACPO）通常采用风险中性范式，不足以应对参考策略偏差带来的风险，且对罕见但潜在灾难性有害行为的鲁棒性有限。需要一种能显式纳入风险意识的方法来确保模型安全性和可信度。

Method: 提出风险感知逐步对齐(RSA)方法：1) 将安全对齐建模为令牌级风险感知约束策略优化问题；2) 通过逐步对齐程序解决，基于嵌套风险度量推导令牌级策略更新；3) 设计旨在减轻参考策略过度偏移的风险，并显式抑制低概率高影响有害行为。

Result: 实验结果表明，该方法在保持高帮助性的同时确保强安全性，显著抑制尾部风险（低概率但高影响的不安全响应）。理论分析在温和假设下证明了策略最优性。

Conclusion: RSA方法通过显式纳入风险意识，有效解决了现有安全对齐方法的局限性，在保持模型帮助性的同时显著提升了安全性和对罕见有害行为的鲁棒性。
...
### [139] [What Drives Success in Physical Planning with Joint-Embedding Predictive World Models?](https://arxiv.org/abs/2512.24497)
*Basile Terver,Tsung-Yen Yang,Jean Ponce,Adrien Bardes,Yann LeCun*

Main category: cs.AI

TL;DR: 该研究系统分析了JEPA-WM系列世界模型，通过实验确定了最优的模型架构、训练目标和规划算法组合，在导航和操作任务上超越了现有基线方法。


<details>
  <summary>Details</summary>
Motivation: AI领域长期面临开发能够解决广泛物理任务并泛化到新环境的智能体挑战。虽然基于世界模型和规划的方法已有进展，但在表示空间进行规划的方法（JEPA-WM系列）缺乏系统性的技术分析，需要确定最优实现方案。

Method: 将JEPA-WM系列模型作为研究对象，通过模拟环境和真实机器人数据实验，系统分析模型架构、训练目标和规划算法三个关键组件对规划成功率的影响，寻找该家族中的最优方法。

Result: 研究提出的优化模型在导航和操作任务上超越了DINO-WM和V-JEPA-2-AC两个基准方法，证明了通过系统分析找到的最优组合的有效性。

Conclusion: 通过对JEPA-WM系列模型的系统性研究，确定了影响规划性能的关键技术选择，提出的优化模型在物理任务中表现优异，为表示空间规划方法的发展提供了重要指导。
...
### [140] [Evaluating the Reasoning Abilities of LLMs on Underrepresented Mathematics Competition Problems](https://arxiv.org/abs/2512.24505)
*Samuel Golladay,Majid Bani-Yaghoub*

Main category: cs.AI

TL;DR: 该研究分析了三个主流大语言模型（GPT-4o-mini、Gemini-2.0-Flash、DeepSeek-V3）在代表性不足的数学竞赛问题（密苏里大学数学竞赛）上的表现，发现DeepSeek-V3在微积分、解析几何和离散数学三个领域表现最佳，但所有模型在几何问题上都表现较弱。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多使用相同的数据集评估大语言模型的数学推理能力，这限制了研究结果的普适性，可能无法全面捕捉数学任务中的多样化挑战。本研究旨在通过分析大语言模型在代表性不足的数学竞赛问题上的表现，更深入地理解其数学推理的局限性。

Method: 研究使用密苏里大学数学竞赛中的微积分、解析几何和离散数学问题，对GPT-4o-mini、Gemini-2.0-Flash和DeepSeek-V3三个大语言模型进行测试。将模型的回答与已知正确答案进行比较，评估每个问题领域的准确性，并分析模型的推理过程以探索不同问题类型和模型间的错误模式。
...
Conclusion: 在代表性不足的数学竞赛数据集上评估大语言模型可以提供对其独特错误模式的更深入洞察，并突显了在结构化推理方面，特别是在几何领域，仍然存在的挑战。
...
### [141] [From Building Blocks to Planning: Multi-Step Spatial Reasoning in LLMs with Reinforcement Learning](https://arxiv.org/abs/2512.24532)
*Amir Tahmasbi,Sadegh Majidi,Kazem Taram,Aniket Bera*

Main category: cs.AI

TL;DR: 论文提出两阶段方法提升大语言模型的空间推理能力：先通过监督微调学习基础空间变换，再通过GRPO框架训练LoRA适配器进行多步规划，在ASCII艺术环境中验证效果优于基线模型。
...
Method: 采用两阶段方法：第一阶段通过监督微调学习旋转、平移、缩放等基础空间变换；第二阶段冻结物理感知模型，在GRPO框架下训练轻量级LoRA适配器，学习组合这些基础模块进行多步规划。为此合成了ASCII艺术数据集并构建了相应的强化学习环境。

Result: 该方法在动态环境（有明确状态更新）和静态环境（模型需依赖内部状态）中均优于基线模型，包括通用骨干模型、物理感知模型和端到端RL模型。此外，该方法收敛更快，训练更稳定，注意力模式分析显示微调确实改善了空间理解能力。

Conclusion: 将空间推理分解为原子构建块及其组合的两阶段方法能有效提升大语言模型的空间推理能力，在结构化环境中表现出更好的多步规划性能，且训练更高效稳定。
...
TL;DR: RLMs是一种新的推理策略，让LLMs能够通过递归调用自身来处理远超上下文窗口的超长提示，性能显著优于传统方法


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型的上下文窗口有限，无法处理超长提示，限制了其在需要处理大量文本的应用场景中的能力

Method: 提出递归语言模型（RLMs）推理策略，将长提示视为外部环境，让LLM能够以编程方式检查、分解提示，并递归调用自身处理提示片段

Result: RLMs成功处理比模型上下文窗口长两个数量级的输入，在四个不同的长上下文任务中，即使对于较短的提示，也显著优于基础LLMs和常见的长上下文支架，同时查询成本相当或更低
...
Method: 将协作建模为去中心化部分可观测马尔可夫决策过程（Dec-POMDP），采用集中训练分散执行（CTDE）架构，引入群体相对策略优化（GRPO）算法，结合简化的联合奖励函数平衡任务质量、速度和协调成本。

Result: 在协作写作和编程基准测试中，任务处理速度比单智能体基线提升3倍，写作结构/风格一致性达到98.7%，编程测试通过率达到74.6%，持续优于现有多智能体LLM基线方法。
...
### [144] [Group Deliberation Oriented Multi-Agent Conversational Model for Complex Reasoning](https://arxiv.org/abs/2512.24613)
*Zheyu Shi,Dong Qiu,Shanlong Yu*

Main category: cs.AI

TL;DR: 提出基于群体审议的多智能体对话模型，通过三层角色架构（生成、验证、整合）提升复杂推理任务性能，相比单一大语言模型在多跳推理准确率和一致性方面有显著改进。


<details>
  <summary>Details</summary>
Motivation: 针对单一大语言模型在复杂推理任务中的局限性，需要更有效的多智能体协作机制来提升推理准确性和逻辑一致性。

Method: 采用三层角色分工架构：观点生成智能体产生多样化推理视角，证据验证智能体检索外部知识并量化事实支持度，一致性仲裁智能体整合逻辑一致的结论。引入自博弈机制扩展多路径推理轨迹，检索增强模块动态补充外部知识，设计结合事实一致性和逻辑连贯性的复合奖励函数，应用改进的近端策略优化策略进行协同训练。

Result: 在HotpotQA上多跳推理准确率提升16.8%，在2WikiMultihopQA上提升14.3%，在MeetingBank上提升19.2%，一致性提升21.5%。相比主流多智能体方法具有更高的推理效率。

Conclusion: 该模型为复杂推理任务提供了有效且稳定的解决方案，通过群体审议的多智能体协作机制显著提升了推理性能和逻辑一致性。
...
### [145] [Youtu-Agent: Scaling Agent Productivity with Automated Generation and Hybrid Policy Optimization](https://arxiv.org/abs/2512.24615)
...
TL;DR: Youtu-Agent是一个用于自动化生成和持续演化的LLM智能体框架，解决现有框架配置成本高和静态能力的问题，通过模块化设计和混合策略优化实现高效智能体构建。
...
Method: 提出模块化框架Youtu-Agent，包含：1) 结构化配置系统，解耦执行环境、工具包和上下文管理；2) 两种生成范式：工作流模式用于标准任务，元智能体模式用于复杂需求；3) 混合策略优化系统：Agent Practice模块通过上下文优化积累经验，Agent RL模块集成分布式训练框架进行大规模强化学习。

Result: 在WebWalkerQA上达到71.47%，GAIA上达到72.8%的SOTA性能；自动化生成管道工具合成成功率超过81%；Practice模块在AIME 2024/2025上分别提升2.7%和5.4%；Agent RL训练在7B LLM上实现40%加速，在数学和通用/多跳QA基准上分别提升编码/推理能力35%和搜索能力21%。

Conclusion: Youtu-Agent通过自动化生成和持续演化能力，有效解决了LLM智能体框架的高配置成本和静态能力问题，实现了高效、可扩展的智能体构建和优化。
...
### [146] [Multi-modal cross-domain mixed fusion model with dual disentanglement for fault diagnosis under unseen working conditions](https://arxiv.org/abs/2512.24679)
*Pengcheng Xia,Yixiang Huang,Chengjin Qin,Chengliang Liu*

Main category: cs.AI

TL;DR: 该论文提出了一种多模态跨域混合融合模型，通过双重解耦框架分离模态不变/特定特征和域不变/特定表示，结合跨域混合融合策略和三模态融合机制，在未见工作条件下实现更优的故障诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有智能故障诊断方法在真实场景中面临两个主要问题：1) 在未见工作条件下性能显著下降；2) 域自适应方法依赖目标域样本。此外，大多数研究依赖单模态传感信号，忽略了多模态信息的互补性对提高模型泛化能力的作用。

Method: 提出多模态跨域混合融合模型，包含：1) 双重解耦框架，分离模态不变特征和模态特定特征，以及域不变表示和域特定表示；2) 跨域混合融合策略，随机混合跨域的模态信息以增强模态和域多样性；3) 三模态融合机制，自适应集成多模态异构信息。

Result: 在感应电机故障诊断实验中，在未见恒定和时变工作条件下，该方法始终优于先进方法。综合消融研究进一步验证了每个提出组件和多模态融合的有效性。

Conclusion: 该方法通过双重解耦、跨域混合融合和多模态自适应融合，有效解决了现有故障诊断方法在未见工作条件下的泛化问题，充分利用了多模态信息的互补性，实现了更鲁棒的故障诊断性能。
...
### [147] [BatteryAgent: Synergizing Physics-Informed Interpretation with LLM Reasoning for Intelligent Battery Fault Diagnosis](https://arxiv.org/abs/2512.24686)
*Songqi Zhou,Ruixue Liu,Boman Su,Jiazhou Wang,Yixing Wang,Benben Jiang*

Main category: cs.AI

TL;DR: 本文提出BatteryAgent框架，将物理知识特征与大型语言模型结合，实现锂离子电池故障的智能诊断，超越传统二元分类方法，提供可解释的根因分析和维护建议。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法虽然检测精度高，但"黑箱"特性缺乏可解释性，且受限于二元分类范式，无法提供根因分析和维护建议。需要一种既能保持高精度又能提供可解释诊断的新方法。

Method: 提出三层框架：1) 物理感知层：基于电化学原理提取10个机制特征；2) 检测归因层：使用梯度提升决策树和SHAP量化特征贡献；3) 推理诊断层：利用LLM作为智能体核心，构建"数值-语义"桥梁，结合SHAP归因和机制知识库生成综合诊断报告。

Result: 实验结果显示BatteryAgent在困难边界样本上有效纠正误分类，AUROC达到0.986，显著优于现有最先进方法。框架将传统二元检测扩展到多类型可解释诊断。

Conclusion: BatteryAgent框架成功整合物理知识与LLM推理能力，实现了从"被动检测"到"智能诊断"的范式转变，为电池安全管理提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的机器人家庭物品重排系统通常依赖从人类演示中推断的潜在偏好模型，这些模型虽然能有效预测，但对指导人类决策的可解释因素提供有限洞察。因此需要开发一个更可解释的物品摆放偏好模型。

Method: 研究提出了四个可解释的构造维度：空间实用性、习惯便利性、语义连贯性和常识适当性。通过设计并验证了一个自报告问卷，在63名参与者的在线研究中确认了这些维度的心理区分度。然后将这些构造集成到蒙特卡洛树搜索（MCTS）规划器中。

Result: 研究结果证实了四个构造维度的心理区分性及其在厨房和客厅两种场景中的解释力。当MCTS规划器基于参与者衍生的偏好进行指导时，能够生成与参与者生成的摆放方案高度一致的合理摆放方案。

Conclusion: 该工作贡献了一个紧凑、可解释的物品摆放偏好模型，并展示了如何将其操作化用于机器人规划。这为机器人系统提供了更透明、可解释的人类偏好建模方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具备广泛的领域知识，但往往无法捕捉对预测任务至关重要的数据集特定模式。现有方法过度依赖基础模型的领域理解，而忽略了从数据中发现的统计模式。

Method: 采用迭代过程发现语义特征描述，通过统计建模误差对比识别项目组。将问题形式化为广义EM算法，联合优化语义特征描述符和统计模型参数。使用冻结的基础模型基于发现的特征对项目进行分类，将这些判断视为预测实值目标的潜在二元特征的噪声观测。

Result: 在房价预测中，使用多模态列表数据发现的语义特征实现了12%的中位数相对误差，显著优于依赖LLM通用领域知识的GPT-5基线（38%误差）。在Netflix电影嵌入预测中，仅从语义描述就能达到0.59的余弦相似度，匹配了传统协同过滤需要约4000个用户评分才能达到的性能。

Conclusion: GenZ成功地将基础模型的领域知识与统计建模的数据驱动发现相结合，揭示了数据集特定的模式（如建筑细节预测当地住房市场、特许经营成员预测用户偏好），这些模式超出了模型单独依赖领域知识所能捕捉的范围。
...
TL;DR: 提出使用约束模板从养老机构管理者访谈中提取设施特定约束条件的方法，避免提取异常约束，通过约束编程生成护理人员排班表


<details>
  <summary>Details</summary>
Motivation: 养老机构的排班条件因机构而异，需要从制定排班的管理者访谈中提取设施特定的约束条件，但现有约束提取技术无法有效处理异常约束

Method: 使用约束模板提取各种组件的组合（如连续工作班次模式、员工组合），通过改变关注天数、员工数量和提取焦点（模式或频率）来提取多样化约束，并包含排除异常约束的机制

Result: 实验表明，该方法成功创建了满足所有硬约束的排班表，并通过避免提取异常约束减少了软约束的违反次数

Conclusion: 提出的约束模板方法能够有效从养老机构管理者访谈中提取设施特定约束条件，生成满足约束的护理人员排班表，解决了现有技术无法处理异常约束的问题
...
TL;DR: 开发了一个半自动数据标注流水线，结合AI与人工专家，用于波兰驾驶场景多模态数据集标注，显著降低标注成本和时间


<details>
  <summary>Details</summary>
Motivation: DARTS项目需要创建大规模波兰驾驶场景多模态数据集，但手动标注异构数据成本高、耗时长，需要自动化解决方案
...
TL;DR: 迭代部署经过用户精心筛选数据微调的LLMs可以显著改变模型特性，在规划任务中观察到性能提升和泛化能力，理论分析表明这相当于隐式强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 研究迭代部署大型语言模型时，用户从先前模型部署中精心筛选数据进行微调，这种机制如何影响最终模型的特性变化。

Method: 在不同规划领域测试迭代部署机制，观察模型规划技能的变化；提供理论分析，证明迭代部署实际上实现了外层循环的强化学习训练，具有隐式奖励函数。

Result: 观察到规划技能显著提升，后期模型展现出涌现的泛化能力，能够发现比初始模型长得多的规划方案。

Conclusion: 迭代部署机制与强化学习有重要联系：对AI安全领域有影响，因为重复部署隐含的奖励函数未明确定义，可能对未来模型部署产生意外影响；同时这种机制可作为显式强化学习的替代训练方案，依赖数据筛选而非显式奖励。
