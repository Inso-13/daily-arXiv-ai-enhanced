...
Method: 提出双框架解决方案：1) EGxFSL：集成放射科医生定义的感兴趣区域作为空间监督，通过基于Grad-CAM的Dice损失与原型分类联合优化，实现可解释的少样本学习；2) xGAL：引入迭代样本获取策略，优先考虑预测不确定性和注意力错位，创建可解释性指导训练和样本选择的闭环框架。

Result: 在BraTS（MRI）、VinDr-CXR（胸部X光）和SIIM-COVID-19（胸部X光）数据集上分别达到92%、76%和62%的准确率，在所有数据集上始终优于非引导基线。在严重数据限制下，xGAL仅用680个样本就达到76%准确率，而随机采样为57%。Grad-CAM可视化显示引导模型专注于诊断相关区域，在乳腺超声上的泛化验证了跨模态适用性。

Conclusion: 提出的EGxFSL和xGAL框架有效解决了医学图像分析中的数据稀缺和可解释性问题，通过专家引导和可解释性驱动的主动学习，在有限数据下实现了高性能和临床可解释性，具有跨模态的泛化能力。
...
Conclusion: MIAR方法通过有效处理模态分布差异和贡献度问题，结合对比学习和归一化策略，在多模态情感识别任务中取得了优越性能
...
TL;DR: CoCoT框架通过动态多区域定位和关系感知推理解决多模态推理中单区域依赖和语义碎片化问题，在多个基准测试上显著提升性能。
...
Result: 构建了包含74,691个高质量样本的CoCoT-70K数据集，实验显示CoCoT显著增强复杂视觉推理能力，在LLaVA-1.5上平均准确率提升15.4%，在Qwen2-VL上提升4.0%。

Conclusion: CoCoT框架通过动态多区域定位和关系感知推理有效解决了多模态推理中的关键限制，显著提升了复杂视觉推理性能，相关数据和代码已开源。
...
TL;DR: NitroGen是一个基于40,000小时游戏视频训练的游戏智能体基础模型，在1,000多款游戏中表现出色，能够实现跨游戏泛化。
...
Result: NitroGen在3D动作游戏战斗、2D平台游戏高精度控制和程序生成世界探索等多样领域表现优异，在未见游戏中任务成功率相比从头训练模型提升高达52%。
...
TL;DR: 该研究从信息流视角分析文本推理在盲图像质量评估中的作用，比较了三种学习图像-文本-分数关系的范式，发现自一致性范式能显著缩小图像与文本条件预测的差距。


<details>
  <summary>Details</summary>
Motivation: 尽管文本推理在盲图像质量评估中得到广泛应用，但文本信息如何贡献于质量预测以及文本能在多大程度上代表与分数相关的图像内容仍不清楚。本研究旨在从信息流视角解决这些问题。

Method: 通过比较现有BIQA模型与三种设计用于学习图像-文本-分数关系的范式：思维链、自一致性和自编码器范式，从信息流角度分析文本推理的作用。

Result: 实验表明：1）仅使用文本信息时现有模型的预测性能显著下降；2）思维链范式对BIQA性能提升有限；3）自一致性范式显著缩小了图像与文本条件预测的差距（PLCC/SRCC差异缩小到0.02/0.03）；4）自编码器范式在缩小图像-文本差距方面效果较差，但指出了进一步优化的方向。
...
TL;DR: MLLMs在医学图像分类任务中表现不佳，特别是在膝骨关节炎X光片分类上，视觉编码器单独训练比完整MLLM管道表现更好，数据平衡比数据规模更重要。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在医学视觉问答和报告生成方面表现良好，但其生成和解释能力并不能可靠地转移到疾病特异性分类任务上。膝骨关节炎影响全球3-4亿人，但在现有医学MLLM基准测试中代表性不足，需要评估MLLM在该特定分类任务上的表现。

Method: 通过系统消融研究，操纵视觉编码器、连接器和大型语言模型组件，采用不同的训练策略，测量每个组件对诊断准确性的贡献。比较了视觉编码器单独训练与完整MLLM管道的性能，评估了基于提示的指导与微调LLM的效果，并对比了小规模平衡数据集（500张图像）与大规模不平衡数据集（5,778张图像）的训练结果。

Result: 在膝骨关节炎X光片分类任务中，单独训练的视觉编码器在分类准确性上优于完整的MLLM管道；微调LLM相比基于提示的指导没有提供有意义的改进；在小型平衡数据集上的LoRA微调比在大型不平衡数据集上训练获得更好的结果，表明数据平衡和质量比原始规模更重要。
...
TL;DR: 该论文提出两种基于隐式神经表示（INR）的框架，用于处理各向异性的视网膜OCT图像，实现密集3D分析，包括利用en-face模态的B扫描插值和创建分辨率无关的视网膜图谱。


<details>
  <summary>Details</summary>
Motivation: 临床OCT成像通常具有较大的切片间距，导致高度各向异性的图像和稀疏扫描的视网膜。现有的2D方法存在相邻B扫描结果不一致的问题，且传统CNN受限于训练数据分辨率，无法适应不同成像协议。INR作为连续表示工具，具有分辨率无关特性，适合处理各向异性数据。

Method: 提出两种基于INR的框架：1）通过结合en-face模态的额外信息进行B扫描间插值，保留B扫描间的相关结构；2）创建分辨率无关的视网膜图谱，实现无需严格数据要求的通用分析。两种方法都利用可泛化的INR，通过基于群体的训练改进视网膜形状表示，并允许对未见病例进行预测。

Result: 提出的分辨率无关框架能够处理具有大B扫描间距的OCT图像，为视网膜结构和病理的容积评估开辟了可能性。INR的连续表示特性使系统能够适应不同分辨率和成像协议的数据。

Conclusion: 基于INR的框架为各向异性OCT图像的密集3D分析提供了有效解决方案，通过B扫描插值和分辨率无关图谱，克服了传统2D方法和CNN的限制，实现了对稀疏扫描视网膜的容积评估。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D基础模型在全局任务上表现良好，但在局部部件级推理上表现不佳。现有方法依赖多视角渲染和文本查询，需要昂贵的推理过程和复杂的LLM提示工程，且未能充分利用3D几何信息

Method: 使用编码器架构的3D模型，直接从点云生成语言对齐的patch级特征。预训练分为两阶段：1) 从视觉编码器（如DINOv2）蒸馏密集2D特征到3D patch；2) 通过多正对比目标将这些patch嵌入与部件级文本嵌入对齐

Result: 3D编码器实现了零样本3D部件分割，具有快速单次推理能力，无需测试时的多视角渲染，在多个3D部件分割基准上显著优于之前的渲染方法和前馈方法

Conclusion: 提出的方法通过直接处理点云并实现语言对齐的patch级特征，克服了现有方法的局限性，为3D部件级推理提供了更高效准确的解决方案
...
TL;DR: 提出一种轻量级计算机视觉框架，将3D CT数据重构为序列视频流，结合YOLO检测和ByteTrack跟踪，实现颅内出血检测，在边缘设备上以低计算成本近似3D上下文推理。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上的3D医学影像分析受限于3D CNN的高内存和计算需求，需要开发轻量级解决方案来平衡2D检测效率和3D上下文必要性。

Method: 将体积CT数据重构为序列视频流；基准测试YOLO v8/v10/v11/v12 Nano配置选择最佳切片级骨干；引入ByteTrack算法确保z轴解剖一致性；提出混合推理策略和时空一致性滤波器减少初始化延迟。
...
TL;DR: MovieRecapsQA：首个提供显式文本上下文的开源多模态视频问答基准，基于电影解说视频构建，包含约8.2K个问答对，支持无参考评估。
...
Method: 利用YouTube电影解说视频构建基准，通过解说摘要生成约8.2K个与电影字幕对齐的问答对，提供验证答案所需的"事实"信息，支持无参考评估。

Result: 评估7个最先进MLLM发现：1)纯视觉问题最具挑战性；2)模型倾向于优先使用文本输入；3)从视频中提取准确事实信息仍然困难；4)专有和开源模型在视频依赖问题上表现相当。

Conclusion: MovieRecapsQA填补了开放端多模态视频问答基准的空白，为评估模型在真实视频理解中的多模态推理能力提供了新工具，揭示了当前模型在处理视觉信息和多模态整合方面的局限性。
...
TL;DR: DreamLoop：首个无需训练数据、从单张照片生成可控电影图（cinemagraph）的视频合成框架，通过时间桥接和运动条件化实现无缝循环动画。


<details>
  <summary>Details</summary>
Motivation: 现有技术存在局限性：传统图像动画方法只能处理简单低频运动，局限于重复纹理场景；大规模视频扩散模型不适用于电影图约束，缺乏专门数据生成无缝可控循环。需要一种能从单张照片生成可控电影图的方法。

Method: 提出DreamLoop框架，通过训练通用视频扩散模型实现两个目标：时间桥接（连接帧间关系）和运动条件化。推理时：1）使用输入图像作为首尾帧条件强制无缝循环；2）通过静态轨迹条件保持背景静止；3）通过用户指定的目标物体运动路径控制动画轨迹和时序。

Result: 方法能生成高质量、复杂的电影图，符合用户意图，优于现有方法。这是首个支持通用场景、具有灵活直观控制的电影图生成方法。

Conclusion: DreamLoop成功解决了从单张照片生成可控电影图的挑战，无需专门的电影图训练数据，通过创新的训练策略实现了灵活可控的电影图生成，为艺术创作提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 随着生成模型（扩散模型和GANs）的快速发展，区分合成图像与真实图像变得越来越困难。现有检测方法在面对新型或未见过的生成模型时，检测准确率往往下降，存在泛化能力不足的挑战。

Method: 提出基于通道移除重建的检测范式：移除图像的绿色（G）通道并进行重建，利用真实图像与AI生成图像在重建误差上的显著差异。具体方法是计算G通道移除重建误差（GRRE），通过这种简单而有效的差异进行检测。

Result: 大量实验表明，GRRE在多个生成模型（包括训练期间未见过的模型）上都能保持高检测准确率。相比现有方法，GRRE不仅对各种扰动和后处理操作具有强鲁棒性，还表现出优越的跨模型泛化能力。

Conclusion: 基于通道移除重建的方法作为强大的取证工具，在生成式AI时代具有保护图像真实性的潜力。GRRE方法简单有效，为解决AI生成图像检测的泛化问题提供了新思路。
...
TL;DR: CAMO是一个无需模板的通用框架，能够从单目2D视频直接将运动转移到多样化的3D网格上，通过形态参数化高斯溅射模型和语义对应解决形状-姿态歧义问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要类别特定的参数化模板，难以处理姿态歧义和多样化的物体形状，限制了从2D视频到3D资产的运动转移的通用性。

Method: 提出CAMO框架，核心是形态参数化的关节3D高斯溅射模型，结合密集语义对应关系，通过优化联合调整形状和姿态，有效缓解形状-姿态歧义问题。

Result: 实验结果表明，CAMO在运动准确性、效率和视觉一致性方面优于现有方法，显著推进了多样化物体类别和日常视频场景中的运动转移技术。

Conclusion: CAMO框架成功实现了无需模板的通用运动转移，通过形态参数化高斯溅射和语义对应解决了形状-姿态歧义问题，为多样化类别的运动转移提供了有效解决方案。
...
### [14] [Robust Mesh Saliency GT Acquisition in VR via View Cone Sampling and Geometric Smoothing](https://arxiv.org/abs/2601.02721)
...
<details>
  <summary>Details</summary>
Motivation: 当前3D网格显著性标注方法大多沿用2D图像方法，忽略了3D几何拓扑与2D图像阵列的本质差异。现有VR眼动追踪流程依赖单射线采样和欧几里得平滑，容易引发纹理注意力和跨间隙信号泄漏问题，需要更符合人类自然感知的高保真3D注意力获取方法。

Method: 提出视锥采样策略，通过高斯分布的射线束模拟人类中央凹感受野，提高复杂拓扑的采样鲁棒性；开发混合流形-欧几里得约束扩散算法，融合流形测地约束与欧几里得尺度，确保拓扑一致的显著性传播。

Result: 该框架通过缓解"拓扑短路"和混叠问题，提供了符合人类自然感知的高保真3D注意力获取范式，为3D网格显著性研究提供了更准确和鲁棒的基准。

Conclusion: 提出的框架解决了现有3D网格显著性标注方法的局限性，通过视锥采样和混合约束扩散算法，实现了更符合人类视觉感知的3D注意力获取，为VR中的人为中心视觉建模提供了可靠基础。
...
TL;DR: 提出了一种前景感知的数据集蒸馏方法，通过内容自适应增强补丁选择，解决传统方法信息丢失问题，提升蒸馏性能


<details>
  <summary>Details</summary>
Motivation: 传统数据集蒸馏方法存在计算开销大、内存限制、生成不真实噪声图像、架构泛化能力有限等问题；最近的基于补丁的非优化方法虽然缓解了部分问题，但刚性补丁选择策略可能丢弃主要对象的关键信息

Method: 首先利用Grounded SAM2识别前景对象并计算每张图像的前景占有率，从中推导类别级补丁决策阈值；基于这些阈值设计动态补丁选择策略，对每张图像要么从多个候选补丁中选择信息量最大的，要么在前景占主导时直接调整全图大小；这种双路径机制在减少冗余背景内容的同时保留更多关于主要对象的关键信息

Result: 在多个基准测试上的广泛实验表明，该方法相比现有方法持续提升蒸馏性能，生成更具信息量和代表性的蒸馏数据集，并增强跨不同架构和图像组合的鲁棒性

Conclusion: 提出的前景感知数据集蒸馏方法通过内容自适应的动态补丁选择，有效解决了传统方法中关键信息丢失的问题，显著提升了数据集蒸馏的效果和实用性
...
### [16] [Unveiling and Bridging the Functional Perception Gap in MLLMs: Atomic Visual Alignment and Hierarchical Evaluation via PET-Bench](https://arxiv.org/abs/2601.02737)
...
TL;DR: 该研究揭示了多模态大语言模型在功能成像中的感知差距，提出了PET-Bench基准，发现了思维链幻觉陷阱，并通过原子视觉对齐方法显著提升了诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在解剖模态的异常检测和报告生成方面表现出色，但它们在功能成像方面的能力尚未得到充分探索。研究者发现当前视觉编码器无法独立于形态先验解码功能示踪剂生物分布，这是一个根本性的功能感知差距。

Method: 研究引入PET-Bench作为首个大规模功能成像基准，包含52,308个层次化QA对。评估了19个最先进的多模态大语言模型，发现了思维链幻觉陷阱。提出了原子视觉对齐（AVA）微调策略，强制模型在高级诊断推理前掌握低级功能感知。

Result: 评估显示标准思维链提示会解耦语言生成与视觉证据，产生临床流畅但事实无根据的诊断。AVA方法有效弥合了感知差距，将思维链从幻觉来源转变为稳健推理工具，诊断准确性提升高达14.83%。

Conclusion: 研究揭示了多模态大语言模型在功能成像中的关键局限性，提出了有效的解决方案。AVA方法通过强制低级功能感知先于高级推理，显著改善了模型在PET成像中的诊断性能，为功能成像的AI应用提供了重要基准和方法。
...
### [17] [D$^3$R-DETR: DETR with Dual-Domain Density Refinement for Tiny Object Detection in Aerial Images](https://arxiv.org/abs/2601.02747)
*Zixiao Wen,Zhen Yang,Xianjie Bao,Lei Zhang,Xiantai Xiang,Wenshuai Li,Yuhan Liu*

Main category: cs.CV

TL;DR: 提出D³R-DETR，一种基于DETR的双域密度细化检测器，用于遥感图像中的微小目标检测，通过融合空间和频域信息改进特征图并预测更准确的目标密度图。
...
Conclusion: D³R-DETR通过双域密度细化有效解决了遥感图像中微小目标检测的挑战，在精度和性能上超越了现有方法。
...
### [18] [Towards Zero-Shot Point Cloud Registration Across Diverse Scales, Scenes, and Sensor Setups](https://arxiv.org/abs/2601.02759)
*Hyungtae Lim,Minkyun Seo,Luca Carlone,Jaesik Park*

Main category: cs.CV

TL;DR: BUFFER-X是一个无需训练的零样本点云配准框架，通过几何引导的超参数估计、分布感知采样和块级坐标归一化解决跨域泛化问题，并在12个数据集上验证了有效性。
...
Method: 提出BUFFER-X框架：1) 几何引导的超参数自动估计；2) 分布感知最远点采样替代学习检测器；3) 块级坐标归一化确保尺度一致性。采用分层多尺度匹配从局部、中间和全局感受野提取对应关系。还提出BUFFER-X-Lite版本，通过提前退出策略和快速位姿求解器减少43%计算时间。

Result: 在包含12个数据集的综合基准测试中验证，涵盖物体尺度、室内和室外场景，包括异构LiDAR配置间的跨传感器配准。结果表明该方法无需手动调参或测试域先验知识即可有效泛化。
...
### [19] [AnyDepth: Depth Estimation Made Easy](https://arxiv.org/abs/2601.02760)
*Zeyu Ren,Zeyu Zhang,Wukai Li,Qingxiang Liu,Hao Tang*

Main category: cs.CV

TL;DR: 提出了一种轻量级、数据中心的零样本单目深度估计框架，使用DINOv3作为视觉编码器，设计了参数减少85%-89%的Simple Depth Transformer解码器，并通过质量过滤策略优化数据集质量。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法依赖大规模数据集和复杂解码器，限制了效率和泛化能力。需要开发更轻量、数据高效且具有良好泛化性的零样本深度估计方法。
...
Conclusion: 这项工作强调了平衡模型设计和数据质量对于实现高效且泛化性强的零样本深度估计的重要性，为轻量级深度估计提供了新的解决方案。
...
### [20] [ClearAIR: A Human-Visual-Perception-Inspired All-in-One Image Restoration](https://arxiv.org/abs/2601.02763)
*Xu Zhang,Huan Zhang,Guoli Wang,Qian Zhang,Lefei Zhang*

Main category: cs.CV

TL;DR: ClearAIR是一个受人类视觉感知启发的全合一图像修复框架，采用分层粗到细的修复策略，通过MLLM质量评估、区域感知任务识别和内部线索重用机制，在合成和真实数据集上取得优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有全合一图像修复方法过度依赖退化特定表示，常导致过度平滑和伪影问题，需要更有效的处理复杂真实世界退化的方法。
...
Conclusion: ClearAIR通过模仿人类视觉感知的分层粗到细策略，有效解决了现有全合一图像修复方法的局限性，在复杂退化场景中表现出色。
...
### [21] [AbductiveMLLM: Boosting Visual Abductive Reasoning Within MLLMs](https://arxiv.org/abs/2601.02771)
*Boyu Chang,Qi Wang,Xi Guo,Zhixiong Nan,Yazhou Yao,Tianfei Zhou*

Main category: cs.CV

TL;DR: AbductiveMLLM：通过模拟人类认知中语言与图像的双重推理模式，增强多模态大语言模型的视觉溯因推理能力，在标准VAR基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然具备强大的通用多模态推理能力，但在溯因推理方面仍不及人类水平。研究受人类认知中语言与图像双重推理模式相互作用的启发，旨在弥合这一差距。

Method: 提出AbductiveMLLM框架，包含两个协同组件：REASONER（语言域操作，先探索可能的解释空间，再基于跨模态因果对齐修剪视觉不一致假设）和IMAGINER（模拟人类图像思维，通过文本到图像扩散模型"想象"与语言解释对应的视觉场景）。两个组件以端到端方式联合训练。

Result: 在标准视觉溯因推理基准测试中，AbductiveMLLM实现了最先进的性能，一致优于传统解决方案和先进的多模态大语言模型。

Conclusion: 通过模拟人类认知中语言与图像双重推理的相互作用，可以有效增强多模态大语言模型的溯因推理能力，在视觉溯因推理任务上取得显著改进。
...
Conclusion: DreamStyle提供了一个统一的视频风格化框架，解决了现有方法的局限性，通过支持多种风格条件和高质量数据训练，实现了更好的风格一致性和视频质量。
...
Conclusion: 提出的StableDPT方法成功将图像深度估计模型适配到视频处理，通过时间模块和高效推理策略实现了更好的时间稳定性和处理效率
...
TL;DR: 该论文提出了一种拓扑感知的H&E到IHC虚拟染色框架，通过拓扑感知一致性匹配和拓扑约束病理匹配机制，解决了相邻切片数据空间错位问题，在四个染色任务上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: IHC染色在癌症诊断中至关重要，但相比常用的H&E染色，IHC染色过程复杂、耗时且昂贵，限制了其临床应用。虚拟染色技术可以将H&E图像转换为IHC图像，但使用相邻切片作为真实标签会导致弱配对数据，存在空间错位和局部变形问题，阻碍了有效的监督学习。

Method: 提出了一种新颖的拓扑感知框架，包含两个核心机制：1) 拓扑感知一致性匹配(TACM)机制，采用图对比学习和拓扑扰动来学习鲁棒的匹配模式，确保结构一致性；2) 拓扑约束病理匹配(TCPM)机制，基于节点重要性对齐病理阳性区域，增强病理一致性。

Result: 在两个基准数据集上的四个染色任务上进行了广泛实验，结果表明该方法超越了现有最先进的方法，实现了更高的生成质量和临床相关性。

Conclusion: 提出的拓扑感知框架有效解决了H&E到IHC虚拟染色中的空间错位问题，通过拓扑感知机制实现了更好的结构一致性和病理一致性，为临床提供了一种成本效益更高的替代方案。
...
TL;DR: SketchThinker-R1通过引入草图式推理，在大规模多模态模型中实现高效推理，减少64%的推理token成本而不影响答案准确性。


<details>
  <summary>Details</summary>
Motivation: 当前大规模多模态模型采用逐步推理方法虽然有效，但计算开销大（token成本高、响应时间长），影响了推理效率。相比之下，人类常使用草图式推理——一种简洁、目标导向的认知过程，能优先处理关键信息并实现高效问题解决。

Method: 方法包含三个阶段：1）草图模式冷启动阶段：将标准长推理过程转换为草图式推理并微调基础多模态模型，培养初始草图式推理能力；2）训练SketchJudge奖励模型：明确评估模型思考过程，为草图式推理分配更高分数；3）在SketchJudge监督下进行草图思维强化学习，进一步泛化草图式推理能力。

Result: 在四个基准测试上的实验评估显示，SketchThinker-R1实现了超过64%的推理token成本降低，同时不损害最终答案准确性。定性分析进一步表明，草图式推理在问题解决过程中更关注关键线索。

Conclusion: SketchThinker-R1通过模仿人类草图式推理的认知效率，成功在大规模多模态模型中实现了高效的推理过程，显著降低了计算开销，为提升模型推理效率提供了有效方法。
...
TL;DR: DGA-Net利用深度提示范式改进SAM模型进行伪装目标检测，通过跨模态图增强模块融合RGB语义和深度几何信息，并设计锚点引导细化模块防止特征层次中的信息衰减。
...
Method: 提出DGA-Net框架，采用"深度提示"范式适配Segment Anything Model。包含两个核心模块：1) 跨模态图增强模块，在异质图中合成RGB语义和深度几何信息形成统一指导信号；2) 锚点引导细化模块，创建全局锚点并建立直接的非局部路径，将指导信号从深层广播到浅层，确保精确一致的分割。

Result: 定量和定性实验结果表明，提出的DGA-Net在伪装目标检测任务上优于现有最先进方法。

Conclusion: DGA-Net通过深度提示范式和两个创新模块，有效利用深度线索提升伪装目标检测性能，为SAM模型在特定视觉任务上的适配提供了新思路。
...
TL;DR: SEF-DETR是一个针对红外小目标检测的DETR改进框架，通过频率引导的补丁筛选、动态嵌入增强和可靠性一致性融合三个模块，解决了自注意力机制中目标特征被背景特征淹没的问题，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测面临信噪比低、目标尺寸小、背景复杂等挑战。现有的DETR基检测器虽然受益于全局上下文建模，但在红外小目标检测上表现不佳。研究发现，由于自注意力机制，红外小目标的相关嵌入不可避免地被主导的背景特征淹没，导致查询初始化不可靠和目标定位不准确。

Method: 提出了SEF-DETR框架，包含三个核心组件：1) 频率引导的补丁筛选(FPS)：利用局部补丁的傅里叶频谱构建目标相关密度图，抑制背景主导特征；2) 动态嵌入增强(DEE)：以目标感知的方式增强多尺度表示；3) 可靠性一致性融合(RCF)：通过强制空间-频率一致性和可靠性来进一步细化对象查询。

Result: 在三个公开的红外小目标检测数据集上进行了广泛实验，结果表明SEF-DETR相比最先进方法实现了更优越的检测性能，为红外小目标检测任务提供了鲁棒且高效的解决方案。

Conclusion: SEF-DETR通过改进查询初始化机制，有效解决了红外小目标检测中目标特征被背景淹没的问题，显著提升了检测性能，为红外小目标检测任务提供了一个有效的框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像分割方法主要依赖掩码基础框架，缺乏对分割不确定性的建模能力，需要一种更通用、能处理模糊性的分割方法

Method: 采用扩散模型框架，引入位置感知调色板和2D格雷码排序优化离散数据表示，使用tanh激活函数处理离散数据，采用sigmoid损失权重和x-prediction策略

Result: 当前模型虽未超越领先的掩码架构，但显著缩小了性能差距，并具备建模分割模糊性的独特能力，为后续改进奠定基础

Conclusion: 扩散模型为通用图像分割提供了有前景的替代方案，结合大规模预训练或提示条件可发展出具有竞争力的模型
...
TL;DR: 提出TA-Prompting方法，通过时间锚点增强VideoLLMs，解决现有方法在未修剪视频中事件边界定位不精确的问题，提升密集视频描述性能。
...
Method: 提出TA-Prompting方法：1) 使用时间锚点学习精确事件定位；2) 引导VideoLLMs进行时间感知的视频事件理解；3) 引入事件连贯性采样策略，从任意数量的事件中选择具有时间连贯性和跨模态相似性的描述。

Result: 在基准数据集上的实验表明，TA-Prompting在密集视频描述、时刻检索和TemporalQA等时间理解任务上优于现有最先进的VideoLLMs方法。

Conclusion: TA-Prompting通过时间锚点机制有效提升了VideoLLMs在未修剪视频中的事件边界定位能力，显著改善了密集视频描述的时间准确性和描述质量。
...
TL;DR: Zoom-IQA：一种基于视觉语言模型的图像质量评估方法，通过模拟人类认知行为（不确定性感知、区域推理和迭代优化）来同时生成质量描述和分数，相比现有方法具有更好的鲁棒性、可解释性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像质量评估方法存在两个主要问题：1）传统方法只预测数值分数而不提供解释；2）基于视觉语言模型的方法虽然能同时生成描述和分数，但由于视觉和文本线索整合能力有限，推理过程不可靠。需要一种能模拟人类认知行为的更可靠的IQA方法。
...
Result: 实验表明Zoom-IQA在鲁棒性、可解释性和泛化能力方面都有显著提升。在下游任务（如图像恢复）中的应用进一步证明了该方法的有效性。

Conclusion: Zoom-IQA通过模拟人类认知的关键行为（不确定性感知、区域推理和迭代优化），成功解决了现有VLM-based IQA方法推理不可靠的问题，为图像质量评估提供了更可靠、可解释的解决方案。
...
TL;DR: DCG-ReID提出动态置信度解耦加权机制，针对多模态车辆重识别中平衡与不平衡质量分布数据的不同需求，分别设计协作融合和引导融合策略，解决模态异质性与类内一致性的冲突问题。


<details>
  <summary>Details</summary>
Motivation: 多模态车辆重识别面临模态质量分布不确定性的挑战，现有方法使用单一融合模型处理所有数据，忽略了平衡与不平衡质量分布数据的不同融合需求，难以解耦类内一致性与模态异质性之间的冲突。

Method: 提出DCG-ReID方法：1) 动态置信度解耦加权机制(DCDW)通过交互衍生的模态置信度动态重加权三模态贡献，构建解耦融合框架；2) 针对平衡质量分布设计协作融合模块(CFM)，挖掘成对共识特征；3) 针对不平衡分布设计引导融合模块(GFM)，差异化放大模态判别差异，强化主导模态优势。

Result: 在三个多模态重识别基准数据集(WMVeID863, MSVR310, RGBNT100)上进行广泛实验，验证了方法的有效性。

Conclusion: DCG-ReID通过解耦异质质量分布模态数据，针对不同场景设计专门融合策略，有效解决了多模态车辆重识别中模态异质性与类内一致性的冲突问题，提升了多模态联合决策性能。
...
Method: 采用两阶段互补架构：1）粗粒度异常评分模块通过文本锚点相似性计算帧级异常分数；2）MLLM精化模块通过系统和用户提示对异常进行上下文理解。文本锚点和提示通过弱监督自动提示工程框架优化
...
TL;DR: 该研究构建了首个专门针对虚拟试穿图像的多维度质量评估数据集VTONQA，包含8,132张图像和24,396个主观评分，用于评估虚拟试穿模型的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿模型常出现服装变形和身体不一致等伪影问题，缺乏可靠的质量评估方法，需要专门的数据集来支持虚拟试穿图像的质量评估。

Method: 构建VTONQA数据集，包含11个代表性虚拟试穿模型生成的8,132张图像，收集了24,396个平均意见分数，涵盖服装合身度、身体兼容性和整体质量三个评估维度。

Result: 基于VTONQA数据集对虚拟试穿模型和多种图像质量评估指标进行了基准测试，揭示了现有方法的局限性，证明了所提数据集的价值。

Conclusion: VTONQA数据集和相应基准为感知对齐评估提供了坚实基础，有助于质量评估方法的发展和虚拟试穿模型的进步。
...
TL;DR: LAMS-Edit：一种基于扩散模型的文本到图像编辑框架，通过潜在表示和注意力图的加权插值来平衡内容保持与编辑应用
...
Method: 提出LAMS-Edit框架，利用反转过程中的中间状态（潜在表示和注意力图），在编辑图像生成过程中通过加权插值进行混合，使用调度器控制混合权重。该方法与Prompt-to-Prompt（P2P）结合，支持区域掩码精确编辑和通过LoRA实现风格迁移

Result: 大量实验表明，LAMS-Edit能够有效平衡内容保持与编辑应用，在真实图像编辑任务中表现良好

Conclusion: LAMS-Edit提供了一个可扩展的文本到图像编辑框架，通过利用反转过程的中间状态，成功解决了内容保持与编辑应用之间的平衡问题，并支持精确编辑和风格迁移功能
...
TL;DR: ULS+是通用病灶分割模型的增强版本，通过整合新数据集和使用更小输入图像尺寸，在CT扫描中实现了更高的病灶分割精度和更快的推理速度。


<details>
  <summary>Details</summary>
Motivation: 原始ULS模型在CT扫描中分割全身病灶，但自发布以来出现了多个新的公共数据集，这些数据集可以进一步提升模型性能。同时，原始模型在精度和推理速度方面仍有改进空间。

Method: ULS+整合了新的公共数据集，并采用了更小的输入图像尺寸。模型在ULS23挑战赛测试数据和Longitudinal-CT数据子集上进行评估，使用Dice分数和点击点位置鲁棒性作为评价指标。

Result: 在所有比较中，ULS+显著优于原始ULS模型。ULS+在ULS23挑战赛测试阶段排行榜上排名第一，表现出更高的准确性和更快的推理速度。

Conclusion: 通过保持数据驱动更新和临床验证的循环，ULS+为构建鲁棒且具有临床相关性的病灶分割模型奠定了基础，展示了持续改进医学影像分析模型的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 漫画视觉问答对多模态大语言模型提出了独特挑战，因为其依赖符号抽象、叙事逻辑和幽默元素。传统思维链提示方法直接应用于CVQA时，特别是在小型模型中，反而会降低性能。

Method: 提出了一种新颖的漫画推理框架，结合模块化思维链生成、基于GRPO的强化微调以及新颖的结构化奖励机制，旨在为小型MLLMs生成更忠实和可迁移的推理链。

Result: 在五个具有挑战性的基准测试中，提出的3B模型超越了现有最先进方法，插件实验在不同MLLMs上带来了平均12.1%的额外改进。

Conclusion: 该研究解决了CVQA中传统思维链方法的局限性，提出的框架不仅适用于漫画VQA，还能扩展到更广泛的幽默中心和抽象视觉推理任务，为小型MLLMs在复杂推理任务中的应用提供了有效解决方案。
...
TL;DR: RiSe框架通过潜在场-轨迹相关模型和意图驱动区域预测模块，实现基于风险意图的选择性检测，将通信量降至全特征共享的0.71%同时保持SOTA检测精度。
...
Method: 提出风险意图选择性检测框架RiSe：1) 基于潜在场理论的潜在场-轨迹相关模型定量评估运动学风险；2) 意图驱动区域预测模块利用自车运动先验主动预测和过滤关键BEV区域；3) 语义选择性融合方案仅传输高交互区域的高保真特征。

Result: 在DeepAccident数据集上的实验表明，该方法将通信量降至全特征共享的0.71%，同时保持了最先进的检测精度，在带宽效率和感知性能之间建立了有竞争力的帕累托前沿。

Conclusion: RiSe框架将范式从识别可见区域转向优先考虑风险关键区域，通过语义选择性融合方案有效充当特征去噪器，在车路协同感知中实现了通信效率与感知性能的良好平衡。
...
TL;DR: SA-ResGS是一个用于主动场景重建中下一最佳视角选择的框架，通过自增强残差3D高斯溅射技术，稳定不确定性量化并增强不确定性感知监督，提高重建质量和视角选择鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决主动场景重建中下一最佳视角选择的两个关键挑战：1）稀疏和宽基线视角导致的监督不足问题；2）不确定性量化不稳定及其在监督中的有效性不足问题。

Method: 提出自增强残差3D高斯溅射框架，包含三个核心组件：1）通过训练视角与光栅化外推视角之间的三角测量生成自增强点云，实现高效场景覆盖估计；2）首个针对3D高斯溅射的残差学习策略，结合不确定性驱动过滤与dropout和硬负样本挖掘启发的采样；3）物理基础的视角选择策略促进高效均匀的场景覆盖。

Result: 在主动视角选择实验中，SA-ResGS在重建质量和视角选择鲁棒性方面均优于最先进的基线方法。该框架还实现了不确定性量化的隐式去偏，缓解了宽基线探索和稀疏视角模糊性在NBV规划中的冲突效应。

Conclusion: SA-ResGS通过自增强点云生成和残差监督策略，有效解决了主动场景重建中的监督不足和不确定性量化不稳定问题，为下一最佳视角选择提供了更可靠和高效的解决方案。
...
Method: 提出DFRCP模块，通过结合大尺度和中尺度特征增强YOLOv11特征金字塔，同时保持原始表示；引入动态鲁棒开关单元，自适应注入模糊特征以增强抖动下的全局感知。模糊特征通过对多尺度特征进行旋转和非线性插值合成，然后通过透明度卷积学习内容自适应权衡原始和模糊线索。还开发了CUDA并行旋转和插值内核，避免边界溢出并实现400倍以上的加速。

Result: 在约3,500张图像的私有小麦病虫害数据集上进行训练，使用两种模糊机制进行三倍增强。在模糊测试集上，配备DFRCP的YOLOv11比YOLOv11基线准确率提高约10.4%，仅增加适度的训练时间开销。

Conclusion: DFRCP作为YOLOv11的插件升级，显著提高了运动模糊条件下的目标检测性能，通过高效的CUDA实现使其适用于边缘部署，减少了数据收集后的人工筛选需求。
...
TL;DR: ViT在空间推理任务中存在系统性失败，这源于架构固有的电路复杂性限制，而非数据规模问题。作者将空间理解形式化为群同态学习，证明对于非可解群（如SO(3)），保持结构保持嵌入的计算复杂度下界为NC^1-complete，而恒定深度ViT仅限TC^0，存在复杂度边界。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在语义识别方面表现出色，但在空间推理任务（如心理旋转）中表现出系统性失败。传统观点常将此归因于数据规模，但作者认为这源于架构固有的电路复杂性限制。

Method: 将空间理解形式化为群同态学习：将图像序列映射到保持底层变换群代数结构的潜在空间。证明对于非可解群（如3D旋转群SO(3)），保持结构保持嵌入的计算复杂度下界为Word Problem（NC^1-complete）。对比证明恒定深度ViT在多项式精度下严格受限于TC^0。

Result: 在TC^0 ⊊ NC^1猜想下，建立了复杂度边界：恒定深度ViT从根本上缺乏有效捕捉非可解空间结构所需的逻辑深度。通过潜在空间探测验证了这一复杂度差距，显示ViT表示在非可解任务上随着组合深度增加而出现结构崩溃。

Conclusion: ViT在空间推理任务中的局限性源于架构固有的电路复杂性边界，而非数据规模问题。恒定深度ViT无法有效捕捉非可解空间结构，这为理解Transformer架构的能力边界提供了理论框架。
...
TL;DR: IBISAgent：一种将医学图像分割重新定义为视觉中心多步决策过程的智能MLLM，通过交错推理和基于文本的点击操作实现高质量分割，无需架构修改


<details>
  <summary>Details</summary>
Motivation: 现有医学MLLM分割方法面临两大挑战：1）引入隐式分割标记并需要同时微调MLLM和外部像素解码器，增加了灾难性遗忘风险并限制了泛化能力；2）依赖单次推理，缺乏迭代优化分割结果的能力

Method: 提出IBISAgent智能MLLM，将分割重新定义为视觉中心的多步决策过程，通过交错推理和文本点击操作调用分割工具；采用两阶段训练框架：冷启动监督微调和带细粒度奖励的智能强化学习

Result: 在医学参考和推理分割任务中，IBISAgent在闭源和开源SOTA方法上均表现优异，展示了强大的像素级视觉推理能力

Conclusion: IBISAgent通过智能多步决策过程有效解决了现有医学图像分割方法的局限性，实现了高质量分割而无需架构修改，为像素级医学图像理解提供了新范式
...
TL;DR: CFSG模型通过将概念和特征空间解耦为共同、特定和混淆三个结构化组件，并引入自适应机制动态调整组件比例，显著提升了细粒度域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 细粒度域泛化比传统域泛化更具挑战性，因为细粒度识别任务中存在微妙的类间差异和明显的类内变化。在域偏移下，模型对细粒度线索过度敏感，导致关键特征被抑制，性能显著下降。认知研究表明人类通过利用共同和特定属性来分类物体，但当前深度学习模型尚未有效融入这一机制。

Method: 提出概念-特征结构化泛化(CFSG)模型，将概念和特征空间明确解耦为三个结构化组件：共同、特定和混淆部分。引入自适应机制动态调整这三个组件的比例，并在最终预测中为每对组件分配显式权重。

Result: 在三个单源基准数据集上的实验表明，CFSG相比基线模型平均性能提升9.87%，优于现有最先进方法平均3.08%。可解释性分析验证了CFSG有效整合了多粒度结构化知识，并确认特征结构化促进了概念结构化的出现。

Conclusion: CFSG通过模拟人类认知机制，将概念和特征空间结构化解耦，并引入自适应调整机制，有效解决了细粒度域泛化中的挑战，显著提升了模型性能。
...
TL;DR: 提出一个多智能体LLM框架，专门用于卡通图像的视觉问答任务，通过视觉、语言和批评三个智能体协作解决卡通图像特有的视觉抽象和叙事上下文挑战。


<details>
  <summary>Details</summary>
Motivation: 标准大型语言模型在自然图像上训练，无法充分处理卡通图像特有的挑战，如夸张的视觉抽象和叙事驱动的上下文，这需要专门的方法来解决卡通视觉问答问题。

Method: 引入一个多智能体LLM框架，包含三个专门化智能体：视觉智能体、语言智能体和批评智能体，它们协作整合视觉线索和叙事上下文，支持结构化推理。

Result: 在Pororo和Simpsons两个卡通VQA数据集上进行系统评估，实验结果详细分析了每个智能体对最终预测的贡献，深入理解了LLM多智能体在卡通VQA和多模态推理中的行为。

Conclusion: 多智能体LLM框架能有效处理卡通图像的视觉问答挑战，通过专门化智能体的协作整合视觉和叙事信息，为卡通VQA任务提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: AI在皮肤病学中的临床采用面临偏见问题，特别是在深色皮肤上诊断模型表现不佳。现有模型存在肤色偏见，阻碍了公平的临床AI应用。

Method: LesionTABE框架结合了对抗性去偏技术和皮肤病专用基础模型嵌入，通过对抗训练减少肤色偏见，同时利用专业医学知识增强诊断能力。

Result: 在涵盖恶性和炎症性皮肤病的多个数据集上评估，相比ResNet-152基线，LesionTABE在公平性指标上提升超过25%，优于现有去偏方法，同时提高了整体诊断准确率。

Conclusion: 基础模型去偏是实现公平临床AI采用的重要一步，LesionTABE展示了结合对抗性去偏和专业医学嵌入在减少皮肤病诊断偏见方面的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型通常只使用视觉编码器的最后一层特征，忽视了视觉编码器丰富的层级视觉线索。这导致模型容易产生视觉未基础化的幻觉，过度依赖语言先验而非图像证据。现有的多层融合方法虽然部分解决了这个问题，但都是静态的，无法根据查询动态调整特征融合。

Method: TGIF将视觉编码器的不同层视为深度方向的"专家"，通过轻量级模块预测基于文本提示的特征融合权重。该方法遵循直接外部融合原则，无需更新视觉编码器，计算开销极小。TGIF被集成到LLaVA-1.5-7B模型中，实现了查询条件化、层级感知的特征融合。

Result: 在LLaVA-1.5-7B中集成TGIF后，在幻觉检测、OCR和VQA基准测试上获得了一致的性能提升。同时，在ScienceQA、GQA和MMBench等任务上保持了原有性能甚至有所改善。这表明查询条件化的层级感知融合能有效增强视觉基础能力。

Conclusion: 文本引导的跨层融合是增强现代多模态大语言模型视觉基础能力和减少幻觉的有效方法。TGIF的轻量级设计和无需视觉编码器更新的特点使其具有很好的实用性和扩展性。
...
TL;DR: Unified Thinker是一个任务无关的推理架构，通过解耦推理模块与生成器，实现可执行推理来提升图像生成逻辑能力


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在逻辑密集型指令跟随方面存在推理-执行差距，闭源系统已展示强大的推理驱动图像生成能力，开源模型存在明显差距，需要可执行推理而非仅改进视觉生成器

Method: 提出Unified Thinker架构：1) 将专用推理器与图像生成器解耦，实现模块化升级；2) 两阶段训练：先构建结构化规划接口，再通过强化学习将策略基于像素级反馈，优化视觉正确性而非文本合理性

Result: 在文本到图像生成和图像编辑任务上的广泛实验表明，Unified Thinker显著提升了图像推理和生成质量

Conclusion: 通过可执行推理架构，将高级意图分解为可验证的规划，直接指导生成过程，有效缩小了推理-执行差距，为通用图像生成提供了任务无关的推理解决方案
...
TL;DR: LSP-DETR是一个用于细胞核实例分割的端到端框架，使用轻量级transformer处理大尺寸病理图像，通过星凸多边形表示和径向距离损失函数实现高效重叠核分割，速度比现有方法快5倍以上。


<details>
  <summary>Details</summary>
Motivation: 全切片病理图像（WSI）的细胞核实例分割面临计算挑战，现有方法依赖基于补丁的处理和昂贵的后处理，牺牲了上下文信息和效率。

Method: 提出LSP-DETR框架：使用线性复杂度的轻量级transformer处理更大图像；将细胞核表示为星凸多边形；引入径向距离损失函数，无需显式重叠标注或手工后处理即可自然分割重叠核。

Result: 在PanNuke和MoNuSeg数据集上评估，显示LSP-DETR在多种组织类型上具有强泛化能力，达到最先进的效率，比次快方法快5倍以上。

Conclusion: LSP-DETR为计算病理学提供了一种精确、可扩展的细胞核实例分割方法，通过端到端框架解决了现有方法的计算效率和上下文限制问题。
...
TL;DR: 提出DiffBench基准和DiffAgent智能体框架，通过LLM驱动的自动化代码生成和评估来加速扩散模型推理


<details>
  <summary>Details</summary>
Motivation: 扩散模型的多步推理过程计算开销大，阻碍实际部署。现有多种加速技术，但如何有效组合这些技术仍具挑战性

Method: 1) DiffBench：包含三阶段自动化评估流程的基准，覆盖多种扩散架构、优化组合和部署场景；2) DiffAgent：基于LLM的智能体，采用规划、代码生成、调试的闭环工作流，结合遗传算法从执行环境提取性能反馈指导代码优化

Result: DiffBench能全面评估生成的加速代码，DiffAgent在生成有效扩散加速策略方面显著优于现有LLM方法

Conclusion: 提出的LLM驱动框架能自动化生成和评估扩散模型加速代码，有效解决多加速技术组合的挑战，为扩散模型实际部署提供实用解决方案
...
TL;DR: AnatomiX是一个专门为胸部X光解剖学基础解释设计的双阶段多任务多模态大语言模型，相比现有方法在解剖学推理和多个任务上实现了超过25%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态医学大语言模型在胸部X光解释中虽然取得了进展，但在空间推理和解剖学理解方面仍面临挑战。现有的基础技术虽然提高了整体性能，但往往无法建立真正的解剖对应关系，导致在医学领域出现错误的解剖学理解。

Method: AnatomiX采用两阶段方法：第一阶段识别解剖结构并提取其特征，第二阶段利用大语言模型执行多种下游任务，包括短语定位、报告生成、视觉问答和图像理解。该方法受到放射学工作流程的启发。

Result: 在多个基准测试中的广泛实验表明，AnatomiX实现了卓越的解剖学推理能力，在解剖学基础、短语定位、基础诊断和基础字幕生成任务上相比现有方法实现了超过25%的性能提升。

Conclusion: AnatomiX通过明确设计用于解剖学基础的胸部X光解释，有效解决了现有多模态医学大语言模型在空间推理和解剖学理解方面的不足，为医学图像分析提供了更准确和可靠的解决方案。
...
TL;DR: UniCorn是一个自改进框架，通过将统一多模态模型划分为三个协作角色来解决模型理解能力强但生成能力弱的问题，无需外部数据或监督，显著提升文本到图像生成质量。
...
Method: 提出UniCorn框架：将单个统一多模态模型划分为三个协作角色（提议者、求解者、评判者），通过自我博弈生成高质量交互，并采用认知模式重构将潜在理解提炼为显式生成信号。

Result: 在六个通用图像生成基准测试中全面大幅超越基础模型，在TIIF(73.8)、DPG(86.8)、CompBench(88.5)和UniCycle上达到SOTA性能，在WISE和OneIG上分别获得+5.0和+6.5的显著提升。

Conclusion: UniCorn显著提升了文本到图像生成能力，同时保持了强大的理解能力，证明了完全自监督精炼方法在统一多模态智能中的可扩展性。
...
TL;DR: LTX-2是一个开源基础模型，能够统一生成高质量、时间同步的视听内容，在计算成本和推理时间上相比专有模型具有显著优势。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频扩散模型虽然能生成引人注目的视频序列，但缺乏音频提供的语义、情感和氛围线索，限制了视听体验的完整性。

Method: 采用非对称双流Transformer架构，包含140亿参数的视频流和50亿参数的音频流，通过双向音视频交叉注意力层连接，配备时间位置嵌入和跨模态AdaLN进行共享时间步调节。使用多语言文本编码器增强提示理解，引入模态感知分类器自由引导机制改善视听对齐和可控性。

Result: 在评估中，该模型在开源系统中实现了最先进的视听质量和提示遵循能力，同时以远低于专有模型的计算成本和推理时间提供可比的结果。不仅能生成语音，还能产生丰富、连贯的音频轨道，包含自然背景音和拟音元素。

Conclusion: LTX-2作为一个开源基础模型，能够统一生成高质量、时间同步的视听内容，在计算效率和性能之间取得了良好平衡，为视听生成领域提供了强大的开源解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度估计方法局限于离散图像网格，限制了输出分辨率的可扩展性并阻碍了几何细节的恢复，需要一种能够支持任意分辨率和精细几何细节的深度表示方法。

Method: 采用神经隐式场表示深度，通过局部隐式解码器在连续2D坐标上查询深度值，实现了任意分辨率和细粒度的深度估计。

Result: 在合成和真实世界基准测试中，InfiniDepth在相对深度和度量深度估计任务上均达到最先进性能，特别是在精细细节区域表现出色，同时在大视角变化下的新视角合成任务中也产生更高质量的结果。

Conclusion: 神经隐式场表示深度的方法突破了传统离散网格的限制，实现了任意分辨率和精细几何细节的深度估计，为深度感知任务提供了更灵活和强大的解决方案。
...
TL;DR: Muses是首个免训练的前馈式3D生物生成方法，通过3D骨架引导实现多样元素的合理组合，解决了传统方法在复杂部件操作和跨域生成方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖部件感知优化、手动组装或2D图像生成，常因复杂的部件级操作和有限的跨域生成能力而产生不真实或不连贯的3D资产。
...
Conclusion: Muses通过将3D骨架作为生物形态的基本表示，实现了免训练的前馈式3D生物生成，为结构感知的3D内容创作提供了新范式。
...
TL;DR: WearVox是首个针对可穿戴设备语音助手的基准测试，包含3842个多通道音频录音，覆盖5种任务，评估显示现有SLLM模型在真实场景中准确率仅为29%-59%，多通道音频能显著提升抗噪能力和设备定向语音识别。


<details>
  <summary>Details</summary>
Motivation: 可穿戴设备（如AI眼镜）将语音助手转变为随时可用、免提的协作工具，但带来了自我中心音频受运动和噪声影响、快速微交互、需要区分设备定向语音与背景对话等挑战。现有基准测试大多忽略了这些复杂性，主要关注干净或通用的对话音频。

Method: 提出了WearVox基准测试，包含3842个多通道、自我中心音频录音，通过AI眼镜在5种不同任务中收集：搜索基础QA、闭卷QA、旁白拒绝、工具调用和语音翻译，覆盖广泛的室内外环境和声学条件。每个录音都配有丰富的元数据，用于在真实世界约束下进行细致的模型性能分析。

Result: 评估领先的专有和开源语音大语言模型（SLLM），发现大多数实时SLLM在WearVox上的准确率在29%到59%之间，在嘈杂的室外音频上性能显著下降。案例研究表明，使用多通道音频输入的SLLM能显著增强模型对环境噪声的鲁棒性，并改善设备定向语音与背景语音的区分能力。

Conclusion: 空间音频线索对于上下文感知语音助手至关重要，WearVox为推进可穿戴语音AI研究建立了一个全面的测试平台，突显了多通道音频在提升真实场景性能方面的重要性。
...
TL;DR: PCEval是首个用于评估大语言模型在物理计算领域能力的基准测试，能够自动评估LLM在逻辑和物理两方面的项目能力，无需人工评估。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在软件开发等领域表现出色，但在考虑硬件约束的物理计算环境中，其有效性尚未得到充分探索。物理计算需要软件与物理硬件交互和控制，现有研究存在空白。

Method: 开发了PCEval基准测试框架，通过模拟环境评估LLM生成电路和兼容代码的能力，涵盖不同项目复杂度。该框架支持完全自动化的评估，无需人工干预。

Result: 对13个领先模型进行全面测试发现：LLM在代码生成和逻辑电路设计方面表现良好，但在物理面包板布局创建方面存在显著困难，特别是在管理正确的引脚连接和避免电路错误方面。
...
TL;DR: 本文提出了一种新的拓扑损失函数，将食材列表表示为嵌入空间中的点云，用于改进食谱生成模型，相比标准训练方法在食材和动作指标上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 烹饪食谱需要流畅准确的文本描述，同时还需要精确的时间、温度、程序连贯性和正确的食材组合。标准的基于交叉熵的训练方法主要关注流畅性，无法充分处理食谱生成中的结构化要求。

Method: 基于RECIPE-NLG框架，研究了几种复合目标函数，提出了一种新的拓扑损失函数，将食材列表表示为嵌入空间中的点云，最小化预测食材与真实食材之间的差异。

Result: 新损失函数在食材和动作层面的指标上显著改善；Dice损失在时间/温度精度上表现优异；混合损失在数量和时间的协同增益方面取得竞争性平衡。人类偏好分析显示，该模型在62%的情况下被优先选择。

Conclusion: 通过引入拓扑损失等复合目标函数，能够显著提升食谱生成模型在结构化要求方面的性能，为食谱生成任务提供了更全面的评估和优化方法。
...
TL;DR: ModeX是一个无需评估器的Best-of-N选择框架，通过构建相似度图并应用谱聚类来识别生成文本中的主导语义共识，从而在开放任务中选择高质量输出。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在开放任务中选择高质量输出的方法通常依赖外部评估器、奖励模型或精确字符串匹配投票，这限制了其适用性和效率。需要一种无需外部评估的通用选择框架。

Method: 提出ModeX框架：1) 构建候选生成文本的相似度图；2) 递归应用谱聚类选择代表性中心点；3) 无需额外推理或辅助模型。还提出ModeX-Lite版本，通过早期剪枝提高效率。

Result: 在文本摘要、代码生成和数学推理等开放任务中，ModeX方法持续优于标准的单路径和多路径基线，提供了计算高效的鲁棒开放文本生成解决方案。

Conclusion: ModeX为LLM在开放任务中选择高质量输出提供了一个无需评估器的高效框架，通过识别语义共识来改进生成质量，具有广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: LLM在需要事实准确性的应用中常产生幻觉响应，现有事实核查方法通常不加区分地检索外部证据，忽略了模型的内部知识并可能引入无关噪声，且缺乏针对性机制来解决模型推理中的具体不确定性。

Method: 引入概率确定性和一致性（PCC）框架，通过联合建模LLM的概率确定性和推理一致性来估计事实置信度，基于置信度信号实现自适应验证策略：模型在自信时直接回答，不确定或不一致时触发针对性检索，歧义高时升级到深度搜索。

Result: 在三个具有挑战性的基准测试中，PCC在不确定性量化方面优于口头置信度表达，并持续超越基于LLM的事实核查基线方法。PCC在不同LLM之间具有良好的泛化能力。

Conclusion: PCC框架通过自适应地决定何时依赖内部知识或启动检索，提高了事实核查的效率和可靠性，为LLM的事实准确性提供了有效的置信度引导路由机制。
...
TL;DR: 使用大语言模型模拟学生回答来估计试题参数，避免传统方法需要大量实地测试的缺点


<details>
  <summary>Details</summary>
Motivation: 传统方法通过实地测试收集学生表现数据来校准项目反应理论参数，成本高昂且耗时。本研究旨在开发一种更高效的方法来估计试题的难度和区分度等心理测量特性。

Method: 使用Qwen-3密集模型系列和低秩适应技术，通过微调大语言模型来模拟不同潜在能力水平学生的多项选择题回答。通过条件化能力描述符，重构正确回答概率作为学生能力的函数，生成合成项目特征曲线来估计IRT参数。

Result: 在六年级英语语言艺术试题数据集和BEA 2024共享任务数据集上的评估表明，该方法与基线方法相当或优于基线方法，特别在建模试题区分度方面表现有效。

Conclusion: 基于大语言模型模拟学生回答的方法能够有效估计试题的心理测量参数，为试题参数估计提供了一种成本效益更高的替代方案，特别在建模试题区分度方面显示出优势。
...
Method: 提出FlowPlan-G2P框架，将任务重新表述为三个阶段：1) 概念图归纳：通过专家式推理提取技术实体和关系构建有向图；2) 段落和章节规划：将图重组为与标准专利章节对齐的连贯簇；3) 图条件生成：使用章节特定子图和定制提示生成法律合规的段落。
...
TL;DR: 该研究开发了一个基于OpenIE的肺癌知识库构建流程，用于增强大型语言模型在生物医学领域的性能，通过监督语义微调显著提升了T5模型的表现。


<details>
  <summary>Details</summary>
Motivation: 将大型语言模型整合到生物医学研究中为领域特定推理和知识表示提供了新机会，但在肿瘤学等需要精确性和可解释性的领域，训练数据的语义质量至关重要。需要可扩展的方法来构建结构化知识库以进行有效的微调。

Method: 研究提出了一个四步流程：1) 使用MeSH词表识别医学概念；2) 筛选具有宽松许可(CC0)的开放获取PubMed文献；3) 使用OpenIE方法提取(主体,关系,客体)三元组；4) 通过命名实体识别(NER)丰富三元组集以确保生物医学相关性。

Result: 构建了一个领域特定、大规模且具有噪声感知的资源用于微调LLMs。通过监督语义微调评估T5模型，使用ROUGE和BERTScore进行对比评估，结果显示性能显著提升，语义连贯性明显改善。

Conclusion: OpenIE衍生的资源作为可扩展、低成本的解决方案，在增强生物医学自然语言处理方面具有巨大潜力，能够显著提升大型语言模型在生物医学领域的性能和语义连贯性。
...
TL;DR: 该论文提出了一种基于大语言模型的文档不一致性检测方法，通过引入新的证据提取指标和"编辑重试"框架，显著改进了直接提示方法的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多领域展现出强大能力，但在文档不一致性检测方面的研究相对有限。文档不一致性检测包含两个关键方面：分类是否存在不一致性，以及提供不一致句子的证据。本研究专注于后者。

Method: 提出了新的综合证据提取指标，并引入了带有约束过滤的"编辑重试"框架。该框架通过编辑和重新尝试的方法，改进了基于大语言模型的文档不一致性检测。

Result: 实验结果显示，该方法相比直接提示方法在文档不一致性检测方面取得了显著改进，表现出有前景的性能。

Conclusion: 提出的证据提取指标和"编辑重试"框架有效提升了基于大语言模型的文档不一致性检测能力，为这一领域提供了新的技术方案。
...
TL;DR: 研究比较了多种编码器语言模型在长文本自动评分中的表现，发现集成多个预训练模型嵌入并使用梯度提升分类器的集成模型显著优于单个模型。


<details>
  <summary>Details</summary>
Motivation: 长文本处理对编码器语言模型构成挑战，特别是在长论文自动评分任务中，需要研究如何有效处理超出标准token限制的长文本。

Method: 训练了BERT、RoBERTa、DistilBERT、DeBERTa等编码器模型，构建了集成多个模型嵌入的集成模型，以及基于特征的监督机器学习集成模型（GBDT、XGBoost、LightGBM）。在17,307篇论文数据集上按80%/10%/10%划分训练、验证和测试集。

Result: 集成多个预训练语言模型表示并使用梯度提升分类器的嵌入集成模型在长论文评分任务中显著优于单个语言模型。

Conclusion: 对于长文本自动评分任务，集成多个编码器模型嵌入的方法比单一模型更有效，为解决长文本处理挑战提供了有效方案。
...
TL;DR: FactArena：一个全自动的竞技场式评估框架，用于全面评估LLMs在完整事实核查流程中的表现，超越了传统仅关注声明验证的局限。
...
Method: FactArena包含三个核心组件：(1) LLM驱动的事实核查流程，标准化声明分解、工具增强的证据检索和基于理由的裁决预测；(2) 竞技场式判断机制，基于统一参考指南进行无偏见的成对比较；(3) 竞技场驱动的声明演化模块，自适应生成更具挑战性的声明来测试LLMs的事实鲁棒性。

Result: 在涵盖7个模型家族的16个最先进LLMs上，FactArena产生了稳定且可解释的排名。分析显示静态声明验证准确性与端到端事实核查能力之间存在显著差异，强调了整体评估的必要性。

Conclusion: FactArena提供了一个可扩展且可信赖的范式，用于诊断LLMs的事实推理能力，指导未来模型开发，并推动LLMs在安全关键的事实核查应用中的可靠部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有越狱方法需要攻击者LLM生成对抗性查询且查询预算高，导致成本昂贵，且生成的查询往往包含不可解释的随机前缀
...
Conclusion: 对话结构是一个强大且保护不足的攻击面，LATS在攻击成功率易达的时代展示了卓越的查询效率
...
TL;DR: 研究显示，即使有安全措施，商业大语言模型仍能提取受版权保护的训练数据，不同模型提取难度和效果差异显著。


<details>
  <summary>Details</summary>
Motivation: 探讨商业大语言模型是否能够提取受版权保护的训练数据，解决关于LLM记忆化与版权问题的未解疑问。

Method: 采用两阶段方法：1) 初始探测（有时使用Best-of-N越狱技术）测试提取可行性；2) 迭代续写提示尝试提取整本书。评估了Claude 3.7 Sonnet、GPT-4.1、Gemini 2.5 Pro和Grok 3四个商业模型，使用基于块的最长公共子串近似（nv-recall）衡量提取成功率。

Result: 不同模型提取效果差异显著：Gemini 2.5 Pro和Grok 3无需越狱即可提取大量文本（如《哈利·波特与魔法石》nv-recall分别为76.8%和70.3%）；Claude 3.7 Sonnet需要越狱但能输出接近原文的整本书（nv-recall=95.8%）；GPT-4.1需要更多越狱尝试且最终拒绝继续（nv-recall=4.0%）。

Conclusion: 即使有模型和系统级安全防护，商业大语言模型提取受版权训练数据的风险仍然存在，不同模型的安全防护效果存在显著差异。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型规模不断增长带来了计算开销、内存占用和推理延迟等实际部署障碍。现有非结构化剪枝技术产生的稀疏模式不规则，需要专用硬件或软件支持，而结构化剪枝可以消除整个架构组件并保持与标准硬件加速器的兼容性。

Method: 提出结构化剪枝框架，采用混合多领域校准集和迭代校准策略来有效识别和移除冗余通道。该方法通过多领域数据校准确保剪枝后的模型在不同任务上保持良好性能。

Result: 在各种模型和多样化下游任务上的广泛实验表明，该方法在最小性能损失的情况下实现了显著的模型压缩效果。

Conclusion: 提出的结构化剪枝框架通过混合多领域校准和迭代策略，有效解决了LLM部署中的计算和内存问题，同时保持硬件兼容性，为实际应用提供了可行的模型压缩方案。
...
TL;DR: 该研究通过系统实验探索了Transformer模型在AddSent对抗数据集上的鲁棒性，发现模型规模扩大可消除鲁棒性-准确率权衡，并提出基于实体感知对比学习的目标缓解策略，将对抗性能差距缩小94.9%


<details>
  <summary>Details</summary>
Motivation: 尽管问答系统在标准基准测试中表现优异，但在对抗样本面前仍然脆弱。本研究旨在系统评估Transformer模型在AddSent对抗数据集上的鲁棒性，并探索有效的缓解策略。

Method: 1) 在模型规模（ELECTRA-small到ELECTRA-base）和对抗微调比例上进行系统实验；2) 使用五种互补分类方案进行多级错误分析；3) 实施三种目标缓解策略，包括实体感知对比学习；4) 结合命名实体识别（NER）指导的对比学习进行针对性改进。

Result: 1) 发现80%干净数据+20%对抗数据为最优微调比例；2) 小模型存在容量瓶颈，扩大模型规模可同时提升干净和对抗数据性能；3) 实体感知对比学习效果最佳：AddSent EM达到89.89%，SQuAD EM达到90.73%，将对抗差距缩小94.9%；4) 否定混淆和实体替换是主要失败模式。

Conclusion: 通过综合语言错误分析和NER引导的对比学习，目标缓解策略能够使对抗性能接近干净数据性能。这是首个将全面语言错误分析与NER指导的对比学习相结合用于对抗问答的研究，证明了针对性缓解可以实现干净和对抗性能的近乎一致。
...
<details>
  <summary>Details</summary>
Motivation: 针对大语言模型中存在的幻觉问题，特别是由提示诱导产生的幻觉，需要开发更有效的方法来约束模型的推理过程，提高输出的准确性和可验证性。

Method: 在知识蒸馏链式模型基础上，引入代码模块指导知识图谱探索，并将代码作为思维链提示的一部分，形成外部知识输入，为模型提供更准确、结构化的信息，从而约束LLM的推理过程。

Result: 在GPT-4和LLaMA-3.3上的实验表明，代码模块显著增强了模型捕捉上下文信息的能力，有效缓解了提示诱导幻觉。HIT@1、HIT@3、HIT@5分别提升15.64%、13.38%、13.28%，多个评估设置下HIT@1、HIT@3、HIT@5得分超过95%。
...
TL;DR: 该研究通过计算模拟和自然语言验证分析，发现层次化处理能最优地解决人类工作记忆容量有限的问题，解释了人类语言普遍具有层次结构的原因。


<details>
  <summary>Details</summary>
Motivation: 探索人类语言为何普遍具有层次结构这一核心问题，研究假设层次化处理是为了优化解决人类有限工作记忆容量的挑战。

Method: 建立量化语言处理机制与人类工作记忆容量匹配程度的似然函数，通过计算模拟符号序列和验证分析自然语言句子，比较线性处理和层次处理在约束平均单元数方面的表现。

Result: 层次处理在约束平均单元数方面远超线性处理，能更好地适应人类工作记忆容量限制，且随着序列/句子长度增加表现更优，模式与儿童工作记忆发展相关。
...
TL;DR: WBC攻击方法通过滑动窗口比较损失信号，显著提升了对LLMs训练数据的成员推断攻击效果


<details>
  <summary>Details</summary>
Motivation: 现有的大多数成员推断攻击依赖全局平均损失信号，这稀释了记忆化的局部细微信号，降低了攻击效果。作者挑战这种全局平均范式，认为成员信号在局部上下文中更为明显。

Method: 提出WBC（基于窗口的比较）方法，采用滑动窗口和基于符号的聚合。通过不同大小的滑动窗口在文本序列上移动，每个窗口基于目标模型和参考模型的损失比较对成员身份进行二元投票。通过几何间隔的窗口大小集成投票，捕捉从token级到短语级的记忆化模式。

Result: 在11个数据集上的广泛实验表明，WBC显著优于现有基线方法，获得更高的AUC分数，在低误报率阈值下检测率提高2-3倍。

Conclusion: 聚合局部证据比全局平均更有效，暴露了微调LLMs中的关键隐私漏洞。
...
TL;DR: EComStage是一个用于评估LLM智能体在电商场景中分阶段推理能力的统一基准，涵盖感知、规划和行动三个阶段，包含7个代表性任务，评估了30多个不同规模的LLM模型。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估LLM智能体是否成功完成最终任务，忽略了中间推理阶段对有效决策的重要性。需要一个新的基准来全面评估LLM在电商场景中的分阶段推理能力。

Method: 提出EComStage基准，涵盖感知（理解用户意图）、规划（制定行动计划）和行动（执行决策）三个阶段。包含7个代表性任务，覆盖多样电商场景，所有样本都经过人工标注和质量检查。不仅评估面向客户的交互，还包括面向商家的场景。

Result: 评估了超过30个LLM模型，参数规模从1B到200B以上，包括开源模型和闭源API。揭示了不同模型在特定阶段和场景下的优势和弱点，为实际应用提供了细粒度的见解。

Conclusion: EComStage为电商场景中LLM智能体的设计和优化提供了细粒度、可操作的见解，填补了现有基准在评估中间推理过程方面的空白。
...
TL;DR: PHSA是一种基于标点符号的混合稀疏注意力机制，通过标点作为语义边界锚点，在保持计算效率的同时减少信息丢失。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏注意力方法在块选择时依赖粗粒度语义表示，模糊了块内语义边界，导致关键信息丢失。需要一种能更好保留语义结构的稀疏注意力机制。

Method: 提出PHSA框架：1）设计双分支聚合机制，融合全局语义表示和标点增强的边界特征；2）引入极端稀疏自适应训练和推理策略，稳定模型在极低token激活率下的行为。

Result: 在通用基准和长上下文评估中，PHSA持续优于密集注意力和最先进的稀疏注意力基线（包括InfLLM v2）。对于32k-token输入序列的0.6B参数模型，在97.3%稀疏比下能将信息损失减少10.8%。
...
TL;DR: 该研究比较了中美开发的大语言模型在中文文化问题上的表现，发现中国模型普遍优于美国模型，性能差异可能源于训练数据分布、本地化策略和文化内容重视程度的不同。


<details>
  <summary>Details</summary>
Motivation: 文化背景影响问题解决方式，而中美作为主要LLM开发国，其模型在中文文化理解上是否存在差异值得探究。研究旨在评估中美开发的大语言模型在中文文化问题上的表现差异。
...
Conclusion: 观察到的性能差异可能源于训练数据分布、本地化策略以及模型开发过程中对中文文化内容的重视程度不同。这表明文化背景确实会影响LLM在特定文化领域任务上的表现。
...
TL;DR: TiMem是一个用于长对话智能体的时间-层次记忆框架，通过时间记忆树组织对话，实现从原始观察到抽象人物表征的系统记忆整合，在保持高准确率的同时显著减少记忆长度。


<details>
  <summary>Details</summary>
Motivation: 长对话智能体面临不断增长的交互历史，容易超出大语言模型的有限上下文窗口。现有记忆框架对跨层次的时间结构化信息支持有限，导致记忆碎片化和长时程个性化不稳定。

Method: 提出TiMem框架，包含三个核心特性：1) 通过时间记忆树实现时间-层次组织；2) 语义引导的整合，无需微调即可跨层次整合记忆；3) 复杂度感知的记忆召回，平衡不同复杂度查询的精度和效率。

Result: 在一致评估设置下，TiMem在两个基准测试中达到最先进准确率：LoCoMo上75.30%，LongMemEval-S上76.88%。在LoCoMo上减少52.20%的召回记忆长度，同时优于所有基线模型。流形分析显示在LoCoMo上人物分离清晰，在LongMemEval-S上分散度降低。

Conclusion: TiMem将时间连续性作为长对话智能体记忆的首要组织原则，通过时间-层次框架有效管理长时程对话记忆，实现更好的记忆整合和个性化表现。
...
TL;DR: 研究提出逆社会人口统计提示（ISDP）方法，通过让LLMs从真实和模拟用户行为中预测人口统计特征，解决了传统社会人口统计提示（SDP）在评估文化能力时存在的混淆因素问题。


<details>
  <summary>Details</summary>
Motivation: 传统的社会人口统计提示（SDP）方法在评估LLMs的文化能力时存在多个混淆因素：提示敏感性、解码参数、以及生成任务相对于判别任务的固有难度。这些因素使得难以确定性能不佳是由于偏见还是任务设计问题。

Method: 提出逆社会人口统计提示（ISDP）方法，让LLMs从真实和模拟的用户行为中判别和预测人口统计特征。使用Goodreads-CSI数据集（包含印度、墨西哥和美国用户对英文书评的理解难度数据），测试了Aya-23、Gemma-2、GPT-4o和LLaMA-3.1四个模型。

Result: 模型在真实行为上的表现优于模拟行为，这与SDP的预期相反。然而，在个体层面上，两种行为类型的性能都下降且几乎相等，表明个性化存在局限性。

Conclusion: ISDP方法能够更准确地评估LLMs的文化能力，揭示了传统SDP方法的局限性，并表明LLMs在个体层面的个性化能力有限。
...
TL;DR: 该论文提出了一种称为"同形令牌"的数据增强方法，通过利用同一词汇的不同有效子词分割来训练语言模型，提高其对分词变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 子词分词在语言模型中引入了一个计算层，其中许多不同的令牌序列可以解码为相同的表面形式并保留含义，但会引发不同的内部计算。尽管存在这种非唯一性，语言模型通常只使用单一的标准最长前缀分词进行训练，这可能导致模型对分词变化不够鲁棒。

Method: 作者提出"同形令牌"概念，即同一词汇项的不同有效子词分割。引入轻量级训练架构，通过辅助因果编码器和块因果交叉注意力，在标准下一个令牌预测的基础上，对采样的同形令牌变体进行条件化，而不修改训练目标或令牌接口。

Result: 在数据受限的预训练中，同形令牌增强持续延迟了重复数据暴露下的过拟合，并提高了在多样化评估数据集上的泛化能力。在多语言微调中，同形令牌的有效性取决于分词器质量：当标准令牌高度压缩时增益最强，而当分词器已经过度分割输入时增益减弱。

Conclusion: 同形令牌为在语言模型中引入分词不变性提供了一种简单且模块化的机制，能够提高模型对分词变化的鲁棒性和泛化能力。
...
TL;DR: LongBench Pro是一个双语长上下文基准测试，包含1500个自然长文本样本，支持8k-256k令牌长度，通过人机协作构建，评估了46个LLM并发现三个关键发现。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文基准测试在可扩展性和真实性之间存在权衡：合成任务无法代表真实世界复杂性，而完全人工标注成本过高，难以扩展到极端长度和多样化场景。

Method: 提出LongBench Pro基准测试，包含1500个自然长文本样本，支持8k-256k令牌长度，涵盖11个主要任务和25个次要任务。采用人机协作构建流程：前沿LLM生成问题和参考答案，专家验证和修正。支持任务特定指标和多维度分类（上下文依赖、长度、难度）。

Result: 评估46个广泛使用的长上下文LLM，发现三个关键结果：(1) 长上下文优化比参数缩放对长上下文理解贡献更大；(2) 有效上下文长度通常短于声称长度，存在显著的跨语言不对齐；(3) "思考"范式主要帮助原生推理训练的模型，混合思考设计提供了有前景的帕累托权衡。

Conclusion: LongBench Pro为推进长上下文理解提供了稳健的测试平台，通过平衡质量和可扩展性的人机协作方法，解决了现有基准测试的局限性。
...
TL;DR: 该研究探索使用大语言模型进行数据压缩，在enwik9数据集上实现了约18%的调整压缩率，达到了新的SOTA，并验证了LLM在非英语文本、代码和字节流序列上的压缩能力。


<details>
  <summary>Details</summary>
Motivation: 尽管先前研究已证明LLM在文本和多模态数据压缩方面有良好表现，但实际应用中仍存在挑战，需要探索如何用LLM替代现有数据压缩算法，特别是在降低调整压缩率方面。

Method: 探索了多种使用LLM作为数据压缩器的方法，通过不同配置优化压缩性能，特别关注非额外模型训练条件下的压缩效果。

Result: 在enwik9数据集上实现了约18%的调整压缩率，达到新的SOTA；在非英语数据、代码数据和字节流序列压缩方面，LLM在文本主导领域表现优异，在非自然文本序列中通过适当配置也能保持竞争力。

Conclusion: LLM在数据压缩任务中具有显著潜力，特别是在文本主导领域表现突出，通过适当配置也能有效压缩非自然文本序列，为替代传统压缩算法提供了新的可能性。
...
TL;DR: 研究发现多语言语音模型（如Whisper）的激活空间中线性编码了文字脚本信息，通过修改推理时的激活向量可以直接控制语音识别输出的文字脚本。


<details>
  <summary>Details</summary>
Motivation: 多语言语音基础模型在不同地区变体中使用的文字脚本存在非确定性，同一语言可能使用不同文字系统，导致语音识别输出脚本不一致。

Method: 发现文字脚本在模型激活空间中线性编码，通过在推理时向激活添加特定的"脚本向量"来直接控制输出脚本，甚至可以处理非常规的语言-脚本配对。

Result: 该方法在所有规模的Whisper模型上都表现出色，能够成功诱导语音识别输出的脚本变化，包括非常规的语言-脚本组合（如意大利语用西里尔字母、日语用拉丁字母）。

Conclusion: 通过激活空间中的线性脚本编码，可以实现对多语言语音识别输出脚本的后期控制，为解决不同地区变体脚本非确定性问题提供了有效方案。
...
TL;DR: 本文提出基于生命周期的统一分类法，将LLM研究分为六个阶段，系统回顾了驱动LLM性能的基础理论和内部机制，旨在将LLM开发从工程启发式转向原则性科学学科。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM取得了巨大的工程成功，但理论理解仍然相对滞后，导致这些系统很大程度上被视为"黑箱"。为了解决这种理论碎片化，需要建立统一的理论框架来理解LLM的内部机制。

Method: 提出基于生命周期的统一分类法，将LLM研究组织为六个阶段：数据准备、模型准备、训练、对齐、推理和评估。在此框架下，系统回顾了驱动LLM性能的基础理论和内部机制，分析了数据混合的数学原理、架构的表征限制、对齐算法的优化动态等核心理论问题。

Result: 建立了结构化的LLM研究分类框架，识别了关键前沿挑战，包括合成数据自我改进的理论限制、安全保证的数学界限、涌现智能的机制起源等。通过连接经验观察与严谨科学探究，为LLM开发提供了系统化的理论指导。

Conclusion: 该工作为将LLM开发从工程启发式转向原则性科学学科提供了结构化路线图，通过统一的生命周期分类法和系统理论分析，有助于弥合LLM经验成功与理论理解之间的差距。
...
TL;DR: 该研究将迭代学习模型应用于七段数码管图像的语言学习任务，首次成功处理复杂含义的通信，实现了表达性、组合性和稳定性的语言涌现。
...
Method: 采用结合监督和无监督学习的半监督迭代学习模型，使用自动编码器架构，应用于七段数码管图像的语言学习任务，模拟代际语言传输。

Result: 智能体能够学习并传输具有表达性（128个字形均有不同编码）、组合性（信号组件一致映射到含义组件）和稳定性（代际间语言不变）的语言。

Conclusion: 迭代学习模型能够成功处理复杂含义的通信任务，验证了语言传输瓶颈在促进表达性、组合性和稳定性语言结构涌现中的作用。
...
TL;DR: RAL2M框架通过将LLM重新定位为检索系统中的查询-响应匹配判断器，消除生成幻觉，并采用查询自适应潜在集成策略进一步减少判断幻觉。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的服务系统中幻觉是一个主要问题，需要显式的知识基础来保证合规性响应。当前纯生成方法存在幻觉风险，需要更稳健的替代方案。

Method: 提出检索增强学习匹配(RAL2M)框架：1) 将LLM重新定位为检索系统中的查询-响应匹配判断器而非生成器；2) 采用查询自适应潜在集成策略，显式建模异构模型能力和LLM间的相互依赖关系，得出校准的共识决策。
...
TL;DR: 研究发现自回归LLMs在关系任务中表现良好，但逻辑语义学习存在相变现象，反转失败主要源于自回归顺序偏差而非语义缺陷
...
Result: 发现关系语义在足够逻辑监督下出现相变现象，即使浅层模型（2-3层）也能学习；成功泛化与稳定的中间层信号相关；反转失败主要源于自回归顺序偏差

Conclusion: 自回归LLMs能够学习关系逻辑语义，但反转失败主要是顺序偏差而非语义缺陷，这对理解模型推理机制有重要意义
...
<details>
  <summary>Details</summary>
Motivation: 人类评估是多语言NLP的黄金标准，但由于现有工具设置复杂、耗时且需要大量工程和操作开销，实践中常被自动指标替代。需要一种简化人类评估流程的解决方案。

Method: 开发了Pearmut平台，支持标准评估协议（如DA、ESA、MQM），具有文档级上下文、绝对和对比评估、注意力检查、ESAAI预标注以及静态和主动学习分配策略等功能。

Result: Pearmut平台移除了常见入门障碍，支持多语言任务评估，特别是机器翻译，使可靠的人类评估成为模型开发和诊断的实际常规组成部分。

Conclusion: Pearmut使端到端的人类评估变得像自动评估一样容易运行，有望将可靠的人类评估从偶尔的工作转变为模型开发和诊断的实用常规组件。
...
TL;DR: 该论文提出了一种去偏语言偏好度量方法DeLP，发现mRAG系统中观察到的英语偏好主要是评估基准的结构性偏差所致，而非模型固有偏见，并基于此开发了DELTA框架来优化跨语言检索和生成。


<details>
  <summary>Details</summary>
Motivation: 多语言检索增强生成系统通常表现出对英语的偏好，但作者认为这种测量受到评估基准结构性偏差的扭曲，需要开发去偏方法来准确评估真实的语言偏好。

Method: 提出DeLP度量方法，显式地排除曝光偏差、黄金可用性先验和文化先验等结构性混杂因素；基于发现构建DELTA框架，利用查询和文档语言之间的单语对齐来优化跨语言检索和生成。

Result: 使用DeLP分析发现，先前报告的英语偏好主要是证据分布的结果而非模型固有偏见；检索器本质上偏好查询和文档语言之间的单语对齐；DELTA框架在多种语言上持续优于英语枢纽方法和mRAG基线。

Conclusion: mRAG系统中观察到的英语偏好主要是评估基准的结构性偏差所致，而非模型固有偏见；利用单语对齐的DELTA框架能够有效优化跨语言检索和生成性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决变点检测领域的两个关键限制：1) 单个检测方法在不同数据特性下表现互补，方法选择困难且易产生次优结果；2) 缺乏对检测到的变化的自动化、上下文解释

Method: 集成十种不同的变点检测算法，结合LLM驱动的解释管道自动生成上下文叙述，将检测到的变点与现实世界历史事件关联；针对私有或特定领域数据，采用检索增强生成(RAG)解决方案

Result: 集成方法相比单个方法实现了更优的性能和鲁棒性；LLM解释管道能自动生成有意义的上下文解释；开源Python框架在金融、政治科学、环境科学等多个领域展示了实用价值

Conclusion: 提出的框架将原始统计输出转化为分析师和决策者可操作的洞察，解决了变点检测中的准确性和可解释性问题，为多领域应用提供了实用工具
...
<details>
  <summary>Details</summary>
Motivation: 巴拿语作为越南、柬埔寨和老挝的少数民族语言，面临保存挑战，现有研究数据有限。扫描文档图像质量退化导致OCR识别错误，影响信息检索系统准确性，亟需改进数字化技术。

Method: 提出综合方法：首先应用先进的表格和非表格检测技术改善输入数据质量，然后对OCR输出采用基于概率的后处理启发式算法进行错误校正。

Result: 实验结果显示识别准确率显著提升，从72.86%提高到79.26%，为巴拿语文档数字化提供了有效的技术解决方案。

Conclusion: 本研究为巴拿语保护提供了有价值的资源，提出的框架可应用于其他少数民族语言的数字化工作，有助于语言文化遗产的保存和利用。
...
TL;DR: 提出一种多阶段高效推理方法，通过监督微调和强化学习结合自适应长度惩罚，减少大语言模型推理过程中的过度思考现象，在保持准确性的同时显著缩短响应长度。
...
Result: 以五大人格特质为案例研究，证明该方法能够实现模型行为的精确双向调控，相比现有激活调控方法（如对比激活加法）具有更好的稳定性和性能

Conclusion: 发现"功能忠实性"经验效应，表明干预特定内部特征会在多个语言维度上引发与目标语义属性一致的可预测变化，证明大语言模型内化了高阶概念的深度集成表示，为复杂AI行为调控提供了新颖稳健的机制路径
...
TL;DR: P-Check是一个个性化奖励建模框架，通过训练即插即用的检查清单生成器来合成动态评估标准，以指导奖励预测，提高个性化生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有个性化奖励建模方法主要将用户上下文视为静态或隐式条件信号，未能捕捉人类判断的动态性和多面性，需要更灵活的方法来适应个性化偏好。

Method: 提出P-Check框架，训练即插即用的检查清单生成器来合成动态评估标准；引入偏好对比标准加权训练策略，根据标准在个性化判断中的区分能力分配显著性分数。

Result: 实验表明P-Check不仅提高了奖励准确性，还增强了下游个性化生成质量，并在分布外场景中保持鲁棒性。

Conclusion: P-Check通过动态检查清单生成和偏好对比加权，有效解决了现有个性化奖励建模方法的局限性，实现了更好的个性化对齐和生成性能。
...
TL;DR: 本文提出一种受System-2认知过程启发的测试时策略，通过将大型计数任务分解为小型子问题来解决LLMs在计数任务中的系统限制，并通过机制分析揭示了计数在模型中的实现方式。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然在复杂数学问题上表现良好，但在计数任务中存在系统限制。这种限制源于Transformer架构的约束，其中计数操作需要跨层执行，导致在大型计数问题上精度下降。

Method: 提出一种简单的测试时策略，受System-2认知过程启发，将大型计数任务分解为模型能够可靠解决的小型独立子问题。使用观察性和因果中介分析来理解这种类似System-2策略的底层机制。

Result: 机制分析识别出关键组件：潜在计数在每个部分的最终项目表示中计算和存储，通过专用注意力头传递到中间步骤，并在最后阶段聚合以产生总计数。实验结果表明，该策略使LLMs能够超越架构限制，在大规模计数任务上实现高精度。

Conclusion: 这项工作为LLMs中的System-2计数提供了机制性见解，并提出了一种可推广的方法来改进和理解它们的推理行为，为解决LLMs在计数任务中的架构限制提供了有效途径。
...
TL;DR: Stable-RAG：一种通过估计检索文档排列敏感性来减少幻觉的新方法，通过多排列运行生成器、聚类隐藏状态，从聚类中心解码来获得稳定推理模式


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成(RAG)方法主要关注低质量检索的鲁棒性和位置偏置，但忽视了检索文档排列顺序对模型行为的影响。研究发现即使黄金文档固定在第一位置，不同排列顺序仍会导致LLM答案显著变化，这揭示了未被充分探索的排列敏感性

Method: 提出Stable-RAG方法：1）在多个检索排列顺序下运行生成器；2）对生成的隐藏状态进行聚类；3）从聚类中心表示解码，捕获主导推理模式；4）利用这些推理结果将幻觉输出对齐到正确答案，鼓励模型在不同文档排列下产生一致且准确的预测

Result: 在三个QA数据集上的实验表明，与基线方法相比，Stable-RAG显著提高了答案准确性、推理一致性，并在不同数据集、检索器和输入长度上展现出更好的鲁棒泛化能力

Conclusion: Stable-RAG通过估计和缓解检索排列敏感性，有效减少了RAG系统中的幻觉问题，提高了模型的稳定性和准确性，为处理检索文档顺序敏感性提供了新思路
...
TL;DR: 研究发现大型推理模型在多语言环境中存在潜在推理能力，但存在不平衡性：高资源语言表现强，低资源语言表现弱，且英语中心化特征明显。


<details>
  <summary>Details</summary>
Motivation: 虽然大型推理模型在数学推理任务中表现出色，但研究发现它们经常在完成文本推理步骤前就得出正确答案，这表明存在潜在推理（内部非语言计算）。然而，这种现象在英语中已有研究，但在多语言环境中的行为仍未知。

Method: 采用基于截断的策略，在11种语言中研究模型仅获得部分推理痕迹时正确答案如何出现，测量逐步潜在预测形成。进一步进行表征分析以了解不同语言是否反映不同的内部机制。

Result: 发现了明显的多语言潜在推理证据，但存在不平衡：高资源语言表现强，低资源语言表现弱，在更难基准测试中普遍较少观察到。尽管表面存在差异，但预测的内部演化在语言间高度一致，且与英语基本对齐，表明存在英语中心的潜在推理路径。

Conclusion: 大型推理模型在多语言环境中确实存在潜在推理能力，但这种能力分布不均，且表现出英语中心化的特征，这对理解多语言AI系统的内部工作机制具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 传统RAG在单跳问答中有效，但在多跳问答中存在局限性。现有的分块检索方法经常提供不相关和逻辑不连贯的上下文，导致证据链不完整和推理错误。

Method: 提出SentGraph框架：1）离线构建分层句子图，采用修辞结构理论区分核心句和卫星句，组织成主题级子图并建立跨文档实体桥梁；2）在线检索时进行图引导的证据选择和路径扩展，检索细粒度句子级证据。

Result: 在四个多跳问答基准测试上进行广泛实验，证明了SentGraph的有效性，验证了显式建模句子级逻辑依赖对多跳推理的重要性。

Conclusion: SentGraph通过显式建模句子级逻辑关系，有效解决了多跳问答中的证据链不完整问题，为RAG框架在多跳推理任务中提供了新的解决方案。
...
TL;DR: MMFormalizer是一个多模态自动形式化方法，通过自适应接地将视觉元素与数学物理实体结合，递归构建形式化命题，在PhyX-AF基准测试中表现优异


<details>
  <summary>Details</summary>
Motivation: 传统自动形式化方法主要处理文本数学，但在现实世界中，物理问题需要从视觉元素推断隐藏约束（如质量或能量），因此需要多模态方法

Method: MMFormalizer通过自适应接地整合真实世界数学物理实体，通过递归接地和公理组合从感知接地原语递归构建形式化命题，自适应递归终止确保每个抽象都有视觉证据支持

Result: 在PhyX-AF基准测试（115个样本）中，GPT-5和Gemini-3-Pro在编译和语义准确性方面表现最佳，GPT-5在物理推理方面表现突出，几何领域仍然最具挑战性
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在临床文本理解方面表现良好，但在需要跨多次就诊进行复杂、非单调症状轨迹推理的纵向预测任务（如痴呆症预后）上存在困难。标准监督训练缺乏症状演变的显式标注，而直接强化学习受到稀疏二元奖励的阻碍。

Method: 提出Dementia-R1框架，采用冷启动强化学习策略，预训练模型从患者历史中预测可验证的临床指标，增强对疾病进展的推理能力，然后再确定最终的临床状态。

Result: 在真实世界非结构化临床数据集上达到77.03%的F1分数。在ADNI基准上，7B模型的表现可与GPT-4o相媲美，有效捕捉波动的认知轨迹。
...
TL;DR: MedDialogRubrics是一个包含5200个合成患者病例和60000多个细粒度评估标准的医疗对话AI基准测试，专门用于评估LLM的多轮诊断能力，采用多智能体系统生成病例并避免隐私问题。


<details>
  <summary>Details</summary>
Motivation: 现有的医疗对话AI评估基准和框架对大型语言模型的信息收集和诊断推理能力评估不够严谨，存在评估不足的问题，需要更系统化的评估方法。

Method: 1) 使用多智能体系统合成患者病例和主诉，不接触真实电子健康记录；2) 设计患者智能体，基于原子医学事实并加入动态指导机制检测纠正幻觉；3) 提出结构化LLM和专家标注的评估标准生成流程，检索循证医学指南并使用拒绝采样确定"必问"项目。

Result: 对最先进模型进行全面评估显示，当前模型在多个评估维度面临重大挑战，表明改进医疗对话需要对话管理架构的进步，而不仅仅是基础模型的增量调整。

Conclusion: MedDialogRubrics为评估医疗LLM的多轮诊断能力提供了系统化基准，揭示了当前模型的局限性，并指出未来需要对话管理架构的创新而不仅仅是模型微调。
...
### [100] [LittiChoQA: Literary Texts in Indic Languages Chosen for Question Answering](https://arxiv.org/abs/2601.03025)
*Aarya Khandelwal,Ritwik Mishra,Rajiv Ratn Shah*

Main category: cs.CL

TL;DR: LittiChoQA是最大的印度文学QA数据集，包含27万+自动生成的问答对，评估多语言LLM在长上下文文学QA中的表现，发现完整上下文微调效果最好但效率低，上下文缩短可提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: 解决印度语言长上下文问答资源稀缺的问题，特别是在文学文本领域，为低资源语言提供评估基准。

Method: 构建LittiChoQA数据集（27万+自动生成的问答对，平衡事实性和非事实性问题），评估多语言LLM在完整上下文和缩短上下文设置下的表现，包括答案段落选择和向量检索方法。

Result: 完整上下文微调获得最高得分（Krutrim-2语义得分76.1），上下文缩短显著提升吞吐量但性能下降（Krutrim-2在缩短上下文中得分74.9和71.4），性能与效率存在明显权衡。

Conclusion: LittiChoQA填补了印度语言文学QA资源空白，为长上下文QA研究提供重要基准，展示了完整上下文与缩短上下文之间的性能-效率权衡，Krutrim-2表现最佳。
...
### [101] [Reducing Hallucinations in LLMs via Factuality-Aware Preference Learning](https://arxiv.org/abs/2601.03027)
*Sindhuja Chaduvula,Ahmed Y. Radwan,Azib Farooq,Yani Ioannou,Shaina Raza*

Main category: cs.CL

TL;DR: F-DPO是一种基于事实感知的直接偏好优化方法，通过标签翻转和事实感知边界来减少大语言模型中的幻觉问题，相比标准DPO能显著提升事实准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的偏好对齐方法（如RLHF和DPO）虽然能改善指令跟随能力，但可能因为奖励流畅性和自信度而非事实准确性，反而强化了模型的幻觉问题。

Method: F-DPO在DPO基础上进行两项改进：(1) 标签翻转转换，确保被选中的回答不比被拒绝的回答事实性更差；(2) 添加事实感知边界，强调有明显正确性差异的配对，当两个回答事实性相同时退化为标准DPO。通过用二元事实性指标和合成的幻觉变体来增强DPO配对数据。

Result: 在七个开源大语言模型（1B-14B）上，F-DPO相比基础模型和标准DPO能持续提升事实性并降低幻觉率。在Qwen3-8B上，幻觉率降低五倍（从0.424到0.084），事实性得分提升50%（从5.26到7.90）。在TruthfulQA上，Qwen2.5-14B的MC1准确率提升17%（0.500到0.585），MC2准确率提升49%（0.357到0.531）。

Conclusion: F-DPO是一种简单有效的改进方法，无需辅助奖励模型、token级标注或多阶段训练，就能显著提升大语言模型的事实准确性并减少幻觉问题，且具有良好的泛化能力。
...
### [102] [NorwAI's Large Language Models: Technical Report](https://arxiv.org/abs/2601.03034)
*Jon Atle Gulla,Peng Liu,Lemei Zhang*

Main category: cs.CL

TL;DR: NorwAI团队开发了专门针对挪威语和斯堪的纳维亚语言的大语言模型家族，基于多种Transformer架构，通过从零预训练或持续预训练，使用挪威语扩展分词器和高级后训练策略，旨在填补挪威语在NLP领域的代表性不足问题。
...
Method: 基于GPT、Mistral、Llama2、Mixtral和Magistral等多种Transformer架构，通过从零预训练或持续预训练（使用25B-88.45B tokens），采用挪威语扩展分词器和高级后训练策略，优化性能、增强鲁棒性和适应性。

Result: 开发了NorwAI大语言模型家族，包括指令调优变体（如Mistral-7B-Instruct和Mixtral-8x7B-Instruct），展示了强大的助手式能力，适合交互式和特定领域应用的实际部署。
...
### [103] [BaseCal: Unsupervised Confidence Calibration via Base Model Signals](https://arxiv.org/abs/2601.03042)
...
<details>
  <summary>Details</summary>
Motivation: 后训练LLM存在严重过度自信问题，而对应的基础LLM通常校准良好，这促使我们利用基础LLM作为参考来校准后训练LLM的置信度

Method: 提出两种方法：1) BaseCal-ReEval：将后训练LLM的响应输入基础LLM，用平均概率作为置信度；2) BaseCal-Proj：训练轻量级投影，将后训练LLM的最终隐藏状态映射回基础LLM的隐藏状态，再通过基础LLM输出层获得校准后的置信度

Result: 在五个数据集和三个LLM家族上的实验表明，BaseCal相比最佳无监督基线平均减少42.90%的预期校准误差

Conclusion: BaseCal是一种无监督、即插即用的解决方案，无需人工标注或LLM修改，能有效校准后训练LLM的置信度，提高模型输出的可信度
...
### [104] [Lil: Less is Less When Applying Post-Training Sparse-Attention Algorithms in Long-Decode Stage](https://arxiv.org/abs/2601.03043)
...
TL;DR: 稀疏注意力算法在解码阶段可能反而增加端到端复杂度，因为信息损失会导致序列显著变长，这种现象称为"越少越少"（Lil）。作者提出早期停止算法来缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署规模扩大，推理效率需求增加。解码阶段通常主导总延迟，现有稀疏注意力算法旨在降低解码阶段的时间和内存复杂度，但可能产生反效果。

Method: 提出早期停止算法，在稀疏解码过程中检测信息损失超过信息增益的阈值，从而减少不必要的token消耗。

Result: 早期停止算法在推理密集型基准测试中，token消耗减少高达90%，准确率下降小于2%。

Conclusion: 稀疏注意力可能因信息损失导致序列变长而增加端到端复杂度（Lil现象），提出的早期停止算法能有效缓解此问题，显著提升推理效率。
...
### [105] [Detecting Hallucinations in Retrieval-Augmented Generation via Semantic-level Internal Reasoning Graph](https://arxiv.org/abs/2601.03052)
*Jianpeng Hu,Yanzeng Li,Jialun Zhong,Wenfa Qi,Lei Zou*

Main category: cs.CL

TL;DR: 本文提出了一种基于语义级内部推理图的忠实性幻觉检测方法，通过扩展层间相关性传播算法构建内部推理图，并设计基于小型预训练语言模型的通用框架，在RAGTruth和Dolly-15k数据集上优于现有方法。
...
Conclusion: 提出的语义级内部推理图方法能够有效检测忠实性幻觉，通过更精细地捕捉模型内部推理过程，提高了幻觉检测的准确性。
...
### [106] [Do LLMs Encode Functional Importance of Reasoning Tokens?](https://arxiv.org/abs/2601.03066)
*Janvijay Singh,Dilek Hakkani-Tür*

Main category: cs.CL

TL;DR: 本文提出贪婪剪枝方法，通过迭代删除对模型似然影响最小的推理标记来压缩推理链，在保持性能的同时减少计算成本。
...
### [107] [Learning to Diagnose and Correct Moral Errors: Towards Enhancing Moral Sensitivity in Large Language Models](https://arxiv.org/abs/2601.03079)
*Bocheng Chen,Han Zi,Xi Chen,Xitong Zhang,Kristen Johnson,Guangliang Liu*

Main category: cs.CL

TL;DR: 本文提出两种语用推理方法，通过诊断道德良性/危险输入和纠正道德错误来增强大语言模型的道德敏感性，在道德相关基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 道德敏感性是人类道德能力的基础，但如何让大语言模型具备道德敏感性一直极具挑战。虽然已有许多方法试图将大语言模型与人类道德价值观对齐，但如何增强其道德敏感性仍是一个重要问题。

Method: 提出两种语用推理方法：1）诊断道德良性和危险输入；2）纠正道德错误。这些方法采用统一视角，不依赖于语义多样且复杂的表层形式建模道德话语，而是基于推理负载设计原则性的语用推理程序。

Result: 实证证据表明，所提出的语用方法能够有效增强大语言模型的道德敏感性，并在代表性的道德相关基准测试中取得了强劲性能。

Conclusion: 通过语用推理方法增强大语言模型的道德敏感性是可行的，这些方法提供了原则性的视角来设计基于推理负载的语用推理程序，为解决大语言模型道德对齐问题迈出了重要一步。
...
### [108] [Grad-ELLM: Gradient-based Explanations for Decoder-only LLMs](https://arxiv.org/abs/2601.03089)
*Xin Huang,Antoni B. Chan*

Main category: cs.CL

TL;DR: Grad-ELLM：一种基于梯度的解码器专用Transformer大语言模型归因方法，通过聚合注意力层梯度和注意力图生成热力图，无需修改模型架构，在多个任务上展现优于现有方法的忠实性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然表现出色，但其黑盒特性引发了对透明度和忠实性的担忧。现有的输入归因方法通常是模型无关的，没有针对Transformer架构优化，导致忠实性有限。

Method: 提出Grad-ELLM方法，基于梯度为解码器专用Transformer LLMs生成归因热力图。通过聚合注意力层的输出logit梯度（通道重要性）和注意力图（空间重要性），在每个生成步骤生成热力图，无需修改模型架构。同时提出了两个改进的忠实性度量指标π-Soft-NC和π-Soft-NS。

Result: 在情感分类、问答和开放生成任务上使用不同模型进行评估，实验结果显示Grad-ELLM在忠实性方面始终优于其他归因方法。

Conclusion: Grad-ELLM为解码器专用Transformer大语言模型提供了一种有效的梯度归因方法，能够生成更忠实的输入贡献热力图，同时提出的改进忠实性度量指标为公平比较提供了更好的评估框架。
...
### [109] [Who Laughs with Whom? Disentangling Influential Factors in Humor Preferences across User Clusters and LLMs](https://arxiv.org/abs/2601.03103)
*Soichiro Murakami,Hidetaka Kamigaito,Hiroya Takamura,Manabu Okumura*

Main category: cs.CL

TL;DR: 该研究通过聚类分析日本创意游戏Oogiri用户的幽默偏好，使用Bradley-Terry-Luce模型估计可解释的偏好因子权重，并探索大语言模型能否模拟特定用户群体的幽默偏好。


<details>
  <summary>Details</summary>
Motivation: 幽默偏好因个体和文化差异而高度多样化，这使得使用大语言模型评估幽默变得复杂。研究旨在建模幽默偏好的异质性，并探索LLMs能否理解和模拟特定用户群体的幽默偏好。

Method: 1. 使用日本创意响应游戏Oogiri的用户投票日志进行聚类分析；2. 采用Bradley-Terry-Luce模型估计各聚类对可解释偏好因子的权重；3. 通过提示LLMs选择更有趣的回应来获取其偏好判断；4. 使用角色提示引导LLMs模拟特定用户群体的偏好。

Result: 1. 用户聚类显示出不同的偏好模式；2. LLMs的偏好结果可以与特定用户聚类相似；3. 通过角色提示，可以引导LLMs的偏好朝向特定用户聚类。

Conclusion: 研究证实了幽默偏好的异质性，并展示了LLMs能够模拟特定用户群体的幽默偏好。通过角色提示可以引导LLMs的偏好方向，为个性化幽默评估提供了可能。研究将发布数据收集和分析脚本以支持可重复性。
...
### [110] [Discovering and Causally Validating Emotion-Sensitive Neurons in Large Audio-Language Models](https://arxiv.org/abs/2601.03115)
*Xiutian Zhao,Björn Schuller,Berrak Sisman*

Main category: cs.CL

TL;DR: 该研究首次对大型音频语言模型中的情感敏感神经元进行了神经元级可解释性分析，在三个主流开源模型中发现并验证了情感特异性神经元的存在及其因果作用。


<details>
  <summary>Details</summary>
Motivation: 情感是语音交流的核心维度，但目前缺乏对现代大型音频语言模型如何内部编码情感机制的深入理解，需要从神经元层面解释这些模型的情感处理机制。

Method: 在Qwen2.5-Omni、Kimi-Audio和Audio Flamingo 3三个开源模型中，使用频率、熵、幅度和对比度等多种神经元选择器，结合推理时干预技术，对情感敏感神经元进行识别和因果验证。

Result: 发现了情感特异性特征：消融特定情感神经元会显著降低该情感的识别准确率而保留其他类别，增益放大则能将预测导向目标情感；这些效应随干预强度系统性地增强，且情感敏感神经元呈现非均匀的层级聚类模式。

Conclusion: 研究为大型音频语言模型的情感决策提供了神经元层面的因果解释，表明针对性神经元干预是实现可控情感行为的有效手段。
...
### [111] [ToxiGAN: Toxic Data Augmentation via LLM-Guided Directional Adversarial Generation](https://arxiv.org/abs/2601.03121)
*Peiran Li,Jan Fillies,Adrian Paschke*

Main category: cs.CL

TL;DR: ToxiGAN：一种结合对抗生成和LLM语义引导的类别感知文本增强框架，用于毒性分类，通过两阶段定向训练和语义锚定解决GAN增强中的模式崩溃和语义漂移问题。


<details>
  <summary>Details</summary>
Motivation: 毒性语言数据的可控、类别特定增强对于提升毒性分类的鲁棒性至关重要，但由于监督有限和分布偏斜，这仍然具有挑战性。现有方法难以在保持语义相关性的同时实现有效的类别特定增强。

Method: 提出ToxiGAN框架：1）结合对抗生成与大型语言模型的语义引导；2）采用两阶段定向训练策略解决模式崩溃和语义漂移；3）利用LLM生成的中性文本作为语义锚定；4）动态选择中性示例提供平衡指导；5）明确优化毒性样本与中性示例的差异以增强类别对比信号。

Result: 在四个仇恨言论基准测试中，ToxiGAN在macro-F1和hate-F1指标上均取得最佳平均性能，一致优于传统和基于LLM的增强方法。消融实验和敏感性分析进一步证实了语义锚定和定向训练对提升分类器鲁棒性的益处。

Conclusion: ToxiGAN通过结合对抗生成和LLM语义引导，有效解决了毒性数据增强中的关键挑战，为类别感知的文本增强提供了新思路，显著提升了毒性分类器的鲁棒性和性能。
...
### [112] [Improving Indigenous Language Machine Translation with Synthetic Data and Language-Specific Preprocessing](https://arxiv.org/abs/2601.03135)
*Aashish Dhawan,Christopher Driggers-Ellis,Christan Grant,Daisy Zhe Wang*

Main category: cs.CL

TL;DR: 使用多语言翻译模型生成合成数据来增强美洲土著语言的机器翻译性能，通过实验验证合成数据对Guarani-西班牙语和Quechua-西班牙语翻译的改进效果。
...
Method: 使用高容量多语言翻译模型生成合成句子对来增强美洲土著语言的平行数据集，在多语言mBART模型上进行微调，并应用语言特定的预处理（包括正字法规范化和噪声感知过滤）来减少语料库伪影。

Result: Guarani-西班牙语和Quechua-西班牙语翻译实验显示合成数据增强带来了chrF++指标的持续改进，而Aymara的诊断实验突显了通用预处理对高度黏着性语言的局限性。
...
### [113] [Limited Linguistic Diversity in Embodied AI Datasets](https://arxiv.org/abs/2601.03136)
*Selma Wanna,Agnes Luhtaru,Jonathan Salfity,Ryan Barron,Juston Moore,Cynthia Matuszek,Mitch Pryor*

Main category: cs.CL

TL;DR: 对多个广泛使用的VLA数据集进行系统审计，发现这些数据集中的指令语言高度重复、模板化，缺乏语言多样性，限制了VLA模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 语言在视觉-语言-动作模型中扮演关键角色，但目前用于训练和评估这些系统的数据集的语料库语言特征缺乏系统文档记录，需要了解这些数据集实际包含什么类型的指令以及提供多少语言多样性。
...
Conclusion: 这些发现为当前VLA训练和评估数据中的语言信号提供了描述性文档，旨在支持更详细的数据集报告、更原则性的数据集选择，以及扩大语言覆盖范围的有针对性数据整理或增强策略。
...
### [114] [Self-Verification is All You Need To Pass The Japanese Bar Examination](https://arxiv.org/abs/2601.03144)
*Andrew Shin*

Main category: cs.CL

TL;DR: 首个通过日本司法考试的大语言模型，使用自验证方法和忠实于考试格式的数据集训练，无需改变原始问题结构或评分规则。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型发展迅速，但在高度专业化和结构化的考试（如日本司法考试）中实现可靠性能仍具挑战性。现有方法虽然通过将复杂问题分解为简单判断来改进，但未在原始考试格式和评分方案下系统评估，无法确定是否真正具备考试水平能力。

Method: 提出自验证模型，使用新构建的数据集训练，该数据集忠实复制了考试的原始格式和评分标准。模型能够在不改变原始问题结构或评分规则的情况下，通过一致性验证来确保答案质量。

Result: 模型在真实考试评分标准下超过了官方及格分数，这是首个已知的在不改变原始问题结构或评分规则的情况下通过日本司法考试的LLM。与多智能体推理和基于分解的监督等替代策略相比，这些方法无法达到可比性能。

Conclusion: 结果强调了格式忠实监督和一致性验证的重要性，表明精心设计的单模型方法在高风险专业推理任务中可以超越更复杂的系统。格式忠实监督对于专业考试性能至关重要。
...
### [115] [Decoupling the Effect of Chain-of-Thought Reasoning: A Human Label Variation Perspective](https://arxiv.org/abs/2601.03154)
*Beiduo Chen,Tiancheng Hu,Caiqi Zhang,Robert Litschko,Anna Korhonen,Barbara Plank*

Main category: cs.CL

TL;DR: 研究发现长链思维推理虽然能提升单一答案任务的准确性，但在处理人类标签变异性这类需要捕捉概率模糊性的任务时，其作为分布校准器的能力有限。


<details>
  <summary>Details</summary>
Motivation: 尽管推理调优的大语言模型在需要单一答案的任务上表现出色，但它们在处理需要捕捉概率模糊性而非解决模糊性的任务（如人类标签变异性）方面的能力尚未得到充分探索。

Method: 通过系统性的解耦实验，在基于分布的任务上使用交叉链思维实验，将推理文本的影响与模型内在先验分离开来。

Result: 观察到"解耦机制"：链思维能改善分布对齐，但最终准确性由链思维内容决定（贡献99%方差），而分布排名则由模型先验主导（超过80%）。逐步分析显示链思维对准确性的影响在推理过程中单调增长，但分布结构主要由大语言模型的内在先验决定。

Conclusion: 长链思维可以作为大语言模型对最佳选项的决策制定者，但在模糊任务中无法作为细粒度的分布校准器。
...
### [116] [WebAnchor: Anchoring Agent Planning to Stabilize Long-Horizon Web Reasoning](https://arxiv.org/abs/2601.03164)
*Yu Xinmiao,Zhang Liwen,Feng Xiaocheng,Jiang Yong,Qin Bing,Xie Pengjun,Zhou Jingren*

Main category: cs.CL

TL;DR: 论文提出Anchor-GRPO框架，通过两阶段强化学习解决LLM智能体在长视野网页信息搜索中的规划瓶颈问题，重点关注"规划锚点"现象并优化首步规划。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的LLM智能体在网页信息搜索中面临长视野规划瓶颈，存在"规划锚点"现象（首步推理对下游行为影响过大），而当前RL算法未能有效处理这一问题。

Method: 提出Anchor-GRPO两阶段RL框架：第一阶段通过自对弈经验和人工校准的细粒度评分优化首步规划；第二阶段通过稀疏奖励将执行与初始规划对齐，确保稳定高效的工具使用。
...
Conclusion: Anchor-GRPO有效解决了长视野网页推理中的规划锚点问题，通过解耦规划和执行的两阶段框架显著提升了LLM智能体的网页信息搜索能力，并展现出良好的可扩展性。
...
### [117] [Can Embedding Similarity Predict Cross-Lingual Transfer? A Systematic Study on African Languages](https://arxiv.org/abs/2601.03168)
*Tewodros Kederalah Idris,Prasenjit Mitra,Roald Eiselen*

Main category: cs.CL

TL;DR: 本文系统评估了五种嵌入相似度指标在跨语言迁移中的预测能力，发现余弦差距和检索指标能可靠预测迁移成功率，但不同模型间相关性符号会反转，需要按模型验证。


<details>
  <summary>Details</summary>
Motivation: 对于低资源非洲语言，跨语言迁移至关重要，但实践者缺乏可靠的方法来选择源语言。本文旨在为源语言选择提供具体指导，并强调模型特定分析的重要性。

Method: 系统评估五种嵌入相似度指标（包括余弦差距、检索指标P@1、CSLS、CKA等），在816个迁移实验中进行测试，涵盖三个NLP任务、三个非洲中心多语言模型和来自四个语系的12种语言。

Result: 余弦差距和检索指标（P@1、CSLS）能可靠预测迁移成功率（ρ=0.4-0.6），而CKA预测能力可忽略（ρ≈0.1）。关键发现是跨模型聚合时相关性符号会反转（辛普森悖论），因此必须按模型验证。嵌入指标与URIEL语言类型学具有相当的预测能力。

Conclusion: 研究结果为源语言选择提供了具体指导，强调了模型特定分析的重要性，并表明嵌入相似度指标是有效的源语言选择工具，但需要在特定模型上下文中使用。
...
### [118] [Maximizing Local Entropy Where It Matters: Prefix-Aware Localized LLM Unlearning](https://arxiv.org/abs/2601.03190)
*Naixin Zhai,Pengyang Shao,Binbin Zheng,Fei Shen,Long Bai,Xun Yang*

Main category: cs.CL

TL;DR: PALU提出了一种针对大语言模型机器遗忘的新框架，通过前缀感知的局部熵最大化，仅在敏感前缀和top-k logits上进行优化，避免了对整个词汇表和参数空间的不必要操作。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法通常对所有响应token进行无差别处理，并在整个词汇表上强制不确定性，导致不必要的效用下降和内容无关区域的优化扩展。

Method: 提出PALU框架，采用时间和词汇维度上的局部熵最大化目标，发现仅抑制敏感前缀就足以切断因果生成链接，仅平坦化top-k logits就足以最大化关键子空间的不确定性。
...
### [119] [MemRL: Self-Evolving Agents via Runtime Reinforcement Learning on Episodic Memory](https://arxiv.org/abs/2601.03192)
...
<details>
  <summary>Details</summary>
Motivation: 人类智能的核心特征是通过情景模拟来掌握新技能，但现有大型语言模型难以实现这种自我进化。微调计算成本高且容易灾难性遗忘，而基于记忆的方法依赖被动语义匹配，容易检索到噪声。
...
Conclusion: MemRL使智能体能够在运行时持续改进而无需权重更新，实现了类似人类的情景模拟学习能力，为LLM的持续自我进化提供了有效解决方案。
...
### [120] [X-MuTeST: A Multilingual Benchmark for Explainable Hate Speech Detection and A Novel LLM-consulted Explanation Framework](https://arxiv.org/abs/2601.03194)
...
TL;DR: X-MuTeST是一个用于仇恨言论检测的可解释性引导训练框架，结合了大型语言模型的高级语义推理与传统注意力增强技术，针对印地语、泰卢固语和英语等资源不足语言提供基准人类标注理由。


<details>
  <summary>Details</summary>
Motivation: 社交媒体仇恨言论检测在准确性和可解释性方面面临挑战，特别是对于资源不足的印度语言。现有方法缺乏足够的可解释性，且针对这些语言的研究不足。

Method: 提出X-MuTeST框架，结合LLM的高级语义推理与传统注意力增强技术。通过计算原始文本与n-gram预测概率差异生成解释，并将LLM解释与X-MuTeST解释结合。在训练中利用人类标注理由来增强分类性能和可解释性。

Result: 在包含6,004个印地语、4,492个泰卢固语和6,334个英语样本的数据集上评估，显示利用人类理由训练能提升分类性能和可解释性。结合人类理由与可解释性方法进一步改进模型注意力，通过合理性（Token-F1、IOU-F1）和忠实性（Comprehensiveness、Sufficiency）指标验证效果。

Conclusion: X-MuTeST框架通过结合LLM推理与传统注意力技术，有效提升了仇恨言论检测的可解释性和准确性，特别针对资源不足语言。该工作推动了跨语言仇恨言论检测的发展，数据集和代码已开源。
...
### [121] [DIP: Dynamic In-Context Planner For Diffusion Language Models](https://arxiv.org/abs/2601.03199)
...
Method: 提出动态上下文规划器(DIP)，利用扩散生成范式允许在生成过程中动态调整上下文的特性，动态选择和插入上下文示例，而不是一次性提供所有示例。

Result: DIP在保持生成质量的同时，相比标准推理实现了高达12.9倍的推理加速，相比KV缓存增强推理实现了1.17倍的加速。

Conclusion: DIP方法有效解决了扩散语言模型在长上下文场景下的计算效率问题，为扩散模型的实用化提供了重要技术支撑。
...
### [122] [UltraLogic: Enhancing LLM Reasoning through Large-Scale Data Synthesis and Bipolar Float Reward](https://arxiv.org/abs/2601.03205)
*Yile Liu,Yixian Liu,Zongwei Li,Yufei Huang,Xinhua Feng,Zhichao Hu,Jinglu Hu,Jianfeng Yan,Fengzong Lian,Yuhong Liu*

Main category: cs.CL

TL;DR: UltraLogic框架通过代码化方法自动生成大规模、高质量、难度分级的多步骤推理数据，解决LLMs在复杂通用推理上的瓶颈，并引入双极浮动奖励机制提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在自然语言处理方面表现出潜力，但在需要多步骤逻辑、规划和验证的复杂通用推理任务上仍存在关键瓶颈。当前缺乏大规模、高质量且难度分级的通用推理数据，限制了强化学习与可验证奖励方法的应用。

Method: 提出UltraLogic框架：1）通过基于代码的求解方法将问题的逻辑核心与自然语言表达解耦，自动化生成高质量数据；2）包含数百种独特任务类型和跨十个难度级别的自动校准流程；3）引入双极浮动奖励机制，使用分级惩罚来区分完美回答与存在逻辑缺陷的回答，避免二元奖励稀疏性和非负奖励陷阱。

Result: 实验表明：1）任务多样性是提升推理能力的主要驱动力；2）双极浮动奖励机制结合难度匹配策略显著提高了训练效率；3）该方法能有效引导模型达到全局逻辑最优。

Conclusion: UltraLogic框架通过自动化数据生成和创新的奖励机制，为解决大语言模型在复杂通用推理任务上的瓶颈提供了有效方案，任务多样性和难度匹配策略对提升模型推理能力至关重要。
...
### [123] [MalruleLib: Large-Scale Executable Misconception Reasoning with Step Traces for Modeling Student Thinking in Mathematics](https://arxiv.org/abs/2601.03217)
*Xinghe Chen,Naiming Liu,Shashank Sonkar*

Main category: cs.CL

TL;DR: MalruleLib是一个将数学学习中的系统性错误转化为可执行程序的框架，通过参数化模板生成大量错误推理轨迹，用于评估AI模型对学生错误概念的识别和预测能力。


<details>
  <summary>Details</summary>
Motivation: 学生在数学学习中经常出现系统性错误，即反复应用错误但一致的程序。现有AI系统难以准确识别和预测这些错误概念，需要专门的基础设施来建模学生的错误推理过程。

Method: 基于67个学习科学和数学教育文献，将文档化的错误概念转化为可执行程序，创建参数化问题模板，生成正确推理和错误推理的双路径轨迹，支持大规模监督和受控评估。

Result: 构建了包含101个错误规则和498个参数化问题模板的库，可生成超过100万个实例。实验显示，语言模型在跨模板错误概念预测上的准确率从直接解题的66%下降到40%，提供学生步骤轨迹可将预测准确率提高3-15%。

Conclusion: MalruleLib为教育AI提供了建模学生跨情境错误推理的基础设施，支持诊断和针对性反馈，有助于开发更有效的教育工具来识别和纠正学生的系统性错误。
...
### [124] [Multi-RADS Synthetic Radiology Report Dataset and Head-to-Head Benchmarking of 41 Open-Weight and Proprietary Language Models](https://arxiv.org/abs/2601.03232)
...
TL;DR: 研究创建了RXL-RADSet多RADS基准数据集，比较了开源小语言模型与专有模型在放射学报告RADS分类任务上的表现，发现20-32B参数的大SLM在引导提示下可接近专有模型性能。


<details>
  <summary>Details</summary>
Motivation: 放射学报告和数据系统（RADS）用于标准化风险沟通，但自动化RADS分配面临挑战：指南复杂、输出格式限制、跨RADS框架和模型大小的基准测试有限。

Method: 创建包含1,600份合成放射学报告的RXL-RADSet基准数据集，涵盖10种RADS标准和多种模态。报告由LLM生成，经过两阶段放射科医生验证。评估了41个量化SLM（0.135-32B参数）和GPT-5.2，使用固定引导提示进行测试。

Result: GPT-5.2在引导提示下达到99.8%有效性和81.1%准确率。SLM整体达到96.8%有效性和61.1%准确率，20-32B参数的顶级SLM达到约99%有效性和70%+准确率。性能随模型规模增加而提升，引导提示相比零样本提示显著改善性能。

Conclusion: RXL-RADSet提供了放射科医生验证的多RADS基准；大型SLM（20-32B）在引导提示下可接近专有模型性能，但在高复杂度RADS方案上仍存在差距。
...
### [125] [Automated Semantic Rules Detection (ASRD) for Emergent Communication Interpretation](https://arxiv.org/abs/2601.03254)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统中的涌现通信研究关注智能体如何自主发展通信策略，但现有研究很少关注涌现语言的可解释性问题。本文旨在解决这一研究空白。

Method: 提出自动化语义规则检测（ASRD）算法，该算法从在Lewis Game上训练的两个不同数据集的智能体交换的消息中提取相关模式，并将这些模式与输入数据的特定属性关联起来。

Result: ASRD算法能够有效提取涌现通信中的相关模式，并将这些模式与输入数据的特定属性相关联，从而显著简化后续分析过程。

Conclusion: ASRD算法为解释多智能体系统中的涌现通信提供了有效工具，通过将涌现语言模式与数据属性关联，提高了涌现语言的可解释性，为后续分析提供了便利。
...
### [126] [Textual Explanations and Their Evaluations for Reinforcement Learning Policy](https://arxiv.org/abs/2601.02514)
*Ahmad Terra,Mohit Ahmed,Rafia Inam,Elena Fersman,Martin Törngren*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的可解释强化学习框架，通过LLM生成文本解释并转换为透明规则，结合专家知识和自动谓词生成器，通过两种精炼技术提高解释质量，在开源环境和电信用例中验证了框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 理解强化学习策略对于确保自主智能体符合人类期望至关重要。虽然文本解释易于人类理解，但确保其正确性仍然是一个挑战，现有评估方法有限。需要一种能够生成高质量文本解释并系统评估其性能的框架。

Method: 1. 使用大型语言模型生成文本解释，结合聚类技术识别频繁条件；2. 将条件转换为透明规则以评估其属性、保真度和性能；3. 提出两种精炼技术改进解释质量并减少冲突信息；4. 引入专家知识和自动谓词生成器确定状态语义信息。

Result: 在三个开源环境中进行了实验以确保可复现性，并在电信用例中评估了工业适用性。该框架解决了现有自主策略解释方法的局限性，生成的透明规则在某些任务上能够达到满意性能，实现了对文本解释的系统化和定量评估。

Conclusion: 提出的XRL框架能够生成高质量的文本解释并将其转换为透明规则，通过结合专家知识和自动谓词生成器提高了解释的准确性。该框架为XRL领域提供了有价值的见解，支持对文本解释的系统评估，具有实际工业应用潜力。
...
TL;DR: 该论文研究了设备端AI模型在实际部署中的关键问题，包括模型选择、资源消耗和领域适应能力，并通过构建韩英直播聊天翻译基准LiveChatBench进行实验验证。


<details>
  <summary>Details</summary>
Motivation: 尽管设备端AI模型效率高，但关于实际部署所需考虑的设备CPU利用率和热条件等实际问题研究较少。需要解决设备端模型选择和资源消耗，以及领域适应能力这两个关键问题。

Method: 1. 构建LiveChatBench基准数据集：包含1000个韩英平行句对的直播聊天翻译任务；2. 在5种移动设备上进行实验；3. 分析不同模型的资源消耗和性能表现。
...
### [128] [Learning from Prompt itself: the Hierarchical Attribution Prompt Optimization](https://arxiv.org/abs/2601.02683)
*Dongyu Chen,Jian Ma,Xianpeng Zhang,Lei Zhang,Haonan Lu,Chen Chen,Chuangchuang Wang,Kai Tang*

Main category: cs.AI

TL;DR: HAPO框架通过分层归因机制解决提示优化中的漂移问题，提升优化效率并保持可解释性
...
Conclusion: HAPO为可扩展的提示工程建立了一个可扩展的范式，解决了提示漂移问题，同时保持了优化过程的可解释性。
...
### [129] [Learning User Preferences Through Interaction for Long-Term Collaboration](https://arxiv.org/abs/2601.02702)
*Shuhaib Mehri,Priyanka Kargupta,Tal August,Dilek Hakkani-Tür*

Main category: cs.AI

TL;DR: MultiSessionCollab基准测试评估智能体在多轮会话中学习用户偏好并提升协作质量的能力，通过配备记忆机制和从用户模拟器中学习信号来优化长期协作效果。
...
Method: 提出MultiSessionCollab基准测试，开发配备持久化记忆的长期协作智能体，记忆随交互经验积累而不断优化。从用户模拟器行为中提取学习信号，训练智能体生成更全面的反思并更有效地更新记忆。

Result: 实验表明配备记忆的智能体显著提升长期协作效果：任务成功率更高、交互更高效、用户努力减少。人类用户研究证实记忆机制在真实场景中改善用户体验。

Conclusion: 记忆机制对智能体的长期协作能力至关重要，MultiSessionCollab基准为评估和提升智能体在多轮会话中学习用户偏好的能力提供了有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 对话中的人类相似性在人机交互中至关重要，但难以定义、测量和优化，目前的改进主要依赖规模扩展或广泛的监督训练，而非有针对性的对齐。

Method: HAL框架从对比对话数据中提取明确的对话特征，将其组合成紧凑的标量分数，并将这个透明奖励信号用于标准偏好优化方法进行模型对齐。

Result: 使用HAL对齐的模型在对话中被更频繁地感知为人类相似，且不影响整体性能；该框架允许检查对齐行为和诊断意外效应。

Conclusion: HAL展示了如何将语言中软性、定性的属性（以前超出对齐范围）变得可测量，并以可解释和可解释的方式进行对齐。
...
TL;DR: 论文提出了一种基于因果图增强的检索增强生成系统，通过整合显式因果推理与双层知识图谱，在医学系统综述任务中实现了95%的准确率和零幻觉，显著优于基线AI模型。


<details>
  <summary>Details</summary>
Motivation: 医学系统综述对于循证医学至关重要，但每年150万+的出版物使得人工审阅不可行。现有AI方法在系统综述任务中存在幻觉问题（早期模型28-40%，现代实现2-15%），这在影响患者护理时是不可接受的。

Method: 提出了因果图增强的检索增强生成系统，整合显式因果推理与双层知识图谱。采用证据优先协议，确保每个因果主张都能追溯到检索到的文献，并自动生成可视化干预-结果路径的有向无环图。

Result: 在234篇痴呆症运动摘要评估中，CausalAgent实现了95%的准确率、100%的检索成功率和零幻觉，而基线AI仅为34%的准确率和10%的幻觉率。自动因果图实现了显式机制建模、可视化合成和增强的可解释性。
...
TL;DR: 提出了一种神经符号深度强化学习方法，通过整合符号知识提高样本效率和泛化能力，将简单任务的部分策略作为先验知识加速复杂任务学习。


<details>
  <summary>Details</summary>
Motivation: 当前深度强化学习算法需要大量训练数据且泛化能力有限，特别是在小规模训练场景下。需要一种方法能够利用先验知识加速学习过程，提高在未见任务上的性能。

Method: 提出神经符号DRL方法，将简单任务的部分策略表示为逻辑规则，通过在线推理指导训练：1) 在探索阶段偏置动作分布；2) 在利用阶段重新缩放Q值。这种方法在完全可观察和部分可观察的网格世界环境中进行了验证。

Result: 在具有挑战性的网格世界环境变体上验证了方法的有效性，相比最先进的奖励机制基线表现出更好的性能，特别是在稀疏奖励环境和长规划视野任务中加速了收敛。

Conclusion: 神经符号集成方法通过整合背景符号知识，提高了深度强化学习的样本效率、泛化能力和可解释性，为复杂环境中的顺序决策问题提供了更有效的解决方案。
...
TL;DR: 该论文提出了M3MAD-Bench，一个统一且可扩展的多智能体辩论基准测试，用于评估多领域、多模态、多维度的MAD方法。


<details>
  <summary>Details</summary>
Motivation: 现有MAD研究存在两个基本限制：评估在碎片化且不一致的设置下进行，阻碍了公平比较；主要局限于依赖纯文本输入的单模态场景。

Method: 提出M3MAD-Bench基准，建立五个核心任务领域的标准化协议（知识、数学、医学、自然科学、复杂推理），系统覆盖纯文本和视觉语言数据集，支持跨模态比较。评估了9个不同架构、规模和模态能力的基础模型，并纳入效率导向指标如token消耗和推理时间。
...
Method: 提出ReTreVal框架，整合树状思维探索、自我精炼、LLM批判评分和反思记忆。构建基于问题复杂度的自适应深度推理树，每个节点进行迭代自我批判和精炼，采用双重验证机制评估推理质量，通过批判性剪枝保留最高评分节点，并将成功路径和失败模式存储在反思记忆缓冲区中实现跨问题学习。

Result: 在500个数学问题和创意写作任务上使用Qwen 2.5 7B作为底层LLM进行评估，ReTreVal在结构化探索、批判驱动精炼和跨问题记忆的结合下，始终优于ReAct、Reflexion和Self-Refine等现有方法。

Conclusion: ReTreVal通过结构化探索、批判驱动精炼和跨问题记忆的组合，特别适用于需要探索性推理、严格验证和知识迁移的任务，为LLM的多步推理提供了有效的解决方案。
...
TL;DR: 该研究发现大语言模型在逻辑推理中存在"逻辑相变"现象：推理性能在逻辑复杂度达到临界深度前保持稳定，但超过临界点后会突然崩溃。作者提出了神经符号课程调优框架来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 符号逻辑推理是大语言模型的关键但未被充分探索的能力，在数学推理和法律判决等高风险领域提供可靠且可验证的决策。目前对逻辑推理在复杂度增加时的表现缺乏系统分析。

Method: 提出神经符号课程调优框架：1) 自适应地对齐自然语言与逻辑符号以建立共享表示；2) 围绕相变边界重塑训练动态，逐步增强在增加逻辑深度下的推理能力。

Result: 在五个基准测试上的实验表明，该方法有效缓解了高复杂度下的逻辑推理崩溃，在朴素提示下平均准确率提升+1.26，在思维链提示下提升+3.95，同时提高了对未见逻辑组合的泛化能力。

Conclusion: 发现了大语言模型逻辑推理中的"逻辑相变"现象，并提出了有效的神经符号课程调优框架来增强模型在高复杂度下的逻辑推理能力，为可靠逻辑推理系统的发展提供了新思路。
...
Method: 提出Batch-of-Thought (BoT)方法，通过批量处理相关查询实现跨实例学习，包括识别高质量推理模板、通过一致性检查检测错误、分摊计算成本。进一步在BoT-R中实现多智能体反射架构，其中Reflector执行联合评估以获取孤立处理无法获得的互信息增益。

Result: 在三个模型系列和六个基准测试上的实验表明，BoT-R持续提高准确性和置信度校准，同时将推理成本降低高达61%。理论和实验分析揭示了批量感知推理何时以及为何有益于LLM系统。

Conclusion: Batch-of-Thought通过利用跨实例信号显著提升LLM推理性能，同时降低计算成本，为LLM系统设计提供了新的批量感知推理范式。
...
TL;DR: RationaleTS方法通过引入基于原理的上下文学习，解决多模态大语言模型在时间序列推理中的不足，使用标签条件化原理作为指导推理单元而非事后解释，提升推理性能。
...
Method: 提出RationaleTS方法：1) 诱导标签条件化原理，构建从可观测证据到潜在结果的推理路径；2) 设计混合检索机制，平衡时间模式和语义上下文，为最终上下文推理检索相关原理先验。
...
TL;DR: 提出了一种更简单但同样有效的自动提示工程系统，无需调参和任务线索，首次应用于数据库表列名扩展任务，并在英语和德语数据集上验证
...
Method: 开发了一个简单设计的自动提示工程系统，无需调参和任务明确线索，专注于数据库表列名扩展任务，支持多语言应用

Result: 在英语和德语数据集上评估了该方法，证明了其有效性，这是首次将自动提示工程应用于列名扩展任务，也是首次在英语以外的语言上应用自动提示工程

Conclusion: 提出的自动提示工程系统设计简单但效果显著，为数据库表列名扩展任务提供了有效解决方案，并展示了在多语言场景下的应用潜力
...
TL;DR: InfiAgent是一个通过外部化状态到文件系统来保持推理上下文有界的通用框架，解决了LLM智能体在长时任务中因上下文无限增长和错误累积而崩溃的问题。


<details>
  <summary>Details</summary>
Motivation: LLM智能体虽然能够推理和使用工具，但在长时任务中经常因上下文无限增长和错误累积而崩溃。现有的上下文压缩或检索增强提示方法在信息保真度和推理稳定性之间存在权衡。

Method: 提出InfiAgent框架，通过将持久状态外部化到以文件为中心的状态抽象中，保持智能体推理上下文严格有界。在每个步骤中，智能体从工作空间状态快照加上最近动作的固定窗口重建上下文。

Result: 在DeepResearch和80篇文献综述任务上的实验表明，无需任务特定微调，使用20B开源模型的InfiAgent与更大的专有系统竞争，并保持比基于上下文的基线显著更高的长时覆盖度。

Conclusion: 这些结果支持显式状态外部化作为稳定长时智能体的实用基础。明确的状态外部化是构建稳定长时智能体的可行方法。
