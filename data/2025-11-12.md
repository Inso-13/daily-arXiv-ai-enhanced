...
Motivation: 现有方法存在两个基本限制：缺乏评估视觉和文本token重要性的鲁棒模态内机制，导致在复杂场景中泛化能力差；以及缺乏细粒度不确定性建模，无法捕捉区域-单词对应关系的一对多和多对一性质。
...
Conclusion: 该方法显著增强了细粒度图像-文本对齐的鲁棒性和可解释性，为解决这一关键多模态学习挑战提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 超声成像作为无创临床诊断的基石，其有限的视野限制了新视角合成，需要开发专门针对超声成像的优化方法。

Method: 1. 引入深度感知高斯泼溅策略，为每个高斯分配可学习的视野；2. 设计SH-DARS轻量级渲染函数，结合低阶球谐函数和超声特定波物理特性；3. 贡献临床超声检查数据集作为基准。

Result: 在三个数据集上实现了最先进的结果：PSNR最高29.55，SSIM最高0.89，MSE低至0.002，同时实现实时合成（64.69 fps）。

Conclusion: UltraGS框架在超声成像的新视角合成任务中表现出色，代码和数据集已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本或布局条件模型无法实现像素级精确的卫星图像合成，需要开发能够对齐图像和语义向量几何的跨模态对应方法。

Method: 使用视觉语言对齐模块从多边形语义生成像素级嵌入，通过条件图像生成框架同时考虑空间范围和语义线索，支持语言提示与几何感知条件的混合交互。

Result: 在语义保真度和结构真实性方面显著优于现有方法，训练出的视觉语言模型展现出细粒度的空间基础能力。

Conclusion: VectorSynth框架能够实现像素级精确的卫星图像合成，支持交互式编辑和空间模拟，为地理信息处理提供了新的工具。
...
### [4] [Class Incremental Medical Image Segmentation via Prototype-Guided Calibration and Dual-Aligned Distillation](https://arxiv.org/abs/2511.07749)
*Shengqian Zhu,Chengrong Yu,Qiang Wang,Ying Song,Guangjun Li,Jiafei Wu,Xiaogang Xu,Zhang Yi,Junjie Hu*

Main category: cs.CV

TL;DR: 本文提出了PGCD和DAPD方法来解决类增量医学图像分割中的知识保留问题，通过原型引导的校准蒸馏和双对齐原型蒸馏来增强旧类知识的保留效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个问题：一是采用一刀切策略，对所有空间区域和特征通道同等对待，阻碍了准确旧知识的保留；二是仅关注旧类全局原型的对齐，忽略了新数据中的局部表示，导致知识退化。

Method: 提出了PGCD和DAPD两种方法：PGCD利用原型到特征的相似性来校准不同空间区域的类特定蒸馏强度；DAPD将当前模型提取的旧类局部原型与全局原型和局部原型进行双重对齐。

Result: 在两个广泛使用的多器官分割基准数据集上的综合评估表明，该方法优于最先进的方法，展现了其鲁棒性和泛化能力。

Conclusion: 所提出的PGCD和DAPD方法有效解决了类增量医学图像分割中的知识保留问题，显著提升了旧类别的分割性能。
...
### [5] [Beyond Randomness: Understand the Order of the Noise in Diffusion](https://arxiv.org/abs/2511.07756)
*Song Yan,Min Li,Bi Xinliang,Jian Yang,Yusen Zhang,Guanye Xiong,Yunwei Lan,Tao Zhang,Wei Zhai,Zheng-Jun Zha*

Main category: cs.CV

TL;DR: 本文揭示了扩散模型中初始噪声的可分析模式，提出了基于信息论的语义擦除-注入两步法来调制初始噪声，实现无训练、通用的语义控制。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为扩散模型的初始噪声是随机元素，仅贡献生成多样性。但本文发现噪声中蕴含丰富的可分析语义模式，这为优化扩散模型生成提供了新视角。

Method: 提出两步"语义擦除-注入"过程：1）基于信息论从噪声中擦除不需要的语义；2）利用扩散模型生成过程与语义注入的等价性，将语义注入清理后的噪声中。该方法无需训练，适用于DiT和UNet架构。

Result: 实验证明该方法在各种基于DiT和UNet架构的T2C模型中均有效，为扩散模型生成优化提供了通用工具，支持一致性生成。

Conclusion: 初始噪声不仅包含丰富语义信息，还能通过简单的方法进行语义调制。该方法为扩散模型生成优化开辟了新途径，提供了通用的语义控制工具。
...
### [6] [Semantic-Consistent Bidirectional Contrastive Hashing for Noisy Multi-Label Cross-Modal Retrieval](https://arxiv.org/abs/2511.07780)
...
<details>
  <summary>Details</summary>
Motivation: 现有跨模态哈希方法依赖全标注数据集且忽略多标签数据的部分语义重叠问题，在真实场景中标签噪声严重影响检索性能
...
Conclusion: SCBCH框架能够有效处理多标签数据中的标签噪声和语义重叠问题，显著提升跨模态检索的鲁棒性和泛化能力
...
### [7] [Divide-and-Conquer Decoupled Network for Cross-Domain Few-Shot Segmentation](https://arxiv.org/abs/2511.07798)
*Runmin Cong,Anpeng Wang,Bin Wan,Cong Zhang,Xiaofei Zhou,Wei Zhang*

Main category: cs.CV

TL;DR: 提出DCDNet方法解决跨域少样本分割中的特征纠缠问题，通过特征解耦和动态融合提升模型泛化能力
...
Method: 提出对抗对比特征分解模块解耦特征，矩阵引导动态融合模块自适应整合特征，交叉自适应调制模块在微调阶段增强泛化

Result: 在四个挑战性数据集上的实验表明，DCDNet优于现有CD-FSS方法，在跨域泛化和少样本适应方面达到新的最优性能

Conclusion: DCDNet通过有效的特征解耦和融合机制，成功解决了跨域少样本分割中的特征纠缠问题，显著提升了模型性能
...
### [8] [PC-Diffusion: Aligning Diffusion Models with Human Preferences via Preference Classifier](https://arxiv.org/abs/2511.07806)
*Shaomeng Wang,He Wang,Xiaolu Wei,Longquan Dai,Jinhui Tang*

Main category: cs.CV

TL;DR: 提出了PC-Diffusion框架，通过轻量级偏好分类器实现扩散模型的人类偏好对齐，解决了DPO方法计算成本高和依赖参考模型的问题。
...
Conclusion: PC-Diffusion通过偏好分类器有效解决了DPO方法的局限性，为扩散模型的人类偏好对齐提供了更高效稳定的解决方案。
...
### [9] [DI3CL: Contrastive Learning With Dynamic Instances and Contour Consistency for SAR Land-Cover Classification Foundation Model](https://arxiv.org/abs/2511.07808)
*Zhongle Ren,Hui Ding,Kai Wang,Biao Hou,Xingyu Luo,Weibin Li,Licheng Jiao*

Main category: cs.CV

TL;DR: 提出了一种用于SAR土地覆盖分类的通用基础模型DI3CL，通过动态实例和轮廓一致性对比学习预训练框架，结合大规模SARSense数据集，在多个下游任务中表现出优越性能。
...
Method: 提出DI3CL预训练框架，包含动态实例模块（增强全局上下文感知）和轮廓一致性模块（引导模型关注SAR对象的几何轮廓），并使用包含460,532张SAR图像的大规模SARSense数据集进行预训练。

Result: 在SAR土地覆盖制图、水体检测和道路提取等多种任务上的实验表明，DI3CL方法优于现有方法。

Conclusion: 该基础模型为SAR土地覆盖分类提供了强大的基础，能够加速下游模型的开发和部署，具有优异的泛化能力。
...
### [10] [Revisiting MLLM Based Image Quality Assessment: Errors and Remedy](https://arxiv.org/abs/2511.07812)
*Zhenchen Tang,Songlin Yang,Bo Peng,Zichuan Wang,Jing Dong*

Main category: cs.CV

TL;DR: 本文提出Q-Scorer框架，通过轻量级回归模块和IQA专用评分标记解决多模态大语言模型在图像质量评估任务中的离散输出与连续质量分数不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在图像质量评估任务中存在离散标记输出与连续质量分数需求不匹配的问题，导致性能受限，且传统方法存在转换误差和语义混淆。

Method: 提出Q-Scorer框架，包含轻量级回归模块和IQA专用评分标记，集成到MLLM流程中，通过理论分析指导设计。
...
TL;DR: Sparse3DPR是一个无需训练的开源3D场景理解框架，利用预训练大语言模型的推理能力，仅需稀疏视图RGB输入，通过层次化平面增强场景图和任务自适应子图提取方法，显著提升推理准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的无需训练3D场景理解方法在实际部署中存在准确性和效率问题，需要一种更可靠的解决方案。

Method: 提出层次化平面增强场景图，利用主导平面结构作为空间锚点，支持开放词汇；设计任务自适应子图提取方法，动态过滤查询无关信息。

Result: 在Space3D-Bench上相比ConceptGraphs实现28.7%的EM@1提升和78.2%的速度提升；在ScanQA上达到与训练方法相当的性能，真实世界实验验证了其鲁棒性和泛化能力。

Conclusion: Sparse3DPR通过结合预训练LLM推理能力和创新的场景表示方法，为无需训练的3D场景理解提供了高效准确的解决方案。
...
TL;DR: 合成相关扩散成像(CDI^s)作为标准扩散成像协议的增强，显著提升了前列腺癌病灶分割性能，在94%的配置中改善或保持分割效果，最高实现72.5%的相对提升。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习的前列腺癌病灶分割方法性能有限，Dice分数在大型患者队列中仅为0.32或更低，需要更有效的成像增强方法。

Method: 使用200名患者的共配准CDI^s、DWI和ADC序列，在六种最先进的分割架构上进行全面评估，比较不同成像组合的性能。

Result: CDI^s集成在94%的配置中可靠地增强或保持分割性能，个别架构相比基线模态实现72.5%的统计显著相对改进。CDI^s + DWI是最安全的增强途径，在一半架构中实现显著改进且无性能下降实例。
...
### [13] [Human Motion Synthesis in 3D Scenes via Unified Scene Semantic Occupancy](https://arxiv.org/abs/2511.07819)
*Gong Jingyu,Tong Kunkun,Chen Zhuoran,Yuan Chuanhan,Chen Mingang,Zhang Zhizhong,Tan Xin,Xie Yuan*

Main category: cs.CV

TL;DR: SSOMotion是一个基于场景语义占用(SSO)的人体运动合成框架，通过双向三平面分解和CLIP编码实现细粒度场景语义理解，在复杂场景中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景中的人体运动合成方法主要关注场景结构而忽略语义理解，需要更全面的场景表示方法。

Method: 提出统一的场景语义占用(SSO)表示，采用双向三平面分解获得紧凑SSO，通过CLIP编码和线性降维将场景语义映射到统一特征空间，结合运动方向进行帧级场景查询控制运动。

Result: 在ShapeNet家具杂乱场景以及PROX和Replica扫描场景上的大量实验表明，该方法具有最先进的性能，验证了其有效性和泛化能力。

Conclusion: SSOMotion框架通过统一的场景语义表示实现了细粒度场景理解，显著减少了冗余计算，在复杂3D场景中实现了高质量的人体运动合成。
...
### [14] [MonoCLUE : Object-Aware Clustering Enhances Monocular 3D Object Detection](https://arxiv.org/abs/2511.07862)
*Sunghun Yang,Minhyeok Lee,Jungho Lee,Sangyoun Lee*

Main category: cs.CV

TL;DR: MonoCLUE通过结合局部聚类和广义场景记忆的视觉特征，提升单目3D物体检测在遮挡和有限视野下的鲁棒性，在KITTI基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 单目3D物体检测存在深度信息不明确和视野受限的问题，导致几何线索不足，在遮挡或截断场景中精度降低。现有方法虽然引入额外深度信息，但忽视了视觉线索对鲁棒识别的重要性。

Method: 1. 对视觉特征进行K-means聚类，捕捉物体级别的外观部分（如引擎盖、车顶），提升部分可见物体的检测；2. 构建广义场景记忆，通过跨图像聚合聚类特征提供一致表示；3. 将局部聚类特征和广义场景记忆集成到物体查询中，引导注意力到信息丰富的区域。
...
### [15] [Visual Bridge: Universal Visual Perception Representations Generating](https://arxiv.org/abs/2511.07877)
*Yilin Gao,Shuguang Dou,Junzhou Li,Zhiheng Yu,Yin Li,Dongsheng Jiang,Shugong Xu*

Main category: cs.CV

TL;DR: 提出基于流匹配的通用视觉感知框架，能够生成多种任务的视觉表示，突破传统"单任务-单模型"限制，在分类、检测、分割、深度估计等任务上取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型局限于"单任务-单模型"范式，限制了多任务场景下的泛化性和可扩展性。受大语言模型跨领域泛化能力启发，希望开发通用视觉感知框架。

Method: 将过程建模为从图像块标记到任务特定表示的通用流匹配问题，利用自监督基础模型作为锚点，引入多尺度循环任务嵌入机制，学习通用速度场来桥接异构任务。

Result: 在分类、检测、分割、深度估计和图像-文本检索等任务上，无论是零样本还是微调设置都取得了竞争性性能，超越先前通用模型和多个专用模型。
...
### [16] [Generating Sketches in a Hierarchical Auto-Regressive Process for Flexible Sketch Drawing Manipulation at Stroke-Level](https://arxiv.org/abs/2511.07889)
*Sicong Zang,Shuhui Gao,Zhijun Fang*

Main category: cs.CV

TL;DR: 本文提出了一种分层自回归草图生成方法，通过三个阶段的层次化过程（笔画预测、锚定和转换）实现草图生成的灵活控制，允许在生成过程中随时调整笔画嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要在生成前同时提供所有笔画条件，无法在生成过程中进行灵活控制。本文旨在实现更灵活的草图绘制操控。

Method: 采用分层自回归生成过程：1)预测笔画嵌入，2)在画布上锚定笔画，3)将嵌入转换为绘制动作序列。整个过程是自回归的，考虑已生成笔画及其位置。

Result: 提出的方法能够在草图生成过程中随时通过调整可编辑的笔画嵌入来灵活控制笔画级草图绘制。

Conclusion: 分层自回归草图生成方法提供了更灵活的草图绘制操控能力，通过在生成过程中暴露可编辑的笔画嵌入，实现了动态的笔画级控制。
...
### [17] [Theoretical Analysis of Power-law Transformation on Images for Text Polarity Detection](https://arxiv.org/abs/2511.07916)
*Narendra Singh Yadav,Pavan Kumar Perepu*

Main category: cs.CV

TL;DR: 本文对文本极性检测和图像二值化预处理任务进行了理论分析，特别是针对基于幂律变换的方法中观察到的类间方差变化现象。


<details>
  <summary>Details</summary>
Motivation: 文本极性检测是图像二值化的重要预处理步骤，现有基于幂律变换的直观方法观察到类间方差随文本极性变化的现象，但缺乏理论分析。

Method: 对基于幂律变换的文本极性检测方法进行理论分析，研究文本和背景两类在变换图像中类间方差的变化规律。

Result: 理论分析验证了经验观察：对于暗文本亮背景，类间方差随变换增加；对于亮文本暗背景，类间方差随变换减少。

Conclusion: 本文为文本极性检测的幂律变换方法提供了理论依据，证实了经验观察到的类间方差变化规律。
...
### [18] [Exploring the Underwater World Segmentation without Extra Training](https://arxiv.org/abs/2511.07923)
*Bingyu Li,Tao Huo,Da Zhang,Zhiyuan Zhao,Junyu Gao,Xuelong Li*

Main category: cs.CV

TL;DR: 本文提出了AquaOV255首个大规模细粒度水下分割数据集和UOVSBench基准，以及无需训练的Earth2Ocean框架，通过几何引导视觉掩码生成和类别-视觉语义对齐模块，将陆地视觉-语言模型迁移到水下领域。
...
Method: 提出Earth2Ocean框架，包含几何引导视觉掩码生成器（GMG）通过自相似性几何先验优化视觉特征，以及类别-视觉语义对齐（CSA）模块通过多模态大语言模型推理和场景感知模板构建增强文本嵌入。

Result: 在UOVSBench基准上的广泛实验表明，Earth2Ocean实现了显著的性能提升，同时保持高效推理。

Conclusion: 该工作为水下开放词汇分割提供了首个大规模数据集和基准，提出的无需训练框架有效解决了陆地模型到水下领域的迁移问题。
...
### [19] [HD$^2$-SSC: High-Dimension High-Density Semantic Scene Completion for Autonomous Driving](https://arxiv.org/abs/2511.07925)
*Zhiwen Yang,Yuxin Peng*

Main category: cs.CV

TL;DR: 提出HD²-SSC框架解决相机3D语义场景补全中的维度差距和密度差距问题，通过高维语义解耦和高密度占用优化来提升3D场景表示质量。
...
Method: HD²-SSC框架包含高维语义解耦模块（沿伪第三维度扩展2D特征，解耦粗粒度像素语义）和高密度占用优化模块（检测-优化架构，利用上下文几何和语义结构完成缺失体素并修正错误体素）。

Result: 在SemanticKITTI和SSCBench-KITTI-360数据集上的广泛实验验证了HD²-SSC框架的有效性。

Conclusion: 提出的HD²-SSC框架成功解决了SSC任务中的维度差距和密度差距问题，显著提升了3D语义场景补全性能。
...
### [20] [An Image-Based Path Planning Algorithm Using a UAV Equipped with Stereo Vision](https://arxiv.org/abs/2511.07928)
*Selim Ahmet Iz,Mustafa Unel*

Main category: cs.CV

TL;DR: 本文提出了一种基于计算机视觉的新型图像路径规划算法，并与A*和PRM算法进行了比较分析。该方法使用无人机生成的地形视差图，结合边缘、线、角点检测等计算机视觉技术来定义轨迹候选路径点。


<details>
  <summary>Details</summary>
Motivation: 地形深度对路径安全有重要影响，但二维图像无法区分陨石坑和山丘等特征。需要利用视差图来准确识别地形特征，提高路径规划的安全性。

Method: 使用无人机生成地形视差图，应用边缘、线、角点检测等计算机视觉技术，结合立体深度重建技术处理捕获图像。使用ArUco标记姿态估计和圆检测技术自动识别起点和目标点。

Result: 在V-REP仿真程序和实验室物理环境中进行了测试，与A*和PRM算法进行比较，结果显示所提算法效果良好。

Conclusion: 提出的基于计算机视觉的路径规划算法在仿真和实际环境中都表现出有效性，为复杂地形下的安全路径规划提供了可行方案。
...
### [21] [Federated CLIP for Resource-Efficient Heterogeneous Medical Image Classification](https://arxiv.org/abs/2511.07929)
*Yihang Wu,Ahmad Chaddad*

Main category: cs.CV

TL;DR: 提出FedMedCLIP方法，基于CLIP的联邦学习框架用于医学图像分类，通过掩码特征适应模块减少通信负载，冻结CLIP编码器降低计算开销，使用掩码MLP作为本地分类器，并通过KL散度蒸馏实现模块间相互学习。
...
Method: 1) 使用掩码特征适应模块作为通信模块减少通信负载；2) 冻结CLIP编码器降低计算开销；3) 掩码MLP作为私有本地分类器；4) 基于KL散度的自适应蒸馏正则化实现模块间相互学习；5) 模型压缩传输参数，集成预测进行分类。

Result: 在四个公开医学数据集上的实验表明，该方法在ISIC2019上比次优基线性能高8%，资源成本合理（比FedAVG快120倍）。

Conclusion: FedMedCLIP方法在医学图像分类中实现了可行性能与合理资源成本的平衡，为联邦学习在医学领域的应用提供了有效解决方案。
...
### [22] [DiffRegCD: Integrated Registration and Change Detection with Diffusion Features](https://arxiv.org/abs/2511.07935)
*Seyedehnanita Madani,Rama Chellappa,Vishal M. Patel*

Main category: cs.CV

TL;DR: DiffRegCD是一个统一的密集配准和变化检测框架，通过将对应估计重新定义为高斯平滑分类任务，利用预训练扩散模型的多尺度特征，在多种数据集上超越了现有基线方法。
...
Method: 将对应估计重新定义为高斯平滑分类任务实现亚像素精度，利用预训练去噪扩散模型的冻结多尺度特征，通过受控仿射扰动生成配准和变化检测的配对真实数据。

Result: 在航空和地面数据集上的广泛实验表明，DiffRegCD持续超越现有基线方法，在广泛的时间和几何变化下保持可靠性。
...
### [23] [Is It Truly Necessary to Process and Fit Minutes-Long Reference Videos for Personalized Talking Face Generation?](https://arxiv.org/abs/2511.07940)
*Rui-Qing Sun,Ang Li,Zhijing Wu,Tian Lan,Qianyu Lu,Xingshan Yao,Chen Xu,Xian-Ling Mao*

Main category: cs.CV

TL;DR: 本文提出ISExplore策略，通过选择信息丰富的5秒参考视频片段，显著提升说话人脸生成的数据处理和训练效率，同时保持高质量输出。


<details>
  <summary>Details</summary>
Motivation: 现有基于NeRF或3DGS的说话人脸生成方法需要处理数分钟的参考视频，耗时数小时，严重限制了实际应用价值。研究发现视频信息质量比长度更重要，短而信息丰富的片段能达到甚至优于长视频的效果。
...
Conclusion: 视频片段的信息质量比长度更重要，ISExplore策略通过智能选择信息丰富的短片段，显著提升了说话人脸生成方法的效率和实用性。
...
### [24] [Libra-MIL: Multimodal Prototypes Stereoscopic Infused with Task-specific Language Priors for Few-shot Whole Slide Image Classification](https://arxiv.org/abs/2511.07941)
*Zhenfeng Zhuang,Fangyu Zhou,Liansheng Wang*

Main category: cs.CV

TL;DR: 提出一种多模态原型多实例学习方法，通过平衡信息压缩方案促进双向交互，利用冻结LLM生成病理实体描述作为文本原型，视觉分支学习实例级原型，使用立体最优传输算法进行融合，在三个癌症数据集上展示优越的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决WSI图像计算成本高和缺乏细粒度医学知识导致的LLM描述偏差问题，现有方法单向指导限制跨模态协同，需要构建任务特异性病理实体原型来学习可泛化特征并增强模型可解释性。

Method: 多模态原型多实例学习：利用冻结LLM生成任务特异性病理实体描述作为文本原型，视觉分支学习实例级原型减少对冗余数据的依赖，使用基于相似度度量的立体最优传输算法进行融合。

Result: 在三个不同癌症数据集上进行少样本分类和可解释性实验，结果表明所提方法具有优越的泛化能力。

Conclusion: 提出的多模态原型多实例学习方法通过双向交互和平衡信息压缩，有效解决了病理图像分析中的计算成本和描述偏差问题，展示了良好的泛化性能。
...
### [25] [ReIDMamba: Learning Discriminative Features with Visual State Space Model for Person Re-Identification](https://arxiv.org/abs/2511.07948)
*Hongyang Gu,Qisong Yang,Lei Pu,Siming Han,Yao Ding*

Main category: cs.CV

TL;DR: 提出了ReIDMamba，一个纯Mamba驱动的人员重识别框架，通过多粒度特征提取器和排序感知三元组正则化来增强特征多样性和鲁棒性，在五个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer在人员重识别中面临的计算和内存需求二次增长问题，同时提取鲁棒的判别性特征。

Method: 设计了基于Mamba的强基线，引入多类令牌；开发了多粒度特征提取器模块和排序感知三元组正则化技术。

Result: ReIDMamba参数仅为TransReID的三分之一，GPU内存使用更低，推理速度更快，在五个人员重识别基准测试中达到最先进性能。

Conclusion: ReIDMamba是首个将纯Mamba方法集成到人员重识别研究中的工作，展示了优越的性能和效率。
...
### [26] [ChexFract: From General to Specialized - Enhancing Fracture Description Generation](https://arxiv.org/abs/2511.07983)
...
<details>
  <summary>Details</summary>
Motivation: 现有通用放射学报告生成模型在描述罕见但临床重要的病理（如骨折）方面存在不足，需要开发专门模型来填补这一空白

Method: 使用MAIRA-2和CheXagent的编码器训练骨折特定的视觉语言模型

Result: 专门模型相比通用模型在生成准确骨折描述方面有显著改进，分析了模型输出在骨折类型、位置和年龄方面的表现

Conclusion: 开发并公开发布了性能最佳的骨折报告模型，促进未来在罕见病理准确报告方面的研究
...
### [27] [CSF-Net: Context-Semantic Fusion Network for Large Mask Inpainting](https://arxiv.org/abs/2511.07987)
...
Conclusion: CSF-Net通过语义引导框架显著提升大掩码图像修复的质量，促进结构准确性和语义一致性，在各种掩码条件下持续提升性能。
...
### [28] [Hardware-Aware YOLO Compression for Low-Power Edge AI on STM32U5 for Weeds Detection in Digital Agriculture](https://arxiv.org/abs/2511.07990)
...
<details>
  <summary>Details</summary>
Motivation: 传统杂草管理方法依赖化学除草剂，存在环境污染和抗药性杂草问题。精准除草技术虽然环保，但通常依赖高功耗计算平台。

Method: 采用YOLOv8n目标检测器，应用结构化剪枝、整数量化和输入图像分辨率缩放等压缩技术，在STM32U575ZI微控制器上部署。

Result: 在包含74种植物物种的CropAndWeed数据集上训练评估，实现了检测精度与效率的平衡，支持实时原位杂草检测。

Conclusion: 该系统在功耗受限的农业环境中具有可扩展部署潜力，为可持续农业提供了低功耗的杂草检测解决方案。
...
### [29] [Sharp Eyes and Memory for VideoLLMs: Information-Aware Visual Token Pruning for Efficient and Reliable VideoLLM Reasoning](https://arxiv.org/abs/2511.08003)
*Jialong Qin,Xin Zou,Di Lu,Yibo Yan,Xuming Hu*

Main category: cs.CV

TL;DR: SharpV是一种高效的自适应视觉token和KV缓存剪枝方法，通过动态调整剪枝比率和自校准机制，解决了VideoLLMs中二次计算复杂度和KV缓存扩展的问题。
...
Method: SharpV采用两阶段自适应剪枝框架：1）基于时空信息动态调整视觉token剪枝比率；2）通过自校准方式剪枝退化的视觉特征，基于与原始视觉特征的相似性指导。

Result: 在多个公共基准测试中表现出优越性，偶尔甚至比密集模型获得性能提升，且无需访问注意力分数，完全兼容硬件加速技术。

Conclusion: SharpV提供了一个新的自适应剪枝范式，从信息瓶颈角度实现了层次化缓存剪枝，为VideoLLMs的信息流提供了新见解。
...
### [30] [EAGLE: Episodic Appearance- and Geometry-aware Memory for Unified 2D-3D Visual Query Localization in Egocentric Vision](https://arxiv.org/abs/2511.08007)
...
Method: 提出EAGLE框架，整合外观感知元学习记忆(AMM)指导的分割和几何感知定位记忆(GLM)驱动的跟踪，通过结构化外观和几何记忆库存储高置信度检索样本，并利用视觉几何基础Transformer(VGGT)实现2D-3D任务的高效统一。

Result: 在Ego4D-VQ基准上实现了最先进的性能，能够实现精确的轮廓描绘和稳健的空间判别，显著提高了检索准确性。

Conclusion: EAGLE框架通过记忆整合机制有效支持目标外观变化的长期和短期建模，实现了自我中心视觉查询的精确2D-3D统一定位。
...
### [31] [Invisible Triggers, Visible Threats! Road-Style Adversarial Creation Attack for Visual 3D Detection in Autonomous Driving](https://arxiv.org/abs/2511.08015)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于RGB相机的视觉3D检测系统容易受到对抗性攻击，但传统的对抗性海报外观不自然且容易被发现和防御，因此需要开发更隐蔽的攻击方法。

Method: 采用两阶段方法：道路风格对抗性生成和场景关联适应，确保海报具有自然外观的同时最大化攻击效果。
...
### [32] [High-Quality Proposal Encoding and Cascade Denoising for Imaginary Supervised Object Detection](https://arxiv.org/abs/2511.08018)
*Zhiyuan Chen,Yuelin Guo,Zitong Huang,Haoyu He,Renhao Lu,Weizhe Zhang*

Main category: cs.CV

TL;DR: 本文提出了Cascade HQP-DETR方法，通过高质量合成数据生成、基于高质量建议的查询编码和级联去噪算法，解决了合成数据训练对象检测模型时的三个主要问题，在仅使用合成数据训练12个周期后，在PASCAL VOC 2007上达到了61.04% mAP@0.5的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于合成图像的监督对象检测方法面临的三个关键问题：合成数据集质量差且监督弱、DETR检测器收敛慢且容易过拟合合成模式、均匀去噪压力导致模型过拟合伪标签噪声。

Method: 1) 使用LLaMA-3、Flux和Grounding DINO构建高质量数据流水线生成FluxVOC和FluxCOCO数据集；2) 提出高质量建议引导的查询编码，利用SAM生成建议和RoI池化特征初始化对象查询；3) 设计级联去噪算法，通过逐层增加IoU阈值动态调整训练权重。

Result: 仅使用FluxVOC合成数据训练12个周期，在PASCAL VOC 2007测试集上达到61.04% mAP@0.5的SOTA性能，超越了强基线方法，其竞争性的真实数据性能证实了架构的普适适用性。

Conclusion: Cascade HQP-DETR通过高质量合成数据生成、图像特定的查询初始化和动态去噪策略，有效解决了合成数据训练对象检测模型的关键挑战，实现了优异的真实世界泛化性能。
...
### [33] [Multi-modal Deepfake Detection and Localization with FPN-Transformer](https://arxiv.org/abs/2511.08031)
*Chende Zheng,Ruiqi Suo,Zhoulin Ji,Jingyi Deng,Fangbin Yi,Chenhao Lin,Chao Shen*

Main category: cs.CV

TL;DR: 提出基于特征金字塔变换器（FPN-Transformer）的多模态深度伪造检测与定位框架，利用预训练自监督模型提取音频和视频特征，通过多尺度特征金字塔和局部注意力机制实现跨模态关联分析，同时预测伪造概率和精确定位篡改片段。


<details>
  <summary>Details</summary>
Motivation: 现有单模态检测方法无法利用跨模态相关性且难以精确定位伪造片段，面对日益复杂的细粒度伪造操作存在局限性，需要开发能够同时进行跨模态检测和精确定位的解决方案。

Method: 使用WavLM和CLIP预训练模型提取音频视频的层次化时序特征，构建多尺度特征金字塔，通过R-TLM块和局部注意力机制分析跨上下文时序依赖，采用双分支预测头同时预测伪造概率和优化篡改片段的时间偏移。

Result: 在IJCAI'25 DDL-AV基准测试集上获得0.7535的最终得分，在具有挑战性的环境中实现了跨模态深度伪造检测和定位的良好性能。

Conclusion: 该方法在跨模态深度伪造检测和定位方面表现出有效性，为广义深度伪造检测提供了新思路，代码已开源。
...
### [34] [Perceptual Quality Assessment of 3D Gaussian Splatting: A Subjective Dataset and Prediction Metric](https://arxiv.org/abs/2511.08032)
*Zhaolin Wan,Yining Diao,Jingqi Xu,Hao Wang,Zhiyang Li,Xiaopeng Fan,Wangmeng Zuo,Debin Zhao*

Main category: cs.CV

TL;DR: 本文提出了首个3D高斯泼溅(3DGS)主观质量评估数据集3DGS-QA，包含225个退化重建样本，并开发了无需参考的质量预测模型，直接从3D高斯基元提取特征来评估感知质量。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注3DGS的算法性能和视觉保真度，但在不同重建条件下渲染内容的感知质量尚未得到系统研究。视角稀疏性、训练迭代次数不足、点云下采样、噪声和颜色失真等因素会显著降低视觉质量。

Method: 构建了包含15种对象类型的225个退化重建数据集，开发了无需参考的质量预测模型，直接从3D高斯基元中提取空间和光度线索，以结构感知的方式估计感知质量。

Result: 实验结果表明，该方法在3DGS内容评估中始终实现卓越性能，证明了其鲁棒性和有效性。同时对现有质量评估方法进行了基准测试。
...
### [35] [WEDepth: Efficient Adaptation of World Knowledge for Monocular Depth Estimation](https://arxiv.org/abs/2511.08036)
*Gongshu Wang,Zhirui Wang,Kan Yang*

Main category: cs.CV

TL;DR: WEDepth是一种新颖的单目深度估计方法，通过适配视觉基础模型而不修改其结构和预训练权重，有效激发和利用其固有先验知识。
...
Conclusion: 该方法在多样化场景中展现出强大的零样本迁移能力，证明了视觉基础模型先验知识在单目深度估计任务中的有效性。
...
### [36] [Generalized-Scale Object Counting with Gradual Query Aggregation](https://arxiv.org/abs/2511.08048)
...
### [37] [Taming Identity Consistency and Prompt Diversity in Diffusion Models via Latent Concatenation and Masked Conditional Flow Matching](https://arxiv.org/abs/2511.08061)
*Aditi Singhania,Arushi Jain,Krutik Malani,Riddhi Dhawan,Souymodip Chakraborty,Vineet Batra,Ankit Phogat*

Main category: cs.CV

TL;DR: 本文提出了一种基于LoRA微调的扩散模型，采用潜在连接策略和掩码条件流匹配目标，实现强身份一致性和高提示多样性的主题驱动图像生成。同时引入了两阶段蒸馏数据策展框架和细粒度评估框架CHARIS。


<details>
  <summary>Details</summary>
Motivation: 主题驱动图像生成需要在保持主体核心身份特征的同时实现多样化的上下文合成，但身份一致性和提示多样性之间存在根本性权衡。

Method: 使用LoRA微调的扩散模型，结合潜在连接策略（联合处理参考图像和目标图像）和掩码条件流匹配目标；提出两阶段蒸馏数据策展框架：第一阶段通过数据恢复和VLM过滤从多样来源创建高质量种子数据集，第二阶段使用这些数据高效微调；开发CHARIS评估框架进行细粒度质量评估。

Result: 该方法实现了强大的身份保持能力，无需架构修改，能够扩展到各种主题和上下文，在身份一致性、提示遵循、颜色保真度、视觉质量和变换多样性方面表现优异。

Conclusion: 提出的方法有效解决了主题驱动图像生成中的身份一致性与多样性权衡问题，通过高效的数据策展和细粒度评估框架，实现了高质量的主题驱动图像合成。
...
### [38] [Radar-APLANC: Unsupervised Radar-based Heartbeat Sensing via Augmented Pseudo-Label and Noise Contrast](https://arxiv.org/abs/2511.08071)
*Ying Wang,Zhaodong Sun,Xu Cheng,Zuxian He,Xiaobai Li*

Main category: cs.CV

TL;DR: 提出了第一个用于雷达心跳传感的无监督框架Radar-APLANC，通过增强伪标签和噪声对比，无需昂贵的地面真实生理信号即可实现与有监督方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统雷达心跳传感方法因噪声而性能下降，基于学习的方法需要昂贵的标记信号进行监督训练。

Method: 使用雷达范围矩阵中的心跳范围和噪声范围分别构建正负样本，提出噪声对比三元组损失，仅使用正样本、负样本和传统雷达方法生成的伪标签信号，并设计了具有自适应噪声感知标签选择的伪标签增强方法。

Result: 在Equipleth数据集和自收集雷达数据集上的广泛实验表明，该无监督方法达到了与最先进有监督方法相当的性能。

Conclusion: Radar-APLANC框架成功实现了无需地面真实生理信号的无监督雷达心跳传感，具有实际应用价值。
...
### [39] [CLIP is All You Need for Human-like Semantic Representations in Stable Diffusion](https://arxiv.org/abs/2511.08075)
*Cameron Braunstein,Mariya Toneva,Eddy Ilg*

Main category: cs.CV

TL;DR: 本文研究发现Stable Diffusion等潜在扩散模型在文本到图像生成中的语义理解主要来自CLIP文本编码器，而非反向扩散过程。扩散过程主要作为视觉解码器，而语义表示能力主要源于CLIP模型。
...
Result: 发现语义理解的成功主要归因于CLIP的文本编码，而非反向扩散过程；不同语义属性的解码准确度差异显著；在反向扩散过程中属性变得越来越难以区分。
...
### [40] [Beyond the Pixels: VLM-based Evaluation of Identity Preservation in Reference-Guided Synthesis](https://arxiv.org/abs/2511.08087)
...
TL;DR: 该论文提出了一个名为"Beyond the Pixels"的分层评估框架，用于解决生成模型中身份保持评估的挑战。该框架通过结构化推理将身份评估分解为特征级转换，使用视觉语言模型进行具体转换分析而非抽象相似度评分。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标依赖全局嵌入或粗略的VLM提示，无法捕捉细粒度身份变化且诊断洞察有限，因此需要更精细的身份保持评估方法。

Method: 分层分解主体为(类型,风格)→属性→特征决策树，并提示具体转换而非抽象相似度评分，将VLM分析基于可验证的视觉证据以减少幻觉并提高一致性。

Result: 在四个最先进的生成模型上验证了该框架，显示在测量身份一致性方面与人类判断有很强的一致性。同时创建了包含1078个图像-提示对的新基准，涵盖多样主体类型和每个提示6-7个转换轴。

Conclusion: 提出的分层评估框架能够有效评估生成模型中的身份保持，减少幻觉并提高分析一致性，为细粒度身份变化评估提供了新方法。
...
### [41] [StableMorph: High-Quality Face Morph Generation with Stable Diffusion](https://arxiv.org/abs/2511.08090)
...
<details>
  <summary>Details</summary>
Motivation: 现有的人脸融合生成方法往往产生模糊、充满伪影或构建不良的图像，这些图像容易被检测且不能代表最危险的攻击。需要高质量、逼真的融合图像来开发和评估有效的融合攻击检测系统。

Method: 使用基于扩散模型的现代图像合成技术生成融合人脸图像，能够产生全头部图像、保持清晰细节、避免常见视觉缺陷，并提供对视觉属性的无与伦比的控制。

Result: StableMorph生成的图像不仅媲美或超过真实人脸图像的质量，而且保持强大的欺骗人脸识别系统的能力，对现有MAD解决方案构成更大挑战。

Conclusion: StableMorph通过创建更逼真和有效的攻击来改进生物特征安全评估，并支持开发更强大的检测系统，为研究和操作测试中的融合质量设定了新标准。
...
### [42] [Introducing Nylon Face Mask Attacks: A Dataset for Evaluating Generalised Face Presentation Attack Detection](https://arxiv.org/abs/2511.08114)
*Manasa,Sushrut Patwardhan,Narayan Vetrekar,Pavan Kumar,R. S. Gad,Raghavendra Ramachandra*

Main category: cs.CV

TL;DR: 该论文提出了一个针对尼龙面罩（NFMs）的新型演示攻击数据集，使用iPhone 11 Pro收集了3760个真实样本和51281个攻击样本，评估了五种先进PAD方法的性能。


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统易受演示攻击影响，尼龙面罩因其弹性结构和逼真外观能有效模拟受害者面部几何特征，构成严重安全威胁。

Method: 创建了专注于尼龙面罩攻击的新数据集，使用iPhone 11 Pro在真实智能手机使用条件下收集数据，涵盖人类和人体模型四种不同演示场景。

Result: 五种最先进PAD方法在未见攻击条件下表现出显著性能差异，表明尼龙面罩对现有防御技术构成严峻挑战。

Conclusion: 尼龙面罩攻击凸显了开发能够有效泛化到新兴欺骗威胁的PAD技术的重要性。
...
### [43] [LatentPrintFormer: A Hybrid CNN-Transformer with Spatial Attention for Latent Fingerprint identification](https://arxiv.org/abs/2511.08119)
*Arnab Maity,Manasa,Pavan Kumar C,Raghavendra Ramachandra*

Main category: cs.CV

TL;DR: LatentPrintFormer是一种新颖的潜指纹识别方法，结合CNN和Transformer架构，通过空间注意力机制提升识别性能，在两个公开数据集上超越了现有技术。
...
Method: 集成EfficientNet-B0 CNN和Swin Tiny Transformer提取局部和全局特征，使用空间注意力模块强调高质量脊线区域，特征融合后投影到512维嵌入空间，通过余弦相似度进行匹配。

Result: 在两个公开数据集上的实验表明，LatentPrintFormer在Rank-10识别率上持续优于三种最先进的潜指纹识别技术。

Conclusion: 提出的多模态融合方法有效提升了潜指纹识别性能，证明了结合局部和全局特征提取的优势。
...
### [44] [Foam Segmentation in Wastewater Treatment Plants: A Federated Learning Approach with Segment Anything Model 2](https://arxiv.org/abs/2511.08130)
*Mehmet Batuhan Duman,Alejandro Carnero,Cristian Martín,Daniel Garrido,Manuel Díaz*

Main category: cs.CV

TL;DR: 本文提出了一种结合联邦学习和SAM2图像分割模型的框架，用于污水处理厂泡沫自动监测，解决数据隐私和标注数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 污水处理厂泡沫形成会降低处理效率并增加成本，但传统机器学习方法需要大量标注数据且存在数据隐私问题，不同污水处理厂之间难以共享数据。

Method: 采用联邦学习范式结合SAM2图像分割模型，在分布式客户端（边缘节点）上微调SAM2，通过Flower框架由中央Fog服务器协调聚合模型权重而不访问私有数据。

Result: 该框架加速了训练收敛并提高了分割性能，即使在有限的本地数据集上也能取得良好效果，使用真实污水处理厂图像、合成泡沫数据集和公开数据集进行验证。
...
### [45] [PEOD: A Pixel-Aligned Event-RGB Benchmark for Object Detection under Challenging Conditions](https://arxiv.org/abs/2511.08140)
...
Method: 提出PEOD数据集，包含130+时空对齐序列和34万手动标注框，57%数据在挑战条件下采集，并在三种输入配置(事件、RGB、事件-RGB融合)上对14种方法进行基准测试。
...
### [46] [Boomda: Balanced Multi-objective Optimization for Multimodal Domain Adaptation](https://arxiv.org/abs/2511.08152)
*Jun Sun,Xinxin Zhang,Simin Hong,Jian Zhu,Xiang Gao*

Main category: cs.CV

TL;DR: 本文提出了一种名为Boomda的多模态领域自适应方法，通过平衡多目标优化解决异构多模态领域自适应问题，其中不同模态在源域和目标域之间存在不同的领域偏移。


<details>
  <summary>Details</summary>
Motivation: 多模态学习面临手动标注成本高昂的问题，而无监督领域自适应在单模态设置中已被广泛研究，但在多模态设置中仍较少探索。本文旨在解决异构多模态领域自适应问题。

Method: 首先引入信息瓶颈方法独立学习每个模态的表示，然后在表示空间中使用相关对齐匹配源域和目标域。将问题构建为多目标任务，寻求帕累托最优解，通过特定模型特性简化为二次规划问题，进一步近似得到闭式解。

Result: 广泛的实证结果表明所提方法的有效性，Boomda优于竞争方案。

Conclusion: 提出的Boomda方法通过平衡多目标优化实现了有效的多模态领域自适应，在异构多模态设置中表现出优越性能。
...
### [47] [Non-Aligned Reference Image Quality Assessment for Novel View Synthesis](https://arxiv.org/abs/2511.08155)
...
<details>
  <summary>Details</summary>
Motivation: 新视角合成图像的感知质量评估面临挑战，全参考方法在不对齐时失效，而无参考方法泛化能力不足。需要一种能处理部分场景共享但像素级不对齐的参考图像的质量评估方法。

Method: 构建大规模合成失真图像数据集，使用对比学习框架结合LoRA增强的DINOv2嵌入，仅使用合成失真数据训练以避免过拟合，通过现有IQA方法监督指导。

Result: 模型在不对齐参考情况下优于最先进的全参考、无参考和非对齐参考IQA方法，在用户研究中与主观评分有强相关性。

Conclusion: 提出的NAR-IQA框架能有效评估新视角合成图像质量，在不对齐参考情况下表现优异，具有良好的泛化能力和与人类感知的一致性。
...
### [48] [LandSegmenter: Towards a Flexible Foundation Model for Land Use and Land Cover Mapping](https://arxiv.org/abs/2511.08156)
*Chenying Liu,Wei Huang,Xiao Xiang Zhu*

Main category: cs.CV

TL;DR: 本文提出LandSegmenter框架，通过弱监督方式构建LULC基础模型，解决传统模型依赖特定模态和固定分类体系的问题，在零样本场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统LULC模型受限于特定模态和固定分类体系，基础模型需要大量标注数据，在遥感领域成本高昂且不切实际。

Method: 提出LandSegmenter框架：输入端使用弱标签构建LAS数据集；模型端集成遥感专用适配器和文本编码器；输出端采用类别置信度引导融合策略。

Result: 在6个LULC数据集上的实验表明，LandSegmenter在迁移学习和零样本设置中达到竞争性或更优性能，特别是在零样本场景下。

Conclusion: LandSegmenter框架和弱监督方法有效构建了任务特定的基础模型，为LULC领域提供了可扩展的解决方案。
...
### [49] [KPLM-STA: Physically-Accurate Shadow Synthesis for Human Relighting via Keypoint-Based Light Modeling](https://arxiv.org/abs/2511.08169)
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散方法在生成真实且几何准确的阴影方面仍存在不足，特别是在合成图像中难以同时实现高外观真实性和几何精度。

Method: 使用关键点线性模型建模人体关节，结合阴影三角形算法通过显式几何公式计算阴影角度、长度和空间位置。

Result: 在阴影真实度基准测试中达到最先进性能，特别是在复杂人体姿态下表现优异，并能有效泛化到多方向重光照场景。

Conclusion: 该方法在阴影生成方面显著提升了真实感和几何精度，为图像合成提供了更逼真的阴影效果。
...
### [50] [Distributed Zero-Shot Learning for Visual Recognition](https://arxiv.org/abs/2511.08170)
*Zhi Chen,Yadan Luo,Zi Huang,Jingjing Li,Sen Wang,Xin Yu*

Main category: cs.CV

TL;DR: 提出分布式零样本学习框架DistZSL，通过跨节点属性正则化和全局属性-视觉一致性来解决分布式数据学习问题，在异构数据上实现有效的零样本学习。


<details>
  <summary>Details</summary>
Motivation: 现有零样本学习方法主要依赖集中式数据，但实际应用中数据往往分布在多个节点上且存在异构性问题，需要开发能够充分利用分布式数据学习未见类别的有效方法。

Method: 提出两个关键组件：1）跨节点属性正则化器，强制不同节点间属性特征距离相似；2）全局属性-视觉一致性，确保不同节点学习的视觉-属性映射双边一致性。

Result: 大量实验表明，DistZSL在分布式数据学习方面优于现有最先进方法，实现了优越的性能。

Conclusion: DistZSL框架通过解决数据异构性问题，能够有效利用分布式数据学习零样本模型，为分布式环境下的零样本学习提供了有效解决方案。
...
### [51] [VLMDiff: Leveraging Vision-Language Models for Multi-Class Anomaly Detection with Diffusion](https://arxiv.org/abs/2511.08173)
*Samet Hicsonmez,Abd El Rahman Shabayek,Djamila Aouada*

Main category: cs.CV

TL;DR: 提出VLMDiff框架，结合潜在扩散模型和视觉语言模型进行多类视觉异常检测，无需人工标注或额外训练，在多个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决多类真实图像中视觉异常检测的挑战，现有基于扩散的方法依赖合成噪声生成且需要逐类训练，限制了泛化能力和可扩展性。

Method: 使用预训练视觉语言模型提取图像描述作为扩散模型的额外条件，学习鲁棒的正常图像特征表示，实现多类异常检测。

Result: 在Real-IAD数据集上像素级PRO指标提升25点，在COCO-AD数据集上提升8点，优于最先进的基于扩散的方法。

Conclusion: VLMDiff框架通过结合LDM和VLM，在无监督多类视觉异常检测任务中表现出色，具有更好的泛化能力和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 传统WSVAD模型依赖显式视觉特征，缺乏可解释性。TbVAD旨在通过语言驱动的方式实现知识基础的可解释推理。

Method: 三阶段框架：1) 使用视觉语言模型将视频内容转换为细粒度描述；2) 将描述组织为四个语义槽（动作、对象、上下文、环境）构建结构化知识；3) 生成槽级解释，揭示哪些语义因素对异常决策贡献最大。

Result: 在UCF-Crime和XD-Violence两个公共基准测试中验证，文本知识推理为现实世界监控场景提供可解释且可靠的异常检测。

Conclusion: TbVAD通过纯文本域的异常检测和解释，实现了可解释和知识基础化的视频异常检测方法。
...
### [53] [WarpGAN: Warping-Guided 3D GAN Inversion with Style-Based Novel View Inpainting](https://arxiv.org/abs/2511.08178)
*Kaitao Huang,Yan Yan,Jing-Hao Xue,Hanzi Wang*

Main category: cs.CV

TL;DR: 本文提出了一种新颖的3D GAN反演方法WarpGAN，通过引入扭曲-修复策略将图像修复技术融入3D GAN反演，以解决单视角图像到3D生成中遮挡区域质量差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D GAN反演方法主要关注可见区域的重建，而遮挡区域的生成仅依赖3D GAN的生成先验，由于低比特率潜在码导致的信息丢失，生成的遮挡区域质量往往较差。

Method: 提出WarpGAN方法：1）使用3D GAN反演编码器将单视角图像投影为潜在码；2）利用3D GAN生成的深度图进行扭曲到新视角；3）开发SVINet，利用对称先验和相同潜在码下的多视角图像对应关系修复扭曲图像中的遮挡区域。
...
TL;DR: 提出了一种新的像素级质量评估框架PQA，通过像素级空间一致性来替代传统的框级IoU预测，解决了现有方法中预测框与估计GT框之间的结构耦合问题，从而更准确地评估定位质量。


<details>
  <summary>Details</summary>
Motivation: 现有的定向目标检测器通常基于框级IoU预测来评估定位质量，但这种方法存在结构耦合问题：由于预测框来源于检测器对GT框的内部估计，导致对于定位不佳的框，预测的IoU会被高估。

Method: 提出像素级质量评估框架PQA，通过测量每个像素相对于预测框和GT框的相对位置对齐程度来评估空间一致性，避免直接比较预测框和估计GT框，并引入新的集成度量将像素级空间一致性聚合成统一的质量分数。

Result: 在HRSC2016和DOTA数据集上的大量实验表明，PQA可以无缝集成到各种定向目标检测器中，持续提升性能（例如在Rotated RetinaNet上AP50:95提升+5.96%，在STD上提升+2.32%）。

Conclusion: PQA框架通过像素级空间一致性评估有效解决了框级IoU预测的结构耦合问题，能够更准确地近似实际定位质量，显著提升定向目标检测器的性能。
...
TL;DR: UI2Code⁰是一个通过多阶段训练构建的视觉语言模型，采用交互式UI到代码范式，统一了UI生成、编辑和优化三种能力，在多个基准测试中达到开源模型最优性能。


<details>
  <summary>Details</summary>
Motivation: 当前UI编程复杂且耗时，现有视觉语言模型在多模态编码能力和迭代视觉反馈利用方面存在不足，需要开发更符合实际工作流程的交互式UI编码方法。

Method: 采用分阶段预训练、微调和强化学习训练视觉语言模型，统一UI到代码生成、UI编辑和UI优化三种能力，并探索测试时扩展以利用多轮反馈。

Result: 在UI到代码和UI优化基准测试中，UI2Code⁰在开源模型中达到最优性能，与Claude-4-Sonnet和GPT-5等闭源领先模型性能相当。

Conclusion: 交互式UI到代码范式通过多阶段训练和测试时扩展，显著提升了UI编程的自动化水平，为实际应用提供了有效的解决方案。
...
### [56] [Modulo Video Recovery via Selective Spatiotemporal Vision Transformer](https://arxiv.org/abs/2511.07479)
*Tianyu Geng,Feng Ji,Wee Peng Tay*

Main category: cs.CV

TL;DR: 本文提出了SSViT（选择性时空视觉变换器），这是首个用于模数视频重建的深度学习框架，通过token选择策略提高效率并专注于关键区域，在模数视频恢复中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像传感器动态范围有限，模数相机通过折叠入射辐照度来解决这个问题，但需要专门的展开算法来重建底层信号。现有的HDR方法不适用于模数恢复，而Transformer能够捕捉解决折叠视频帧所需的关键全局依赖和时空关系。

Method: 提出了选择性时空视觉变换器（SSViT），采用token选择策略来提高效率并专注于最关键的区域，这是首个用于模数视频重建的深度学习框架。

Result: 实验证实SSViT能够从8位折叠视频中产生高质量重建，并在模数视频恢复中达到最先进的性能。
...
### [57] [2D Representation for Unguided Single-View 3D Super-Resolution in Real-Time](https://arxiv.org/abs/2511.08224)
...
### [58] [Laplacian Score Sharpening for Mitigating Hallucination in Diffusion Models](https://arxiv.org/abs/2511.07496)
...
Conclusion: 提出的拉普拉斯校正方法能有效减少扩散模型中的模式插值幻觉问题，在多个维度上均表现出良好效果。
...
TL;DR: 提出一种基于点云曲率自适应调节局部区域间距和大小的方法，以提高表面重建的精度和效率
...
TL;DR: 提出了一种基于语义和关系改进多模态对齐与融合的方法，通过提取多层次语义特征、学习相关语义分组以及使用可继承的交叉注意力来消除冗余视觉关系，在视觉问答和图像描述任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言微调方法通常忽略了文本上下文中强调的图像内部语义关系信息，导致次优性能。
...
Conclusion: 提出的方法能够有效改进多模态对齐和融合，基于语义和关系信息显著提升性能。
...
### [61] [Toward the Frontiers of Reliable Diffusion Sampling via Adversarial Sinkhorn Attention Guidance](https://arxiv.org/abs/2511.07499)
*Kwanyoung Kim*

Main category: cs.CV

TL;DR: 本文提出了ASAG方法，通过最优传输理论和Sinkhorn算法重新解释扩散模型中的注意力机制，在自注意力层注入对抗性成本来降低查询和键之间的像素相似度，从而改善条件和非条件样本质量。
...
Method: ASAG方法从最优传输角度重新解释注意力分数，通过Sinkhorn算法故意破坏传输成本，在自注意力层注入对抗性成本来减少查询和键之间的像素相似度。

Result: ASAG在文本到图像扩散中表现出持续改进，并在IP-Adapter和ControlNet等下游应用中增强了可控性和保真度。

Conclusion: ASAG是一种轻量级、即插即用的方法，无需模型重新训练即可提高可靠性，通过故意破坏误导性注意力对齐来改善样本质量。
...
TL;DR: NERVE是一种无需训练的开集词汇语义分割方法，通过整合全局和局部信息、利用稳定扩散模型的自注意力层邻域结构，以及引入随机游走进行亲和度优化，在7个基准测试中实现了最先进的零样本分割性能。
...
Method: 提出NERVE方法：1）利用稳定扩散模型自注意力层的邻域结构；2）引入随机游走进行亲和度优化；3）基于熵的不确定性选择最相关的注意力图；4）无需传统后处理技术。

Result: 在7个流行语义分割基准测试中实现了最先进的零样本分割性能，有效解决了任意形状物体的分割问题。

Conclusion: NERVE为开集词汇语义分割提供了一个强大的基准方法，通过整合全局局部信息和随机游走策略，在无需训练和后处理的情况下实现了优异的性能。
...
TL;DR: LayerEdit提出了一种新的多层解耦编辑框架，通过精确的对象分层分解和连贯融合，实现无冲突的对象分层编辑，解决了多对象图像编辑中的注意力纠缠问题。
...
Conclusion: LayerEdit是一种无需训练的方法，首次通过精确的对象分层分解和连贯融合，实现了无冲突的对象分层编辑，有效解决了多对象图像编辑中的注意力纠缠问题。
...
Method: 在去噪过程中联合使用VAE编码的空间特征（来自航拍RGB图像和估计高度图）和CLIP语义嵌入，确保生成结果既受场景3D结构几何约束，又保持语义一致性。

Result: 在CVUSA、CVACT和Auto Arborist三个数据集上评估，SSIM指标平均提升7.3%，能稳健处理宽窄视野，展现强泛化能力。

Conclusion: Top2Ground通过直接生成方法，在几何约束和语义一致性方面表现出色，为航拍到地面视图转换提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据压缩技术在单模态场景中成功，但在多模态场景中往往失败，因为无法保留复杂的模态间依赖关系。需要开发能够有效处理多模态数据压缩的方法。
...
Conclusion: ImageBindDC在多模态数据压缩方面实现了新的最先进性能，证明了通过精确统计对齐保持模态间依赖关系的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 在极端条件下（如光线不足、相机剧烈运动），现有语义分割方法因RGB信息严重丢失而性能下降。虽然事件模态具有高速和高动态特性，但与RGB存在天然异构性，导致特征级不匹配和融合效果不佳。

Method: 提出边缘感知语义一致性框架：1）边缘感知潜在重编码，通过重编码分布指导将事件-RGB特征对齐到统一语义空间，利用预建边缘字典作为线索转换特征分布；2）重编码整合和不确定性优化，利用重编码边缘特征和不确定性指标解决异构融合问题。

Result: 在提出的合成和真实世界事件-RGB语义分割数据集上进行实验，方法在DERS-XS数据集上比现有最优方法提升2.55% mIoU，并在空间遮挡下表现出优越的鲁棒性。

Conclusion: 该框架通过挖掘两种模态的边缘秘密实现弹性融合，有效解决了极端条件下事件-RGB异构特征的融合问题，显著提升了语义分割性能。
...
Method: 开发了SWAN（SWipeable ANnotations），一个开源、MIT许可的网页应用，支持桌面和移动平台，通过滑动手势进行图像块分类，提供实时元数据捕获和灵活的滑动手势到类别标签的映射。

Result: 在四位病理学家标注600个有丝分裂图像块的试点研究中，SWAN的成对百分比一致性为86.52%-93.68%（Cohen's Kappa = 0.61-0.80），文件夹方法的成对百分比一致性为86.98%-91.32%（Cohen's Kappa = 0.63-0.75）。参与者认为工具高度可用，并赞赏在移动设备上标注的能力。
...
TL;DR: 提出了一种机制感知的无监督通用图像融合方法MAUGIF，基于双交叉图像自编码器，根据融合任务的内在机制区分加性和乘性融合，通过双编码器捕获共享内容并隔离模态特定细节，双解码器作为特征注入器选择性整合模态特征。
...
Conclusion: MAUGIF方法通过机制感知设计，能够在不同融合任务中实现有效的信息整合，提高性能和可解释性。
...
TL;DR: SkelSplat是一种基于可微分高斯渲染的多视角3D人体姿态估计新框架，无需3D真值监督即可实现跨视角融合，在遮挡场景下表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决现有多视角方法依赖大量标注数据、泛化能力差的问题，特别是在测试场景与训练场景不同时性能下降明显。

Method: 将人体姿态建模为3D高斯骨架（每个关节一个高斯），通过可微分渲染优化，提出新颖的one-hot编码方案实现关节独立优化。

Result: 在Human3.6M和CMU数据集上优于不依赖3D真值的方法，相比基于学习的方法跨数据集误差降低达47.8%，在遮挡场景中表现出鲁棒性。
...
### [70] [NeuSpring: Neural Spring Fields for Reconstruction and Simulation of Deformable Objects from Videos](https://arxiv.org/abs/2511.08310)
...
Method: 1) 使用零阶优化的分段拓扑解决方案，高效建模多区域弹簧连接拓扑；2) 基于规范坐标的神经弹簧场，表示不同帧间的弹簧物理特性。

Result: 在真实世界数据集上的实验表明，NeuSpring在当前状态建模和未来预测方面实现了优越的重建和模拟性能，Chamfer距离分别提高了20%和25%。

Conclusion: NeuSpring通过结合弹簧质量模型和神经表示，有效解决了可变形物体物理学习中的泛化问题，为物理数字孪生提供了更好的解决方案。
...
TL;DR: 提出一种新方法来减少AI系统更新时的负翻转（negative flips），通过保持原始模型的边界同时学习改进模型，结合边界校准和双重源焦点蒸馏损失来平衡新旧类别的性能。


<details>
  <summary>Details</summary>
Motivation: 随着训练类别数量随时间增长，AI系统更新时会出现负翻转问题，即更新后的模型错误分类之前正确分类的测试样本，这会降低原始子集的性能。

Method: 提出保留原始模型边界的方法，通过引入显式的边界校准项来鼓励原始学习类别和新引入类别之间更大的相对边界，并集成双重源焦点蒸馏损失来平衡新旧数据的决策边界。

Result: 在图像分类基准测试上的广泛实验表明，该方法在保持高整体准确率的同时，持续降低了负翻转率。

Conclusion: 该方法有效缓解了模型更新时的负翻转问题，在保持高准确率的同时显著减少了不一致性。
...
TL;DR: 本研究比较了三种乳腺X线摄影纵向风险建模中的对齐策略：基于图像的配准、特征级对齐和隐式对齐方法。结果表明基于图像的配准在所有指标上表现最佳，能提供更准确的时间一致性预测和解剖学合理的形变场。


<details>
  <summary>Details</summary>
Motivation: 在乳腺X线摄影筛查中，利用深度学习风险模型可以个性化筛查间隔，但准确的时间点间空间对齐是关键挑战。错位会掩盖有意义的组织变化并降低模型性能。

Method: 研究评估了三种对齐策略：基于图像的配准、特征级对齐（有/无正则化）和隐式对齐方法。使用两个大规模乳腺X线摄影数据集，评估预测准确性、精确度、召回率和形变场质量等关键指标。

Result: 基于图像的配准在所有指标上始终优于特征级和隐式方法，能生成平滑、解剖学合理的形变场。在特征空间中应用基于图像的形变场可获得最佳风险预测性能。

Conclusion: 基于图像的形变场在纵向风险建模中对于空间对齐至关重要，能提高预测准确性和鲁棒性，有潜力增强个性化筛查和高风险个体的早期干预。
...
<details>
  <summary>Details</summary>
Motivation: 水下实例分割在海洋资源勘探和生态保护中至关重要，但现有方法在复杂水下环境中的表现有限。作者发现DINO预训练模型可以作为有效的特征学习器用于水下实例分割。

Method: 提出DiveSeg框架，包含两个关键组件：(1) AquaStyle Aligner：在水下颜色风格特征嵌入DINO微调过程中，促进对水下领域的更好适应；(2) ObjectPrior Prompter：引入基于二值分割的提示，为需要对象级和实例级推理的实例分割任务提供必要的对象级先验。
...
<details>
  <summary>Details</summary>
Motivation: sEMG手势识别在人机交互中至关重要，但训练数据稀缺导致深度学习模型容易过拟合且泛化能力差。现有数据增强方法在保持生成样本忠实度和促进多样性方面存在挑战。
...
Conclusion: 该方法通过提供忠实且多样化的样本，有效解决了sEMG手势识别中的数据稀缺问题，提高了模型的识别性能和泛化能力。
...
TL;DR: VideoChain是一个新颖的多跳视频问题生成框架，能够生成需要跨多个时间分离视频片段进行推理的问题，在TVQA+数据集上构建了MVQ-60数据集，在各项生成指标上表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决现有多跳问题生成仅限于文本领域，而视频问题生成仅限于单片段零跳问题的局限性，旨在生成需要跨多个视频片段进行推理的复杂问题。

Method: 采用基于改进BART骨干的模块化架构，增强视频嵌入以捕捉文本和视觉依赖关系，利用TVQA+数据集自动构建大规模MVQ-60数据集。
...
Method: 使用分层向量量化变分自编码器学习解耦的运动-干净图像特征嵌入，部署码本在多分辨率捕获有限运动模式集合，实现从粗到细的校正。训练自回归模型学习无运动图像的先验分布，在推理时指导校正过程。
...
TL;DR: 本文质疑了旋转位置编码(RoPE)成功的关键因素，提出了非交换生成器的球面RoPE，并发现其性能不亚于严格等变的位置编码，挑战了相对位置嵌入在计算机视觉中的重要性。


<details>
  <summary>Details</summary>
Motivation: 传统认为RoPE的成功源于其位置等变性（相对位置编码），但本文质疑严格等变性是否真的在RoPE性能中起主要作用，希望探索非等变位置编码的潜力。

Method: 提出了球面RoPE(Spherical RoPE)，该方法类似于混合RoPE但假设非交换生成器，不要求严格的位置等变性。通过数学分析和实验验证其有效性。

Result: 实证研究发现球面RoPE具有与等变对应方法相当或更好的学习行为，表明相对位置嵌入在计算机视觉中可能没有传统认为的那么重要。

Conclusion: 相对位置嵌入在计算机视觉中的重要性被高估，移除必须使用相对位置编码的先入之见可以促进未来视觉位置编码的发展，实现更快速度和更好泛化能力。
...
TL;DR: 本文提出了基于文本的空中-地面行人检索任务，构建了TAG-PEDES数据集，并开发了TAG-CLIP框架来解决跨视角异构图像检索的挑战。
...
Conclusion: TAG-PR任务具有重要实践意义，提出的TAG-CLIP框架能有效处理跨视角异构图像检索问题，为相关研究提供了新的数据集和解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统雷达室内3D人体姿态估计依赖昂贵的3D关键点标注，特别是在复杂室内环境中存在遮挡、杂乱或多人的情况下。本文旨在开发仅需更易获取的3D边界框和2D关键点标签的弱监督方法。

Method: RAPTR采用两阶段姿态解码器架构：姿态解码器使用3D模板损失估计初始3D姿态，关节解码器使用2D关键点标签和3D重力损失细化姿态。利用伪3D可变形注意力增强多视图雷达特征。

Result: 在两个室内雷达数据集上评估，RAPTR显著优于现有方法，在HIBER数据集上关节位置误差减少34.3%，在MMVR数据集上减少76.9%。

Conclusion: RAPTR证明了仅使用弱监督标签（3D边界框和2D关键点）即可实现有效的雷达3D人体姿态估计，为大规模应用提供了可行的解决方案。
...
TL;DR: Anatomy-VLM是一个细粒度的视觉语言模型，通过多尺度信息整合来改进放射学疾病诊断，模拟临床医生分析图像的工作流程。


<details>
  <summary>Details</summary>
Motivation: 解决现有视觉语言模型在医学影像诊断中忽视细粒度图像细节的问题，这些细节对疾病诊断至关重要。

Method: 设计模型编码器定位关键解剖特征，用结构化知识丰富这些区域，并对齐多尺度医学信息生成临床可解释的疾病预测。

Result: Anatomy-VLM在分布内和分布外数据集上都表现出色，在下游图像分割任务中验证了其细粒度对齐能力，支持零样本解剖学解释。

Conclusion: 该模型通过模拟临床医生的分析流程，实现了专家级的临床解释能力，为医学影像诊断提供了有效的细粒度视觉语言模型解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通过获取可转移的语义嵌入来增强多模态表示学习，但现有方法通过大规模对比学习同时优化两个互补目标。作者认为这两个目标可以解耦：对输入的全面理解有助于嵌入模型通过对比学习在下游任务中实现更优性能。

Method: 提出CoMa压缩预训练阶段，作为对比学习的热身阶段。该方法只需要少量预训练数据，就能将视觉语言模型转化为有竞争力的嵌入模型。

Result: CoMa在MMEB基准上实现了可比规模视觉语言模型中的最新最优结果，同时在效率和效果上都实现了优化。

Conclusion: 通过解耦全面理解和对比学习目标，CoMa证明了只需少量预训练数据就能有效提升视觉语言模型的嵌入能力，实现了效率和性能的双重提升。
...
TL;DR: CleverBirds是一个大规模知识追踪基准，用于细粒度鸟类物种识别，包含超过40,000名参与者、1,700万道多选题和10,000多种鸟类，支持视觉知识追踪方法的发展。
...
Method: 通过公民科学平台eBird收集数据，包含长期学习模式（平均每人400个问题），分析不同参与者群体和问题类型下的知识追踪挑战。

Result: 追踪学习者的知识具有挑战性，不同形式的上下文信息提供不同程度的预测效益，CleverBirds是该类型中最大的基准之一。

Conclusion: CleverBirds基准为研究视觉专业知识在时间和个体间的发展提供了新的途径，支持视觉知识追踪方法的开发与评估。
...
<details>
  <summary>Details</summary>
Motivation: 现实应用需要结合多种视频能力的复杂迭代工作流，而现有专用AI模型只能处理孤立任务，无法满足这一需求。

Method: 采用Plan-and-Act双智能体架构：规划智能体解析用户意图并分解为结构化步骤，执行智能体通过模块化MCP工具服务器执行具体任务。支持分层多级记忆系统维持长期推理和上下文连续性。

Result: 实现了迭代和任意条件视频工作流，支持文本/图像/视频条件生成→多轮编辑→对象分割→组合合成等复杂流程。

Conclusion: UniVA和UniVA-Bench完全开源，旨在推动交互式、智能化和通用视频智能的研究，为下一代多模态AI系统提供基础。
...
Conclusion: 3D4D框架为复杂4D环境的交互式可视化提供了有效的解决方案，支持实时多模态交互。
...
<details>
  <summary>Details</summary>
Motivation: 真实场景中获取精确相机位姿具有挑战性，现有基于NeRF的方法严重依赖准确外参估计，且其隐式体积表示与广泛采用的多边形网格差异大，导致在标准3D软件中渲染和操作效率低下。

Method: 联合优化相机位姿同时学习隐式场景表示，捕获精细几何细节和逼真外观，直接重建可编辑3D网格。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习定位模型存在两个主要问题：一是对输入的所有部分给予相同关注度；二是不能很好地处理非序列数据，如瞬时信道状态信息。

Method: 采用基于注意力的Vision Transformer架构，专注于从CSI矩阵中提取角度延迟剖面特征。

Result: 在DeepMIMO数据集上室内RMSE为0.55m，室外为13.59m；在ViWi室外阻塞场景下RMSE为3.45m，比现有最优方法提升约38%。

Conclusion: 所提出的方案在定位精度和误差分布方面均显著优于其他方法，证明了基于注意力机制的ViT架构在UE定位中的有效性。
...
TL;DR: 提出了SENCA-st架构，通过共享编码器和邻域交叉注意力机制，在保持两种模态特征的同时，特别关注组织病理学中结构相似但空间转录组学中功能不同的区域，在肿瘤异质性和肿瘤微环境区域检测方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前的组织病理学-空间转录组学区域分割方法存在两种极端：要么过度依赖空间转录组学数据而仅将组织病理学特征作为辅助，要么使用普通对比学习导致组织病理学图像过于突出而丢失功能信息。这两种方法都会导致模型要么陷入空间转录组学的噪声，要么过度平滑而丢失关键信息。

Method: 提出了SENCA-st（共享编码器与邻域交叉注意力）架构，该架构保持两种模态的特征，并通过交叉注意力机制特别强调在组织病理学中结构相似但在空间转录组学中功能不同的区域。

Result: 模型在检测肿瘤异质性和肿瘤微环境区域方面表现出优越性能，超越了现有最先进方法。

Conclusion: SENCA-st架构能够有效整合组织病理学和空间转录组学数据，在保持两种模态特征的同时，特别关注结构相似但功能不同的区域，为临床重要的肿瘤异质性检测提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 虽然RAG在知识密集型任务中表现出潜力，但很少有研究将其应用于台湾历史档案。本研究旨在填补这一空白，探索RAG在历史档案处理中的应用效果。
...
Conclusion: RAG在台湾历史档案处理中具有应用价值，元数据集成策略对性能有显著影响，但需要解决幻觉和多跳查询处理等挑战。
...
TL;DR: 该调查系统综述了LLM驱动的科学创意生成方法，将其分为五类：外部知识增强、基于提示的分布引导、推理时间扩展、多智能体协作和参数级适应，并使用Boden创造力分类法和Rhodes 4P框架来分析这些方法的创意贡献。
...
Method: 将现有LLM科学创意生成方法分为五类：外部知识增强、基于提示的分布引导、推理时间扩展、多智能体协作和参数级适应，并使用Boden创造力分类法（组合性、探索性、变革性）和Rhodes 4P框架（人、过程、压力、产品）来分析这些方法。

Result: 通过将方法论进展与创造力框架对齐，该调查阐明了该领域现状，并概述了实现LLM在科学发现中可靠、系统和变革性应用的关键方向。

Conclusion: 该调查为理解LLM在科学创意生成中的能力提供了结构化框架，有助于推动LLM在科学发现中的系统应用，平衡创意性与科学严谨性。
...
<details>
  <summary>Details</summary>
Motivation: 解决术语约束机器翻译中严格约束执行可能导致的翻译质量问题，探索更灵活的术语处理方法。

Method: 两阶段架构：第一阶段使用在大型合成数据上微调的术语感知NMT模型；第二阶段使用基于提示的LLM进行后编辑，优化NMT输出并确保术语一致性。

Result: 在英语到德语、英语到西班牙语和英语到俄语的WMT 2025术语共享任务语料上评估，上下文驱动的术语处理始终产生更高质量的翻译。

Conclusion: LLM在高质量翻译中作为上下文驱动的修改器比作为生成器效果更好，揭示了术语约束翻译中的关键权衡。
...
TL;DR: Motif-2-12.7B是一个新的开放权重基础模型，通过架构创新和系统级优化提升大语言模型的效率，在有限计算预算下实现可扩展的语言理解和强大的指令泛化能力。


<details>
  <summary>Details</summary>
Motivation: 设计一个在有限计算预算下具有可扩展语言理解和强大指令泛化能力的高效大语言模型，通过架构创新和系统优化来匹敌更大模型的性能。

Method: 基于Motif-2.6B构建，集成分组差分注意力(GDA)来分离信号和噪声控制注意力路径；使用包含5.5万亿令牌的多样化数据集进行预训练，采用课程驱动的数据调度器；使用MuonClip优化器和自定义高性能内核；采用三阶段监督微调流程。

Result: 在多样化基准测试中表现出竞争力，证明经过深思熟虑的架构扩展和优化训练设计可以匹敌更大模型的能力。

Conclusion: 通过架构创新和系统级优化的结合，Motif-2-12.7B展示了在有限计算预算下实现高效大语言模型的可行性，为模型效率提升提供了新的方向。
...
TL;DR: 本文提出了LAHIS方法，通过单次前向和反向传播识别多语言能力中的注意力头重要性，发现了语言特定和语言通用头，并引入轻量级适配器改善多语言性能。


<details>
  <summary>Details</summary>
Motivation: 探索多头自注意力在支持大语言模型多语言处理中的作用，目前该领域研究不足。

Method: 提出LAHIS方法，通过单次前向和反向传播计算注意力头重要性分数；引入轻量级适配器学习软头掩码来调节注意力输出。

Result: 在Aya-23-8B、Llama-3.2-3B和Mistral-7B-v0.1模型中发现语言特定和语言通用头；仅需20个可调参数即可提升XQuAD准确率。

Conclusion: 从多头自注意力角度增强了大语言模型的可解释性和多语言能力，语言特定头能实现跨语言注意力转移，缓解脱靶语言生成问题。
...
TL;DR: 本文针对基于大语言模型的成对重排序方法存在的延迟问题，提出了一系列优化策略，实现了166倍的延迟降低，从61.36秒降至0.37秒每查询，同时保持Recall@k性能基本不变。


<details>
  <summary>Details</summary>
Motivation: 解决基于大语言模型的成对重排序方法在实际应用中面临的高延迟和计算复杂度问题，使其更适合实时部署。

Method: 采用多种优化策略：使用更小的模型、限制重排序集大小、降低精度、通过单向顺序推断减少位置偏差、限制输出标记数量。

Result: 优化后延迟从61.36秒显著降低至0.37秒每查询，延迟减少166倍，Recall@k性能仅有轻微下降。

Conclusion: 通过精心设计的优化方法，可以大幅提升基于大语言模型的重排序效率，使其在延迟敏感的实时应用中变得可行。
...
Method: 评估三个荷兰语专用LLMs与LIWC和Pattern工具在佛兰芒语情感价预测中的表现，使用约25000条自发文本响应数据。

Result: 尽管有架构进步，荷兰语调优的LLMs表现不如传统方法，Pattern工具表现最优。
...
TL;DR: 本文重新评估了轻量级的自然语言推理（NLI）评分方法，发现这种已有数十年历史的技术在长问答任务中能达到GPT-4o的准确率（89.9%），同时需要的参数数量少几个数量级。


<details>
  <summary>Details</summary>
Motivation: 评估最先进大语言模型（LLM）的答案具有挑战性：词汇指标忽略了语义细微差别，而'LLM作为评判者'的评分方法计算成本高昂。

Method: 采用现成的自然语言推理（NLI）评分，辅以简单的词汇匹配标志，并引入DIVER-QA基准数据集进行严格的人类对齐测试。

Result: NLI评分方法在长问答任务中达到89.9%的准确率，与GPT-4o相当，但计算成本显著降低。
...
TL;DR: 本文系统评估了六种广泛使用的无参考事实性度量在长文档摘要中的可靠性，发现这些原本为短文本设计的度量在长文档场景下表现不一致，特别是在信息密集的声明和语义相似内容方面可靠性下降。


<details>
  <summary>Details</summary>
Motivation: 评估抽象文本摘要的事实一致性仍然是一个重大挑战，特别是在长文档场景下，传统度量方法在处理输入长度限制和长距离依赖方面存在困难。

Method: 通过七种事实保持扰动（包括转述、简化、同义词替换、逻辑等价否定、词汇缩减、压缩和源文本插入）来探测度量鲁棒性，并分析其对检索上下文和声明信息密度的敏感性。

Result: 在涵盖科幻、法律和科学领域的三个长文档基准数据集上的结果显示，现有短文本度量对语义等价摘要产生不一致分数，对于信息密集且与源文档多个部分语义相似的声明，其可靠性下降。

Conclusion: 虽然扩展检索上下文在某些领域提高了稳定性，但没有度量能在长上下文条件下持续保持事实对齐。研究结果为改进事实性评估指明了具体方向，包括多跨度推理、上下文感知校准和在意义保持变体上训练以增强长文档摘要的鲁棒性。
...
TL;DR: 提出CAPO方法，通过基于相对奖励的动态损失缩放机制替代DPO的固定偏好对处理，增强多语言环境下对噪声和低边际比较的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法（如DPO）在英语中有效，但在多语言环境中泛化能力不足，需要更鲁棒的方法来处理多语言文本中的噪声和低边际比较问题。

Method: CAPO方法使用动态损失缩放机制，根据每个偏好对的置信度调节学习信号，替代DPO的固定处理方式。

Result: CAPO在奖励准确率上比现有基线方法至少提升16%，并通过扩大跨语言偏好响应和非偏好响应之间的差距来改善对齐效果。
...
TL;DR: 本文提出"关键虚构"方法，利用LLM的幻觉来填补因社会政治不平等造成的档案空白，为历史中的"隐藏人物"重建证据约束的叙事。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM会产生幻觉，但经过精心约束的虚构可以具有社会价值，特别是用于填补因社会政治不平等导致的历史档案空白。

Method: 使用开放式叙事填空任务，要求LLM在基于未发表文本构建的人物时间线中生成被掩盖的事件，评估不同模型在受控幻觉提示下的表现。

Result: 验证了LLM具备执行关键虚构的基础叙事理解能力，展示了受控幻觉如何支持知识生产应用而不损害历史准确性。

Conclusion: 精心约束和明确指定的LLM幻觉可以支持知识生产，避免将推测与历史准确性混为一谈。
...
TL;DR: 本文研究了上下文可预测性对在线语言产生的影响，特别是提出了新的信息论可预测性度量方法，整合了未来和过去上下文的可预测性，并通过自然语音语料库分析揭示了单词时长和替代错误的规律。


<details>
  <summary>Details</summary>
Motivation: 研究动机是深入理解语言产生中上下文可预测性的作用，特别是对过去和未来上下文可预测性的综合影响，以及这些因素如何影响单词选择和错误产生。

Method: 方法包括使用改进的度量和更强大的语言模型，提出新的信息论可预测性度量，分析自然语音语料库中的单词时长和替代错误，并在生成框架中独立建模词汇、上下文和交际因素对单词选择的影响。

Result: 研究发现，提出的概念驱动替代后向可预测性方法在两个研究中产生了类似的效果，通过细粒度分析替代错误，揭示了说话者在词汇规划中如何优先处理形式、意义和基于上下文的信息。

Conclusion: 结论是这些发现阐明了过去和未来上下文在说话者编码和选择单词中的功能作用，为上下文可预测性效应和句子规划机制之间提供了桥梁。
...
Method: 采用"量化导向、专家驱动、多重验证"原则，建立涵盖5个核心维度、54个子指标、14430个高质量问题的评价框架，包括保险理论知识、行业理解、安全合规、智能体应用和逻辑严谨性。

Result: 评估发现通用模型存在精算能力薄弱、合规适配不足等瓶颈；高质量领域专用训练在保险垂直场景中优势显著但在业务适配和合规方面存在短板；准确识别了当前大模型在保险精算、核保理赔推理、合规营销文案等专业场景中的共性瓶颈。

Conclusion: CUFEInse基准的建立不仅填补了保险领域专业评测空白，其构建理念和方法论对垂直领域大模型评测范式具有重要参考价值，为学术模型优化和产业模型选择提供权威参考，并展望了保险大模型"领域适配+推理增强"的核心发展方向。
...
### [101] [From Experience to Strategy: Empowering LLM Agents with Trainable Graph Memory](https://arxiv.org/abs/2511.07800)
*Siyu Xia,Zekun Xu,Jiajun Chai,Wentian Fan,Yan Song,Xiaohan Wang,Guojun Yin,Wei Lin,Haifeng Zhang,Jun Wang*

Main category: cs.CL

TL;DR: 本文提出了一种基于可训练多层图记忆框架的LLM智能体，通过将原始轨迹抽象为状态机中的结构化决策路径，并进一步提炼为高层战略元认知，增强了LLM在自主任务解决中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体获取经验的方式存在局限性：隐式记忆训练存在灾难性遗忘和可解释性差的问题，而显式记忆提示缺乏适应性。需要一种更好的方法来利用先验经验指导当前决策。

Method: 提出可训练的多层图记忆框架，将智能体轨迹抽象为状态机决策路径，提炼为战略元认知。采用基于强化学习的权重优化程序估计每个元认知的效用，并通过元认知提示动态集成到LLM训练循环中。

Result: 实验表明，可学习的图记忆框架能够实现稳健的泛化，提高LLM智能体的战略推理性能，并在强化学习训练中提供持续收益。

Conclusion: 该可训练图记忆框架有效增强了LLM智能体利用参数化信息的能力，提供了更好的经验利用机制，改善了推理性能和训练效果。
...
### [102] [AlignSurvey: A Comprehensive Benchmark for Human Preferences Alignment in Social Surveys](https://arxiv.org/abs/2511.07871)
*Chenxi Lin,Weikang Yuan,Zhuoren Jiang,Biao Huang,Ruitao Zhang,Jianan Ge,Yueqian Xu,Jianxing Yu*

Main category: cs.CL

TL;DR: AlignSurvey是首个系统复制和评估完整社会调查流程的基准，定义了四个与关键调查阶段对齐的任务，并提供了多层级数据集架构和评估指标，重点关注对齐保真度、一致性和公平性。


<details>
  <summary>Details</summary>
Motivation: 传统社会调查面临固定问题格式、高成本、适应性有限和跨文化等效性困难等挑战，而现有基于大语言模型的研究大多局限于结构化问题，忽视了完整调查流程，且存在训练数据偏见导致边缘群体代表性不足的风险。

Method: 提出AlignSurvey基准，定义四个任务：社会角色建模、半结构化访谈建模、态度立场建模和调查响应建模；构建多层级数据集架构，包括社会基础语料库和完整流程调查数据集；通过两阶段微调开源LLMs获得SurveyLM系列模型。

Result: 开发了包含44K+访谈对话和400K+结构化调查记录的社会基础语料库，以及专家标注的AlignSurvey-Expert数据集和两个具有全国代表性的跨文化评估调查数据集。

Conclusion: AlignSurvey为透明和负责任的社会科学研究提供了全面的基准、数据集和工具支持，有助于评估LLMs在社会调查领域的对齐性能，特别关注人口多样性。
...
### [103] [Planned Event Forecasting using Future Mentions and Related Entity Extraction in News Articles](https://arxiv.org/abs/2511.07879)
...
<details>
  <summary>Details</summary>
Motivation: 在印度等民主国家，公民表达诉求可能导致未经批准的抗议活动。预测这些事件有助于行政官员采取必要行动，而抗议活动通常会提前宣布以吸引更多参与者。

Method: 使用主题建模和word2vec过滤相关新闻文章，应用NER识别人员、组织、地点和日期等实体，进行时间标准化，并提出相关实体提取方法。

Result: 开发了一个地理独立、通用的模型来识别过滤社会动荡事件的关键特征，能够有效提取与事件相关的实体。

Conclusion: 该系统能够通过分析新闻公告预测计划中的社会动荡事件，为行政管理提供决策支持。
...
### [104] [Breaking the Adversarial Robustness-Performance Trade-off in Text Classification via Manifold Purification](https://arxiv.org/abs/2511.07888)
*Chenhao Dang,Jing Ma*

Main category: cs.CL

TL;DR: MC^2F是一种用于文本分类的双模块系统，通过在编码器嵌入流形上建模干净样本分布来解决对抗攻击鲁棒性与干净数据性能之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 解决文本分类中增强模型对抗攻击鲁棒性通常会降低在干净数据上性能的持久挑战。

Method: 提出Manifold-Correcting Causal Flow (MC^2F)系统，包含分层黎曼连续归一化流(SR-CNF)学习干净数据流形密度，以及测地线净化求解器将对抗点投影回学习到的流形。
...
### [105] [Last Layer Logits to Logic: Empowering LLMs with Logic-Consistent Structured Knowledge Reasoning](https://arxiv.org/abs/2511.07910)
*Songze Li,Zhiqiang Liu,Zhaoyan Gong,Xiaoke Guo,Zhengke Gui,Huajun Chen,Wen Zhang*

Main category: cs.CL

TL;DR: 本文提出Logits-to-Logic框架，通过logits强化和过滤模块解决LLM在结构化知识推理中的逻辑漂移问题，显著提升了KGQA任务的性能。


<details>
  <summary>Details</summary>
Motivation: LLM在非结构化文本预训练后能理解自然语言逻辑，但在结构化知识推理中由于表示差异导致逻辑一致性差，现有方法仅提供输入级指导无法从根本上解决逻辑漂移问题。

Method: 提出Logits-to-Logic框架，针对自回归生成过程中的logits输出，包含logits强化和logits过滤两个核心模块来修正LLM输出中的逻辑缺陷。

Result: 大量实验表明，该方法显著提升了LLM在结构化知识推理中的逻辑一致性，在多个KGQA基准测试中达到了最先进的性能。
...
TL;DR: WorkBench是首个统一评估套件，涵盖6个工作相关任务作为排序问题，发现显著的正向跨任务迁移，并据此构建了UWE模型，在零样本排序性能上优于通用嵌入模型。


<details>
  <summary>Details</summary>
Motivation: 工作领域自然语言处理需求增长，但工作相关任务具有长尾分布、极端多标签和稀疏数据等复杂性。通用嵌入模型在工作领域的性能尚不明确，且领域进展主要关注单个任务。

Method: 引入WorkBench评估套件，基于跨任务迁移洞察构建任务特定的二分图，使用真实数据并通过基础增强进行合成丰富。开发UWE任务无关双编码器，利用多对多InfoNCE目标和任务无关软延迟交互。

Result: UWE在工作领域未见目标空间上展示零样本排序性能，通过缓存任务目标空间嵌入实现低延迟推理，在宏平均MAP和RP@10指标上显著优于通用嵌入模型。

Conclusion: UWE模型通过利用工作领域特定数据结构和训练方法，在工作相关任务上实现了优于通用嵌入模型的性能，证明了领域特定方法的价值。
...
TL;DR: NOTAM-Evolve是一个自演化框架，通过结合知识图谱增强检索和闭环学习过程，使大语言模型能够自主掌握复杂的NOTAM（航行通告）解析任务，在结构化NOTAM解析上实现了30.4%的绝对准确率提升。
...
Method: 提出NOTAM-Evolve自演化框架，结合知识图谱增强检索模块进行数据接地，并引入闭环学习过程，使LLM能够从自身输出中逐步改进，减少对大量人工标注推理轨迹的需求。
...
Method: 提出自校正蒸馏（SCD）方法，包括：1）错误提示机制（EPM）在推理时检测错误并提供定制化错误信息；2）两阶段蒸馏策略，将大模型的查询生成和错误校正能力迁移到小模型。

Result: 在5个基准测试和3种结构化数据类型上的实验表明，SCD在小规模语言模型（8B）上实现了最佳性能和优越的泛化能力，在某些数据集上接近GPT4的性能。配备EPM的大模型在大多数数据集上超越了最先进结果。

Conclusion: SCD方法有效提升了小规模语言模型在结构化数据问答中的能力，同时EPM机制也能显著提升大模型的性能，为结构化数据问答领域提供了有效的解决方案。
...
TL;DR: 该研究提出使用学习压缩令牌来压缩LLM上下文，可在2x到8x压缩率下保持性能，在30亿参数LLaMA模型上达到与替代方法相当的结果但压缩率更高。


<details>
  <summary>Details</summary>
Motivation: 为了减少处理长序列时的内存和计算需求，研究LLM上下文压缩方法。

Method: 通过微调预训练LLM，使用学习压缩令牌来压缩上下文，压缩因子为2x到8x。

Result: 在短上下文和长上下文基准测试中，压缩后性能没有显著下降；在30亿参数LLaMA模型上，与替代压缩技术结果相当但压缩率更高。

Conclusion: 预训练LLM可以通过微调实现有效的上下文压缩，在保持性能的同时显著减少计算资源需求。
...
TL;DR: 本文提出了一种新的关系作为先验（RelPrior）范式，用于解决大型语言模型在文档级关系抽取中的性能差距问题。该方法通过利用二元关系作为先验来过滤无关实体对，并使用预定义关系作为先验进行三元组抽取，从而减少噪声干扰并避免严格预定义关系标签导致的误判。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在文档级关系抽取中存在性能差距，主要原因是：（1）大量无关实体对引入噪声干扰关系预测；（2）超出预定义集合的关系标签仍被视为预测错误。需要解决这些挑战以提升模型性能。

Method: 提出RelPrior范式：首先利用二元关系作为先验提取和确定实体是否相关，过滤无关实体对；然后利用预定义关系作为先验进行三元组抽取，避免直接预测关系导致的误判。

Result: 在两个基准测试上的广泛实验表明，RelPrior实现了最先进的性能，超越了现有的基于LLM的方法。

Conclusion: RelPrior范式通过将关系作为先验知识，有效解决了文档级关系抽取中的噪声干扰和关系标签限制问题，显著提升了大型语言模型在该任务上的表现。
...
TL;DR: 本文比较了指令调优和上下文提示的大型语言模型与专门针对梵文诗歌转散文任务的编码器-解码器模型。研究发现，领域特定的ByT5-Sanskrit模型显著优于所有基于指令的LLM方法。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型在低资源、形态丰富的语言（如梵文）上的表现，特别是针对具有挑战性的诗歌转散文任务，该任务需要多步推理。
...
Conclusion: 对于梵文诗歌转散文这样的复杂任务，领域特定的序列到序列模型比通用大型语言模型表现更好，但提示策略在缺乏训练数据时仍是有用的替代方案。
...
TL;DR: 分析BabyLM语料库和CHILDES中年龄组的句法特性，发现CHILDES在年龄上的句法区分不明显，但训练数据的句法知识有助于解释模型在语言任务上的表现。探索了发展性和其他认知启发的课程学习方法，发现某些课程对阅读任务有帮助，但主要性能提升来自使用可句法分类的数据子集而非完整噪声语料库。


<details>
  <summary>Details</summary>
Motivation: 研究儿童语言发展语料库的句法特性，探索如何利用句法知识来理解和改进语言模型的性能，特别是通过课程学习方法来优化训练过程。

Method: 分析BabyLM语料库和CHILDES的句法属性，探索发展性课程学习和多种认知启发的课程学习方法，比较使用完整语料库与可句法分类数据子集的效果。

Result: CHILDES在年龄上的句法区分不明显；句法知识有助于解释模型性能；某些课程学习方法对阅读任务有帮助；主要性能提升来自使用可句法分类的数据子集而非完整噪声语料库。

Conclusion: 句法特性分析为理解语言模型性能提供了有价值的信息，课程学习方法特别是使用可句法分类的数据子集能够有效提升模型性能，而非依赖完整的噪声语料库。
...
Method: 实现了一个基于SciBERT模型的多任务transformer系统，通过对天文学语料库进行微调，在训练数据中随机采样段落，并在推理时使用多数投票。
...
<details>
  <summary>Details</summary>
Motivation: 随着教师越来越多地在教育实践中使用生成式AI，需要可靠的方法来为教学目的基准测试大型语言模型的偏见问题。

Method: 使用600篇真实学生论文构建控制反事实，通过词汇替换和提示中的作者背景两个维度操纵性别线索，分析6个代表性LLM的响应差异。

Result: 所有模型对隐式性别操纵都表现出不对称语义偏移，只有GPT和Llama模型对显式性别线索敏感，定性分析显示男性线索下反馈更自主支持，女性线索下反馈更控制性。

Conclusion: 即使最先进的LLMs也存在性别偏见，需要建立公平性审计标准、反事实评估报告标准，并提供提示设计和部署的实践指导以确保公平反馈。
...
TL;DR: 本文提出了VocalBench-zh，一个针对普通话环境的语音到语音评估套件，包含10个子集和超过10K个高质量实例，覆盖12个用户导向特征。通过对14个主流模型的评估，揭示了当前方法的共同挑战。
...
Method: 开发了VocalBench-zh评估套件，包含10个精心设计的子集和超过10K个高质量实例，涵盖12个用户导向特征，专门针对普通话环境。
...
TL;DR: 本文提出了一种通过提示调优进行NL到SQL错误校正的方法，结合生成式预训练LLMs和RAG技术，显著提升了自然语言查询到SQL语句转换的准确性。
...
Method: 提出集成错误校正机制的新框架，包括错误类型诊断、原因识别、修复指令提供和SQL查询修正。结合嵌入微调和RAG技术，利用外部知识库提升准确性和透明度。

Result: 通过综合实验验证，该框架相比现有基线方法实现了12%的准确率提升。

Conclusion: 该框架具有革新当代数据驱动环境中数据访问和处理方式的潜力，展示了在NL到SQL转换任务中的显著性能改进。
...
TL;DR: ParliaBench是一个专门用于议会演讲生成的基准测试，通过构建英国议会演讲数据集，结合计算指标和LLM评估，从语言质量、语义连贯性和政治真实性三个维度评估模型性能。
...
Method: 构建英国议会演讲数据集，提出结合计算指标和LLM评估的框架，引入两个新的基于嵌入的指标（政治光谱对齐和党派对齐），对五个大型语言模型进行微调并生成28k个演讲。

Result: 微调在大多数指标上产生统计显著改进，新提出的指标在政治维度上表现出强大的区分能力。

Conclusion: ParliaBench为议会演讲生成提供了有效的评估框架，微调能显著提升模型性能，新指标能有效量化政治意识形态定位。
...
TL;DR: 该研究探索视觉大语言模型理解科学论文中表格结构的能力，特别是推断复杂分层表格结构的能力，使用PubTables-1M数据集中的复杂分层表格作为基准测试。


<details>
  <summary>Details</summary>
Motivation: 研究VLLMs是否能够理解表格的层次结构，而不需要额外的处理，为将结构化数据理解集成到通用VLLMs中提供指导。

Method: 使用PubTables-1M数据集中的复杂分层表格作为基准，采用多种提示工程策略测试模型理解能力，评估多个最先进的开放权重VLLMs，并与人类表现进行比较。

Result: 实验表明，未专门设计用于理解表格结构的通用VLLMs能够完成此任务，揭示了VLLMs处理复杂表格的潜力和局限性。

Conclusion: 该研究为未来在通用VLLMs中集成结构化数据理解提供了见解和指导，证明了VLLMs在理解表格结构方面的基本能力。
...
TL;DR: 提出了一种基于多智能体框架的LLM响应精炼方法，通过角色分工的动态通信策略来提升对话质量，在事实性、个性化和连贯性方面显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在对话系统中缺乏个性化和特定知识的问题，避免依赖用户检测错误，通过精炼响应来提升对话质量。

Method: 采用多智能体框架，每个智能体负责特定方面（事实性、个性化、连贯性）的审查和精炼，引入动态通信策略自适应选择相关智能体。

Result: 在具有挑战性的对话数据集上验证，该方法在涉及知识或用户角色的任务中显著优于相关基线。

Conclusion: 多智能体框架结合动态通信策略能有效提升LLM对话响应的质量，特别是在需要个性化和知识整合的场景中。
...
### [121] [DPRM: A Dual Implicit Process Reward Model in Multi-Hop Question Answering](https://arxiv.org/abs/2511.08364)
*Xinyi Wang,Yiping Song,Zhiliang Tian,Bo Liu,Tingjin Luo,Minlie Huang*

Main category: cs.CL

TL;DR: 本文提出DPRM（双重隐式过程奖励模型），通过训练两个隐式过程奖励模型分别处理CoT和KG推理，解决多跳问答任务中现有方法无法处理图结构约束和推理路径不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 在多跳问答任务中，现有隐式过程奖励模型仅适用于纯文本场景，无法处理知识图谱的图结构约束，也无法捕捉思维链和知识图谱路径之间的潜在不一致性。

Method: 提出DPRM模型，包含KG-PRM和CoT-PRM两个隐式过程奖励模型，通过奖励参数化从结果信号推导步骤级奖励，无需额外显式标注。KG-PRM使用偏好对学习知识图谱的结构约束，并引入一致性约束使两个PRM相互验证和协同优化推理路径。
...
### [122] [The Dynamic Articulatory Model DYNARTmo: Dynamic Movement Generation and Speech Gestures](https://arxiv.org/abs/2511.08372)
*Bernd J. Kröger*
...
Method: 使用手势库结构和手势分数中的手势协调，将手势转换为控制DYNARTmo声道模型的连续发音器轨迹。

Result: 成功实现了基于语音手势概念的动态发音模型，能够生成连续的发音器运动。
...
### [123] [TurkEmbed: Turkish Embedding Model on NLI & STS Tasks](https://arxiv.org/abs/2511.08376)
...
Method: 使用多样化数据集结合先进训练技术，包括嵌套表示学习，以生成更鲁棒和准确的嵌入表示，并适应资源受限环境。

Result: 在土耳其STS-b-TR数据集上的评估显示语义相似性任务显著改进，在All-NLI-TR和STS-b-TR基准测试中超越当前最优模型Emrecan，提升1-4%。
...
TL;DR: 提出PCRLLM框架，通过单步推理约束LLM推理过程，保持自然语言表达的同时支持形式化验证，解决LLM逻辑一致性问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在推理时缺乏逻辑一致性，往往直接将前提映射到结论而不遵循明确的推理规则，存在可信度问题。

Method: 提出PCRLLM框架，约束推理为单步过程，明确指定前提、规则和结论，支持形式化验证和多LLM协作。

Result: 该框架能够缓解可信度担忧，支持黑盒环境下的链级验证，并促进系统化的多LLM协作。

Conclusion: PCRLLM结合了自然语言表达能力和形式化严谨性，为大规模步级推理数据生成提供了基准方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有社交机器人检测器在基准测试中表现良好，但在多样化真实场景中鲁棒性有限，主要由于不清楚的真实标签和多样的误导线索，特别是捷径学习（模型依赖虚假相关性而非因果任务相关特征）的影响未得到足够关注。

Method: 设计了一系列捷径场景，通过构建用户标签与表面文本线索之间的虚假关联来评估模型鲁棒性；提出了基于大语言模型的反事实数据增强缓解策略，从数据和模型两个角度在三个层面解决问题：个体用户文本数据分布、整体数据集分布以及模型提取因果信息的能力。

Result: 结果显示不相关特征分布的偏移显著降低了社交机器人检测器性能，基线模型的平均相对准确率下降了32%；提出的缓解策略在捷径场景下实现了平均相对性能提升56%。
...
TL;DR: SPEAR-MM框架通过选择性参数评估和模型融合，在金融领域适应中保留LLM的通用推理能力，避免灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 金融领域适应的LLM经常出现灾难性遗忘，丢失对客户交互和复杂金融分析至关重要的通用推理能力。

Method: 通过后验分析近似层对基准测试的影响，然后通过球面插值融合选择性冻结或恢复transformer层。

Result: 在LLaMA-3.1-8B上，SPEAR-MM实现91.2%的通用能力保留（标准持续预训练为69.7%），同时保持94%的领域适应增益，计算成本降低90%。

Conclusion: 该方法为资源受限的金融机构提供了可解释的权衡控制和显著的计算成本节约。
...
<details>
  <summary>Details</summary>
Motivation: 当前RAG方法主要关注少量相关文档的查询，无法有效处理需要从大量文档中收集信息并进行推理的聚合查询。

Method: 在数据摄取时构建语料库的结构化表示，在推理时将自然语言查询转换为对该表示的形式化查询。

Result: 在新提出的HOTELS和WORLD CUP数据集以及公共基准测试中，S-RAG显著优于传统RAG系统和长上下文LLM。

Conclusion: S-RAG为聚合查询提供了有效的解决方案，填补了当前RAG方法的空白，并通过新数据集推动了该领域的研究。
...
TL;DR: 本文介绍了Bangla-SGP数据集，包含1000个人工标注的句子-手势对和3000个通过基于规则的RAG管道生成的合成对，用于孟加拉手语翻译任务。作者微调了多个基于Transformer的模型，并在BLEU分数上评估了性能。
...
Method: 构建了Bangla-SGP数据集，包含人工标注和基于语法形态规则的合成生成。使用基于规则的RAG管道进行数据增强，并微调了mBart50、Google mT5、GPT4.1-nano等Transformer模型。

Result: 通过BLEU分数评估模型性能，并在Bangla-SGP数据集和RWTH-PHOENIX-2014T基准上比较了模型的手势翻译一致性。

Conclusion: 该工作填补了孟加拉手语句子级翻译数据集的空白，为低资源手语翻译任务提供了新的数据集和基准评估方法。
...
TL;DR: 本文首次系统研究了CoT可监控性的挑战与潜力，围绕LRM是否在思维链中真实表达决策因素以及基于CoT的监控器可靠性两个核心问题展开，提出了MoME监控范式。


<details>
  <summary>Details</summary>
Motivation: LRM在复杂任务中表现出色，其详细推理轨迹为AI安全监控提供了新机会，但面临模型不真实表达内部决策和监控器可能被欺骗两大挑战。

Method: 通过数学、科学和伦理领域的实证证据和相关分析，研究言语化质量、监控可靠性与LLM性能的关系，并探讨不同CoT干预方法对监控效果的影响。

Result: 提供了言语化质量、监控可靠性与LLM性能之间的相关分析证据，并提出了MoME新范式，让LLM通过其他模型的CoT监控其不当行为。

Conclusion: CoT监控性在AI安全中具有重要潜力，但需要解决言语化真实性和监控可靠性问题，MoME范式为此提供了新的解决方案。
...
TL;DR: 本文研究了大型语言模型在角色扮演情境下的道德响应，提出了道德易感性和道德鲁棒性两个量化指标，发现模型家族对道德鲁棒性影响最大，而模型规模对道德易感性有显著影响。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在社交环境中应用增多，需要分析它们如何表达和改变道德判断，特别是在角色扮演情境下。

Method: 使用道德基础问卷(MFQ)建立基准，通过角色扮演提示量化道德易感性和道德鲁棒性，分析不同模型家族和规模的影响。

Result: Claude家族道德鲁棒性最强，模型家族是道德鲁棒性的主要影响因素，模型规模对道德易感性有显著影响，且鲁棒性与易感性呈正相关。

Conclusion: 角色扮演调节显著影响LLMs的道德行为，为理解模型在社交情境中的道德表现提供了系统性视角。
...
TL;DR: 语言模型可以通过微调学习描述自己的内部计算过程，包括特征编码信息、激活因果结构和输入令牌影响，且自我解释效果优于其他模型解释。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型是否能利用其内部访问特权来生成对其行为的忠实描述，探索自我解释是否比外部模型解释更有效。

Method: 使用现有可解释性技术作为真实标签，微调语言模型生成自然语言描述，涵盖特征信息编码、激活因果结构和输入令牌影响三个方面。

Result: 仅用数万个解释样本训练的解释模型在新查询上表现出非平凡泛化能力，自我解释效果优于其他模型解释，即使其他模型能力更强。
...
### [133] [REFLEX: Reference-Free Evaluation of Log Summarization via Large Language Model Judgment](https://arxiv.org/abs/2511.07458)
*Priyanka Mudgal*

Main category: cs.CL

TL;DR: REFLEX是一种基于大语言模型判断的无参考日志摘要评估指标，无需黄金标准参考或人工标注，通过评估相关性、信息量和连贯性等维度来评估摘要质量。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏高质量的参考摘要，以及现有指标如ROUGE和BLEU依赖表面词汇重叠的局限性，评估日志摘要系统具有挑战性。
...
### [134] [Beyond Correctness: Confidence-Aware Reward Modeling for Enhancing Large Language Model Reasoning](https://arxiv.org/abs/2511.07483)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于规则的强化学习奖励机制在小型模型上经常导致推理链质量差或推理过程与最终答案不一致的问题，限制了资源有限组织对小型模型进行直接强化学习训练的能力。

Method: 开发了一种新颖的基于置信度的奖励模型，不仅惩罚错误答案，还惩罚低置信度的正确答案，通过静态评估、Best-of-N推理测试和基于PPO的强化学习训练进行验证。

Result: 该方法在多个STEM基准测试中优于几种最先进的开源奖励模型，证明了其有效性。

Conclusion: 提出的基于置信度的奖励模型能够有效提升STEM推理能力，为资源有限组织训练小型模型提供了可行的解决方案，相关代码和模型已开源。
...
### [135] [Beyond Fact Retrieval: Episodic Memory for RAG with Generative Semantic Workspaces](https://arxiv.org/abs/2511.07587)
...
<details>
  <summary>Details</summary>
Motivation: LLM面临长上下文推理的基本挑战：许多文档超出其有限上下文窗口，且性能随序列长度下降，需要外部记忆框架增强。现有解决方案针对基于事实的检索，无法构建时空锚定的叙事表示来跟踪实体在事件中的演变。

Method: 提出生成语义工作空间（GSW）框架，包含操作器（将观察映射到中间语义结构）和协调器（将这些结构整合到持久工作空间，强制执行时间、空间和逻辑一致性）。

Result: 在包含10万到100万token语料库的Episodic Memory Benchmark上，GSW比现有RAG基线性能提升高达20%，查询时上下文token减少51%，显著降低推理时间成本。

Conclusion: GSW为赋予LLM类人情景记忆提供了具体蓝图，为能够进行长视野推理的更强大智能体铺平道路。
...
### [136] [AI-Driven Contribution Evaluation and Conflict Resolution: A Framework & Design for Group Workload Investigation](https://arxiv.org/abs/2511.07667)
*Jakub Slapek,Mir Seyedebrahimi,Yang Jianhua*

Main category: cs.AI

TL;DR: 提出一个AI增强的框架来解决团队中个体贡献评估的公平性问题，通过整合多种工作产物和沟通记录，使用客观指标和不平等度量来识别冲突，并利用LLM生成可解释的建议判断。


<details>
  <summary>Details</summary>
Motivation: 团队中个体贡献的公平评估存在挑战，冲突和工作量差异导致不公平绩效评估，需要手动干预，这是一个成本高且困难的过程。现有工具在冲突解决方法和AI集成方面存在空白。

Method: 提出一个框架和实现设计，将异构工作产物（代码、文本、媒体）、沟通记录（聊天、邮件）、协调记录（会议日志、任务）、同行评估和上下文信息组织到三个维度和九个基准中：贡献、互动和角色。客观度量被标准化、按维度聚合，并与不平等度量（基尼指数）配对以识别冲突标记。使用大型语言模型架构对这些度量进行验证和上下文分析，生成可解释和透明的建议判断。

Result: 该框架能够整合多种数据源，通过客观指标和不平等度量识别团队中的潜在冲突，并利用AI生成可解释的评估建议。

Conclusion: 该AI增强工具框架在当前法规和机构政策下具有可行性，能够帮助解决团队贡献评估中的公平性问题，但需要关注实际分析（情感分析、任务保真度、字数/行数等）、偏见防护、局限性和实际挑战。
...
### [137] [Making LLMs Reliable When It Matters Most: A Five-Layer Architecture for High-Stakes Decisions](https://arxiv.org/abs/2511.07669)
*Alejandro R. Jadad*

Main category: cs.AI

TL;DR: 该研究提出了一个框架，通过7阶段校准序列和5层保护架构，使人类-AI团队在高风险战略决策中实现认知伙伴关系，防止可避免的遗憾。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在可验证领域表现出色，但在具有不确定结果的高风险战略决策中可靠性不足，这威胁到估值和投资可持续性。

Method: 对7个前沿级LLM和3个市场风险案例进行系统定性评估，开发了包含4阶段初始化过程和7阶段校准序列的5层保护架构。

Result: 发现伙伴关系状态可通过有序校准实现但需要维护协议；当架构漂移和上下文耗尽同时发生时可靠性下降；解散纪律可防止追求错误方向。
...
### [138] [Towards AI-Assisted Generation of Military Training Scenarios](https://arxiv.org/abs/2511.07690)
*Soham Hans,Volkan Ustun,Benjamin Nye,James Sterrett,Matthew Green*

Main category: cs.AI

TL;DR: 本文提出了一种基于大型语言模型的多智能体多模态推理框架，用于自动生成军事训练场景中的关键文档（如作战命令OPORDs），通过分层分解任务和专用智能体协作，克服了传统AI工具在复杂场景生成中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统模拟训练中的复杂场景创建过程耗时耗力，而前LLM时代的AI工具难以生成足够复杂或适应性强的场景，需要一种更高效的自动化解决方案。

Method: 采用多智能体多模态推理框架，将场景生成分解为层次化子问题，定义AI工具的三种角色：生成选项供选择、生成候选产品供审批、完全自动生成文本。使用专用LLM智能体处理不同子问题，整合文本和视觉信息，保持逻辑一致性。

Result: 通过概念验证生成了作战命令的机动方案和移动部分，同时估计地图位置和移动路径，证明了框架的可行性和准确性。
...
### [139] [WaterMod: Modular Token-Rank Partitioning for Probability-Balanced LLM Watermarking](https://arxiv.org/abs/2511.07863)
*Shinwoo Park,Hyejin Park,Hyeseon Ahn,Yo-Sub Han*

Main category: cs.AI

TL;DR: WaterMod是一种新的文本水印方法，通过概率感知的模块化规则在保持生成质量的同时嵌入可检测的水印信号，支持零位和多位设置。


<details>
  <summary>Details</summary>
Motivation: 现有基于logit的水印方法通过随机分割词汇表来嵌入水印，但这种随机分割可能排除最高概率的token，从而影响生成流畅性。需要一种既能保证水印检测性能又不损害生成质量的方法。

Method: 首先按模型概率降序排列词汇表，然后通过余数rank mod k进行分区，将语义相似的相邻token分配到不同类别。在零位设置(k=2)中使用熵自适应门选择奇偶类，在多位设置(k>2)中根据当前有效载荷数字选择颜色类别。

Result: 实验结果表明，WaterMod在零位和多位设置下都能保持强大的水印检测性能，同时在自然语言生成、数学推理和代码合成等多种任务中维持生成质量。
...
### [140] [Toward Robust EEG-based Intention Decoding during Misarticulated Speech in Aphasia](https://arxiv.org/abs/2511.07895)
*Ha-Na Jo,Jung-Sun Lee,Eunyeong Ko*

Main category: cs.AI

TL;DR: 本研究针对失语症患者开发了基于EEG的通信支持系统，通过分析正确发音和错误发音时的脑电信号差异，构建了一个软多任务学习框架来解码患者的意图，即使在发音错误的情况下也能有效识别。


<details>
  <summary>Details</summary>
Motivation: 失语症严重限制了患者的语言表达能力，导致频繁发音错误。尽管脑机接口技术不断发展，但针对失语症患者的EEG通信支持系统研究相对较少。

Method: 招募一名表达性失语症患者进行韩语自动语音任务，记录EEG信号并标记正确/错误发音试验。使用频谱分析识别神经激活模式差异，开发基于最大均值差异正则化的软多任务学习框架，重点关注delta特征。

Result: 错误发音试验显示广泛通道的过度delta功率和额叶区域增加的theta-alpha活动。所提模型在正确试验中达到58.6%准确率，错误试验中达到45.5%准确率，比基线方法在错误试验上提升超过45%。

Conclusion: 研究证明了基于EEG的辅助系统在失语症患者现实世界不完美语音条件下的可行性，能够有效支持意图解码。
...
### [141] [SparseRM: A Lightweight Preference Modeling with Sparse Autoencoder](https://arxiv.org/abs/2511.07896)
...
Method: 使用稀疏自编码器分解LLM表示为可解释的偏好相关方向，通过投影计算对齐分数，简单奖励头聚合分数预测偏好得分。
...
### [142] [Data Descriptions from Large Language Models with Influence Estimation](https://arxiv.org/abs/2511.07897)
*Chaeri Kim,Jaeyeon Bae,Taehwan Kim*

Main category: cs.AI

TL;DR: 本文提出了一种利用大语言模型生成数据文本描述的新方法，通过整合外部知识库和影响力估计来筛选最有信息量的描述，并在跨模态迁移分类任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型的行为理解仍是一个黑盒问题，现有XAI方法主要关注模型预测的解释，而本文希望从数据角度理解深度学习模型训练过程，通过人类易于理解的语言媒介来解释数据。

Method: 提出一个生成文本描述的流水线，利用大语言模型整合外部知识库，引入影响力估计和CLIP分数来选择最有信息量的文本描述，并基于跨模态可迁移性提出跨模态迁移分类基准任务。

Result: 在零样本设置实验中，本文生成的文本描述比其他基线描述更有效，成功提升了仅使用图像训练的模型在九个图像分类数据集上的性能，GPT-4o评估进一步支持了这些结果。
...
### [143] [DANS-KGC: Diffusion Based Adaptive Negative Sampling for Knowledge Graph Completion](https://arxiv.org/abs/2511.07901)
...
Conclusion: DANS-KGC通过创新的负采样策略显著提升了知识图谱表示学习的性能，特别是在处理不同难度样本方面表现出色。
...
### [144] [Neurophysiological Characteristics of Adaptive Reasoning for Creative Problem-Solving Strategy](https://arxiv.org/abs/2511.07912)
*Jun-Young Kim,Young-Seok Kweon,Gi-Hwan Shin,Seong-Whan Lee*

Main category: cs.AI

TL;DR: 本研究通过脑电图和卡片分类任务研究人类适应性推理的神经机制，发现delta-theta-alpha振荡协调参与规则推断和注意力稳定，而多模态大语言模型仅能进行短期反馈调整，缺乏真正的适应性推理能力。
...
Method: 使用卡片分类范式结合脑电图技术，分析刺激锁定和反馈锁定的神经活动，同时比较人类表现与多模态大语言模型的性能。

Result: 研究发现协调的delta-theta-alpha动态：早期delta-theta活动反映探索性监测和规则推断，而枕叶alpha参与表明成功规则识别后的注意力确认稳定。多模态大语言模型仅表现出短期反馈驱动调整，缺乏层次规则抽象或真正的适应性推理。

Conclusion: 这些发现识别了人类适应性推理的神经特征，并强调了需要开发融入振荡反馈协调的脑启发人工智能，以实现真正的上下文敏感适应。
...
### [145] [Lightweight Diffusion-based Framework for Online Imagined Speech Decoding in Aphasia](https://arxiv.org/abs/2511.07920)
*Eunyeong Ko,Soowon Kim,Ha-Na Jo*

Main category: cs.AI

TL;DR: 基于扩散模型的实时想象语音分类神经解码框架，针对失语症患者优化，使用轻量级条件扩散编码器和卷积分类器处理EEG数据，在线试验达到65% top-1准确率。


<details>
  <summary>Details</summary>
Motivation: 为严重表达性语言障碍患者开发临床通信支持系统，将想象语音脑机接口转化为实际临床应用，解决环境变异性和有限校准数据下的稳定性能问题。

Method: 集成轻量级条件扩散编码器和卷积分类器，使用双标准早停策略实现快速收敛，采用dropout正则化和分组时间卷积确保泛化稳定性，在线处理两秒滑动窗口的连续EEG流。

Result: 在20次在线试验中达到65% top-1和70% top-2准确率，优于离线评估的50% top-1准确率，在环境变异性和最小预处理条件下保持可靠性能。
...
### [146] [Computational Blueprints: Generating Isomorphic Mathematics Problems with Large Language Models](https://arxiv.org/abs/2511.07932)
*Jeong-Hoon Kim,Jinwoo Nam,Geunsik Jo*

Main category: cs.AI

TL;DR: 该论文提出了同构数学问题生成(IMPG)任务，开发了基于LLM的CBIT框架，通过元级生成和模板化选择变体，在保持数学正确性和结构一致性的同时降低生成成本。


<details>
  <summary>Details</summary>
Motivation: 个性化数学教育快速发展，需要大量相似练习题，但现有数学问题生成研究主要关注数据增强而非直接教育部署，需要填补这一空白。
...
Conclusion: CBIT框架能够高效生成高质量的同构数学问题，满足个性化数学教育的需求，具有实际部署价值。
...
### [147] [Toward Practical BCI: A Real-time Wireless Imagined Speech EEG Decoding System](https://arxiv.org/abs/2511.07936)
*Ji-Ha Park,Heon-Gyu Kwak,Gi-Hwan Shin,Yoo-In Jeon,Sun-Min Park,Ji-Yeon Hwang,Seong-Whan Lee*

Main category: cs.AI

TL;DR: 本文介绍了一种实时无线想象语音脑电图解码系统，旨在实现灵活实用的脑机接口技术，支持从有线设备扩展到便携无线硬件，通过个性化解码器实现实时用户命令分类。
...
Method: 采用实验室流层管理实时脑电信号流，结合用户识别模块提供个性化服务，构建端到端实时应用框架，支持有线设备和便携无线头戴设备。

Result: 在有线设备上实现4类想象语音脑电信号分类准确率62.00%，在便携无线头戴设备上达到46.67%的准确率。

Conclusion: 该研究为实现真正实用和可访问的脑机接口技术迈出了重要一步，为未来稳健、实用和个性化神经接口研究确立了明确方向。
...
### [148] [Thinker: Training LLMs in Hierarchical Thinking for Deep Search via Multi-Turn Interaction](https://arxiv.org/abs/2511.07943)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的检索增强方法缺乏对推理过程的监督，难以保证逻辑一致性和严谨性。

Method: 分层思维模型，将复杂问题分解为子问题，使用自然语言和逻辑函数双重表示，子问题间依赖通过逻辑函数参数传递，并进行知识边界确定。

Result: 仅需数百训练样本即可与基线方法竞争，在完整训练集上显著优于现有方法，在不同数据集和模型规模上表现一致。

Conclusion: Thinker通过可监督和可验证的推理过程，有效提升了LLM在复杂问题解决中的逻辑一致性和性能。
...
### [149] [TimeFlow: Towards Stochastic-Aware and Efficient Time Series Generation via Flow Matching Modeling](https://arxiv.org/abs/2511.07968)
*He Panjing,Cheng Mingyue,Li Li,Zhang XiaoHan*

Main category: cs.AI

TL;DR: TimeFlow是一个基于随机微分方程(SDE)的流匹配框架，用于高效生成高质量时间序列数据，通过组件分解速度场和增强随机项来捕捉时间序列的多方面结构和随机性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在效率问题：扩散模型计算成本高，传统流匹配方法无法有效捕捉时间序列的随机性。随机微分方程(SDE)天然适合建模随机性和不确定性。

Method: 提出TimeFlow框架，采用编码器架构，设计组件分解的速度场来捕捉时间序列的多方面结构，并在流匹配优化中增加随机项以增强表达能力。支持无条件生成和条件生成任务。

Result: 在多个数据集上的广泛实验表明，该模型在生成质量、多样性和效率方面始终优于强基线方法。

Conclusion: TimeFlow是一个灵活通用的框架，通过SDE-based流匹配有效解决了时间序列生成中的随机性建模问题，在质量和效率方面都表现出色。
...
### [150] [Versatile and Risk-Sensitive Cardiac Diagnosis via Graph-Based ECG Signal Representation](https://arxiv.org/abs/2511.07973)
...
Method: 使用基于图的表示统一建模异构ECG信号，将ECG信号转换为图结构以捕捉关键诊断特征；集成去噪重建和对比学习来促进表示转换，保留原始ECG信息同时突出病理特征。

Result: 在三个不同ECG数据集上的评估显示，VARS在所有数据集上一致超越现有最先进模型，并在识别风险信号方面表现出显著改进；同时提供可解释性，能够精确定位导致特定模型输出的波形。

Conclusion: VARS有望成为全面心脏健康评估的宝贵工具，具有处理信号多样性、提高诊断敏感性和提供临床决策支持的能力。
...
### [151] [Towards Fine-Grained Interpretability: Counterfactual Explanations for Misclassification with Saliency Partition](https://arxiv.org/abs/2511.07974)
...
<details>
  <summary>Details</summary>
Motivation: 现有的归因解释技术在细粒度任务中缺乏足够的粒度，特别是在模型误分类情况下，解释不够详细，无法提供深入洞察。

Method: 采用非生成式方法生成可解释的反事实，通过量化相似性和加权感兴趣区域内正确分类与误分类样本之间的组件贡献；引入基于Shapley值贡献的显著性分割模块，分离具有区域特定相关性的特征。

Result: 大量实验表明，该方法在捕捉更细粒度、直观有意义的区域方面优于现有细粒度方法。

Conclusion: 提出的细粒度反事实解释框架能够有效解决模型误分类问题，提供更详细和可解释的特征分析。
...
### [152] [Benchmarking Multi-Step Legal Reasoning and Analyzing Chain-of-Thought Effects in Large Language Models](https://arxiv.org/abs/2511.07979)
*Wenhan Yu,Xinbo Lin,Lanxin Ni,Jinhua Cheng,Lei Sha*

Main category: cs.AI

TL;DR: MSLR是中国首个基于真实司法决策的多步骤法律推理数据集，采用IRAC框架模拟结构化专家推理，并通过人机协作标注流程生成细粒度推理标注。


<details>
  <summary>Details</summary>
Motivation: 现有法律基准往往混淆事实记忆与真正推理，忽视推理过程质量，需要构建更有效的法律推理评估数据集。

Method: 基于IRAC框架构建多步骤法律推理数据集，设计可扩展的人机协作标注流程，评估多种LLM在法律推理任务上的表现。

Result: 多种LLM在MSLR上表现中等，显示复杂法律推理的挑战性；自主生成的思维链提示在推理连贯性和质量上优于人工设计的提示。

Conclusion: MSLR为推进LLM推理和思维链策略提供了开放资源，为未来研究提供可重用的方法论框架。
...
### [153] [Capturing Complex Spatial-Temporal Dependencies in Traffic Forecasting: A Self-Attention Approach](https://arxiv.org/abs/2511.07980)
...
Method: 使用区域嵌入层学习区域的时间特定嵌入，然后基于自注意力机制的时空依赖学习模块来捕捉附近和远处区域的联合时空依赖关系。
...
### [154] [The One Where They Brain-Tune for Social Cognition: Multi-Modal Brain-Tuning on Friends](https://arxiv.org/abs/2511.07988)
...
<details>
  <summary>Details</summary>
Motivation: 扩展脑调谐方法到多模态领域，通过针对社交处理关键区域STS进行模型优化，以增强社交认知能力。

Method: 使用多模态音频-视频模型，针对观看《老友记》时被试的STS区域进行脑调谐（fine-tuning），使模型更好地预测相应的fMRI活动。

Result: 发现STS及邻近ROI的大脑对齐度显著提高，同时在训练数据相关的社交认知任务（情景喜剧讽刺检测）上表现改善。
...
### [155] [VSPO: Validating Semantic Pitfalls in Ontology via LLM-Based CQ Generation](https://arxiv.org/abs/2511.07991)
*Hyojun Choi,Seokju Hwang,Kyong-Ho Lee*

Main category: cs.AI

TL;DR: 本文提出了一种新的VSPO数据集和模型，用于验证本体设计中的语义陷阱，通过微调LLaMA-3.1-8B-Instruct模型生成能够检测语义差异的能力问题，相比GPT-4.1在精度和召回率上分别提高了26%和28.2%。


<details>
  <summary>Details</summary>
Motivation: 传统基于规则的方法无法可靠检测语义陷阱（如"Misusing allValuesFrom"），现有方法主要评估生成CQs与现有数据集的相似性，但无法验证语义陷阱。需要一种能够自动验证本体设计中语义陷阱的方法。

Method: 使用LLMs生成类和属性的自然语言定义，通过移除公理或改变逻辑运算符（如用交集替换并集）引入定义与本体之间的错位，然后微调LLaMA-3.1-8B-Instruct模型生成验证这些语义差异的CQs。

Result: 微调后的模型在生成用于陷阱验证的CQs方面表现出色，比GPT-4.1精度高26%，召回率高28.2%，能够检测比现有公共数据集更广泛的建模错误。

Conclusion: 这项研究实现了使用LLMs自动生成TBox验证CQs，显著减少了人工工作量，同时改善了本体与专家知识之间的语义对齐。这是首个使用LLMs针对CQ生成中语义陷阱验证的研究。
...
TL;DR: 该研究提出了一种将多元时间序列转换为单变量时间序列的异常检测方法，结合了FCM聚类、模糊积分和隐马尔可夫模型，并通过实验比较了不同转换方法的效果。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发一种有效的多元时间序列异常检测方法，通过将多元数据转换为单变量形式来简化异常检测过程。

Method: 方法包括使用Fuzzy C-Means聚类和模糊积分将多元时间序列转换为单变量序列，然后应用隐马尔可夫模型进行异常检测。

Result: 研究构建了基于HMM的异常检测器，并通过实验比较了多种转换方法，提供了详细的实验研究和对比分析结果。

Conclusion: 该研究证明通过适当的转换技术将多元时间序列简化为单变量形式，结合HMM模型能够有效进行异常检测，为多元时间序列分析提供了新的思路。
...
### [157] [Numerical Sensitivity and Robustness: Exploring the Flaws of Mathematical Reasoning in Large Language Models](https://arxiv.org/abs/2511.08022)
...
Method: 提出扰动框架，包括注入语义无关的扰动句子和核心提问指令缺失两种方法，逐步增加扰动强度来测试LLMs。

Result: LLMs在面对无数字扰动时表现稳定但有稳健性边界；面对数字扰动时性能显著下降，开源模型下降近10%以上，最大下降达51.55%；商业LLMs也有3%-10%下降。核心指令缺失时仍能保持20%-40%准确率。
...
### [158] [Knowledge-Augmented Long-CoT Generation for Complex Biomolecular Reasoning](https://arxiv.org/abs/2511.08024)
*Tianwen Lyu,Xiang Zhuang,Keyan Ding,Xinzhe Cao,Lei Liang,Wei Zhao,Qiang Zhang,Huajun Chen*

Main category: cs.AI

TL;DR: 本文提出了一个知识增强的长链推理框架，通过结合LLMs与知识图谱的多跳推理链来解决生物分子推理中的逻辑不一致和领域知识缺失问题，并在新构建的PrimeKGQA基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在生物分子推理任务中存在的逻辑不一致、缺乏领域知识基础以及难以捕捉长程机制依赖的问题。

Method: 提出知识增强的长链推理框架，通过知识图谱的引导多跳遍历和剪枝构建机制链，结合监督微调和强化学习来提升事实基础和推理可靠性。

Result: 在PrimeKGQA和现有数据集上的实验表明，随着推理深度增加，该方法展现出明显优势，在需要遍历结构化生物知识的多跳任务中达到最先进性能。
...
### [159] [Towards a Standard, Enterprise-Relevant Agentic AI Benchmark: Lessons from 5.5 billion tokens' worth of agentic AI evaluations](https://arxiv.org/abs/2511.08042)
*JV Roig*

Main category: cs.AI

TL;DR: 提出了Kamiwaza Agentic Merit Index (KAMI) v0.1企业级基准测试，解决了传统LLM基准测试的数据污染问题和无法评估智能体能力的问题。通过大规模测试发现传统基准排名无法预测实际智能体性能，新一代模型在企业任务中不一定优于旧版本。
...
Method: 开发了KAMI v0.1企业级基准测试，通过处理170,000个LLM测试项、超过55亿个token、涵盖35种模型配置，进行大规模评估。

Result: 传统基准排名无法预测实际智能体性能，新一代模型如Llama 4或Qwen 3在企业相关任务中不一定优于旧版本，与传统基准趋势相矛盾。

Conclusion: 研究提供了关于成本-性能权衡、模型特定行为模式以及推理能力对token效率影响的重要发现，对企业部署决策具有关键意义。
...
### [160] [Dual-Process Scaffold Reasoning for Enhancing LLM Code Debugging](https://arxiv.org/abs/2511.08052)
*Po-Chung Hsieh,Chin-Po Chen,Jeng-Lin Li,Ming-Ching Chang*

Main category: cs.AI

TL;DR: 本文提出了一种基于心理学理论的Scaffold Reasoning框架用于代码调试，通过三个流（Scaffold流、分析流、集成流）来平衡推理复杂度和计算效率，在DebugBench上达到88.91%的通过率和5.36秒的平均推理时间。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs虽然具备复杂问题解决能力，但如何在推理步骤中平衡复杂性和计算效率仍是未解问题。需要深入探索System 2推理过程，借鉴心理学理论优化认知路径。

Method: 提出心理学支持的Scaffold Reasoning框架，包含三个流：Scaffold流构建参考代码，分析流分析错误代码，集成流将两者整合。

Result: 在DebugBench上达到88.91%的通过率，平均推理时间5.36秒/问题，在推理准确性和效率上均优于其他方法。

Conclusion: 该框架在不同问题难度和错误类型下展现出优势，并且与人类认知过程具有一致性，为优化LLM推理提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 由于大语言模型快速发展和广泛应用导致计算资源需求激增，而测试时缩放进一步加剧了模型能力与资源消耗之间的紧张关系，但目前缺乏能够准确反映不同规模和架构LLM效率的统一度量标准。

Method: 引入信息容量这一基于文本压缩性能相对于计算复杂度的模型效率度量方法，通过评估主流开源模型在不同数据集上的表现来验证该指标的有效性。

Result: 实证评估表明，同一系列中不同规模的模型表现出一致的信息容量，该指标能够公平比较模型系列间的效率，并准确预测同系列模型的性能。

Conclusion: 信息容量是一个有效的LLM效率度量标准，它考虑了分词器效率、预训练数据和专家混合架构等因素的影响，为模型效率评估提供了统一框架。
...
TL;DR: 提出一种基于聚类的多变量时间序列异常检测方法，使用滑动窗口生成多元子序列，通过扩展模糊聚类揭示结构，利用重构准则和置信指数量化异常程度，并用粒子群优化进行优化。
...
Method: 使用滑动窗口生成多元子序列，应用扩展模糊聚类揭示结构，采用重构准则重构子序列，构建置信指数量化异常程度，并使用粒子群优化进行优化。
...
TL;DR: 本文开发了一个评估再保险中大型语言模型可靠性的审慎框架，通过五支柱架构将监管期望转化为可衡量的生命周期控制，并在六个任务族中验证了检索基础配置的有效性。
...
Method: 提出了一个五支柱架构（治理、数据溯源、保证、韧性和监管对齐），通过再保险AI可靠性和保证基准（RAIRAB）来评估治理嵌入式LLM是否符合基础性、透明度和问责制的审慎标准。

Result: 在六个任务族中，检索基础配置实现了更高的基础准确性（0.90），幻觉和解释漂移减少了约40%，透明度几乎翻倍，降低了风险转移和资本配置中的信息摩擦。

Conclusion: 当治理明确、数据可追溯且保证可验证时，现有的审慎原则已经能够适应可靠的AI应用，为再保险行业提供了实用的监管框架。
...
TL;DR: 本文通过参数化复杂性理论重新审视Pearl因果层次结构的可满足性问题，识别了首个可处理性入口，提出了基于原始树宽和变量数量的固定参数和XP算法，并提供了匹配的硬度结果来界定可处理性的边界。


<details>
  <summary>Details</summary>
Motivation: Pearl因果层次结构的可满足性问题在几乎所有经典设置中都是计算不可行的，本文旨在通过参数化复杂性理论寻找可处理性解决方案。

Method: 采用参数化复杂性方法，使用原始树宽和变量数量作为参数，开发固定参数和XP算法，并利用结构良好的因果模型的结构特征，而非传统的动态规划方法。

Result: 为关键概率和反事实片段提供了首个可满足性的固定参数和XP算法，并建立了匹配的硬度结果来界定可处理性边界。

Conclusion: 本文为因果推理提供了一个新的算法工具包，通过参数化方法成功识别了Pearl因果层次结构可满足性问题的首个可处理性入口。
...
TL;DR: 本研究探讨了使用可解释人工智能技术进行特征降维对注塑件质量分类的影响，通过SHAP、Grad-CAM和LIME分析LSTM模型的特征重要性，将19个输入特征减少到9个和6个，评估了模型准确性、推理速度和可解释性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 机器学习在工业质量控制中应用广泛，但模型复杂性和缺乏可解释性限制了实际应用，且许多工业设备传感器技术不完善导致数据采集困难。可解释人工智能能提供模型决策洞察并识别分类中最相关的特征。

Method: 应用SHAP、Grad-CAM和LIME分析长短期记忆模型中的特征重要性，将原始19个输入特征减少到9个和6个，评估模型准确性、推理速度和可解释性之间的权衡。

Result: 特征降维可以提高模型的泛化能力，同时保持高分类性能，推理速度略有提升。
...
TL;DR: 本文研究在注塑成型过程中使用合成数据增强训练数据的可行性，通过模拟生产周期生成合成数据，并找到合成数据与真实数据的最佳比例来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 机器学习在优化工业过程中具有巨大潜力，但数据采集既耗时又昂贵。合成数据可以增强不足的数据集并提高机器学习模型的鲁棒性。

Method: 使用现有的长短期记忆架构，通过模拟生产周期生成合成数据，并将其纳入训练数据集。通过迭代实验不同比例的合成数据，寻找最优平衡。

Result: 结果表明，包含合成数据提高了模型处理不同场景的能力，有望减少人工劳动、机器使用和材料浪费。
...
TL;DR: SpeechCARE是一个多模态语音处理框架，利用预训练的多语言声学和语言转换器模型检测认知障碍，通过动态融合架构整合声学、语言学和人口统计学特征，在认知障碍分类任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病及相关痴呆症影响五分之一60岁以上成年人，但超过一半认知衰退患者未被诊断。基于语音的评估有望实现早期检测，但传统语音处理方法性能有限且泛化性差。

Method: 采用预训练的多语言声学和语言转换器模型，基于混合专家范式构建动态融合架构，整合声学、语言学和人口统计学输入，包含自动转录、LLM异常检测和任务识别等预处理步骤。

Result: 在认知健康、轻度认知障碍和阿尔茨海默病分类任务中达到AUC=0.88和F1=0.72，在轻度认知障碍检测中达到AUC=0.90和F1=0.62，偏差分析显示除80岁以上人群外差异最小。

Conclusion: SpeechCARE在认知障碍检测方面表现出色，未来工作包括在真实医疗环境中部署和针对纽约市代表性不足人群的EHR集成可解释性。
...
TL;DR: SciAgent是一个统一的多智能体系统，用于通用科学推理，通过分层组织问题解决过程，在不同学科和难度级别上实现专家级性能。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在特定科学任务上已达到专家水平，但仍局限于狭窄领域且需要手工定制。需要开发能够跨学科适应推理策略的通用科学推理系统。

Method: 采用分层多智能体架构：协调器智能体分析问题领域和复杂度，动态编排由符号推理、概念建模、数值计算和验证子智能体组成的专业工作系统，协作构建和优化推理管道。

Result: 在数学和物理奥林匹克竞赛（IMO、IMC、IPhO、CPhO）中持续达到或超越人类金牌得主表现，并在国际化学奥林匹克和Humanity's Last Exam基准测试中验证了跨学科泛化能力。

Conclusion: SciAgent代表了迈向通用科学智能的具体进展，能够实现跨学科连贯推理的AI系统。
...
TL;DR: 开发了日本首个开源商业图像生成AI模型'oboro:'，旨在解决日本动漫产业劳动力短缺问题，使用纯版权清理图像训练，模型架构设计可在有限数据集上生成高质量图像。


<details>
  <summary>Details</summary>
Motivation: 解决日本动漫制作行业劳动力短缺等挑战，推动日本本土AI开发生态系统发展，为日本AI研究者和工程师社区做出贡献。

Method: 从零开始开发图像生成模型'oboro:'，仅使用版权清理图像进行训练，采用特殊架构设计以在有限数据集上生成高质量图像。

Result: 成功开发并公开发布了'oboro:'基础模型权重和推理代码，这是日本首个完全本土开发的开源商业图像生成AI。

Conclusion: 该项目标志着日本首个开源商业图像生成AI的发布，通过保持开发过程透明性，旨在促进日本AI研究社区发展和国内AI开发生态系统建设。
...
TL;DR: 提出了一种高效的视觉定位训练流程，结合基于模型的数据筛选和参数高效微调，从480万合成样本中筛选出1.2万个高质量样本，在三个训练策略下训练30亿参数视觉语言模型，性能优于更大规模的基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位方法依赖大规模噪声合成数据集，需要更高效的数据筛选和训练方法来构建紧凑但能力强的多模态推理智能体。

Method: 采用基于模型的数据过滤方法，从480万合成样本中筛选出1.2万个干净多样的实例，然后在三个训练策略下微调30亿参数视觉语言模型：监督微调、思维链增强微调和基于组相对策略优化的强化学习。

Result: 在ScreenSpot、Multimodal-Mind2Web和AndroidControl等基准测试中，使用筛选数据和轻量级训练策略训练的模型匹配或超越了更大的基线模型。

Conclusion: 原则性的数据筛选和稳健的适应方法可以与大规模训练相媲美，能够构建紧凑但能力强的多模态推理智能体。
...
TL;DR: 本文提出了一种基于最大化贝叶斯误差的新方法来构建不可学习的样本，该方法通过优化方法系统地增加分类的不可约误差，并在混合干净数据时仍保持有效性。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型依赖大规模数据训练，用户数据保护问题日益突出。现有不可学习样本方法缺乏理论保证且在混合干净数据时失效，需要一种更可靠的方法来保护用户数据不被模型学习。

Method: 采用基于优化的方法，通过投影梯度上升来最大化贝叶斯误差，系统性地构造不可学习样本。

Result: 实验结果表明，该方法在多个数据集和模型架构上都能有效限制数据可学习性，且在混合干净数据时仍保持有效性。

Conclusion: 提出的基于贝叶斯误差最大化的方法为构建不可学习样本提供了理论保证和实际有效性，为解决用户数据保护问题提供了新思路。
...
TL;DR: EHRStruct是一个专门用于评估大语言模型在结构化电子健康记录数据上性能的基准测试，包含11个代表性任务和2200个评估样本，评估了20个先进LLM并分析了关键影响因素。
...
Method: 提出了EHRStruct基准测试，定义了11个临床任务，使用两个广泛使用的EHR数据集创建了2200个任务特定评估样本。

Result: 评估结果显示许多结构化EHR任务对LLM的理解和推理能力要求很高。提出了EHRMaster方法，实现了最先进的性能。

Conclusion: EHRStruct为结构化EHR数据的LLM评估提供了标准化框架，EHRMaster方法在解决这些挑战性任务方面表现出色。
...
Conclusion: MADD为药物设计的智能化未来提供了新工具，并发布了包含300多万个化合物的查询-分子对和对接评分新基准。
...
TL;DR: GAC是一种新颖的动作生成范式，通过方向向量和可学习集中参数分解动作生成，在保持球形分布几何优势的同时简化计算，在连续控制任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决高斯策略在有界动作空间中的几何失真问题，以及vMF分布计算复杂、难以实际应用的问题。

Method: 将动作生成分解为方向向量和可学习集中参数，实现确定性动作和均匀球形噪声之间的高效插值，参数数量从2d减少到d+1，复杂度从O(dk)降低到O(d)。

Result: 在6个MuJoCo基准测试中一致匹配或超越最先进方法，在Ant-v4上比SAC提升37.6%，在4/6任务中取得最佳结果。消融研究表明球形归一化和自适应集中控制对成功至关重要。
...
TL;DR: 本文提出了一个包含11个基于结果的、任务无关的AI智能体性能评估框架，通过大规模模拟实验验证了该框架的有效性，并发现混合智能体在多数指标上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 当前仅基于基础设施指标（如延迟、时间到第一个token等）评估AI智能体性能的方法不足以捕捉智能体决策质量、操作自主性或最终业务价值，需要更全面的评估框架。

Method: 提出了11个基于结果的、任务无关的性能指标，并通过大规模模拟实验，在四个不同智能体架构（ReAct、思维链、工具增强、混合）和五个不同领域（医疗、金融、营销、法律、客户服务）中验证框架效果。

Result: 实验结果显示不同智能体设计之间存在显著性能权衡，混合智能体在大多数提出的指标上表现最一致，平均目标完成率达到88.8%，并实现最高的投资回报率。
...
TL;DR: 提出了STV框架，通过敏感性感知的任务向量插入方法解决多模态模型在少样本学习中的上下文长度限制和推理成本问题，确定插入位置和内容。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在上下文学习中表现出潜力，但由于上下文长度限制和高推理成本，扩展到多样本设置仍然困难。现有任务向量方法要么忽略插入位置的重要性，要么难以确定每个位置的合适值。

Method: 提出敏感性感知任务向量插入框架(STV)，基于查询-上下文对之间的激活差异模式识别敏感位置，构建预聚类激活库，并使用强化学习选择最合适的插入值。

Result: 在多种多模态模型(Qwen-VL、Idefics-2)和任务(VizWiz、OK-VQA)上评估STV，证明其有效性，相比之前的方法显示出持续改进和强泛化能力。

Conclusion: STV框架通过系统性地确定任务向量插入的位置和内容，显著提升了多模态模型在多样本上下文学习中的性能，具有很好的泛化性。
...
TL;DR: 本文提出了Multi-Agent GraphRAG系统，利用多智能体LLM生成Cypher查询，作为LPG图数据的自然语言接口，填补了当前GraphRAG研究中Cypher和LPG数据库应用的空白。


<details>
  <summary>Details</summary>
Motivation: 当前GraphRAG方法主要关注RDF知识图谱，而Cypher和LPG数据库作为可扩展且有效的推理引擎在GraphRAG管道中的潜力尚未得到充分探索。

Method: 提出模块化LLM智能体系统，采用基于LLM的工作流程自动生成和执行Cypher查询，使用Memgraph作为图数据库后端，通过迭代内容感知校正和归一化确保查询的语义和句法优化。

Result: 在CypherBench图数据集上评估系统性能，涵盖多个通用领域的不同查询类型，并在基于IFC数据的属性图上展示工作流程性能。

Conclusion: 该方法能够将AI与大规模现实世界应用连接，实现工业数字自动化用例，展示了GraphRAG在LPG数据库中的实际应用潜力。
...
TL;DR: 提出了DiagramIR，一种用于几何图形的自动可扩展评估管道，基于LaTeX TikZ代码的中间表示，能够以更低成本实现与人类评估者更高的一致性。
...
Result: DiagramIR方法比基线方法更接近人类评估者的判断，且使GPT-4.1-Mini等较小模型以10倍更低的推理成本达到与GPT-5相当的性能。

Conclusion: DiagramIR为部署可访问和可扩展的教育技术提供了重要的评估方法，解决了图形生成评估的瓶颈问题。
...
TL;DR: Spark是一个新颖的共享智能体记忆架构，旨在模拟人类开发者社区的集体智能，使AI编程智能体能够贡献和获取持续演化的经验记忆，实现集体持续学习。


<details>
  <summary>Details</summary>
Motivation: 传统的人类中心软件开发知识共享环境正在被智能体中心实践颠覆，传统开发者社区参与度急剧下降，而AI智能体缺乏有价值的共享学习资源库。

Method: 引入Spark共享智能体记忆架构，作为AI编程智能体的教练，提供持续演化的经验记忆库，智能体可以在相同问题空间中使用该共享内存作为新知识库。

Result: Spark推荐显著提高了通用代码生成模型的代码质量，一个300亿参数的小型模型在Spark加持下能够匹配更大规模最先进模型的代码质量；Spark推荐在软件开发最佳实践标准下的帮助性达到98.2%。

Conclusion: Spark架构有效解决了AI编程智能体缺乏共享学习资源的问题，通过集体持续学习机制显著提升了代码生成质量，为智能体中心软件开发提供了可行的知识共享解决方案。
...
### [180] [AI-Powered Data Visualization Platform: An Intelligent Web Application for Automated Dataset Analysis](https://arxiv.org/abs/2511.08363)
...
Method: 结合Python Flask后端和React前端，集成Firebase云存储，采用四种算法进行智能特征选择，自动处理缺失值和异常值检测。

Result: 平台能实时处理10万行数据集，云架构支持多用户并发请求，保持高质量可视化输出和用户体验。

Conclusion: 该云数据可视化应用显著减少了数据分析过程中的人工输入，同时保持了高质量的可视化输出和用户体验。
...
### [181] [SOM Directions are Better than One: Multi-Directional Refusal Suppression in Language Models](https://arxiv.org/abs/2511.08379)
*Giorgio Piras,Raffaele Mura,Fabio Brau,Luca Oneto,Fabio Roli,Battista Biggio*

Main category: cs.AI

TL;DR: 本文提出了一种利用自组织映射(SOMs)从语言模型中提取多个拒绝方向的新方法，通过识别多个神经元并减去无害表示质心，获得表达拒绝概念的多方向集合，在抑制模型拒绝行为方面优于单方向基线方法。
...
Method: 使用自组织映射(SOMs)在有害提示表示上训练以识别多个神经元，然后从每个神经元减去无害表示质心，从而推导出表达拒绝概念的多个方向集合。

Result: 实验验证表明，从模型内部消融多个方向不仅优于单方向基线，还优于专门的越狱算法，能有效抑制模型的拒绝行为。

Conclusion: 该方法为拒绝行为的机制解释提供了新的见解，证明了多方向表示在捕获复杂概念方面的优越性。
...
### [182] [FaithAct: Faithfulness Planning and Acting in MLLMs](https://arxiv.org/abs/2511.08409)
*Junxian Li,Xinyue Xu,Sai Ma,Sichao Li*

Main category: cs.AI

TL;DR: 本文提出了FaithEval用于评估多模态推理中的忠实性，并开发了FaithAct框架来强制每个推理步骤的证据基础，显著提高了感知忠实性而不降低任务准确性。
...
Method: 区分行为忠实性（推理与输出的一致性）和感知忠实性（推理与输入的一致性），引入FaithEval量化步骤级和链级忠实性，并提出了FaithAct框架来强制每个推理步骤的证据基础。
...
### [183] [Dataset Safety in Autonomous Driving: Requirements, Risks, and Assurance](https://arxiv.org/abs/2511.08439)
...
<details>
  <summary>Details</summary>
Motivation: 数据集完整性对AI系统的安全性和可靠性至关重要，特别是在自动驾驶领域。现有数据集在安全性方面存在不足，需要建立系统化的安全框架。

Method: 引入AI数据飞轮和数据集生命周期概念，结合严格的安全分析来识别危险源和缓解数据集不足带来的风险，定义数据集安全要求建立流程，并提出验证和确认策略。

Result: 开发了一个结构化框架，能够系统化地确保数据集符合安全标准，为构建稳健的自动驾驶AI系统提供方法论支持。
...
### [184] [Patching LLM Like Software: A Lightweight Method for Improving Safety Policy in Large Language Models](https://arxiv.org/abs/2511.08484)
*Huzaifa Arif,Keerthiram Murugesan,Ching-Yun Ko,Pin-Yu Chen,Payel Das,Alex Gittens*

Main category: cs.AI

TL;DR: 提出了一种轻量级的模块化方法，通过为大型语言模型添加可学习前缀来修复安全漏洞，仅增加0.003%参数就能有效提升模型安全性。
...
Method: 在现有模型前添加紧凑的可学习前缀作为"补丁"，这种方法仅需极少量额外参数就能引导模型行为向更安全的参考模型靠拢。

Result: 在毒性缓解、偏见减少和有害拒绝三个关键领域，策略补丁实现了与下一代安全对齐模型相当的安全改进，同时保持了流畅性。

Conclusion: LLM可以像软件一样进行"打补丁"，为供应商和从业者提供了在大版本发布之间分发可扩展、高效且可组合安全更新的实用机制。
...
### [185] [A Matter of Interest: Understanding Interestingness of Math Problems in Humans and Language Models](https://arxiv.org/abs/2511.08548)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地参与数学研究和教育，了解其判断与人类判断的一致性变得重要，特别是在数学问题的趣味性和难度评估方面。

Method: 通过两个实证研究，分别对众包平台参与者和国际数学奥林匹克竞赛选手进行调查，比较人类与多种LLM对数学问题趣味性和难度的评估。

Result: LLM在广泛层面上与人类趣味性概念一致，但无法准确捕捉人类判断的分布特征；大多数LLM与人类选择趣味性理由的相关性较弱。

Conclusion: 当前LLM在捕捉人类对数学问题的趣味性判断方面既有潜力也有局限性，对于数学AI思维伙伴关系的发展具有重要意义。
...
TL;DR: 这篇论文系统性地综述了视频生成模型向世界模型发展的演进过程，将现代视频基础模型概念化为隐式世界模型和视频渲染器的组合，探讨了四代视频生成技术的核心能力演进。
...
Method: 将现代视频基础模型分解为两个核心组件：隐式世界模型（编码物理规律、交互动态和智能体行为）和视频渲染器（将潜在模拟转换为逼真视觉观察）。通过四代演进分析技术发展。

Result: 提出了视频生成模型的演进框架，从基础视觉生成逐步发展到具备物理合理性、实时多模态交互和多时空尺度规划能力的世界模型。

Conclusion: 视频生成模型正在向具备内在物理合理性、实时交互和多尺度规划能力的世界模型发展，未来需要关注智能体智能在塑造和评估这些系统中的作用。
